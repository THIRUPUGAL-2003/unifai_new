#!/bin/bash
# Load test: detect JSON key ordering mutations in UnifAI's request proxying.
# Sends randomized payloads with different schema shapes and compares the input
# key order against what UnifAI actually sent to the provider (via
...
#
# Prerequisites:
#   - UnifAI running with send_back_raw_request: true on the openai provider
...
UNIFAI_URL="http://localhost:8080/litellm/v1/chat/completions"
RPS="${1:-20}"
DURATION="${2:-10}"
...
export UNIFAI_URL TMPDIR NUM_PAYLOADS
