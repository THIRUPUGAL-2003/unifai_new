...
// Integration route tests verify that x-uf-async and x-uf-async-id headers work on
...
//
//	Submit  (x-uf-async: true)     → HTTP 200 (not 202)
//	Retrieve (x-uf-async-id: <id>) → HTTP 200 for any job state
//
// Optional env:
//
//	UNIFAI_INTEGRATION_PATH  — override the default /openai/v1/responses
//	UNIFAI_INTEGRATION_MODEL — model string; defaults to ASYNC_RESPONSES_MODEL default
//
// Note: only routes with AsyncResponsesResponseConverter support x-uf-async.
...
func integrationPath() string {
	return envOr("UNIFAI_INTEGRATION_PATH", "/openai/v1/responses")
}

func integrationModel() string {
	if v := os.Getenv("UNIFAI_INTEGRATION_MODEL"); v != "" {
		return v
	}
...
// pollIntegration POSTs to an integration path with x-uf-async-id header to retrieve a job.
...
// route with x-uf-async header and confirms the response is 200 OK with a job UUID.
...
func TestIntegration_AsyncCreate_Returns200WithJobID(t *testing.T) {
	for _, mode := range testModes() {
		t.Run(mode.name, func(t *testing.T) {
			headers := withRawHeader(mode.headers, "x-uf-async", "true")
...
// and polls it via x-uf-async-id header, confirming retrieve also returns 200 OK.
...
// via x-uf-async is rejected — streaming and async are mutually exclusive.
...
func TestIntegration_VKScope_DifferentKey_Returns4xx(t *testing.T) {
	if cfg.VK == "" || cfg.AltVK == "" {
		t.Skip("both UNIFAI_VK and UNIFAI_ALT_VK must be set")
	}
	headers := withRawHeader(vkHeaders(cfg.VK), "x-uf-async", "true")
	code, _, body := submitJSON(t, integrationPath(), integrationSubmitBody(), headers)
	if code != http.StatusOK {
		t.Fatalf("submit failed with %d: %s", code, body)
	}
