...
var cfg = struct {
	BaseURL      string
	VK           string // UNIFAI_VK — primary virtual key
	AltVK        string // UNIFAI_ALT_VK — a second, different virtual key for auth tests
	PollTimeout  time.Duration
	PollInterval time.Duration
}{
	BaseURL:      envOr("UNIFAI_BASE_URL", defaultBaseURL),
	VK:           os.Getenv("UNIFAI_VK"),
	AltVK:        os.Getenv("UNIFAI_ALT_VK"),
	PollTimeout:  parseDuration(os.Getenv("UNIFAI_POLL_TIMEOUT"), defaultPollTimeout),
	PollInterval: parseDuration(os.Getenv("UNIFAI_POLL_INTERVAL"), defaultPollInterval),
}
...
// When UNIFAI_VK is unset, only the global (no-VK) round runs.
...
func vkHeaders(vk string) map[string]string {
	...
	return map[string]string{"x-uf-vk": vk}
}

// withTTLHeader copies headers and appends x-uf-async-job-result-ttl.
func withTTLHeader(headers map[string]string, ttlSeconds int) map[string]string {
	out := make(map[string]string, len(headers)+1)
	for k, v := range headers {
		out[k] = v
	}
	out["x-uf-async-job-result-ttl"] = fmt.Sprintf("%d", ttlSeconds)
	return out
}
...
// TestMain checks that the UnifAI gateway is reachable before running any tests.
// Set UNIFAI_BASE_URL to override the default http://localhost:8080.
func TestMain(m *testing.M) {
	resp, err := httpClient.Get(cfg.BaseURL + "/health")
	if err != nil || resp.StatusCode >= 500 {
		fmt.Printf("SKIP: UnifAI gateway not reachable at %s (err=%v)\n", cfg.BaseURL, err)
		os.Exit(0)
	}
	resp.Body.Close()
	os.Exit(m.Run())
