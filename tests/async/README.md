...
End-to-end tests for UnifAI's async inference feature (`/v1/async/*` endpoints and integration route headers).
...
```bash
UNIFAI_VK=sk-uf-... UNIFAI_ALT_VK=sk-uf-... go test ./... -timeout 300s
```
...
|---|---|---|
| `UNIFAI_BASE_URL` | `http://localhost:8080` | UnifAI gateway URL |
| `UNIFAI_VK` | — | Primary virtual key; enables VK-mode tests |
| `UNIFAI_ALT_VK` | — | Second virtual key; enables cross-VK auth tests |
| `UNIFAI_POLL_TIMEOUT` | `30s` | Max time to wait for a job to reach terminal state |
| `UNIFAI_POLL_INTERVAL` | `500ms` | Polling cadence |
| `UNIFAI_INTEGRATION_PATH` | `/openai/v1/responses` | Override integration route path |
| `UNIFAI_INTEGRATION_MODEL` | `openai/gpt-4o-mini` | Override model for integration route tests |
...
| `integration_route_test.go` | `x-uf-async` / `x-uf-async-id` headers on `/openai/v1/responses` |

## Notes

- Tests skip gracefully when the gateway is unreachable (`/health` check at startup).
- Most tests run in two modes: **global** (no VK) and **with_vk** (when `UNIFAI_VK` is set).
...
- `UNIFAI_ALT_VK` is only required for cross-VK isolation tests (`TestAuth_VKScoped_DifferentKey_Returns404`, `TestIntegration_VKScope_DifferentKey_Returns4xx`).
