...
// Requires UNIFAI_VK to be set, which proves VK governance is active on the server.
func TestAuth_Submit_InvalidVK_Returns400(t *testing.T) {
	if cfg.VK == "" {
		t.Skip("UNIFAI_VK not set — governance may not be active")
	}
	ec := chatCompletionCase()
	code, _, body := submitCase(t, ec, vkHeaders("sk-uf-nonexistent-key-for-auth-test"))
...
// vkPrefixed returns true when vk begins with the governance virtual-key prefix "sk-uf-".
// Only keys with this prefix are recognised by the Authorization, x-api-key, and
// x-goog-api-key header paths in ConvertToUnifAIContext.
func vkPrefixed(vk string) bool {
	return strings.HasPrefix(strings.ToLower(vk), "sk-uf-")
}

// TestAuth_BearerVK_SameKey_Succeeds submits with "Authorization: Bearer <vk>" and
// polls with the same header.  Verifies the Bearer token path in ConvertToUnifAIContext.
func TestAuth_BearerVK_SameKey_Succeeds(t *testing.T) {
	if cfg.VK == "" || !vkPrefixed(cfg.VK) {
		t.Skip("UNIFAI_VK not set or does not start with sk-uf- prefix")
	}
	ec := chatCompletionCase()
...
// the same header.  Verifies the x-api-key path in ConvertToUnifAIContext.
...
// with the same header.  Verifies the x-goog-api-key path in ConvertToUnifAIContext.
