...
// TestTTL_CustomHeaderApplied verifies that x-uf-async-job-result-ttl overrides the
...
func TestTTL_InvalidHeader_FallsBackToDefault(t *testing.T) {
	for _, mode := range testModes() {
		t.Run(mode.name, func(t *testing.T) {
			ec := chatCompletionCase()
			headers := withRawHeader(mode.headers, "x-uf-async-job-result-ttl", "not-a-number")
			_, submitted, body := submitCase(t, ec, headers)
			if submitted.ID == "" {
				t.Fatalf("submit returned no job id: %s", body)
			}
...
func TestTTL_ExpiredJob_Returns404(t *testing.T) {
	const shortTTL = 10 // seconds — must be larger than UNIFAI_POLL_INTERVAL
	for _, mode := range testModes() {
		t.Run(mode.name, func(t *testing.T) {
			ec := chatCompletionCase()
