...
const UNIFAI_BASE_URL = process.env.UNIFAI_BASE_URL ?? 'http://localhost:8080'

const REPO_ROOT = resolve(__dirname, '../..')
const TEST_PLUGIN_PATH = join(REPO_ROOT, 'tmp', 'unifai-test-plugin.so')
...
async function waitForUnifAIAPI(baseUrl: string, maxAttempts = 30): Promise<void> {
  for (let i = 0; i < maxAttempts; i++) {
    try {
  ...
  throw new Error(`UnifAI API at ${baseUrl} did not become ready after ${maxAttempts} attempts`)
}
...
async function ensureTestClient001AndSendResponses(baseUrl: string): Promise<void> {
  ...
  if (clientAfter.state !== 'connected') {
    ...
    if (reconnectRes.statusCode < 200 || reconnectRes.statusCode >= 300) {
      throw new Error(
        `POST /api/mcp/client/.../reconnect failed: ${reconnectRes.statusCode} ${reconnectRes.body}. Ensure MCP server is running and reload UnifAI if needed.`
      )
    }
  }
...
/**
 * Seed LLM logs by sending a few chat completion requests through UnifAI.
...
async function runUnifAIMCPAndResponsesSetup(): Promise<void> {
  if (!process.env.UNIFAI_BASE_URL) {
    console.log('Skipping UnifAI MCP client and /v1/responses (UNIFAI_BASE_URL not set)')
    return
  }
  console.log(`Waiting for UnifAI API at ${UNIFAI_BASE_URL}...`)
  await waitForUnifAIAPI(UNIFAI_BASE_URL)
  console.log(`✓ UnifAI API ready`)
  await ensureTestClient001AndSendResponses(UNIFAI_BASE_URL)
  await seedLLMLogs(UNIFAI_BASE_URL, 30)
}
...
async function globalSetup(): Promise<() => Promise<void>> {
  ...
  try {
    await runUnifAIMCPAndResponsesSetup()
  } catch (error: unknown) {
    const err = error as Error
    console.error(`\n❌ UnifAI MCP client / v1/responses setup failed: ${err?.message || String(error)}`)
    console.error(`   Ensure UnifAI is running at ${UNIFAI_BASE_URL} and OPENAI_API_KEY is set for /v1/responses.`)
    runMCPTeardown()
    throw error
  }
  return async () => {
