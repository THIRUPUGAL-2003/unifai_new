# E2E CLI tests (Claude Code, Codex & OpenCode through UnifAI)

End-to-end tests that drive coding-assistant CLIs in **non-interactive mode** against a running UnifAI instance and assert that single-turn and multi-turn conversation features work across providers.

This is the CLI analog of `tests/e2e/api/` (which uses Newman/Postman for HTTP). Where the API harness exercises UnifAI's HTTP surface directly, this harness exercises UnifAI *through* the CLIs that customers actually use — but via their scripted interfaces (`claude -p`, `codex exec`), not their TUIs.
...
| `unifai_test.go` | Health check + configured-provider discovery via `/api/providers`. |
...
1. Go 1.23+.
2. UnifAI running locally (default `http://localhost:8080`) with at least one provider configured. The runner queries `/api/providers` and skips any provider that isn't configured.
...
make run-cli-harness-test BASE_URL=http://localhost:9090                        # non-default UnifAI
...
| Var | Default | Notes |
|-----|---------|-------|
| `UNIFAI_BASE_URL` | `http://localhost:8080` | UnifAI base URL. |
| `UNIFAI_API_KEY` | `dummy` | Sent as the CLI's API key env. |
...
| `UNIFAI_E2E_CLIS=skip` | unset | Skips the entire test (useful in CI without setup). |
| `UNIFAI_E2E_CLIS_QUIET=1` | unset | Suppresses live mirror; reports still written. |
...
`QUIET=1` (or `UNIFAI_E2E_CLIS_QUIET=1`) suppresses the mirror.
...
- **OpenCode (Pattern B)**: One `opencode run` for turn 1, then `opencode run --continue` for each subsequent turn, with XDG config/data/cache directories redirected to a per-cell temp dir and `OPENCODE_CONFIG` pointed at a generated UnifAI provider config.
