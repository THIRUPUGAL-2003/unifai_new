...
// unifaiClient queries the UnifAI management API to (a) confirm the server
...
func newUnifAIClient(baseURL string) *unifaiClient {
	return &unifaiClient{
...
func (c *unifaiClient) Health(ctx context.Context) error {
	...
	if resp.StatusCode >= 500 {
		return fmt.Errorf("unifai /api/providers -> %d", resp.StatusCode)
	}
	return nil
}
...
// ConfiguredProviders returns the set of provider IDs UnifAI reports as
...
func (c *unifaiClient) ConfiguredProviders(ctx context.Context) (map[string]bool, error) {
	req, _ := http.NewRequestWithContext(ctx, http.MethodGet, c.baseURL+"/api/providers", nil)
	resp, err := c.http.Do(req)
	if err != nil {
		return nil, err
	...
	if resp.StatusCode != 200 {
		return nil, fmt.Errorf("unifai /api/providers -> %d: %s", resp.StatusCode, string(body))
	}
