...
func TestAssertionOutputCodexExtractsAgentMessageOnly(t *testing.T) {
	raw := strings.Join([]string{
		`{"type":"item.completed","item":{"type":"user_message","content":[{"type":"input_text","text":"Reply with OKUNIFAI"}]}}`,
	...
	if strings.Contains(got, "OKUNIFAI") {
...
func TestAssertionOutputOpenCodeExtractsTextPartEvents(t *testing.T) {
	raw := strings.Join([]string{
		...
		`{"type":"text","part":{"type":"text","text":"OKUNIFAI"}}`,
