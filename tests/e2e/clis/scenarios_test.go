...
func simpleChatScenario() scenario {
	return scenario{
		...
		ErrorIgnore: []string{"OKUNIFAI"},
		Turns: []Turn{{
			Send:       "This is a harmless connectivity test. Reply with exactly the single token OKUNIFAI and nothing else.",
			AssertText: []string{"OKUNIFAI"},
...
func toolCallScenario() scenario {
	const token = "UNIFAI_TOOL_EXEC_73129"
	return scenario{
		ID:          "tool-call",
		ModelKind:   "chat",
		Supports:    supportsAnyChat,
