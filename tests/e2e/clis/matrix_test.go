...
type CLI struct {
	...
	Binary     string
	InstallPkg string
	BasePath   string // UnifAI base-path the CLI's wire format targets
	BaseURLEnv string
	APIKeyEnv  string
...
// Provider describes a UnifAI-configured provider and its model catalog.
//
...
// unifaiModelRef is the provider-prefixed model id UnifAI expects when
// routing across the wire-format boundary.
func unifaiModelRef(provider, model string) string {
...
func opencodePreLaunch(baseURL, apiKey, model string) ([]string, func(), error) {
  ...
  "provider": {
	...
	f, err := os.CreateTemp("", "unifai-e2e-opencode-*.json")
	if err != nil {
