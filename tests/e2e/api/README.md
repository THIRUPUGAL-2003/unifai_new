...
End-to-end API tests for the UnifAI API using Postman collections and [Newman](https://www.npmjs.com/package/newman) (CLI).
...
| Path | Description |
|------|-------------|
| `unifai-v1-complete.postman_collection.json` | Postman collection: all `/v1` endpoints (models, chat, completions, responses, embeddings, audio, images, count tokens, batches, files, containers, MCP) |
| `unifai-v1.postman_environment.json` | Optional/legacy Postman environment (OpenAI). `run-newman-inference-tests.sh` uses **UNIFAI_*** environment variables as the fallback when no provider-specific env file is passed (see script and `--help`). |
...
| Path | Description |
|------|-------------|
| `unifai-openai-integration.postman_collection.json` | OpenAI integration endpoints: `/openai/v1/*`, `/openai/*`, `/openai/deployments/*` (38 requests) |
| `unifai-anthropic-integration.postman_collection.json` | Anthropic integration endpoints: `/anthropic/v1/*` (13 requests) |
| `unifai-bedrock-integration.postman_collection.json` | Bedrock integration endpoints: `/bedrock/model/*`, `/bedrock/files/*`, `/bedrock/model-invocation-*` (13 requests, including List Objects S3 ListObjectsV2). Auth via request headers set from collection/environment variables. |
| `unifai-composite-integrations.postman_collection.json` | Composite integrations: GenAI, Cohere, LiteLLM, LangChain, PydanticAI, Health (21 requests) |
...
| Path | Description |
|------|-------------|
| `collections/unifai-model-catalog-wiring.postman_collection.json` | Generated collection asserting that management-API mutations (add/update/delete provider and key, toggle key, alias) propagate into the model catalog read endpoints. **Generated — do not hand-edit.** |
...
| Path | Description |
|------|-------------|
| `provider_config/` | Per-provider Postman env `.json` files (`unifai-v1-openai.postman_environment.json`, etc.). Reused across all collections. |
...
- UnifAI server running (e.g. `http://localhost:8080`) with at least one provider configured (API keys, etc.)
...
```bash
# Run for all providers in parallel (each provider_config/unifai-v1-*.postman_environment.json except sgl and ollama)
...
./runners/run-newman-inference-tests.sh --env provider_config/unifai-v1-openai.postman_environment.json
...
```bash
UNIFAI_API_EXTRA_COLLECTION=/path/to/extra.postman_collection.json \
...
- UnifAI running at `{{base_url}}` (default `http://localhost:8080`), ideally
...
- `openai_api_key` available — either in the seed env file (`generated/seed.env`
  or `$UNIFAI_E2E_SEED_ENV`) or exported in the shell. Scenarios whose required
...
| Path | Description |
|------|-------------|
| `collections/unifai-v1-auth-matrix.postman_collection.json` | Asserts the separation between inference auth (governance / virtual key) and dashboard auth (admin password on `/api/*`). |
...
server. It requires a built `unifai-http` binary.
...
```bash
./runners/individual/run-newman-auth-matrix-tests.sh --binary /path/to/unifai-http
...
| Path | Description |
|------|-------------|
| `collections/unifai-v1-mcp-auth.postman_collection.json` | Asserts inbound `/mcp` authentication across the three server auth modes (`headers` / `both` / `oauth`): discovery gating, the credential connect matrix, the full issuance flow, refresh rotation + family revocation, the revocation window, and the runtime `headers`→`both` upgrade. |
...
- **`headers` (default):** discovery endpoints 404; every virtual-key credential
  (`x-uf-vk`, `Authorization: Bearer <vk>`, `x-api-key`) connects exactly as before;
...
```bash
./runners/individual/run-newman-mcp-auth-tests.sh --binary /path/to/unifai-http
...
- **Batch API config (500)** – **"no batch-enabled keys found"** / **"no config found for batch APIs"** when batch endpoints are not configured for that provider.
  ...
  **To fix:** In UnifAI's config (or provider config), enable "Use for Batch APIs" on at least one API key for the provider, e.g. in config JSON:
  ```json
  {
    "providers": [
      {
...
- **Bedrock** – Model (converse/invoke) or S3 file operations may fail with 403/500 if AWS keys or S3 are not configured. The Bedrock **integration** collection (`unifai-bedrock-integration.postman_collection.json`) tests `/bedrock/*` and supports auth via **request headers** (set from collection or environment variables by the collection’s pre-request script). Credentials can be provided via env vars (e.g. `UNIFAI_BEDROCK_API_KEY`, `UNIFAI_BEDROCK_ACCESS_KEY`, `UNIFAI_BEDROCK_SECRET_KEY`, `UNIFAI_BEDROCK_REGION`) when using the runner with `--env bedrock`; the runner passes these into Postman variables, which the pre-request script forwards as `x-uf-bedrock-*` headers. Set authentication in `provider_config/unifai-v1-bedrock.postman_environment.json` or collection variables:
...
3. **Re-inject variables into a collection**: From this directory run:
   ```bash
   node scripts/update-collection-capabilities.js unifai-openai-integration.postman_collection.json --inject
   ```
...
Request bodies match core (e.g. batch inline with `custom_id`/`body`/`Say hello`, container create with `name: "unifai-test-container"`).

## Syncing from OpenAPI docs

The collection and supporting files are maintained under `docs/openapi/`. To refresh this e2e copy:

- `unifai-v1-complete.postman_collection.json` ← `docs/openapi/unifai-v1-complete.postman_collection.json`
- `unifai-v1.postman_environment.json` ← `docs/openapi/unifai-v1.postman_environment.json`
