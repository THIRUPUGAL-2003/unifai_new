 ...
 *   2. UNIFAI_DB_URL env var       (explicit DSN)
 *   3. --reporter-dbverify-config   (path to UnifAI config.json; auto-detects type + DSN)
...
// ─── UnifAI config.json reader ───────────────────────────────────────────────

/**
 * Resolve an EnvVar field from UnifAI config.
...
function postgresUrlFromConfig(c) {
  ...
  const user     = resolveEnvVar(c.user)     || 'unifai';
  const password = resolveEnvVar(c.password) || '';
  const dbName   = resolveEnvVar(c.db_name)  || 'unifai';
...
/**
 * Read a UnifAI config.json and return a DB connection URL for the main
 * config_store, or null if not enabled / unreadable.
 */
function dbUrlFromUnifAIConfig(configPath) {
  let raw;
...
/**
 * Read a UnifAI config.json and return a DB connection URL for the logs_store,
 * or null if not enabled / unreadable.
 */
function logsDbUrlFromUnifAIConfig(configPath) {
  let raw;
  try { raw = fs.readFileSync(configPath, 'utf8'); }
  catch (_) { return null; }
...
// Authoritative pricing source — the same datasheet UnifAI itself syncs from
// (framework/modelcatalog/datasheet/store.go DefaultURL). Override via env.
const PRICING_URL = process.env.UNIFAI_PRICING_URL || 'https://getunifai.ai/datasheet';
 ...
 * table is keyed by request id (Log.ID == UnifAIContextKeyRequestID), which
...
async function verifyCostingRequest(db, reqId, name, results, silent) {
  ...
  if (!silent) {
    ...
    console.log(`[dbverify]     expected $ : ${expectedStr}    (recomputed from getunifai.ai/datasheet)`);
...
module.exports = function (newman, options) {
  ...
  const configPath = (options && options['config'])
    || process.env.UNIFAI_CONFIG_PATH
  ...
  let dbUrl = (options && (options['db-url'] || options['dbUrl'])) || process.env.UNIFAI_DB_URL || null;
  if (!dbUrl) {
    dbUrl = dbUrlFromUnifAIConfig(configPath);
    if (dbUrl && !silent) console.log(`[dbverify] Auto-detected main DB from config: ${configPath}`);
  }
  if (!dbUrl) {
    console.warn('[dbverify] No main DB URL found. Provide --reporter-dbverify-db-url, UNIFAI_DB_URL, or --reporter-dbverify-config. Skipping DB checks.');
  }

  // Logs DB (logs_store)
  let logsDbUrl = (options && (options['logs-db-url'] || options['logsDbUrl'])) || process.env.UNIFAI_LOGS_DB_URL || null;
  if (!logsDbUrl) {
    logsDbUrl = logsDbUrlFromUnifAIConfig(configPath);
  ...
  newman.on('request', function (err, args) {
    ...
    const expectCost = getHeader(request, 'x-uf-expect-cost');
    if (expectCost && /^(1|true|yes)$/i.test(String(expectCost))) {
      const reqId = getHeader(request, 'x-request-id');
      if (!reqId) {
        results.push({ name, result: 'FAIL', detail: 'x-uf-expect-cost set but no x-request-id header to locate the log row' });
        return;
      }
      if (!logsDbReady || !logsDb) {
        if (!logsDbUrl) {
