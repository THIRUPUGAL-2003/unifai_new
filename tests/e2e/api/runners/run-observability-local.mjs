...
const baseURL = (process.env.UNIFAI_E2E_BASE_URL || process.env.UNIFAI_BASE_URL || "http://localhost:8080").replace(/\/+$/, "");
const adminAuthHeader = process.env.UNIFAI_E2E_AUTH_HEADER || "";
...
async function assertOtelMetricsReceived() {
  ...
  assertBufferContainsAll("OTEL metrics export", entry.body, [
    "unifai-e2e",
    "unifai_upstream_requests_total",
    "unifai_success_requests_total",
    "unifai_input_tokens_total",
    "unifai_output_tokens_total",
    "unifai_upstream_latency_seconds",
    "unifai_request_retries",
    "provider",
    providerName,
    "model",
    requestedModel,
...
async function assertPrometheusScrape() {
  const metrics = await poll("Prometheus scrape", 20000, async () => {
    ...
    const hasLLMSuccessMetric = res.text
      .split("\n")
      .some((line) => line.startsWith("unifai_success_requests_total{") &&
  ...
  const successLine = findPrometheusSample(metrics, "unifai_success_requests_total", labels);
  const upstreamLine = findPrometheusSample(metrics, "unifai_upstream_requests_total", labels);
  const inputLine = findPrometheusSample(metrics, "unifai_input_tokens_total", labels);
  const outputLine = findPrometheusSample(metrics, "unifai_output_tokens_total", labels);

  if (!successLine || parsePrometheusValue(successLine) < 1) {
    throw new Error("Prometheus scrape is missing unifai_success_requests_total for the LLM call");
  }
  if (!upstreamLine || parsePrometheusValue(upstreamLine) < 1) {
    throw new Error("Prometheus scrape is missing unifai_upstream_requests_total for the LLM call");
  }
