...
# UnifAI Auth Matrix Newman Test Runner
#
# Boots a fresh UnifAI server (sqlite, pre-seeded virtual key, unreachable dummy
...
# and runs collections/unifai-v1-auth-matrix.postman_collection.json against each.
#
...
#
# Requires a built unifai-http binary; this runner boots its own servers (it does
...
COLLECTION="collections/unifai-v1-auth-matrix.postman_collection.json"
REPORT_DIR="newman-reports/auth-matrix"
ADMIN_USER="admin"
ADMIN_PASS="Matrix-Admin-Pass1!"
VK_VALUE="sk-uf-matrix-test-key"
...
echo -e "${GREEN}==============================================${NC}"
echo -e "${GREEN}UnifAI Auth Matrix Test Runner${NC}"
echo -e "${GREEN}==============================================${NC}"
echo ""
...
run_combo() {
    ...
    while [ $elapsed -lt 60 ]; do
        if grep -q "successfully started unifai" "$server_log" 2>/dev/null; then
            break
        fi
