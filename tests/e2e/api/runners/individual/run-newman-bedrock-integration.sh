...
# UnifAI Bedrock Integration API Newman Test Runner
...
# Configuration
COLLECTION="collections/unifai-bedrock-integration.postman_collection.json"
ENVIRONMENT="unifai-v1.postman_environment.json"
...
if [ "$ci_normalized" = "1" ] || [ "$ci_normalized" = "true" ]; then
    echo -e "${GREEN}UnifAI Bedrock Integration API Test Runner with retries: 10${NC}"
else
    echo -e "${GREEN}UnifAI Bedrock Integration API Test Runner${NC}"
fi
echo -e "${GREEN}========================================${NC}"
echo ""
...
if [ -z "$PROVIDER_ENV_FILE" ] && [ -d "$PROVIDER_CONFIG_DIR" ]; then
    ...
    while IFS= read -r -d '' f; do
        # basename: unifai-v1-openai.postman_environment.json -> openai
        name="${f##*/}"
        name="${name#unifai-v1-}"
    ...
    done < <(find "$PROVIDER_CONFIG_DIR" -maxdepth 1 -name "unifai-v1-*.postman_environment.json" -print0 2>/dev/null | sort -z)
fi
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help)
            ...
            echo "  --env <path>        Postman env .json path or provider name (e.g. provider_config/unifai-v1-openai.postman_environment.json or openai)"
            echo "  --help              Show this help message"
            echo ""
            echo "Environment Variables:"
            echo "  UNIFAI_BASE_URL    Override base URL (default: http://localhost:8080)"
            echo "  UNIFAI_PROVIDER    Override provider (default: openai)"
            echo "  UNIFAI_MODEL       Override model name (default: gpt-4o)"
            echo "  UNIFAI_MODEL_INVOKE Override model for Bedrock invoke/invoke-stream (default: amazon.titan-text-express-v1)"
            echo "  UNIFAI_EMBEDDING_MODEL    Override embedding model (default: text-embedding-3-small)"
            echo "  UNIFAI_SPEECH_MODEL       Override speech model (default: tts-1)"
            echo "  UNIFAI_TRANSCRIPTION_MODEL  Override transcription model (default: whisper-1)"
            echo "  UNIFAI_IMAGE_MODEL        Override image model (default: dall-e-3)"
            echo "  UNIFAI_BEDROCK_API_KEY    Bedrock API key (when --env bedrock)"
            echo "  UNIFAI_BEDROCK_ACCESS_KEY Bedrock AWS access key (when --env bedrock)"
            echo "  UNIFAI_BEDROCK_SECRET_KEY Bedrock AWS secret key (when --env bedrock)"
            echo "  UNIFAI_BEDROCK_REGION     Bedrock region (default: us-east-1)"
            echo ""
            echo "Examples:"
            echo "  $0                                    # Run collection for all providers (each provider_config/unifai-v1-*.postman_environment.json)"
            ...
            echo "  UNIFAI_BASE_URL=http://api:8080 $0  # Custom base URL"
            exit 0
            ;;
        *)
...
# Optional second arg: path to Postman env .json file (e.g. provider_config/unifai-v1-openai.postman_environment.json).
# When given, uses only that env file; otherwise uses default env and UNIFAI_* overrides.
run_newman() {
    ...
    if [ -n "${2:-}" ] && [ -f "${2}" ]; then
        ...
        if [[ "${1:-}" == "bedrock" ]]; then
            [ -n "${UNIFAI_BEDROCK_API_KEY:-}" ] && cmd+=(--env-var "bedrock_api_key=$UNIFAI_BEDROCK_API_KEY")
            [ -n "${UNIFAI_BEDROCK_ACCESS_KEY:-}" ] && cmd+=(--env-var "bedrock_access_key=$UNIFAI_BEDROCK_ACCESS_KEY")
            [ -n "${UNIFAI_BEDROCK_SECRET_KEY:-}" ] && cmd+=(--env-var "bedrock_secret_key=$UNIFAI_BEDROCK_SECRET_KEY")
            [ -n "${UNIFAI_BEDROCK_REGION:-}" ] && cmd+=(--env-var "bedrock_region=$UNIFAI_BEDROCK_REGION")
            [ -n "${UNIFAI_BEDROCK_SESSION_TOKEN:-}" ] && cmd+=(--env-var "bedrock_session_token=$UNIFAI_BEDROCK_SESSION_TOKEN")
        fi
    else
        local base_url="${UNIFAI_BASE_URL:-http://localhost:8080}"
        local provider="${UNIFAI_PROVIDER:-openai}"
        local model="${UNIFAI_MODEL:-gpt-4o}"
        local model_invoke="${UNIFAI_MODEL_INVOKE:-amazon.titan-text-express-v1}"
        local embedding_model="${UNIFAI_EMBEDDING_MODEL:-text-embedding-3-small}"
        local speech_model="${UNIFAI_SPEECH_MODEL:-tts-1}"
        local transcription_model="${UNIFAI_TRANSCRIPTION_MODEL:-whisper-1}"
        local image_model="${UNIFAI_IMAGE_MODEL:-dall-e-3}"
...
if [ -n "$PROVIDER_ENV_FILE" ]; then
    ...
    elif [ -f "$PROVIDER_CONFIG_DIR/unifai-v1-${PROVIDER_ENV_FILE}.postman_environment.json" ]; then
        SINGLE_JSON_ENV="$PROVIDER_CONFIG_DIR/unifai-v1-${PROVIDER_ENV_FILE}.postman_environment.json"
    fi
    if [ -z "$SINGLE_JSON_ENV" ]; then
        echo -e "${RED}Error: Env file not found: $PROVIDER_ENV_FILE${NC}"
        echo "Use a path to a .json env (e.g. provider_config/unifai-v1-openai.postman_environment.json) or provider name (e.g. openai)"
        exit 1
    fi
    SINGLE_PROVIDER_NAME="${SINGLE_JSON_ENV##*/}"
    SINGLE_PROVIDER_NAME="${SINGLE_PROVIDER_NAME#unifai-v1-}"
...
if [ -z "${PROVIDER_JSON_FILES+x}" ] || [ ${#PROVIDER_JSON_FILES[@]} -eq 0 ]; then
    ...
    echo -e "  Base URL: ${YELLOW}${UNIFAI_BASE_URL:-http://localhost:8080}${NC}"
    echo -e "  Provider: ${YELLOW}${UNIFAI_PROVIDER:-openai}${NC}"
    echo -e "  Reports:  ${YELLOW}$REPORT_DIR${NC}"
    echo ""
    echo -e "${GREEN}Running tests...${NC}"
    echo ""
