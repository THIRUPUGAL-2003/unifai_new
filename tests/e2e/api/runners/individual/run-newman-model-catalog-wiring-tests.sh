...
# UnifAI Model Catalog Wiring Test Runner
# Drives the model-catalog wiring collection against a running UnifAI instance.
...
COLLECTION="collections/unifai-model-catalog-wiring.postman_collection.json"
ENVIRONMENT="unifai-v1.postman_environment.json"
REPORT_DIR="newman-reports/model-catalog-wiring"
...
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}UnifAI Model Catalog Wiring Tests${NC}"
echo -e "${GREEN}========================================${NC}"
echo ""
...
# Seed env loading: prefer an explicit UNIFAI_E2E_SEED_ENV, else the standard
...
# the collection's keys use UnifAI's `env.<NAME>` resolution, so each key reads
# its credential from the UnifAI process env at request time (the UnifAI server
...
set +e
"${cmd[@]}" --reporter-htmlextra-export "$REPORT_DIR/report.html" --reporter-htmlextra-title "UnifAI Model Catalog Wiring"
EXIT_CODE=$?
set -e
