...
# UnifAI MCP Auth Newman Test Runner
#
# Boots a fresh UnifAI server (sqlite, pre-seeded virtual keys + an upstream MCP
...
# and runs collections/unifai-v1-mcp-auth.postman_collection.json against each.
#
...
#
# Requires a built unifai-http binary; this runner boots its own servers (each
...
COLLECTION="collections/unifai-v1-mcp-auth.postman_collection.json"
REPORT_DIR="newman-reports/mcp-auth"
VK_VALUE="sk-uf-mcp-test-key"
VK_INACTIVE_VALUE="sk-uf-mcp-inactive-key"
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        --binary) UNIFAI_BINARY="$2"; shift 2 ;;
        --port) PORT="$2"; shift 2 ;;
        ...
        --help)
            echo "Usage: $0 --binary <path-to-unifai-http> [OPTIONS]"
            echo ""
            echo "Options:"
            echo "  --binary <path>   Path to a built unifai-http binary (required)"
...
echo -e "${GREEN}==============================================${NC}"
echo -e "${GREEN}UnifAI MCP Auth Test Runner${NC}"
echo -e "${GREEN}==============================================${NC}"
echo ""
...
if [ -z "$UNIFAI_BINARY" ] || [ ! -x "$UNIFAI_BINARY" ]; then
    echo -e "${RED}Error: --binary must point to an executable unifai-http binary${NC}"
    exit 1
fi
...
run_mode() {
    ...
    "$UNIFAI_BINARY" --app-dir "$CURRENT_DIR" --port "$PORT" --log-level info > "$server_log" 2>&1 &
    CURRENT_PID=$!

    local elapsed=0
    while [ $elapsed -lt 60 ]; do
        grep -q "successfully started unifai" "$server_log" 2>/dev/null && break
