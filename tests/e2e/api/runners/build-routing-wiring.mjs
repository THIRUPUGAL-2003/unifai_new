...
const DEFAULT_OUT = join(HERE, "..", "collections", "unifai-routing-wiring.postman_collection.json");

const NAME_PREFIX = "catwiring-rt-";

// Provider catalog. Key values use UnifAI's `env.<NAME>` resolution (credential
// read from the UnifAI process env at request time — no secret in the collection,
  ...
  for (const step of sc.steps) {
    switch (step.type) {
      ...
      case "route": {
        ...
        const headers = step.useVk === false ? [] : [{ key: "x-uf-vk", value: `{{vkval_${sid}}}` }];
        const body = {
...
const collection = buildCollection({
  id: "unifai-routing-wiring",
  name: "UnifAI Routing Wiring (Governance x Catalog)",
  description:
