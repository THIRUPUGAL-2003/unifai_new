#!/usr/bin/env node
// Exercises UnifAI's server-side stream cancellation path by opening a stream,
...
const logsDbUrlArg = args["logs-db-url"] || process.env.UNIFAI_LOGS_DB_URL || "";
const configPathArg = args["config"] || process.env.UNIFAI_CONFIG_PATH || "config.json";
const pricingUrl = args["pricing-url"] || process.env.UNIFAI_PRICING_URL || "https://getunifai.ai/datasheet";
...
function resolveLogsDbUrl() {
  ...
  try {
    ...
    if (ls.type === "postgres") {
      const c = ls.config || {};
      const host = c.host || "localhost", port = c.port || "5432", user = c.user || "unifai";
      const pass = c.password || "", db = c.db_name || "unifai", ssl = c.ssl_mode || "disable";
...
const resolveVariables = (value) => {
  if (typeof value === "string") {
    return value.replaceAll("{{azureDeployment}}", process.env.AZURE_DEPLOYMENT || process.env.UNIFAI_AZURE_DEPLOYMENT || "gpt-4o-mini");
  }
...
if (skipCostCheck) {
  ...
  if (!logsUrl) {
    console.error("[stream-cancel] no logs DB configured (set UNIFAI_LOGS_DB_URL or a config.json logs_store); skipping cost verification");
  } else {
    let db = null;
