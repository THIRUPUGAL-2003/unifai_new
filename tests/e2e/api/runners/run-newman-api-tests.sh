...
# UnifAI API Management & Health Tests
...
# Configuration
COLLECTION="$API_DIR/collections/unifai-api-management.postman_collection.json"
...
# Print banner
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}UnifAI API Management & Health Tests${NC}"
echo -e "${GREEN}========================================${NC}"
echo ""
...
BAIL=""
DB_VERIFY=""
DB_URL="${UNIFAI_DB_URL:-}"
LOGS_DB_URL="${UNIFAI_LOGS_DB_URL:-}"
DB_CONFIG_PATH=""
SEED_ENV_PATH="${UNIFAI_E2E_SEED_ENV:-}"
EXPECTED_PATH="${UNIFAI_E2E_SEED_EXPECTED:-}"
EXTRA_COLLECTIONS=()
BASE_URL="${UNIFAI_E2E_BASE_URL:-${UNIFAI_BASE_URL:-http://localhost:8080}}"
ADMIN_USERNAME="${UNIFAI_E2E_ADMIN_USERNAME:-admin}"
ADMIN_PASSWORD="${UNIFAI_E2E_ADMIN_PASSWORD:-UnifAI-E2E-Admin-Pass1!}"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help)
            ...
            echo "  --logs-db-url <dsn> Explicit logs DB url (also reads UNIFAI_LOGS_DB_URL; auto-detected)"
            echo "                      PostgreSQL: postgresql://user:pass@host:port/db"
            echo "                      SQLite:     sqlite:///path/to/file.db"
            echo "  --config-path <p>   Path to UnifAI config.json for auto DB detection"
            echo "                      (default: ./config.json; also reads UNIFAI_CONFIG_PATH env)"
...
if [ -n "${UNIFAI_API_EXTRA_COLLECTION:-}" ]; then
    EXTRA_COLLECTIONS+=("$UNIFAI_API_EXTRA_COLLECTION")
fi
...
UNIFAI_ROOT="$(cd "$API_DIR/../../.." && pwd)"
PLUGIN_DIR="$UNIFAI_ROOT/examples/plugins/hello-world"
...
HTTP_SERVER_DIR="$UNIFAI_ROOT/examples/mcps/http-no-ping-server"
...
cleanup() {
    if [ "$AUTH_ENABLED_BY_RUN" = "1" ]; then
        ...
        UNIFAI_E2E_AUTH_HEADER="$ADMIN_AUTH_HEADER" \
        UNIFAI_E2E_BASE_URL="$BASE_URL" \
        UNIFAI_E2E_ADMIN_USERNAME="$ADMIN_USERNAME" \
        UNIFAI_E2E_ADMIN_PASSWORD="$ADMIN_PASSWORD" \
...
build_newman_cmd() {
    ...
    if [[ "$REPORTERS" == *"htmlextra"* ]]; then
        ...
        cmd+=(--reporter-htmlextra-title "UnifAI API Management & Health${report_suffix}")
...
if [ $EXIT_CODE -eq 0 ] && [ "${UNIFAI_E2E_SKIP_OBSERVABILITY:-0}" != "1" ]; then
    echo "" | tee -a "$LOG_FILE"
    ...
    echo "" | tee -a "$LOG_FILE"
    echo -e "${YELLOW}Skipping local OTEL and Prometheus observability checks (UNIFAI_E2E_SKIP_OBSERVABILITY=1).${NC}" | tee -a "$LOG_FILE"
fi
