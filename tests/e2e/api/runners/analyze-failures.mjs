...
//     --unifai-log tmp/unifai-dev.log --out tmp/harness-failures.md
...
const UNIFAI_LOG = args["unifai-log"] || "tmp/unifai-dev.log";
...
const FIX_HINTS = {
  provider_not_configured: "Add the provider in `tests/integrations/python/config.json` (the APP_DIR config used by the harness) with a valid key/region. Restart `make dev` so UnifAI picks it up.",
  ...
  auth_invalid: "Re-source secrets via `INFISICAL=1` or update `.env`; verify the key matches the project/path your UnifAI config references. If the key is correct, check the provider account's quota/restrictions.",
  ...
  network_or_timeout: "Check UnifAI is still up (`curl /health`) and that the upstream provider isn't blocking egress. Also possible: the upstream provider is unreachable from your network.",
  unknown: "Open a bug; attach the unifai-dev.log excerpt below. Check the response body for a more specific error string.",
};
...
const categorize = (code, body, unifaiLines) => {
  // Check body content first - UnifAI's "failed to get config for provider" can manifest as
...
const grepUnifAIForUrl = (logText, url) => {
...
//   cross-model — UnifAI's unified /v1/chat/completions or /v1/responses
//   passthrough — /*_passthrough/* catch-all forwarding
...
const main = () => {
  ...
  const logText = existsSync(UNIFAI_LOG) ? readFileSync(UNIFAI_LOG, "utf8") : "";
  ...
  for (const e of execs) {
    ...
    if (!isFail) continue;
    const url = reconstructUrl(e.request?.url);
    const unifaiLines = grepUnifAIForUrl(logText, url);
    const category = categorize(code, body, unifaiLines);
  ...
  const lines = [];
  lines.push(`# UnifAI Provider Harness - Failure Report`);
  lines.push("");
  lines.push(`Generated: ${new Date().toISOString()}`);
  lines.push(`Source report: \`${REPORT}\``);
  lines.push(`UnifAI log: \`${UNIFAI_LOG}\` (${logText ? logText.split("\n").length + " lines" : "empty/missing"})`);
  lines.push("");
  ...
  for (const cat of orderedCats) {
    ...
    for (const f of items) {
      ...
      lines.push("```");
      if (f.unifaiLines.length) {
        lines.push("");
        lines.push(`Matching UnifAI log lines:`);
        lines.push("```");
        for (const l of f.unifaiLines) lines.push(truncate(l, 240));
        lines.push("```");
      }
      lines.push("");
    }
