# UnifAI Harness Coverage Backlog
...
## Cross-cutting (UnifAI-specific)

These exercise UnifAI's translation layer between provider shapes — every check below uses the unified
...
- [ ] **Computer use via cross-model** (`anthropic/claude-...` with computer_2025x tools - verifies UnifAI's translation; currently only tested via /anthropic drop-in and `vertex/claude-opus-4-7` preview at L1279)
...
- [~] **Sampling-params normalization** (UnifAI should silently drop temperature for Opus 4.7+; Anthropic-direct + Vertex Claude Opus 4.7 covered; **Bedrock Opus 4.7 via cross-model still missing**)
- [x] **MCP tool stripping for non-MCP providers** (UnifAI silently drops provider-side `type:"mcp"` server tools from a Responses request for Bedrock + Vertex instead of erroring; function tools — how local/configured MCP servers surface — survive — regression #3795. Folder "11. Cross-Provider Feature Tests / MCP Tool Handling cross-cut": 16-item matrix over {opus, sonnet} × {lone-mcp, mcp+function, multi-tool #3795 shape} plus /openai drop-in and streaming axes)
- [ ] **Failover scenarios** (request to provider X falls back to provider Y on 5xx)
- [ ] **Virtual keys / governance** (`X-UnifAI-VK` header with allowed_models)
- [ ] **Rate limit propagation** (provider 429 → UnifAI 429 with Retry-After preserved)

---

## Passthrough surface (UnifAI catch-all forwarding)
...
2. **Anthropic beta headers + new-gen tools** — lots of low-hanging features UnifAI claims to support
