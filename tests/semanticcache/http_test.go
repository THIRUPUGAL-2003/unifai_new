...
func loadConfig() {
	cfg.UnifAIURL = strings.TrimRight(getenv("UNIFAI_URL", "http://localhost:8080"), "/")
	...
	cfg.Namespace = getenv("SC_NAMESPACE", "UnifAISemanticCachePluginE2E")
...
// cacheDebug mirrors schemas.UnifAICacheDebug as it arrives over the wire.
...
type cacheHeaders struct {
	Key       string // x-uf-cache-key
	TTL       string // x-uf-cache-ttl
	Threshold *float64
	Type      string // x-uf-cache-type
	NoStore   string // x-uf-cache-no-store
}

func (h cacheHeaders) apply(req *http.Request) {
	if h.Key != "" {
		req.Header.Set("x-uf-cache-key", h.Key)
	}
	if h.TTL != "" {
		req.Header.Set("x-uf-cache-ttl", h.TTL)
	}
	if h.Threshold != nil {
		req.Header.Set("x-uf-cache-threshold", fmt.Sprintf("%v", *h.Threshold))
	}
	if h.Type != "" {
		req.Header.Set("x-uf-cache-type", h.Type)
	}
	if h.NoStore != "" {
		req.Header.Set("x-uf-cache-no-store", h.NoStore)
	}
}
...
func postChatStream(t *testing.T, lc logCtx, step int, req chatRequest, ch cacheHeaders) *streamResponse {
	...
	httpReq, err := http.NewRequest("POST", cfg.UnifAIURL+"/v1/chat/completions", bytes.NewReader(bodyBytes))
	if err != nil {
