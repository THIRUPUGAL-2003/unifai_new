...
// TestPreconditions verifies the test env is ready (UnifAI reachable,
...
func TestPreconditions(t *testing.T) {
	...
	logf(t, lc.at(0), "SETUP", "phase_start", map[string]any{"unifai_url": cfg.UnifAIURL})

	t.Run("0.1_unifai_reachable", func(t *testing.T) {
		lc := lc
		lc.name = "0.1_unifai_reachable"
		status, body, _, err := doJSON(t, "GET", "/api/plugins", nil, nil)
		if err != nil || status != http.StatusOK {
			logf(t, lc.at(1), "FAIL", "unifai_unreachable", map[string]any{
				"status": status, "err": err,
			})
			t.Fatalf("GET /api/plugins failed: status=%d err=%v body=%s",
				status, err, truncate(string(body), 200))
		}
		logf(t, lc.at(1), "PASS", "unifai_reachable", map[string]any{"status": status})
	})
