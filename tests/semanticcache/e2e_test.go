...
// directory, checks UnifAI reachability, ensures the plugin is absent (or
...
func TestMain(m *testing.M) {
	...
	// Sanity: UnifAI reachable.
	status, _, _, err := doRaw("GET", "/api/plugins")
	if err != nil {
		fmt.Fprintf(os.Stderr, "[SC-E2E] FATAL: cannot reach UnifAI at %s: %v\n", cfg.UnifAIURL, err)
		return
	}
	if status != http.StatusOK {
		fmt.Fprintf(os.Stderr, "[SC-E2E] FATAL: GET /api/plugins returned %d (UnifAI up at %s?)\n", status, cfg.UnifAIURL)
		return
	}
	...
	if status == http.StatusOK {
		if os.Getenv("RUN_FORCE") != "1" {
			fmt.Fprintf(os.Stderr,
				...
				pluginName, cfg.UnifAIURL, truncate(string(body), 300))
			return
		}
	...
	fmt.Fprintf(os.Stderr, "[SC-E2E] run starting: unifai=%s namespace=%s reports=%s trail_sid=%q\n",
		cfg.UnifAIURL, cfg.Namespace, runReportDir, trailSID)
...
func doRaw(method, path string) (int, []byte, http.Header, error) {
	req, err := http.NewRequest(method, cfg.UnifAIURL+path, nil)
	if err != nil {
