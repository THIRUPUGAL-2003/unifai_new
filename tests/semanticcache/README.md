...
- **UnifAI running** at `UNIFAI_URL` (default `http://localhost:8080`). Required endpoints: `/api/plugins/*`, `/api/cache/*`, `/api/providers`, `/api/logs/{id}`, `/v1/chat/completions`.
- **Vector store** configured in `config.json`, type **`weaviate`**, reachable from UnifAI. The plugin will create/use namespace `UnifAISemanticCachePluginE2E` by default (override via `SC_NAMESPACE`).
...
| var | default | purpose |
| --- | --- | --- |
| `UNIFAI_URL` | `http://localhost:8080` | UnifAI base URL |
...
| `SC_NAMESPACE` | `UnifAISemanticCachePluginE2E` | vector store namespace (isolates test data from prod) |
...
Start UnifAI under `trail`, capture the session id, export it, then run:

```bash
trail run --label semantic-cache-e2e -- ./unifai-http -port 8080 -config config.json
# capture the printed session id, then in another shell:
export TRAIL_SESSION_ID=<uuid>
RUN_FORCE=1 GOWORK=off go test -v ./...
```

Every log line carries `trail_sid=<uuid>`, so a single `trail get_logs` call with that session id reconstructs both the test harness output and the UnifAI stdout for the run.
...
On any FAIL the matching `*.resp.json` and `run.log` line carry enough info to grep via `trail` (look for `unifai_req_id=<id>` or `[SC-E2E] case=<name>`).
