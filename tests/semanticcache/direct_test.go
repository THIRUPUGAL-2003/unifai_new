...
func TestDirect(t *testing.T) {
	...
	// 1.4 no_key_no_default — when DefaultCacheKey="" and no x-uf-cache-key,
	...
	// 1.14 ttl_per_request_override — x-uf-cache-ttl=3s overrides plugin default (10s).
	...
	// 1.15 ttl_invalid_header_ignored — bogus x-uf-cache-ttl is silently ignored
	...
	// 1.16 no_store_header — both A and B send x-uf-cache-no-store=true; nothing
	...
	// 1.18 cache_type_direct_header — explicit x-uf-cache-type=direct in direct-only
	...
	// In direct-only mode with x-uf-cache-type=semantic, the plugin has no
	...
	// 1.41 threshold_header_ignored_direct_only — x-uf-cache-threshold has no
	...
	// 1.40 clear_unknown_id — DELETE with a random uuid. Whether UnifAI returns
	...
	// provider (or UnifAI validation) without crashing UnifAI. Accept any
	...
	// 1.54 ttl_zero_per_request — x-uf-cache-ttl=0s (or negative) falls back to
