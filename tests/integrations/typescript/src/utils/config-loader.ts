...
class ConfigLoader {
  ...
  getIntegrationUrl(integration: string): string {
    ...
    const unifaiConfig = this.config.unifai
    const baseUrl = unifaiConfig.base_url
    const endpoint = unifaiConfig.endpoints[integration]
  ...
  getUnifAIConfig(): UnifAIConfig {
    if (!this.config) throw new Error('Config not loaded')
    return this.config.unifai
  }
  ...
  validateConfig(): boolean {
    ...
    const requiredSections = ['unifai', 'providers', 'api', 'test_settings']
    ...
    const unifai = this.config.unifai
    if (!unifai.base_url || !unifai.endpoints) {
      throw new Error('UnifAI configuration missing base_url or endpoints')
    }
  ...
  printConfigSummary(): void {
    ...
    console.log('🔧 UNIFAI INTEGRATION TEST CONFIGURATION (TypeScript)')
    console.log('='.repeat(80))

    // UnifAI configuration
    const unifai = this.getUnifAIConfig()
    console.log('\n🌉 UNIFAI GATEWAY:')
    console.log(`  Base URL: ${unifai.base_url}`)
    console.log('  Endpoints:')
    for (const [integration, endpoint] of Object.entries(unifai.endpoints)) {
      const fullUrl = `${unifai.base_url.replace(/\/$/, '')}/${endpoint}`
