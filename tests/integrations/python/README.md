# UnifAI Integration Tests

Production-ready end-to-end test suite for testing AI integrations through UnifAI proxy. This test suite provides uniform testing across multiple AI integrations with comprehensive coverage of chat, tool calling, image processing, embeddings, speech synthesis, and multimodal workflows.
...
cd unifai/tests/integrations
uv sync

# 3. Set environment variables
export UNIFAI_BASE_URL="http://localhost:8080"
...
The UnifAI integration tests use a centralized configuration system that routes all AI integration requests through UnifAI as a gateway/proxy:

```text
┌─────────────────┐    ┌─────────────────┐    ┌─────────────────┐
│   Test Client   │───▶│  UnifAI Gateway │───▶│  AI Integration    │
...
- **Base URL**: `http://localhost:8080` (configurable via `UNIFAI_BASE_URL`)
...
- **🌉 UnifAI Gateway Integration**: All integrations route through UnifAI proxy
...
- UnifAI gateway settings and endpoints
...
```yaml
unifai:
  base_url: "${UNIFAI_BASE_URL:-http://localhost:8080}"
  endpoints:
    openai: "openai"
    anthropic: "anthropic"
    google: "genai"
...
3. **UnifAI Configuration**
   ```bash
   # Ensure UnifAI is running and accessible
...
# Check UnifAI logs for audio endpoint requests
# (Check your UnifAI instance logs)
```
...
- ✅ **Full UnifAI Integration**: Complete base URL support
...
- ✅ **Full UnifAI Integration**: Complete custom transport implementation
...
- ✅ **Full UnifAI Integration**: Global base URL configuration
...
- [ ] **UnifAI Integration**: Base URL properly configured and tested
...
```bash
# Error: Connection refused to UnifAI
ConnectionError: Connection refused to localhost:8080

# Solutions:
# 1. Check if UnifAI is running
curl http://localhost:8080/health

# 2. Ensure UNIFAI_BASE_URL is set correctly
echo $UNIFAI_BASE_URL
```
...
**Note**: This test suite is designed for testing AI integrations through UnifAI proxy. Ensure your UnifAI instance is properly configured and running before executing tests. The configuration system provides UnifAI routing for maximum flexibility.
