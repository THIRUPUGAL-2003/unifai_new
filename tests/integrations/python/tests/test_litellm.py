...
# that UnifAI's LiteLLM integration doesn't support
...
def setup_litellm(monkeypatch):
    """Setup LiteLLM with UnifAI configuration and dummy credentials"""
    import os
    ...
    os.environ["MISTRAL_API_KEY"] = "dummy-mistral-key-unifai-handles-auth"
    ...
    # LiteLLM will load these dummy credentials but all actual requests go through UnifAI
    ...
    # Since UnifAI handles auth, we don't need LiteLLM to authenticate
    def mock_refresh(self, request):
        """Mock refresh that sets a dummy token - UnifAI handles real auth"""
        import datetime

        self.token = "dummy-access-token-unifai-handles-auth"
