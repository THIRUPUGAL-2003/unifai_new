...
This test suite uses the AzureOpenAI SDK to test Azure-routed requests through UnifAI.
...
- Model names are passed RAW (e.g., "gpt-4o") without format_provider_model() prefix
  because UnifAI's AzureEndpointPreHook automatically adds the "azure/" prefix
...
def get_provider_azure_client(provider="azure", vk_enabled=False):
    ...
    It places the model/deployment-id in the URL path, and UnifAI's
...
class TestAzureIntegration:
    """Test suite for Azure OpenAI SDK integration through UnifAI"""
