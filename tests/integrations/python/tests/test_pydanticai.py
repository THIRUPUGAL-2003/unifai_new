...
This test suite uses Pydantic AI to test against multiple AI providers through UnifAI.
...
Tests Pydantic AI standard interface compliance and UnifAI integration:
...
def setup_pydanticai():
    """Setup Pydantic AI with UnifAI configuration and dummy credentials"""
    ...
    os.environ["GEMINI_API_KEY"] = "dummy-gemini-api-key-unifai-handles-auth"
    os.environ["CO_API_KEY"] = "dummy-cohere-key-unifai-handles-auth"
...
def get_openai_model(model_name: str | None = None) -> OpenAIChatModel:
    """Create an OpenAI model configured for UnifAI"""
    ...
    provider = OpenAIProvider(
        ...
        api_key="dummy-openai-key-unifai-handles-auth"
    )
    return OpenAIChatModel(model_name, provider=provider)


def get_anthropic_model(model_name: str = "claude-3-haiku-20240307") -> Optional[Any]:
    """Create an Anthropic model configured for UnifAI"""
    ...
    provider = AnthropicProvider(
        base_url=base_url,
        api_key="dummy-anthropic-key-unifai-handles-auth"
    )
    return AnthropicModel(model_name, provider=provider)


def get_google_model(model_name: str = "gemini-2.0-flash") -> Optional[Any]:
    """Create a Google model configured for UnifAI"""
    if not GOOGLE_AVAILABLE:
        return None

    base_url = get_integration_url("pydanticai")

    # Configure GoogleProvider with UnifAI endpoint
    provider = GoogleProvider(
        api_key="dummy-google-api-key-unifai-handles-auth",
        base_url=base_url
    )
...
def get_cohere_model(model_name: str = "command-r7b-12-2024") -> Optional[Any]:
    """Create a Cohere model configured for UnifAI"""
    ...
    # We create a custom client pointing to UnifAI and pass it to CohereProvider
    cohere_client = CohereAsyncClient(
        api_key="dummy-cohere-key-unifai-handles-auth",
        base_url=base_url
    )
...
class TestPydanticAIIntegration:
    """Comprehensive Pydantic AI integration tests through UnifAI"""
