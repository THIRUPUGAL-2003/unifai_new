...
This test suite uses the Anthropic SDK to test against multiple AI providers through UnifAI.
...
class TestAnthropicIntegration:
    ...
    def test_12_error_handling_invalid_roles(self, anthropic_client, test_config):
        ...
        # unifai handles invalid roles internally so this test should not raise an exception
        response = anthropic_client.messages.create(
            model=get_model("anthropic", "chat"),
...
This document is used to verify that the AI can read and understand text document inputs."""
    ...
    def test_53_openai_prompt_cache_key_from_metadata_user_id(self, test_config):
        ...
        When an Anthropic-format request carries metadata.user_id, UnifAI sets
        ...
        user_id = "unifai-test-cache-user"
...
class TestAnthropicCodeExecution:
    """Code execution tool — every invocation type a user can drive through UnifAI.
    ...
    def code_exec_client(self):
        """Anthropic client pointed at UnifAI (no beta header needed)."""
        from .utils.config_loader import get_config, get_integration_url
    ...
    def test_04_text_editor_create_view_edit(self, code_exec_client):
        ...
        # Verify the multi-key input round-tripped through UnifAI intact.
    ...
    def test_08_programmatic_tool_calling(self, code_exec_client):
        ...
        # UnifAI must auto-inject the advanced-tool-use beta header.
        custom_tool = {
