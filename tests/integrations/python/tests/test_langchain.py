...
Tests LangChain standard interface compliance and UnifAI integration:
...
def setup_langchain():
    """Setup LangChain with UnifAI configuration and dummy credentials"""
    # Set dummy credentials since UnifAI handles actual authentication
    os.environ["OPENAI_API_KEY"] = "dummy-openai-key-unifai-handles-auth"
    os.environ["ANTHROPIC_API_KEY"] = "dummy-anthropic-key-unifai-handles-auth"
    os.environ["GOOGLE_API_KEY"] = "dummy-google-api-key-unifai-handles-auth"
    os.environ["VERTEX_PROJECT"] = "dummy-vertex-project"
    os.environ["VERTEX_LOCATION"] = "us-central1"

    # Get UnifAI URL for LangChain
    base_url = get_integration_url("langchain")
    config = get_config()
    integration_settings = config.get_integration_settings("langchain")

    # Store original base URLs and set UnifAI URLs
    original_openai_base = os.environ.get("OPENAI_BASE_URL")
    original_anthropic_base = os.environ.get("ANTHROPIC_BASE_URL")

    if base_url:
        # Configure provider base URLs to route through UnifAI
...
class TestLangChainChatOpenAI(ChatModelIntegrationTests):
    """Standard LangChain tests for ChatOpenAI through UnifAI"""
...
class TestLangChainOpenAIEmbeddings(EmbeddingsIntegrationTests):
    """Standard LangChain tests for OpenAI Embeddings through UnifAI"""
...
class TestLangChainIntegration:
    """Comprehensive LangChain integration tests through UnifAI"""
    ...
    def test_14_gemini_chat_integration(self, test_config):
        ...
        try:
            # Use ChatGoogleGenerativeAI with UnifAI routing
            llm = ChatGoogleGenerativeAI(
                model="gemini-2.5-flash",
                google_api_key="dummy-google-api-key-unifai-handles-auth",
                temperature=0.7,
    ...
    def test_15_mistral_chat_integration(self, test_config):
        ...
        try:
            # Mistral is OpenAI-compatible, so it can route through UnifAI easily
            base_url = get_integration_url("langchain")
            if base_url:
                chat = ChatMistralAI(
                    model="mistral/mistral-small-2506",
                    mistral_api_key="dummy-mistral-api-key-unifai-handles-auth",
                    endpoint=f"{base_url}/v1",  # Route through UnifAI
                    temperature=0.7,
                    max_tokens=100,
                )
                ...
                pytest.skip("UnifAI URL not configured for LangChain integration")
    ...
    def test_19_structured_outputs(self, test_config, provider, model):
        ...
        try:
            # Create LangChain ChatOpenAI instance with UnifAI routing
            llm = ChatOpenAI(
                model=format_provider_model(provider, model),
                base_url=(
                    get_integration_url("langchain")
                ...
                api_key="dummy-key",  # Keys managed by UnifAI
            )
    ...
    def test_24_thinking_azure(self, test_config):
        ...
        try:
            default_headers = {}
            # Azure routing requires specific headers for UnifAI
            ...
            default_headers = {
                ...
                "x-uf-azure-endpoint": azure_endpoint,
            }
    ...
    def test_30_bedrock_guardrail_config_forwarded(self, test_config):
        ...
        traffic routed through UnifAI's Responses API path.  LangChain serialises the
        guardrails kwarg into guardrailConfig in the Bedrock Converse request body;
        UnifAI must forward it unchanged so the guardrail actually fires.
        ...
        try:
            ...
            assert stop_reason == "guardrail_intervened", (
                ...
                f"dropped by UnifAI before reaching Bedrock.  Got: {stop_reason!r}"
            )
    ...
    def test_31_bedrock_guardrail_outputAssessments_is_map(self, test_config):
        ...
        UnifAI 1.4.6 crashed with mismatch type []bedrock.BedrockGuardrailAssessment
