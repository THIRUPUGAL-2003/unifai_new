...
This test suite uses the Google GenAI SDK to test against multiple AI providers through UnifAI.
...
def get_vertex_job_service_client():
    """Build a native Vertex AI JobServiceClient pointed at the UnifAI gateway.
    ...
    UnifAI via the gateway base URL. Auth is anonymous because UnifAI injects the
    real Vertex credentials from its key config; UnifAI detects Vertex routing from
    ...
    # Route through UnifAI's genai integration (the Vertex batch routes are mounted
    # under the /genai prefix alongside the other GenAI endpoints).
    api_endpoint = get_unifai_base_url().rstrip("/") + "/genai"
...
class TestGoogleIntegration:
    ...
    def test_pdf_file_input(self, test_config):
        ...
        # Create direct Google client (without UnifAI) for file upload
        direct_client = genai.Client(
            api_key=os.getenv("GEMINI_API_KEY")
        )
        
        # Get UnifAI client for generate_content
        unifai_client = get_provider_google_client("gemini")
        ...
        try:
            # Upload the PDF file using direct Google client (not through UnifAI)
            uploaded_file = direct_client.files.upload(
                file=temp_pdf_path,
                config=types.UploadFileConfig(display_name='test_pdf_gemini')
            )
            
            # Use the uploaded file in a generate_content request through UnifAI
            response = unifai_client.models.generate_content(
...
Joe: Pretty good, thanks for asking."""
    ...
    def test_30_multimodal_function_response_image(self, test_config, model, expect_image_understood):
        ...
        The image must survive UnifAI's Gemini<->UnifAI translation instead of being dropped.
        Regression guard for multimodal function_call_output (text + image), and for the
        model-version gating UnifAI applies:

          - gemini-3-flash-preview: UnifAI forwards the image as functionResponse.parts (no $ref;
          ...
          - gemini-2.5-flash: multimodal function responses are unsupported (a hard 400 upstream),
            so UnifAI drops the image and sends text only. The request must still SUCCEED; the
        ...
        # the SDK base64-encodes it on the wire (what UnifAI receives).
    ...
    def test_30b_multimodal_function_response_full_workflow(self, test_config, model, expect_image_understood):
        ...
        Gemini docs example) through UnifAI.
        ...
        UnifAI adapts the $ref per provider: it keeps it for Vertex and strips it for the Gemini
        Developer API (where the $ref form is an upstream 400 bug), so this works on both.
          ...
          - gemini-2.5-flash: multimodal tool output is unsupported, so UnifAI drops the image and
    ...
    def test_41a_image_generation_simple(self, test_config, provider, model):
        """Test Case 41a: Simple image generation with Gemini model via UnifAI"""
        if provider == "_no_providers_" or model == "_no_model_":
    ...
    def test_41b_imagen_predict(self, test_config, provider, model):
        """Test Case 41b: Image generation using Imagen model via UnifAI"""
        if provider == "_no_providers_" or model == "_no_model_":
        ...
        # UnifAI will automatically route to the :predict endpoint for Imagen models
        # Use provider/model format so UnifAI can parse the provider and route correctly
        try:
            response = client.models.generate_content(
                model=format_provider_model(provider, model),
                contents=IMAGE_GENERATION_SIMPLE_PROMPT,
    ...
    def test_42a_image_edit_simple(self, test_config, provider, model):
        """Test Case 42a: Simple image edit with Gemini/Imagen via UnifAI"""
        if provider == "_no_providers_" or model == "_no_model_":
    ...
    def test_42b_imagen_edit(self, test_config, provider, model):
        """Test Case 42b: Image editing using Imagen model via UnifAI"""
        if provider == "_no_providers_" or model == "_no_model_":
        ...
        # For Imagen edit models, UnifAI routes to :editImage endpoint
        try:
            response = client.models.generate_content(
                model=format_provider_model(provider, model),
                contents=[
    ...
    # through UnifAI. Inputs/outputs live in GCS (instances_format /
    ...
    def test_vertex_batch_create(self, test_config):
        ...
        body = build_vertex_batch_prediction_job(
            display_name="unifai-vertex-batch-create",
            model=model,
    ...
    def test_vertex_batch_get(self, test_config):
        ...
        body = build_vertex_batch_prediction_job(
            display_name="unifai-vertex-batch-get",
            model=model,
            gcs_source_uri=gcs_source_uri,
            gcs_destination_output_uri_prefix=get_vertex_batch_dest_uri(),
        )
    ...
    def test_vertex_batch_list(self, test_config):
        ...
        body = build_vertex_batch_prediction_job(
            display_name="unifai-vertex-batch-list",
            model=model,
            gcs_source_uri=gcs_source_uri,
            gcs_destination_output_uri_prefix=get_vertex_batch_dest_uri(),
        )
    ...
    def test_vertex_batch_cancel(self, test_config):
        ...
        body = build_vertex_batch_prediction_job(
            display_name="unifai-vertex-batch-cancel",
            model=model,
            gcs_source_uri=gcs_source_uri,
            gcs_destination_output_uri_prefix=get_vertex_batch_dest_uri(),
        )
    ...
    def test_vertex_batch_delete(self, test_config):
        ...
        body = build_vertex_batch_prediction_job(
            display_name="unifai-vertex-batch-delete",
            model=model,
            gcs_source_uri=gcs_source_uri,
            gcs_destination_output_uri_prefix=get_vertex_batch_dest_uri(),
        )
    ...
    # =========================================================================
    # CONTEXT CACHING TEST CASES (Gemini Caches API - pass-through via UnifAI)
    # =========================================================================
