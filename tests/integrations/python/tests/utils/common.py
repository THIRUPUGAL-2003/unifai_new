...
# GenAI-specific invalid role content that passes SDK validation but fails at UnifAI
GENAI_INVALID_ROLE_CONTENT = [
    {
        "role": "tester",  # Invalid role that should be caught by UnifAI
...
def assert_error_propagation(error_response: Any, integration: str):
    """
    Assert that error is properly propagated through UnifAI to the integration.
...
def get_bedrock_s3_config() -> Dict[str, Optional[str]]:
    ...
    return {
        ...
        "output_s3_prefix": os.environ.get("AWS_OUTPUT_S3_PREFIX", "unifai-batch-output/"),
...
def create_bedrock_batch_s3_uri(
    bucket: str, prefix: str = "unifai-batch-input/", filename: str | None = None
) -> str:
    """
...
def get_vertex_gcs_config() -> Dict[str, Optional[str]]:
    ...
    return {
        ...
        "prefix": os.environ.get("VERTEX_GCS_PREFIX", "unifai-batch-tests/"),
    }
...
def get_vertex_batch_dest_uri() -> str:
    ...
    Returns:
        GCS URI string (e.g., gs://bucket/unifai-batch-tests/output)
...
def get_unifai_base_url() -> str:
    """Base URL of the UnifAI gateway (from UNIFAI_BASE_URL)."""
    return os.environ.get("UNIFAI_BASE_URL", "http://localhost:8080")
...
def get_vertex_google_credentials(scopes: Optional[List[str]] = None):
    ...
    The Vertex service-account key is provided to UnifAI via VERTEX_CREDENTIALS, which
...
def get_ws_base_url():
    ...
    config = get_config()
    base_url = config._config["unifai"]["base_url"]
...
def run_ws_responses_test(
    ...
    Args:
        ...
        extra_headers: Additional headers dict (e.g. {"x-uf-vk": "..."})
