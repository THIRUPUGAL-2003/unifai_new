"""
Configuration loader for UnifAI integration tests.

This module loads configuration from config.yml and provides utilities
for constructing integration URLs through the UnifAI gateway.
"""
...
class ConfigLoader:
    ...
    def get_integration_url(self, integration: str) -> str:
        ...
        """
        unifai_config = self._config["unifai"]
        base_url = unifai_config["base_url"]
        endpoint = unifai_config["endpoints"].get(integration, "")
    ...
    def get_unifai_config(self) -> UnifAIConfig:
        """Get UnifAI configuration"""
        unifai_data = self._config["unifai"]
        return UnifAIConfig(
            base_url=unifai_data["base_url"], endpoints=unifai_data["endpoints"]
        )
    ...
    def validate_config(self) -> bool:
        ...
        required_sections = ["unifai", "providers", "api", "test_settings"]

        for section in required_sections:
            if section not in self._config:
                raise ValueError(f"Missing required configuration section: {section}")

        # Validate UnifAI configuration
        unifai = self._config["unifai"]
        if "base_url" not in unifai or "endpoints" not in unifai:
            raise ValueError("UnifAI configuration missing base_url or endpoints")
    ...
    def print_config_summary(self):
        ...
        print("🔧 UNIFAI INTEGRATION TEST CONFIGURATION")
        print("=" * 80)

        # UnifAI configuration
        unifai = self.get_unifai_config()
        print("\n🌉 UNIFAI GATEWAY:")
        print(f"  Base URL: {unifai.base_url}")
        print("  Endpoints:")
        for integration, endpoint in unifai.endpoints.items():
            full_url = f"{unifai.base_url.rstrip('/')}/{endpoint}"
