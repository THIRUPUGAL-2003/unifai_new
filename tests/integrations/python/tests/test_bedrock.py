...
This test suite uses the AWS SDK (boto3) to test against multiple AI providers through UnifAI.
...
Tests core scenarios using AWS SDK (boto3) directly against UnifAI:
...
@pytest.fixture
def s3_client():
    """Create S3 client for file operations via UnifAI's S3-compatible API"""
    base_url = get_integration_url("bedrock")
    config = get_config()
    integration_settings = config.get_integration_settings("bedrock")
    region = integration_settings.get("region", "us-west-2")

    # Point boto3 S3 client to UnifAI's S3-compatible endpoint
    client = boto3.client(
        "s3",
        region_name=region,
        endpoint_url=f"{base_url}/files",
...
class TestBedrockIntegration:
    ...
    def test_11_file_upload(self, test_config, provider, model):
        ...
        # Upload to S3
        s3_key = f"unifai-test-files/batch_input_{int(time.time())}.jsonl"
    ...
    def test_12_file_list(self, test_config, provider, model):
        ...
        s3_key = f"unifai-test-files/test_list_{int(time.time())}.jsonl"
        ...
        try:
            # List files
            response = s3_client.list_objects_v2(Bucket=s3_bucket, Prefix="unifai-test-files/")
    ...
    def test_13_file_retrieve(self, test_config, provider, model):
        ...
        s3_key = f"unifai-test-files/test_retrieve_{int(time.time())}.jsonl"
    ...
    def test_14_file_delete(self, test_config, provider, model):
        ...
        s3_key = f"unifai-test-files/test_delete_{int(time.time())}.jsonl"
    ...
    def test_15_file_content(self, test_config, provider, model):
        ...
        s3_key = f"unifai-test-files/test_content_{int(time.time())}.jsonl"
    ...
    def test_16_batch_create(self, test_config, provider, model):
        ...
        s3_bucket = "unifai-batch-api-file-upload-testing"
        s3_key = f"unifai-batch-input/batch_input_{int(time.time())}.jsonl"
        upload_response = s3_client.put_object(
            Bucket=s3_bucket,
            Key=s3_key,
            Body=jsonl_content.encode(),
        ...
        try:
            ...
            response = bedrock_client.create_model_invocation_job(
                jobName=f"unifai-test-batch-{int(time.time())}",
                modelId=model,
                roleArn="",
                inputDataConfig={
                ...
                outputDataConfig={
                    "s3OutputDataConfig": {"s3Uri": f"s3://{s3_bucket}/unifai-batch-output/"}
                },
                tags=[
    ...
    def test_19_batch_cancel(self, test_config, provider, model):
        ...
        s3_key = f"unifai-batch-cancel/batch_cancel_{int(time.time())}.jsonl"
        upload_response = s3_client.put_object(
            Bucket=s3_bucket,
            Key=s3_key,
            Body=jsonl_content.encode(),
        ...
        try:
            ...
            create_response = bedrock_client.create_model_invocation_job(
                jobName=f"unifai-cancel-test-{int(time.time())}",
                modelId=model,
                roleArn=role_arn,
                inputDataConfig={
                    "s3InputDataConfig": {"s3Uri": input_uri, "s3InputFormat": "JSONL"}
                ...
                outputDataConfig={
                    "s3OutputDataConfig": {
                        "s3Uri": f"s3://{s3_bucket}/unifai-batch-cancel-output/"
                    }
                },
                tags=[
                    {"key": "endpoint", "value": "/v1/chat/completions"},
    ...
    def test_20_batch_e2e(self, test_config, provider, model):
        ...
        s3_key = f"unifai-batch-e2e/batch_e2e_{int(time.time())}.jsonl"
        upload_response = s3_client.put_object(
            Bucket=s3_bucket,
            Key=s3_key,
            Body=jsonl_content.encode(),
        ...
        try:
            ...
            create_response = bedrock_client.create_model_invocation_job(
                jobName=f"unifai-e2e-{int(time.time())}",
                modelId=model,
                roleArn=role_arn,
                inputDataConfig={
                    "s3InputDataConfig": {"s3Uri": input_uri, "s3InputFormat": "JSONL"}
                ...
                outputDataConfig={
                    "s3OutputDataConfig": {"s3Uri": f"s3://{s3_bucket}/unifai-batch-e2e-output/"}
                },
                tags=[
                    {"key": "endpoint", "value": "/v1/chat/completions"},
                    {"key": "file_id", "value": file_id},
...
class TestBedrockInvokeEndpoint:
    ...
    def test_32_invoke_cohere_embeddings(self, bedrock_client):
        ...
        # UnifAI may return Titan-compat single "embedding" for invoke
        if "embedding" in out:
            _assert_invoke_embedding(out)
        else:
...
# ---------------------------------------------------------------------------
# These tests exercise Bedrock Nova system tools through the UnifAI converse
...
#
# nova_grounding  → maps to web_search in UnifAI neutral schema
# nova_code_interpreter → maps to code_interpreter in UnifAI neutral schema
# ---------------------------------------------------------------------------
...
class TestNovaSystemTools:
    ...
    def test_50_nova_grounding_non_streaming(self, bedrock_client):
        ...
        automatically searches the web and returns a grounded text response. UnifAI
    ...
    def test_51_nova_grounding_streaming(self, bedrock_client):
        ...
        UnifAI must reproduce these citation deltas as contentBlockDelta.citation events.
        ...
        assert len(citation_urls) > 0, (
            ...
            f"nova_grounding must emit citation deltas that UnifAI preserves as "
    ...
    def test_52_nova_code_interpreter_non_streaming(self, bedrock_client):
        ...
        UnifAI merges these into a code_interpreter_call output item and converts
    ...
    def test_53_nova_code_interpreter_streaming(self, bedrock_client):
        ...
        UnifAI must reproduce this exact shape on the converse-stream route.
        """
        ...
        assert has_code_interpreter_block_start, (
            ...
            "UnifAI must emit this event for the nova_code_interpreter block"
        )
        ...
        assert len(code_snippets) > 0, (
            ...
            "from nova_code_interpreter — UnifAI must emit these code deltas"
        )
...
class TestBedrockGuardrail:
    ...
    def test_55_guardrail_trace_in_response(self, bedrock_client):
        ...
        Regression test for ToUnifAIResponsesResponse silently dropping
        response.Trace.  The trace must survive the UnifAI round-trip so callers
        ...
        assert trace is not None, (
            "response.trace is missing — guardrail trace was dropped during UnifAI conversion"
        )
    ...
    def test_56_outputAssessments_is_map(self, bedrock_client):
        ...
        Regression for UnifAI 1.4.6: BedrockGuardrailTrace.OutputAssessments was
