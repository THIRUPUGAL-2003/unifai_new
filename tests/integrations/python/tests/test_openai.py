...
This test suite uses the OpenAI SDK to test against multiple AI providers through UnifAI.
...
def get_provider_openai_client(provider, vk_enabled=False):
    ...
    Args:
        ...
        vk_enabled: If True, include x-uf-vk header for virtual key testing
    """
...
def get_file_storage_config(provider):
    ...
    return {
        "s3": {
            ...
            "prefix": settings.get("output_s3_prefix", "unifai-batch-output"),
        }
    }
...
class TestOpenAIIntegration:
        ...
        try:
            ...
            for _ in range(6):
                page = client.videos.list(
                    limit=20,
                    extra_headers={"x-uf-video-list-provider": "openai"},
                )
    ...
    # UnifAI to xAI's API.  The openai_client fixture hits UnifAI's /openai
    ...
    def test_46_batch_create_with_file(self, test_config, provider, model, vk_enabled):
        ...
        s3_prefix = integration_settings.get("output_s3_prefix", "unifai-batch-output")
    ...
    def test_50_batch_e2e_file_api(self, test_config, provider, model, vk_enabled):
        ...
        if provider == "vertex":
            ...
            try:
                ...
                batch = client.batches.create(
                    ...
                    endpoint="/v1/chat/completions",
                    completion_window="24h",
                    metadata={"test": "e2e_file", "source": "unifai-integration-tests"},
                    extra_body={
                        "provider": provider,
                        "model": model,
                        "output_folder": {"url": get_vertex_batch_dest_uri()},
        ...
        try:
            ...
            batch = client.batches.create(
                ...
                metadata={
                    "test": "e2e_file",
                    "source": "unifai-integration-tests",
                },
                extra_body={
                    "provider": provider,
                    "model": model,
    ...
    def test_58_async_chat_completions(self, test_config, provider, model, vk_enabled):
        ...
        for i in range(max_polls):
            ...
            poll = client.chat.completions.create(
                **request_params,
                extra_headers={"x-uf-async-id": initial.id},
            )
    ...
    def test_64_realtime_client_secret_routes(self, test_config, provider, _model, vk_enabled):
        ...
        config = get_config()
        base_url = config._config["unifai"]["base_url"].rstrip("/")
        api_key = get_api_key(provider)
    ...
    def test_65_realtime_client_secret_openai_base_url_compatibility(
        ...
        config = get_config()
        api_key = get_api_key(provider)
        root_base_url = config._config["unifai"]["base_url"].rstrip("/")
