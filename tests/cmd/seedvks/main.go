...
// encryption key and postgres DSN are read from a UnifAI config.json.
//
...
// "sk-uf-<uuid>". It is duplicated here to avoid importing the governance plugin
// (which lives in a separate module) into this seeding utility.
const virtualKeyPrefix = "sk-uf-"
...
// fileConfig is the subset of a UnifAI config.json this tool reads.
...
// UnifAI config.json at the given path.
...
func run(cfg fileConfig, count, batch int, prefix string, skipSeed bool, rl rateLimitParams, governanceMode string) (int, error) {
	...
	encrypt.Init(cfg.EncryptionKey, unifai.NewDefaultLogger(schemas.LogLevelWarn))
...
func main() {
	configPath := flag.String("config", "", "path to UnifAI config.json (provides encryption_key and postgres DSN)")
