...
import (
	...
	e2eseed "github.com/unifai/unifai/tests/cmd/seed"
)
...
func run(ctx context.Context, args []string) error {
	...
	fs.StringVar(&opts.ConfigPath, "config-path", opts.ConfigPath, "optional UnifAI config.json path used to derive DB settings")
	fs.StringVar(&opts.EncryptionKey, "encryption-key", opts.EncryptionKey, "optional UnifAI encryption key for encrypted config rows")
