...
func NormalizeOptions(opts Options) (Options, error) {
	...
	if opts.EncryptionKey == "" {
		opts.EncryptionKey = os.Getenv("UNIFAI_ENCRYPTION_KEY")
	}
	if opts.ConfigDSN == "" {
		opts.ConfigDSN = "postgres://unifai:unifai_password@localhost:5432/unifai?sslmode=disable"
	}
	if opts.LogsDialect == "" {
		opts.LogsDialect = def.LogsDialect
	}
	if opts.LogsDSN == "" {
		opts.LogsDSN = "postgres://unifai:unifai_password@localhost:5432/unifai?sslmode=disable"
	}
...
// InitEncryption initializes UnifAI field encryption for DBs containing encrypted rows.
func InitEncryption(opts Options) {
	if opts.EncryptionKey == "" {
		return
	}
	encrypt.Init(opts.EncryptionKey, unifai.NewDefaultLogger(schemas.LogLevelWarn))
}

// optionsFromConfigFile extracts config/log store connection settings from a UnifAI config.json.
...
// configValue returns a raw JSON config value, resolving UnifAI env-var wrappers.
func configValue(config map[string]json.RawMessage, key string) string {
	return rawConfigValue(config[key])
}

// rawConfigValue returns a raw JSON config value, resolving UnifAI env-var wrappers.
