...
// APIRequest represents a request to the UnifAI API
...
// APIResponse represents a response from the UnifAI API
...
// MakeRequest makes an HTTP request to the UnifAI API
func MakeRequest(t *testing.T, req APIRequest) *APIResponse {
	...
	if req.VKHeader != nil {
		httpReq.Header.Set("x-uf-vk", *req.VKHeader)
	}
