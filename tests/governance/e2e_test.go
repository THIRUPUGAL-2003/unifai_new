...
// TestVirtualKeyHeaderFormats verifies that UnifAI accepts all documented VK header formats
// Reference: https://docs.unifai.ai/features/governance/virtual-keys
// Supported headers:
//   - x-uf-vk: Virtual key header (UnifAI native)
...
func TestVirtualKeyHeaderFormats(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			headerName:   "x-uf-vk",
			headerValue:  vkValue,
			description:  "UnifAI native VK header",
			expectedPass: true,
		},
		{
