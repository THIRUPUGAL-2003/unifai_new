...
// for the UnifAI HTTP service. This file contains the setup and configuration
...
// - Custom dimension headers (x-uf-dim-*) — the canonical header prefix
// - Deprecated custom prometheus headers (x-uf-prom-*) — kept for backward compatibility
...
func collectPrometheusKeyValues(ctx *fasthttp.RequestCtx) map[string]string {
	...
	ctx.Request.Header.All()(func(key, value []byte) bool {
		...
		// x-uf-dim-* (canonical; replaces x-uf-prom-*)
		...
		// x-uf-prom-* (deprecated; kept for backward compatibility)
		if labelName, ok := strings.CutPrefix(keyStr, "x-uf-prom-"); ok && labelName != "" {
			// Only set if not already provided via x-uf-dim-* (x-uf-dim takes precedence)
