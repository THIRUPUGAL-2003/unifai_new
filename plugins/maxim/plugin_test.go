...
// It includes tests for plugin initialization, UnifAI integration, and request/response tracing.
package maxim
...
// TestMaximLoggerPlugin tests the integration of the Maxim Logger plugin with UnifAI.
...
func TestMaximLoggerPlugin(t *testing.T) {
	...
	_, unifaiErr := client.ChatCompletionRequest(schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline), &schemas.UnifAIChatRequest{
		...
		Input: []schemas.ChatMessage{
			{
				...
				Content: &schemas.ChatMessageContent{
					ContentStr: unifai.Ptr("Hello, how are you?"),
				},
			},
		},
	})
	...
	if unifaiErr != nil {
		log.Printf("Error in UnifAI request: %v", unifaiErr)
	}

	log.Println("UnifAI request completed, check your Maxim Dashboard for the trace")
