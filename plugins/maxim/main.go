...
// These IDs can be propagated from external systems through HTTP headers (x-uf-maxim-trace-id and x-uf-maxim-generation-id).
...
func (plugin *Plugin) PostLLMHook(ctx *schemas.UnifAIContext, result *schemas.UnifAIResponse, unifaiErr *schemas.UnifAIError) (*schemas.UnifAIResponse, *schemas.UnifAIError, error) {
	...
	go func() {
		...
		// add x-uf-dim-* dimensions as tags (lower priority than explicit x-uf-maxim-* tags)
		if hasDims {
			for key, value := range dims {
				// Only add if not already set by x-uf-maxim-* (to avoid overriding explicit tags)
