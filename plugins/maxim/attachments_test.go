...
// Test URLs and assets from core/internal/llmtests/utils.go for consistency across UnifAI tests.
var (
...
func TestExtractAttachmentsFromRequest_ImageGenerationInputImagesUrl(t *testing.T) {
	req := &schemas.UnifAIRequest{
		...
		ImageGenerationRequest: &schemas.UnifAIImageGenerationRequest{
...
// TestVisionWithImageUrl_Integration sends a real OpenAI vision request via UnifAI
...
func TestVisionWithImageUrl_Integration(t *testing.T) {
	...
	client, err := unifai.Init(context.Background(), schemas.UnifAIConfig{
		Account:    &BaseAccount{},
					...
					ContentBlocks: []schemas.ChatContentBlock{
						{
							...
							Text: unifai.Ptr("Describe this image in one sentence."),
						},
						{
							Type: schemas.ChatContentBlockTypeImage,
							ImageURLStruct: &schemas.ChatInputImage{
	...
	if unifaiErr != nil {
		t.Fatalf("ChatCompletionRequest failed: %v", unifaiErr)
	}
...
// and sends to OpenAI vision via UnifAI. The maxim plugin extracts the data URL and
		...
		Input: []schemas.ChatMessage{
			{
				...
				Content: &schemas.ChatMessageContent{
					ContentBlocks: []schemas.ChatContentBlock{
						{
							Type: schemas.ChatContentBlockTypeText,
							Text: unifai.Ptr("What do you see in this image? One sentence."),
						},
						{
							Type: schemas.ChatContentBlockTypeImage,
							ImageURLStruct: &schemas.ChatInputImage{
