...
func TestPostLLMHookNoPendingErrorPreservesMetadata(t *testing.T) {
	...
	ctx.SetValue(schemas.UnifAIContextKeyRequestID, "req-error-no-pending")
	ctx.SetValue(schemas.UnifAIContextKeyRequestHeaders, map[string]string{
		"x-uf-lh-tenant": "acme",
		"x-custom-log":   "custom-value",
	})
	ctx.SetValue(schemas.UnifAIContextKeyDimensions, map[string]string{
		"region": "us-east",
	})
	ctx.SetValue(schemas.UnifAIIsAsyncRequest, true)
	...
	_, _, err = plugin.PostLLMHook(ctx, nil, unifaiErr)
	if err != nil {
...
func TestPostLLMHookStreamingErrorPreservesHeaderMetadata(t *testing.T) {
	...
	ctx := schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline)
	ctx.SetValue(schemas.UnifAIContextKeyRequestID, "req-stream-error-metadata")
	ctx.SetValue(schemas.UnifAIContextKeyRequestHeaders, map[string]string{
		"x-custom-log":   "custom-value",
		"x-uf-lh-user":   `{"device_id":"device-1","session_id":"session-1"}`,
		"x-uf-lh-tag":    "from-header",
		"x-uf-lh-shared": "from-header",
	})
	ctx.SetValue(schemas.UnifAIContextKeyDimensions, map[string]string{
		"environment": "staging",
	})
	...
	if _, _, err = plugin.PostLLMHook(ctx, nil, unifaiErr); err != nil {
		t.Fatalf("PostLLMHook() error = %v", err)
	}
	if err := plugin.Cleanup(); err != nil {
		t.Fatalf("Cleanup() error = %v", err)
...
// partial usage the provider already processed (UnifAIError.ExtraFields.BilledUsage)
...
func TestPostLLMHookCancelledStreamLogsCost(t *testing.T) {
	...
	ctx := schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline)
	ctx.SetValue(schemas.UnifAIContextKeyRequestID, "req-cancel-cost")
...
func TestPostLLMHookContextTimeoutLogsCancelledStatus(t *testing.T) {
	...
	ctx := schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline)
	ctx.SetValue(schemas.UnifAIContextKeyRequestID, "req-context-timeout")
...
func TestPostLLMHookProviderTimeoutRemainsErrorStatus(t *testing.T) {
	...
	ctx := schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline)
	ctx.SetValue(schemas.UnifAIContextKeyRequestID, "req-provider-timeout")
...
func TestLogStatusForErrorDoesNotTreatGenericDeadlineMessageAsCancelled(t *testing.T) {
	...
	if got := logStatusForError(unifaiErr); got != "error" {
		t.Fatalf("expected generic provider deadline message to remain error, got %q", got)
	}
}

func TestLogStatusForErrorTreatsRateLimitAsWarning(t *testing.T) {
	statusCode := 429
	unifaiErr := &schemas.UnifAIError{
		IsUnifAIError: true,
		StatusCode:    &statusCode,
		Type:          schemas.Ptr("token_limited"),
		Error: &schemas.ErrorField{
			Message: "rate limit violated for Model:cohere/north-mini-code:free:Provider:openrouter: [token limit exceeded (60/10, resets every 1h)]",
			Type:    schemas.Ptr("token_limited"),
		},
	}
	if got := logStatusForError(unifaiErr); got != "warning" {
		t.Fatalf("expected rate-limit errors to log as warning, got %q", got)
	}
}
...
func TestApplyErrorBillingFromBilledUsage_ComputesCostWhenTokensAlreadyParsed(t *testing.T) {
	...
	entry := &logstore.Log{
		...
		CompletionTokens: completionTokens,
		TotalTokens:      promptTokens + completionTokens,
		TokenUsageParsed: &schemas.UnifAILLMUsage{
			PromptTokens:     promptTokens,
			CompletionTokens: completionTokens,
			TotalTokens:      promptTokens + completionTokens,
		},
...
func TestMCPHooksDeferDBWriteUntilPostHookBatch(t *testing.T) {
	...
	ctx := schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline)
	ctx.SetValue(schemas.UnifAIContextKeyRequestID, "req-mcp-batch")
	ctx.SetValue(schemas.UnifAIContextKeyMCPLogID, "mcp-batch-flow")
	ctx.SetValue(schemas.UnifAIContextKeyUserID, "user-1")
	ctx.SetValue(schemas.UnifAIContextKeyGovernanceTeamID, "team-1")
	ctx.SetValue(schemas.UnifAIContextKeyGovernanceCustomerID, "customer-1")
	ctx.SetValue(schemas.UnifAIContextKeyGovernanceBusinessUnitID, "bu-1")

	toolName := "docs-search"
	_, _, err = plugin.PreMCPHook(ctx, &schemas.UnifAIMCPRequest{
	...
	result := `{"answer":"done"}`
	_, _, err = plugin.PostMCPHook(ctx, &schemas.UnifAIMCPResponse{
...
func TestPostMCPHookFallbackStampsGovernanceFields(t *testing.T) {
	...
	ctx := schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline)
	ctx.SetValue(schemas.UnifAIContextKeyRequestID, "req-mcp-fallback")
	ctx.SetValue(schemas.UnifAIContextKeyMCPLogID, "mcp-fallback-flow")
	ctx.SetValue(schemas.UnifAIContextKeyUserID, "user-fallback")
	ctx.SetValue(schemas.UnifAIContextKeyGovernanceTeamID, "team-fallback")
	ctx.SetValue(schemas.UnifAIContextKeyGovernanceCustomerID, "customer-fallback")
	ctx.SetValue(schemas.UnifAIContextKeyGovernanceBusinessUnitID, "bu-fallback")
...
func TestPreMCPHookSkipsPrefixedCodemodeTool(t *testing.T) {
	...
	ctx := schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline)
	ctx.SetValue(schemas.UnifAIContextKeyRequestID, "req-prefixed-codemode")
	ctx.SetValue(schemas.UnifAIContextKeyMCPLogID, "mcp-prefixed-codemode")
...
func TestContentLoggingEnabledHelper(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			var ctx *schemas.UnifAIContext
			if tc.ctxOverride != nil {
				ctx = schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline)
				ctx.SetValue(schemas.UnifAIContextKeyAllowPerRequestStorageOverride, true)
				ctx.SetValue(schemas.UnifAIContextKeyDisableContentLogging, *tc.ctxOverride)
			}
...
func TestUpdateLogEntryPerRequestOverrideDisablesContent(t *testing.T) {
	...
	// Explicitly pass false — simulates x-uf-disable-content-logging: true on this request
	if err := plugin.updateLogEntry(context.Background(), requestID, "", "", 10, "", "", "", "", 0, nil, "", update, false); err != nil {
		t.Fatalf("updateLogEntry() error = %v", err)
	}
