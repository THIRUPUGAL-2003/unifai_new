...
// TestJsonParserPluginEndToEnd tests the integration of the JSON parser plugin with UnifAI.
// It performs the following steps:
// 1. Initializes the JSON parser plugin with AllRequests usage
// 2. Sets up a test UnifAI instance with the plugin
...
func TestJsonParserPluginEndToEnd(t *testing.T) {
	...
	// Initialize UnifAI with the plugin
		...
		Input: []schemas.ChatMessage{
			{
				...
				Content: &schemas.ChatMessageContent{
					ContentStr: unifai.Ptr("Return a JSON object with name, age, and city fields. Example: {\"name\": \"John\", \"age\": 30, \"city\": \"New York\"}"),
				},
			},
		},
	...
	responseChan, unifaiErr := client.ChatCompletionStreamRequest(ctx, request)

	if unifaiErr != nil {
		t.Fatalf("Error in UnifAI request: %v", unifaiErr)
	}
	...
	if responseChan != nil {
		...
		for streamResponse := range responseChan {
			...
			if streamResponse.UnifAIError != nil {
				t.Logf("Streaming response error: %v", streamResponse.UnifAIError)
			}

			if streamResponse.UnifAIChatResponse != nil {
				if streamResponse.UnifAIChatResponse.Choices != nil {
					for _, outputMsg := range streamResponse.UnifAIChatResponse.Choices {
...
func TestJsonParserPluginPerRequest(t *testing.T) {
		...
		Input: []schemas.ChatMessage{
			{
				Role: schemas.ChatMessageRoleUser,
				Content: &schemas.ChatMessageContent{
					ContentStr: unifai.Ptr("Return a JSON object with name and age fields."),
				},
			},
		},
		Params: &schemas.ChatParameters{
	...
	newContext := schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline).WithValue(EnableStreamingJSONParser, true)

	// Make the streaming request
	responseChan, unifaiErr := client.ChatCompletionStreamRequest(newContext, request)

	if unifaiErr != nil {
		t.Logf("Error in UnifAI request: %v", unifaiErr)
	}
	...
	if responseChan != nil {
		...
		for streamResponse := range responseChan {
			...
			if streamResponse.UnifAIChatResponse != nil {
				for _, choice := range streamResponse.UnifAIChatResponse.Choices {
...
// newResponsesStreamResponse builds a UnifAIResponse for a responses stream delta event.
func newResponsesStreamResponse(responseID, delta string) *schemas.UnifAIResponse {
	d := delta
	id := responseID
...
// newResponsesStreamNonDeltaResponse builds a UnifAIResponse for a non-delta responses stream event.
func newResponsesStreamNonDeltaResponse(responseID string, eventType schemas.ResponsesStreamResponseType) *schemas.UnifAIResponse {
	id := responseID
	return &schemas.UnifAIResponse{
		ResponsesStreamResponse: &schemas.UnifAIResponsesStreamResponse{
			Type: eventType,
...
func TestPostLLMHookResponsesStreamDelta(t *testing.T) {
	...
	for i, chunk := range chunks {
		...
		result, unifaiErr, hookErr := plugin.PostLLMHook(ctx, resp, nil)
		if hookErr != nil {
			t.Fatalf("chunk %d: unexpected error: %v", i, hookErr)
		}
		if unifaiErr != nil {
			t.Fatalf("chunk %d: unexpected unifai error: %v", i, unifaiErr)
		}
...
func TestPostLLMHookResponsesStreamNonDeltaPassthrough(t *testing.T) {
	...
	for _, eventType := range nonDeltaTypes {
		...
		if unifaiErr != nil {
			t.Fatalf("event %q: unexpected unifai error: %v", eventType, unifaiErr)
		}
		if result.ResponsesStreamResponse.Delta != nil {
...
func TestPostLLMHookResponsesStreamDoesNotMutateOriginal(t *testing.T) {
	...
	result, unifaiErr, hookErr := plugin.PostLLMHook(ctx, original, nil)
	if hookErr != nil {
		t.Fatalf("unexpected error: %v", hookErr)
	}
	if unifaiErr != nil {
		t.Fatalf("unexpected unifai error: %v", unifaiErr)
	}
...
func TestPostLLMHookResponsesStreamPerRequest(t *testing.T) {
	...
	ctxOff := schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline)
	result, unifaiErr, hookErr := plugin.PostLLMHook(ctxOff, resp, nil)
	if hookErr != nil {
		t.Fatalf("unexpected error (no-op path): %v", hookErr)
	}
	if unifaiErr != nil {
		t.Fatalf("unexpected unifai error (no-op path): %v", unifaiErr)
	}
	...
	ctxOn := schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline).
		WithValue(EnableStreamingJSONParser, true)
	result, unifaiErr, hookErr = plugin.PostLLMHook(ctxOn, resp, nil)
	if hookErr != nil {
		t.Fatalf("unexpected error (enabled path): %v", hookErr)
	}
	if unifaiErr != nil {
		t.Fatalf("unexpected unifai error (enabled path): %v", unifaiErr)
	}
	if !plugin.isValidJSON(*result.ResponsesStreamResponse.Delta) {
...
func TestJsonParserPluginResponsesStreamEndToEnd(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("failed to init unifai: %v", err)
	}
	defer client.Shutdown()
	...
	responseChan, unifaiErr := client.ResponsesStreamRequest(ctx, request)
	if unifaiErr != nil {
		t.Fatalf("request error: %v", unifaiErr)
	}

	chunkCount := 0
	for streamChunk := range responseChan {
		if streamChunk.UnifAIError != nil {
			t.Logf("stream error: %v", streamChunk.UnifAIError)
			continue
		}
		if streamChunk.UnifAIResponsesStreamResponse != nil {
			resp := streamChunk.UnifAIResponsesStreamResponse
