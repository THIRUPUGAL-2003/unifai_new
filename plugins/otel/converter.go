...
// from an x-uf-session-id value. Used to pin every request sharing a session into one
...
// from an x-uf-session-id value. It serves as the synthetic (never-emitted) parent of
...
func (p *OtelPlugin) convertTraceToResourceSpan(serviceName string, trace *schemas.Trace, requestHeaders []string, disableContentLogging bool, groupTracesBySession bool, disableRootSpanContent bool) *ResourceSpan {
	...
	// The x-uf-session-id header is a trace-level attribute, so it is not emitted as a span
	...
	// Session grouping: when enabled and this request carries an x-uf-session-id but no
