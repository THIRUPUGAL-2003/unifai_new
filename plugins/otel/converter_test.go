...
// x-uf-session-id attribute and an inbound traceparent parent on the root span.
func makeSessionTrace(traceID, sessionID, rootParentID string) *schemas.Trace {
	...
	if sessionID != "" {
		tr.Attributes = map[string]any{"x-uf-session-id": sessionID}
	}
	return tr
}
...
// TestNoSessionIDNoTag asserts that when no x-uf-session-id is present the root span carries no
