...
func TestMockerPlugin_Disabled(t *testing.T) {
	...
	_, unifaiErr := client.ChatCompletionRequest(ctx, &schemas.UnifAIChatRequest{
		...
		Input: []schemas.ChatMessage{
			{
				...
				Content: &schemas.ChatMessageContent{
					ContentStr: unifai.Ptr("Hello, test message"),
				},
			},
		},
	})
...
func TestMockerPlugin_DefaultMockRule(t *testing.T) {
	...
	response, unifaiErr := client.ChatCompletionRequest(ctx, &schemas.UnifAIChatRequest{
		Provider: schemas.OpenAI,
		Model:    "gpt-4",
		Input: []schemas.ChatMessage{
			{
	...
	if unifaiErr != nil {
		t.Fatalf("Expected no error, got: %v", unifaiErr)
	}
	if response == nil {
...
func TestMockerPlugin_ErrorResponse(t *testing.T) {
				...
				Responses: []Response{
					{
						...
						Error: &ErrorResponse{
							...
							Type:       unifai.Ptr("rate_limit"),
							Code:       unifai.Ptr("429"),
							StatusCode: unifai.Ptr(429),
						},
					},
				},
			},
	...
	if unifaiErr.Error.Message != "Rate limit exceeded" {
		t.Errorf("Expected 'Rate limit exceeded', got: %s", unifaiErr.Error.Message)
	}
	if unifaiErr.StatusCode == nil || *unifaiErr.StatusCode != 429 {
		t.Errorf("Expected status code 429, got: %v", unifaiErr.StatusCode)
	}
}
				...
				Responses: []Response{
					{
						Type: ResponseTypeSuccess,
						Content: &SuccessResponse{
							MessageTemplate: unifai.Ptr("Hello from {{provider}} using model {{model}}"),
						},
					},
				},
			},
...
func TestMockerPlugin_Statistics(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		_, _ = client.ChatCompletionRequest(ctx, &schemas.UnifAIChatRequest{
			Provider: schemas.OpenAI,
			Model:    "gpt-4",
			Input: []schemas.ChatMessage{
				{
