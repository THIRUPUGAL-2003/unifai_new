...
func BenchmarkMockerPlugin_PreHook_SimpleRule(b *testing.B) {
		...
		Input: []schemas.ChatMessage{
			{
				...
				Content: &schemas.ChatMessageContent{
					ContentStr: unifai.Ptr("Hello, benchmark test"),
				},
			},
		},
	}
	...
	// Convert to UnifAIRequest for PreLLMHook compatibility
	...
	for i := 0; i < b.N; i++ {
		_, _, _ = plugin.PreLLMHook(ctx, unifaiReq)
	}
}
		...
		Rules: []MockRule{
			{
				...
				Probability: 1.0,
				Conditions: Conditions{
					MessageRegex: unifai.Ptr(`(?i).*hello.*`),
				},
				Responses: []Response{
					{
						Type: ResponseTypeSuccess,
		...
		Input: []schemas.ChatMessage{
			{
				Role: schemas.ChatMessageRoleUser,
				Content: &schemas.ChatMessageContent{
					ContentStr: unifai.Ptr("Hello, this should match the regex pattern"),
				},
			},
		},
	}
