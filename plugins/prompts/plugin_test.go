...
func TestHTTPTransportPreHook_CaseInsensitiveHeaders(t *testing.T) {
	p := newTestPlugin(nil, nil, nil)
	ctx := bfCtx()
	// "X-Uf-Prompt-Id" is a title-case variant of the canonical "x-uf-prompt-id".
	req := &schemas.HTTPRequest{
		Headers: map[string]string{"X-Uf-Prompt-Id": "upper-case"},
	}
...
func TestChatMessageFromStoredJSON(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			// payload is a UnifAIChatResponse; message lives at choices[0].message.
...
// UnifAIChatRequest. The model chosen by the caller must always win.
...
// in the UnifAI UI's completion_result format (payload.choices[0].message) are
