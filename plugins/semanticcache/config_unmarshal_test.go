...
func TestUnmarshalJSON_BoolPointerFields(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expectCacheByModel: unifai.Ptr(true),
			expectCacheByProv:  unifai.Ptr(true),
			expectExcludeSys:   unifai.Ptr(true),
		},
		{
			name:               "all set to false",
			json:               `{"dimension": 1536, "cache_by_model": false, "cache_by_provider": false, "exclude_system_prompt": false}`,
			expectCacheByModel: unifai.Ptr(false),
			expectCacheByProv:  unifai.Ptr(false),
			expectExcludeSys:   unifai.Ptr(false),
		},
		{
			name:               "all omitted",
			json:               `{"dimension": 1536}`,
