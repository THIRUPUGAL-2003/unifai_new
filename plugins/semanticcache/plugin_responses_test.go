...
func TestResponsesAPIBasicFunctionality(t *testing.T) {
	...
	testRequest := CreateBasicResponsesRequest(
		"What is UnifAI? Answer in one short sentence.",
		0.7,
		500,
	)
	...
	// Verify cache hit
	AssertCacheHit(t, &schemas.UnifAIResponse{ResponsesResponse: response2}, string(CacheTypeDirect))
...
func TestResponsesAPIDifferentParameters(t *testing.T) {
	...
	tests := []struct {
		name        string
		request1    *schemas.UnifAIResponsesRequest
		request2    *schemas.UnifAIResponsesRequest
		shouldCache bool
	}{
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.shouldCache {
				AssertCacheHit(t, &schemas.UnifAIResponse{ResponsesResponse: response2}, "direct")
				t.Log("✓ Parameters match: cache hit as expected")
			} else {
				AssertNoCacheHit(t, &schemas.UnifAIResponse{ResponsesResponse: response2})
...
func TestResponsesAPISemanticMatching(t *testing.T) {
	...
	AssertNoCacheHit(t, &schemas.UnifAIResponse{ResponsesResponse: response1})
	WaitForCache(setup.Plugin)
	...
	AssertCacheHit(t, &schemas.UnifAIResponse{ResponsesResponse: response2}, "semantic")
...
func TestResponsesAPICacheExpiration(t *testing.T) {
	...
	AssertNoCacheHit(t, &schemas.UnifAIResponse{ResponsesResponse: response3})
...
func TestResponsesAPINoStoreFlag(t *testing.T) {
	...
	AssertNoCacheHit(t, &schemas.UnifAIResponse{ResponsesResponse: response2}) // Should not be cached
...
func TestResponsesAPIStreaming(t *testing.T) {
	...
	for streamMsg := range stream2 {
		...
		if streamMsg.UnifAIResponsesStreamResponse != nil {
			chunkCount2++
			if cd := streamMsg.UnifAIResponsesStreamResponse.ExtraFields.CacheDebug; cd != nil && cd.CacheHit {
				cacheHitFound = true
			}
		}
	}
...
func TestResponsesAPIComplexParameters(t *testing.T) {
	...
	serviceTier := schemas.UnifAIServiceTierDefault
