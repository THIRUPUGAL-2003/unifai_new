...
func TestImageGenerationCacheBasicFunctionality(t *testing.T) {
	...
	// Verify cache hit
	AssertCacheHit(t, &schemas.UnifAIResponse{ImageGenerationResponse: response2}, string(CacheTypeDirect))
...
func TestImageGenerationDifferentParameters(t *testing.T) {
	...
	AssertNoCacheHit(t, &schemas.UnifAIResponse{ImageGenerationResponse: response2})
	...
	AssertNoCacheHit(t, &schemas.UnifAIResponse{ImageGenerationResponse: response3})
...
func TestImageGenerationStreamCaching(t *testing.T) {
	...
	var responses1 []schemas.UnifAIImageGenerationStreamResponse
	for streamMsg := range stream1 {
		...
		if streamMsg.UnifAIImageGenerationStreamResponse != nil {
			responses1 = append(responses1, *streamMsg.UnifAIImageGenerationStreamResponse)
		}
	}
	duration1 := time.Since(start1)
	...
	var responses2 []schemas.UnifAIImageGenerationStreamResponse
	for streamMsg := range stream2 {
		if streamMsg.UnifAIError != nil {
			t.Fatalf("Error in second stream: %v", streamMsg.UnifAIError)
		}
		if streamMsg.UnifAIImageGenerationStreamResponse != nil {
			responses2 = append(responses2, *streamMsg.UnifAIImageGenerationStreamResponse)
		}
	}
	duration2 := time.Since(start2)
