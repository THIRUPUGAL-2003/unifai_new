...
func TestPostLLMHook_SkipsOnUnifAIError(t *testing.T) {
	...
	// Pass a non-nil unifai error to PostLLMHook.
	...
	res := &schemas.UnifAIResponse{
		ChatResponse: &schemas.UnifAIChatResponse{
			ExtraFields: schemas.UnifAIResponseExtraFields{RequestType: schemas.ChatCompletionRequest},
		},
	}
	if _, _, err := plugin.PostLLMHook(ctx, res, unifaiErr); err != nil {
...
func TestInit_RejectsNilConfig(t *testing.T) {
	if _, err := Init(context.Background(), nil, unifai.NewDefaultLogger(schemas.LogLevelError), newObservableStore()); err == nil {
...
func TestInit_RejectsNilStore(t *testing.T) {
	...
	if _, err := Init(context.Background(), cfg, unifai.NewDefaultLogger(schemas.LogLevelError), nil); err == nil {
...
func TestInit_RejectsNegativeDimension(t *testing.T) {
	...
	if _, err := Init(context.Background(), cfg, unifai.NewDefaultLogger(schemas.LogLevelError), newObservableStore()); err == nil || !strings.Contains(err.Error(), "dimension") {
...
func TestInit_AllowsDirectOnlyMode(t *testing.T) {
	...
	cfg := &Config{Dimension: 1}
	plugin, err := Init(context.Background(), cfg, unifai.NewDefaultLogger(schemas.LogLevelError), newObservableStore())
	if err != nil {
...
func TestExpiredEntry_DetectedAndDeleted(t *testing.T) {
	...
	chunkJSON, _ := json.Marshal(&schemas.UnifAIResponse{
		ChatResponse: &schemas.UnifAIChatResponse{},
	})
...
// scopedTestContext returns a plugin-scoped UnifAIContext so ctx.Log entries
...
func scopedTestContext(t testing.TB, suffix string) *schemas.UnifAIContext {
	t.Helper()
...
func TestPreLLMHook_EmitsPluginLogOnEmbeddingFailure(t *testing.T) {
	store := newObservableStore()
	plugin := newTestPlugin(t, store)
	plugin.SetEmbeddingRequestExecutor(func(_ *schemas.UnifAIContext, _ *schemas.UnifAIEmbeddingRequest) (*schemas.UnifAIEmbeddingResponse, *schemas.UnifAIError) {
		return nil, &schemas.UnifAIError{Error: &schemas.ErrorField{Message: "rate limit exceeded"}}
	})
