...
func TestCacheTypeDirectOnly(t *testing.T) {
	...
	testRequest := CreateBasicChatRequest("What is UnifAI?", 0.7, 50)
	...
	AssertNoCacheHit(t, &schemas.UnifAIResponse{ChatResponse: response1})
	...
	AssertCacheHit(t, &schemas.UnifAIResponse{ChatResponse: response2}, "direct")
...
func TestCacheTypeSemanticOnly(t *testing.T) {
	...
	if response2.ExtraFields.CacheDebug.CacheHit {
		AssertCacheHit(t, &schemas.UnifAIResponse{ChatResponse: response2}, "semantic")
		...
		AssertNoCacheHit(t, &schemas.UnifAIResponse{ChatResponse: response2})
	}
...
func TestCacheTypeDirectWithSemanticFallback(t *testing.T) {
	...
	if response3.ExtraFields.CacheDebug.CacheHit {
		AssertCacheHit(t, &schemas.UnifAIResponse{ChatResponse: response3}, "semantic")
		t.Log("✅ Default behavior correctly found semantic match")
	} else {
		t.Log("ℹ️  No semantic match found (normal for different wording)")
		AssertNoCacheHit(t, &schemas.UnifAIResponse{ChatResponse: response3})
	}
...
func TestCacheTypeInvalidValue(t *testing.T) {
	...
	AssertCacheHit(t, &schemas.UnifAIResponse{ChatResponse: response2}, string(CacheTypeDirect))
...
func TestCacheTypeWithEmbeddingRequests(t *testing.T) {
	...
	AssertNoCacheHit(t, &schemas.UnifAIResponse{EmbeddingResponse: response1})
	...
	AssertCacheHit(t, &schemas.UnifAIResponse{EmbeddingResponse: response2}, "direct")
	...
	AssertNoCacheHit(t, &schemas.UnifAIResponse{EmbeddingResponse: response3})
...
func TestCacheTypePerformanceCharacteristics(t *testing.T) {
	...
	AssertCacheHit(t, &schemas.UnifAIResponse{ChatResponse: response3}, "direct")
...
func newCrossProviderChatRequest(provider schemas.ModelProvider, model string, requestType schemas.RequestType, prompt string) *schemas.UnifAIRequest {
...
func TestDirectCacheHitPreservesCachedProviderMetadataAcrossProviders(t *testing.T) {
	...
	config.CacheByProvider = unifai.Ptr(false)
	config.CacheByModel = unifai.Ptr(false)
	...
	seedResponse := &schemas.UnifAIResponse{
			...
			Choices: []schemas.UnifAIResponseChoice{
				{
					ChatNonStreamResponseChoice: &schemas.ChatNonStreamResponseChoice{
						Message: &schemas.ChatMessage{
							...
							Content: &schemas.ChatMessageContent{
								ContentStr: unifai.Ptr("Go schedules lightweight goroutines in user space onto a smaller pool of OS threads."),
							},
						},
					},
				},
...
func TestStreamingDirectCacheHitPreservesCachedProviderMetadataAcrossProviders(t *testing.T) {
	...
	chunks := []struct {
		...
		{content: " onto a smaller pool of OS threads.", chunkIndex: 1, finishReason: unifai.Ptr("stop"), streamEnd: true},
	}

	for _, chunk := range chunks {
		seedCtx.SetValue(schemas.UnifAIContextKeyStreamEndIndicator, chunk.streamEnd)
		chunkResponse := &schemas.UnifAIResponse{
			ChatResponse: &schemas.UnifAIChatResponse{
				...
				Choices: []schemas.UnifAIResponseChoice{
					{
						...
						ChatStreamResponseChoice: &schemas.ChatStreamResponseChoice{
							Delta: &schemas.ChatStreamResponseChoiceDelta{
								Content: unifai.Ptr(chunk.content),
							},
						},
					},
				},
	...
	for chunk := range shortCircuit.Stream {
		...
		extraFields := chunk.UnifAIChatResponse.ExtraFields
		if extraFields.Provider != schemas.OpenAI {
...
func runDirectSearchForTest(t *testing.T, plugin *Plugin, ctx *schemas.UnifAIContext, req *schemas.UnifAIRequest, cacheKey string) (*cacheState, *schemas.LLMPluginShortCircuit, error) {
	t.Helper()
...
func TestCacheTypeDirectUsesChunkLookup(t *testing.T) {
	...
	req := &schemas.UnifAIRequest{
		RequestType: schemas.ChatCompletionRequest,
		ChatRequest: CreateBasicChatRequest("What is UnifAI?", 0.7, 50),
	}
	...
	cachedResponse := &schemas.UnifAIResponse{
		ChatResponse: &schemas.UnifAIChatResponse{
			Choices: []schemas.UnifAIResponseChoice{
				{
					ChatNonStreamResponseChoice: &schemas.ChatNonStreamResponseChoice{
