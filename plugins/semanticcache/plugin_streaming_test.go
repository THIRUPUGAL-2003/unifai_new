...
func TestStreamingCacheBasicFunctionality(t *testing.T) {
	...
	var responses1 []schemas.UnifAIChatResponse
	for streamMsg := range stream1 {
		if streamMsg.UnifAIError != nil {
			t.Fatalf("Error in first stream: %v", streamMsg.UnifAIError)
		}
		if streamMsg.UnifAIChatResponse != nil {
			responses1 = append(responses1, *streamMsg.UnifAIChatResponse)
		}
	}
	...
	var responses2 []schemas.UnifAIChatResponse
	for streamMsg := range stream2 {
		if streamMsg.UnifAIError != nil {
			t.Fatalf("Error in second stream: %v", streamMsg.UnifAIError)
		}
		if streamMsg.UnifAIChatResponse != nil {
			responses2 = append(responses2, *streamMsg.UnifAIChatResponse)
		}
	}
...
func TestStreamingVsNonStreaming(t *testing.T) {
	...
	var streamResponses []schemas.UnifAIChatResponse
	for streamMsg := range stream {
		if streamMsg.UnifAIError != nil {
			t.Fatalf("Error in stream: %v", streamMsg.UnifAIError)
		}
		if streamMsg.UnifAIChatResponse != nil {
			streamResponses = append(streamResponses, *streamMsg.UnifAIChatResponse)
		}
	}
	...
	for _, response := range streamResponses {
		if response.ExtraFields.RawResponse != nil {
			if rawMap, ok := response.ExtraFields.RawResponse.(map[string]interface{}); ok {
				if cachedFlag, exists := rawMap["unifai_cached"]; exists {
	...
	AssertNoCacheHit(t, &schemas.UnifAIResponse{ChatResponse: nonStreamResponse})
...
func TestStreamingChunkOrdering(t *testing.T) {
	...
	var originalChunks []schemas.UnifAIChatResponse
	for streamMsg := range stream1 {
		if streamMsg.UnifAIError != nil {
			t.Fatalf("Error in first stream: %v", streamMsg.UnifAIError)
		}
		if streamMsg.UnifAIChatResponse != nil {
			originalChunks = append(originalChunks, *streamMsg.UnifAIChatResponse)
		}
	}
	...
	var cachedChunks []schemas.UnifAIChatResponse
	for streamMsg := range stream2 {
		if streamMsg.UnifAIError != nil {
			t.Fatalf("Error in second stream: %v", streamMsg.UnifAIError)
		}
		if streamMsg.UnifAIChatResponse != nil {
			cachedChunks = append(cachedChunks, *streamMsg.UnifAIChatResponse)
		}
	}
	...
	for i := 0; i < len(cachedChunks) && i < len(originalChunks); i++ {
		...
		if i == len(cachedChunks)-1 {
			AssertCacheHit(t, &schemas.UnifAIResponse{ChatResponse: &cachedChunks[i]}, string(CacheTypeDirect))
		}
	}
...
func TestSpeechSynthesisStreaming(t *testing.T) {
	...
	AssertCacheHit(t, &schemas.UnifAIResponse{SpeechResponse: response2}, string(CacheTypeDirect))
