...
func (plugin *Plugin) buildRequestMetadataForCaching(state *cacheState, req *schemas.UnifAIRequest) (map[string]interface{}, error) {
	metadata := map[string]interface{}{
		"stream": unifai.IsStreamRequestType(req.RequestType),
	}
...
func (plugin *Plugin) extractAttachmentsForCaching(state *cacheState, req *schemas.UnifAIRequest) []string {
	switch req.RequestType {
	case schemas.ChatCompletionRequest, schemas.ChatCompletionStreamRequest:
...
func (plugin *Plugin) extractTextForEmbedding(state *cacheState, req *schemas.UnifAIRequest) (string, error) {
	switch {
...
// provider, model, params_hash, expires_at) plus the from_unifai marker
...
func (plugin *Plugin) buildUnifiedMetadata(provider schemas.ModelProvider, model string, paramsHash string, cacheKey string, ttl time.Duration) map[string]interface{} {
	...
	unifiedMetadata["from_unifai_semantic_cache_plugin"] = true
...
func (plugin *Plugin) addNonStreamingResponse(ctx context.Context, responseID string, res *schemas.UnifAIResponse, embedding []float32, metadata map[string]interface{}, ttl time.Duration) error {
...
// Errors never reach this function: PostLLMHook returns early on unifaiErr
...
func (plugin *Plugin) addStreamingResponse(ctx context.Context, requestID string, storageID string, res *schemas.UnifAIResponse, embedding []float32, metadata map[string]interface{}, ttl time.Duration, isFinalChunk bool) error {
...
func (plugin *Plugin) getInputForCaching(state *cacheState, req *schemas.UnifAIRequest) interface{} {
...
func (plugin *Plugin) getNormalizedInputForCaching(req *schemas.UnifAIRequest) interface{} {
	excludeSystem := plugin.config.ExcludeSystemPrompt != nil && *plugin.config.ExcludeSystemPrompt
	switch req.RequestType {
	case schemas.TextCompletionRequest, schemas.TextCompletionStreamRequest:
...
func (plugin *Plugin) isConversationHistoryThresholdExceeded(state *cacheState, req *schemas.UnifAIRequest) bool {
	switch {
	case req.ChatRequest != nil:
