...
type requestCapturer struct {
	mu       sync.Mutex
	captured *schemas.UnifAIRequest
}

func (p *requestCapturer) GetName() string { return "test-request-capturer" }
func (p *requestCapturer) Cleanup() error  { return nil }

func (p *requestCapturer) PreRequestHook(_ *schemas.UnifAIContext, _ *schemas.UnifAIRequest) error {
	return nil
}

func (p *requestCapturer) PreLLMHook(ctx *schemas.UnifAIContext, req *schemas.UnifAIRequest) (*schemas.UnifAIRequest, *schemas.LLMPluginShortCircuit, error) {
	p.mu.Lock()
	...
	if err == nil {
		var snapshot schemas.UnifAIRequest
...
func (p *requestCapturer) PostLLMHook(_ *schemas.UnifAIContext, resp *schemas.UnifAIResponse, e *schemas.UnifAIError) (*schemas.UnifAIResponse, *schemas.UnifAIError, error) {
	return resp, e, nil
}
...
func TestCachingDoesNotMutateRequestSentToProvider(t *testing.T) {
	...
	cachePlugin, err := Init(schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline), cfg, logger, store)
	if err != nil {
	...
	bctx := schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline)
	client, err := unifai.Init(bctx, schemas.UnifAIConfig{
		Account: &BaseAccount{},
		...
		Input: []schemas.ChatMessage{
			{
				...
				Content: &schemas.ChatMessageContent{
					ContentStr: unifai.Ptr(systemContent),
				},
			},
			{
				Role: schemas.ChatMessageRoleUser,
				Content: &schemas.ChatMessageContent{
					ContentStr: unifai.Ptr(userContent),
				},
			},
		},
		Params: &schemas.ChatParameters{
			Temperature:         unifai.Ptr(0.0),
			MaxCompletionTokens: unifai.Ptr(5),
		},
	}
