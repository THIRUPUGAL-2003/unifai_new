...
func TestParameterVariations(t *testing.T) {
	...
	tests := []struct {
		name        string
		request1    *schemas.UnifAIChatRequest
		request2    *schemas.UnifAIChatRequest
		shouldCache bool
	}{
		{
...
func TestToolVariations(t *testing.T) {
	...
	baseRequest := &schemas.UnifAIChatRequest{
		...
		Input: []schemas.ChatMessage{
			{
				...
				Content: &schemas.ChatMessageContent{
					ContentStr: unifai.Ptr("What's the weather like today?"),
				},
			},
		},
	...
	// Request with tools
	requestWithTools := &schemas.UnifAIChatRequest{
		Provider: schemas.OpenAI,
		Model:    "gpt-4o-mini",
		Input: []schemas.ChatMessage{
			{
			...
			Tools: []schemas.ChatTool{
				{
					...
					Function: &schemas.ChatToolFunction{
						Name:        "get_weather",
						Description: unifai.Ptr("Get the current weather"),
	...
	requestWithDifferentTools := &schemas.UnifAIChatRequest{
		Provider: schemas.OpenAI,
		Model:    "gpt-4o-mini",
		Input: []schemas.ChatMessage{
			{
			...
			Tools: []schemas.ChatTool{
				{
					...
					Function: &schemas.ChatToolFunction{
						...
						Description: unifai.Ptr("Get current weather information"),
						Parameters: &schemas.ToolFunctionParameters{
							Type: "object",
							Properties: schemas.NewOrderedMapFromPairs(
	...
	AssertCacheHit(t, &schemas.UnifAIResponse{ChatResponse: response3}, "")
							...
							ContentBlocks: []schemas.ChatContentBlock{
								{
									...
									Text: unifai.Ptr("Analyze this image"),
								},
								{
				...
				Input: []schemas.ChatMessage{
					{
						...
						Content: &schemas.ChatMessageContent{
							ContentBlocks: []schemas.ChatContentBlock{
								{
									Type: schemas.ChatContentBlockTypeText,
									Text: unifai.Ptr("Compare these images"),
								},
								{
									Type: schemas.ChatContentBlockTypeImage,
									ImageURLStruct: &schemas.ChatInputImage{
				...
				Input: []schemas.ChatMessage{
					{
						Role: schemas.ChatMessageRoleUser,
						Content: &schemas.ChatMessageContent{
							ContentStr: unifai.Ptr(strings.Repeat("This is a very long prompt. ", 100)),
						},
					},
				},
				Params: &schemas.ChatParameters{
					MaxCompletionTokens: unifai.Ptr(50),
					Temperature:         unifai.Ptr(0.2),
				},
			},
		},
		{
				...
				Input: []schemas.ChatMessage{
					{
						Role: schemas.ChatMessageRoleUser,
						Content: &schemas.ChatMessageContent{
							ContentStr: unifai.Ptr("What is AI?"),
						},
					},
					{
						Role: schemas.ChatMessageRoleAssistant,
						Content: &schemas.ChatMessageContent{
							ContentStr: unifai.Ptr("AI stands for Artificial Intelligence..."),
						},
					},
					{
						Role: schemas.ChatMessageRoleUser,
						Content: &schemas.ChatMessageContent{
							ContentStr: unifai.Ptr("Can you give me examples?"),
						},
					},
				},
				Params: &schemas.ChatParameters{
				...
				Input: []schemas.ChatMessage{
					{
						Role: schemas.ChatMessageRoleUser,
						Content: &schemas.ChatMessageContent{
							ContentStr: unifai.Ptr("Test max parameters"),
						},
					},
				},
				Params: &schemas.ChatParameters{
					MaxCompletionTokens: unifai.Ptr(4096),
					PresencePenalty:     unifai.Ptr(2.0),
					FrequencyPenalty:    unifai.Ptr(2.0),
					Temperature:         unifai.Ptr(2.0),
					TopP:                unifai.Ptr(1.0),
				},
			},
		},
				...
				Input: []schemas.ChatMessage{
					{
						Role: schemas.ChatMessageRoleUser,
						Content: &schemas.ChatMessageContent{
							ContentStr: unifai.Ptr("Test min parameters"),
						},
					},
				},
				Params: &schemas.ChatParameters{
					MaxCompletionTokens: unifai.Ptr(1),
					PresencePenalty:     unifai.Ptr(-2.0),
					FrequencyPenalty:    unifai.Ptr(-2.0),
					Temperature:         unifai.Ptr(0.0),
					TopP:                unifai.Ptr(0.01),
				},
			},
		},
		{
				...
				Input: []schemas.ChatMessage{
					{
						Role: schemas.ChatMessageRoleUser,
						Content: &schemas.ChatMessageContent{
							ContentStr: unifai.Ptr("Test edge case parameters"),
						},
					},
				},
				Params: &schemas.ChatParameters{
					MaxCompletionTokens: unifai.Ptr(1),
					User:                unifai.Ptr("test-user-id-12345"),
					Temperature:         unifai.Ptr(0.0),
					TopP:                unifai.Ptr(0.1),
				},
			},
...
func TestSemanticSimilarityEdgeCases(t *testing.T) {
	...
	similarTests := []struct {
		...
		},
		{
			...
			prompt2:     "What do you know about unifai?",
			shouldMatch: false,
...
func TestErrorHandlingEdgeCases(t *testing.T) {
	...
	t.Run("Request with invalid cache key type", func(t *testing.T) {
		...
		AssertNoCacheHit(t, &schemas.UnifAIResponse{ChatResponse: response})
