			...
			Input: []schemas.ChatMessage{
				{
					...
					Content: &schemas.ChatMessageContent{
						ContentStr: unifai.Ptr("Hello, world!"),
					},
				},
			},
			...
			Choices: []schemas.UnifAIResponseChoice{
				{
					...
					ChatNonStreamResponseChoice: &schemas.ChatNonStreamResponseChoice{
						Message: &schemas.ChatMessage{
							...
							Content: &schemas.ChatMessageContent{
								ContentStr: unifai.Ptr("Hello! How can I help you today?"),
							}},
					},
				},
			},
...
func TestSemanticCacheStrictFiltering(t *testing.T) {
	...
	// Base request
	baseRequest := &schemas.UnifAIRequest{
		RequestType: schemas.ChatCompletionRequest,
		ChatRequest: &schemas.UnifAIChatRequest{
			Provider: schemas.OpenAI,
			Model:    "gpt-4o-mini",
			Input: []schemas.ChatMessage{
				{
					Role: schemas.ChatMessageRoleUser,
					Content: &schemas.ChatMessageContent{
						ContentStr: unifai.Ptr("What is the weather like?"),
					},
				},
			},
			Params: &schemas.ChatParameters{
			...
			Choices: []schemas.UnifAIResponseChoice{
				{
					ChatNonStreamResponseChoice: &schemas.ChatNonStreamResponseChoice{
						Message: &schemas.ChatMessage{
							Role: schemas.ChatMessageRoleAssistant,
							Content: &schemas.ChatMessageContent{
								ContentStr: unifai.Ptr("It's sunny today!"),
							}},
					},
				},
			},
	...
	modifiedRequest := &schemas.UnifAIRequest{
		RequestType: schemas.ChatCompletionRequest,
		ChatRequest: &schemas.UnifAIChatRequest{
			Provider: schemas.OpenAI,
			Model:    "gpt-4o-mini",
			...
			Params: &schemas.ChatParameters{
				Temperature:         unifai.Ptr(0.5), // Different temperature
				MaxCompletionTokens: unifai.Ptr(100),
			},
		},
	}
	...
	modifiedRequest2 := &schemas.UnifAIRequest{
		RequestType: schemas.ChatCompletionRequest,
		ChatRequest: &schemas.UnifAIChatRequest{
			Provider: schemas.OpenAI,
...
func TestSemanticCacheStreamingFlow(t *testing.T) {
			...
			Input: []schemas.ChatMessage{
				{
					Role: schemas.ChatMessageRoleUser,
					Content: &schemas.ChatMessageContent{
						ContentStr: unifai.Ptr("Tell me a short story"),
					},
				},
			},
			Params: &schemas.ChatParameters{
				Temperature: unifai.Ptr(0.8),
			},
		},
	}
	...
	for i, chunk := range chunks {
		...
		if isFinal {
			finishReason = unifai.Ptr("stop")
		}

		// UnifAI's stream pipeline sets this on the final chunk before
		// invoking PostLLMHook (see core/unifai.go where it stamps
		// UnifAIContextKeyStreamEndIndicator=true). The cache plugin's
		...
		// request misses.
		ctx.SetValue(schemas.UnifAIContextKeyStreamEndIndicator, isFinal)
			...
			Input: []schemas.ChatMessage{
				{
					Role: schemas.ChatMessageRoleUser,
					Content: &schemas.ChatMessageContent{
						ContentStr: unifai.Ptr("Test message"),
					},
				},
			},
		},
			...
			Input: []schemas.ChatMessage{
				{
					Role: schemas.ChatMessageRoleUser,
					Content: &schemas.ChatMessageContent{
						ContentStr: unifai.Ptr("TTL test message"),
					},
				},
			},
		},
			...
			Choices: []schemas.UnifAIResponseChoice{
				{
					ChatNonStreamResponseChoice: &schemas.ChatNonStreamResponseChoice{
						Message: &schemas.ChatMessage{
							Role: "assistant",
							Content: &schemas.ChatMessageContent{
								ContentStr: unifai.Ptr("TTL test response"),
							},
						},
					},
				},
...
func TestSemanticCache_CustomThresholdHandling(t *testing.T) {
			...
			Input: []schemas.ChatMessage{
				{
					Role: schemas.ChatMessageRoleUser,
					Content: &schemas.ChatMessageContent{
						ContentStr: unifai.Ptr("Threshold test message"),
					},
				},
			},
		},
	...
	seedRes := &schemas.UnifAIResponse{
		ChatResponse: &schemas.UnifAIChatResponse{
			ID: "threshold-test",
			Choices: []schemas.UnifAIResponseChoice{{
				ChatNonStreamResponseChoice: &schemas.ChatNonStreamResponseChoice{
					Message: &schemas.ChatMessage{
						Role:    "assistant",
						Content: &schemas.ChatMessageContent{ContentStr: unifai.Ptr("seed response")},
					},
				},
			}},
			ExtraFields: schemas.UnifAIResponseExtraFields{
...
func TestSemanticCache_ProviderModelCachingFlags(t *testing.T) {
	...
	setup.Config.CacheByProvider = unifai.Ptr(false)
	setup.Config.CacheByModel = unifai.Ptr(false)

	ctx := newBaseTestContext()
	ctx.SetValue(CacheKey, keyForTest(t, "test-cache-enabled"))

	request1 := &schemas.UnifAIRequest{
		RequestType: schemas.ChatCompletionRequest,
		ChatRequest: &schemas.UnifAIChatRequest{
			Provider: schemas.OpenAI,
			Model:    "gpt-4o-mini",
			Input: []schemas.ChatMessage{
				{
					Role: schemas.ChatMessageRoleUser,
					Content: &schemas.ChatMessageContent{
						ContentStr: unifai.Ptr("Provider model flags test"),
					},
				},
			},
		},
			...
			Choices: []schemas.UnifAIResponseChoice{
				{
					ChatNonStreamResponseChoice: &schemas.ChatNonStreamResponseChoice{
						Message: &schemas.ChatMessage{
							Role: "assistant",
							Content: &schemas.ChatMessageContent{
								ContentStr: unifai.Ptr("Provider model test response"),
							},
						},
					},
				},
	...
	request2 := &schemas.UnifAIRequest{
		RequestType: schemas.ChatCompletionRequest,
		ChatRequest: &schemas.UnifAIChatRequest{
			...
			Input: []schemas.ChatMessage{
				{
					Role: schemas.ChatMessageRoleUser,
					Content: &schemas.ChatMessageContent{
						ContentStr: unifai.Ptr("Provider model flags test"), // Same content
					},
				},
			},
		},
...
func TestSemanticCache_ConfigurationEdgeCases(t *testing.T) {
			...
			Input: []schemas.ChatMessage{
				{
					Role: schemas.ChatMessageRoleUser,
					Content: &schemas.ChatMessageContent{
						ContentStr: unifai.Ptr("Edge case test"),
					},
				},
			},
		},
	...
	res := &schemas.UnifAIResponse{
		ChatResponse: &schemas.UnifAIChatResponse{
			ID: "edge-ttl",
			Choices: []schemas.UnifAIResponseChoice{{
				ChatNonStreamResponseChoice: &schemas.ChatNonStreamResponseChoice{
					Message: &schemas.ChatMessage{Role: "assistant", Content: &schemas.ChatMessageContent{ContentStr: unifai.Ptr("ok")}},
				},
			}},
			ExtraFields: schemas.UnifAIResponseExtraFields{Provider: schemas.OpenAI, OriginalModelRequested: "gpt-4o-mini", RequestType: schemas.ChatCompletionRequest},
		},
	}
