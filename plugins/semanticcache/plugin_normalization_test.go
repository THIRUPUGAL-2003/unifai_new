...
func testChatCompletionNormalization(t *testing.T, setup *TestSetup) {
	...
	requests := make([]*schemas.UnifAIChatRequest, len(testCases))
	for i, tc := range testCases {
		requests[i] = &schemas.UnifAIChatRequest{
	...
	for i := 1; i < len(testCases); i++ {
		...
		AssertCacheHit(t, &schemas.UnifAIResponse{ChatResponse: response}, "direct")
...
func testSpeechNormalization(t *testing.T, setup *TestSetup) {
	...
	requests := make([]*schemas.UnifAISpeechRequest, len(testCases))
	for i, tc := range testCases {
	...
	AssertNoCacheHit(t, &schemas.UnifAIResponse{SpeechResponse: response1})
	WaitForCache(setup.Plugin)
	...
	for i := 1; i < len(testCases); i++ {
		...
		// Should be cache hit due to normalization
		AssertCacheHit(t, &schemas.UnifAIResponse{SpeechResponse: response}, "direct")
		t.Logf("✓ Cache hit for '%s' variation", tc.name)
	}
}
