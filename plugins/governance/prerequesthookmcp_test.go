...
// x-uf-mcp-include-tools list is pruned against the virtual key's tool grant, and when
...
const mcpTestVKValue = "sk-uf-mcp-test"
...
// mirroring what lib/ctx.go stamps from the x-uf-mcp-include-tools and
// x-uf-mcp-include-clients request headers.
func newPreRequestCtx(includeTools, includeClients []string) *schemas.UnifAIContext {
	ctx := schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline)
	ctx.SetValue(schemas.UnifAIContextKeyVirtualKey, mcpTestVKValue)
	if includeTools != nil {
...
func newChatRequest() *schemas.UnifAIRequest {
...
func stampedIncludeTools(t *testing.T, p *GovernancePlugin, ctx *schemas.UnifAIContext, req *schemas.UnifAIRequest) []string {
	t.Helper()
...
// An empty x-uf-mcp-include-tools header value reaches ctx as [""] (see lib/ctx.go).
...
func TestPreRequestHookMCP_PassthroughRequest_NoStamping(t *testing.T) {
	p := newPluginForMCPStamping(t, buildVKForMCPStamping([]string{"tool_a"}), false)
	ctx := newPreRequestCtx(nil, []string{"sentry"})
	req := &schemas.UnifAIRequest{RequestType: schemas.PassthroughRequest}
...
func newLargePayloadCtx(includeTools, includeClients []string) (*schemas.UnifAIContext, *schemas.LargePayloadMetadata) {
	ctx := newPreRequestCtx(includeTools, includeClients)
	metadata := &schemas.LargePayloadMetadata{Model: "openai/gpt-4o"}
	ctx.SetValue(schemas.UnifAIContextKeyLargePayloadMetadata, metadata)
	return ctx, metadata
}
