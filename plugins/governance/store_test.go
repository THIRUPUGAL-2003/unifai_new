...
func TestGovernanceStore_GetVirtualKey(t *testing.T) {
	...
	store, err := NewLocalGovernanceStore(context.Background(), logger, nil, &configstore.GovernanceConfig{
		VirtualKeys: []configstoreTables.TableVirtualKey{
			*buildVirtualKey("vk1", "sk-uf-test1", "Test VK 1", true),
			*buildVirtualKey("vk2", "sk-uf-test2", "Test VK 2", false),
		},
	}, nil)
...
func TestGovernanceStore_ConcurrentReads(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		...
		go func() {
			defer wg.Done()
			for j := 0; j < 100; j++ {
				vk, exists := store.GetVirtualKey(context.Background(), "sk-uf-test")
				if !exists || vk == nil {
					errorCount.Add(1)
					return
				}
...
func TestGovernanceStore_CheckBudget_SingleBudget(t *testing.T) {
	...
	vk := buildVirtualKeyWithBudget("vk1", "sk-uf-test", "Test VK", budget)
	...
	vk, _ = store.GetVirtualKey(context.Background(), "sk-uf-test")
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			testVK := buildVirtualKeyWithBudget("vk1", "sk-uf-test", "Test VK", testBudget)
			...
			testVK, _ = testStore.GetVirtualKey(context.Background(), "sk-uf-test")
...
func TestGovernanceStore_MultiBudget_AllUnderLimit(t *testing.T) {
	...
	vk := buildVirtualKeyWithMultiBudgets("vk1", "sk-uf-test", "Test VK",
...
func TestGovernanceStore_MultiBudget_ProviderConfigBudgets(t *testing.T) {
	...
	vk := buildVirtualKeyWithProviders("vk1", "sk-uf-test", "Test VK",
...
func TestGovernanceStore_MultiBudget_ResolverBlocksOnBudgetExceeded(t *testing.T) {
	...
	result := resolver.EvaluateVirtualKeyRequest(ctx, "sk-uf-test", schemas.OpenAI, "gpt-4", schemas.ChatCompletionRequest, false)
...
func TestGovernanceStore_MultiBudget_InMemoryCreateAndDelete(t *testing.T) {
	...
	retrieved, found := store.GetVirtualKey(context.Background(), "sk-uf-test")
	...
	_, found = store.GetVirtualKey(context.Background(), "sk-uf-test")
...
func TestGovernanceStore_CreateVirtualKeyInMemory_DecouplesFromCallerPointer(t *testing.T) {
	...
	tracked, found := store.GetVirtualKey(context.Background(), "sk-uf-test")
	require.True(t, found)
...
func TestGovernanceStore_UpdateVirtualKeyInMemory_RotatedValueRemovesOldLookup(t *testing.T) {
	...
	vk := buildVirtualKeyWithBudget("vk1", "sk-uf-old", "Test VK", budget)
	...
	updated := *vk
	updated.Value = *schemas.NewSecretVar("sk-uf-new")
	store.UpdateVirtualKeyInMemory(context.Background(), &updated, nil, nil, nil)

	oldVK, oldFound := store.GetVirtualKey(context.Background(), "sk-uf-old")
	assert.False(t, oldFound)
	assert.Nil(t, oldVK)

	newVK, newFound := store.GetVirtualKey(context.Background(), "sk-uf-new")
...
func TestGovernanceStore_ResetExpiredRateLimits(t *testing.T) {
	...
	updatedVK, _ := store.GetVirtualKey(context.Background(), "sk-uf-test")
...
func TestGovernanceStore_RoutingRules_CreateAndRetrieve(t *testing.T) {
	...
	rule1 := &configstoreTables.TableRoutingRule{
		...
		Enabled:       unifai.Ptr(true),
		CelExpression: "model == 'gpt-4o'",
		Targets: []configstoreTables.TableRoutingTarget{
			{Provider: unifai.Ptr("openai"), Model: unifai.Ptr("gpt-4"), Weight: 1.0},
		},
		Fallbacks:       nil,
	...
	rule2 := &configstoreTables.TableRoutingRule{
		...
		Targets: []configstoreTables.TableRoutingTarget{
			{Provider: unifai.Ptr("azure"), Weight: 1.0},
		},
		Fallbacks:       nil,
...
func TestGovernanceStore_RoutingRules_MultipleScopes(t *testing.T) {
	...
	globalRule := &configstoreTables.TableRoutingRule{
		ID: "1", Name: "Global", Scope: "global", ScopeID: nil, Priority: 10, Enabled: unifai.Ptr(true),
	}
	customerRule := &configstoreTables.TableRoutingRule{
		ID: "2", Name: "Customer", Scope: "customer", ScopeID: &customerID, Priority: 20, Enabled: unifai.Ptr(true),
	}
	teamRule := &configstoreTables.TableRoutingRule{
		ID: "3", Name: "Team", Scope: "team", ScopeID: &teamID, Priority: 30, Enabled: unifai.Ptr(true),
	}
