...
func TestEvaluateRoutingRules_NilContext(t *testing.T) {
	...
	_, err = engine.EvaluateRoutingRules(schemas.NewUnifAIContext(context.Background(), time.Now()), nil)
	assert.Error(t, err)
...
func TestEvaluateRoutingRules_NoRulesMatch(t *testing.T) {
	...
	decision, err := engine.EvaluateRoutingRules(schemas.NewUnifAIContext(context.Background(), time.Now()), ctx)
	assert.NoError(t, err)
	assert.Nil(t, decision)
}
...
func TestEvaluateRoutingRules_GlobalRuleMatches(t *testing.T) {
	store, err := NewLocalGovernanceStore(context.Background(), NewMockLogger(), nil, &configstore.GovernanceConfig{}, nil)
	require.NoError(t, err)
	bgCtx := schemas.NewUnifAIContext(context.Background(), time.Now())
	...
	rule := &configstoreTables.TableRoutingRule{
		...
		Targets: []configstoreTables.TableRoutingTarget{
			{Provider: unifai.Ptr("azure"), Model: unifai.Ptr("gpt-4-turbo"), Weight: 1.0},
		},
...
// UnifAIContext under UnifAIContextKeyAPIKeyID.
func TestEvaluateRoutingRules_MultiTargetDeterministicWithPinnedKey(t *testing.T) {
	...
	rule := &configstoreTables.TableRoutingRule{
		...
		Targets: []configstoreTables.TableRoutingTarget{
			{
				Provider: unifai.Ptr("azure"),
				Model:    unifai.Ptr("gpt-4-turbo"),
				KeyID:    unifai.Ptr(pinnedKeyID),
				Weight:   1.0,
			},
			{
				Provider: unifai.Ptr("openai"),
				Model:    unifai.Ptr("gpt-3.5"),
				Weight:   0.0,
			},
		},
		Enabled:  unifai.Ptr(true),
	...
	// PreRequestHook (core/unifai.go: ctx.BlockRestrictedWrites + ctx.WithPluginScope).
	// This is what production actually does — the routing pin lands on the dedicated,
	// non-reserved UnifAIContextKeyRoutingPinnedAPIKeyID (a write to the reserved
	// UnifAIContextKeyAPIKeyID would be silently dropped during this phase). Key selection
	...
	req := &schemas.UnifAIRequest{
		...
		ChatRequest: &schemas.UnifAIChatRequest{Provider: schemas.OpenAI, Model: "gpt-4o"},
	}

	root := schemas.NewUnifAIContext(context.Background(), time.Now())
	...
	ctxKeyID, _ := root.Value(schemas.UnifAIContextKeyRoutingPinnedAPIKeyID).(string)
	assert.Equal(t, pinnedKeyID, ctxKeyID,
		"routing-rule pinned key_id must reach UnifAIContextKeyRoutingPinnedAPIKeyID that selectKeyFromProviderForModelWithPool reads")
}
...
func TestEvaluateRoutingRules_ScopePrecedence(t *testing.T) {
	...
	globalRule := &configstoreTables.TableRoutingRule{
		...
		CelExpression: "model == 'gpt-4o'",
		Targets: []configstoreTables.TableRoutingTarget{
			{Provider: unifai.Ptr("openai"), Model: unifai.Ptr("gpt-4o"), Weight: 1.0},
		},
		Enabled:  unifai.Ptr(true),
		Scope:    "global",
		Priority: 0,
...
func TestResolveRoutingWithFallback_NoMatch(t *testing.T) {
	...
	decision, err := resolveRoutingWithFallback(schemas.NewUnifAIContext(context.Background(), time.Now()), ctx, engine)
	require.NoError(t, err)
	require.NotNil(t, decision)
...
func TestEvaluateRoutingRules_ChainRuleReEvaluation(t *testing.T) {
	...
	ruleA := &configstoreTables.TableRoutingRule{
		...
		CelExpression: "model == 'gpt-4o'",
		Targets: []configstoreTables.TableRoutingTarget{
			{Provider: unifai.Ptr("openai"), Model: unifai.Ptr("gpt-4-turbo"), Weight: 1.0},
		},
		Enabled:   unifai.Ptr(true),
		Scope:     "global",
		Priority:  0,
	...
	ruleB := &configstoreTables.TableRoutingRule{
		...
		Targets: []configstoreTables.TableRoutingTarget{
			{Provider: unifai.Ptr("azure"), Model: unifai.Ptr("gpt-4"), Weight: 1.0},
		},
		Enabled:   unifai.Ptr(true),
		Scope:     "global",
		Priority:  1,
...
func TestEvaluateRoutingRules_SelfLoopContinuesToNextRule(t *testing.T) {
	...
	ruleB := &configstoreTables.TableRoutingRule{
		...
		CelExpression: "model == 'gpt-4o'",
		Targets: []configstoreTables.TableRoutingTarget{
			{Provider: unifai.Ptr("anthropic"), Model: unifai.Ptr("claude-3"), Weight: 1.0},
		},
		Enabled:   unifai.Ptr(true),
		Scope:     "global",
		Priority:  1,
...
func TestCompileAndCacheProgram_ValidExpression_Routing(t *testing.T) {
	...
	rule := &configstoreTables.TableRoutingRule{
		...
		CelExpression: "model == 'gpt-4o'",
		Targets: []configstoreTables.TableRoutingTarget{
			{Provider: unifai.Ptr("openai"), Weight: 1.0},
		},
		Enabled: unifai.Ptr(true),
	}
