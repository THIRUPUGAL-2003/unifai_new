...
func TestBudgetResolver_EvaluateRequest_VirtualKeyNotFound(t *testing.T) {
	...
	result := resolver.EvaluateVirtualKeyRequest(ctx, "sk-uf-nonexistent", schemas.OpenAI, "gpt-4", schemas.ChatCompletionRequest, false)
...
func TestBudgetResolver_EvaluateRequest_ListModelsBypassesProviderBlock(t *testing.T) {
	...
	result := resolver.EvaluateVirtualKeyRequest(ctx, "sk-uf-test", schemas.OpenAI, "gpt-4", schemas.ListModelsRequest, false)
...
func TestBudgetResolver_EvaluateRequest_ModelBlocked(t *testing.T) {
	...
	result := resolver.EvaluateVirtualKeyRequest(ctx, "sk-uf-test", schemas.OpenAI, "gpt-4o-mini", schemas.ChatCompletionRequest, false)
...
func TestBudgetResolver_EvaluateRequest_ProviderLevelRateLimit(t *testing.T) {
	...
	vk := buildVirtualKeyWithProviders("vk1", "sk-uf-test", "Test VK", []configstoreTables.TableVirtualKeyProviderConfig{providerConfig})
...
func TestBudgetResolver_IsProviderAllowed(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			vk:              buildVirtualKey("vk1", "sk-uf-test", "Test", true),
			provider:        schemas.OpenAI,
			shouldBeAllowed: false,
		},
		{
			name: "Provider in allowlist",
			vk: buildVirtualKeyWithProviders("vk1", "sk-uf-test", "Test",
...
func TestBudgetResolver_ContextPopulation(t *testing.T) {
	...
	vkID, _ := ctx.Value(schemas.UnifAIContextKeyGovernanceVirtualKeyID).(string)
	teamID, _ := ctx.Value(schemas.UnifAIContextKeyGovernanceTeamID).(string)
	customerID, _ := ctx.Value(schemas.UnifAIContextKeyGovernanceCustomerID).(string)
...
func TestBudgetResolver_EvaluateRequest_PassthroughModelFiltering(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result := resolver.EvaluateVirtualKeyRequest(ctx, "sk-uf-test", schemas.OpenAI, tt.model, tt.requestType, false)
...
func TestBudgetResolver_EvaluateVirtualKeyRequest_InactiveWithFutureExpiry(t *testing.T) {
	logger := NewMockLogger()
	future := time.Now().UTC().Add(time.Hour)
	vk := buildVirtualKey("vk1", "sk-uf-test", "Test VK", false)
	vk.ExpiresAt = &future
