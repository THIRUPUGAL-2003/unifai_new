...
// newCtxWithIncludeTools returns a UnifAIContext pre-stamped with a caller-provided
// include-tools list, mirroring what lib/ctx.go does for the x-uf-mcp-include-tools header.
func newCtxWithIncludeTools(tools []string) *schemas.UnifAIContext {
...
func includeToolsFromCtx(t *testing.T, ctx *schemas.UnifAIContext) []string {
	t.Helper()
