...
// the header-driven customer scope: when UnifAIContextKeyGovernanceScopedCustomerID
...
func TestCollectHierarchy_ScopedCustomerSkipsScalarTeamCustomer(t *testing.T) {
	...
	scopedCtx := func(id string) context.Context {
		...
		ctx.SetValue(schemas.UnifAIContextKeyGovernanceScopedCustomerID, id)
		return ctx
	}
...
func TestEvaluateGovernanceRequest_ScopedCustomerSkipsScalarTeamCustomerEnforcement(t *testing.T) {
	...
	evaluate := func(scope string) *EvaluationResult {
		ctx := schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline)
		if scope != "" {
			ctx.SetValue(schemas.UnifAIContextKeyGovernanceScopedCustomerID, scope)
		}
