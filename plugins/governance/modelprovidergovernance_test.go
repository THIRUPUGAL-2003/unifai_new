...
func TestPreLLMHook_ModelProviderPass_VirtualKeyBudgetExceeded(t *testing.T) {
	...
	vk := buildVirtualKeyWithBudget("vk1", "sk-uf-test", "Test VK", vkBudget)
	store, err := NewLocalGovernanceStore(context.Background(), logger, nil, &configstore.GovernanceConfig{
	...
	parentCtx := context.WithValue(context.Background(), schemas.UnifAIContextKeyVirtualKey, "sk-uf-test")
	parentCtx = context.WithValue(parentCtx, schemas.UnifAIContextKeyRequestID, "req-1")
	ctx := schemas.NewUnifAIContext(parentCtx, schemas.NoDeadline)
	req := &schemas.UnifAIRequest{
		RequestType: schemas.ChatCompletionRequest,
		ChatRequest: &schemas.UnifAIChatRequest{
			Provider: schemas.OpenAI,
...
func TestPreLLMHook_ModelProviderPass_VirtualKeyRateLimitExceeded_Token(t *testing.T) {
	...
	vk := buildVirtualKeyWithRateLimit("vk1", "sk-uf-test", "Test VK", vkRateLimit)
	store, err := NewLocalGovernanceStore(context.Background(), logger, nil, &configstore.GovernanceConfig{
		VirtualKeys: []configstoreTables.TableVirtualKey{*vk},
...
func TestPreLLMHook_ModelProviderPass_VirtualKeyChecksPass(t *testing.T) {
	...
	vk := buildVirtualKey("vk1", "sk-uf-test", "Test VK", true)
...
func TestPreLLMHook_SkipKeySelection(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "skip + valid VK is enforced (budget exceeded)", skipKeySelection: true, virtualKey: "sk-uf-test", isVkMandatory: false, expectShortCircuit: true, msgContains: "budget exceeded"},
		{name: "skip + unknown VK is rejected", skipKeySelection: true, virtualKey: "sk-uf-unknown", isVkMandatory: false, expectShortCircuit: true, msgContains: "not found"},
		{name: "no skip + valid VK enforced identically", skipKeySelection: false, virtualKey: "sk-uf-test", isVkMandatory: false, expectShortCircuit: true, msgContains: "budget exceeded"},
	}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			parentCtx := context.WithValue(context.Background(), schemas.UnifAIContextKeyRequestID, "req-1")
			if tc.virtualKey != "" {
				parentCtx = context.WithValue(parentCtx, schemas.UnifAIContextKeyVirtualKey, tc.virtualKey)
			}
			ctx := schemas.NewUnifAIContext(parentCtx, schemas.NoDeadline)
			if tc.skipKeySelection {
				ctx.SetValue(schemas.UnifAIContextKeySkipKeySelection, true)
			}
			req := &schemas.UnifAIRequest{
				RequestType: schemas.ChatCompletionRequest,
				ChatRequest: &schemas.UnifAIChatRequest{
...
func TestPreLLMHook_RequiredHeaders_SkipKeySelection(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			ctx.SetValue(schemas.UnifAIContextKeySkipKeySelection, true)
			if tc.headers != nil {
				ctx.SetValue(schemas.UnifAIContextKeyRequestHeaders, tc.headers)
			}
			req := &schemas.UnifAIRequest{
				RequestType: schemas.ChatCompletionRequest,
				ChatRequest: &schemas.UnifAIChatRequest{
...
func TestPreLLMHook_ModelProviderPass_VirtualKeyNotFound(t *testing.T) {
	...
	parentCtx := context.WithValue(context.Background(), schemas.UnifAIContextKeyVirtualKey, "sk-uf-nonexistent")
	parentCtx = context.WithValue(parentCtx, schemas.UnifAIContextKeyRequestID, "req-1")
	ctx := schemas.NewUnifAIContext(parentCtx, schemas.NoDeadline)
	req := &schemas.UnifAIRequest{
		RequestType: schemas.ChatCompletionRequest,
...
func TestPreLLMHook_ModelProviderPass_VirtualKeyBlocked(t *testing.T) {
	...
	vk := buildVirtualKey("vk1", "sk-uf-test", "Test VK", false) // Inactive
	store, err := NewLocalGovernanceStore(context.Background(), logger, nil, &configstore.GovernanceConfig{
		VirtualKeys: []configstoreTables.TableVirtualKey{*vk},
	}, nil)
	require.NoError(t, err)
...
func TestPreLLMHook_ModelProviderPass_VirtualKeyProviderBlocked(t *testing.T) {
	...
	vk := buildVirtualKeyWithProviders("vk1", "sk-uf-test", "Test VK", providerConfigs)
	store, err := NewLocalGovernanceStore(context.Background(), logger, nil, &configstore.GovernanceConfig{
		VirtualKeys: []configstoreTables.TableVirtualKey{*vk},
	}, nil)
	require.NoError(t, err)
...
func TestPostHook_UpdatesProviderBudgetUsage_NoVirtualKey(t *testing.T) {
	...
	parentCtx1 := context.WithValue(context.Background(), schemas.UnifAIContextKeyRequestID, "req-1")
	ctx1 := schemas.NewUnifAIContext(parentCtx1, schemas.NoDeadline)
	req1 := &schemas.UnifAIRequest{
		RequestType: schemas.ChatCompletionRequest,
		ChatRequest: &schemas.UnifAIChatRequest{
			Provider: schemas.OpenAI,
			Model:    "gpt-4",
	...
	result1 := &schemas.UnifAIResponse{
	...
	parentCtx2 := context.WithValue(context.Background(), schemas.UnifAIContextKeyRequestID, "req-2")
	ctx2 := schemas.NewUnifAIContext(parentCtx2, schemas.NoDeadline)
	req2 := &schemas.UnifAIRequest{
		RequestType: schemas.ChatCompletionRequest,
		ChatRequest: &schemas.UnifAIChatRequest{
			Provider: schemas.OpenAI,
			Model:    "gpt-4",
...
func TestPostHook_TracksVirtualKeyUsageWhenUserIDPresent(t *testing.T) {
	...
	vk := buildVirtualKeyWithRateLimit("vk1", "sk-uf-test", "Test VK", rateLimit)
	store, err := NewLocalGovernanceStore(context.Background(), logger, nil, &configstore.GovernanceConfig{
		VirtualKeys: []configstoreTables.TableVirtualKey{*vk},
		RateLimits:  []configstoreTables.TableRateLimit{*rateLimit},
	}, nil)
	...
	ctx := schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline)
	ctx.SetValue(schemas.UnifAIContextKeyVirtualKey, "sk-uf-test")
	ctx.SetValue(schemas.UnifAIContextKeyUserID, "user1")
...
func TestPostHook_SkipVirtualKeyUsageTrackingFlag(t *testing.T) {
	...
	ctx.SetValue(schemas.UnifAIContextKeyVirtualKey, "sk-uf-test")
	ctx.SetValue(schemas.UnifAIContextKeyUserID, "user1")
	ctx.SetValue(schemas.UnifAIContextKeySkipVirtualKeyUsageTracking, true)
	result := &schemas.UnifAIResponse{
		ChatResponse: &schemas.UnifAIChatResponse{
			Model: "gpt-4",
			Usage: &schemas.UnifAILLMUsage{
	...
	assert.Equal(t, "user1", unifai.GetStringFromContext(ctx, schemas.UnifAIContextKeyUserID))
}
...
func TestPostMCPHook_TracksVirtualKeyUsageWhenUserIDPresent(t *testing.T) {
	...
	ctx.SetValue(schemas.UnifAIContextKeyVirtualKey, "sk-uf-test")
	ctx.SetValue(schemas.UnifAIContextKeyUserID, "user1")
	resp := &schemas.UnifAIMCPResponse{
