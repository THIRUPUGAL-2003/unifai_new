...
export default function APIKeysView() {
	...
	const isAuthConfigure = useMemo(() => {
		return unifaiConfig?.auth_config?.is_enabled;
	}, [unifaiConfig]);
			...
			<Alert variant="default">
				...
				<AlertDescription>
					<p className="text-md text-muted-foreground">
						...
						.<br />
						<br />
						Once generated you will need to use this API key for all API calls to the UnifAI admin APIs and UI.
					</p>
				</AlertDescription>
			</Alert>
		);
				...
				<AlertDescription>
					<p className="text-md text-muted-foreground">
						{isInferenceAuthDisabled ? (
							...
							<>
								Use Basic auth with your admin credentials when making API calls to UnifAI. Encode your credentials in the standard{" "}
								<code className="bg-muted rounded px-1 py-0.5 text-sm">username:password</code> format with base64 encoding.
							</>
						)}
					</p>
