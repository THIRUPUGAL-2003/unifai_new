...
export function buildClaudeCodeCommand({
	...
	const lines = [
		...
		`  --header ${quoteShellValue(`x-uf-vk: ${virtualKey.value}`)}`,
	];

	const includeClients = getIncludeClients(selectedServers);
	if (includeClients) {
		lines[lines.length - 1] += " \\";
		lines.push(`  --header ${quoteShellValue(`x-uf-mcp-include-clients: ${includeClients}`)}`);
	}
...
export function buildCodexConfig({
	...
	const headerEntries = [`"x-uf-vk" = ${quoteTomlString(virtualKey.value)}`];
	if (includeClients) {
		headerEntries.push(`"x-uf-mcp-include-clients" = ${quoteTomlString(includeClients)}`);
	}
...
function buildCursorServer({
	...
	const gatewayUrl = `${getExternalBaseUrl(clientConfig)}/mcp`;
	const registrationName = getRegistrationName(selectedServers);
	const headers: Record<string, string> = { "x-uf-vk": virtualKey.value };

	const includeClients = getIncludeClients(selectedServers);
	if (includeClients) {
		headers["x-uf-mcp-include-clients"] = includeClients;
	}
