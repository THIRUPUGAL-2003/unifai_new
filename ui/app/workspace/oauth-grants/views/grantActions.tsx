...
export default function GrantActions({ row, revoking, isPendingRow, onRevoke }: GrantActionsProps) {
	...
	// mode plus the identity filter, which exact-matches uf_sub against the
	// session's user_id / virtual key id / session id, so the user lands on
	// just this identity's sessions.
	const authSessionsUrl = `/workspace/mcp-sessions?auth_mode=${row.uf_mode}&identity=${encodeURIComponent(row.uf_sub)}`;
	...
	return (
		<DropdownMenu>
			...
			<DropdownMenuContent align="end">
				{(row.uf_mode === "user" || row.uf_mode === "vk" || row.uf_mode === "session") && (
