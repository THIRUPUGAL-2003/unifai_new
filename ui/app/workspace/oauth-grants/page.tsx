...
export default function OAuthGrantsPage() {
	const [urlState, setUrlState] = useQueryStates(
		{
			...
			uf_mode: parseAsArrayOf(parseAsString).withDefault([]),
	...
	const { data, isLoading, isFetching, isError, error } = useGetOAuth2GrantsQuery({
		...
		uf_mode: urlState.uf_mode.length ? urlState.uf_mode : undefined,
		limit: PAGE_SIZE,
	...
	const hasActiveFilters = !!urlState.q || urlState.uf_mode.length > 0;
	...
	const handleModeChange = (value: string[]) => setUrlState({ uf_mode: value.length ? value : null, offset: 0 });
	const handleOffsetChange = (offset: number) => setUrlState({ offset });
	const clearFilters = () => setUrlState({ q: null, uf_mode: null, offset: 0 });
		...
		<div className="mx-auto flex h-[calc(100dvh-50px)] w-full max-w-7xl flex-col">
			...
			{isLoading ? (
				...
				<div className="rounded-lg border border-destructive bg-destructive/10 p-6 text-sm text-destructive">
					...
					<p className="mt-2 text-muted-foreground text-xs font-normal">
						If this persists after a restart, the oauth2_refresh_tokens schema may need the uf_mode / uf_sub column migration.
					</p>
				</div>
			) : (
				<GrantsTable
