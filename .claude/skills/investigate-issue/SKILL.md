...
The repository is always `unifai/unifai`.

Fetch full issue details:
```bash
# Get issue with all metadata
gh issue view <ISSUE_ID> --repo unifai/unifai --json number,title,body,labels,assignees,state,comments,author,createdAt,updatedAt

# Get issue comments for additional context
gh issue view <ISSUE_ID> --repo unifai/unifai --json comments --jq '.comments[].body'
```

If the issue does not exist or `gh` fails:
- Check authentication: `gh auth status`
- Verify the issue number is valid: `gh issue list --repo unifai/unifai --limit 5 --json number,title`
...
|---|---|---|
| Core (Go) | `core/`, `core/schemas/`, `core/providers/`, `core/mcp/` | `core/unifai.go`, `core/utils.go` |
| Framework | `framework/`, `framework/configstore/`, `framework/logstore/` | `framework/config.go`, `framework/list.go` |
| Transports (HTTP) | `transports/unifai-http/` | `transports/unifai-http/` |
...
# Check the main unifai.go for relevant handlers
grep -n "functionName\|handlerName" core/unifai.go
```
...
grep -rn '"github.com/unifai/unifai/core/schemas"' --include='*.go' .
```
...
- Signature: `func Run{Scenario}Test(t *testing.T, client *unifai.UnifAI, ctx context.Context, testConfig ComprehensiveTestConfig)`
...
### Issue Not Found
```
Issue #<ID> was not found in unifai/unifai.
- Verify the issue number is correct
- Run: gh issue list --repo unifai/unifai --limit 10 --json number,title
```
...
Quick reference for navigating the UnifAI codebase:

```
unifai/
├── core/                          # Go core library
│   ├── unifai.go                 # Main UnifAI implementation (~195K)
...
├── transports/
│   └── unifai-http/              # HTTP transport + Docker
├── ui/                            # React + Vite UI
