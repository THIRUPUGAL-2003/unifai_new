...
| Area | Source | Notes |
|---|---|---|
| Server route wiring | `transports/unifai-http/server/server.go` | `RegisterAPIRoutes`, `RegisterInferenceRoutes`, `RegisterUIRoutes`, direct `/metrics`, middleware lists |
| HTTP handlers/controllers | `transports/unifai-http/handlers/*.go` | `RegisterRoutes` methods contain direct route registrations |
| SDK/provider integrations | `transports/unifai-http/integrations/*.go` | `RouteConfig` factories and `GenericRouter.RegisterRoutes` register OpenAI/Anthropic/GenAI/Bedrock/Cohere/LiteLLM/LangChain/PydanticAI/Cursor/Passthrough routes |
| Auth middleware | `transports/unifai-http/handlers/middlewares.go` | `APIMiddleware`, `InferenceMiddleware`, whitelists, realtime auth skips |
| Context auth extraction | `transports/unifai-http/lib/ctx.go` | Virtual key and API key header extraction |
...
grep -R "func (.*RegisterRoutes" -n transports/unifai-http/handlers transports/unifai-http/integrations --include='*.go'

grep -n "func (s \*UnifAIHTTPServer) RegisterAPIRoutes\|func (s \*UnifAIHTTPServer) RegisterInferenceRoutes\|RegisterUIRoutes" transports/unifai-http/server/server.go
```
...
grep -R "\.GET\|\.POST\|\.PUT\|\.DELETE\|\.PATCH\|\.HEAD\|\.OPTIONS\|r.Handle" \
  -n transports/unifai-http/handlers --include='*.go'
```

For a cleaner first pass:

```bash
for f in transports/unifai-http/handlers/*.go; do
...
```bash
SERVER=transports/unifai-http/server/server.go
...
INFER=$(grep -n "func (s \*UnifAIHTTPServer) RegisterInferenceRoutes" "$SERVER" | head -1 | cut -d: -f1)
...
API=$(grep -n "func (s \*UnifAIHTTPServer) RegisterAPIRoutes" "$SERVER" | head -1 | cut -d: -f1)
...
grep -R "func Create.*RouteConfigs\|func OpenAI.*Paths\|RouteConfig{" \
  -n transports/unifai-http/integrations --include='*.go'
```
...
```bash
SERVER=transports/unifai-http/server/server.go
MW_FILE=transports/unifai-http/handlers/middlewares.go
...
| Virtual-key self-service | Handler itself parses virtual key from `x-uf-vk`, `Authorization: Bearer sk-uf-*`, `x-api-key: sk-uf-*`, or `x-goog-api-key: sk-uf-*` | `VirtualKeyAuth`, `BearerAuth`, `ApiKeyAuth` (and document Google key header if exposed); no admin-only security |
...
grep -n "x-uf-vk\|x-goog-api-key\|sk-uf-\|VirtualKey\|ExtractVirtualKey" transports/unifai-http/lib/ctx.go

# VK quota handler (locate by symbol so line numbers don't drift)
LINE=$(grep -n "func.*getVirtualKeyQuota" transports/unifai-http/handlers/governance.go | head -1 | cut -d: -f1)
if [ -n "$LINE" ]; then
  sed -n "$((LINE)),$((LINE+45))p" transports/unifai-http/handlers/governance.go
fi
```

Virtual key sources commonly include:
- `x-uf-vk`
- `Authorization: Bearer sk-uf-*`
- `x-api-key: sk-uf-*`
- `x-goog-api-key: sk-uf-*`
...
```bash
cd docs/openapi
python3 bundle.py --output /tmp/unifai-openapi.json
cd -
```
...
python3 - <<'PY'
import json
spec = json.load(open('/tmp/unifai-openapi.json'))
...
```bash
grep -n "type .*Request struct" transports/unifai-http/handlers/<handler>.go
grep -n "json.Unmarshal\|sonic.Unmarshal\|parse.*Multipart\|ContentType" transports/unifai-http/handlers/<handler>.go transports/unifai-http/integrations/*.go
```
...
```bash
grep -n "SendJSON\|SendError\|SendUnifAIError\|SetStatusCode\|Status" transports/unifai-http/handlers/<handler>.go
```
...
### Validation commands
```bash
cd docs/openapi && python3 bundle.py --output /tmp/unifai-openapi.json
...
```bash
cd transports && go test ./unifai-http/handlers ./unifai-http/server ./unifai-http/integrations
```
...
cd docs/openapi
python3 bundle.py --output /tmp/unifai-openapi.json
python3 bundle.py --format yaml --output /tmp/unifai-openapi.yaml
cd -
```
