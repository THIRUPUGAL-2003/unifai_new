...
import (
	...
	mistralprovider "github.com/unifai/unifai/core/providers/mistral"
...
// Helper function to create a UnifAIError
func createUnifAIError(message string, statusCode *int, errorType *string, isUnifAIError bool) *schemas.UnifAIError {
	return &schemas.UnifAIError{
		IsUnifAIError: isUnifAIError,
...
func TestExecuteRequestWithRetries_SuccessScenarios(t *testing.T) {
	...
	ctx.SetValue(schemas.UnifAIContextKeyTracer, &schemas.NoOpTracer{})
	// Test immediate success
	t.Run("ImmediateSuccess", func(t *testing.T) {
		callCount := 0
		handler := func(_ schemas.Key) (string, *schemas.UnifAIError) {
			callCount++
			return "success", nil
		}
	...
	t.Run("SuccessAfterRetries", func(t *testing.T) {
		...
		handler := func(_ schemas.Key) (string, *schemas.UnifAIError) {
			...
			if callCount <= 2 {
				...
				return "", createUnifAIError("rate limit exceeded", Ptr(429), nil, false)
			}
			// Third call succeeds
			return "success", nil
		}
...
func TestExecuteRequestWithRetries_NonRetryableErrors(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:  "UnifAIError",
			error: createUnifAIError("validation error", nil, nil, true),
		},
		{
			name:  "RequestCancelled",
			error: createUnifAIError("request cancelled", nil, Ptr(schemas.ErrRequestCancelled), false),
		},
		{
			name:  "Non-retryable status code",
			error: createUnifAIError("bad request", Ptr(400), nil, false),
		},
		{
			name:  "Non-retryable error message",
			error: createUnifAIError("invalid model", nil, nil, false),
		},
	}
	logger := NewDefaultLogger(schemas.LogLevelError)
...
func TestExecuteRequestWithRetries_RetryableConditions(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:  "StatusCode_500",
			error: createUnifAIError("internal server error", Ptr(500), nil, false),
		},
		{
			name:  "StatusCode_502",
			error: createUnifAIError("bad gateway", Ptr(502), nil, false),
		},
		{
			name:  "StatusCode_503",
			error: createUnifAIError("service unavailable", Ptr(503), nil, false),
		},
		{
			name:  "StatusCode_504",
			error: createUnifAIError("gateway timeout", Ptr(504), nil, false),
		},
		{
			name:  "StatusCode_429",
			error: createUnifAIError("too many requests", Ptr(429), nil, false),
		},
		{
			name:  "ErrProviderDoRequest",
			error: createUnifAIError(schemas.ErrProviderDoRequest, nil, nil, false),
		},
		{
			name:  "RateLimitMessage",
			error: createUnifAIError("rate limit exceeded", nil, nil, false),
		},
		{
			name:  "RateLimitType",
			error: createUnifAIError("some error", nil, Ptr("rate_limit"), false),
		},
	}
	logger := NewDefaultLogger(schemas.LogLevelError)
...
func TestHandleProviderRequest_OCROperationNotAllowed(t *testing.T) {
	...
	request := &ChannelMessage{
		Context: ctx,
		UnifAIRequest: schemas.UnifAIRequest{
			RequestType: schemas.OCRRequest,
			OCRRequest: &schemas.UnifAIOCRRequest{
	...
	response, err := unifai.handleProviderRequest(provider, providerConfig, request, schemas.Key{}, nil)
	if response != nil {
...
func TestFilterProvidersByContext(t *testing.T) {
	...
	t.Run("available providers restrict list models fanout", func(t *testing.T) {
		ctx := schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline)
		ctx.SetValue(schemas.UnifAIContextKeyAvailableProviders, []schemas.ModelProvider{schemas.Anthropic})
	...
	t.Run("empty available providers denies all providers", func(t *testing.T) {
		ctx := schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline)
		ctx.SetValue(schemas.UnifAIContextKeyAvailableProviders, []schemas.ModelProvider{})
	...
	t.Run("malformed available providers fails closed", func(t *testing.T) {
		ctx := schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline)
		ctx.SetValue(schemas.UnifAIContextKeyAvailableProviders, "openai")
...
func TestRunStreamPreHooks_FinalChunkFlushesTrace(t *testing.T) {
	...
	client, err := Init(ctx, schemas.UnifAIConfig{
		Account: account,
		Tracer:  tracer,
		Logger:  NewDefaultLogger(schemas.LogLevelError),
	})
	if err != nil {
		t.Fatalf("Error initializing UnifAI: %v", err)
	}
	defer client.Shutdown()

	hooks, unifaiErr := client.RunStreamPreHooks(ctx, &schemas.UnifAIRequest{
	...
	if unifaiErr != nil {
		t.Fatalf("RunStreamPreHooks returned error: %v", unifaiErr)
	}
	defer hooks.Cleanup()

	ctx.SetValue(schemas.UnifAIContextKeyStreamEndIndicator, true)
	_, unifaiErr = hooks.PostHookRunner(ctx, &schemas.UnifAIResponse{
	...
	if unifaiErr != nil {
		t.Fatalf("PostHookRunner returned error: %v", unifaiErr)
	}
...
func TestSelectKeyFromProviderForModel_SessionStickiness(t *testing.T) {
	...
	deterministicSelector := func(ctx *schemas.UnifAIContext, keys []schemas.Key, _ schemas.ModelProvider, _ string) (schemas.Key, error) {
		keySelectorCalls++
		return keys[0], nil // always return first key
	}

	ctx := context.Background()
	unifai, err := Init(ctx, schemas.UnifAIConfig{
		Account:     account,
		Logger:      NewDefaultLogger(schemas.LogLevelError),
		KVStore:     kvStore,
	...
	bfCtx := schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline)
	bfCtx.SetValue(schemas.UnifAIContextKeySessionID, "sess-123")

	// First call: cache miss, keySelector runs, key stored; returns single-element pool (canRotate=false)
	keys1, canRotate1, err := unifai.selectKeyFromProviderForModelWithPool(bfCtx, schemas.ChatCompletionRequest, schemas.OpenAI, "gpt-4", schemas.OpenAI)
	if err != nil {
	...
	keys2, canRotate2, err := unifai.selectKeyFromProviderForModelWithPool(bfCtx, schemas.ChatCompletionRequest, schemas.OpenAI, "gpt-4", schemas.OpenAI)
	if err != nil {
...
func TestSelectKeyFromProviderForModel_NoStickinessWithoutSessionID(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		pool, canRotate, err := unifai.selectKeyFromProviderForModelWithPool(bfCtx, schemas.ChatCompletionRequest, schemas.OpenAI, "gpt-4", schemas.OpenAI)
		if err != nil {
...
func TestSelectKeyFromProviderForModel_SessionStickinessNoRotation(t *testing.T) {
	...
	bfCtx := schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline)
	bfCtx.SetValue(schemas.UnifAIContextKeySessionID, "sess-sticky")
	bfCtx.SetValue(schemas.UnifAIContextKeyTracer, &schemas.NoOpTracer{})

	config := createTestConfig(3, 0, 0)
	logger := NewDefaultLogger(schemas.LogLevelError)

	// Build keyProvider the same way requestWorker does.
	pool, canRotate, poolErr := unifai.selectKeyFromProviderForModelWithPool(bfCtx, schemas.ChatCompletionRequest, schemas.OpenAI, "gpt-4", schemas.OpenAI)
	if poolErr != nil {
	...
	callCount := 0
	handler := func(k schemas.Key) (string, *schemas.UnifAIError) {
...
func TestSelectKeyFromProviderForModel_BlacklistedModels(t *testing.T) {
	...
	t.Run("all keys blacklist model", func(t *testing.T) {
		...
		_, _, err := unifai.selectKeyFromProviderForModelWithPool(bfCtx, schemas.ChatCompletionRequest, schemas.OpenAI, "gpt-4", schemas.OpenAI)
		if err == nil {
...
func TestExecuteRequestWithRetries_KeyRotation(t *testing.T) {
	...
	t.Run("SameKeyOnNetworkError", func(t *testing.T) {
		...
		handler := func(k schemas.Key) (string, *schemas.UnifAIError) {
			...
			callCount++
			if callCount <= 2 {
				return "", createUnifAIError(schemas.ErrProviderDoRequest, nil, nil, false)
			}
			return "success", nil
		}
	...
	t.Run("NilKeyProviderUsesZeroKey", func(t *testing.T) {
		cleanCtx := schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline)
		cleanCtx.SetValue(schemas.UnifAIContextKeyTracer, &schemas.NoOpTracer{})
		...
		if trail, ok := cleanCtx.Value(schemas.UnifAIContextKeyAttemptTrail).([]schemas.KeyAttemptRecord); ok && len(trail) > 0 {
			t.Fatalf("expected no attempt trail for nil keyProvider, got %v", trail)
		}
		if selectedID, _ := cleanCtx.Value(schemas.UnifAIContextKeySelectedKeyID).(string); selectedID != "" {
			t.Fatalf("expected empty selected key id, got %q", selectedID)
		}
		if selectedName, _ := cleanCtx.Value(schemas.UnifAIContextKeySelectedKeyName).(string); selectedName != "" {
...
func TestUpdateProvider(t *testing.T) {
	t.Run("SuccessfulUpdate", func(t *testing.T) {
		...
		if err != nil {
			t.Fatalf("Failed to initialize UnifAI: %v", err)
		}

		// Verify initial provider exists
		initialProvider := unifai.getProviderByKey(schemas.OpenAI)
		...
		// Perform update
		err = unifai.UpdateProvider(schemas.OpenAI)
		if err != nil {
			t.Fatalf("UpdateProvider failed: %v", err)
		}

		// Verify provider was replaced
		updatedProvider := unifai.getProviderByKey(schemas.OpenAI)
	...
	t.Run("UpdateNonExistentProvider", func(t *testing.T) {
		...
		err = unifai.UpdateProvider(schemas.Anthropic)
		if err == nil {
	...
	t.Run("UpdateInactiveProvider", func(t *testing.T) {
		// Setup account with provider but don't initialize it in UnifAI
		account := NewMockAccount()
		...
		if unifai.getProviderByKey(schemas.Ollama) != nil {
			t.Fatal("Provider should not exist initially")
		}

		// Add provider to account after unifai initialization
	...
	t.Run("MultipleProviderUpdates", func(t *testing.T) {
		...
		initialOpenAI := unifai.getProviderByKey(schemas.OpenAI)
		initialAnthropic := unifai.getProviderByKey(schemas.Anthropic)
		initialCohere := unifai.getProviderByKey(schemas.Cohere)
		...
		for _, provider := range providers {
			err = unifai.UpdateProvider(provider)
			if err != nil {
		...
		newOpenAI := unifai.getProviderByKey(schemas.OpenAI)
		newAnthropic := unifai.getProviderByKey(schemas.Anthropic)
		newCohere := unifai.getProviderByKey(schemas.Cohere)
	...
	t.Run("ConcurrentProviderUpdates", func(t *testing.T) {
		...
		provider := unifai.getProviderByKey(schemas.OpenAI)
		if provider == nil {
...
func TestUpdateProvider_ProviderSliceIntegrity(t *testing.T) {
	t.Run("ProviderSliceConsistency", func(t *testing.T) {
		...
		initialProviders := unifai.providers.Load()
		...
		updatedProviders := unifai.providers.Load()
...
// UnifAI instance required. They validate the core safety invariants that
...
func newTestChannelMessage(ctx *schemas.UnifAIContext) *ChannelMessage {
	return &ChannelMessage{
		...
		Context:  ctx,
		Response: make(chan *schemas.UnifAIResponse, 1),
		Err:      make(chan schemas.UnifAIError, 1),
	}
}
...
drainLoop:
	...
	for i, msg := range msgs {
		select {
		case unifaiErr := <-msg.Err:
			if unifaiErr.Error == nil {
				t.Errorf("message %d: received nil Error field", i)
				continue
			}
			if unifaiErr.Error.Message != "provider is shutting down" {
				t.Errorf("message %d: expected 'provider is shutting down', got %q",
					i, unifaiErr.Error.Message)
			}
			if unifaiErr.ExtraFields.Provider != schemas.OpenAI {
				t.Errorf("message %d: expected provider %s, got %s",
					i, schemas.OpenAI, unifaiErr.ExtraFields.Provider)
			}
			if unifaiErr.ExtraFields.RequestType != schemas.ChatCompletionRequest {
				t.Errorf("message %d: expected requestType %v, got %v",
					i, schemas.ChatCompletionRequest, unifaiErr.ExtraFields.RequestType)
			}
		default:
...
//
//	if pq.isClosing() {
//	    if newPq, err := unifai.getProviderQueue(provider); err == nil && newPq != pq {
//	        pq = newPq   // transparent re-route
//	    }
//	}
//
// This test exercises that exact sequence without a full UnifAI instance.
...
drainLoop:
	...
	for i, msg := range msgs {
		...
		case unifaiErr := <-msg.Err:
			if unifaiErr.Error == nil {
				t.Errorf("message %d: got nil Error field in UnifAIError", i)
				continue
			}
			if unifaiErr.Error.Message != "provider is shutting down" {
				t.Errorf("message %d: expected 'provider is shutting down', got %q",
...
// non-reserved UnifAIContextKeyRoutingPinnedAPIKeyID, mirroring what the governance routing
// engine does. It exists to exercise the commit step in PluginPipeline.RunPreRequestHooks.
type fakeRoutingPlugin struct {
	name     string
	pinKeyID string // written to UnifAIContextKeyRoutingPinnedAPIKeyID when non-empty
}

func (f *fakeRoutingPlugin) GetName() string { return f.name }
func (f *fakeRoutingPlugin) Cleanup() error  { return nil }
func (f *fakeRoutingPlugin) PreRequestHook(ctx *schemas.UnifAIContext, req *schemas.UnifAIRequest) error {
	if f.pinKeyID != "" {
		// A direct write to the reserved UnifAIContextKeyAPIKeyID here would be dropped by the
		// restricted-write block; routing must use the non-reserved key.
		ctx.SetValue(schemas.UnifAIContextKeyRoutingPinnedAPIKeyID, f.pinKeyID)
	}
	return nil
}
func (f *fakeRoutingPlugin) PreLLMHook(ctx *schemas.UnifAIContext, req *schemas.UnifAIRequest) (*schemas.UnifAIRequest, *schemas.LLMPluginShortCircuit, error) {
	return req, nil, nil
}
func (f *fakeRoutingPlugin) PostLLMHook(ctx *schemas.UnifAIContext, resp *schemas.UnifAIResponse, unifaiErr *schemas.UnifAIError) (*schemas.UnifAIResponse, *schemas.UnifAIError, error) {
...
// writes to the non-reserved UnifAIContextKeyRoutingPinnedAPIKeyID (during the blocked
// PreRequestHook phase) is committed by core into the reserved UnifAIContextKeyAPIKeyID that
...
func TestRunPreRequestHooks_CommitsRoutingPinnedKey(t *testing.T) {
	...
	t.Run("routing pin is committed to reserved api-key-id", func(t *testing.T) {
		...
		ctx := schemas.NewUnifAIContext(context.Background(), time.Now())
		p.RunPreRequestHooks(ctx, &schemas.UnifAIRequest{})
		if got, _ := ctx.Value(schemas.UnifAIContextKeyAPIKeyID).(string); got != pinned {
	...
	t.Run("routing pin overrides a caller-supplied api-key-id", func(t *testing.T) {
		p := newRoutingCommitPipeline(&fakeRoutingPlugin{name: "gov", pinKeyID: pinned})
		ctx := schemas.NewUnifAIContext(context.Background(), time.Now())
		ctx.SetValue(schemas.UnifAIContextKeyAPIKeyID, "caller-pin")
		p.RunPreRequestHooks(ctx, &schemas.UnifAIRequest{})
		if got, _ := ctx.Value(schemas.UnifAIContextKeyAPIKeyID).(string); got != pinned {
	...
	t.Run("caller api-key-id preserved when no routing pin", func(t *testing.T) {
		...
		ctx.SetValue(schemas.UnifAIContextKeyAPIKeyID, "caller-pin")
		p.RunPreRequestHooks(ctx, &schemas.UnifAIRequest{})
		if got, _ := ctx.Value(schemas.UnifAIContextKeyAPIKeyID).(string); got != "caller-pin" {
...
func TestClearAnthropicPassthroughForNonNativeProvider(t *testing.T) {
	flagKeys := []schemas.UnifAIContextKey{
		schemas.UnifAIContextKeyUseRawRequestBody,
		schemas.UnifAIContextKeySendBackRawResponse,
		schemas.UnifAIContextKeyPassthroughOverridesPresent,
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.integrationType != "" {
				ctx.SetValue(schemas.UnifAIContextKeyIntegrationType, tt.integrationType)
			}
