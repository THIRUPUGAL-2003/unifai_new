...
func TestPerServerTimeout_OverridesGlobal(t *testing.T) {
	...
	result, unifaiErr := client.ExecuteChatMCPTool(ctx, &toolCall)
	elapsed := time.Since(start)

	assert.Less(t, elapsed, 2500*time.Millisecond,
		"per-server timeout (1s) should fire before the 3s tool delay; got %v", elapsed)
	// Timeout surfaces either as a UnifAIError or as an error message in the result content.
	if unifaiErr != nil && unifaiErr.Error != nil {
		t.Logf("elapsed: %v, error: %s", elapsed, unifaiErr.Error.Message)
	} else if result != nil {
		t.Logf("elapsed: %v, timeout returned in result", elapsed)
	} else {
		t.Errorf("expected either a unifai error or a result indicating timeout, got both nil")
	}
}
...
func TestPerServerTimeout_AllowsLongerThanGlobal(t *testing.T) {
	...
	require.Nil(t, unifaiErr, "tool should succeed when delay < per-server timeout")
...
func TestPerServerTimeout_FallsBackToGlobal(t *testing.T) {
	...
	// Cancellation surfaces either as a UnifAIError or as an error message in the result content.
	if unifaiErr != nil && unifaiErr.Error != nil {
		t.Logf("elapsed: %v, error: %s", elapsed, unifaiErr.Error.Message)
	} else if result != nil {
		t.Logf("elapsed: %v, cancellation returned in result", elapsed)
	} else {
		t.Errorf("expected either a unifai error or a result indicating cancellation, got both nil")
	}
}
