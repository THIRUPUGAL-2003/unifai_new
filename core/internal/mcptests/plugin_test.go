...
import (
	...
	core "github.com/unifai/unifai/core"
...
func TestPlugin_PreMCPHook(t *testing.T) {
	...
	unifai, err := core.Init(context.Background(), schemas.UnifAIConfig{
		Account:    &testAccount{},
	...
	result, unifaiErr := unifai.ExecuteChatMCPTool(ctx, &echoCall)
...
func TestPlugin_ShortCircuit(t *testing.T) {
	...
	governancePlugin.BlockTool("unifaiInternal-echo")
	...
	// Tool name might be "echo" or "unifaiInternal-echo" in the response
	hasToolName := strings.Contains(*result.Content.ContentStr, "echo") || strings.Contains(*result.Content.ContentStr, "unifaiInternal-echo")
...
func TestPlugin_ShortCircuit_CustomMessage(t *testing.T) {
	...
	governancePlugin.BlockTool("unifaiInternal-calculator")
	...
	result, unifaiErr := unifai.ExecuteChatMCPTool(ctx, &calcCall)
	...
	require.NotNil(t, result.Content)
	require.NotNil(t, result.Content.ContentStr)
	// Tool name might be "calculator" or "unifaiInternal-calculator" in the response
	hasToolName := strings.Contains(*result.Content.ContentStr, "calculator") || strings.Contains(*result.Content.ContentStr, "unifaiInternal-calculator")
...
func TestPlugin_MultiplePlugins(t *testing.T) {
	...
	// Setup UnifAI with multiple plugins in pipeline
	unifai, err := core.Init(context.Background(), schemas.UnifAIConfig{
		Account:    &testAccount{},
...
func TestPlugin_ResponsesFormat(t *testing.T) {
	...
	responsesToolCall := GetSampleResponsesToolCallMessage("test-responses-format", "unifaiInternal-echo", map[string]interface{}{
		"message": "test message",
	})
	result, unifaiErr := unifai.ExecuteResponsesMCPTool(ctx, responsesToolCall.ResponsesToolMessage)
...
func TestPlugin_LoggingPlugin(t *testing.T) {
	...
	_, err1 := unifai.ExecuteChatMCPTool(ctx, &echoCall)
	require.Nil(t, err1)

	calcCall := GetSampleCalculatorToolCall("log-test-2", "add", 1.0, 2.0)
	_, err2 := unifai.ExecuteChatMCPTool(ctx, &calcCall)
	require.Nil(t, err2)

	weatherCall := GetSampleWeatherToolCall("log-test-3", "Tokyo", "celsius")
	_, err3 := unifai.ExecuteChatMCPTool(ctx, &weatherCall)
	require.Nil(t, err3)
