...
func TestHealthCheckSTDIOServerDropAndRecoverIn20Seconds(t *testing.T) {
	...
	unifaiRoot := "/Users/prathammaxim/Desktop/unifai"
	clientConfig := GetTemperatureMCPClientConfig(unifaiRoot)
	clientConfig.ID = "stdio-health-recovery-test"

	// 1. Create STDIO client with unifai manager
...
func TestHealthCheckDuringToolExecution(t *testing.T) {
	...
	clients := manager.GetClients()
	require.Len(t, clients, 1, "should have unifaiInternal client")
	...
	require.Nil(t, unifaiErr, "tool execution should succeed during health monitoring")
