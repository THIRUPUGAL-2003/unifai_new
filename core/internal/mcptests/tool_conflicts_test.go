...
func TestToolNameConflict_MultipleClients(t *testing.T) {
	...
	if unifaiErr == nil {
		...
		t.Logf("Tool execution failed: %v", unifaiErr)
	}
}
...
func TestToolNameConflict_Resolution(t *testing.T) {
	...
	// Register echo tool - will be available as unifaiInternal-echo
	...
	for i := 0; i < 10; i++ {
		...
		result, unifaiErr := unifai.ExecuteChatMCPTool(ctx, &toolCall)
		require.Nil(t, unifaiErr, "tool should execute")
...
func TestToolNameConflict_WithFiltering(t *testing.T) {
	...
	// Register the echo tool for unifaiInternal
	require.NoError(t, RegisterEchoTool(manager))
	...
	// Execute "echo" - should use unifaiInternal client (it's the only one with the tool registered in-process)
...
func TestMultipleSameNameTools_ThreeClients(t *testing.T) {
	...
	// Register calculator tool - will be available as unifaiInternal-calculator
	...
	for i := 0; i < 15; i++ {
		...
		result, unifaiErr := unifai.ExecuteChatMCPTool(ctx, &toolCall)
		require.Nil(t, unifaiErr, "calculator should execute")
		require.NotNil(t, result)
	}

	// Verify only the unifaiInternal client is active (disconnected clients may be
	...
	assert.Equal(t, 1, activeClients, "should have 1 unifaiInternal client")
...
func TestMultipleSameNameTools_DifferentImplementations(t *testing.T) {
	...
	// Use unifaiInternal client only with registered tools
	inProcessClient := GetSampleInProcessClientConfig()
	...
	for i := 0; i < 5; i++ {
		toolCall := schemas.ChatAssistantMessageToolCall{
			...
			Function: schemas.ChatAssistantMessageToolCallFunction{
				Name:      schemas.Ptr("unifaiInternal-process_data"),
				Arguments: `{"data": "test"}`,
			},
		}

		result, unifaiErr := unifai.ExecuteChatMCPTool(ctx, &toolCall)
		require.Nil(t, unifaiErr, "process_data should execute")
		require.NotNil(t, result)
	}
...
func TestToolConflict_OneClientDisconnected(t *testing.T) {
	...
	// Register echo tool - will be available as unifaiInternal-echo on client1
	require.NoError(t, RegisterEchoTool(manager))
	...
	// Verify only unifaiInternal is active (disconnected clients may be retained
	...
	require.Equal(t, 1, activeClients, "should only have 1 active client (unifaiInternal)")
	...
	result, unifaiErr := unifai.ExecuteChatMCPTool(ctx, &toolCall)
	require.Nil(t, unifaiErr, "should use connected client")
	require.NotNil(t, result)
...
func TestToolConflict_BothClientsDisconnected(t *testing.T) {
	...
	assert.NotNil(t, unifaiErr, "should fail when all clients are disconnected")
	assert.Nil(t, result)
	if unifaiErr != nil && unifaiErr.Error != nil {
		errorMsg := unifaiErr.Error.Message
...
func TestToolConflict_ChatFormat(t *testing.T) {
	...
	// Use unifaiInternal clients with registered tools
	inProcessClient := GetSampleInProcessClientConfig()
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			result, unifaiErr := unifai.ExecuteChatMCPTool(ctx, &tc.toolCall)
			if unifaiErr != nil && unifaiErr.Error != nil {
				fmt.Println("unifaiErr", unifaiErr.Error.Message)
			}
			require.Nil(t, unifaiErr, "should resolve conflict and execute")
			require.NotNil(t, result)
...
func TestToolConflict_ResponsesFormat(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			result, unifaiErr := unifai.ExecuteResponsesMCPTool(ctx, &tc.responsesToolMsg)
			require.Nil(t, unifaiErr, "should resolve conflict and execute")
			require.NotNil(t, result)
...
func TestToolConflict_ComprehensiveScenarios(t *testing.T) {
	...
	scenarios := []struct {
		...
		{
			name:          "echo_tool",
			toolName:      "unifaiInternal-echo",
			expectSuccess: true,
		},
		{
			name:          "calculator_tool",
			toolName:      "unifaiInternal-calculator",
			expectSuccess: true,
		},
		{
			name:          "weather_tool",
			toolName:      "unifaiInternal-get_weather",
			expectSuccess: true,
		},
	}
	...
	for _, scenario := range scenarios {
		t.Run(scenario.name, func(t *testing.T) {
			...
			if scenario.expectSuccess {
				require.Nil(t, unifaiErr, "should execute successfully")
				require.NotNil(t, result)
				t.Logf("Tool %s executed successfully", scenario.toolName)
			} else {
				assert.NotNil(t, unifaiErr, "should fail")
			}
		})
	}
}
