...
func setupCodeModeWithSTDIOServers(t *testing.T, serverNames ...string) (*mcp.MCPManager, *unifai.UnifAI) {
	t.Helper()
	...
	for _, serverName := range serverNames {
		...
		switch serverName {
		case "temperature":
			config = GetTemperatureMCPClientConfig(unifaiRoot)
		...
		case "go-test-server":
			config = GetGoTestServerConfig(unifaiRoot)
		...
		case "edge-case-server":
			config = GetEdgeCaseServerConfig(unifaiRoot)
		...
		case "error-test-server":
			config = GetErrorTestServerConfig(unifaiRoot)
		...
		case "parallel-test-server":
			config = GetParallelTestServerConfig(unifaiRoot)
...
func TestCodeMode_STDIO_SingleServerBasicExecution(t *testing.T) {
	...
	_, unifai := setupCodeModeWithSTDIOServers(t, "test-tools-server")
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			require.Nil(t, unifaiErr, "execution should succeed")
...
func TestCodeMode_STDIO_MultipleServers(t *testing.T) {
	...
	_, unifai := setupCodeModeWithSTDIOServers(t, "test-tools-server", "temperature")
	ctx := createTestContext()
...
func TestCodeMode_STDIO_ServerFiltering(t *testing.T) {
	...
	manager, unifai := setupCodeModeWithSTDIOServers(t, "test-tools-server", "temperature")
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.shouldSucceed {
				...
				// Should fail - either unifai error or error in result
				errorFound := false
				if unifaiErr != nil {
					assert.Contains(t, unifaiErr.Error.Message, tc.expectedError)
					errorFound = true
...
func TestCodeMode_STDIO_ToolFiltering(t *testing.T) {
	...
	manager, unifai := setupCodeModeWithSTDIOServers(t, "test-tools-server")
...
func TestCodeMode_STDIO_EdgeCaseServer_Unicode(t *testing.T) {
	...
	_, unifai := setupCodeModeWithSTDIOServers(t, "edge-case-server")
	ctx := createTestContext()
...
func TestCodeMode_STDIO_ErrorTestServer_NetworkErrors(t *testing.T) {
	...
	_, unifai := setupCodeModeWithSTDIOServers(t, "error-test-server")
	ctx := createTestContext()
...
func TestCodeMode_STDIO_ParallelTestServer_Sequential(t *testing.T) {
	...
	_, unifai := setupCodeModeWithSTDIOServers(t, "parallel-test-server")
	ctx := createTestContext()
...
func TestCodeMode_STDIO_MultiServer_AllServers(t *testing.T) {
	...
	_, unifai := setupCodeModeWithSTDIOServers(t, "go-test-server", "edge-case-server", "error-test-server", "parallel-test-server")
	ctx := createTestContext()
...
func TestCodeMode_STDIO_MultiServer_FilteringAcrossServers(t *testing.T) {
	...
	_, unifai := setupCodeModeWithSTDIOServers(t, "go-test-server", "edge-case-server", "parallel-test-server")
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.shouldSucceed {
				...
				// Should fail - check either unifaiErr or error in result
				if unifaiErr == nil && result != nil && result.Content != nil && result.Content.ContentStr != nil {
