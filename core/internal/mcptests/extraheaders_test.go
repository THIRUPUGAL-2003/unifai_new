...
//
// These tests assert the end-to-end contract that UnifAIContextKeyMCPExtraHeaders
// set during the plugin gate is forwarded to the upstream MCP server — on
// unifai-generated health-check probes (ping AND list_tools) as well as on a
...
// extraHeaderInjectPlugin sets UnifAIContextKeyMCPExtraHeaders on every MCP
...
func (p *extraHeaderInjectPlugin) PreMCPHook(ctx *schemas.UnifAIContext, req *schemas.UnifAIMCPRequest) (*schemas.UnifAIMCPRequest, *schemas.MCPPluginShortCircuit, error) {
	ctx.SetValue(schemas.UnifAIContextKeyMCPExtraHeaders, map[string][]string{
...
// a hook-set extra header must ride a unifai-generated health-check ping onto
...
func TestExtraHeadersHealthCheckPingReachWire(t *testing.T) {
	...
	manager, _ := setupUnifAIWithPlugins(t, []schemas.MCPPlugin{&extraHeaderInjectPlugin{}})
...
func TestExtraHeadersToolCallReachWire(t *testing.T) {
	...
	ts, rec := buildRecordingHTTPServer(t)
	manager, client := setupUnifAIWithPlugins(t, []schemas.MCPPlugin{&extraHeaderInjectPlugin{}})
	...
	_, unifaiErr := client.ExecuteChatMCPTool(createTestContext(), &toolCall)
	require.Nil(t, unifaiErr, "echo tool call should succeed")
...
// extra header must ride a unifai-generated health-check ping onto the wire.
