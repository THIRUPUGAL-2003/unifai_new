...
func TestAgent_Timeout(t *testing.T) {
	...
	_, unifaiErr := manager.CheckAndExecuteAgentForChatRequest(
		ctx,
		originalReq,
		initialResponse,
		mockLLM.MakeChatRequest,
	...
	require.NotNil(t, unifaiErr, "Expected timeout error but got success - timeout is not being enforced!")
	t.Logf("✅ Timeout correctly enforced: %v", unifaiErr.Error)
}
...
func TestAgent_TimeoutDuringExecution(t *testing.T) {
	...
	require.NotNil(t, unifaiErr, "Expected timeout during tool execution but got success - timeout not enforced mid-execution!")
	t.Logf("✅ Timeout during execution correctly enforced: %v", unifaiErr.Error)
}
...
func TestAgent_Timeout_ChatFormat(t *testing.T) {
	...
	_, unifaiErr := manager.CheckAndExecuteAgentForChatRequest(ctx,
		&schemas.UnifAIChatRequest{Provider: schemas.OpenAI, Model: "gpt-4o",
			Input: []schemas.ChatMessage{{Role: schemas.ChatMessageRoleUser, Content: &schemas.ChatMessageContent{ContentStr: schemas.Ptr("Test")}}}},
		mockLLM.chatResponses[0], mockLLM.MakeChatRequest)

	require.NotNil(t, unifaiErr, "Chat format timeout not enforced!")
	t.Logf("✅ Chat format timeout enforced: %v", unifaiErr.Error)
}
...
func TestAgent_Timeout_ResponsesFormat(t *testing.T) {
	...
	mockLLM := &MockLLMCaller{
		responsesResponses: []*schemas.UnifAIResponsesResponse{
			CreateResponsesResponseWithToolCalls([]schemas.ResponsesToolMessage{
				{CallID: schemas.Ptr("call-1"), Name: schemas.Ptr("unifaiInternal-slow_responses_tool"), Arguments: schemas.Ptr(`{}`)},
			}),
		},
	}

	_, unifaiErr := manager.CheckAndExecuteAgentForResponsesRequest(ctx,
		&schemas.UnifAIResponsesRequest{Provider: schemas.OpenAI, Model: "gpt-4o",
	...
	require.NotNil(t, unifaiErr, "Responses format timeout not enforced!")
	t.Logf("✅ Responses format timeout enforced: %v", unifaiErr.Error)
}
			...
			CreateChatResponseWithToolCalls([]schemas.ChatAssistantMessageToolCall{
				{
					...
					Type: schemas.Ptr("function"),
					Function: schemas.ChatAssistantMessageToolCallFunction{
						Name: schemas.Ptr("unifaiInternal-nonexistent_tool"),
						Arguments: `{}`,
					},
				},
			}),
			...
			CreateChatResponseWithToolCalls([]schemas.ChatAssistantMessageToolCall{
				{
					...
					Type: schemas.Ptr("function"),
					Function: schemas.ChatAssistantMessageToolCallFunction{
						Name: schemas.Ptr("unifaiInternal-nonexistent"),
						Arguments: `{}`,
					},
				},
			}),
