...
func TestAgent_ContextToolFilter_BlockedToolError(t *testing.T) {
	...
	require.NotNil(t, unifaiErr, "Should return error when tool is filtered")
	...
	t.Logf("Error message: %s", unifaiErr.Error.Message)
}
...
func TestAgent_ContextClientFilter_Whitelist(t *testing.T) {
	...
	// Context only allows InProcess client (unifaiInternal)
	...
	manager, mocker, ctx := SetupAgentTestWithClients(t, AgentTestConfig{
		...
		AutoExecuteTools: []string{"*"},
		ClientFiltering:  []string{"unifaiInternal"}, // Only allow InProcess client
		MaxDepth:         5,
...
func TestAgent_ContextToolFilter_EmptyList(t *testing.T) {
	...
	require.NotNil(t, unifaiErr, "Should return error when tool is filtered by empty context")
	require.Nil(t, result)

	// Verify error indicates tool is not permitted
	require.NotNil(t, unifaiErr.Error)
	t.Logf("Error (expected): %s", unifaiErr.Error.Message)
}
...
func TestAgent_ContextClientFilter_MultipleClients(t *testing.T) {
	...
	manager, mocker, ctx := SetupAgentTestWithClients(t, AgentTestConfig{
		InProcessTools:   []string{"echo"},
		AutoExecuteTools: []string{"*"},
		ClientFiltering:  []string{"unifaiInternal", "temperature-mcp-client"}, // Allow 2 clients
		MaxDepth:         10,
...
func TestAgent_ContextToolFilter_ParallelMixed(t *testing.T) {
	...
	if unifaiErr != nil {
		...
		require.NotNil(t, unifaiErr.Error)
		t.Logf("Error returned: %s", unifaiErr.Error.Message)
		require.Nil(t, result)
	} else {
