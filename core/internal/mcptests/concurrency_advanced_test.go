...
func TestConcurrent_CodeModeExecution(t *testing.T) {
	...
	for i := 0; i < numConcurrent; i++ {
		...
		go func(id int) {
			...
			if unifaiErr != nil {
				errors <- fmt.Errorf("execution %d failed: %v", id, unifaiErr.Error.Message)
				return
			}
			if result == nil {
...
func TestConcurrent_CodeModeExecutionWithToolCalls(t *testing.T) {
	...
	for i := 0; i < numConcurrent; i++ {
		...
		go func(id int) {
			...
			switch id % 3 {
			case 0:
				code = fmt.Sprintf(`await unifaiInternal.echo({message: "test-%d"})`, id)
			case 1:
				code = fmt.Sprintf(`await unifaiInternal.calculator({operation: "add", x: %d, y: 10})`, id)
			case 2:
				code = `await unifaiInternal.get_time({timezone: "UTC"})`
			}
	...
	for i := 0; i < 10; i++ {
		...
		go func(id int) {
			...
			for _, client := range clients {
				if client.ExecutionConfig.ID == "unifaiInternal" {
		...
		go func(id int) {
			...
			case 2:
				...
				toolCall = schemas.ChatAssistantMessageToolCall{
					...
					Type: schemas.Ptr("function"),
					Function: schemas.ChatAssistantMessageToolCallFunction{
						Name:      schemas.Ptr("unifaiInternal-get_time"),
						Arguments: toJSON(argsMap),
					},
				}
			case 3:
				...
				toolCall = schemas.ChatAssistantMessageToolCall{
					...
					Type: schemas.Ptr("function"),
					Function: schemas.ChatAssistantMessageToolCallFunction{
						Name:      schemas.Ptr("unifaiInternal-search"),
						Arguments: toJSON(argsMap),
					},
				}
			}
	...
	for i := 0; i < 50; i++ {
		...
		go func(id int) {
			...
			if id%2 == 0 {
				...
				baseCtx = context.WithValue(baseCtx, schemas.MCPContextKeyIncludeTools, []string{"unifaiInternal-*"})
				ctx = schemas.NewUnifAIContext(baseCtx, schemas.NoDeadline)
			} else {
				// Odd: allow only echo
				baseCtx := context.Background()
				baseCtx = context.WithValue(baseCtx, schemas.MCPContextKeyIncludeClients, []string{"*"})
				baseCtx = context.WithValue(baseCtx, schemas.MCPContextKeyIncludeTools, []string{"unifaiInternal-echo"})
				ctx = schemas.NewUnifAIContext(baseCtx, schemas.NoDeadline)
			}
	...
	for i := 0; i < numConcurrent; i++ {
		...
		go func(id int) {
			...
			for time.Now().Before(stopTime) {
				...
				result, unifaiErr := unifai.ExecuteChatMCPTool(ctx, &toolCall)
				if unifaiErr != nil {
					errors <- fmt.Errorf("execution %d-%d failed: %v", id, counter, unifaiErr.Error.Message)
					return
				}
				if result != nil {
					successCount.Add(1)
