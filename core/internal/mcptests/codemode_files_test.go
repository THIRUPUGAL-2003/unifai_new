...
func TestReadToolFile_Basic(t *testing.T) {
	...
	readResult, unifaiErr := unifai.ExecuteChatMCPTool(ctx, &readCall)
	require.Nil(t, unifaiErr)
...
func TestReadToolFile_NotFound(t *testing.T) {
	...
	result, unifaiErr := unifai.ExecuteChatMCPTool(ctx, &readCall)

	// The tool returns an error or informational message about the non-existent file
	if unifaiErr != nil {
		if unifaiErr.Error != nil {
			assert.Contains(t, unifaiErr.Error.Message, "not found")
		}
...
func TestCodeModeFiles_ResponsesFormat(t *testing.T) {
	...
	result, unifaiErr := unifai.ExecuteChatMCPTool(ctx, &listCall)
	require.Nil(t, unifaiErr, "listToolFiles should not error")
...
func TestCodeModeFiles_FullWorkflow(t *testing.T) {
	...
	listResult, unifaiErr := unifai.ExecuteChatMCPTool(ctx, &listCall)
	require.Nil(t, unifaiErr)
...
result = {"completed": True, "servers": ["TestCodeModeServer"], "methodCount": len(server_methods)}`
	...
	execResult, unifaiErr := unifai.ExecuteChatMCPTool(ctx, &execCall)
	require.Nil(t, unifaiErr)
