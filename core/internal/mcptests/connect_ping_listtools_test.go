...
// tools and returns it. The server is independent of the unifai-internal server
...
// setupUnifAIWithPlugins returns a manager + unifai where the manager's plugin
// pipeline is wired to a UnifAI instance carrying the given MCP plugins. Plugins
// fire for any AddClient performed after this returns.
func setupUnifAIWithPlugins(t *testing.T, plugins []schemas.MCPPlugin) (*mcp.MCPManager, *core.UnifAI) {
	t.Helper()
	manager := setupMCPManager(t)
	client, err := core.Init(context.Background(), schemas.UnifAIConfig{
		Account:    &testAccount{},
		MCPPlugins: plugins,
...
func TestConnectHook_FiresOnAddClient(t *testing.T) {
	...
	manager, _ := setupUnifAIWithPlugins(t, []schemas.MCPPlugin{plugin})
...
func TestConnectHook_PreHookShortCircuitError_FailsAddClient(t *testing.T) {
	...
	plugin := NewTestConnectPlugin()
	plugin.SetShortCircuitError(&schemas.UnifAIError{
		IsUnifAIError: false,
...
func TestConnectHook_PreHookShortCircuitResponse_RegistersWithEmptyTools(t *testing.T) {
	...
	plugin := NewTestConnectPlugin()
	plugin.SetShortCircuitResponse(&schemas.UnifAIMCPConnectResponse{
...
func TestConnectHook_OnlyFiresForConnectRequestType(t *testing.T) {
	...
	manager, client := setupUnifAIWithPlugins(t, []schemas.MCPPlugin{connectPlugin, logPlugin})

	// Trigger execute-tool path (no connect involved on the unifai-internal client).
	require.NoError(t, RegisterEchoTool(manager))
	echoCall := GetSampleEchoToolCall("filter_test", "hello")
	_, unifaiErr := client.ExecuteChatMCPTool(createTestContext(), &echoCall)
...
func TestListToolsHook_FiresOnAddClient(t *testing.T) {
	...
	require.NotNil(t, resp.UnifAIMCPListToolsResponse)
...
func TestListToolsHook_PreHookShortCircuitWithSyntheticTools(t *testing.T) {
	...
	plugin := NewTestListToolsPlugin()
	synthetic := &schemas.UnifAIMCPListToolsResponse{
...
func TestPingHook_DoesNotFireWhenPingUnavailable(t *testing.T) {
	...
	manager, _ := setupUnifAIWithPlugins(t, []schemas.MCPPlugin{pingPlugin, listPlugin})
...
func TestMCPGate_AllRequestTypesCarryClientName(t *testing.T) {
	...
	manager, _ := setupUnifAIWithPlugins(t, []schemas.MCPPlugin{logPlugin})
...
func TestMCPGate_NoPluginsConfigured_OpStillRuns(t *testing.T) {
	...
	manager, _ := setupUnifAIWithPlugins(t, []schemas.MCPPlugin{})
