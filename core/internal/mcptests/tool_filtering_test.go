...
func getActualToolsFromGoTestServer(t *testing.T) (tool1, tool2 string) {
	...
	clientConfig := GetGoTestServerConfig(unifaiRoot)
...
func executeToolViaManager(t *testing.T, manager interface {
	...
	_, unifaiErr := manager.ExecuteChatTool(ctx, toolCall)
	if unifaiErr == nil {
		return nil
	}
	if unifaiErr.Error != nil {
		return fmt.Errorf("%s", unifaiErr.Error.Message)
	}
...
func TestFilteringMultipleClients_DifferentRules(t *testing.T) {
	...
	client1 := GetGoTestServerConfig(unifaiRoot)
	...
	client2 := GetEdgeCaseServerConfig(unifaiRoot)
