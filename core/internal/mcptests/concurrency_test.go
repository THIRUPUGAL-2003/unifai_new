...
func TestConcurrent_MultipleToolExecutions(t *testing.T) {
	...
	for i := 0; i < concurrency; i++ {
		...
		go func(id int) {
			...
			if unifaiErr != nil {
				errors <- fmt.Errorf("execution %d failed: %v", id, unifaiErr)
				return
			}
...
func TestConcurrent_DifferentTools(t *testing.T) {
	...
	for i := 0; i < concurrency; i++ {
		...
		go func(id, tType int) {
			...
			if unifaiErr != nil {
				errors <- fmt.Errorf("tool type %d execution %d failed: %v", tType, id, unifaiErr)
				return
			}
...
func TestConcurrent_AddClientDuringExecution(t *testing.T) {
	...
	go func() {
		...
		for i := 0; i < 20; i++ {
			...
			if unifaiErr != nil {
				errors <- fmt.Errorf("execution %d failed: %v", i, unifaiErr)
			}
...
func TestConcurrent_RemoveClientDuringExecution(t *testing.T) {
	...
	go func() {
		...
		for i := 0; i < 10; i++ {
			...
			if unifaiErr != nil {
				t.Logf("Execution %d failed (expected after removal): %v", i, unifaiErr)
			}
			executions <- true
	...
	go func() {
		...
		for i := 0; i < 10; i++ {
			...
			if len(clients) > 0 && len(clients[0].ToolMap) > 0 {
				...
				_, unifaiErr := unifai.ExecuteChatMCPTool(ctx, &toolCall)
				if unifaiErr != nil {
					t.Logf("Execution %d: %v", i, unifaiErr)
				}
			}
			executions <- true
...
func TestConcurrent_HealthCheckDuringExecution(t *testing.T) {
	...
	go func() {
		...
		if unifaiErr != nil {
			errors <- fmt.Errorf("long-running tool failed: %v", unifaiErr)
		} else {
	...
	for i := 0; i < 5; i++ {
		...
		go func(id int) {
			...
			if unifaiErr != nil {
				errors <- fmt.Errorf("health check %d failed: %v", id, unifaiErr)
			}
		}(i)
	}
	...
	for i := 0; i < 10; i++ {
		...
		go func(id int) {
			...
			case 1:
				...
				_, unifaiErr := unifai.ExecuteChatMCPTool(ctx, &toolCall)
				if unifaiErr != nil {
					errors <- fmt.Errorf("calc %d failed: %v", id, unifaiErr)
				}
			case 2:
				toolCall := GetSampleWeatherToolCall(fmt.Sprintf("weather-%d", id), "Tokyo", "celsius")
				_, unifaiErr := unifai.ExecuteChatMCPTool(ctx, &toolCall)
				if unifaiErr != nil {
					errors <- fmt.Errorf("weather %d failed: %v", id, unifaiErr)
				}
			}
			...
			for j := 0; j < 5; j++ {
				...
				default:
					...
					if unifaiErr != nil {
						...
						t.Logf("Execution %d-%d: %v", id, j, unifaiErr)
					}
					time.Sleep(10 * time.Millisecond)
				}
			}
...
func TestConcurrent_PluginHooks(t *testing.T) {
	...
	// Setup UnifAI with plugin
	account := &testAccount{}
	unifaiInstance, err := core.Init(context.Background(), schemas.UnifAIConfig{
		Account:    account,
		MCPPlugins: []schemas.MCPPlugin{loggingPlugin},
		Logger:     core.NewDefaultLogger(schemas.LogLevelInfo),
	})
	require.NoError(t, err)
	unifaiInstance.SetMCPManager(manager)
	...
	for i := 0; i < concurrency; i++ {
		...
		go func(id, tType int) {
			...
			case 0: // Echo
				...
				result, unifaiErr = unifaiInstance.ExecuteChatMCPTool(ctx, &toolCall)
			case 1: // Calculator
				toolCall := GetSampleCalculatorToolCall(fmt.Sprintf("calc-%d", id), "add", float64(id), float64(id+1))
				result, unifaiErr = unifaiInstance.ExecuteChatMCPTool(ctx, &toolCall)
			}
...
func TestConcurrent_MultiplePlugins(t *testing.T) {
	...
	// Setup UnifAI with multiple plugins
	account := &testAccount{}
	unifaiInstance, err := core.Init(context.Background(), schemas.UnifAIConfig{
		Account: account,
	...
	for i := 0; i < concurrency; i++ {
		...
		go func(id, opType int) {
			...
			case 0: // Execute echo tool
				...
				_, unifaiErr := unifai.ExecuteChatMCPTool(ctx, &toolCall)
				if unifaiErr != nil {
					errors <- fmt.Errorf("echo %d failed: %v", id, unifaiErr)
				}
