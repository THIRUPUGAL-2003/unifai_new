...
func TestCodeMode_MultiServer_BasicCalls(t *testing.T) {
	...
	temperatureClient := GetTemperatureMCPClientConfig(unifaiRoot)
	temperatureClient.IsCodeModeClient = true
	temperatureClient.ToolsToExecute = []string{"*"}

	goTestClient := GetGoTestServerConfig(unifaiRoot)
...
echo = unifaiInternal.echo(message="multi-server")
...
`
	...
	require.Nil(t, unifaiErr, "should execute without unifai error")
...
func TestCodeMode_MultiServer_ParallelExecution(t *testing.T) {
	...
	parallelClient := GetParallelTestServerConfig(unifaiRoot)
	parallelClient.ToolsToExecute = []string{"*"}

	edgeClient := GetEdgeCaseServerConfig(unifaiRoot)
