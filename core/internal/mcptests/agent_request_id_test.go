...
func setupMCPManagerWithRequestIDFunc(t *testing.T, fetchNewRequestIDFunc func(ctx *schemas.UnifAIContext) string, clientConfigs ...schemas.MCPClientConfig) *mcp.MCPManager {
	t.Helper()
...
//
// Key concepts:
// - UnifAIContextKeyRequestID: Current request ID (updated each iteration)
// - UnifAIMCPAgentOriginalRequestID: Original request ID (preserved)
...
func TestAgent_RequestID_Propagation(t *testing.T) {
	...
	// Assertions
	require.Nil(t, unifaiErr, "Should complete without error")
	...
	storedOriginalID, ok := ctx.Value(schemas.UnifAIMCPAgentOriginalRequestID).(string)
	...
	currentRequestID, ok := ctx.Value(schemas.UnifAIContextKeyRequestID).(string)
...
func TestAgent_RequestID_MixedAutoAndNonAuto(t *testing.T) {
	...
	// Setup context and mocker
	ctx := createTestContext()
	ctx.SetValue(schemas.UnifAIContextKeyRequestID, "req-mixed-001")
