...
func (m *MockLLMCaller) MakeChatRequest(ctx *schemas.UnifAIContext, req *schemas.UnifAIChatRequest) (*schemas.UnifAIChatResponse, *schemas.UnifAIError) {
...
func (m *MockLLMCaller) MakeResponsesRequest(ctx *schemas.UnifAIContext, req *schemas.UnifAIResponsesRequest) (*schemas.UnifAIResponsesResponse, *schemas.UnifAIError) {
...
func TestAgent_BasicLoop(t *testing.T) {
	...
	// Execute agent mode
	result, unifaiErr := manager.CheckAndExecuteAgentForChatRequest(
		ctx,
		originalReq,
		initialResponse,
	...
	require.Nil(t, unifaiErr, "agent loop should complete successfully")
...
func TestAgent_BasicLoop_ResponsesFormat(t *testing.T) {
	...
	result, unifaiErr := manager.CheckAndExecuteAgentForResponsesRequest(
		ctx,
		originalReq,
		initialResponse,
...
func TestAgent_MixedAutoAndNonAutoTools(t *testing.T) {
	...
	for _, tc := range finalMessage.ChatAssistantMessage.ToolCalls {
		if *tc.Function.Name == "unifaiInternal-calculator" {
			found = true
			break
		}
	}
...
func TestAgent_OnlyNonAutoTools(t *testing.T) {
	...
	require.NotNil(t, finalMessage.ChatAssistantMessage)
	require.NotEmpty(t, finalMessage.ChatAssistantMessage.ToolCalls)
	assert.Equal(t, "unifaiInternal-echo", *finalMessage.ChatAssistantMessage.ToolCalls[0].Function.Name)
}
...
func TestAgent_WithRealLLM_Simple(t *testing.T) {
	...
	result, unifaiErr := unifai.ChatCompletionRequest(ctx, req)
	if unifaiErr != nil {
		// Skip if there's an API error (likely missing/invalid API key or config issue)
		t.Skipf("Skipping real LLM test due to API error: %v", unifaiErr.Error)
	}
	require.NotNil(t, result)
