	...
	for _, tc := range pathTraversalTests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.expectError {
				...
				if unifaiErr != nil {
					assert.Contains(t, unifaiErr.Error.Message, tc.errorMessage)
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.shouldFail {
				...
				if unifaiErr != nil {
					...
					failureReason = fmt.Sprintf("unifaiErr: %v", unifaiErr)
				} else if result != nil && result.Content != nil && result.Content.ContentStr != nil {
				...
				if unifaiErr != nil {
					t.Errorf("%s (%s): unexpected unifaiErr: %v", tc.name, tc.description, unifaiErr)
					return
				}
...
func TestListToolFiles_InputValidation(t *testing.T) {
	...
	result, unifaiErr := unifai.ExecuteChatMCPTool(ctx, &toolCall)
	require.Nil(t, unifaiErr, "listToolFiles should succeed")
...
func TestReadToolFile_LineNumberBoundaries(t *testing.T) {
	...
	listResult, _ := unifai.ExecuteChatMCPTool(ctx, &listCall)
	...
	for _, tc := range boundaryTests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if unifaiErr != nil {
				t.Logf("%s: Got unifai error (acceptable): %v", tc.description, unifaiErr)
			} else {
