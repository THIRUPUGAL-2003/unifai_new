...
// listModelsUnifAIContext returns a context for ListModels. For Replicate, pins the deployments-endpoint
...
// before the API runs.
func listModelsUnifAIContext(parent context.Context, provider schemas.ModelProvider) *schemas.UnifAIContext {
	bfCtx := schemas.NewUnifAIContext(parent, schemas.NoDeadline)
	if provider == schemas.Replicate {
		bfCtx.SetValue(schemas.UnifAIContextKeyAPIKeyName, ReplicateKeyNameListModels)
	}
	return bfCtx
}

// RunListModelsTest executes the list models test scenario
func RunListModelsTest(t *testing.T, client *unifai.UnifAI, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("ListModels", func(t *testing.T) {
		...
		response, unifaiErr := WithListModelsTestRetry(t, listModelsRetryConfig, retryContext, expectations, "ListModels", func() (*schemas.UnifAIListModelsResponse, *schemas.UnifAIError) {
			bfCtx := listModelsUnifAIContext(ctx, testConfig.Provider)
			return client.ListModelsRequest(bfCtx, request)
		})

		if unifaiErr != nil {
			t.Fatalf("❌ List models request failed after retries: %v", GetErrorMessage(unifaiErr))
		}
...
func RunListModelsResponseMarshalTest(t *testing.T, client *unifai.UnifAI, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.ListModels {
		t.Logf("List models not supported for provider %s", testConfig.Provider)
		return
	}
	...
	t.Run("ListModelsResponseMarshal", func(t *testing.T) {
		...
		response, unifaiErr := WithListModelsTestRetry(t, listModelsRetryConfig, retryContext, expectations, "ListModelsResponseMarshal", func() (*schemas.UnifAIListModelsResponse, *schemas.UnifAIError) {
			bfCtx := listModelsUnifAIContext(ctx, testConfig.Provider)
			return client.ListModelsRequest(bfCtx, request)
		})
...
// RunListModelsErrorMarshalTest verifies that the KeyStatus ↔ UnifAIError circular
...
func RunListModelsErrorMarshalTest(t *testing.T, _ *unifai.UnifAI, _ context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.ListModels {
		t.Logf("List models not supported for provider %s", testConfig.Provider)
		return
	}
	...
	t.Run("ListModelsErrorMarshal", func(t *testing.T) {
		...
		// Create the cycle: UnifAIError → ExtraFields.KeyStatuses → KeyStatus → Error → UnifAIError
		unifaiErr.ExtraFields.KeyStatuses = []schemas.KeyStatus{keyStatus}

		// Marshal the UnifAIError (top-level, contains the cycle via KeyStatuses)
		errData, err := schemas.Marshal(unifaiErr)
		if err != nil {
			t.Fatalf("❌ Failed to marshal UnifAIError with circular KeyStatuses: %v", err)
		}
		t.Logf("✅ UnifAIError with circular KeyStatuses marshaled successfully (%d bytes)", len(errData))
...
func RunListModelsPaginationTest(t *testing.T, client *unifai.UnifAI, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.ListModels {
		t.Logf("List models not supported for provider %s", testConfig.Provider)
		return
	}
	...
	t.Run("ListModelsPagination", func(t *testing.T) {
		...
		response, unifaiErr := WithListModelsTestRetry(t, listModelsRetryConfig, retryContext, expectations, "ListModelsPagination", func() (*schemas.UnifAIListModelsResponse, *schemas.UnifAIError) {
			bfCtx := listModelsUnifAIContext(ctx, testConfig.Provider)
			return client.ListModelsRequest(bfCtx, request)
		})

		if unifaiErr != nil {
			t.Fatalf("❌ List models pagination request failed after retries: %v", GetErrorMessage(unifaiErr))
		}
		...
		if response.NextPageToken != "" {
			...
			nextPageRequest := &schemas.UnifAIListModelsRequest{
				Provider:  testConfig.Provider,
				PageSize:  pageSize,
			...
			nextPageResponse, nextPageErr := WithListModelsTestRetry(t, listModelsRetryConfig, nextPageRetryContext, expectations, "ListModelsPagination_NextPage", func() (*schemas.UnifAIListModelsResponse, *schemas.UnifAIError) {
				bfCtx := listModelsUnifAIContext(ctx, testConfig.Provider)
