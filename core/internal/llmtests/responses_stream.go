...
func RunResponsesStreamTest(t *testing.T, client *unifai.UnifAI, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("ResponsesStream", func(t *testing.T) {
		...
		request := &schemas.UnifAIResponsesRequest{
			...
			Params: &schemas.ResponsesParameters{
				MaxOutputTokens: unifai.Ptr(300),
			},
				...
				for {
					...
					case response, ok := <-responseChannel:
						...
						if response.UnifAIResponsesStreamResponse != nil {
							if response.UnifAIResponsesStreamResponse.ExtraFields.Provider != testConfig.Provider {
								t.Logf("⚠️ Warning: Provider mismatch - expected %s, got %s", testConfig.Provider, response.UnifAIResponsesStreamResponse.ExtraFields.Provider)
							}

							// Log latency for each chunk (can be 0 for inter-chunks)
							t.Logf("📊 Chunk %d latency: %d ms", responseCount+1, response.UnifAIResponsesStreamResponse.ExtraFields.Latency)
	...
	if testConfig.Scenarios.Reasoning && testConfig.ReasoningModel != "" {
		t.Run("ResponsesStreamWithReasoning", func(t *testing.T) {
			...
			request := &schemas.UnifAIResponsesRequest{
				...
				Input:    messages,
				Params: &schemas.ResponsesParameters{
					MaxOutputTokens: unifai.Ptr(1800),
					Reasoning: &schemas.ResponsesParametersReasoning{
						Effort: unifai.Ptr("high"),
						// Summary: unifai.Ptr("detailed"),
					},
...
func validateResponsesStreamingResponse(t *testing.T, eventTypes map[schemas.ResponsesStreamResponseType]int, sequenceNumbers []int, finalContent string, lastResponse *schemas.UnifAIStreamChunk, testConfig ComprehensiveTestConfig) StreamingValidationResult {
	var errors []string
	...
	if lastResponse == nil {
		...
		if lastResponse.UnifAIResponsesStreamResponse == nil {
			errors = append(errors, "Last streaming response should have UnifAIResponsesStreamResponse")
		} else {
			if lastResponse.UnifAIResponsesStreamResponse.ExtraFields.Provider != testConfig.Provider {
				errors = append(errors, fmt.Sprintf("Provider mismatch: expected %s, got %s", testConfig.Provider, lastResponse.UnifAIResponsesStreamResponse.ExtraFields.Provider))
			}
		}
	}
	...
	if lastResponse != nil && lastResponse.UnifAIResponsesStreamResponse != nil {
		if lastResponse.UnifAIResponsesStreamResponse.ExtraFields.Latency < 0 {
			errors = append(errors, fmt.Sprintf("Last streaming chunk has negative latency (got %d ms)", lastResponse.UnifAIResponsesStreamResponse.ExtraFields.Latency))
		} else {
			t.Logf("✅ Total streaming latency: %d ms", lastResponse.UnifAIResponsesStreamResponse.ExtraFields.Latency)
		}
	}
