...
func NewOpenAIJudge(client *unifai.UnifAI, judgeModel string, t *testing.T) *OpenAIJudge {
	return &OpenAIJudge{
		client:     client,
...
func (judge *OpenAIJudge) EvaluateResponse(ctx *schemas.UnifAIContext, evaluation EvaluationRequest) (*EvaluationResult, error) {
  ...
  "fatal_issues": ["serious problems"]
	...
	request := &schemas.UnifAIChatRequest{
		...
		Params: &schemas.ChatParameters{
			MaxCompletionTokens: unifai.Ptr(600),
...
func NewOpenAIConversationDriver(client *unifai.UnifAI, driverModel string, t *testing.T) *OpenAIConversationDriver {
...
func (driver *OpenAIConversationDriver) GenerateNextMessage(ctx *schemas.UnifAIContext, request NextMessageRequest) (*GeneratedFollowup, error) {
...
JSON response:
{
  ...
  "test_focus": "what capability this tests"
	...
	llmRequest := &schemas.UnifAIChatRequest{
		Provider: schemas.OpenAI,
...
func RunCrossProviderScenarioTest(t *testing.T, client *unifai.UnifAI, ctx *schemas.UnifAIContext, config CrossProviderTestConfig, scenario CrossProviderScenario, useResponsesAPI bool) {
...
func RunCrossProviderConsistencyTest(t *testing.T, client *unifai.UnifAI, ctx *schemas.UnifAIContext, config CrossProviderTestConfig, useResponsesAPI bool) {
	apiType := "Chat Completions"
	if useResponsesAPI {
		apiType = "Responses API"
	}
...
func executeStepWithProvider(t *testing.T, client *unifai.UnifAI, ctx *schemas.UnifAIContext,
	provider ProviderConfig, history []schemas.ChatMessage, step ScenarioStep, useResponsesAPI bool) (*schemas.UnifAIResponse, *schemas.UnifAIError) {
	...
	if useResponsesAPI {
		...
		return &schemas.UnifAIResponse{ResponsesResponse: responsesResponse}, nil
	} else {
		// Use Chat Completions API
		request := &schemas.UnifAIChatRequest{
			Provider: provider.Provider,
		...
		return &schemas.UnifAIResponse{ChatResponse: chatResponse}, nil
	}
}
