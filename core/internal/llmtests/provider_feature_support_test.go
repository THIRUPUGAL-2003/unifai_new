...
func TestProviderWebSearchVersionSelection(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result, err := anthropic.ToAnthropicResponsesRequest(ctx, unifaiReq)
...
func TestProviderAnthropicRequestPipeline(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Step 2: Convert unifai request → anthropic request
			unifaiReq := &schemas.UnifAIResponsesRequest{
				Provider: tt.provider,
				Model:    model,
				Input: []schemas.ResponsesMessage{
...
// unifai tool → anthropic tool version → correct beta header
func TestComputerUseVersionAndBetaHeaderEndToEnd(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Step 1: Convert unifai tool → anthropic tool (selects version based on model)
			unifaiReq := &schemas.UnifAIResponsesRequest{
				Provider: tt.provider,
				Model:    tt.model,
				Input: []schemas.ResponsesMessage{
