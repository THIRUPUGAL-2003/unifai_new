	...
	return []schemas.ChatTool{
		{
			...
			Function: &schemas.ChatToolFunction{
				...
				Description: unifai.Ptr("Retrieve account information for a given account ID"),
		...
		},
		{
			...
			Function: &schemas.ChatToolFunction{
				...
				Description: unifai.Ptr("Calculate the cost for cloud resource usage based on service type and quantity"),
				Parameters: &schemas.ToolFunctionParameters{
					Type: "object",
					Properties: schemas.NewOrderedMapFromPairs(
		...
		},
		{
			...
			Function: &schemas.ChatToolFunction{
				...
				Description: unifai.Ptr("Check the current status and health of the API service"),
				Parameters: &schemas.ToolFunctionParameters{
					Type: "object",
					Properties: schemas.NewOrderedMapFromPairs(
		...
		},
		{
			...
			Function: &schemas.ChatToolFunction{
				...
				Description: unifai.Ptr("Create a new support ticket for a customer issue"),
				Parameters: &schemas.ToolFunctionParameters{
					Type: "object",
					Properties: schemas.NewOrderedMapFromPairs(
		...
		},
		{
			...
			Function: &schemas.ChatToolFunction{
				...
				Description: unifai.Ptr("Retrieve documentation for a specific service or feature"),
				Parameters: &schemas.ToolFunctionParameters{
					Type: "object",
					Properties: schemas.NewOrderedMapFromPairs(
		...
		},
		{
			...
			Function: &schemas.ChatToolFunction{
				...
				Description: unifai.Ptr("Get a list of available cloud regions for deployment"),
				Parameters: &schemas.ToolFunctionParameters{
					Type: "object",
					Properties: schemas.NewOrderedMapFromPairs(
						schemas.KV("service_type", map[string]interface{}{
		...
		},
		{
			...
			Function: &schemas.ChatToolFunction{
				...
				Description: unifai.Ptr("Estimate the monthly cost for a cloud deployment configuration"),
				Parameters: &schemas.ToolFunctionParameters{
					Type: "object",
					Properties: schemas.NewOrderedMapFromPairs(
		...
		},
		{
			...
			Function: &schemas.ChatToolFunction{
				...
				Description: unifai.Ptr("Validate an API key and return its permissions and status"),
				Parameters: &schemas.ToolFunctionParameters{
					Type: "object",
					Properties: schemas.NewOrderedMapFromPairs(
		...
		},
		{
			...
			Function: &schemas.ChatToolFunction{
				...
				Description: unifai.Ptr("Retrieve usage analytics and metrics for an account"),
				Parameters: &schemas.ToolFunctionParameters{
					Type: "object",
					Properties: schemas.NewOrderedMapFromPairs(
						schemas.KV("account_id", map[string]interface{}{
		...
		},
		{
			...
			Function: &schemas.ChatToolFunction{
				...
				Description: unifai.Ptr("Check compliance status and certifications for security and regulatory requirements"),
				Parameters: &schemas.ToolFunctionParameters{
					Type: "object",
					Properties: schemas.NewOrderedMapFromPairs(
...
// content blocks survives the UnifAI round-trip (Anthropic format -> UnifAI ResponsesMessage -> Provider format).
...
func RunPromptCachingToolBlocksTest(t *testing.T, client *unifai.UnifAI, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("PromptCachingToolBlocks", func(t *testing.T) {
		...
		systemMsg := schemas.ResponsesMessage{
			...
			Role: unifai.Ptr(schemas.ResponsesInputMessageRoleSystem),
			Content: &schemas.ResponsesMessageContent{
				ContentBlocks: []schemas.ResponsesMessageContentBlock{
					{
						Type:         schemas.ResponsesInputMessageContentBlockTypeText,
						Text:         unifai.Ptr(longSharedPrefix),
		...
		userMsg1 := schemas.ResponsesMessage{
			...
			Content: &schemas.ResponsesMessageContent{
				ContentStr: unifai.Ptr("What's the weather in San Francisco?"),
			},
		}

		// Assistant responds with a tool call (function_call with cache_control)
		toolCallMsg := schemas.ResponsesMessage{
			Type:         unifai.Ptr(schemas.ResponsesMessageTypeFunctionCall),
			Status:       unifai.Ptr("completed"),
			CacheControl: cacheControl,
			ResponsesToolMessage: &schemas.ResponsesToolMessage{
				CallID:    unifai.Ptr("call_weather_001"),
				Name:      unifai.Ptr("weather"),
				Arguments: unifai.Ptr(`{"location":"San Francisco"}`),
			},
		}
		...
		toolResultMsg := schemas.ResponsesMessage{
			...
			ResponsesToolMessage: &schemas.ResponsesToolMessage{
				...
				Output: &schemas.ResponsesToolMessageOutputStruct{
					ResponsesToolCallOutputStr: unifai.Ptr(`{"temperature": 18, "unit": "celsius", "condition": "partly cloudy", "humidity": 72}`),
				},
			},
		}
		...
		userMsg2 := schemas.ResponsesMessage{
			...
			Role: unifai.Ptr(schemas.ResponsesInputMessageRoleUser),
			Content: &schemas.ResponsesMessageContent{
				ContentStr: unifai.Ptr("Summarize the weather information you received."),
			},
		}
		...
		// (per unifai.go:5541 — opt-in gate added by the per-request-overrides feature).
		...
		operation := func() (*schemas.UnifAIResponsesResponse, *schemas.UnifAIError) {
			bfCtx := schemas.NewUnifAIContext(rawCtx, schemas.NoDeadline)
		...
		require.NotNil(t, rawReq, "Raw request should be present (UnifAIContextKeySendBackRawRequest was set)")
...
// Vertex caching bug where UnifAI's key reordering in tool_use input fields caused
...
func RunPromptCachingMultipleToolCallsTest(t *testing.T, client *unifai.UnifAI, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.PromptCaching {
	...
	t.Run("PromptCachingMultipleToolCalls", func(t *testing.T) {
		...
		makeToolCall := func(callID, name, args string, withCacheControl bool) schemas.ResponsesMessage {
			msg := schemas.ResponsesMessage{
				...
				Status: unifai.Ptr("completed"),
				ResponsesToolMessage: &schemas.ResponsesToolMessage{
					CallID:    unifai.Ptr(callID),
					Name:      unifai.Ptr(name),
					Arguments: unifai.Ptr(args),
				},
			}
			if withCacheControl {
		...
		makeToolResult := func(callID, output string, withCacheControl bool) schemas.ResponsesMessage {
			msg := schemas.ResponsesMessage{
				...
				ResponsesToolMessage: &schemas.ResponsesToolMessage{
					CallID: unifai.Ptr(callID),
					Output: &schemas.ResponsesToolMessageOutputStruct{
						ResponsesToolCallOutputStr: unifai.Ptr(output),
					},
				},
			}
			if withCacheControl {
		...
		// If UnifAI re-orders these keys, the cache prefix diverges at this point.
...
func RunPromptCachingTest(t *testing.T, client *unifai.UnifAI, ctx context.Context, testConfig ComprehensiveTestConfig) {
		...
		for i, query := range testQueries {
			t.Run(query.name, func(t *testing.T) {
				userMessage := schemas.ChatMessage{
					...
					Content: &schemas.ChatMessageContent{
						ContentStr: unifai.Ptr(query.message),
					},
				}
				...
				chatReq := &schemas.UnifAIChatRequest{
					...
					Params: &schemas.ChatParameters{
						...
						ToolChoice: &schemas.ChatToolChoice{
							ChatToolChoiceStr: unifai.Ptr("auto"),
						},
					},
...
func RunPromptCachingMultiTurnTest(t *testing.T, client *unifai.UnifAI, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.SimpleChat {
			...
			t.Run(turnName, func(t *testing.T) {
				...
				chatReq := &schemas.UnifAIChatRequest{
					...
					Params: &schemas.ChatParameters{
						Tools: tools,
						ToolChoice: &schemas.ChatToolChoice{
							ChatToolChoiceStr: unifai.Ptr("none"),
						},
					},
					Fallbacks: testConfig.Fallbacks,
				}
