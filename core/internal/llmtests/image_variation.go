...
func RunImageVariationTest(t *testing.T, client *unifai.UnifAI, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("ImageVariation", func(t *testing.T) {
		...
		imageVariationOperation := func() (*schemas.UnifAIImageGenerationResponse, *schemas.UnifAIError) {
			request := &schemas.UnifAIImageVariationRequest{
				...
				Params: &schemas.ImageVariationParameters{
					...
					N:    unifai.Ptr(2), // Generate 2 variations
				},
				Fallbacks: testConfig.ImageVariationFallbacks,
			}

			response, err := client.ImageVariationRequest(schemas.NewUnifAIContext(ctx, schemas.NoDeadline), request)
			if err != nil {
				return nil, err
			}
			if response != nil {
...
func RunImageVariationStreamTest(t *testing.T, client *unifai.UnifAI, ctx context.Context, testConfig ComprehensiveTestConfig) {
