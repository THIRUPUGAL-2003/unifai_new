...
// basePassthroughChatRequest returns a minimal UnifAIChatRequest suitable for
// conversion into a provider-native passthrough body.
func basePassthroughChatRequest(model string) *schemas.UnifAIChatRequest {
...
// buildPassthroughChatReq converts a minimal UnifAIChatRequest into the
...
func buildPassthroughChatReq(t *testing.T, provider schemas.ModelProvider, model string, stream bool) (passthroughChatReq, bool) {
	...
	case schemas.OpenAI:
		...
		if stream {
			nativeReq.Stream = unifai.Ptr(true)
		}
...
// RunPassthroughAPITest exercises UnifAI's raw HTTP passthrough API for the
...
func RunPassthroughAPITest(t *testing.T, client *unifai.UnifAI, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("PassthroughAPI", func(t *testing.T) {
		t.Run("NonStream", func(t *testing.T) {
			...
			resp, unifaiErr := client.Passthrough(bfCtx, testConfig.Provider, &schemas.UnifAIPassthroughRequest{
				Method:   "POST",
				Path:     req.path,
				Body:     req.body,
				RawQuery: req.query,
			...
			if unifaiErr != nil {
				t.Fatalf("❌ Passthrough request failed: %s", GetErrorMessage(unifaiErr))
			}
			if resp == nil {
		...
		t.Run("Stream", func(t *testing.T) {
			...
			ch, unifaiErr := client.PassthroughStream(bfCtx, testConfig.Provider, &schemas.UnifAIPassthroughRequest{
				Method:   "POST",
				Path:     req.path,
				Body:     req.body,
				RawQuery: req.query,
			...
			if unifaiErr != nil {
				t.Fatalf("❌ PassthroughStream failed: %s", GetErrorMessage(unifaiErr))
			}
			if ch == nil {
			...
			for chunk := range ch {
				...
				if chunk.UnifAIError != nil {
					t.Fatalf("❌ Stream chunk contained error: %s", GetErrorMessage(chunk.UnifAIError))
				}
				if chunk.UnifAIPassthroughResponse != nil {
					totalBytes += len(chunk.UnifAIPassthroughResponse.Body)
					if len(chunk.UnifAIPassthroughResponse.Body) > 0 {
						chunkCount++
					}
				}
			}
