...
func RunImageEditTest(t *testing.T, client *unifai.UnifAI, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("ImageEdit", func(t *testing.T) {
		...
		imageEditOperation := func() (*schemas.UnifAIImageGenerationResponse, *schemas.UnifAIError) {
			request := &schemas.UnifAIImageEditRequest{
				...
				Params: &schemas.ImageEditParameters{
					...
					N:    unifai.Ptr(1),
					Type: unifai.Ptr("inpainting"),
					Mask: maskBytes,
				},
				Fallbacks: testConfig.ImageEditFallbacks,
			}

			response, err := client.ImageEditRequest(schemas.NewUnifAIContext(ctx, schemas.NoDeadline), request)
			if err != nil {
				return nil, err
			}
			if response != nil {
...
func RunImageEditStreamTest(t *testing.T, client *unifai.UnifAI, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("ImageEditStream", func(t *testing.T) {
		...
		validationResult := WithImageGenerationStreamRetry(
			...
			func() (chan *schemas.UnifAIStreamChunk, *schemas.UnifAIError) {
				return client.ImageEditStreamRequest(schemas.NewUnifAIContext(streamCtx, schemas.NoDeadline), request)
			},
