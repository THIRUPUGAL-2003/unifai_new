...
// BuildMCPCallbackBaseURL extracts the base URL set on the UnifAIContext by
...
func BuildMCPCallbackBaseURL(ctx *schemas.UnifAIContext) string {
	if base, ok := ctx.Value(schemas.UnifAIContextKeyMCPCallbackBaseURL).(string); ok && base != "" {
...
func BuildOAuthRedirectURIFromContext(ctx *schemas.UnifAIContext) string {
...
// in the UnifAIContext (UnifAIContextKeyMCPExtraHeaders), scoped by the
...
func ExtractFilteredExtras(ctx context.Context, config *schemas.MCPClientConfig) http.Header {
	...
	extraHeaders, ok := ctx.Value(schemas.UnifAIContextKeyMCPExtraHeaders).(map[string][]string)
	if !ok {
		return headers
	}
