...
func (m *mockToolClientManager) AcquireClientConn(ctx *schemas.UnifAIContext, state *schemas.MCPClientState) (*client.Client, func(), error) {
	return nil, func() {}, nil
}
func (m *mockToolClientManager) RunWithPluginPipeline(ctx *schemas.UnifAIContext, req *schemas.UnifAIMCPRequest, op MCPOpFunc) (*schemas.UnifAIMCPResponse, *schemas.UnifAIError) {
	resp, err := op(req)
	if err != nil {
		return nil, &schemas.UnifAIError{IsUnifAIError: false, Error: &schemas.ErrorField{Message: err.Error()}}
	}
	return resp, nil
}
...
func toolNamesFromChatRequest(req *schemas.UnifAIRequest) []string {
...
func toolNamesFromResponsesRequest(req *schemas.UnifAIRequest) []string {
...
// contextWithUserAgent creates a UnifAIContext with UnifAIContextKeyUserAgent set.
func contextWithUserAgent(ua string) *schemas.UnifAIContext {
	ctx := schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline)
	ctx.SetValue(schemas.UnifAIContextKeyUserAgent, ua)
	return ctx
}
...
func TestBuildIntegrationDuplicateCheckMap_ClaudeCLI_StripsPrefix(t *testing.T) {
	...
	// UnifAI MCP server. The prefix is mcp__{server}__{tool_name}.
	tools := []schemas.ChatTool{
		makeTool("mcp__unifai__echo"),
		makeTool("mcp__unifai__calculator"),
	}

	m := buildIntegrationDuplicateCheckMap(tools, schemas.ClaudeCLI.String(), defaultLogger)

	// Bare tool names must be recognised so UnifAI does not inject them again.
	for _, want := range []string{"echo", "calculator"} {
		if !m[want] {
	...
	for _, want := range []string{"mcp__unifai__echo", "mcp__unifai__calculator"} {
		if !m[want] {
...
func TestBuildIntegrationDuplicateCheckMap_ClaudeCLI_MultipleServers(t *testing.T) {
	...
	tools := []schemas.ChatTool{
		makeTool("mcp__unifai__executeToolCode"),
		makeTool("mcp__unifai__listToolFiles"),
...
func TestBuildIntegrationDuplicateCheckMap_ClaudeCLI_MixedPrefixedAndBare(t *testing.T) {
	...
	for _, want := range []string{"echo", "mcp__unifai__echo", "search"} {
		if !m[want] {
...
func TestBuildIntegrationDuplicateCheckMap_GeminiCLI_PrefixedTools_StripsPrefix(t *testing.T) {
	...
	tools := []schemas.ChatTool{
		makeTool("mcp_unifai_echo"),
		makeTool("mcp_unifai_calculator"),
	}
...
// dual-gateway scenario: UnifAI stores tools as "{client}-{tool}" and Gemini CLI
...
func TestBuildIntegrationDuplicateCheckMap_GeminiCLI_ClientPrefixedTools(t *testing.T) {
	...
	tools := []schemas.ChatTool{
		makeTool("mcp_unifai_testing_exa-web_fetch_exa"),
		makeTool("mcp_unifai_testing_exa-web_search_exa"),
		makeTool("mcp_unifai_testing_websets-cancel_enrichment"),
		makeTool("mcp_unifai_ctx7-resolve-library-id"),
	}
...
// hyphens in the original UnifAI tool name are converted to underscores. The dedup
...
func TestBuildIntegrationDuplicateCheckMap_CodexCLI_ClientPrefixedTools(t *testing.T) {
	...
	tools := []schemas.ChatTool{
		makeTool("mcp__unifai__testing_exa_web_fetch_exa"),
		makeTool("mcp__unifai__testing_exa_web_search_exa"),
		makeTool("mcp__unifai__testing_websets_cancel_enrichment"),
		makeTool("mcp__unifai__ctx7_query_docs"),
	}

	m := buildIntegrationDuplicateCheckMap(tools, schemas.CodexCLI.String(), defaultLogger)

	// All-underscore forms must be in the map (callers normalize UnifAI names before lookup).
	for _, want := range []string{
...
// underscores). Strip "mcp__" then skip past first "__" to get the UnifAI tool name.
func TestBuildIntegrationDuplicateCheckMap_QwenCLI_ClientPrefixedTools(t *testing.T) {
	t.Parallel()

	tools := []schemas.ChatTool{
		makeTool("mcp__unifai__testing_exa-web_fetch_exa"),
		makeTool("mcp__unifai__testing_exa-web_search_exa"),
		makeTool("mcp__unifai__testing_websets-cancel_enrichment"),
		makeTool("mcp__unifai__ctx7-resolve-library-id"),
	}
...
// buildChatRequest builds a minimal UnifAIChatRequest with the given pre-existing
// tool names already populated in Params.Tools.
func buildChatRequest(existingToolNames ...string) *schemas.UnifAIRequest {
...
func TestParseAndAddToolsToRequest_NoUserAgent_NoDuplicate(t *testing.T) {
	...
	// Request already has "echo" — UnifAI should not add it again.
...
func TestParseAndAddToolsToRequest_ClaudeCLI_NoDuplicate(t *testing.T) {
	...
	// Available MCP tools registered on the UnifAI server.
	cm := &mockToolClientManager{
		tools: []schemas.ChatTool{
			makeTool("echo"),
			makeTool("calculator"),
	...
	req := buildChatRequest("mcp__unifai__echo", "mcp__unifai__calculator")
...
func TestParseAndAddToolsToRequest_ClaudeCLI_NewToolsInjected(t *testing.T) {
	...
	// UnifAI should inject search (not already present in any form).
	cm := &mockToolClientManager{
		tools: []schemas.ChatTool{
			makeTool("echo"),
			makeTool("search"),
	...
	req := buildChatRequest("mcp__unifai__echo")
	ctx := contextWithUserAgent(schemas.ClaudeCLI.String())
...
// buildResponsesRequest creates a minimal UnifAIResponsesRequest with the given
// pre-existing tool names already populated in Params.Tools.
func buildResponsesRequest(existingToolNames ...string) *schemas.UnifAIRequest {
...
func TestParseAndAddToolsToRequest_ResponsesAPI_ClaudeCLI_NoDuplicate(t *testing.T) {
	...
	req := buildResponsesRequest("mcp__unifai__echo")
	ctx := contextWithUserAgent(schemas.ClaudeCLI.String())
	...
	if countOccurrences(names, "mcp__unifai__echo") != 1 {
		t.Errorf("Responses/ClaudeCLI: prefixed name should remain exactly once, got %d",
			countOccurrences(names, "mcp__unifai__echo"))
	}
}
...
// single underscore separator, hyphens in the UnifAI tool name are preserved).
// Strip up to and including the first "_" to recover the UnifAI tool name.
func TestBuildIntegrationDuplicateCheckMap_OpenCode_ClientPrefixedTools(t *testing.T) {
	t.Parallel()

	tools := []schemas.ChatTool{
		makeTool("unifai_testing_exa-web_fetch_exa"),
		makeTool("unifai_testing_exa-web_search_exa"),
		makeTool("unifai_testing_websets-cancel_enrichment"),
		makeTool("unifai_ctx7-query-docs"),
		makeTool("unifai_filesystem-create_directory"),
	}
	...
	for _, want := range []string{
		"unifai_testing_exa-web_fetch_exa",
		"unifai_ctx7-query-docs",
	} {
		if !m[want] {
...
func TestParseAndAddToolsToRequest_OpenCode_NoDuplicate(t *testing.T) {
	...
	req := buildChatRequest(
		"unifai_testing_exa-web_fetch_exa",
		"unifai_ctx7-query-docs",
		"unifai_filesystem-create_directory",
	)
	...
	// Bare UnifAI names must NOT be injected again — they're covered by prefixed entries.
