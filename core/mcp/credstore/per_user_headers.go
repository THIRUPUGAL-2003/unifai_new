...
func (r *perUserHeadersResolver) ConnectionHeaders(ctx *schemas.UnifAIContext, config *schemas.MCPClientConfig) (http.Header, error) {
	if r.provider == nil {
	...
	if identity == "" {
		return nil, fmt.Errorf(
			"per-user headers for %s requires an identity: send a Virtual Key (x-uf-vk), authenticate as a user, or set x-uf-mcp-session-id to any opaque string you'll re-send on subsequent calls",
			config.Name,
		)
	}
...
// session.
func (r *perUserHeadersResolver) buildAuthRequiredError(ctx *schemas.UnifAIContext, config *schemas.MCPClientConfig) error {
