...
func TestToOpenAITextCompletionRequest_FireworksUsesCacheIsolation(t *testing.T) {
	...
	result := ToOpenAITextCompletionRequest(unifaiReq)
	if result == nil {
	...
	if _, ok := unifaiReq.Params.ExtraParams["prompt_cache_key"]; !ok {
		t.Fatalf("expected original extra params to remain unchanged, got %#v", unifaiReq.Params.ExtraParams)
	}
	...
	wireBody, unifaiErr := providerUtils.CheckContextAndGetRequestBody(
		...
		func() (providerUtils.RequestBodyWithExtraParams, error) {
			return ToOpenAITextCompletionRequest(unifaiReq), nil
		},
	)
	if unifaiErr != nil {
