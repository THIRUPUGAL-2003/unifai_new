...
func TestNormalizeRealtimeClientSecretRequest(t *testing.T) {
	...
	body, model, unifaiErr := NormalizeRealtimeClientSecretRequest(
		json.RawMessage(`{"model":"openai/gpt-4o-realtime-preview","voice":"alloy"}`),
		schemas.OpenAI,
		schemas.RealtimeSessionEndpointClientSecrets,
	)
	if unifaiErr != nil {
		t.Fatalf("NormalizeRealtimeClientSecretRequest() error = %v", unifaiErr)
	}
...
func TestToProviderRealtimeEventSerializesTopLevelClientFields(t *testing.T) {
	...
	out, err := provider.ToProviderRealtimeEvent(&schemas.UnifAIRealtimeEvent{
...
func TestToUnifAIRealtimeEventParsesTopLevelClientFields(t *testing.T) {
	t.Parallel()

	provider := &OpenAIProvider{}
	event, err := provider.ToUnifAIRealtimeEvent(json.RawMessage(`{"type":"conversation.item.truncate","item_id":"item_123","content_index":0,"audio_end_ms":640}`))
	if err != nil {
		t.Fatalf("ToUnifAIRealtimeEvent() error = %v", err)
	}
	var itemID string
...
func TestToUnifAIRealtimeEventParsesCompletedInputAudioTranscript(t *testing.T) {
	t.Parallel()

	provider := &OpenAIProvider{}
	event, err := provider.ToUnifAIRealtimeEvent(json.RawMessage(`{"type":"conversation.item.input_audio_transcription.completed","event_id":"evt_123","item_id":"item_123","content_index":0,"transcript":"Who are you?"}`))
	if err != nil {
		t.Fatalf("ToUnifAIRealtimeEvent() error = %v", err)
	}
...
func TestToUnifAIRealtimeEventParsesModernOutputTextDelta(t *testing.T) {
	t.Parallel()

	provider := &OpenAIProvider{}
	event, err := provider.ToUnifAIRealtimeEvent(json.RawMessage(`{
...
func TestShouldStartRealtimeTurn(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			event: &schemas.UnifAIRealtimeEvent{Type: schemas.RTEventResponseCreate},
			want:  true,
		},
		{
			name:  "audio buffer committed starts turn",
			event: &schemas.UnifAIRealtimeEvent{Type: schemas.RTEventInputAudioBufferCommitted},
			want:  true,
		},
		{
			name:  "response done does not start turn",
			event: &schemas.UnifAIRealtimeEvent{Type: schemas.RTEventResponseDone},
			want:  false,
		},
		{
...
func TestToUnifAIRealtimeEventPreservesTopLevelResponsePayload(t *testing.T) {
	t.Parallel()
...
func TestToUnifAIRealtimeEventPreservesTopLevelPartPayload(t *testing.T) {
	t.Parallel()
...
func TestInputAudioBufferAppendRoundtrip(t *testing.T) {
	...
	unifaiEvent, err := schemas.ParseRealtimeEvent(clientJSON)
	if err != nil {
		t.Fatalf("ParseRealtimeEvent() error = %v", err)
	}
	if len(unifaiEvent.Audio) == 0 {
		t.Fatal("unifaiEvent.Audio is empty after ParseRealtimeEvent")
	}

	// Step 2: Convert back to provider format.
	provider := &OpenAIProvider{}
	providerJSON, err := provider.ToProviderRealtimeEvent(unifaiEvent)
	if err != nil {
		t.Fatalf("ToProviderRealtimeEvent() error = %v", err)
	}
