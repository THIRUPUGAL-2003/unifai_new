...
func TestParseTranscriptionFormDataBodyFromRequest_OrdersMetadataBeforeFile(t *testing.T) {
	...
	if unifaiErr := ParseTranscriptionFormDataBodyFromRequest(writer, req, schemas.OpenAI); unifaiErr != nil {
		t.Fatalf("unexpected unifai error: %v", unifaiErr.Error.Message)
	}
