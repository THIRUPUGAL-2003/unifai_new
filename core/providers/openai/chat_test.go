...
func TestToOpenAIChatRequest_ToolNormalization(t *testing.T) {
	...
	defer cancel()
	result := ToOpenAIChatRequest(ctx, unifaiReq)
	if result == nil {
	...
	// Verify original unifaiReq.Params.Tools was NOT mutated
	origKeys := unifaiReq.Params.Tools[0].Function.Parameters.Properties.Keys()
	...
	if result.ChatParameters.Tools[0].Function == unifaiReq.Params.Tools[0].Function {
...
func TestToOpenAIChatRequest_PreservesN(t *testing.T) {
	...
	out := ToOpenAIChatRequest(schemas.NewUnifAIContext(nil, schemas.NoDeadline), req)
	if out == nil {
...
func TestToOpenAIChatRequest_FireworksPreservesReasoningAndCacheIsolation(t *testing.T) {
	...
	wireBody, unifaiErr := providerUtils.CheckContextAndGetRequestBody(
		...
		func() (providerUtils.RequestBodyWithExtraParams, error) {
			return ToOpenAIChatRequest(ctx, unifaiReq), nil
		},
	)
	if unifaiErr != nil {
		t.Fatalf("failed to build request body: %v", unifaiErr.Error.Message)
	}
...
func TestToOpenAIChatRequest_CacheControl_OpenRouterOnly(t *testing.T) {
	makeReq := func(provider schemas.ModelProvider) *schemas.UnifAIChatRequest {
		return &schemas.UnifAIChatRequest{
			Provider: provider,
...
// ToUnifAIChatRequest) and asserts the top-level server-tool name survives.
func TestOpenAIInbound_ServerToolNameSurvives(t *testing.T) {
	...
	bifReq := req.ToUnifAIChatRequest(ctx)
	if bifReq.Params == nil || len(bifReq.Params.Tools) != 1 || bifReq.Params.Tools[0].Name != "bash" {
		t.Fatalf("ToUnifAIChatRequest dropped name: %+v", bifReq.Params)
	}
}
