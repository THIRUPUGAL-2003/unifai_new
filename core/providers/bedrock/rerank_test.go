...
func TestToBedrockRerankRequest(t *testing.T) {
	...
	req, err := ToBedrockRerankRequest(&schemas.UnifAIRerankRequest{
...
func TestBedrockRerankResponseToUnifAIRerankResponse(t *testing.T) {
	...
	}).ToUnifAIRerankResponse(nil, false)
...
func TestBedrockRerankResponseToUnifAIRerankResponseReturnDocuments(t *testing.T) {
	...
	}).ToUnifAIRerankResponse(requestDocs, true)
...
func TestBedrockRerankRequestToUnifAIRerankRequest(t *testing.T) {
	topN := 3
	...
	result := bedrockReq.ToUnifAIRerankRequest(unifaiCtx)

	require.NotNil(t, result)
	// A Bedrock rerank ModelARN has no UnifAI provider prefix, so ParseModelString
...
func TestBedrockRerankRequestToUnifAIRerankRequestNil(t *testing.T) {
	var req *BedrockRerankRequest
	assert.Nil(t, req.ToUnifAIRerankRequest(nil))
}
...
func TestBedrockRerankRequiresARNModelIdentifier(t *testing.T) {
	...
	response, unifaiErr := provider.Rerank(ctx, key, &schemas.UnifAIRerankRequest{
		Model: "cohere-rerank",
		Query: "capital of france",
		Documents: []schemas.RerankDocument{
			{Text: "Paris is the capital of France."},
	...
	assert.Contains(t, unifaiErr.Error.Message, "requires an ARN")
}
