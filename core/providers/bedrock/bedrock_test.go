...
import (
	...
	"github.com/unifai/unifai/core/internal/llmtests"
...
func TestBedrock(t *testing.T) {
	...
	// takes: Anthropic SDK -> /v1/messages -> ToUnifAIResponsesRequest ->
	...
	t.Run("BedrockOpus47Tests", func(t *testing.T) {
		t.Run("TestBedrockOpus47StructuredOutputRegression", func(t *testing.T) {
			...
			// (transports/unifai-http/integrations/anthropic.go RequestConverter
			// at lines 92-100 calls anthropicReq.ToUnifAIResponsesRequest(ctx)).
			reqCtx := schemas.NewUnifAIContext(ctx, schemas.NoDeadline)
			unifaiReq := anthropicReq.ToUnifAIResponsesRequest(reqCtx)
			require.NotNil(t, unifaiReq, "ToUnifAIResponsesRequest returned nil")
			unifaiReq.Provider = schemas.Bedrock
			unifaiReq.Model = modelID

			// Send. NO UnifAIContextKeyExtraHeaders — this is the key delta
			...
			resp, unifaiErr := client.ResponsesRequest(reqCtx, unifaiReq)
			...
			if unifaiErr != nil {
				...
				t.Fatalf("Bedrock Opus 4.7 structured-output request failed (this is the regression repro): %s", llmtests.GetErrorMessage(unifaiErr))
			}
...
// TestUnifAIToBedrockRequestConversion tests the conversion from UnifAI request to Bedrock request
func TestUnifAIToBedrockRequestConversion(t *testing.T) {
	...
	trace := testTrace
	latency := testLatency
	serviceTier := schemas.UnifAIServiceTierPriority
	props := testProps

	tests := []struct {
		name     string
		input    *schemas.UnifAIChatRequest
...
// TestBedrockToUnifAIRequestConversion tests the conversion from Bedrock request to UnifAI request
func TestBedrockToUnifAIRequestConversion(t *testing.T) {
	maxTokens := testMaxTokens
	temp := testTemp
	topP := testTopP
	trace := testTrace
	...
	tests := []struct {
		...
		expected *schemas.UnifAIResponsesRequest
		wantErr  bool
	}{
		{
			name: "BasicTextMessage",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			var actual *schemas.UnifAIResponsesRequest
			var err error
			if tt.input == nil {
				var bedrockReq *bedrock.BedrockConverseRequest
				actual, err = bedrockReq.ToUnifAIResponsesRequest(ctx)
			} else {
				actual, err = tt.input.ToUnifAIResponsesRequest(ctx)
			}
			if tt.wantErr {
				assert.Error(t, err)
				assert.Nil(t, actual)
...
// TestUnifAIToBedrockResponseConversion tests the conversion from UnifAI Responses response to Bedrock response
func TestUnifAIToBedrockResponseConversion(t *testing.T) {
	inputTokens := 10
	outputTokens := 20
	totalTokens := 30
	latency := int64(100)
	...
	tests := []struct {
		name     string
		input    *schemas.UnifAIResponsesResponse
...
// TestBedrockToUnifAIResponseConversion tests the conversion from Bedrock response to UnifAI Responses response
func TestBedrockToUnifAIResponseConversion(t *testing.T) {
	inputTokens := 10
	outputTokens := 20
	totalTokens := 30
	toolUseID := "call-123"
	...
	tests := []struct {
		...
		expected *schemas.UnifAIResponsesResponse
		wantErr  bool
	}{
		{
			name: "BasicTextResponse",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			var actual *schemas.UnifAIResponsesResponse
			var err error
			if tt.input == nil {
				var bedrockResp *bedrock.BedrockConverseResponse
				actual, err = bedrockResp.ToUnifAIResponsesResponse(ctx)
			} else {
				actual, err = tt.input.ToUnifAIResponsesResponse(ctx)
			}
			if tt.wantErr {
				assert.Error(t, err)
				assert.Nil(t, actual)
...
// TestBedrockToUnifAIResponse_TraceStoredInProviderExtraFields verifies that
// ToUnifAIResponsesResponse carries guardrail trace in ProviderExtraFields.
func TestBedrockToUnifAIResponse_TraceStoredInProviderExtraFields(t *testing.T) {
	action := "BLOCKED"
	ctx := schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline)
	...
	unifaiResp, err := input.ToUnifAIResponsesResponse(ctx)
	require.NoError(t, err)
	require.NotNil(t, unifaiResp)

	require.NotNil(t, unifaiResp.ProviderExtraFields, "ProviderExtraFields must be set when trace is present")
	traceData, ok := unifaiResp.ProviderExtraFields["trace"]
...
// TestUnifAIToBedrockResponse_TraceRestoredFromProviderExtraFields verifies that
// ToBedrockConverseResponse restores trace from ProviderExtraFields (round-trip).
func TestUnifAIToBedrockResponse_TraceRestoredFromProviderExtraFields(t *testing.T) {
...
//	  → ToUnifAIResponsesRequest  (stored as ExtraParams["guardrailConfig"])
...
func TestGuardrailConfigRequestRoundTrip(t *testing.T) {
	...
	// Step 1: BedrockConverseRequest → UnifAIResponsesRequest
	unifaiReq, err := original.ToUnifAIResponsesRequest(ctx)
	require.NoError(t, err)
	require.NotNil(t, unifaiReq.Params)
	require.NotNil(t, unifaiReq.Params.ExtraParams)
	_, hasGuardrail := unifaiReq.Params.ExtraParams["guardrailConfig"]
	require.True(t, hasGuardrail, "guardrailConfig must be present in ExtraParams after ToUnifAIResponsesRequest")

	// Step 2: UnifAIResponsesRequest → BedrockConverseRequest
	result, err := bedrock.ToBedrockResponsesRequest(ctx, unifaiReq)
	require.NoError(t, err)
	require.NotNil(t, result.GuardrailConfig, "GuardrailConfig must survive the round-trip through UnifAI")
...
//	  → ToUnifAIResponsesResponse  (stored in ProviderExtraFields["trace"])
...
// by ToUnifAIResponsesResponse, making it invisible to callers.
func TestGuardrailTraceResponseRoundTrip(t *testing.T) {
	...
	t.Run("ConversePath_ActionReason", func(t *testing.T) {
		...
		unifaiResp, err := original.ToUnifAIResponsesResponse(ctx)
		require.NoError(t, err)
		require.NotNil(t, unifaiResp.ProviderExtraFields)
		_, hasTrace := unifaiResp.ProviderExtraFields["trace"]
		require.True(t, hasTrace)

		result, err := bedrock.ToBedrockConverseResponse(unifaiResp)
		require.NoError(t, err)
...
func TestToBedrockResponsesRequest_AnthropicTextFormatUsesOutputConfig(t *testing.T) {
	...
	// (rejected by Opus 4.7) onto the synthetic uf_so_* tool path used by all
	...
	for _, tool := range bedrockReq.ToolConfig.Tools {
		if tool.ToolSpec != nil && strings.HasPrefix(tool.ToolSpec.Name, "uf_so_") {
			foundSyntheticTool = true
			break
		}
	}
	require.True(t, foundSyntheticTool, "expected synthetic uf_so_* tool for structured output")
}
...
func TestToBedrockResponsesRequest_NonAnthropicTextFormatStillUsesToolConversion(t *testing.T) {
	...
	assert.Contains(t, bedrockReq.ToolConfig.ToolChoice.Tool.Name, "uf_so_", "expected forced tool choice to target the synthetic structured output tool")
}
...
func TestToBedrockResponsesRequest_NonAnthropicTextFormatPreservedWithUserTools(t *testing.T) {
	...
	require.NotNil(t, bedrockReq.ToolConfig.ToolChoice, "expected structured output tool choice to be forced")
	require.NotNil(t, bedrockReq.ToolConfig.ToolChoice.Tool, "expected structured output tool choice to target the synthetic tool")
	assert.Equal(t, "uf_so_classification", bedrockReq.ToolConfig.ToolChoice.Tool.Name)
	assert.Equal(t, "uf_so_classification", bedrockReq.ToolConfig.Tools[0].ToolSpec.Name)
...
// UnifAI-hosted `mcp` server tool alongside function tools must no longer fail
	...
	req := &schemas.UnifAIResponsesRequest{
		...
		Params: &schemas.ResponsesParameters{
			Tools: []schemas.ResponsesTool{
				...
				{
					// UnifAI-hosted MCP server tool — server_url points back at
					// UnifAI itself; Bedrock's Converse API can't consume it.
					Type: schemas.ResponsesToolTypeMCP,
					ResponsesToolMCP: &schemas.ResponsesToolMCP{
						ServerLabel: "mongodb",
						ServerURL:   schemas.Ptr("https://unifai.example.com/mcp/mongodb"),
					},
				},
			},
		},
...
func TestToolResultJSONParsingResponsesAPI(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			messages, _, err := bedrock.ConvertUnifAIMessagesToBedrockMessages(context.Background(), input, false)
			require.NoError(t, err)
...
// TestConvertUnifAIResponsesMessageContentBlocksToBedrockContentBlocks_EmptyBlocks tests that
...
func TestConvertUnifAIResponsesMessageContentBlocksToBedrockContentBlocks_EmptyBlocks(t *testing.T) {
	tests := []struct {
		name           string
		input          *schemas.UnifAIResponsesResponse
...
func TestAnthropicReasoningConfigUsesThinkingField(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ctx := schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline)
			result, err := bedrock.ToBedrockChatCompletionRequest(ctx, unifaiReq)
			require.NoError(t, err)
...
func TestAnthropicOrderedOutputConfigRoundTripsReasoning(t *testing.T) {
	...
	ctx := schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline)
	result, err := request.ToUnifAIResponsesRequest(ctx)
	require.NoError(t, err)
	require.NotNil(t, result)
...
func TestAnthropicStructuredOutputUsesOutputConfigWithoutForcedToolChoice(t *testing.T) {
	...
	// PR #3184 moved Anthropic structured output off native output_config.format
	// onto the synthetic uf_so_* tool path used by all Bedrock models.
...
func TestAnthropicStructuredOutputAcceptsOrderedMaps(t *testing.T) {
	...
	// PR #3184 moved Anthropic structured output off native output_config.format
	// onto the synthetic uf_so_* tool path. Test asserts the synthetic tool path
	...
	require.NotNil(t, syntheticTool, "expected synthetic uf_so_* tool for structured output")
...
// synthetic `uf_so_*` tool path (same as non-Anthropic Bedrock providers),
...
func TestBedrockAnthropicChatStructuredOutputUsesSyntheticTool(t *testing.T) {
	...
	// Positive: synthetic uf_so_* tool present and forced via tool_choice —
	// this is the contract that replaces output_config.format on Bedrock.
	require.NotNil(t, result.ToolConfig, "expected toolConfig with synthetic uf_so_* tool")
	require.NotEmpty(t, result.ToolConfig.Tools, "expected at least one tool (the synthetic uf_so_*)")
	...
	assert.Contains(t, result.ToolConfig.ToolChoice.Tool.Name, "uf_so_", "expected forced tool_choice to target uf_so_*")
	assert.Equal(t, "uf_so_classification", result.ToolConfig.ToolChoice.Tool.Name)
}
...
// (`client.messages.create`), routed via /v1/messages -> ToUnifAIResponsesRequest
...
func TestToBedrockResponsesRequest_AnthropicStructuredOutputUsesSyntheticTool(t *testing.T) {
	...
	// Positive: synthetic uf_so_* tool injected and forced.
	require.NotNil(t, bedrockReq.ToolConfig, "expected toolConfig with synthetic uf_so_* tool")
	require.NotEmpty(t, bedrockReq.ToolConfig.Tools, "expected at least one tool (the synthetic uf_so_*)")
	...
	assert.Contains(t, bedrockReq.ToolConfig.ToolChoice.Tool.Name, "uf_so_", "expected forced tool_choice to target uf_so_*")
	assert.Equal(t, "uf_so_classification", bedrockReq.ToolConfig.ToolChoice.Tool.Name)
}
...
func TestNonAnthropicStructuredOutputStillUsesToolConversion(t *testing.T) {
	...
	assert.Equal(t, "uf_so_classification", result.ToolConfig.ToolChoice.Tool.Name)
	assert.Equal(t, "uf_so_classification", result.ToolConfig.Tools[0].ToolSpec.Name)
...
func TestAnthropicStructuredOutputMergesAdditionalModelRequestFieldPaths(t *testing.T) {
	...
	// Structured output is routed through the synthetic uf_so_* tool path on all
					...
					{Role: schemas.ChatMessageRoleUser, Content: &schemas.ChatMessageContent{ContentStr: schemas.Ptr("hi")}},
					{
						...
						ChatAssistantMessage: &schemas.ChatAssistantMessage{
							ReasoningDetails: []schemas.ChatReasoningDetails{
								{Type: schemas.UnifAIReasoningDetailsTypeText, Text: schemas.Ptr("thinking"), Signature: tc.in},
							},
						},
					},
				},
...
func TestBedrockStopReasonMapping(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			unifaiResp, err := response.ToUnifAIChatResponse(context.Background(), "test-model")
			require.NoError(t, err)
			require.NotNil(t, unifaiResp)
			require.Len(t, unifaiResp.Choices, 1)
			require.NotNil(t, unifaiResp.Choices[0].FinishReason)
			assert.Equal(t, tt.expectedUnifAI, *unifaiResp.Choices[0].FinishReason,
				"Bedrock stop reason %q should map to %q", tt.bedrockStopReason, tt.expectedUnifAI)
		})
	}
}

// TestBedrockStopReasonMappingResponsesPath tests stop reason normalisation for
// the Responses API path (BedrockConverseResponse.ToUnifAIResponsesResponse).
func TestBedrockStopReasonMappingResponsesPath(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			unifaiResp, err := response.ToUnifAIResponsesResponse(ctx)
			require.NoError(t, err)
			require.NotNil(t, unifaiResp)
			require.NotNil(t, unifaiResp.StopReason, "StopReason should be set")
			assert.Equal(t, tt.expectedUnifAI, *unifaiResp.StopReason,
				"Bedrock stop reason %q should map to %q in responses path", tt.bedrockReason, tt.expectedUnifAI)
			...
			if tt.expectedStatus == "" {
				assert.Nil(t, unifaiResp.Status, "Status must be nil for unmapped stop reasons")
				assert.Nil(t, unifaiResp.IncompleteDetails, "IncompleteDetails must be nil for unmapped stop reasons")
			} else {
				require.NotNil(t, unifaiResp.Status, "Status must be set for mapped stop reason %q", tt.bedrockReason)
				assert.Equal(t, tt.expectedStatus, *unifaiResp.Status)
			}
			if tt.expectedIncompleteDetails == "" {
				assert.Nil(t, unifaiResp.IncompleteDetails)
			} else {
				require.NotNil(t, unifaiResp.IncompleteDetails)
				assert.Equal(t, tt.expectedIncompleteDetails, unifaiResp.IncompleteDetails.Reason)
			}
		})
	}
}
...
// TestUnifAIToBedrockStopReasonReverseMapping tests the reverse conversion
// (UnifAIResponsesResponse.StopReason → BedrockConverseResponse.StopReason).
func TestUnifAIToBedrockStopReasonReverseMapping(t *testing.T) {
	t.Parallel()
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, tt.expectedBedrock, actual.StopReason,
				"UnifAI stop reason %v should reverse-map to Bedrock %q", tt.stopReason, tt.expectedBedrock)
		})
	}
}
...
// responses path (Bedrock -> UnifAI -> Bedrock) preserves the document format
...
func TestDocumentFormatResponsesPathRoundTrip(t *testing.T) {
	...
	for _, format := range formats {
		t.Run(format, func(t *testing.T) {
			...
			// Inbound: Bedrock -> UnifAI responses messages
			unifaiMessages := bedrock.ConvertBedrockMessagesToUnifAIMessages(ctx, bedrockMessages, nil, false)
			require.NotEmpty(t, unifaiMessages, "expected at least one UnifAI message")

			// Outbound: UnifAI responses messages -> Bedrock
			roundTripped, _, err := bedrock.ConvertUnifAIMessagesToBedrockMessages(ctx, unifaiMessages, false)
			require.NoError(t, err)
...
func TestBedrockToolInputKeyOrderPreservation(t *testing.T) {
	...
	ctx, cancel := schemas.NewUnifAIContextWithCancel(nil)
	defer cancel()
	result, err := bedrock.ToBedrockChatCompletionRequest(ctx, unifaiReq)
	require.NoError(t, err)
...
func TestToBedrockInvokeMessagesStreamResponse_NoDuplicateContentBlockStop(t *testing.T) {
	...
	events := []*schemas.UnifAIResponsesStreamResponse{
		{
			Type:         schemas.ResponsesStreamResponseTypeOutputTextDone,
			ContentIndex: &contentIdx,
			ExtraFields:  schemas.UnifAIResponseExtraFields{OriginalModelRequested: model},
		},
		{
...
// toolConfig.toolChoice.tool and try again."). The synthetic `uf_so_*` tool
...
func TestBedrockLlamaChatStructuredOutputOmitsForcedToolChoice(t *testing.T) {
	...
	// Positive: synthetic uf_so_* tool still injected so the model has the
	// schema available to call.
	require.NotNil(t, result.ToolConfig, "expected toolConfig with synthetic uf_so_* tool")
	require.NotEmpty(t, result.ToolConfig.Tools, "expected at least one tool (the synthetic uf_so_*)")
	require.NotNil(t, result.ToolConfig.Tools[0].ToolSpec, "expected ToolSpec on synthetic tool")
	assert.Contains(t, result.ToolConfig.Tools[0].ToolSpec.Name, "uf_so_", "expected synthetic uf_so_* tool to be present")
	assert.Equal(t, "uf_so_PlannerOutput", result.ToolConfig.Tools[0].ToolSpec.Name)
...
// pinning the synthetic uf_so_* tool — that's the contract that makes
...
func TestBedrockNonLlamaChatStructuredOutputForcesToolChoice(t *testing.T) {
	...
	require.NotNil(t, result.ToolConfig.ToolChoice.Tool, "expected tool_choice to target a specific tool")
	assert.Equal(t, "uf_so_PlannerOutput", result.ToolConfig.ToolChoice.Tool.Name)
}
...
func TestToBedrockResponsesRequest_LlamaStructuredOutputOmitsForcedToolChoice(t *testing.T) {
	...
	// Positive: synthetic uf_so_* tool still injected.
	require.NotNil(t, bedrockReq.ToolConfig, "expected toolConfig with synthetic uf_so_* tool")
	require.NotEmpty(t, bedrockReq.ToolConfig.Tools, "expected at least one tool (the synthetic uf_so_*)")
	require.NotNil(t, bedrockReq.ToolConfig.Tools[0].ToolSpec, "expected ToolSpec on synthetic tool")
	assert.Contains(t, bedrockReq.ToolConfig.Tools[0].ToolSpec.Name, "uf_so_", "expected synthetic uf_so_* tool to be present")
...
func TestBedrockToUnifAIChatResponse_RestoresAliasedToolName(t *testing.T) {
	toolName := "mcp__plugin_chrome-devtools-mcp_chrome-devtools__list_network_requests"
	req := &schemas.UnifAIChatRequest{
		Model: "us.anthropic.claude-sonnet-4-5-20250929-v1:0",
		Input: []schemas.ChatMessage{{
	...
	converted, err := response.ToUnifAIChatResponse(ctx, req.Model)
	require.NoError(t, err)
...
func TestToBedrockResponsesRequest_AliasesLongMCPToolNames(t *testing.T) {
	toolName := "mcp__unifai-this-is-imp-nasdkjadk-kanbsdjkabdkjbaskjdbasdaskjdbajksdkas__notion-notion-search"
	req := &schemas.UnifAIResponsesRequest{
...
func TestBedrockToUnifAIResponsesResponse_RestoresAliasedToolName(t *testing.T) {
	toolName := "mcp__unifai-this-is-imp-nasdkjadk-kanbsdjkabdkjbaskjdbasdaskjdbajksdkas__notion-notion-search"
	req := &schemas.UnifAIResponsesRequest{
		Model: "us.anthropic.claude-opus-4-7",
		Input: []schemas.ResponsesMessage{{
	...
	converted, err := response.ToUnifAIResponsesResponse(ctx)
	require.NoError(t, err)
...
// TestBedrockToUnifAIChatResponse_StructuredOutput_FinishReasonStop verifies that when
// the model returns only the synthetic uf_so_* tool block (no real tool calls),
// finish_reason is mapped to "stop", not "tool_calls".
func TestBedrockToUnifAIChatResponse_StructuredOutput_FinishReasonStop(t *testing.T) {
	const soToolName = "uf_so_my_schema"
	...
	ctx := schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline)
	ctx.SetValue(schemas.UnifAIContextKeyStructuredOutputToolName, soToolName)

	result, err := response.ToUnifAIChatResponse(ctx, "claude-opus-4-6")
	require.NoError(t, err)
	require.NotNil(t, result)
	...
	assert.Equal(t, string(schemas.UnifAIFinishReasonStop), *choice.FinishReason,
		"expected finish_reason=stop when only SO tool was consumed")
}

// TestBedrockToUnifAIChatResponse_StructuredOutput_MixedWithRealTools verifies that
...
func TestBedrockToUnifAIChatResponse_StructuredOutput_MixedWithRealTools(t *testing.T) {
	const soToolName = "uf_so_my_schema"
	...
	require.NotNil(t, choice.FinishReason)
	assert.Equal(t, string(schemas.UnifAIFinishReasonToolCalls), *choice.FinishReason,
		"expected finish_reason=tool_calls when real tool calls are also present")
}

// TestBedrockToUnifAIResponsesResponse_StructuredOutput_FinishReasonStop verifies that
// ToUnifAIResponsesResponse maps stop_reason to "stop" (not "tool_calls") when only the
// synthetic SO tool was consumed.
func TestBedrockToUnifAIResponsesResponse_StructuredOutput_FinishReasonStop(t *testing.T) {
	const soToolName = "uf_so_user_info"
	...
	result, err := response.ToUnifAIResponsesResponse(ctx)
	require.NoError(t, err)
	require.NotNil(t, result)
...
// TestBedrockToUnifAIResponsesResponse_StructuredOutput_MixedWithRealTools verifies that
// stop_reason stays "tool_calls" when both the SO tool and a real tool call are present.
func TestBedrockToUnifAIResponsesResponse_StructuredOutput_MixedWithRealTools(t *testing.T) {
	const soToolName = "uf_so_user_info"
...
// https://github.com/unifai/unifai/issues/3537 — a Bedrock-native passthrough
// request containing toolResult.content[].searchResult must survive
// ToUnifAIResponsesRequest → ToBedrockResponsesRequest with all fields intact.
...
func TestBedrockSearchResultToolResultRoundTrip(t *testing.T) {
							...
							Content: []bedrock.BedrockContentBlock{
								{
									SearchResult: &bedrock.BedrockSearchResultBlock{
										...
										Content: []bedrock.BedrockSearchResultContent{
											{Text: "Apptio is a company that makes calls to Bedrock using passthrough APIs via UnifAI"},
										},
	...
	// First leg: Bedrock → UnifAI intermediate.
	unifaiReq, err := original.ToUnifAIResponsesRequest(ctx)
	require.NoError(t, err)
	require.NotNil(t, unifaiReq)

	// Second leg: UnifAI intermediate → Bedrock.
	rebuilt, err := bedrock.ToBedrockResponsesRequest(ctx, unifaiReq)
	require.NoError(t, err)
	...
	assert.Equal(t, "Apptio is a company that makes calls to Bedrock using passthrough APIs via UnifAI", got.Content[0].Text)
...
// toolResult.content survives ToUnifAIResponsesRequest → ToBedrockResponsesRequest
...
func TestMidConversationSystemReminderStaysInline(t *testing.T) {
	...
	messages, systemMessages, err := bedrock.ConvertUnifAIMessagesToBedrockMessages(context.Background(), input, true)
	require.NoError(t, err)
...
func TestMidConversationSystemReminderHoistedForNonAnthropic(t *testing.T) {
	...
	messages, systemMessages, err := bedrock.ConvertUnifAIMessagesToBedrockMessages(context.Background(), input, false)
	require.NoError(t, err)
...
func TestSystemReminderBetweenToolCallAndResult(t *testing.T) {
	...
	messages, _, err := bedrock.ConvertUnifAIMessagesToBedrockMessages(context.Background(), input, true)
	require.NoError(t, err)
...
func TestLoneSystemMessageReturnsUserMessage(t *testing.T) {
	...
	for role, msg := range roles {
		for _, inline := range []bool{true, false} {
			...
			messages, systemMessages, err := bedrock.ConvertUnifAIMessagesToBedrockMessages(context.Background(), input, inline)
			require.NoError(t, err)
