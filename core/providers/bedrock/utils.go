...
func convertChatParameters(ctx *schemas.UnifAIContext, unifaiReq *schemas.UnifAIChatRequest, bedrockReq *BedrockConverseRequest) error {
	...
	// Handle structured output conversion through the synthetic `uf_so_*` tool
	...
	if serverTools, betaHeaders := collectBedrockServerToolsFromFiltered(filteredTools); len(serverTools) > 0 {
		...
		// uf_so_* tool at lines 263-275 below; otherwise Bedrock receives two
	...
	if responseFormatTool != nil {
		...
		// With only the synthetic uf_so_* tool bound, omitting tool_choice
...
func convertResponseFormatToTool(
	...
	// All Bedrock models (including Anthropic) use the synthetic `uf_so_*` tool
...
func convertTextFormatToTool(ctx *schemas.UnifAIContext, model string, textConfig *schemas.ResponsesTextConfig) (*BedrockTool, any) {
	...
	// All Bedrock models use the synthetic `uf_so_*` tool path here as well.
