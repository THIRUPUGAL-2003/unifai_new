...
func TestConvertChatParameters_ServerToolNameFromBody(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			bedrockReq, err := ToBedrockChatCompletionRequest(ctx, unifaiReq)
			if err != nil {
...
func TestConvertChatParameters_PinnedServerToolE2E(t *testing.T) {
	...
	if err := convertChatParameters(nil, unifaiReq, bedrockReq); err != nil {
...
// when response_format forces the synthetic uf_so_* tool via
...
func TestConvertChatParameters_ResponseFormatWithPinnedServerTool_NoConflictingChoice(t *testing.T) {
	...
	// Synthetic uf_so_* tool must be injected and pinned via Converse's typed path.
	if bedrockReq.ToolConfig == nil {
		t.Fatalf("expected ToolConfig with synthetic uf_so_* tool, got nil")
	}
	...
	if !strings.HasPrefix(bedrockReq.ToolConfig.ToolChoice.Tool.Name, "uf_so_") {
		t.Errorf("expected ToolConfig.ToolChoice.Tool.Name to start with uf_so_, got %q", bedrockReq.ToolConfig.ToolChoice.Tool.Name)
	}
	...
	if _, ok := bedrockReq.AdditionalModelRequestFields.Get("tool_choice"); ok {
		t.Errorf("expected NO additionalModelRequestFields.tool_choice when response_format pins uf_so_* (conflict hazard)")
	}
}
