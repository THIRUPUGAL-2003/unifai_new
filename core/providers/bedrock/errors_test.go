...
// UnifAIError. Without this, a retired/unsupported model error surfaces as a
...
func TestParseBedrockHTTPError_PreservesExceptionType(t *testing.T) {
	...
	unifaiErr := parseBedrockHTTPError(http.StatusBadRequest, http.Header{}, body)

	require.NotNil(t, unifaiErr)
	require.NotNil(t, unifaiErr.Type, "exception type must be preserved from __type")
	assert.Equal(t, "ValidationException", *unifaiErr.Type)
	require.NotNil(t, unifaiErr.Error)
	assert.Equal(t, "Invocation of model ID anthropic.claude-v2 with on-demand throughput isn't supported.", unifaiErr.Error.Message)
	require.NotNil(t, unifaiErr.StatusCode)
	assert.Equal(t, http.StatusBadRequest, *unifaiErr.StatusCode)
}
...
func TestParseBedrockHTTPError_TypeFromHeader(t *testing.T) {
	...
	unifaiErr := parseBedrockHTTPError(http.StatusBadRequest, headers, body)

	require.NotNil(t, unifaiErr)
	require.NotNil(t, unifaiErr.Type, "type must be recovered from X-Amzn-Errortype header")
	assert.Equal(t, "ValidationException", *unifaiErr.Type)
	require.NotNil(t, unifaiErr.Error)
	assert.Contains(t, unifaiErr.Error.Message, "reached the end of its life")
}
...
func TestParseBedrockHTTPError_HeaderTypeQualifierStripped(t *testing.T) {
	...
	require.NotNil(t, unifaiErr)
	require.NotNil(t, unifaiErr.Type)
	assert.Equal(t, "ValidationException", *unifaiErr.Type)
}
...
func TestParseBedrockHTTPError_RoundTripToBedrockError(t *testing.T) {
	...
	unifaiErr := parseBedrockHTTPError(http.StatusBadRequest, headers, body)
	bedrockErr := ToBedrockError(unifaiErr)
...
func TestNewBedrockStreamException_TypeFromBody(t *testing.T) {
	...
	assert.True(t, streamErr.IsUnifAIError, "non-retryable exceptions are terminal")
...
// exception is marked with IsUnifAIError:false and a gate-recognized transient
...
func TestNewBedrockStreamException_Retryable(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.excType, func(t *testing.T) {
			...
			require.NotNil(t, streamErr)
			assert.False(t, streamErr.IsUnifAIError, "retryable exceptions must keep IsUnifAIError:false for the retry gate")
...
// exceptions are terminal (IsUnifAIError:true, no retry status) yet still
...
func TestNewBedrockStreamException_Terminal(t *testing.T) {
	for _, excType := range []string{"validationException", "accessDeniedException", "resourceNotFoundException"} {
		t.Run(excType, func(t *testing.T) {
			...
			require.NotNil(t, streamErr)
			assert.True(t, streamErr.IsUnifAIError, "non-retryable exceptions must be terminal")
			assert.Nil(t, streamErr.StatusCode)
			require.NotNil(t, streamErr.Type)
...
func TestParseBedrockHTTPError_NoTypeFallsBack(t *testing.T) {
	...
	unifaiErr := parseBedrockHTTPError(http.StatusInternalServerError, http.Header{}, body)
	require.NotNil(t, unifaiErr)
	assert.Nil(t, unifaiErr.Type, "no __type present, so none should be set")
