...
func TestToUnifAIChatResponse_Citations(t *testing.T) {
	t.Parallel()

	t.Run("citations are mapped to unifai response", func(t *testing.T) {
		...
		unifaiResp := response.ToUnifAIChatResponse("sonar-pro")

		if unifaiResp == nil {
			t.Fatal("expected non-nil unifai response")
		}

		// Verify citations are preserved
		if len(unifaiResp.Citations) != 3 {
			t.Fatalf("expected 3 citations, got %d", len(unifaiResp.Citations))
		}
		if unifaiResp.Citations[0] != "https://example.com/article1" {
			t.Errorf("expected first citation to be 'https://example.com/article1', got '%s'", unifaiResp.Citations[0])
		}
		if unifaiResp.Citations[1] != "https://example.com/article2" {
			t.Errorf("expected second citation to be 'https://example.com/article2', got '%s'", unifaiResp.Citations[1])
		}
		if unifaiResp.Citations[2] != "https://example.com/article3" {
			t.Errorf("expected third citation to be 'https://example.com/article3', got '%s'", unifaiResp.Citations[2])
		}

		// Verify search results are preserved
		if len(unifaiResp.SearchResults) != 1 {
			t.Fatalf("expected 1 search result, got %d", len(unifaiResp.SearchResults))
		}

		// Verify videos are preserved
		if len(unifaiResp.Videos) != 1 {
			t.Fatalf("expected 1 video, got %d", len(unifaiResp.Videos))
		}

		// Verify usage citation tokens are mapped
		if unifaiResp.Usage == nil || unifaiResp.Usage.CompletionTokensDetails == nil {
			t.Fatal("expected usage with completion token details")
		}
		if unifaiResp.Usage.CompletionTokensDetails.CitationTokens == nil || *unifaiResp.Usage.CompletionTokensDetails.CitationTokens != 5 {
			t.Error("expected citation_tokens to be 5")
		}
		if unifaiResp.Usage.CompletionTokensDetails.NumSearchQueries == nil || *unifaiResp.Usage.CompletionTokensDetails.NumSearchQueries != 2 {
	...
	t.Run("nil citations remain nil", func(t *testing.T) {
		...
		if unifaiResp.Citations != nil {
			t.Errorf("expected nil citations, got %v", unifaiResp.Citations)
		}
		if unifaiResp.SearchResults != nil {
			t.Errorf("expected nil search results, got %v", unifaiResp.SearchResults)
		}
	})
	...
	t.Run("empty citations slice is preserved", func(t *testing.T) {
		...
		if len(unifaiResp.Citations) != 0 {
			t.Errorf("expected 0 citations, got %d", len(unifaiResp.Citations))
		}
	})
}
...
func TestWebSearchOption_JSONSerialization(t *testing.T) {
	...
	t.Run("search_type extraction from ExtraParams", func(t *testing.T) {
		...
		result := perplexity.ToPerplexityChatCompletionRequest(unifaiReq)
...
func TestUnifAICost_DeserializationWithPerplexityFields(t *testing.T) {
	t.Parallel()
	...
	t.Run("deserializes new cost breakdown fields", func(t *testing.T) {
		...
		var cost schemas.UnifAICost
		if err := cost.UnmarshalJSON([]byte(jsonStr)); err != nil {
			t.Fatalf("failed to unmarshal UnifAICost: %v", err)
		}
