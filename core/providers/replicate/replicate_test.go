...
func TestFileUpload_OrdersMetadataBeforeFile(t *testing.T) {
	...
	_, unifaiErr := provider.FileUpload(ctx, schemas.Key{}, &schemas.UnifAIFileUploadRequest{
...
// TestUnifAIToReplicateChatRequestConversion tests the conversion from UnifAI chat request to Replicate format
// with special handling based on model names
func TestUnifAIToReplicateChatRequestConversion(t *testing.T) {
	maxTokens := 100
	temp := 0.7
	topP := 0.9
	seed := 42
...
// TestUnifAIToReplicateImageGenerationConversion tests the conversion from UnifAI image generation request
// to Replicate format with special handling for different model families
func TestUnifAIToReplicateImageGenerationConversion(t *testing.T) {
	...
	tests := []struct {
		name     string
		input    *schemas.UnifAIImageGenerationRequest
		validate func(t *testing.T, result *replicate.ReplicatePredictionRequest)
		wantErr  bool
	}{
		{
...
// TestUnifAIToReplicateResponsesRequestConversion tests the conversion from UnifAI responses request to Replicate format
func TestUnifAIToReplicateResponsesRequestConversion(t *testing.T) {
	maxOutputTokens := 100
	temp := 0.7
	topP := 0.9
...
// TestReplicateToUnifAIResponsesResponse tests the conversion from Replicate prediction response to UnifAI responses format
func TestReplicateToUnifAIResponsesResponse(t *testing.T) {
	...
	tests := []struct {
		...
		validate func(t *testing.T, result *schemas.UnifAIResponsesResponse)
	}{
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			actual := tt.input.ToUnifAIResponsesResponse()
			if tt.validate != nil {
				tt.validate(t, actual)
			}
		})
