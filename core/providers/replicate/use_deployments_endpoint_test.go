...
func TestUseDeploymentsEndpoint_AliasOverride(t *testing.T) {
	...
	ctxOverrideTrue := schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline)
	defer ctxOverrideTrue.Cancel()
	trueVal := true
	ctxOverrideTrue.SetValue(schemas.UnifAIContextKeyResolvedAlias, &schemas.ResolvedAlias{
		Key: "prod-llm",
	...
	ctxOverrideFalse := schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline)
	defer ctxOverrideFalse.Cancel()
	falseVal := false
	ctxOverrideFalse.SetValue(schemas.UnifAIContextKeyResolvedAlias, &schemas.ResolvedAlias{
	...
	ctxNoCfg := schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline)
	defer ctxNoCfg.Cancel()
	ctxNoCfg.SetValue(schemas.UnifAIContextKeyResolvedAlias, &schemas.ResolvedAlias{
		Key: "x",
