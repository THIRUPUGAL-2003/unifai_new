...
func checkForErrorStatus(prediction *ReplicatePredictionResponse) *schemas.UnifAIError {
...
func listenToReplicateStreamURL(
	...
	streamURL string,
	key schemas.Key,
) (io.Reader, *fasthttp.Response, *schemas.UnifAIError) {
	// Create request
	...
	if err != nil {
		...
		if errors.Is(err, context.Canceled) {
			return nil, nil, providerUtils.SetErrorLatency(&schemas.UnifAIError{
				IsUnifAIError: false,
		...
		if errors.Is(err, fasthttp.ErrTimeout) || errors.Is(err, context.DeadlineExceeded) {
			return nil, nil, providerUtils.SetErrorLatency(providerUtils.NewUnifAITimeoutError(schemas.ErrProviderRequestTimedOut, err), latency)
		}
		return nil, nil, providerUtils.SetErrorLatency(providerUtils.NewUnifAIOperationError(schemas.ErrProviderDoRequest, err), latency)
	}
...
func buildPredictionURL(ctx *schemas.UnifAIContext, baseURL, model string, customProviderConfig *schemas.CustomProviderConfig, requestType schemas.RequestType, useDeploymentsEndpoint bool) string {
