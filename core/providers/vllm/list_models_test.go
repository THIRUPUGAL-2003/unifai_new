...
func TestListModels_QueriesAllBackends(t *testing.T) {
	...
	resp, unifaiErr := provider.ListModels(ctx, keys, request)
	if unifaiErr != nil {
		t.Fatalf("ListModels returned error: %v", unifaiErr.Error)
	}
	...
	// Model IDs are prefixed with "vllm/" by ToUnifAIListModelsResponse.
...
func TestListModels_SingleBackendFailure(t *testing.T) {
	...
	resp, unifaiErr := provider.ListModels(ctx, keys, request)
	if unifaiErr != nil {
		t.Fatalf("ListModels should not return a top-level error when one backend succeeds, got: %v", unifaiErr.Error)
	}
...
func TestListModels_ErrorsWithoutPerKeyURL(t *testing.T) {
	...
	_, unifaiErr := provider.ListModels(ctx, keys, request)
	if unifaiErr == nil {
