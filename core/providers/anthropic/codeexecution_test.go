...
// blocks survive Anthropic -> UnifAI -> Anthropic on the non-streaming path,
...
func TestCodeExecution_BashStream(t *testing.T) {
	...
	ctx.SetValue(schemas.UnifAIContextKeyIntegrationType, "anthropic")
	state := AcquireAnthropicResponsesStreamState()
	defer ReleaseAnthropicResponsesStreamState(state)

	var emitted []*schemas.UnifAIResponsesStreamResponse
	seq := 0
	...
	// Reverse direction: the emitted UnifAI stream events must reconstruct the
...
// multi-key input must round-trip Anthropic -> UnifAI -> Anthropic.
...
// Anthropic -> UnifAI -> Anthropic round trip on the web_search server_tool_use
...
func TestCodeExecution_ProgrammaticCallerRoundTrip(t *testing.T) {
	...
	bif := resp.ToUnifAIResponsesResponse(ctx)
...
// the forward (Anthropic→UnifAI) then reverse (UnifAI→Anthropic) stream
...
func TestCodeExecution_ToolVersionRoundTrip(t *testing.T) {
	...
	for _, v := range versions {
		...
		neutral := convertAnthropicToolToUnifAI(in)
		...
		back := convertUnifAIToolToAnthropic("claude-opus-4-8", neutral, schemas.Anthropic, false)
	...
	back := convertUnifAIToolToAnthropic("claude-opus-4-8", noVer, schemas.Anthropic, false)
