...
// the neutral ResponsesTool schema (Anthropic -> UnifAI -> Anthropic) with its
...
func TestAdvisor_ResponsesRoundTrip(t *testing.T) {
	...
	if unifaiTool == nil {
		t.Fatal("convertAnthropicToolToUnifAI returned nil")
	}
	if unifaiTool.Type != schemas.ResponsesToolTypeAdvisor {
		t.Fatalf("neutral type = %q, want advisor", unifaiTool.Type)
	}
	if unifaiTool.ResponsesToolAdvisor == nil || unifaiTool.ResponsesToolAdvisor.Model != "claude-opus-4-8" {
		t.Fatalf("neutral advisor model not carried: %+v", unifaiTool.ResponsesToolAdvisor)
	}
	if unifaiTool.ResponsesToolAdvisor.MaxUses == nil || *unifaiTool.ResponsesToolAdvisor.MaxUses != 3 {
		t.Fatalf("neutral advisor max_uses not carried: %+v", unifaiTool.ResponsesToolAdvisor.MaxUses)
	}
	if unifaiTool.ResponsesToolAdvisor.MaxTokens == nil || *unifaiTool.ResponsesToolAdvisor.MaxTokens != 2048 {
		t.Fatalf("neutral advisor max_tokens not carried: %+v", unifaiTool.ResponsesToolAdvisor.MaxTokens)
	}
	if unifaiTool.ResponsesToolAdvisor.Caching == nil ||
		unifaiTool.ResponsesToolAdvisor.Caching.Type != "ephemeral" ||
		unifaiTool.ResponsesToolAdvisor.Caching.TTL != "5m" {
		t.Fatalf("neutral advisor caching not carried: %+v", unifaiTool.ResponsesToolAdvisor.Caching)
	}

	back := convertUnifAIToolToAnthropic("claude-sonnet-4-6", unifaiTool, schemas.Anthropic, false)
...
// user-reported bug: Anthropic -> UnifAI -> Anthropic dropped the advisor
...
func TestAdvisorResponse_RoundTripPreservesBlocks(t *testing.T) {
	...
	unifaiResp := resp.ToUnifAIResponsesResponse(ctx)
	if unifaiResp == nil {
		t.Fatal("ToUnifAIResponsesResponse returned nil")
	}

	var foundAdvisor bool
	for _, out := range unifaiResp.Output {
	...
	back := ToAnthropicResponsesResponse(ctx, unifaiResp)
	if back == nil {
...
// (Anthropic SSE -> UnifAI stream -> Anthropic SSE) on the normalized path.
func TestAdvisorStream_RoundTrip(t *testing.T) {
	...
	for _, raw := range advisorStreamEvents {
		...
		responses, unifaiErr, _ := chunk.ToUnifAIResponsesStream(ctx, seq, state)
		if unifaiErr != nil {
			t.Fatalf("ToUnifAIResponsesStream error: %v", unifaiErr)
		}
...
// (transports/unifai-http/integrations/anthropic.go): for each unifai stream
...
func countPassthroughMessageStarts(ctx *schemas.UnifAIContext, responses []*schemas.UnifAIResponsesStreamResponse) (int, string) {
	count := 0
	forwarded := ""
	for _, r := range responses {
...
func TestResponsesStream_MessageStart_NoDuplicateOnPassthrough(t *testing.T) {
	...
	responses, unifaiErr, _ := chunk.ToUnifAIResponsesStream(ctx, 0, state)
	if unifaiErr != nil {
		t.Fatalf("ToUnifAIResponsesStream error: %v", unifaiErr)
	}
	...
	if len(responses) != 2 {
		t.Fatalf("expected 2 unifai responses (created, in_progress), got %d", len(responses))
	}
	createdIdx := -1
