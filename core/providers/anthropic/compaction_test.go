...
// --- Streaming: Anthropic → UnifAI (inbound) ---

func TestToUnifAIResponsesStream_CompactionContentBlockStart(t *testing.T) {
	t.Parallel()
	...
	responses, err, isLast := chunk.ToUnifAIResponsesStream(context.Background(), 0, state)
	if err != nil {
...
func TestToUnifAIResponsesStream_CompactionDelta(t *testing.T) {
	t.Parallel()
...
func TestToUnifAIResponsesStream_CompactionContentBlockStop(t *testing.T) {
	t.Parallel()
...
// --- Streaming: UnifAI → Anthropic (outbound, non-passthrough) ---
...
func TestToAnthropicResponsesStreamResponse_CompactionOutputItemAdded(t *testing.T) {
	...
	unifaiResp := &schemas.UnifAIResponsesStreamResponse{
	...
	events := ToAnthropicResponsesStreamResponse(ctx, unifaiResp)
...
func TestToUnifAIResponsesResponse_PreservesStopReason(t *testing.T) {
	t.Parallel()
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if *unifaiResp.StopReason != tt.expectedStopReason {
				t.Errorf("StopReason = %q, want %q", *unifaiResp.StopReason, tt.expectedStopReason)
			}
		})
	}
}

func TestToUnifAIResponsesResponse_EmptyStopReason(t *testing.T) {
	t.Parallel()
	...
	if unifaiResp.StopReason != nil {
		t.Errorf("expected nil StopReason for empty stop_reason, got %q", *unifaiResp.StopReason)
	}
}

func TestToAnthropicResponsesResponse_StopReasonFromUnifAI(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{
			name:           "compaction stop reason from unifai",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result := ToAnthropicResponsesResponse(ctx, unifaiResp)
...
func TestCompactionContentBlock_NonStreamingRoundTrip(t *testing.T) {
	...
	// Step 1: Anthropic → UnifAI
	unifaiResp := anthropicResp.ToUnifAIResponsesResponse(ctx)

	if unifaiResp.StopReason == nil || *unifaiResp.StopReason != "compaction" {
		t.Fatalf("expected stop_reason='compaction', got %v", unifaiResp.StopReason)
	}
	...
	if !foundCompaction {
		t.Error("compaction block not found in UnifAI output")
	}

	// Step 2: UnifAI → Anthropic
	result := ToAnthropicResponsesResponse(ctx, unifaiResp)
...
func TestMixedTextThenToolCallsStreamMapsStopReasonToToolUse(t *testing.T) {
	...
	chatChunks := []*schemas.UnifAIChatResponse{
		{
	...
	for _, chunk := range chatChunks {
		for _, responseEvent := range chunk.ToUnifAIResponsesStreamResponse(state) {
