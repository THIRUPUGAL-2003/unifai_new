...
// TestConvertUnifAIMessages_OrphanToolResultBecomesUserText verifies that a
...
// previous message."
func TestConvertUnifAIMessages_OrphanToolResultBecomesUserText(t *testing.T) {
	t.Parallel()
	...
	t.Run("orphan_only", func(t *testing.T) {
		msgs, err := ConvertUnifAIMessagesToAnthropicMessages(ctx,
			[]schemas.ResponsesMessage{functionOutput("toolu_orphan", "Sunny, 72F")},
			true, provider, model)
		if err != nil {
			t.Fatalf("ConvertUnifAIMessagesToAnthropicMessages() error = %v", err)
		}
