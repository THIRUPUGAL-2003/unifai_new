...
// TestConvertUnifAIFunctionCallToAnthropicToolUse_Input verifies that the
...
func TestConvertUnifAIFunctionCallToAnthropicToolUse_Input(t *testing.T) {
	t.Parallel()
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			block := convertUnifAIFunctionCallToAnthropicToolUse(ctx, msg)
			if block == nil {
...
// TestConvertUnifAIMCPCallToAnthropicToolUse_Input verifies that the
// mcp_tool_use block always carries an "input" object, defaulting to "{}" when
// the MCP call has nil or empty arguments.
func TestConvertUnifAIMCPCallToAnthropicToolUse_Input(t *testing.T) {
	t.Parallel()
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			block := convertUnifAIMCPCallToAnthropicToolUse(msg)
			if block == nil {
...
// TestConvertUnifAIMCPApprovalToAnthropicToolUse_Input verifies that the
...
func TestConvertUnifAIMCPApprovalToAnthropicToolUse_Input(t *testing.T) {
	t.Parallel()
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			block := convertUnifAIMCPApprovalToAnthropicToolUse(msg)
			if block == nil {
				t.Fatal("expected non-nil mcp_tool_use block")
			}
			if block.Type != AnthropicContentBlockTypeMCPToolUse {
