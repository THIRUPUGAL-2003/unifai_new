...
func TestToAnthropicChatRequest_ServerTools(t *testing.T) {
	mk := func(rawTool string) *schemas.UnifAIChatRequest {
		var tool schemas.ChatTool
...
// TestToUnifAIResponsesRequest_MCPToolsetPreservesAnthropicFlags verifies
...
// the flags because convertAnthropicToolToUnifAI skips mcp_toolset entries.
func TestToUnifAIResponsesRequest_MCPToolsetPreservesAnthropicFlags(t *testing.T) {
	...
	got := req.ToUnifAIResponsesRequest(nil)
	if got == nil || got.Params == nil {
		t.Fatalf("ToUnifAIResponsesRequest returned nil params")
	}

	// The mcp_toolset tool should have been dropped by convertAnthropicToolToUnifAI
