...
func TestAddMissingBetaHeadersToContext_PassthroughWins(t *testing.T) {
	...
	t.Run("passthrough_mcp_header_prevents_auto_inject", func(t *testing.T) {
		...
		extraHeaders := ctx.Value(schemas.UnifAIContextKeyExtraHeaders).(map[string][]string)
...
func TestStripUnsupportedFieldsFromRawBody(t *testing.T) {
	...
	t.Run("advisor_tool_model_prefix_stripped", func(t *testing.T) {
		// Clients that read UnifAI's model catalog (e.g. Claude Code's /advisor)
	...
	t.Run("advisor_tool_unknown_prefix_passes_through", func(t *testing.T) {
		// Namespaced model ids that aren't a UnifAI provider prefix (e.g.
...
func TestGetRequestBodyForResponses_RawBodyStripsFallbacks(t *testing.T) {
	...
	result, unifaiErr := BuildAnthropicResponsesRequestBody(ctx, request, AnthropicRequestBuildConfig{
		Provider:    schemas.Anthropic,
		IsStreaming: false,
	})
	if unifaiErr != nil {
		t.Fatalf("unexpected error: %v", unifaiErr)
	}
...
func TestApplyMCPToolsetConfigToUnifAITool(t *testing.T) {
	t.Run("allowlist pattern merges correctly", func(t *testing.T) {
		...
		applyMCPToolsetConfigToUnifAITool(unifaiTool, toolset)

		if unifaiTool.ResponsesToolMCP.AllowedTools == nil {
			t.Fatal("expected AllowedTools to be set")
		}
		allowedNames := unifaiTool.ResponsesToolMCP.AllowedTools.ToolNames
	...
	t.Run("nil inputs are handled safely", func(t *testing.T) {
		// Should not panic
		applyMCPToolsetConfigToUnifAITool(nil, nil)
		applyMCPToolsetConfigToUnifAITool(&schemas.ResponsesTool{}, nil)
	})
}
...
func TestDiagnostics_ResponsesRequestRoundTrip(t *testing.T) {
	// The diagnostics opt-in must survive the AnthropicMessageRequest -> UnifAI
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			unifaiReq := req.ToUnifAIResponsesRequest(nil)
			if unifaiReq == nil || unifaiReq.Params == nil {
				t.Fatal("ToUnifAIResponsesRequest returned nil")
			}
			back, err := ToAnthropicResponsesRequest(nil, unifaiReq)
			if err != nil {
...
func TestDiagnostics_ResponseRoundTrip(t *testing.T) {
	...
	if unifaiResp == nil || unifaiResp.Diagnostics == nil {
		t.Fatal("diagnostics dropped in ToUnifAIResponsesResponse")
	}
...
func TestDiagnostics_ChatResponseRoundTrip(t *testing.T) {
	...
	ctx := schemas.NewUnifAIContext(context.Background(), time.Time{})
	unifaiResp := resp.ToUnifAIChatResponse(ctx)
	if unifaiResp == nil || unifaiResp.Diagnostics == nil {
		t.Fatal("diagnostics dropped in ToUnifAIChatResponse")
	}
	back := ToAnthropicChatResponse(unifaiResp)
	if back == nil || back.Diagnostics == nil || back.Diagnostics.CacheMissReason == nil {
...
// via the User-Agent stored on UnifAIContext. ClaudeCLI.Matches uses a
...
func TestIsClaudeCodeRequest(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			ctx := schemas.NewUnifAIContext(context.Background(), time.Time{})
			if tc.setUA {
				ctx.SetValue(schemas.UnifAIContextKeyUserAgent, tc.userAgent)
			}
