...
func ExtractAnthropicPassthroughUsage(path string, _, body []byte) *schemas.UnifAIPassthroughUsage {
...
// buildAnthropicPassthroughUsage converts AnthropicUsage directly into UnifAIPassthroughUsage.
func buildAnthropicPassthroughUsage(au *AnthropicUsage) *schemas.UnifAIPassthroughUsage {
	if au == nil {
		return nil
	}
	...
	if au.ServiceTier != nil {
		t := MapAnthropicServiceTierToUnifAI(*au.ServiceTier)
		u.ServiceTier = &t
	}
	if au.Speed != nil {
		u.Speed = au.Speed
...
func (a *AnthropicPassthroughStreamUsage) ObserveEvent(event []byte) *schemas.UnifAIPassthroughUsage {
...
func (a *AnthropicPassthroughStreamUsage) usage() *schemas.UnifAIPassthroughUsage {
	if !a.seen {
		return nil
	}
...
func extractAnthropicMessagesUsage(body []byte) *schemas.UnifAIPassthroughUsage {
	if len(body) == 0 {
		return nil
	}
...
func extractAnthropicCompleteUsage(body []byte) *schemas.UnifAIPassthroughUsage {
	if len(body) == 0 {
		return nil
	}
	var resp struct {
