...
func TestToAnthropicChatRequest_PreservesPropertyOrder(t *testing.T) {
	...
	defer cancel()
	result, err := ToAnthropicChatRequest(ctx, unifaiReq)
	if err != nil {
...
func TestToAnthropicChatRequest_OpenAICompatibleFileIDUsesFileSource(t *testing.T) {
	...
	ctx := schemas.NewUnifAIContext(nil, schemas.NoDeadline)
	unifaiReq := openAIReq.ToUnifAIChatRequest(ctx)
	result, err := ToAnthropicChatRequest(ctx, unifaiReq)
	if err != nil {
...
func TestToAnthropicChatRequest_CachingDeterminism(t *testing.T) {
	makeReq := func(props *schemas.OrderedMap) *schemas.UnifAIChatRequest {
...
func TestToUnifAIChatResponse_MultipleTextBlocksWithThinking(t *testing.T) {
	...
	ctx, cancel := schemas.NewUnifAIContextWithCancel(context.Background())
	defer cancel()
	result := response.ToUnifAIChatResponse(ctx)

	if result == nil {
		t.Fatal("expected non-nil result")
	}

	// With multiple text blocks, ToUnifAIChatResponse preserves them as ContentBlocks
	...
	if rd[0].Type != schemas.UnifAIReasoningDetailsTypeText {
		t.Errorf("expected reasoning detail type %s, got %s", schemas.UnifAIReasoningDetailsTypeText, rd[0].Type)
	}
...
func TestToUnifAIChatResponse_SingleTextBlockNoThinking(t *testing.T) {
			...
			},
			{
				...
				ChatAssistantMessage: &schemas.ChatAssistantMessage{
					ReasoningDetails: []schemas.ChatReasoningDetails{
						{Index: 0, Type: schemas.UnifAIReasoningDetailsTypeText, Text: &modifiedContent, Signature: &signature},
					},
				},
			},
			{
...
func TestToAnthropicChatRequest_NormalFlowUnchanged(t *testing.T) {
	...
	// no unifai.content_blocks) produces the same output as before.
...
// UnifAI adds a synthetic uf_so_* tool AND forces tool_choice to that tool.
func TestToAnthropicChatRequest_StructuredOutput_Vertex_NoThinking(t *testing.T) {
	...
	// A synthetic tool with the uf_so_ prefix must be present.
	var soTool *AnthropicTool
	for i := range result.Tools {
		if len(result.Tools[i].Name) > 6 && result.Tools[i].Name[:6] == "uf_so_" {
			soTool = &result.Tools[i]
			break
		}
	}
	if soTool == nil {
		t.Fatal("expected a synthetic uf_so_* tool to be added for Vertex structured output")
	}
...
// response_format=json_schema + reasoning_effort='medium' is sent to Vertex, UnifAI
...
func TestToAnthropicChatRequest_StructuredOutput_Vertex_ThinkingEffort(t *testing.T) {
	...
	for _, tool := range result.Tools {
		if len(tool.Name) > 6 && tool.Name[:6] == "uf_so_" {
			found = true
			break
		}
	}
	if !found {
		t.Fatal("expected synthetic uf_so_* tool to be present even with thinking enabled")
	}
...
// TestToUnifAIChatResponse_StructuredOutput_FinishReasonStop verifies that when
// the model responds with only the synthetic SO tool (no real tool calls), the
// finish_reason is mapped to "stop", not "tool_calls".
func TestToUnifAIChatResponse_StructuredOutput_FinishReasonStop(t *testing.T) {
	soToolName := "uf_so_my_schema"
	...
	if *choice.FinishReason != string(schemas.UnifAIFinishReasonStop) {
		t.Errorf("expected FinishReason=%q, got %q", schemas.UnifAIFinishReasonStop, *choice.FinishReason)
	}
}

// TestToUnifAIChatResponse_StructuredOutput_MixedWithRealTools verifies that when
...
func TestToUnifAIChatResponse_StructuredOutput_MixedWithRealTools(t *testing.T) {
	soToolName := "uf_so_my_schema"
	...
	if *choice.FinishReason != string(schemas.UnifAIFinishReasonToolCalls) {
		t.Errorf("expected FinishReason=%q, got %q", schemas.UnifAIFinishReasonToolCalls, *choice.FinishReason)
	}
}
...
// TestToUnifAIChatCompletionStream_NoArgToolFlushesEmptyObject covers the
...
func TestToUnifAIChatCompletionStream_NoArgToolFlushesEmptyObject(t *testing.T) {
	...
	startResp, unifaiErr, isLast := start.ToUnifAIChatCompletionStream(nil, "", state)
	if unifaiErr != nil {
		t.Fatalf("unexpected error on start: %v", unifaiErr)
	}
	if isLast {
	...
	stopResp, unifaiErr, isLast := stop.ToUnifAIChatCompletionStream(nil, "", state)
	if unifaiErr != nil {
		t.Fatalf("unexpected error on stop: %v", unifaiErr)
	}
	if isLast {
	...
	dupResp, _, _ := stop.ToUnifAIChatCompletionStream(nil, "", state)
	if dupResp != nil {
		t.Errorf("duplicate stop should not re-flush, got %#v", dupResp)
	}
}

// TestToUnifAIChatCompletionStream_EmptyPartialJSONSuppressedBeforeArgs
...
func TestToUnifAIChatCompletionStream_EmptyPartialJSONSuppressedBeforeArgs(t *testing.T) {
	state := NewAnthropicStreamState()
	...
	startResp, _, _ := start.ToUnifAIChatCompletionStream(nil, "", state)
	if startResp == nil {
	...
	resp, unifaiErr, isLast := emptyDelta.ToUnifAIChatCompletionStream(nil, "", state)
	if unifaiErr != nil {
		t.Fatalf("unexpected error on empty delta: %v", unifaiErr)
	}
	if isLast {
	...
	for _, frag := range []*string{&frag1, &frag2} {
		...
		r, _, _ := ev.ToUnifAIChatCompletionStream(nil, "", state)
		if r == nil {
	...
	stopResp, _, _ := stop.ToUnifAIChatCompletionStream(nil, "", state)
	if stopResp != nil {
...
// TestToUnifAIChatCompletionStream_ContinuationOmitsTypeField is the
...
func TestToUnifAIChatCompletionStream_ContinuationOmitsTypeField(t *testing.T) {
	state := NewAnthropicStreamState()
	...
	deltaResp, _, _ := delta.ToUnifAIChatCompletionStream(nil, "", state)
	if deltaResp == nil {
...
// TestToUnifAIChatCompletionStream_MixedToolBlocks interleaves a no-arg tool
...
func TestToUnifAIChatCompletionStream_MixedToolBlocks(t *testing.T) {
	state := NewAnthropicStreamState()
	...
	r, _, _ := stopNoArg.ToUnifAIChatCompletionStream(nil, "", state)
	if r == nil {
	...
	r, _, _ = stopArg.ToUnifAIChatCompletionStream(nil, "", state)
	if r != nil {
