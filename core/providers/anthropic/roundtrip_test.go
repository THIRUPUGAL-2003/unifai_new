...
// Round-trip tests: AnthropicMessageRequest ──► UnifAI ──► AnthropicMessageRequest
//
// Coverage matrix:
...
//
//	ConvertAnthropicMessagesToUnifAIMessages  (Anthropic → UnifAI)
//	ConvertUnifAIMessagesToAnthropicMessages  (UnifAI   → Anthropic)
func roundTrip(t *testing.T, messages []AnthropicMessage, system *AnthropicContent, provider schemas.ModelProvider, model string) ([]AnthropicMessage, *AnthropicContent) {
	...
	unifai := ConvertAnthropicMessagesToUnifAIMessages(ctx, messages, system, false, false)
	outMsgs, outSystem := ConvertUnifAIMessagesToAnthropicMessages(ctx, unifai, true, provider, model)
