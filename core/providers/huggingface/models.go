...
func (response *HuggingFaceListModelsResponse) ToUnifAIListModelsResponse(providerKey schemas.ModelProvider, inferenceProvider inferenceProvider, allowedModels schemas.WhiteList, blacklistedModels schemas.BlackList, aliases schemas.KeyAliases, unfiltered bool) *schemas.UnifAIListModelsResponse {
	if response == nil {
		return nil
	}
	...
	for _, model := range response.Models {
		...
		for _, result := range pipeline.FilterModel(model.ModelID) {
			...
			unifaiResponse.Data = append(unifaiResponse.Data, newModel)
	...
	for _, m := range pipeline.BackfillModels(included) {
		...
		unifaiResponse.Data = append(unifaiResponse.Data, m)
	}
