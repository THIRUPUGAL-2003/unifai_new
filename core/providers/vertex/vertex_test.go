...
func TestVertexGeminiResponsesPreservesGCSImageURL(t *testing.T) {
	result, err := gemini.ToGeminiResponsesRequestWithImageURLSchemes(nil, &schemas.UnifAIResponsesRequest{
		Provider: schemas.Vertex,
		Model:    "gemini-3-flash-preview",
...
func TestVertexGeminiRejectsUnsupportedScheme(t *testing.T) {
	_, err := gemini.ToGeminiChatCompletionRequestWithImageURLSchemes(nil, &schemas.UnifAIChatRequest{
		Model: "gemini-3-flash-preview",
		Input: []schemas.ChatMessage{
			{
				Role: schemas.ChatMessageRoleUser,
