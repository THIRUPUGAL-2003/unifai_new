...
func TestVertexGeminiImageURLSchemesAllowGCS(t *testing.T) {
	result, err := gemini.ToGeminiChatCompletionRequestWithImageURLSchemes(nil, &schemas.UnifAIChatRequest{
...
func TestResolveVertexProjectID_AliasOverride(t *testing.T) {
	...
	ctx.SetValue(schemas.UnifAIContextKeyResolvedAlias, &schemas.ResolvedAlias{
		Key: "best-claude",
	...
	ctx2 := schemas.NewUnifAIContext(nil, schemas.NoDeadline)
	ctx2.SetValue(schemas.UnifAIContextKeyResolvedAlias, &schemas.ResolvedAlias{
		Key: "x",
		Config: &schemas.AliasConfig{
			ModelID: "x",
			VertexAliasCfg: &schemas.VertexAliasCfg{
...
func TestResolveVertexRegion_AliasOverride(t *testing.T) {
	...
	ctx0 := schemas.NewUnifAIContext(nil, schemas.NoDeadline)
