...
func TestCheckFirstStreamChunk_ErrorInFirstChunk(t *testing.T) {
	stream := make(chan *schemas.UnifAIStreamChunk, 2)
...
func TestCheckFirstStreamChunk_ValidFirstChunk(t *testing.T) {
	stream := make(chan *schemas.UnifAIStreamChunk, 3)
	chunk1 := &schemas.UnifAIStreamChunk{
		UnifAIChatResponse: &schemas.UnifAIChatResponse{
			ID: "chatcmpl-123",
		},
	}
	chunk2 := &schemas.UnifAIStreamChunk{
		UnifAIChatResponse: &schemas.UnifAIChatResponse{
			ID: "chatcmpl-123",
		},
	}
	...
	got1 := <-wrapped
	if got1.UnifAIChatResponse == nil || got1.UnifAIChatResponse.ID != "chatcmpl-123" {
		t.Error("first chunk not re-injected correctly")
	}

	// Second chunk should follow
	got2 := <-wrapped
	if got2.UnifAIChatResponse == nil || got2.UnifAIChatResponse.ID != "chatcmpl-123" {
...
func TestCheckFirstStreamChunk_EmptyStream(t *testing.T) {
	stream := make(chan *schemas.UnifAIStreamChunk)
	close(stream)
...
func TestCheckFirstStreamChunk_ErrorInSecondChunk(t *testing.T) {
	...
	// Read all chunks
	got1 := <-wrapped
	if got1.UnifAIChatResponse == nil {
		t.Error("first chunk should be valid data")
	}
	got2 := <-wrapped
	if got2.UnifAIError == nil {
...
func TestCheckFirstStreamChunk_ErrorDrainsSource(t *testing.T) {
	stream := make(chan *schemas.UnifAIStreamChunk, 5)
	stream <- &schemas.UnifAIStreamChunk{
		UnifAIError: &schemas.UnifAIError{
			Error: &schemas.ErrorField{
	...
	stream <- &schemas.UnifAIStreamChunk{
		UnifAIChatResponse: &schemas.UnifAIChatResponse{ID: "1"},
	}
	stream <- &schemas.UnifAIStreamChunk{
		UnifAIChatResponse: &schemas.UnifAIChatResponse{ID: "2"},
	}
	close(stream)
...
func TestCheckFirstStreamChunk_CtxCancelUnblocksWrapper(t *testing.T) {
	...
	src := make(chan *schemas.UnifAIStreamChunk, 1)
	src <- &schemas.UnifAIStreamChunk{
		UnifAIChatResponse: &schemas.UnifAIChatResponse{ID: "1"},
	}
	...
	src <- &schemas.UnifAIStreamChunk{
		UnifAIChatResponse: &schemas.UnifAIChatResponse{ID: "3"},
	}
	close(src)
...
func TestAttachBilledUsageFromContext_KeepsUsageWithOnlyDetails(t *testing.T) {
	...
	attachBilledUsageFromContext(ctx, unifaiErr)

	if unifaiErr.ExtraFields.BilledUsage == nil {
		t.Fatal("BilledUsage should be attached when cache details are present")
	}
	if unifaiErr.ExtraFields.BilledUsage.PromptTokensDetails == nil ||
		unifaiErr.ExtraFields.BilledUsage.PromptTokensDetails.CachedReadTokens != 5 {
		t.Fatalf("cached read tokens not preserved: %+v", unifaiErr.ExtraFields.BilledUsage)
	}
}
...
func TestAttachBilledUsageFromContext_CopiesUsage(t *testing.T) {
	...
	billed := unifaiErr.ExtraFields.BilledUsage
...
func TestAttachBilledUsageFromContext_NoOpWhenEmpty(t *testing.T) {
	ctx := schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline)
	ctx.SetValue(schemas.UnifAIContextKeyStreamAccumulatedUsage, &schemas.UnifAILLMUsage{})
	unifaiErr := &schemas.UnifAIError{}
	attachBilledUsageFromContext(ctx, unifaiErr)
	if unifaiErr.ExtraFields.BilledUsage != nil {
