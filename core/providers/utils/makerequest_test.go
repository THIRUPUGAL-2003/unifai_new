...
func TestMakeRequestWithContext_SuccessReturnsNoopWait(t *testing.T) {
	...
	latency, unifaiErr, wait := MakeRequestWithContext(context.Background(), client, req, resp)
	defer wait()

	if unifaiErr != nil {
		t.Fatalf("expected no error, got: %v", unifaiErr.Error.Message)
	}
	if latency <= 0 {
...
func TestMakeRequestWithContext_DeadlineExceededReturnsTimeoutError(t *testing.T) {
	...
	_, unifaiErr, wait := MakeRequestWithContext(ctx, client, req, resp)

	// Should get a timeout error with 504 status
	if unifaiErr == nil {
		t.Fatal("expected timeout error")
	}
	if unifaiErr.Error.Type == nil || *unifaiErr.Error.Type != schemas.RequestTimedOut {
		t.Fatalf("expected RequestTimedOut error type, got: %v", unifaiErr.Error.Type)
	}
	if unifaiErr.StatusCode == nil || *unifaiErr.StatusCode != 504 {
		t.Fatalf("expected status 504, got: %v", unifaiErr.StatusCode)
	}
...
func TestMakeRequestWithContext_ContextCancelReturnsCancelledError(t *testing.T) {
	...
	if unifaiErr.Error.Type == nil || *unifaiErr.Error.Type != schemas.RequestCancelled {
		t.Fatalf("expected RequestCancelled error type, got: %v", unifaiErr.Error.Type)
	}
	if unifaiErr.StatusCode == nil || *unifaiErr.StatusCode != 499 {
		t.Fatalf("expected status 499, got: %v", unifaiErr.StatusCode)
	}
...
func TestNewUnifAITimeoutError(t *testing.T) {
	err := NewUnifAITimeoutError("test timeout", context.DeadlineExceeded)

	if !err.IsUnifAIError {
		t.Fatal("expected IsUnifAIError to be true")
	}
	...
	// Note: ExtraFields.Provider is populated by unifai.go's dispatcher via
	// PopulateExtraFields, not by NewUnifAITimeoutError — the constructor has
	// no provider context.
}
...
func TestMakeRequestWithContext_ClientError(t *testing.T) {
	...
	_, unifaiErr, wait := MakeRequestWithContext(context.Background(), client, req, resp)
	defer wait()
	...
	// not the default 400 — see NewUnifAIUpstreamConnectionError.
	if unifaiErr.StatusCode == nil || *unifaiErr.StatusCode != 502 {
		t.Fatalf("expected StatusCode 502, got %v", unifaiErr.StatusCode)
	}
	if unifaiErr.Error.Type == nil || *unifaiErr.Error.Type != schemas.ProviderConnectionFailed {
		t.Fatalf("expected ProviderConnectionFailed type, got %v", unifaiErr.Error.Type)
	}
...
func TestNewUnifAIUpstreamConnectionError(t *testing.T) {
	err := NewUnifAIUpstreamConnectionError("upstream dropped connection", context.DeadlineExceeded)

	if err.IsUnifAIError {
		t.Fatal("expected IsUnifAIError to be false (upstream is at fault)")
	}
