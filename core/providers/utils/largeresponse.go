...
// ctx is held to check UnifAIContextKeyConnectionClosed in Close, so a mid-stream
...
func (r *LargeResponseReader) Close() error {
	...
	// before we read UnifAIContextKeyConnectionClosed. The goroutine's done-branch can set the
	...
	if r.ctx != nil {
		if closed, ok := r.ctx.Value(schemas.UnifAIContextKeyConnectionClosed).(bool); ok && closed {
			r.Resp = nil
			return nil
		}
	}
...
func PrepareResponseStreaming(ctx *schemas.UnifAIContext, client *fasthttp.Client, resp *fasthttp.Response) *fasthttp.Client {
...
func MaterializeStreamErrorBody(ctx *schemas.UnifAIContext, resp *fasthttp.Response) {
	responseThreshold, _ := ctx.Value(schemas.UnifAIContextKeyLargeResponseThreshold).(int64)
	if responseThreshold <= 0 {
		return
	}
...
func FinalizeResponseWithLargeDetection(
	...
	if responseThreshold <= 0 {
		...
		if err != nil {
			return nil, false, NewUnifAIOperationError(schemas.ErrProviderResponseDecode, err)
		}
	...
	if contentLength > 0 && int64(contentLength) <= responseThreshold {
		if bodyStream := resp.BodyStream(); bodyStream != nil {
			...
			if readErr != nil {
				return nil, false, NewUnifAIOperationError(schemas.ErrProviderResponseDecode, readErr)
			}
	...
	if contentLength <= 0 {
		if bodyStream := resp.BodyStream(); bodyStream != nil {
			...
			ctx.SetValue(schemas.UnifAIContextKeyLargeResponseMode, true)
			ctx.SetValue(schemas.UnifAIContextKeyLargeResponseReader, closableReader)
			ctx.SetValue(schemas.UnifAIContextKeyLargeResponseContentLength, contentLength)
			if ct := string(resp.Header.ContentType()); ct != "" {
				ctx.SetValue(schemas.UnifAIContextKeyLargeResponseContentType, ct)
			}
			previewLen := min(len(bodyBytes), 1048576)
			ctx.SetValue(schemas.UnifAIContextKeyLargePayloadResponsePreview, string(bodyBytes[:previewLen]))
			return nil, true, nil
		}
		// No stream — buffered fallback
		body, err := CheckAndDecodeBody(resp)
	...
	if ps, ok := ctx.Value(schemas.UnifAIContextKeyLargePayloadPrefetchSize).(int); ok && ps > 0 {
		prefetchSize = ps
	}
	...
	ctx.SetValue(schemas.UnifAIContextKeyLargePayloadResponsePreview, string(prefetchBuf[:previewLen]))

	return nil, true, nil
}

// ParseOpenAIUsageFromBytes parses OpenAI-format usage from raw JSON bytes into UnifAILLMUsage.
...
func ParseOpenAIUsageFromBytes(data []byte) *schemas.UnifAILLMUsage {
	var usage struct {
...
func SetupStreamingPassthrough(ctx *schemas.UnifAIContext, resp *fasthttp.Response) bool {
	...
	// UnifAIContextKeyConnectionClosed so LargeResponseReader.Close skips the double release.
