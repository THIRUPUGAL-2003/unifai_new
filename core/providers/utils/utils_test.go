...
func TestApplyLargePayloadRequestBodyWithModelNormalization(t *testing.T) {
	...
	ctx.SetValue(schemas.UnifAIContextKeyLargePayloadMode, true)
	ctx.SetValue(
		schemas.UnifAIContextKeyLargePayloadReader,
		strings.NewReader(payload),
	)
	ctx.SetValue(schemas.UnifAIContextKeyLargePayloadContentLength, len(payload))
	ctx.SetValue(schemas.UnifAIContextKeyLargePayloadContentType, "application/json")
	ctx.SetValue(schemas.UnifAIContextKeyLargePayloadMetadata, &schemas.LargePayloadMetadata{
		Model: "openai/gpt-5",
	})
...
// always includes the raw response body in UnifAIError.ExtraFields.RawResponse
func TestHandleProviderAPIError_RawResponseIncluded(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			unifaiErr := HandleProviderAPIError(resp, &errorResp)

			if unifaiErr == nil {
				t.Fatal("HandleProviderAPIError() returned nil")
			}

			if unifaiErr.ExtraFields.RawResponse == nil {
			...
			if tt.name != "Decode failure" {
				rawResponseBytes, err := sonic.Marshal(unifaiErr.ExtraFields.RawResponse)
				if err != nil {
...
func TestEnrichError_PreservesExistingRawResponse(t *testing.T) {
	...
	enrichedErr := EnrichError(ctx, unifaiErr, requestBody, nil, true, true)
...
func TestEnrichError_OverwritesWithProvidedResponse(t *testing.T) {
	...
	enrichedErr := EnrichError(ctx, unifaiErr, requestBody, responseBody, true, true)
...
func TestEnrichError_SetsLatency(t *testing.T) {
	...
	enrichedErr := EnrichError(ctx, unifaiErr, nil, nil, false, false, 42*time.Millisecond)
...
func TestEnrichError_DoesNotSetLatencyWithoutExplicitValue(t *testing.T) {
	...
	enrichedErr := EnrichError(ctx, unifaiErr, nil, nil, false, false)

	if enrichedErr == nil {
		t.Fatal("EnrichError() returned nil")
	}
	if enrichedErr.ExtraFields.Latency != 0 {
		t.Fatalf("latency = %d, want 0", unifaiErr.ExtraFields.Latency)
	}
}
...
func TestEnrichError_RespectsFlags(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			enrichedErr := EnrichError(ctx, unifaiErr, requestBody, responseBody, tt.sendBackRawRequest, tt.sendBackRawResponse)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.contextPath != nil {
				ctx = context.WithValue(ctx, schemas.UnifAIContextKeyURLPath, *tt.contextPath)
			}
...
func TestParseAndSetRawRequest_CompactsJSON(t *testing.T) {
  ...
  "temperature": 0.7
	...
	var extraFields schemas.UnifAIResponseExtraFields
...
func TestParseAndSetRawRequest_SSEStreamingChunks(t *testing.T) {
  ...
  "temperature": 0.7
	...
	// Uses UnifAIChatResponse which is the actual type marshaled in the streaming path.
	chunk := schemas.UnifAIChatResponse{
		ID:     "chatcmpl-test",
		Model:  "gpt-4",
	...
	var parsed schemas.UnifAIChatResponse
...
// BuildClientStreamChunk correctly handles UnifAIImageGenerationStreamResponse:
...
func TestBuildClientStreamChunk_ImageGenerationStripping(t *testing.T) {
	...
	imgResp := &schemas.UnifAIImageGenerationStreamResponse{
	...
	response := &schemas.UnifAIResponse{ImageGenerationStreamResponse: imgResp}

	t.Run("logging-only: raw fields stripped from image gen chunk, original preserved", func(t *testing.T) {
		ctx := schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline)
		ctx.SetValue(schemas.UnifAIContextKeyDropRawRequestFromClient, true)
		ctx.SetValue(schemas.UnifAIContextKeyDropRawResponseFromClient, true)

		chunk := BuildClientStreamChunk(ctx, response, nil)
		if chunk.UnifAIImageGenerationStreamResponse == nil {
			t.Fatal("expected UnifAIImageGenerationStreamResponse in chunk")
		}
		if chunk.UnifAIImageGenerationStreamResponse.ExtraFields.RawRequest != nil {
			t.Error("expected RawRequest stripped from chunk, but it was present")
		}
		if chunk.UnifAIImageGenerationStreamResponse.ExtraFields.RawResponse != nil {
			t.Error("expected RawResponse stripped from chunk, but it was present")
		}
		// Original must not be mutated.
		if imgResp.ExtraFields.RawRequest == nil {
			t.Error("original UnifAIImageGenerationStreamResponse.ExtraFields.RawRequest was mutated")
		}
		if imgResp.ExtraFields.RawResponse == nil {
			t.Error("original UnifAIImageGenerationStreamResponse.ExtraFields.RawResponse was mutated")
		}
		if chunk.UnifAIImageGenerationStreamResponse == imgResp {
...
// It also verifies that the original UnifAIResponse is not mutated
...
func TestProcessAndSendResponse_StoreRawLoggingOnly_StripsRawDataFromResponseChunk(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			passThrough := func(ctx *schemas.UnifAIContext, resp *schemas.UnifAIResponse, err *schemas.UnifAIError) (*schemas.UnifAIResponse, *schemas.UnifAIError) {
				return resp, err
			}

			responseChan := make(chan *schemas.UnifAIStreamChunk, 1)
			ProcessAndSendResponse(ctx, passThrough, response, responseChan, nil)

			chunk := <-responseChan
			if chunk.UnifAIChatResponse == nil {
				t.Fatal("expected non-nil UnifAIChatResponse in stream chunk")
			}

			hasRawReq := chunk.UnifAIChatResponse.ExtraFields.RawRequest != nil
			hasRawResp := chunk.UnifAIChatResponse.ExtraFields.RawResponse != nil
			...
			if tt.expectStripped {
				...
				if response.ChatResponse.ExtraFields.RawRequest == nil {
					t.Error("original UnifAIResponse.ChatResponse.ExtraFields.RawRequest was mutated (nil); shared object must be preserved")
				}
				if response.ChatResponse.ExtraFields.RawResponse == nil {
					t.Error("original UnifAIResponse.ChatResponse.ExtraFields.RawResponse was mutated (nil); shared object must be preserved")
				}
				// The chunk must be a copy, not the same pointer as the original.
				if chunk.UnifAIChatResponse == response.ChatResponse {
					t.Error("chunk.UnifAIChatResponse is the same pointer as the original; it must be a copy to avoid data races")
				}
			} else {
				if !hasRawReq {
...
// that when drop-raw context flags are set, raw data is stripped from UnifAIError
// payloads embedded in stream chunks, without mutating the shared UnifAIError object
// (shared object safety for PostLLMHook goroutines).
func TestProcessAndSendResponse_StoreRawLoggingOnly_StripsRawDataFromErrorChunk(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Use a postHookRunner that converts the response to a UnifAIError with raw data
			unifaiErr := &schemas.UnifAIError{
				IsUnifAIError: false,
			...
			errorRunner := func(ctx *schemas.UnifAIContext, resp *schemas.UnifAIResponse, err *schemas.UnifAIError) (*schemas.UnifAIResponse, *schemas.UnifAIError) {
				return nil, unifaiErr
			}

			responseChan := make(chan *schemas.UnifAIStreamChunk, 1)
			ProcessAndSendResponse(ctx, errorRunner, &schemas.UnifAIResponse{
				ChatResponse: &schemas.UnifAIChatResponse{ID: "chatcmpl-001"},
			}, responseChan, nil)

			chunk := <-responseChan
			if chunk.UnifAIError == nil {
				t.Fatal("expected non-nil UnifAIError in stream chunk")
			}

			hasRawReq := chunk.UnifAIError.ExtraFields.RawRequest != nil
			hasRawResp := chunk.UnifAIError.ExtraFields.RawResponse != nil
			...
			if tt.expectStripped {
				...
				// Critical: the original shared UnifAIError must NOT have been mutated.
				if unifaiErr.ExtraFields.RawRequest == nil {
					t.Error("original UnifAIError.ExtraFields.RawRequest was mutated (nil); shared object must be preserved")
				}
				if unifaiErr.ExtraFields.RawResponse == nil {
					t.Error("original UnifAIError.ExtraFields.RawResponse was mutated (nil); shared object must be preserved")
				}
				// The chunk must hold a copy, not the same pointer as the original.
				if chunk.UnifAIError == unifaiErr {
					t.Error("chunk.UnifAIError is the same pointer as the original; it must be a copy to avoid data races")
				}
			} else {
				if !hasRawReq {
...
//   - UnifAIContextKeyCaptureRawRequest=true in context: always returns true
//   - Logging-only mode: requestWorker sets UnifAIContextKeyCaptureRawRequest=true,
...
func TestShouldSendBackRawRequest(t *testing.T) {
	tests := []struct {
		...
		{
			// requestWorker sets UnifAIContextKeyCaptureRawRequest=true in logging-only
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.contextSendBack {
				ctx.SetValue(schemas.UnifAIContextKeyCaptureRawRequest, true)
			}
...
func TestShouldSendBackRawResponse(t *testing.T) {
	tests := []struct {
		...
		{
			// requestWorker sets UnifAIContextKeyCaptureRawResponse=true in logging-only
			// mode so a single flag covers both full send-back and logging-only cases.
			name:            "logging-only: context SendBack=true set by requestWorker",
			contextSendBack: true,
			providerDefault: false,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			ctx := schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline)
			if tt.contextSendBack {
				ctx.SetValue(schemas.UnifAIContextKeyCaptureRawResponse, true)
			}
...
// UnifAI injects upstream (and some upstreams echo back, e.g. Google's file-download 302) are
...
func TestEnsureStreamFinalizerCalled_ClearsOrphanedDeferredSpan(t *testing.T) {
	...
	ctx.SetValue(schemas.UnifAIContextKeyTraceID, "trace-1")
...
func TestProcessAndSendResponse_CompletesSpanWhenFinalSendFails(t *testing.T) {
	...
	ctx.SetValue(schemas.UnifAIContextKeyTracer, tracer)
	ctx.SetValue(schemas.UnifAIContextKeyTraceID, "trace-1")
	ctx.SetValue(schemas.UnifAIContextKeyStreamEndIndicator, true) // final chunk
	...
	passthrough := func(_ *schemas.UnifAIContext, resp *schemas.UnifAIResponse, err *schemas.UnifAIError) (*schemas.UnifAIResponse, *schemas.UnifAIError) {
		return resp, err
	}

	ProcessAndSendResponse(ctx, passthrough, &schemas.UnifAIResponse{}, responseChan, func(context.Context) {})
