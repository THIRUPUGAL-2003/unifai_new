...
// stuck inside Close() so UnifAIContextKeyConnectionClosed is guaranteed to
...
func TestIdleTimeoutReader_ClosedContextReturnsError(t *testing.T) {
	...
	ctx.SetValue(schemas.UnifAIContextKeyConnectionClosed, true)
...
// SetupStreamCancellation calls Close() before setting UnifAIContextKeyConnectionClosed.
//
...
func TestSetupStreamCancellation_NoPanicOnCancelledContext(t *testing.T) {
	...
	unifaiCtx := schemas.NewUnifAIContext(goCtx, time.Time{})

	reader, cleanupReader := NewIdleTimeoutReader(body, body, time.Minute, unifaiCtx)
	...
	defer cleanupReader()
	stopCancel := SetupStreamCancellation(unifaiCtx, body, getLogger())
	defer stopCancel()
...
func TestStreamClose_ExactlyOnceAcrossOwners(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		...
		goCtx, cancel := context.WithCancel(context.Background())
		ctx := schemas.NewUnifAIContext(goCtx, time.Time{})
...
// relies on: GetAndSetValue on UnifAIContextKeyConnectionClosed is an atomic
...
func TestReleaseStreamingResponse_SkipsWhenAlreadyClaimed(t *testing.T) {
	...
	ctx.GetAndSetValue(schemas.UnifAIContextKeyConnectionClosed, true)
...
// connection-closed, because retries/fallbacks reuse the same UnifAIContext.
func TestReleaseStreamingResponse_NoBodyDoesNotClaimConnection(t *testing.T) {
	...
	if closed, _ := ctx.Value(schemas.UnifAIContextKeyConnectionClosed).(bool); closed {
