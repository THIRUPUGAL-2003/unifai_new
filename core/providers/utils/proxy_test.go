...
func TestConfigureProxy_HTTPProxy_WithEnvURL_ConfiguresDialer(t *testing.T) {
	t.Setenv("UNIFAI_TEST_PROXY_URL", "http://127.0.0.1:1")

	client := &fasthttp.Client{}
	logger := testLogger{}
	cfg := &schemas.ProxyConfig{
		Type: schemas.HTTPProxy,
		URL:  schemas.NewSecretVar("env.UNIFAI_TEST_PROXY_URL"),
	}
...
func TestConfigureProxy_HTTPProxy_WithEmptyEnvValue_FailsFast(t *testing.T) {
	t.Setenv("UNIFAI_TEST_PROXY_URL_EMPTY", "")

	client := &fasthttp.Client{}
	logger := testLogger{}
	cfg := &schemas.ProxyConfig{
		Type: schemas.HTTPProxy,
		URL:  schemas.NewSecretVar("env.UNIFAI_TEST_PROXY_URL_EMPTY"),
	}
	...
	if !strings.Contains(err.Error(), "proxy.url") || !strings.Contains(err.Error(), "env.UNIFAI_TEST_PROXY_URL_EMPTY") {
