...
func TestHandleProviderAPIErrorWithHTML(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if unifaiErr.StatusCode == nil || *unifaiErr.StatusCode != tt.statusCode {
				t.Errorf("HandleProviderAPIError() status code = %v, want %v", unifaiErr.StatusCode, tt.statusCode)
			}
			...
			if tt.expectedInMessage != "" && !strings.Contains(unifaiErr.Error.Message, tt.expectedInMessage) {
				t.Errorf("Expected message to contain %q, got %q", tt.expectedInMessage, unifaiErr.Error.Message)
			}

			t.Logf("Handled %s: status=%d, message=%q", tt.name, *unifaiErr.StatusCode, unifaiErr.Error.Message)
		})
	}
}
