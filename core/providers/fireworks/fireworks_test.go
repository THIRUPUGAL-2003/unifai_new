...
import (
	...
	fireworksprovider "github.com/unifai/unifai/core/providers/fireworks"
...
func resolveFireworksModels(t *testing.T, client *unifai.UnifAI, ctx context.Context) (string, string, string) {
	t.Helper()
	...
	if chatModel == "" || textModel == "" || embeddingModel == "" {
		...
		for page := 0; page < 5; page++ {
			...
			resp, unifaiErr := client.ListModelsRequest(bfCtx, req)
			if unifaiErr != nil {
				if chatModel == "" {
					t.Fatalf("Failed to list Fireworks models for test discovery: %v", llmtests.GetErrorMessage(unifaiErr))
				}
				t.Logf("Fireworks model discovery failed: %v", llmtests.GetErrorMessage(unifaiErr))
				break
			}
...
func fireworksModelSupportsTextCompletions(t *testing.T, client *unifai.UnifAI, ctx context.Context, model string) bool {
	t.Helper()

	prompt := "Say ok"
	bfCtx := schemas.NewUnifAIContext(ctx, schemas.NoDeadline)
	resp, unifaiErr := client.TextCompletionRequest(bfCtx, &schemas.UnifAITextCompletionRequest{
		Provider: schemas.Fireworks,
		Model:    model,
	...
	if unifaiErr != nil {
		t.Logf("Fireworks /v1/completions probe failed for %q: %v", model, llmtests.GetErrorMessage(unifaiErr))
		return false
	}
...
func fireworksModelSupportsEmbeddings(t *testing.T, client *unifai.UnifAI, ctx context.Context, model string) bool {
	t.Helper()

	text := "embedding probe"
	bfCtx := schemas.NewUnifAIContext(ctx, schemas.NoDeadline)
	resp, unifaiErr := client.EmbeddingRequest(bfCtx, &schemas.UnifAIEmbeddingRequest{
		Provider: schemas.Fireworks,
		Model:    model,
	...
	if unifaiErr != nil {
		t.Logf("Fireworks /v1/embeddings probe failed for %q: %v", model, llmtests.GetErrorMessage(unifaiErr))
		return false
	}
...
// normalizeFireworksModelID strips any provider prefix so tests can pass raw Fireworks model IDs to UnifAI requests.
...
func TestFireworksProviderUsesNativeEndpoints(t *testing.T) {
	tests := []struct {
		name         string
		expectedPath string
		run          func(t *testing.T, provider *fireworksprovider.FireworksProvider, ctx *schemas.UnifAIContext, key schemas.Key)
	}{
		{
			name:         "TextCompletion",
			...
			run: func(t *testing.T, provider *fireworksprovider.FireworksProvider, ctx *schemas.UnifAIContext, key schemas.Key) {
				...
				resp, err := provider.TextCompletion(ctx, key, &schemas.UnifAITextCompletionRequest{
					Provider: schemas.Fireworks,
		...
		},
		{
			...
			run: func(t *testing.T, provider *fireworksprovider.FireworksProvider, ctx *schemas.UnifAIContext, key schemas.Key) {
				resp, err := provider.Responses(ctx, key, &schemas.UnifAIResponsesRequest{
					Provider: schemas.Fireworks,
					Model:    "accounts/fireworks/models/deepseek-v3p2",
		...
		},
		{
			...
			run: func(t *testing.T, provider *fireworksprovider.FireworksProvider, ctx *schemas.UnifAIContext, key schemas.Key) {
				resp, err := provider.Embedding(ctx, key, &schemas.UnifAIEmbeddingRequest{
					Provider: schemas.Fireworks,
...
func TestFireworksResponsesStreamUsesNativeResponsesEndpoint(t *testing.T) {
	...
	ctx := schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline)
	key := schemas.Key{Value: *schemas.NewSecretVar("test-key")}
	postHookRunner := func(_ *schemas.UnifAIContext, result *schemas.UnifAIResponse, err *schemas.UnifAIError) (*schemas.UnifAIResponse, *schemas.UnifAIError) {
		return result, err
	}

	stream, err := provider.ResponsesStream(ctx, postHookRunner, nil, key, &schemas.UnifAIResponsesRequest{
		Provider: schemas.Fireworks,
		Model:    "accounts/fireworks/models/deepseek-v3p2",
		Input: []schemas.ResponsesMessage{
			llmtests.CreateBasicResponsesMessage("hello"),
	...
	for chunk := range stream {
		if chunk != nil && chunk.UnifAIResponsesStreamResponse != nil &&
			chunk.UnifAIResponsesStreamResponse.Type == schemas.ResponsesStreamResponseTypeCompleted {
			sawCompleted = true
		}
	}
