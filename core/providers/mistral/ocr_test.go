...
// TestToMistralOCRRequest tests conversion from UnifAI OCR request to Mistral OCR request.
func TestToMistralOCRRequest(t *testing.T) {
	t.Parallel()

	tests := []struct {
		name     string
		input    *schemas.UnifAIOCRRequest
...
// TestToUnifAIOCRResponse tests conversion from Mistral OCR response to UnifAI OCR response.
func TestToUnifAIOCRResponse(t *testing.T) {
	t.Parallel()

	tests := []struct {
		name     string
		input    *MistralOCRResponse
		validate func(t *testing.T, result *schemas.UnifAIOCRResponse)
	}{
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			result := tt.input.ToUnifAIOCRResponse()
			tt.validate(t, result)
		})
	}
}
...
func TestOCRWithMockServer(t *testing.T) {
	...
	tests := []struct {
		name           string
		request        *schemas.UnifAIOCRRequest
		statusCode     int
		responseBody   interface{}
		expectError    bool
		errorContains  string
		validateError  func(t *testing.T, err *schemas.UnifAIError)
		validateResult func(t *testing.T, resp *schemas.UnifAIOCRResponse)
	}{
		{
...
func TestMistralOCRIntegration(t *testing.T) {
	...
	resp, unifaiErr := provider.OCR(ctx, schemas.Key{Value: *schemas.NewSecretVar(apiKey)}, request)

	require.Nil(t, unifaiErr, "OCR request failed: %v", unifaiErr)
	require.NotNil(t, resp)
	assert.Equal(t, "mistral-ocr-latest", resp.Model)
