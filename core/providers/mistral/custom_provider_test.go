...
func TestParseMistralError_UsesExportedConverterMetadata(t *testing.T) {
	...
	unifaiErr := ParseMistralError(resp)
	require.NotNil(t, unifaiErr)
	require.NotNil(t, unifaiErr.Error)

	assert.Equal(t, "invalid request", unifaiErr.Error.Message)
	assert.Equal(t, schemas.Ptr("invalid_request_error"), unifaiErr.Error.Type)
	assert.Equal(t, schemas.Ptr("bad_request"), unifaiErr.Error.Code)
...
func TestMistralProvider_CustomAliasChatStreamUsesBaseCompatibilityAndAliasMetadata(t *testing.T) {
	...
	ctx.SetValue(schemas.UnifAIContextKeyIsCustomProvider, true)
	...
	postHookRunner := func(_ *schemas.UnifAIContext, response *schemas.UnifAIResponse, err *schemas.UnifAIError) (*schemas.UnifAIResponse, *schemas.UnifAIError) {
		return response, err
	}

	stream, unifaiErr := provider.ChatCompletionStream(ctx, postHookRunner, nil, schemas.Key{}, request)
	require.Nil(t, unifaiErr)

	var firstResponse *schemas.UnifAIChatResponse
	for chunk := range stream {
		if chunk.UnifAIError != nil {
			t.Fatalf("unexpected stream error: %s", chunk.UnifAIError.Error.Message)
		}
		if chunk.UnifAIChatResponse != nil {
			firstResponse = chunk.UnifAIChatResponse
			break
		}
	}

	require.NotNil(t, firstResponse)
	// Note: ExtraFields.Provider on stream chunks is populated by unifai.go's
...
func TestMistralProvider_CustomAliasEmbeddingReportsAliasMetadata(t *testing.T) {
	...
	response, unifaiErr := provider.Embedding(ctx, schemas.Key{}, request)
	require.Nil(t, unifaiErr)
	require.NotNil(t, response)

	// Note: ExtraFields.Provider and ResolvedModelUsed are populated by
	// unifai.go's dispatcher via PopulateExtraFields, not by provider
