...
func TestEmptyCandidatesRegression(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			var unifaiResp *schemas.UnifAIChatResponse

			if tt.isStream {
				unifaiResp, _, _ = tt.response.ToUnifAIChatCompletionStream(gemini.NewGeminiStreamState())
			} else {
				unifaiResp = tt.response.ToUnifAIChatResponse()
			}

			// Critical: Choices must NEVER be empty (this was the PR #1018 bug)
			require.NotNil(t, unifaiResp, "Response should not be nil")
			require.NotEmpty(t, unifaiResp.Choices, "Empty choices array")
			require.Len(t, unifaiResp.Choices, 1, "Should have exactly one error choice")
...
func TestToUnifAIEmbeddingResponsePreservesPrecision(t *testing.T) {
	const want = 0.12345678901234568

	resp := gemini.ToUnifAIEmbeddingResponse(&gemini.GeminiEmbeddingResponse{
...
func TestThoughtSignatureInToolCalls(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.NotNil(t, unifaiResp, "Response should not be nil")
			require.NotEmpty(t, unifaiResp.Choices, "Should have choices")
			...
			for _, detail := range reasoningDetails {
				if detail.Type == schemas.UnifAIReasoningDetailsTypeEncrypted && detail.Signature != nil {
					foundEncrypted = true
					break
				}
			}
...
func TestMissingThoughtSignatureUsesBypassSentinel(t *testing.T) {
	result, err := gemini.ToGeminiChatCompletionRequest(nil, &schemas.UnifAIChatRequest{
...
func TestGeminiChatCompletionRejectsGCSImageURL(t *testing.T) {
	_, err := gemini.ToGeminiChatCompletionRequest(nil, &schemas.UnifAIChatRequest{
...
func TestGeminiResponsesRejectsGCSImageURL(t *testing.T) {
	_, err := gemini.ToGeminiResponsesRequest(nil, &schemas.UnifAIResponsesRequest{
		Model: "gemini-3-flash-preview",
...
// TestUnifAIToGeminiToolConversion tests the conversion of tools from UnifAI to Gemini format
func TestUnifAIToGeminiToolConversion(t *testing.T) {
	tests := []struct {
		name     string
...
func TestUnifAIToGeminiToolConversion_PropertyOrdering(t *testing.T) {
	input := &schemas.UnifAIChatRequest{
		Model: "gemini-2.0-flash",
		Input: []schemas.ChatMessage{{
			Role:    schemas.ChatMessageRoleUser,
...
func TestUnifAIToGeminiToolConversion_NestedPropertyOrdering(t *testing.T) {
	input := &schemas.UnifAIChatRequest{
		Model: "gemini-2.0-flash",
		Input: []schemas.ChatMessage{{
			Role:    schemas.ChatMessageRoleUser,
...
func TestResponsesStructuredOutputConversion(t *testing.T) {
	tests := []struct {
		name     string
		input    *schemas.UnifAIResponsesRequest
		validate func(t *testing.T, result *gemini.GeminiGenerationRequest)
	}{
		{
...
func TestServiceTierReverseMapping(t *testing.T) {
	tests := []struct {
		...
		expectedTier *schemas.UnifAIServiceTier
	}{
		{
			name:         "standard maps to default",
			geminiTier:   gemini.ServiceTierStandard,
			expectedTier: schemas.Ptr(schemas.UnifAIServiceTierDefault),
		},
		{
			name:         "flex maps to flex",
			geminiTier:   gemini.ServiceTierFlex,
			expectedTier: schemas.Ptr(schemas.UnifAIServiceTierFlex),
		},
		{
			name:         "priority maps to priority",
			geminiTier:   gemini.ServiceTierPriority,
			expectedTier: schemas.Ptr(schemas.UnifAIServiceTierPriority),
		},
		{
			name:         "unspecified maps to auto",
			geminiTier:   gemini.ServiceTierUnspecified,
			expectedTier: schemas.Ptr(schemas.UnifAIServiceTierAuto),
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			unifaiReq := geminiReq.ToUnifAIResponsesRequest(ctx)
			require.NotNil(t, unifaiReq)
			require.NotNil(t, unifaiReq.Params)
			assert.Equal(t, tt.expectedTier, unifaiReq.Params.ServiceTier)
		})
	}
}
...
// TestUnifAIResponsesToGeminiToolConversion tests the conversion of tools from UnifAI Responses API to Gemini format
func TestUnifAIResponsesToGeminiToolConversion(t *testing.T) {
	tests := []struct {
		name     string
		input    *schemas.UnifAIResponsesRequest
		validate func(t *testing.T, result *gemini.GeminiGenerationRequest)
...
func TestConvertGeminiUsageMetadataToChatUsage(t *testing.T) {
	tests := []struct {
		...
		expected *schemas.UnifAILLMUsage
	}{
		{
...
// TestConvertGeminiUsageMetadataToResponsesUsage tests the conversion of Gemini usage metadata to UnifAI responses usage
...
func TestGeminiToolInputKeyOrderPreservation(t *testing.T) {
	...
	result, err := gemini.ToGeminiChatCompletionRequest(nil, unifaiReq)
	require.NoError(t, err)
	require.NotNil(t, result)
...
func TestGenAIThinkingLevel_RoundTripPreservesLevelNotBudget(t *testing.T) {
	...
	unifaiReq := geminiReq.ToUnifAIResponsesRequest(unifaiCtx)
	require.NotNil(t, unifaiReq.Params)
	require.NotNil(t, unifaiReq.Params.Reasoning)
	require.NotNil(t, unifaiReq.Params.Reasoning.Effort)
	assert.Equal(t, "minimal", *unifaiReq.Params.Reasoning.Effort)
	assert.Nil(t, unifaiReq.Params.Reasoning.MaxTokens, "thinkingLevel must not populate reasoning max_tokens")

	roundTrip, err := gemini.ToGeminiResponsesRequest(nil, unifaiReq)
	require.NoError(t, err)
...
// Regression: MAX_TOKENS from Gemini must survive Gemini → UnifAI → Gemini on the GenAI path
// (StopReason used to be dropped, so clients saw STOP instead of MAX_TOKENS).
func TestGenAIFinishReasonMaxTokens_PersistsThroughUnifAIRoundTrip(t *testing.T) {
	...
	unifaiResp := geminiResp.ToResponsesUnifAIResponsesResponse()
	require.NotNil(t, unifaiResp)
	require.NotNil(t, unifaiResp.StopReason)
	assert.Equal(t, "length", *unifaiResp.StopReason)

	out := gemini.ToGeminiResponsesResponse(unifaiResp)
...
func TestGenAIFallbacks_PreservedInUnifAIResponsesRequest(t *testing.T) {
	geminiReq := &gemini.GeminiGenerationRequest{
	...
	require.NotNil(t, unifaiReq)
	require.Len(t, unifaiReq.Fallbacks, 1)
	assert.Equal(t, schemas.Vertex, unifaiReq.Fallbacks[0].Provider)
	assert.Equal(t, "gemini-3-flash-preview", unifaiReq.Fallbacks[0].Model)
}

// TestNormalizeRawGenerateContentRequestForCompatibility tests that UnifAI-internal fields
...
// AllowedFunctionNames survive the Gemini→UnifAI→Gemini round-trip on the GenAI passthrough path.
...
func TestFunctionCallingConfigModeAny_RoundTrip(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.NotNil(t, unifaiReq)
			require.NotNil(t, unifaiReq.Params)
			require.NotNil(t, unifaiReq.Params.ToolChoice, "ToolChoice must be set")
...
// GeminiGenerationRequest → UnifAIResponsesRequest → GeminiGenerationRequest round-trip
...
func TestMultimodalFunctionResponse_RoundTrip(t *testing.T) {
	...
	// --- Gemini -> UnifAI: image must be reconstructed as content blocks ---
	unifaiCtx := schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline)
	unifaiReq := geminiReq.ToUnifAIResponsesRequest(unifaiCtx)
	require.NotNil(t, unifaiReq)

	var outputMsg *schemas.ResponsesMessage
	for i := range unifaiReq.Input {
		m := &unifaiReq.Input[i]
	...
	// --- UnifAI -> Gemini: image must land back in functionResponse.parts ---
	roundTrip, err := gemini.ToGeminiResponsesRequest(nil, unifaiReq)
	require.NoError(t, err)
	require.NotNil(t, roundTrip)
...
// survive the Gemini -> UnifAI -> Gemini round-trip instead of being dropped, while the
...
// GeminiGenerationRequest → UnifAIImageGenerationRequest → GeminiGenerationRequest round-trip
...
func TestImageSizeRoundtrip(t *testing.T) {
	tests := []struct {
		...
		wantSize        string // expected UnifAI WxH
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			unifaiReq := inReq.ToUnifAIImageGenerationRequest(ctx)
			require.NotNil(t, unifaiReq)
			require.NotNil(t, unifaiReq.Params.Size, "Size must be extracted from ImageConfig")
			assert.Equal(t, tt.wantSize, *unifaiReq.Params.Size)

			outReq := gemini.ToGeminiImageGenerationRequest(unifaiReq)
...
func TestImageEditSizeRoundtrip(t *testing.T) {
	...
	unifaiReq := inReq.ToUnifAIImageEditRequest(ctx)
	require.NotNil(t, unifaiReq)
	require.NotNil(t, unifaiReq.Params.Size, "Size must be extracted from ImageConfig in edit path")
	assert.Equal(t, "2048x2048", *unifaiReq.Params.Size)

	outReq := gemini.ToGeminiImageEditRequest(unifaiReq)
	require.NotNil(t, outReq)
...
func TestImagenImageSizeCasing(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.wantImageSize, func(t *testing.T) {
			...
			imagenReq := gemini.ToImagenImageGenerationRequest(unifaiReq)
...
func TestImagenImageEditSizeRoundtrip(t *testing.T) {
	...
	imagenReq := gemini.ToImagenImageEditRequest(unifaiReq)
	require.NotNil(t, imagenReq)
