...
// TestConvertUnifAIToolsToGemini_UnionTypeProperty verifies that tool parameters
// with JSON Schema union types are passed through unchanged in parametersJsonSchema.
func TestConvertUnifAIToolsToGemini_UnionTypeProperty(t *testing.T) {
	toolJSON := `{
		"type": "function",
		"function": {
	...
	geminiTools, err := convertUnifAIToolsToGemini([]schemas.ChatTool{chatTool})
...
// TestConvertUnifAIToolsToGemini_WirePayload verifies that the final
...
// are not sufficient.
func TestConvertUnifAIToolsToGemini_WirePayload(t *testing.T) {
	tests := []struct {
		name         string
		propertyJSON string
		propertyName string
...
func TestConvertUnifAIToolsToGemini_WirePayloadPreservesDefs(t *testing.T) {
	...
	geminiTools, err := convertUnifAIToolsToGemini([]schemas.ChatTool{copied})
	require.NoError(t, err)
	require.Len(t, geminiTools, 1)
