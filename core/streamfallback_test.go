...
// Regression tests for https://github.com/unifai/unifai/issues/4788.
//
...
// UnifAIContext (ReleaseStreamingResponse). That claim is scoped to the
...
func drainChatStream(ch chan *schemas.UnifAIStreamChunk) (string, []string) {
	var content strings.Builder
	var errs []string
	for chunk := range ch {
		if chunk.UnifAIError != nil && chunk.UnifAIError.Error != nil {
			errs = append(errs, chunk.UnifAIError.Error.Message)
			continue
		}
		if chunk.UnifAIChatResponse == nil {
			continue
		}
		for _, choice := range chunk.UnifAIChatResponse.Choices {
...
func newStreamTestClient(t *testing.T, account *MockAccount) *UnifAI {
	t.Helper()
	client, err := Init(context.Background(), schemas.UnifAIConfig{
		Account: account,
...
func TestStreamFallbackAfterFirstChunkError(t *testing.T) {
	...
	ctx := schemas.NewUnifAIContext(context.Background(), time.Now().Add(30*time.Second))
	stream, unifaiErr := client.ChatCompletionStreamRequest(ctx, &schemas.UnifAIChatRequest{
	...
	if unifaiErr != nil {
		t.Fatalf("fallback stream failed (fallback server hit %d time(s)): %s", fallbackHits.Load(), unifaiErr.Error.Message)
	}
...
func TestStreamRetryAfterFirstChunkError(t *testing.T) {
	...
	if unifaiErr != nil {
		t.Fatalf("retried stream failed (server hit %d time(s)): %s", hits.Load(), unifaiErr.Error.Message)
	}
	content, errs := drainChatStream(stream)
	if hits.Load() != 2 {
