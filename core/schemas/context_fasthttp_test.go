...
// TestUnifAIContext_Value_DoesNotConsultPooledRequestCtx is the deterministic
// half of the streaming use-after-release crash (the chronic nil-pointer panic
// in HandleAnthropicResponsesStream → PostLLMHook → UnifAIContext.Value).
//
// Production shape:
//
//	transports/unifai-http/lib/ctx.go:199
//	    parent := context.Context(ctx)            // ctx is *fasthttp.RequestCtx
//	    unifaiCtx, _ = NewUnifAIContextWithCancel(parent)
...
//
// Because ConvertToUnifAIContext already copies every RequestCtx user value
// into userValues, the RequestCtx parent is never legitimately needed for value
// lookups. The contract this test locks in: UnifAIContext.Value must NOT read
...
func TestUnifAIContext_Value_DoesNotConsultPooledRequestCtx(t *testing.T) {
	var rctx fasthttp.RequestCtx
	rctx.Init(&fasthttp.Request{}, nil, nil) // wires ctx.s = fakeServer (non-cancelling)

	// A value present ONLY on the RequestCtx, never copied into the unifai ctx.
	...
	bc := NewUnifAIContext(&rctx, NoDeadline)

	if got := bc.Value(key); got != nil {
		t.Fatalf("UnifAIContext.Value fell through to the pooled *fasthttp.RequestCtx "+
			"(got %v); after the fix it must return nil without reading the recycled ctx", got)
	}
}

// TestUnifAIContext_Value_RacesRecycledRequestCtx faithfully reproduces the
...
// the crash exists.
func TestUnifAIContext_Value_RacesRecycledRequestCtx(t *testing.T) {
	var rctx fasthttp.RequestCtx
