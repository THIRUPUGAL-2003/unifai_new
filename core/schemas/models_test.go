...
// Raw types that mirror the real KeyStatus → UnifAIError → ExtraFields → KeyStatus
...
type rawKeyStatusUnifAIError struct {
...
type rawKeyStatus struct {
	...
	Error    *rawKeyStatusUnifAIError `json:"error,omitempty"`
}

// TestKeyStatusMarshalJSON_ReproduceCycle proves that without the custom MarshalJSON,
// the circular reference between KeyStatus and UnifAIError causes a marshaling failure.
func TestKeyStatusMarshalJSON_ReproduceCycle(t *testing.T) {
	unifaiErr := &rawKeyStatusUnifAIError{
		IsUnifAIError: true,
	...
	unifaiErr.ExtraFields.KeyStatuses = []rawKeyStatus{keyStatus}
...
func TestKeyStatusMarshalJSON_NoCycle(t *testing.T) {
	...
	// Create the same cycle that HandleKeylessListModelsRequest creates
	unifaiErr.ExtraFields.KeyStatuses = []KeyStatus{keyStatus}
