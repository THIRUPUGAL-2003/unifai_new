...
// UnifAIHTTPMiddleware is a middleware function for the UnifAI HTTP transport.
...
type UnifAIHTTPMiddleware func(next fasthttp.RequestHandler) fasthttp.RequestHandler
...
type LLMPluginShortCircuit struct {
	...
	Stream   chan *UnifAIStreamChunk // If set, short-circuit with this stream (skips provider call)
...
type MCPPluginShortCircuit struct {
	Response *UnifAIMCPResponse // If set, short-circuit with this response (skips MCP call)
	Error    *UnifAIError       // If set, short-circuit with this error (can set AllowFallbacks field)
}
...
// authors don't have to wrap responses in UnifAIMCPResponse.
type MCPConnectionShortCircuit struct {
	Response *UnifAIMCPConnectResponse // If set, short-circuit with this synthetic Connect outcome
	Error    *UnifAIError              // If set, short-circuit with this error
}
