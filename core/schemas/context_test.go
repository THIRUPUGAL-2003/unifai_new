...
func TestUnifAIContext_SetTraceAttribute_UsesRootSpanHandle(t *testing.T) {
	tracer := &traceAttributeTestTracer{}
	ctx := NewUnifAIContext(context.Background(), NoDeadline)
	ctx.SetValue(UnifAIContextKeyTracer, tracer)
	ctx.SetValue(UnifAIContextKeyTraceID, "trace-123")
...
func TestNewUnifAIContext_NoGoroutineLeakWithBackgroundAndNoDeadline(t *testing.T) {
	...
	contexts := make([]*UnifAIContext, 100)
	for i := 0; i < 100; i++ {
		contexts[i] = NewUnifAIContext(context.Background(), NoDeadline)
	}
...
func TestNewUnifAIContext_GoroutineStartsWithDeadline(t *testing.T) {
	// Get baseline goroutine count
	runtime.GC()
	time.Sleep(10 * time.Millisecond)
	baseline := runtime.NumGoroutine()
	...
	ctx := NewUnifAIContext(context.Background(), deadline)
...
func TestNewUnifAIContext_GoroutineStartsWithCancellableParent(t *testing.T) {
	// Get baseline goroutine count
	runtime.GC()
	time.Sleep(10 * time.Millisecond)
	baseline := runtime.NumGoroutine()
	...
	// Create UnifAIContext with cancellable parent but no deadline
...
func TestNewUnifAIContext_DeadlineExpires(t *testing.T) {
...
func TestNewUnifAIContext_SetAndGetValue(t *testing.T) {
	ctx := NewUnifAIContext(context.Background(), NoDeadline)
...
func TestNewUnifAIContext_NilParent(t *testing.T) {
	// Should not panic with nil parent
	// Note: passing nil is allowed by NewUnifAIContext which converts it to context.Background()
	var nilCtx context.Context //nolint:staticcheck // testing nil parent handling
	ctx := NewUnifAIContext(nilCtx, NoDeadline)
...
func TestPluginLog_ScopedContextValueDelegation(t *testing.T) {
	...
	val := scoped.Value(UnifAIContextKeyTraceID)
	if val != "trace-123" {
...
func TestRoot_UnwrapsChainedValueDelegates(t *testing.T) {
	root := NewUnifAIContext(context.Background(), NoDeadline)
...
// TestNewUnifAIContext_DerivedFromReleasedScope_NoPanic locks in the
// deterministic half of the scoped-parent-release bug: a derived UnifAIContext
...
//
//	root := NewUnifAIContext(...)
//	scope := root.WithPluginScope(...)
//	derived := NewUnifAIContext(scope, NoDeadline)   // derived.parent = scope
...
func TestNewUnifAIContext_DerivedFromReleasedScope_NoPanic(t *testing.T) {
	root := NewUnifAIContext(context.Background(), NoDeadline)
	pluginName := "release-race"
	scope := root.WithPluginScope(&pluginName)

	derived := NewUnifAIContext(scope, NoDeadline)

	// Release the scope while a derived child is still alive — same shape as
	// core/unifai.go's plugin pipeline releasing pluginCtx after PreLLMHook
...
// TestNewUnifAIContext_WatchdogRaceWithReleasedScope is the stress companion
...
func TestNewUnifAIContext_WatchdogRaceWithReleasedScope(t *testing.T) {
	root := NewUnifAIContext(context.Background(), NoDeadline)
	pluginName := "watchdog-race"

	const iterations = 200
	derivedCtxs := make([]*UnifAIContext, 0, iterations)
	for i := 0; i < iterations; i++ {
		scope := root.WithPluginScope(&pluginName)
		// Spawning watchCancellation: parent (scope) has a non-nil Done()
		// channel, so NewUnifAIContext schedules the goroutine.
		derivedCtxs = append(derivedCtxs, NewUnifAIContext(scope, NoDeadline))
		scope.ReleasePluginScope()
	}
