...
type StreamAccumulatorResult struct {
	...
	TokenUsage            *UnifAILLMUsage                // Token usage
	Cost                  *float64                        // Cost in dollars
	CacheDebug            *UnifAICacheDebug              // Semantic cache debug info if available
	ErrorDetails          *UnifAIError                   // Error details if any
	AudioOutput           *UnifAISpeechResponse          // For speech streaming
	TranscriptionOutput   *UnifAITranscriptionResponse   // For transcription streaming
	ImageGenerationOutput *UnifAIImageGenerationResponse // For image generation streaming
	PassthroughOutput     *UnifAIPassthroughResponse     // For passthrough streaming
	FinishReason          *string                         // Finish reason
	RawResponse           *string                         // Raw response
	RawRequest            interface{}                     // Raw request
}

// Tracer defines the interface for distributed tracing in UnifAI.
// Implementations can be injected via UnifAIConfig to enable automatic instrumentation.
...
type Tracer interface {
	...
	PopulateLLMRequestAttributes(handle SpanHandle, req *UnifAIRequest)
	...
	PopulateLLMResponseAttributes(ctx *UnifAIContext, handle SpanHandle, resp *UnifAIResponse, err *UnifAIError)
	...
	// Pass the full UnifAIResponse to capture content, tool calls, reasoning, etc.
	// This is called for each streaming chunk to build up the complete response.
	AddStreamingChunk(traceID string, response *UnifAIResponse)
	...
	GetAccumulatedChunks(traceID string) (response *UnifAIResponse, ttftNs int64, chunkCount int)
	...
	ProcessStreamingChunk(ctx *UnifAIContext, traceID string, isFinalChunk bool, result *UnifAIResponse, err *UnifAIError) *StreamAccumulatorResult
	...
	EndStream(traceID string, err *UnifAIError)
	...
	GetAccumulatedResponse(traceID string) *UnifAIResponse
	...
	GateSend(traceID string, chunk *UnifAIStreamChunk, isFinal, isHardErr bool, ch chan *UnifAIStreamChunk, ctx *UnifAIContext) bool
...
func (n *NoOpTracer) PopulateLLMRequestAttributes(_ SpanHandle, _ *UnifAIRequest) {}

// PopulateLLMResponseAttributes does nothing.
func (n *NoOpTracer) PopulateLLMResponseAttributes(_ *UnifAIContext, _ SpanHandle, _ *UnifAIResponse, _ *UnifAIError) {
}
...
func (n *NoOpTracer) AddStreamingChunk(_ string, _ *UnifAIResponse) {}

// GetAccumulatedChunks returns nil, 0, 0.
func (n *NoOpTracer) GetAccumulatedChunks(_ string) (*UnifAIResponse, int64, int) { return nil, 0, 0 }
...
func (n *NoOpTracer) ProcessStreamingChunk(_ *UnifAIContext, _ string, _ bool, _ *UnifAIResponse, _ *UnifAIError) *StreamAccumulatorResult {
	return nil
}
...
func (n *NoOpTracer) EndStream(_ string, _ *UnifAIError) {}
...
func (n *NoOpTracer) GetAccumulatedResponse(_ string) *UnifAIResponse { return nil }
...
func (n *NoOpTracer) GateSend(_ string, chunk *UnifAIStreamChunk, _ bool, _ bool, ch chan *UnifAIStreamChunk, ctx *UnifAIContext) (ok bool) {
	defer func() {
		if recover() != nil {
			ok = false
		}
...
// real Tracer via the UnifAI config — typically `framework/streaming/Accumulator`,
