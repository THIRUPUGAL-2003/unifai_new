...
func TestSecretVar_IsSet(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			input:    &SecretVar{Val: "vault.unifai/key", ref: "vault.unifai/key", SecretType: SecretTypeVault},
			expected: true,
		},
	}
...
func TestSecretVar_Scan_VaultRef(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			input:          "vault.unifai/providers/openai/key",
			wantVal:        "",
			wantRef:        "vault.unifai/providers/openai/key",
			wantFromSecret: true,
		},
		{
...
func TestSecretVar_UnmarshalJSON_VaultRef(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			input:          `{"value":"","ref":"vault.unifai/key","type":"vault"}`,
			wantVal:        "",
			wantRef:        "vault.unifai/key",
			wantFromSecret: true,
		},
		{
...
func TestSecretVar_Value_VaultRef(t *testing.T) {
	e := &SecretVar{Val: "actual-secret", ref: "vault.unifai/key", SecretType: SecretTypeVault}
	got, err := e.Value()
	if err != nil {
		t.Fatalf("Value() error: %v", err)
	}
	if got != "vault.unifai/key" {
		t.Errorf("Value() = %q, want %q", got, "vault.unifai/key")
	}
}

func TestSecretVar_Redacted_VaultRef(t *testing.T) {
	e := &SecretVar{Val: "actual-secret-value", ref: "vault.unifai/key", SecretType: SecretTypeVault}
	r := e.Redacted()
	wantVal := "actu************************alue"
	if r.Val != wantVal {
	...
	if r.GetRawRef() != "vault.unifai/key" {
		t.Errorf("Redacted().GetRawRef() = %q, want %q", r.GetRawRef(), "vault.unifai/key")
	}
}

func TestSecretVar_IsSet_VaultRef(t *testing.T) {
	set := &SecretVar{Val: "vault.unifai/key", ref: "vault.unifai/key", SecretType: SecretTypeVault}
...
func TestNewSecretVar_VaultRef(t *testing.T) {
	e := NewSecretVar("vault.unifai/mykey")
	if !e.IsFromVault() {
		t.Error("IsFromVault() = false, want true")
	}
	if e.GetRawRef() != "vault.unifai/mykey" {
		t.Errorf("Ref() = %q, want %q", e.GetRawRef(), "vault.unifai/mykey")
	}
	if e.Val != "" {
