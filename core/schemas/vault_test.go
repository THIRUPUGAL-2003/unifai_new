...
func TestStoreVaultSecretVar_StoresPlaintext(t *testing.T) {
	...
	if err := StoreVaultSecretVar(context.Background(), "unifai/tbl/id/value", e); err != nil {
		t.Fatalf("StoreVaultSecretVar: %v", err)
	}
	if got := stored["unifai/tbl/id/value"]; got != "secret-key" {
		t.Errorf("stored plaintext = %q, want %q", got, "secret-key")
	}
	if !e.IsFromVault() {
		t.Error("IsFromVault() should be true after store")
	}
	if e.GetRawRef() != "vault.unifai/tbl/id/value" {
		t.Errorf("Ref() = %q, want %q", e.GetRawRef(), "vault.unifai/tbl/id/value")
	}
	if e.Val != "vault.unifai/tbl/id/value" {
...
func TestStoreVaultSecretVar_NoOps(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			stored := withStubVaultStore(t)
			if err := StoreVaultSecretVar(context.Background(), "unifai/tbl/id/f", tc.e); err != nil {
				t.Fatalf("StoreVaultSecretVar: %v", err)
			}
			if len(stored) != 0 {
...
func TestRemoveOwnedVaultSecretVars_SkipsFragmentRefs(t *testing.T) {
	...
	m := &model{
		Normal:   SecretVar{Val: "vault.unifai/m/1/normal", ref: "vault.unifai/m/1/normal", SecretType: SecretTypeVault},
		Fragment: SecretVar{Val: "vault.external/db#apiKey", ref: "vault.external/db#apiKey", SecretType: SecretTypeVault},
	}

	RemoveOwnedVaultSecretVars(context.Background(), "unifai/m/1", m)

	if len(removed) != 1 || removed[0] != "unifai/m/1/normal" {
		t.Errorf("removed = %v, want only [unifai/m/1/normal]", removed)
	}
}
...
func TestStoreOwnedVaultSecretVars_WalksFields(t *testing.T) {
	...
	if err := StoreOwnedVaultSecretVars(context.Background(), "unifai/m/1", m); err != nil {
		t.Fatalf("StoreOwnedVaultSecretVars: %v", err)
	}

	want := map[string]string{
		"unifai/m/1/plain_col": "p1",
		"unifai/m/1/ptr_col":   "p2",
		"unifai/m/1/snake":     "p3",
	}
...
func TestStoreOwnedVaultSecretVars_WalksMap(t *testing.T) {
	...
	if got := stored["unifai/m/1/headers/Authorization"]; got != "secret-token" {
		t.Errorf("stored Authorization = %q, want %q", got, "secret-token")
	}
	auth := m.Headers["Authorization"]
	if !auth.IsFromVault() || auth.GetRawRef() != "vault.unifai/m/1/headers/Authorization" {
	...
	if env := m.Headers["X-Env"]; env.IsFromSecret() && env.GetRawRef() == "env.X" {
		if stored["unifai/m/1/headers/X-Env"] != "" {
...
func TestRemoveOwnedVaultSecretVars_WalksMap(t *testing.T) {
	...
	m := &model{
		Headers: map[string]SecretVar{
			"Owned":    SecretVar{Val: "vault.unifai/m/1/headers/Owned", ref: "vault.unifai/m/1/headers/Owned", SecretType: SecretTypeVault},
	...
	if len(removed) != 1 || removed[0] != "unifai/m/1/headers/Owned" {
		t.Errorf("removed = %v, want only [unifai/m/1/headers/Owned]", removed)
	}
}
