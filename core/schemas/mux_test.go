...
func TestToUnifAIResponsesStreamResponse_PopulatesFinalDoneTextAndCompletedOutput(t *testing.T) {
	state := AcquireChatToResponsesStreamState()
	defer ReleaseChatToResponsesStreamState(state)

	makeChunk := func(role *string, content *string, finishReason *string) *UnifAIChatResponse {
		return &UnifAIChatResponse{
			ID:    "chatcmpl-test",
			Model: "test-model",
	...
	part1 := "Hello"
	part2 := " world"
	stop := string(UnifAIFinishReasonStop)

	var all []*UnifAIResponsesStreamResponse
	all = append(all, makeChunk(&role, nil, nil).ToUnifAIResponsesStreamResponse(state)...)
	all = append(all, makeChunk(nil, &part1, nil).ToUnifAIResponsesStreamResponse(state)...)
	all = append(all, makeChunk(nil, &part2, nil).ToUnifAIResponsesStreamResponse(state)...)
	all = append(all, makeChunk(nil, nil, &stop).ToUnifAIResponsesStreamResponse(state)...)

	var outputTextDone *UnifAIResponsesStreamResponse
	var completed *UnifAIResponsesStreamResponse
	for _, evt := range all {
		if evt == nil {
			continue
		}
...
func TestToUnifAIResponsesResponse_MapsLengthToIncomplete(t *testing.T) {
	length := string(UnifAIFinishReasonLength)
	resp := (&UnifAIChatResponse{
		Choices: []UnifAIResponseChoice{
			{FinishReason: &length},
		},
	}).ToUnifAIResponsesResponse()
...
func TestToUnifAIResponsesResponse_MapsToolCallsToCompleted(t *testing.T) {
	toolCalls := string(UnifAIFinishReasonToolCalls)
	resp := (&UnifAIChatResponse{
		Choices: []UnifAIResponseChoice{
...
func TestToUnifAIResponsesResponse_PrioritizesLengthAcrossChoices(t *testing.T) {
	stop := string(UnifAIFinishReasonStop)
	length := string(UnifAIFinishReasonLength)
	resp := (&UnifAIChatResponse{
		Choices: []UnifAIResponseChoice{
...
func TestToUnifAIResponsesResponse_UnknownFinishReasonLeavesStatusUnset(t *testing.T) {
...
func TestToUnifAIResponsesStreamResponse_IncludesFunctionCallsInCompletedOutput(t *testing.T) {
	state := AcquireChatToResponsesStreamState()
	defer ReleaseChatToResponsesStreamState(state)

	role := string(ChatMessageRoleAssistant)
	part1 := "Let me help"
	toolCallsFinish := string(UnifAIFinishReasonToolCalls)
	...
	// Role chunk
	all = append(all, (&UnifAIChatResponse{
		ID:    "chatcmpl-test",
		Model: "test-model",
		Choices: []UnifAIResponseChoice{
			{
	...
	}).ToUnifAIResponsesStreamResponse(state)...)
...
func TestToUnifAIResponsesStreamResponse_ToolCallsOnlyInCompletedOutput(t *testing.T) {
	state := AcquireChatToResponsesStreamState()
	defer ReleaseChatToResponsesStreamState(state)
...
func TestToUnifAIResponsesStreamResponse_MapsLengthToIncompleteEvent(t *testing.T) {
	state := AcquireChatToResponsesStreamState()
	defer ReleaseChatToResponsesStreamState(state)
	...
	var all []*UnifAIResponsesStreamResponse
	all = append(all, makeChunk(&role, nil, nil).ToUnifAIResponsesStreamResponse(state)...)
	all = append(all, makeChunk(nil, &part, nil).ToUnifAIResponsesStreamResponse(state)...)
	all = append(all, makeChunk(nil, nil, &length).ToUnifAIResponsesStreamResponse(state)...)

	var completed *UnifAIResponsesStreamResponse
	var incomplete *UnifAIResponsesStreamResponse
	for _, evt := range all {
		if evt == nil {
			continue
		}
