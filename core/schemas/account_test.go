...
func TestResolveFamilyPrecedence(t *testing.T) {
	...
	// Helper to build a UnifAIContext carrying a ResolvedAlias.
	withAlias := func(ra *ResolvedAlias) *UnifAIContext {
		bc := NewUnifAIContext(nil, NoDeadline)
		if ra != nil {
			bc.SetValue(UnifAIContextKeyResolvedAlias, ra)
		}
		return bc
	}
