...
type ListModelsByKeyResult struct {
	Response *UnifAIListModelsResponse
	Err      *UnifAIError
	KeyID    string
}
...
// circular reference: KeyStatus.Error → UnifAIError.ExtraFields.KeyStatuses → KeyStatus.
...
type UnifAIListModelsRequest struct {
	...
	// Unfiltered: If true, the response will include all models for the provider, regardless of the allowed models (internal unifai use only, not sent to the provider)
	...
	// invalidation. Internal unifai use only; not sent to the provider.
	//
...
type UnifAIListModelsResponse struct {
...
func (response *UnifAIListModelsResponse) FilterDeprecatedModels() {
...
// ApplyPagination applies offset-based pagination to a UnifAIListModelsResponse.
...
func (response *UnifAIListModelsResponse) ApplyPagination(pageSize int, pageToken string) *UnifAIListModelsResponse {
	if response == nil {
		return nil
	}
	...
	if offset >= totalItems {
		...
		return &UnifAIListModelsResponse{
			Data:          []Model{},
	...
	paginatedResponse := &UnifAIListModelsResponse{
		Data:        paginatedData,
		ExtraFields: response.ExtraFields,
		KeyStatuses: response.KeyStatuses,
	}
...
type Model struct {
	...
	// through the UnifAI pipeline for integration reverse-conversion. Never serialized.
