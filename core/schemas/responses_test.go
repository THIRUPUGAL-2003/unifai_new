...
func TestUnifAIResponsesStreamResponsePreservesOpenAIStreamMetadata(t *testing.T) {
	...
	var resp UnifAIResponsesStreamResponse
	...
	encodedChunk, err := MarshalSorted(UnifAIStreamChunk{UnifAIResponsesStreamResponse: defaulted})
	if err != nil {
...
func TestUnifAIResponsesResponseUnmarshalTimestamps(t *testing.T) {
...
// UnifAI superset path keeps the carry).
func TestWithDefaultsStripsCodeExecutionCarry(t *testing.T) {
	code := "print(1)"
	resp := &UnifAIResponsesResponse{
		ID: Ptr("resp_1"),
...
func TestStreamWithDefaultsStripsCodeExecutionCarry(t *testing.T) {
	...
	for _, typ := range []ResponsesStreamResponseType{
		...
		src := &UnifAIResponsesStreamResponse{Type: typ, Item: mkItem()}
