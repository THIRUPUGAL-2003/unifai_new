...
func TestOpencodeModelRef(t *testing.T) {
	...
	if got := opencodeModelRef("gpt-4.1"); got != "unifai/gpt-4.1" {
		t.Fatalf("opencodeModelRef() = %q, want %q", got, "unifai/gpt-4.1")
	}
}
...
`
	...
	if err := codexWriteNativeConfig("http://localhost:8080/openai/v1", "sk-uf-new", "gpt-5.5"); err != nil {
	...
	if got, _ := auth["OPENAI_API_KEY"].(string); got != "sk-uf-new" {
		t.Fatalf("auth.OPENAI_API_KEY = %q, want %q", got, "sk-uf-new")
	}
...
func TestCodexWriteNativeConfigCreatesFilesWhenMissing(t *testing.T) {
	...
	if err := codexWriteNativeConfig("http://localhost:8080/openai/v1", "sk-uf-fresh", ""); err != nil {
		t.Fatalf("codexWriteNativeConfig() error = %v", err)
	}
	...
	if got, _ := auth["OPENAI_API_KEY"].(string); got != "sk-uf-fresh" {
