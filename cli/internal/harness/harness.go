...
// Harness defines a coding assistant CLI that UnifAI can launch and manage.
type Harness struct {
	ID               string
	Label            string
	Binary           string
	...
	// WriteNativeConfig persists the unifai connection settings into the
	// harness CLI's own config file so the same configuration is available
	// when users launch the CLI directly outside unifai.
...
// opencodePreLaunch writes temporary OpenCode config files when UnifAI needs
...
func opencodePreLaunch(baseURL, apiKey, model string) ([]string, func(), error) {
  ...
  "provider": {
	...
	f, err := os.CreateTemp("", "unifai-opencode-*.json")
	if err != nil {
...
func opencodeModelRef(model string) string {
	return "unifai/" + strings.TrimSpace(model)
}
...
func opencodeTUIPreLaunch() ([]string, func(), error) {
	...
	f, err := os.CreateTemp("", "unifai-opencode-tui-*.json")
	if err != nil {
