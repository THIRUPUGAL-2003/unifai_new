...
// claudeWriteNativeConfig writes the unifai endpoint, API key, and model
...
// codexPreLaunch persists the unifai endpoint and virtual key into Codex
...
// auth.json from a prior run will otherwise shadow whatever UnifAI passes
...
// launches outside UnifAI keep working too.
...
// codexWriteNativeConfig writes the unifai endpoint, API key, and model
...
// codexWriteAuth merges the unifai virtual key into Codex's auth.json,
...
// codexWriteConfigTOML merges UnifAI's endpoint and (optionally) model into
