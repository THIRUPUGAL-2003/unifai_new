...
const issuesURL = "https://github.com/unifai/unifai/issues/new"
const repoURL = "https://github.com/unifai/unifai"
const docsURL = "https://docs.unifai.ai/quickstart/cli/getting-started"
...
func newChooserModel(cfg ChooserConfig) chooserModel {
	...
	vk := textInput.New()
	vk.Placeholder = "optional (x-uf-vk)"
	vk.Prompt = ""
	...
	case tea.KeyMsg:
		...
		case phaseVirtualKey:
			...
			if s == "f1" {
				...
				if baseURL != "" {
					openBrowser(baseURL)
					m.notify("opened unifai dashboard", false)
				}
				return m, nil
			}
			var cmd tea.Cmd
...
func (m chooserModel) View() string {
	...
	if updateVersion := strings.TrimSpace(m.cfg.UpdateVersion); updateVersion != "" {
		...
		body.WriteString(hint.Render("unifai " + updateVersion + "  "))
	...
	switch m.phase {
	case phaseBaseURL:
		title = accent.Render("Base URL (UnifAI Base URL)")
	...
	case phaseHarness:
		...
		if m.returnToSummary {
			...
			title = accent.Render("UnifAI CLI")
		} else {
	...
	case phaseSummary:
		...
		renderSummaryLine(summaryActionQuit, "Exit", "UnifAI CLI")
...
// from the UnifAI API and sends the result as a modelsMsg.
