...
// chooser when a newer UnifAI CLI version is available.
...
func RunConfirmUpdateIO(header, version string, in io.Reader, out io.Writer) (bool, error) {
	return runConfirm(confirmModel{
		header:     header,
		prompt:     fmt.Sprintf("UnifAI %s is available. Update now?", strings.TrimSpace(version)),
...
func (m confirmModel) homeStyleView(yes, no string) string {
	...
	block := centerBlock(accent.Render("UnifAI CLI")+"\n"+hint.Render("Home")+"\n\n"+popup, w)
