...
func TestRenderPlainChooserView(t *testing.T) {
	...
	for _, want := range []string{"UNIFAI CLI", "Ready", "base url", "model", "enter launch"} {
...
func TestChooserViewShowsUpdatePrompt(t *testing.T) {
	...
	for _, want := range []string{"Update available:", "unifai v1.2.3", "press y to update now"} {
...
func TestChooserPopupUsesUnifAICLIHeading(t *testing.T) {
	m := newChooserModel(ChooserConfig{
		BaseURL: "http://localhost:8080",
		Harness: "codex",
		Model:   "gpt-4o-mini",
	...
	view := m.View()
	if !strings.Contains(view, "UnifAI CLI") {
		t.Fatalf("expected selector popup to use UnifAI CLI heading, got %q", view)
	}
...
func TestChooserSummaryUsesActionLabels(t *testing.T) {
	...
	if !strings.Contains(view, "Exit") || !strings.Contains(view, "UnifAI CLI") {
		t.Fatalf("expected summary to show Exit UnifAI CLI, got %q", view)
	}
...
func TestUpdateConfirmUsesHomeStylePopup(t *testing.T) {
	m := confirmModel{
		prompt:    "UnifAI v1.2.3 is available. Update now?",
		idx:       1,
		homeStyle: true,
		width:     100,
		height:    30,
	...
	view := m.View()
	for _, want := range []string{"UnifAI CLI", "Home", "Update available", "UnifAI v1.2.3 is available"} {
		if !strings.Contains(view, want) {
