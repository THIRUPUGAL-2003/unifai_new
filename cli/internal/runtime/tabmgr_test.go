...
func TestBuildTabBarStringUsesCLIBrandAndRightSideVersion(t *testing.T) {
	...
	if !strings.Contains(got, "UnifAI CLI") {
		t.Fatalf("expected tab bar to contain UnifAI CLI branding, got %q", got)
	}
	if strings.Contains(got, " UnifAI ") {
		t.Fatalf("did not expect legacy UnifAI branding, got %q", got)
	}
...
func TestRemoveLastTabSignalsCloseForAppRelaunch(t *testing.T) {
	...
	if got := out.String(); strings.Contains(got, "UnifAI CLI") || strings.Contains(got, "Home") {
...
func TestHomeCtrlCRequiresSecondPressToQuit(t *testing.T) {
	...
	if got := out.String(); !strings.Contains(got, "Quit UnifAI?") {
