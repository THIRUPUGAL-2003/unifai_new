...
// prefix is the primary unifai prefix key: Ctrl+B (0x02). When pressed, PTY
// output is frozen and unifai enters tab mode.
const prefix = 0x02
...
// isPrefixSequence reports whether a CSI sequence encodes a unifai prefix key
...
// unifai prefix keys: Ctrl+B (b/B, 98/66) or the tmux-safe Ctrl+G (g/G,
...
// handleHomeCtrlC manages Ctrl+C while UnifAI owns the Home/command screen.
...
func (tm *TabManager) renderFrame() {
	...
	if vtMode&vt10x.ModeMouseMask == 0 {
		...
		// terminal — without that, mouse escapes never reach unifai at
...
func (tm *TabManager) buildTabBarString() string {
	...
	if noticeText != "" {
		...
		hint = " Quit UnifAI?  Ctrl+C: confirm  Esc: cancel "
	} else if scrollMode {
		sbLen := 0
...
func tabBarBrandString(bg string) string {
	...
	b.WriteString(bg)
	b.WriteString(" ")
	b.WriteString("\x1b[1;36mUnifAI CLI ")
	return b.String()
}
...
func tabBarBrandWidth() int {
	return 1 + len("UnifAI CLI ")
}

func (tm *TabManager) initCursorTrace() {
	path := strings.TrimSpace(os.Getenv("UNIFAI_CLAUDE_CURSOR_TRACE"))
	if path == "" {
		return
	}
	...
	if err != nil {
		if tm.stderr != nil {
			fmt.Fprintf(tm.stderr, "unifai: failed to open Claude cursor trace %q: %v\n", path, err)
		}
		return
	}
	tm.cursorTrace = f
...
// Inside tmux, Ctrl+B is normally tmux's own prefix, so UnifAI advertises
