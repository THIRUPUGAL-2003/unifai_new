...
func TestResolveManagedBinaryTargetFallsBackToCLIInstallLocation(t *testing.T) {
	...
	got, err := unifaiCLIManagedBinaryPath(binaryName)
	if err != nil {
		t.Fatalf("unifaiCLIManagedBinaryPath() error = %v", err)
	}

	want := filepath.Join(home, ".unifai", "bin", binaryName)
	if got != want {
		t.Fatalf("unifaiCLIManagedBinaryPath() = %q, want %q", got, want)
	}
}
...
func TestIsWrapperManagedBinaryPathMatchesCLIInstallLocation(t *testing.T) {
	...
	path := filepath.Join(home, ".unifai", "bin", binaryName)
...
func TestIsWrapperManagedBinaryPathMatchesCacheInstallLocation(t *testing.T) {
	if runtime.GOOS == "windows" {
		...
		path := filepath.Join(localAppData, "unifai", "v1.2.3", "bin", "unifai.exe-0")
		if !isWrapperManagedBinaryPath(path, "unifai.exe") {
	...
	if runtime.GOOS == "linux" {
		...
		path := filepath.Join(xdg, "unifai", "v1.2.3", "bin", "unifai-0")
		if !isWrapperManagedBinaryPath(path, "unifai") {
			t.Fatalf("expected %q to be recognized as wrapper-managed cache path", path)
		}
		return
	}

	path := filepath.Join(home, "Library", "Caches", "unifai", "v1.2.3", "bin", "unifai-0")
	if !isWrapperManagedBinaryPath(path, "unifai") {
		t.Fatalf("expected %q to be recognized as wrapper-managed cache path", path)
	}
}
...
func TestStageUpdateBinaryCopiesIntoTargetDirectory(t *testing.T) {
	...
	root := t.TempDir()
	targetDir := filepath.Join(root, ".unifai", "bin")
	if err := os.MkdirAll(targetDir, 0o755); err != nil {
		t.Fatalf("mkdir target dir: %v", err)
	}

	downloadPath := filepath.Join(t.TempDir(), "downloaded-unifai")
	if err := os.WriteFile(downloadPath, []byte("new-binary"), 0o600); err != nil {
		t.Fatalf("write download: %v", err)
	}

	targetPath := filepath.Join(targetDir, "unifai")
