...
func RunSelfUpdate(currentVersion string) error {
	fmt.Println("Checking latest unifai version...")
	...
	fmt.Printf("Updating unifai from %s to %s...\n", currentVersion, latest)

	binaryName := "unifai"
	if runtime.GOOS == "windows" {
		binaryName = "unifai.exe"
	}
	downloadURL := fmt.Sprintf("%s/unifai-cli/%s/%s/%s/%s", baseURL, latest, runtime.GOOS, runtime.GOARCH, binaryName)
	...
	tmpFile, err := os.CreateTemp("", ".unifai-update-*")
	if err != nil {
	...
	fmt.Printf("Updated unifai from %s to %s. Please restart.\n", currentVersion, latest)
	return nil
}
...
func resolveManagedBinaryTarget(binaryName string) (string, error) {
	...
	return unifaiCLIManagedBinaryPath(binaryName)
}

func unifaiCLIManagedBinaryPath(binaryName string) (string, error) {
	home, err := os.UserHomeDir()
	if err != nil {
		return "", err
	}
	return filepath.Join(home, ".unifai", "bin", binaryName), nil
}
...
func isWrapperManagedBinaryPath(path, binaryName string) bool {
	...
	homePath, err := unifaiCLIManagedBinaryPath(binaryName)
	...
	cachePrefix := filepath.Clean(filepath.Join(cacheRoot, "unifai")) + string(os.PathSeparator)
...
func stageUpdateBinary(downloadPath, targetPath string, mode os.FileMode) (string, error) {
	stageFile, err := os.CreateTemp(filepath.Dir(targetPath), ".unifai-stage-*")
	if err != nil {
		return "", err
	}
