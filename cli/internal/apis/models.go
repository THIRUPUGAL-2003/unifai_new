...
// Client wraps HTTP calls to the UnifAI gateway API used by the CLI setup
// flow.
type Client struct {
	http *http.Client
}

// NewClient creates a UnifAI API client with a default HTTP timeout.
...
// ListModels fetches available model IDs from the UnifAI /v1/models endpoint,
...
func (c *Client) ListModels(ctx context.Context, baseURL, virtualKey string) ([]string, error) {
	...
	if strings.TrimSpace(virtualKey) != "" {
		req.Header.Set("x-uf-vk", strings.TrimSpace(virtualKey))
	}
