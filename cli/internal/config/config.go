...
const (
	defaultDirName  = ".unifai"
	defaultFileName = "config.json"
	stateFileName   = "state.json"
)

// FileConfig represents the on-disk unifai configuration file (~/.unifai/config.json).
...
// Profile represents a named UnifAI connection profile with a base URL.
type Profile struct {
	ID      string `json:"id"`
...
// DefaultConfigPath returns the default path to the unifai config file (~/.unifai/config.json).
...
// DefaultStatePath returns the default path to the unifai state file (~/.unifai/state.json).
