...
import (
	...
	"github.com/unifai/unifai/cli/internal/apis"
	"github.com/unifai/unifai/cli/internal/harness"
)

// AttachBestEffort attempts to register the UnifAI MCP server with the harness.
...
func AttachBestEffort(ctx context.Context, stdout, stderr io.Writer, h harness.Harness, baseURL, vk string) {
	...
	if strings.TrimSpace(vk) == "" {
		...
		defer cancel()
		cmd := exec.CommandContext(tCtx, "claude", "mcp", "add", "--transport", "http", "unifai", mcpURL)
		if err := cmd.Run(); err != nil {
			fmt.Fprintf(stderr, "warning: auto MCP add failed. Run: claude mcp add --transport http unifai %s\n", mcpURL)
			return
		}
		fmt.Fprintln(stdout, "MCP: attached Claude MCP server 'unifai'.")
		return
	}
	...
	tCtx, cancel := context.WithTimeout(ctx, 10*time.Second)
	defer cancel()
	cmd := exec.CommandContext(tCtx, "claude", "mcp", "add-json", "unifai", payload)
	if err := cmd.Run(); err != nil {
		fmt.Fprintf(stderr, "warning: auto MCP add failed. Run: claude mcp add-json unifai '<payload>' (with your Authorization header)\n")
		return
	}
	fmt.Fprintln(stdout, "MCP: attached Claude MCP server 'unifai' with virtual key.")
}
