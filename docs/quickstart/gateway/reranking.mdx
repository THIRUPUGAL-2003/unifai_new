...
--data '{
  ...
  "documents": [
    {"text": "UnifAI is an AI gateway that unifies many LLM providers."},
    {"text": "Paris is the capital of France."},
    {"text": "UnifAI exposes an OpenAI-compatible API."}
  ]
}'
```
...
--data '{
  ...
  "documents": [
    {"id": "a", "text": "UnifAI supports observability plugins like OTEL and Maxim."},
    {"id": "b", "text": "UnifAI can run in Kubernetes and ECS."},
...
When using a `vllm/...` model, UnifAI sends rerank requests to `/v1/rerank` first and automatically retries `/rerank` when the upstream endpoint responds with `404`, `405`, or `501`.
  ...
  "results": [
    {
      ...
      "document": {
        "id": "a",
        "text": "UnifAI supports observability plugins like OTEL and Maxim."
      }
    },
    {
      "index": 2,
