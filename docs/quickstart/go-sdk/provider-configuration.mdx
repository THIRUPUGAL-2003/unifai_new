...
> If UnifAI receives a new provider at runtime (i.e., one that is not returned by `GetConfiguredProviders()` initially on `unifai.Init()`), it will set up the provider at runtime using `GetConfigForProvider()`, which may cause a delay in the first request to that provider.

## Making Requests

Once providers are configured, you can make requests to any specific provider. This example shows how to send a request directly to Mistral's latest vision model. UnifAI handles the provider-specific API formatting automatically.

```go
response, err := client.ChatCompletionRequest(schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline), &schemas.UnifAIChatRequest{
...
UnifAI supports two ways to add custom headers to provider requests: **static headers** configured at the provider level, and **dynamic headers** passed per-request via context.
...
func makeRequestWithCustomHeaders() {
    ...
    // Add custom headers using UnifAIContextKeyExtraHeaders
    extraHeaders := map[string][]string{
        "user-id":         {"user-123"},
...
```go
ctx := context.Background()
ctx = context.WithValue(ctx, schemas.UnifAIContextKeySendBackRawResponse, true) // or false to suppress
...
```go
ctx := context.Background()
ctx = context.WithValue(ctx, schemas.UnifAIContextKeySendBackRawRequest, true) // or false to suppress
...
```go
ctx := context.Background()
ctx = context.WithValue(ctx, schemas.UnifAIContextKeyStoreRawRequestResponse, true) // or false to disable
