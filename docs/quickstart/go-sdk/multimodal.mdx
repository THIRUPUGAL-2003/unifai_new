...
description: "Process multiple types of content including images, audio, and text with AI models. UnifAI supports vision analysis, image generation, speech synthesis, and audio transcription across various providers."
icon: "images"
---
...
```go
response, err := client.ImageGenerationRequest(schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline), &schemas.UnifAIImageGenerationRequest{
	Provider: schemas.OpenAI,
	Model:    "dall-e-3",
...
```go
response, err := client.SpeechRequest(schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline), &schemas.UnifAISpeechRequest{
	Provider: schemas.OpenAI,
...
response, err := client.TranscriptionRequest(schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline), &schemas.UnifAITranscriptionRequest{
	Provider: schemas.OpenAI,
