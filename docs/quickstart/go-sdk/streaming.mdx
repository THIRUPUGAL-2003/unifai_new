...
```go
stream, err := client.TextCompletionStreamRequest(schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline), &schemas.UnifAITextCompletionRequest{
	Provider: schemas.OpenAI,
	Model:    "gpt-4o-mini",
	Input: &schemas.TextCompletionInput{
		PromptStr: unifai.Ptr("A for apple and B for"),
	},
})
...
for chunk := range stream {
	...
	if chunk.UnifAIError != nil {
		log.Printf("Stream error: %v", chunk.UnifAIError)
		break
	}

	// Process response chunks
	if chunk.UnifAITextCompletionResponse != nil && len(chunk.UnifAITextCompletionResponse.Choices) > 0 {
		choice := chunk.UnifAITextCompletionResponse.Choices[0]
		
		// Check for streaming content
		if choice.TextCompletionResponseChoice != nil &&
			choice.TextCompletionResponseChoice.Text != nil {
			content := *choice.UnifAITextCompletionResponseChoice.Text
...
```go
stream, err := client.ChatCompletionStreamRequest(schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline), &schemas.UnifAIChatRequest{
	Provider: schemas.OpenAI,
	Model:    "gpt-4o-mini",
	Input:    messages,
})
...
for chunk := range stream {
	...
	// Process response chunks
	if chunk.UnifAIChatResponse != nil && len(chunk.UnifAIChatResponse.Choices) > 0 {
...
<Note>
UnifAI standardizes all stream responses to send usage and finish reason only in the last chunk, and content in the previous chunks.
</Note>
...
messages := []schemas.ResponsesMessage{
	{
		...
		Content: &schemas.ResponsesMessageContent{
			ContentStr: unifai.Ptr("Hello, UnifAI!"),
		},
	},
}

stream, err := client.ResponsesStreamRequest(schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline), &schemas.UnifAIResponsesRequest{
	Provider: schemas.OpenAI,
	Model:    "gpt-4o-mini",
	Input:    messages,
})
...
for chunk := range stream {
	...
	if chunk.UnifAIResponsesStreamResponse != nil {
		delta := chunk.UnifAIResponsesStreamResponse.Delta
...
```go
stream, err := client.SpeechStreamRequest(schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline), &schemas.UnifAISpeechRequest{
	Provider: schemas.OpenAI,
	Model:    "tts-1", // Using text-to-speech model
	Input: &schemas.SpeechInput{
		Input: "Hello! This is a sample text that will be converted to speech using UnifAI's speech synthesis capabilities. The weather today is wonderful, and I hope you're having a great day!",
	},
...
for chunk := range stream {
	if chunk.UnifAIError != nil {
		panic(fmt.Sprintf("Stream error: %s", chunk.UnifAIError.Error.Message))
	}

	if chunk.UnifAISpeechStreamResponse != nil {
		// Accumulate audio data from each chunk
		audioData = append(audioData, chunk.UnifAISpeechStreamResponse.Audio...)
		totalChunks++
		fmt.Printf("Received chunk %d, size: %d bytes\n", totalChunks, len(chunk.UnifAISpeechStreamResponse.Audio))
	}
}
...
stream, err := client.TranscriptionStreamRequest(schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline), &schemas.UnifAITranscriptionRequest{
	Provider: schemas.OpenAI,
	...
	Params: &schemas.TranscriptionParameters{
		Prompt: schemas.Ptr("This is a sample audio transcription from UnifAI speech synthesis."), // Optional: provide context
	},
})
...
for chunk := range stream {
	...
	if chunk.UnifAITranscriptionStreamResponse != nil && chunk.UnifAITranscriptionStreamResponse.Delta != nil {
		// Print each chunk of text as it arrives
		fmt.Print(*chunk.UnifAITranscriptionStreamResponse.Delta)
	}
}
```
...
for chunk := range stream {
	if chunk.UnifAISpeechStreamResponse != nil {
		audioBuffer.Write(chunk.UnifAISpeechStreamResponse.Audio)
...
stream, err := client.ChatCompletionStreamRequest(schemas.NewUnifAIContext(ctx, schemas.NoDeadline), &schemas.UnifAIChatRequest{
	// ... your request
})
