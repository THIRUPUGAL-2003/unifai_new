---
title: "UnifAI CLI"
description: "Launch any coding agent through UnifAI with a single command. Automatic configuration, model selection, and MCP integration - no environment variables needed."
icon: "laptop-code"
---
...
UnifAI CLI is an interactive terminal tool that connects your favorite coding agents - Claude Code, Codex CLI, Gemini CLI, and Opencode - to your UnifAI gateway with zero manual configuration.

Instead of setting environment variables, editing config files, and looking up provider paths, you just run `unifai` and pick your agent, model, and go.

**What it does for you:**

- Automatically configures base URLs, API keys, and model settings for each agent
- Fetches available models from your UnifAI gateway's `/v1/models` endpoint
- Installs missing agents via npm if needed
- Auto-attaches UnifAI's MCP server to Claude Code for tool access
...
```bash
npx -y @unifai/unifai-cli
```

This downloads and runs the latest UnifAI CLI. No global install required - npx handles everything.

<Note>
UnifAI CLI requires **Node.js 18+** (for npx) and a running UnifAI gateway. See [Gateway Setup](/quickstart/gateway/setting-up) if you haven't started one yet.
</Note>

### Install a Specific Version

```bash
npx -y @unifai/unifai-cli --cli-version v1.0.0
```

## Quick Start

### 1. Start your UnifAI gateway

Make sure your UnifAI gateway is running. The default is `http://localhost:8080`:
...
Enter your UnifAI gateway URL. If you're running locally, this is typically `http://localhost:8080`.

{/* TODO: Add screenshot of the base URL input screen */}

**Step 2 - Virtual Key (optional)**

If your UnifAI gateway has [virtual key authentication](/features/governance/virtual-keys) enabled, enter your virtual key here. Otherwise, press Enter to skip.
...
The CLI fetches available models from your UnifAI gateway and presents a searchable list. Type to filter, arrow keys to navigate:
...
After launch, UnifAI CLI keeps you inside a tabbed terminal UI instead of exiting after the first session. The bottom tab bar shows:

- The `UnifAI CLI` label and current CLI version
...
<Tip>
If you press `Ctrl+B` before launching your first session, UnifAI CLI stays open on the Home tab bar so you can create a new tab from there.
</Tip>
...
The CLI stores its configuration at `~/.unifai/config.json`. This file is created automatically on first run and updated when you change settings through the TUI.
...
| Field | Description |
|-------|-------------|
| `base_url` | Your UnifAI gateway URL |
...
```bash
npx -y @unifai/unifai-cli -config ./my-project/unifai.json
```

This is useful when working across multiple UnifAI gateways or projects with different configurations.
...
| `u` | Change the UnifAI gateway URL |
| `v` | Update your virtual key |
| `w` | Set worktree name (Claude Code only) |
| `d` | Open the UnifAI dashboard in your browser |
...
UnifAI CLI applies two OpenCode-specific behaviors automatically:

- **Custom model selection**: when you pick a model in UnifAI CLI, OpenCode is launched with the correct provider-qualified model reference and a generated OpenCode runtime config.
- **Theme handling**: if your OpenCode `tui.json` already defines a theme, UnifAI preserves it. If not, UnifAI supplies the adaptive `system` theme so OpenCode does not fall back to its default dark-only appearance.
...
2. **Work** - Use your agent as normal, all traffic routes through UnifAI
...
Your last selections are remembered across sessions. The next time you run `unifai`, you'll see the summary screen with your previous configuration ready to go. Press Enter to re-launch immediately, or adjust any setting.
...
```bash
npx -y @unifai/unifai-cli -worktree feature-branch
```
...
When launching **Claude Code**, the CLI automatically registers UnifAI's MCP server endpoint (`/mcp`) so all your configured MCP tools are available inside the agent.
...
1. Your UnifAI gateway is running and accessible at the configured base URL
2. You have at least one provider configured in UnifAI
...
<CardGroup cols={2}>
  <Card title="Gateway Setup" icon="server" href="/quickstart/gateway/setting-up">
    Set up a UnifAI gateway if you haven't already
  </Card>
  <Card title="Provider Configuration" icon="gear" href="/quickstart/gateway/provider-configuration">
    Configure AI providers in your UnifAI gateway
  </Card>
