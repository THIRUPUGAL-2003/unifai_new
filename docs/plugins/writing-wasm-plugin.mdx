...
class StreamChunkInput {
  ...
  chunk: JSON.Raw = new JSON.Raw('null') // UnifAIStreamChunk as JSON (see below)
}
...
// UnifAIStreamChunk is one of: UnifAIChatResponse, UnifAITextCompletionResponse,
// UnifAIResponsesStreamResponse, UnifAISpeechStreamResponse, UnifAITranscriptionStreamResponse,
// UnifAIImageGenerationStreamResponse, or UnifAIError.
...
type HTTPStreamChunkHookInput struct {
	Context map[string]interface{} `json:"context"`
	Request json.RawMessage        `json:"request"`
	Chunk   json.RawMessage        `json:"chunk"` // UnifAIStreamChunk JSON
}

// HTTPStreamChunkHookOutput represents the output from http_stream_chunk_hook
type HTTPStreamChunkHookOutput struct {
	Context  map[string]interface{} `json:"context"`
	Chunk    json.RawMessage        `json:"chunk,omitempty"` // UnifAIStreamChunk JSON, nil to skip
...
The `chunk` field contains a `UnifAIStreamChunk` struct serialized as JSON. It will contain the data from whichever response type is active:
...
<Note>
**Go Native vs WASM Plugins**: In Go native plugins (`.so`), you work directly with `*schemas.UnifAIStreamChunk` typed structs. In WASM plugins, this struct is serialized to JSON for crossing the WASM boundary. The underlying data structure is the same.
</Note>
...
Configure your WASM plugin in UnifAI's `config.json`:
...
Complete hello-world examples are available in the UnifAI repository:

- **TypeScript**: [examples/plugins/hello-world-wasm-typescript](https://github.com/unifai/unifai/tree/main/examples/plugins/hello-world-wasm-typescript)
- **Go (TinyGo)**: [examples/plugins/hello-world-wasm-go](https://github.com/unifai/unifai/tree/main/examples/plugins/hello-world-wasm-go)
- **Rust**: [examples/plugins/hello-world-wasm-rust](https://github.com/unifai/unifai/tree/main/examples/plugins/hello-world-wasm-rust)
