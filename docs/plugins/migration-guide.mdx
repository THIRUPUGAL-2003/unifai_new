...
description: "How to migrate your UnifAI plugins from v1.3.x to v1.4.x"
icon: "arrow-up-right-dots"
---

## Overview

UnifAI v1.4.x introduces a new plugin interface for HTTP transport layer interception. This guide helps you migrate existing plugins from the v1.3.x `TransportInterceptor` pattern to the v1.4.x `HTTPTransportPreHook` and `HTTPTransportPostHook` pattern.
...
| WASM support | No | Yes |
| Context | Limited `UnifAIContext` | Full `*UnifAIContext` with `SetValue`/`Value` |
...
5. **Full context access** - UnifAIContext available for sharing data between hooks
...
func HTTPTransportPreHook(ctx *schemas.UnifAIContext, req *schemas.HTTPRequest) (*schemas.HTTPResponse, error) {
	...
	ctx.SetValue(schemas.UnifAIContextKey("my-plugin-key"), "my-value")
...
2. **Update UnifAI to v1.4.x:**
   ```bash
   go get github.com/unifai/unifai/core@v1.4.0
   ```
...
This error occurs if UnifAI v1.4.x is looking for the old function. Make sure:
...
ctx.SetValue(schemas.UnifAIContextKey("my-key"), "my-value")

// In post-hook - read value
if val := ctx.Value(schemas.UnifAIContextKey("my-key")); val != nil {
	// Use val
}
```
...
UnifAI v1.4.x introduces a new hook for intercepting streaming response chunks:
...
| Input | Full HTTPResponse | `*UnifAIStreamChunk` (typed struct) |
