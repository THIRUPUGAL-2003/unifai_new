...
description: "Control the execution order of custom plugins relative to UnifAI's built-in plugins using placement groups and ordering."
icon: "arrow-down-1-9"
---
...
Plugin sequencing lets you control **where** your custom plugins execute relative to UnifAI's built-in plugins (telemetry, logging, governance, etc.) and **in what order** they execute relative to each other.

---

## How it works

UnifAI organizes plugins into three **placement groups** that execute in a fixed order:
...
<Note>
If your `config.json` file has plugin sequence configured, it will take precedence over the sequence configured in the UI after restarting UnifAI.
</Note>
