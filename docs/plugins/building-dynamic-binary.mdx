---
title: "Building Dynamically Linked UnifAI Binary"
description: "Learn how to build a dynamically linked UnifAI binary required for custom plugin support"
icon: "hammer"
---

## Why Dynamic Linking?

Go's plugin system requires **dynamic linking** to load `.so` files at runtime. By default, UnifAI builds are **statically linked** for maximum portability across Linux distributions - they bundle all dependencies including the C standard library (libc). However, statically linked binaries **cannot load Go plugins**.

To use custom plugins with UnifAI, you must build a dynamically linked binary that links against the system's libc at runtime.
...
UnifAI's default build configuration creates statically linked binaries:
...
This creates `tmp/unifai-http` as a dynamically linked binary.
...
COPY --from=ui-builder /app/out ./unifai-http/ui
...
```bash
# Build the image
docker build -f transports/Dockerfile -t unifai:dynamic-alpine .

# Run the container
docker run -p 8080:8080 -v ./plugins:/app/data/plugins unifai:dynamic-alpine
```
...
```bash
# Build the image
docker build -f transports/Dockerfile.debian -t unifai:dynamic-debian .

# Run the container
docker run -p 8080:8080 -v ./plugins:/app/data/plugins unifai:dynamic-debian
```
...
- **Plugins and UnifAI MUST use the same libc.**
</Warning>
...
Plugins **must** be built with the **exact same environment** as your UnifAI binary:

```bash
# If UnifAI was built with Alpine/musl
docker run --rm \
  -v "$PWD:/work" \
  -w /work \
...
# If UnifAI was built with Debian/glibc  
docker run --rm \
  -v "$PWD:/work" \
  -w /work \
...
See the [hello-world plugin Makefile](https://github.com/unifai/unifai/blob/main/examples/plugins/hello-world/Makefile) for a complete example.
...
```bash
# Start UnifAI with your plugin configured
./tmp/unifai-http -config config.json
...
UnifAI is built with **Go 1.26.1**. Your plugin **must** be compiled with the exact same Go version to ensure compatibility.
...
UnifAI uses the following key packages across its three main modules that may affect plugin development:
...
### Checking UnifAI's Dependencies

To see all dependencies used by UnifAI across its three main modules:
...
When creating a plugin, your `go.mod` should match UnifAI's Go version:
...
require (
    github.com/unifai/unifai/core v1.2.38
    // Optional: Add framework for advanced features
    // github.com/unifai/unifai/framework v1.1.48
    
    // Add other dependencies as needed, matching versions from UnifAI's go.mod files
...
<Tip>
Import only the UnifAI modules you need. Most plugins only require `core`. Use `framework` if you need access to config stores, vector stores, or other framework features.
</Tip>
...
**Cause:** Plugin and UnifAI were built with different Go versions.

**Solution:** Use the exact same Go version (Go 1.26.1) for both:

```bash
# Check Go version used for UnifAI
./tmp/unifai-http -version
...
**Cause:** Plugin and UnifAI built for different architectures (amd64 vs arm64).
...
Match UnifAI's exact Go version and key dependencies (see [Go Version and Package Compatibility](#go-version-and-package-compatibility)):
...
```bash
# Build both UnifAI and plugin
make build DYNAMIC=1
cd examples/plugins/hello-world && make build

# Test loading
./tmp/unifai-http -config examples/plugins/hello-world/config.json
```
...
Build plugins in the same Dockerfile as UnifAI:
...
# Build UnifAI
FROM golang:1.26.1-alpine3.23 AS unifai-builder
# ... (unifai build steps)

# Runtime
FROM alpine:3.23
COPY --from=unifai-builder /app/main .
COPY --from=plugin-builder /plugin/myplugin.so /app/plugins/
```

This ensures plugins and UnifAI use identical build environments.

## Next Steps

Now that you have a dynamically linked UnifAI binary:
...
3. **[Example plugins](https://github.com/unifai/unifai/tree/main/examples/plugins)** - Study working examples

<Note>
For questions or issues with dynamic builds and plugins, visit our [GitHub Discussions](https://github.com/unifai/unifai/discussions) or [Discord community](https://discord.gg/exN5KAydbU).
</Note>
