...
`auth_type: "oauth"` covers **server-level OAuth**: the admin authenticates once during MCP client setup, UnifAI stores the resulting token, and every subsequent request to that MCP server uses the same token regardless of which caller hit UnifAI.
...
UnifAI implements the **Authorization Code** flow:
...
sequenceDiagram
    ...
    User->>UnifAI: Create MCP client (auth_type=oauth)
    UnifAI-->>User: authorize_url
    User->>AuthServer: Sign in and authorize
    AuthServer-->>UnifAI: /api/oauth/callback?code=…&state=…
    UnifAI->>AuthServer: Exchange code for token
    AuthServer-->>UnifAI: access_token + refresh_token
    UnifAI->>UnifAI: Encrypt and store
    UnifAI->>MCPServer: Connect with Authorization: Bearer …
    UnifAI-->>User: MCP client connected
```
...
5. Click **Create** — UnifAI runs the OAuth dance in a popup
...
Redirect the admin to `authorize_url`. After they authorize, the upstream redirects to `/api/oauth/callback`, UnifAI exchanges the code for tokens, and you finalize the client:
...
For applications without a client secret, omit `client_secret` and UnifAI will automatically generate PKCE code verifiers:
...
<Warning>
The `redirect_uri` UnifAI registers with the upstream provider is locked to UnifAI's current public URL (`mcp_external_client_url`, or the request `Host` header if unset). If you change UnifAI's public URL later, the upstream provider will reject the next authorize call with **"Invalid redirect URI"**. To recover, clear the stored client credentials for the affected MCP server so UnifAI re-runs DCR with the new URL.
</Warning>
...
If only `client_id` and `server_url` are provided, UnifAI will probe in order:
...
UnifAI refreshes access tokens automatically using the stored refresh token, in two layers:
...
- **On use** — if a token is already expired when a request needs it, UnifAI refreshes it inline before forwarding the request.
...
This deletes the stored token from UnifAI and marks the OAuth configuration `revoked` (the config row is kept, not deleted). UnifAI does **not** call the upstream provider's revocation endpoint — revoke at the provider's dashboard if you need the upstream token invalidated there.
...
2. **Homepage URL**: `https://your-unifai-domain.com`
3. **Authorization callback URL**: `https://your-unifai-domain.com/api/oauth/callback`
4. Copy **Client ID** and generate a **Client Secret**
5. Paste into the UnifAI config above
...
4. Add `https://your-unifai-domain.com/api/oauth/callback` to **Authorized redirect URIs**
5. Copy Client ID + Client Secret into the UnifAI config above
...
The `redirect_uri` UnifAI registers and the consent URLs it builds are derived from the request `Host` header by default. Behind a reverse proxy, override them with:

- `mcp_external_client_url` — public base URL UnifAI uses both for the consent pages it surfaces and as the `redirect_uri` registered with upstream providers

See [Reverse Proxy configuration →](../../deployment-guides/config-json/client#reverse-proxy) for the full reference.

<Warning>
**Changing `mcp_external_client_url` after an upstream provider has been registered breaks already-authorized clients.** Upstream providers lock the `redirect_uri` to whatever was registered during DCR. To recover, clear the stored OAuth client credentials so UnifAI re-registers with the new URL.
</Warning>
...
- Provide `authorize_url` or a `server_url` UnifAI can discover from
</Accordion>
...
- Confirm UnifAI is reachable at the registered redirect URI (DNS, firewall, reverse-proxy headers)
- Check `mcp_external_client_url` matches what was registered upstream
- Look at UnifAI logs for `oauth` errors
</Accordion>

<Accordion title="`Invalid redirect URI` from the upstream provider">

You changed UnifAI's public URL after the upstream client was registered. Clear the stored credentials so UnifAI re-runs DCR with the new URL.
</Accordion>
</AccordionGroup>
...
- Tokens are stored encrypted at rest (set `UNIFAI_ENCRYPTION_KEY`)
