...
<Info>Per-user headers is available in **UnifAI v1.5.4 and above**.</Info>

`auth_type: "per_user_headers"` lets each end-user supply their own HTTP headers for an upstream MCP server. The admin declares the **header names** that callers must fill in; each end-user submits their **values** the first time they hit the server. UnifAI stores one credential per `(identity, mcp_client)` and reuses it on every later call.
...
3. UnifAI looks up an existing credential for `(identity, mcp_client)`:
   - **Found and `active`** → UnifAI attaches the stored header values and calls upstream.
   - **Missing, `orphaned`, or `needs_update`** → UnifAI returns an `mcp_auth_required` payload with an inline `submit_url`. The tool is **not** executed.
4. The user opens the URL, sees a UnifAI form listing the required header names, fills in values, and submits.
5. UnifAI runs a one-time upstream verify with the submitted values, then stores the credential.
...
```text
Authentication required for acme_api. Open this URL to submit the required headers: https://your-unifai-domain.com/workspace/mcp-sessions/auth?flow=<flow-id>&kind=headers
```
...
The admin configures the MCP client once, declaring the schema (header names) end-users will need to fill in. During setup, UnifAI asks the admin for sample values, runs a one-time upstream verify, and discovers the tool list — same as the per-user OAuth setup pattern, just with a values form instead of an OAuth popup. Sample values are discarded after the test.
...
On success, UnifAI runs the upstream verify with `user_headers`, attaches the discovered tools, persists the MCP client, and returns:
...
File-based config is **not supported** for `per_user_headers`. UnifAI needs to run the one-time upstream verify with the admin's sample values during create, which can only happen via an interactive admin step (Web UI or API).

If you want a declarative-ish setup, run the API create once with your sample values, then check the resulting `mcp_client_id` into your config to reference it from other parts of the system. The MCP client itself stays in UnifAI's database.
...
2. `Authorization` is treated as a credential by UnifAI and is **never** exposed to connect-plugins or accidentally leaked.
...
3. UnifAI runs a one-time upstream verify against the live MCP client config
...
Same model as [Per-User OAuth](./per-user-oauth#identity-modes) — `user` > `vk` > `session`. A per-user-headers request with no identity is rejected with an inline-401 explaining the caller must send a VK, sign in, or set `x-uf-mcp-session-id`.
...
| State          | Badge        | Meaning                                                                                                       |
| -------------- | ------------ | ------------------------------------------------------------------------------------------------------------- |
| `active`       | Active       | Credential present and usable. UnifAI attaches the stored values on every call.                              |
...
The VK isn't resolving. Confirm the VK exists and the caller is sending it under one of `x-uf-vk`, `Authorization: Bearer …`, `x-api-key`, or `x-goog-api-key`. If you're behind a proxy that strips `Authorization`, switch the caller to `x-uf-vk`.
</Accordion>
...
Make sure UnifAI can build a public URL — if behind a proxy, set `mcp_external_client_url` so the redirect lands on the right host.
</Accordion>
</AccordionGroup>
