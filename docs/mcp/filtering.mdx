...
UnifAI provides **three levels of tool filtering** to control which MCP tools are available:
...
curl -X POST http://localhost:8080/v1/chat/completions \
  -H "x-uf-mcp-include-clients: filesystem,web_search" \
  -d '...'

# Include only specific tools
curl -X POST http://localhost:8080/v1/chat/completions \
  -H "x-uf-mcp-include-tools: filesystem-read_file,web_search-search" \
  -d '...'

# Include all tools from one client, specific tools from another
curl -X POST http://localhost:8080/v1/chat/completions \
  -H "x-uf-mcp-include-tools: filesystem-*,web_search-search" \
  -d '...'

# Include internal tools registered via RegisterTool()
curl -X POST http://localhost:8080/v1/chat/completions \
  -H "x-uf-mcp-include-tools: unifaiInternal-echo,unifaiInternal-calculator" \
  -d '...'

# Empty clients filter blocks ALL tools - no tools available to LLM
curl -X POST http://localhost:8080/v1/chat/completions \
  -H "x-uf-mcp-include-clients:" \
  -d '...'
# Result: No MCP tools available (deny-all)

# Empty tools filter also blocks ALL tools
curl -X POST http://localhost:8080/v1/chat/completions \
  -H "x-uf-mcp-include-tools:" \
  -d '...'
# Result: No MCP tools available (deny-all)
```
...
ctx := context.WithValue(context.Background(),
    schemas.UnifAIContextKey("mcp-include-clients"),
    []string{"filesystem", "web_search"})

// Include only specific tools
ctx = context.WithValue(ctx,
    schemas.UnifAIContextKey("mcp-include-tools"),
...
- **Internal (In-Process) Tools**: Tools registered via `RegisterTool()` use the prefix `unifaiInternal-`
  - Example: `unifaiInternal-echo`, `unifaiInternal-my_custom_tool`
...
<Note>
When a Virtual Key has no MCP configurations, **no MCP tools are available** (deny-by-default). You must explicitly add MCP client configurations to allow tools. When a Virtual Key has MCP configurations, it generates the `x-uf-mcp-include-tools` header automatically, overriding any manually sent header.
</Note>
...
curl -X POST http://localhost:8080/v1/chat/completions \
  ...
  -H "x-uf-mcp-include-tools: filesystem-write_file" \  # This is IGNORED
  -d '...'
```
...
```bash
curl -X POST http://localhost:8080/v1/chat/completions \
  -H "x-uf-mcp-include-tools: filesystem-write_file" \
  -d '...'
```
...
response, _ := client.ChatCompletionRequest(schemas.NewUnifAIContext(ctx, schemas.NoDeadline), request)
```
