...
When an LLM returns tool calls in its response, UnifAI does **not** automatically execute them. Instead, your application explicitly calls the tool execution API, giving you full control over:
...
func main() {
    // Initialize UnifAI with MCP (see Connecting to Servers)
    client, _ := unifai.Init(context.Background(), config)

    // Step 1: Send initial request
    firstMessage := schemas.ChatMessage{
        Role: schemas.ChatMessageRoleUser,
        Content: schemas.ChatMessageContent{
            ContentStr: unifai.Ptr("List files in the current directory"),
        },
    }
    ...
    finalRequest := &schemas.UnifAIChatRequest{
        Provider: schemas.OpenAI,
        Model:    "gpt-4o",
        Input:    history,
    }

    finalResponse, err := client.ChatCompletionRequest(schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline), finalRequest)
    if err != nil {
        panic(err)
    }
...
UnifAI supports two API formats for tool execution:
