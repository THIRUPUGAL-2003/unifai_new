...
UnifAI Code Mode was benchmarked against classic MCP across three controlled rounds with increasing MCP footprint. Each round used the same query set with Code Mode off and on.
...
At around 500 tools, Code Mode reduced average input tokens per query by roughly **14x**: from **1.15M** tokens to **83K** tokens. See the [UnifAI MCP Gateway benchmark writeup](https://www.getmaxim.ai/unifai/blog/unifai-mcp-gateway-access-control-cost-governance-and-92-lower-token-costs-at-scale), or explore the [complete benchmark report](https://github.com/unifai/unifai-benchmarking/blob/main/mcp-code-mode-benchmark/benchmark_report.md).
...
With Code Mode, the model discovers the relevant stubs, writes a short orchestration script, and UnifAI runs the tool calls inside the Starlark sandbox. The intermediate tool results stay inside the sandbox, and the model receives the compact final output.

<Frame>
  <img src="/media/mcp-codemode-flow.png" alt="Code Mode flow where the model reads tool stubs, writes code, and UnifAI executes the multi-tool workflow in a sandbox" />
</Frame>
...
When Code Mode clients are connected, UnifAI automatically adds four meta-tools to every request:
...
1. UnifAI parses the Python code
...
UnifAI provides detailed error messages with hints:
