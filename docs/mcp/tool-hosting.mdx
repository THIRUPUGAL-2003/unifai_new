...
<Info>
This feature is only available when using UnifAI as a **Go SDK**. It is not available in the Gateway deployment.
</Info>
...
UnifAI automatically creates an internal MCP server (`unifaiInternal`) when you register your first tool.
...
Register your tool with UnifAI:
...
func main() {
    // Initialize UnifAI with MCP enabled (even empty config is fine)
        ...
        Input: []schemas.ChatMessage{
            {
                ...
                Content: schemas.ChatMessageContent{
                    ContentStr: unifai.Ptr("What is 15 * 7? Also, what time is it?"),
                },
            },
        },
    })
...
func registerCalculator(client *unifai.UnifAI) {
...
func registerTimeTool(client *unifai.UnifAI) {
    schema := schemas.ChatTool{
        Type: schemas.ChatToolTypeFunction,
        Function: &schemas.ChatToolFunction{
...
Tool names from `RegisterMCPTool` are prefixed with `unifaiInternal_` when exposed to LLMs:

| Registered Name | LLM Sees |
|-----------------|----------|
| `calculator` | `unifaiInternal_calculator` |
| `get_weather` | `unifaiInternal_get_weather` |
