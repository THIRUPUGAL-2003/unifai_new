...
description: "Connect UnifAI to external MCP servers via STDIO, HTTP, or SSE protocols."
icon: "plug"
---

## Overview

UnifAI can connect to any MCP-compatible server to discover and execute tools. Each connection is called an **MCP Client** in UnifAI terminology.

## Connection Types

UnifAI supports three connection protocols:
...
Configure MCP in your UnifAI initialization:
    ...
    mcpConfig := &schemas.MCPConfig{
        ClientConfigs: []*schemas.MCPClientConfig{
            ...
            },
            {
                ...
                ConnectionString: unifai.Ptr("http://localhost:3001/mcp"),
...
| `disable_auto_tool_inject` | boolean          | `false` | When `true`, MCP tools are not auto-injected into requests; callers must opt in via `x-uf-mcp-include-tools` (for LLM calls) |
...
Set `MCPConfig.ToolManagerConfig` before passing it to `unifai.Init`:
...
```go
{
    Name:             "secure_api",
    ConnectionType:   schemas.MCPConnectionTypeHTTP,
    ConnectionString: unifai.Ptr(os.Getenv("SECURE_MCP_URL")),
}
```
...
By default, UnifAI does not forward incoming request headers to MCP servers during tool execution. The `allowed_extra_headers` field lets you define a per-client allowlist of headers that callers may inject at request time and have forwarded to that MCP server when tools are executed.
...
2. UnifAI captures those headers from the request (using the union of all clients' allowlists)
...
**Send headers:** When an external MCP client (e.g., Claude Desktop, Cursor) connects to UnifAI's `/mcp` endpoint, include the allowed headers in that HTTP request. UnifAI forwards them during any tool call made within that session:
...
schemas.MCPClientConfig{
    Name:             "my_api",
    ConnectionType:   schemas.MCPConnectionTypeHTTP,
    ConnectionString: unifai.Ptr("https://mcp.example.com/mcp"),
...
**Send headers:** Set `UnifAIContextKeyMCPExtraHeaders` on the context before calling `ChatCompletionRequest` or `ExecuteChatMCPTool`:

```go
unifaiCtx := schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline)
unifaiCtx.SetValue(schemas.UnifAIContextKeyMCPExtraHeaders, map[string][]string{
    "x-user-token": {"eyJhbGci..."},
    "x-tenant-id":  {"acme-corp"},
})

response, err := client.ChatCompletionRequest(unifaiCtx, request)
```
...
err = client.AddMCPClient(schemas.MCPClientConfig{
    Name:           "new_client",
    ConnectionType: schemas.MCPConnectionTypeHTTP,
    ConnectionString: unifai.Ptr("http://localhost:3002/mcp"),
    ToolsToExecute: []string{"*"},
})
...
UnifAI automatically monitors MCP client health with periodic checks every 10 seconds by default.

### Health Check Methods

By default, UnifAI uses the lightweight **ping method** for health checks. However, you can configure the health check method based on your MCP server's capabilities:
...
UnifAI automatically implements **exponential backoff retry logic** to handle transient network failures and temporary service unavailability. This ensures that brief connection issues don't immediately cause tool unavailability.

<Warning>
**Important:** UnifAI only retries on transient errors (network failures, timeouts, temporary service unavailability). Permanent errors like authentication failures, configuration errors, and missing commands fail immediately without retry.
</Warning>

### Automatic Retry Strategy

UnifAI retries failed operations using the following strategy, as implemented by `ExecuteWithRetry` and `DefaultRetryConfig` in the MCP layer:
...
UnifAI intelligently classifies errors as either **transient** (retryable) or **permanent** (fail immediately):
...
UnifAI applies retry logic to these critical operations:
...
2. UnifAI automatically attempts reconnection **in the background**
...
You can temporarily disable an MCP client without removing it. When disabled, UnifAI shuts down the client's connection, health monitor, and tool syncer. The client entry is preserved and its tools are invisible to inference requests until it is re-enabled.
