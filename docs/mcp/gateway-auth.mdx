...
description: "How MCP clients authenticate to UnifAI's /mcp endpoint — virtual key headers or browser-based OAuth 2.1."
icon: "key"
---

## Overview

When UnifAI acts as an [MCP Gateway](./gateway), external MCP clients connect to its `/mcp` endpoint. This page covers how those **inbound clients authenticate to UnifAI**.
...
- **OAuth 2.1** — UnifAI acts as an OAuth authorization server, and the client connects through a browser consent flow, receiving a short-lived JWT. Ideal for interactive clients like Claude Desktop, Claude Code, or Cursor, where pasting a raw key into client config is awkward.

<Note>
This page is about authenticating clients **to** UnifAI. For how UnifAI authenticates **to upstream MCP servers** it connects to, see the outbound [Authentication](./auth/overview) guides instead — that's the opposite direction.
</Note>
...
| Mode | Header credentials (VK / api-key / session) | UnifAI-issued JWT | Discovery endpoints |
|---|---|---|---|
...
- **`both`** — `/mcp` accepts header credentials **and** UnifAI-issued JWTs. Discovery is served so OAuth clients can connect.
- **`oauth`** — `/mcp` accepts UnifAI-issued JWTs only; header credentials are rejected.

A request must carry **exactly one** credential type. If an OAuth access token and a header credential (`x-uf-vk`, `X-Api-Key`, or a Bearer VK) arrive on the same request, UnifAI rejects it with a `conflicting credentials` error — even in `both` mode. `both` means either credential is accepted, not both at once.
...
When `mcp_server_auth_mode` is `both` or `oauth`, UnifAI is a full OAuth 2.1 authorization server for the `/mcp` resource. A client that doesn't yet have a token discovers the server, registers itself, and walks the user through a browser consent step.

```mermaid
sequenceDiagram
    participant C as MCP Client
    participant U as User (Browser)
    participant B as UnifAI
...
During the consent step, UnifAI shows the user how they can identify themselves. The page header names the connecting client — for example **"Claude Code wants to connect"** — and offers the modes available for your deployment:
...
- **User** binds the grant to the authenticated person, so per-user upstream tool authorizations UnifAI under one identity.
  ...
  -d '{
    "client_config": {
      ...
      "oauth2_server_config": {
        "issuer_url": "https://unifai.example.com",
        "auth_code_ttl": 300,
...
With `both` or `oauth` enabled, point the MCP client at UnifAI's `/mcp` URL — for example `https://unifai.example.com/mcp`. No key needs to be pasted into the client config.
...
Each completed OAuth connection is a **grant** — a refresh-token lineage UnifAI issued to a client. The **OAuth Grants** page lists them: the client, the bound identity, when the grant was created, and when it was last used, with a **Revoke** action.
...
<Info>
The **OAuth Grants** page lists credentials UnifAI **issued to clients** for inbound `/mcp` access. This is distinct from [MCP Sessions](./sessions), which tracks per-user credentials UnifAI holds for **upstream** MCP servers.
</Info>
...
When discovery is enabled (`both` or `oauth`), UnifAI serves the standard documents MCP clients fetch automatically:
...
**Cause:** The client completed an OAuth flow but is also configured with a VK header, so both arrive on one request. Claude Code does this in `both` mode when the VK is set under `x-uf-vk` / `X-Api-Key` instead of `Authorization`.
...
- **[UnifAI as an MCP Gateway](./gateway)** — Expose aggregated tools to external MCP clients.
