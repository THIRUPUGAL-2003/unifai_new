...
UnifAI clustering uses a **peer-to-peer (P2P) network** where all nodes are equal participants. Each node:
...
UnifAI uses two transports for different responsibilities. Membership and node-liveness signals run over a memberlist gossip layer; everything else (configuration changes, governance counters, routing rules, all replicated entity types) travels over a dedicated gRPC channel.
...
Each replicated message carries an `EntityType` identifying the kind of state being broadcast. UnifAI replicates 30+ entity types across the cluster, including: model catalog, virtual keys, providers, governance counters, routing rules, RBAC, MCP tools and tool groups, pricing and pricing overrides, access profiles, prompt deployments, auth configuration, and cluster diagnostics. See the [Replicated Entity Types](#replicated-entity-types) reference for the complete list.
...
UnifAI runs two leader elections in parallel: one cluster-wide and one per region.
...
The broker is **not** a separate binary - the same UnifAI Enterprise image
runs as the broker when started with the `-mode=broker` flag (or the
`UNIFAI_MODE=broker` environment variable):

```bash
unifai-enterprise -mode=broker -app-dir /app/data
```
...
UnifAI supports 6 service discovery methods to fit any infrastructure. Choose based on your deployment environment:
...
1. Each UnifAI pod queries the Kubernetes API for pods matching the label selector
...
| `k8s_label_selector` | Yes | Label selector for pod discovery | `"app=unifai"`, `"app=unifai,env=prod"` |
  ...
  template:
    metadata:
      labels:
        app: unifai
    spec:
      serviceAccountName: unifai
      containers:
      - name: unifai
        image: <enterprise_repo_base_url>/unifai:latest
        ports:
        - containerPort: 8080
          name: http
        - containerPort: 10101
...
**Solutions**:
- Create ServiceAccount for UnifAI pods
...
1. Each UnifAI node registers itself with Consul on startup
2. Nodes query Consul to discover other UnifAI instances
...
services:
  ...
  unifai-1:
    image: <enterprise_repo_base_url>/unifai:latest
    environment:
      - UNIFAI_CONFIG=/etc/unifai/config.json
    volumes:
      - ./config-node1.json:/etc/unifai/config.json
    ports:
      - "8080:8080"
    depends_on:
      - consul
  ...
  unifai-2:
    ...
    environment:
      - UNIFAI_CONFIG=/etc/unifai/config.json
    volumes:
      - ./config-node2.json:/etc/unifai/config.json
    ports:
      - "8081:8080"
    depends_on:
      - consul
  ...
  unifai-3:
    ...
    environment:
      - UNIFAI_CONFIG=/etc/unifai/config.json
    volumes:
      - ./config-node3.json:/etc/unifai/config.json
    ports:
      - "8082:8080"
    depends_on:
      - consul
...
- Verify network connectivity between UnifAI and Consul
...
- Review UnifAI logs for startup errors
...
1. Each UnifAI node registers itself in etcd with a lease
...
services:
  ...
  unifai-1:
    ...
    environment:
      - UNIFAI_CONFIG=/etc/unifai/config.json
    volumes:
      - ./config.json:/etc/unifai/config.json
    ports:
      - "8080:8080"
    depends_on:
      - etcd
...
2. UnifAI queries DNS to resolve configured names
...
```json
{
  "cluster_config": {
    ...
    "discovery": {
      ...
      "service_name": "unifai-cluster",
      "dns_names": [
        "unifai-cluster.local",
        "unifai-nodes.internal.company.com"
      ],
      "bind_port": 10101
    },
    "gossip": {
...
| Parameter | Required | Description | Example |
|-----------|----------|-------------|---------|
| `dns_names` | Yes | Array of DNS names to resolve | `["unifai.local"]`, `["node1.local", "node2.local", "node3.local"]` |
...
aws route53 change-resource-record-sets \
  ...
  --change-batch '{
    "Changes": [{
      "Action": "CREATE",
      "ResourceRecordSet": {
        "Name": "unifai-cluster.internal.company.com",
        "Type": "A",
        "TTL": 60,
        "ResourceRecords": [
          {"Value": "10.0.1.10"},
...
spec:
  ...
  - port: 10101
    name: gossip
---
# DNS will resolve unifai-cluster.default.svc.cluster.local
...
```bash
# /etc/dnsmasq.conf
address=/unifai-cluster.local/192.168.1.10
address=/unifai-cluster.local/192.168.1.11
address=/unifai-cluster.local/192.168.1.12

# Or use /etc/hosts on each node
echo "192.168.1.10 node1.unifai.local" >> /etc/hosts
echo "192.168.1.11 node2.unifai.local" >> /etc/hosts
echo "192.168.1.12 node3.unifai.local" >> /etc/hosts
```
  </Tab>
</Tabs>
...
**Solutions**:
- Verify DNS names are resolvable: `nslookup unifai-cluster.local`
- Check DNS server is accessible from UnifAI nodes
...
| Parameter | Required | Description | Example |
|-----------|----------|-------------|---------|
| `mdns_service` | No | mDNS service type | `"_unifai._tcp"` (default), `"_myapp._tcp"` |
...
docker run -p 8080:8080 -p 10101:10101 -p 10201:10102 \
  -v $(pwd)/config-mdns.json:/etc/unifai/config.json \
  <enterprise_repo_base_url>/unifai:latest
...
services:
  postgres:
    ...
    environment:
      ...
      POSTGRES_PASSWORD: unifai_password
    volumes:
...
http {
    upstream unifai_cluster {
        least_conn;
        server unifai-1:8080 max_fails=3 fail_timeout=30s;
        server unifai-2:8080 max_fails=3 fail_timeout=30s;
        server unifai-3:8080 max_fails=3 fail_timeout=30s;
    }
    ...
    server {
        ...
        location / {
            proxy_pass http://unifai_cluster;
  ...
  config.json: |
    {
      "cluster_config": {
        ...
        "discovery": {
          ...
          "k8s_label_selector": "app=unifai,component=gateway"
        },
        "gossip": {
          "port": 10101,
          "config": {
...
**Step 1: Install UnifAI on each node**

```bash
# Download UnifAI Enterprise binary
curl -O https://releases.getmaxim.ai/unifai-enterprise/latest/unifai-enterprise-linux-amd64
chmod +x unifai-enterprise-linux-amd64
sudo mv unifai-enterprise-linux-amd64 /usr/local/bin/unifai-enterprise
```

**Step 2: Create configuration file**

```bash
sudo mkdir -p /etc/unifai
sudo cat > /etc/unifai/config.json <<EOF
{
  "cluster_config": {
    "enabled": true,
    "discovery": {
      ...
      "type": "dns",
      "service_name": "unifai-cluster",
      "dns_names": ["unifai-cluster.internal.company.com"]
    },
    "gossip": {
      "port": 10101,
      "config": {
...
```bash
sudo cat > /etc/systemd/system/unifai.service <<EOF
[Unit]
Description=UnifAI Enterprise API Gateway
...
User=unifai
Group=unifai
Environment="UNIFAI_CONFIG=/etc/unifai/config.json"
ExecStart=/usr/local/bin/unifai-enterprise
Restart=always
RestartSec=10
...
```bash
# Add A records for unifai-cluster.internal.company.com
...
```bash
sudo useradd -r -s /bin/false unifai
sudo mkdir -p /var/lib/unifai
sudo chown unifai:unifai /var/lib/unifai
sudo systemctl daemon-reload
sudo systemctl enable unifai
...
UnifAI negotiates per-peer capabilities so newer and older cluster members can run side-by-side during rolling upgrades. Peers advertise the `ack:v1` capability in their gossip metadata; a node only tracks ACKs from peers that advertise it. Older peers still receive broadcasts but are excluded from the pending-ACK set so they don't trigger false "unacked" metrics or unnecessary retries.

In practice this means: you can roll out a new UnifAI version one pod at a time, and the cluster will degrade ACK tracking gracefully for the older pods until they're replaced. Rolling-update strategies (Kubernetes `RollingUpdate`, Nomad canaries, etc.) are supported without quorum loss.
...
{
  "local_node": "unifai-remote-10101-...",
  "members": 3,
  "healthy_members": 3,
...
This clustering implementation ensures UnifAI can handle enterprise-scale deployments with high availability, automatic service discovery, and intelligent traffic distribution across any infrastructure.
