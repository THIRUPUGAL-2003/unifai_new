...
description: "Mandatory controls before exposing UnifAI Enterprise to production traffic: IdP-enforced identity, scoped virtual keys, locked-down CORS, and a minimal header allowlist."
icon: "shield-halved"
---

<Warning>
**Do not skip this page.** UnifAI Enterprise ships with sensible defaults, but a production deployment is only as safe as the controls you actually turn on. The items below are the minimum required hardening before exposing the gateway to real users or real inference traffic.
</Warning>
...
- Group claims in the IdP token drive team attachment. As of Enterprise v1.4.0, UnifAI no longer enriches groups via directory API calls (see the [v1.4.0 migration notes](/enterprise/migration-guides/v1.4.0)).
...
There is no anonymous path to a model in Enterprise. Every `/v1/chat/completions`, `/v1/embeddings`, `/v1/images/generations`, and similar call must present a [Virtual Key](/features/governance/virtual-keys), which UnifAI resolves to one or more upstream provider keys with explicit provider, model, and budget scopes.
...
- Virtual keys are the **only** credential clients present on inference calls. Raw upstream provider keys (OpenAI, Anthropic, Bedrock, etc.) never leave UnifAI.
...
The default of `*` is for development only. Production deployments must restrict CORS to the specific origins that legitimately call UnifAI. Set `client.allowed_origins` to the explicit list:
...
UnifAI has **two distinct header controls** and both should be locked down in production. They live at different points in the request path:

| Control | Schema field | What it gates |
|---|---|---|
| **CORS / WebSocket allowlist** | `client.allowed_headers` | Headers callers can send TO UnifAI over CORS / WebSocket |
| **Provider-forwarded allowlist** | `client.header_filter_config.allowlist` | Which `x-uf-eh-*` prefixed headers UnifAI forwards FROM clients TO upstream LLM providers |

### CORS / WebSocket allowlist

Narrow `allowed_headers` to the minimum your callers actually need - typically `Authorization`, `Content-Type`, a tracing header, and any UnifAI-specific headers documented in [Request Options](/providers/request-options):
...
### Provider-forwarded `x-uf-eh-*` allowlist

Callers can attach `x-uf-eh-*` prefixed headers that UnifAI will forward to upstream providers (e.g., to set provider-specific feature flags or trace IDs). In production, set an explicit allowlist for which of these are permitted:
...
```json
{
  "client": {
    "header_filter_config": {
      "allowlist": [
        "x-uf-eh-anthropic-beta",
        "x-uf-eh-anthropic-version",
        "x-uf-eh-x-trace-id"
      ],
      "denylist": [
        "x-uf-eh-cookie",
        "x-uf-eh-proxy-authorization",
        "x-uf-eh-host"
      ]
    }
  }
