...
2. **User copy** - When a user becomes eligible for a profile, UnifAI clones the template into a per-user copy with fresh budget and rate-limit counters. Each user accumulates usage independently.
3. **Virtual key** - UnifAI issues a virtual key whose provider configs, budgets, rate limit, and MCP configs are built from the user copy. The key is marked profile-managed so it cannot be edited around the policy.
...
3. UnifAI issues a virtual key for every member of the Engineer role and continues to auto-issue for any user who later gains the role.
