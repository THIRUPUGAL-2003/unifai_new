...
1. UnifAI writes per-request metadata (timestamps, provider, model, latency, token counts, cost, status, IDs) to the logs database.
...
| `prefix` | optional | Object key prefix. Defaults to `unifai`. |
...
UnifAI ships a background log cleaner that deletes old logs on a fixed schedule.
...
| Startup behaviour | A cleanup pass runs immediately when UnifAI starts |
| Delete batch size | 100 rows per query |
| Per-pass timeout | 30 minutes |

Source: `framework/logstore/cleaner.go` and `transports/unifai-http/server/server.go`.
...
```json
{
  "Rules": [
    {
      "ID": "unifai-logs-expire-30d",
      "Status": "Enabled",
      "Filter": { "Prefix": "unifai/logs/" },
      "Expiration": { "Days": 30 }
    }
  ]
}
...
The UnifAI Helm chart at `helm-charts/unifai/` does not expose dedicated values for `logs_store.object_storage`. It renders the same `config.json` schema documented above, so you supply the block under the chart's `unifai.*` configuration tree (or mount your own `config.json` via an existing ConfigMap/Secret).
...
The chart converts camelCase keys to the `snake_case` form UnifAI expects when it writes the runtime `config.json`. Provide the referenced env vars (`AWS_*`, `GCS_*`) through `extraEnv`, `envFrom`, or an existing Kubernetes Secret.
...
2. Restart UnifAI so the new log store wiring takes effect.
...
1. Send a request through UnifAI.
...
If the bucket stays empty, check the UnifAI logs for `objectstore` errors. The most common causes are missing credentials, a region/endpoint mismatch, or a bucket policy that blocks the credentials' principal.
