...
|-------|------|----------|---------|-------------|
| `service_name` | `string` | No | `unifai` | Service name displayed in Datadog APM |
...
| `group_traces_by_session` | `bool` | No | `false` | Group requests sharing the same `x-uf-session-id` into one APM trace (agent mode only). See [Grouping APM Traces by Session](#grouping-apm-traces-by-session) |
...
Configure the Datadog plugin through the UnifAI UI:
...
import (
    ...
    datadog "github.com/unifai/unifai-enterprise/plugins/datadog"
)
...
The plugin supports session tracking via the `x-uf-session-id` header. Include this header in your requests to group related LLM calls into a conversation session:

```bash
curl -X POST https://your-unifai-gateway/v1/chat/completions \
  -H "Authorization: Bearer $API_KEY" \
  -H "x-uf-session-id: user-123-session-456" \
  -d '{...}'
```
...
curl ... -H "x-uf-session-id: user-123-session-456" -d '{...}'
curl ... -H "x-uf-session-id: user-123-session-456" -d '{...}'
```

UnifAI derives a stable Datadog trace ID from the `x-uf-session-id` value, so all requests carrying that header resolve to the same trace.
...
The plugin supports [W3C Trace Context](https://www.w3.org/TR/trace-context/) for distributed tracing across services. When your upstream service sends a `traceparent` header, UnifAI automatically links its spans as children of the parent trace.
...
- **Cross-service correlation** - Link frontend requests → backend services → UnifAI → LLM providers
...
All Datadog APM spans created by UnifAI will be linked to the parent span, appearing as children in the Datadog trace view.
...
|--------|------|-------------|------|
| `unifai.requests.total` | Counter | Total LLM requests | provider, model, method |
| `unifai.success.total` | Counter | Successful requests | provider, model, method |
| `unifai.errors.total` | Counter | Failed requests | provider, model, method, reason |
| `unifai.latency.seconds` | Histogram | Request latency distribution | provider, model, method |
| `unifai.tokens.input` | Counter | Input/prompt tokens consumed | provider, model |
| `unifai.tokens.output` | Counter | Output/completion tokens generated | provider, model |
| `unifai.tokens.total` | Counter | Total tokens (input + output) | provider, model |
| `unifai.request.cost.usd` | Distribution | Per-request cost in USD | provider, model |
| `unifai.cache.hits` | Counter | Cache hits | provider, model, cache_type |
| `unifai.stream.first_token_latency` | Histogram | Time to first token (streaming) | provider, model |
| `unifai.stream.inter_token_latency` | Histogram | Inter-token latency (streaming) | provider, model |

### Migrating from `unifai.cost.usd`

The cost metric was renamed from `unifai.cost.usd` to `unifai.request.cost.usd`, and its type changed from **Gauge** to **Distribution**. The gauge was last-write-wins per flush window, so concurrent requests with the same tags collapsed to a single value and no query could recover the true total spend. The new name is required because Datadog permanently associates a metric name with its first-seen type per organization — orgs that previously received the gauge cannot receive the same name as a distribution.

**Affected assets:** any dashboards, monitors, saved views, or alerts that query `unifai.cost.usd`.

**To migrate:**

1. Replace `unifai.cost.usd` with `unifai.request.cost.usd` in all queries.
2. Update aggregations for Distribution semantics — each sample is one request's cost:
   - Total spend: `sum:unifai.request.cost.usd{*}` (do **not** append `.as_count()` or `.rollup(sum)`; the `sum:` aggregator already returns the additive total)
...
`unifai.cost.usd` stops receiving data once the upgrade completes; during a rolling deploy both metrics receive data, so update dashboards at or shortly after the upgrade. Historical gauge data remains queryable under the old name for Datadog's standard retention window.
...
- `unifai_node` - Per-instance identity (`UNIFAI_NODE_ID` if set, otherwise `hostname-pid`)
- plus UnifAI-context tags when available (virtual key, selected key, team, customer, fallback index)
...
The Datadog plugin captures all UnifAI request types:
...
1. Enable debug logging in UnifAI:
```bash
unifai-http --log-level debug
```
...
3. Check for errors in the UnifAI logs related to the Datadog plugin
