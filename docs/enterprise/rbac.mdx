...
description: "Manage user access with fine-grained permissions across UnifAI resources using roles and permissions."
icon: "user-shield"
---

## Overview

Role-Based Access Control (RBAC) in UnifAI Enterprise provides fine-grained access management for your organization. RBAC allows you to define roles with specific permissions, controlling what users can view, create, update, or delete across all UnifAI resources.
...
Navigate to **Governance** → **Roles & Permissions** in the UnifAI dashboard to manage roles.
...
UnifAI includes three pre-configured system roles that cover common access patterns:
...
UnifAI protects access to the following resources:
...
- **IdP Groups** - Map identity provider groups to UnifAI roles
