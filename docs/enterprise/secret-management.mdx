...
description: "Connect AWS Secrets Manager, GCP Secret Manager, or HashiCorp Vault so UnifAI never stores plaintext API keys in its database."
icon: "vault"
---

## Overview

By default, UnifAI stores provider API keys, virtual key values, and other credentials in its config database. Secret Management lets you keep those values in your own secret manager - UnifAI stores a reference and resolves the real value at runtime.

Once connected, any secret field in UnifAI (provider keys, virtual key values, MCP auth headers, etc.) accepts a `vault.<path>` reference alongside the existing `env.<VAR>` and plaintext options.
...
Set `access_mode` to control how much UnifAI interacts with your vault:

| Mode | What UnifAI does |
|------|-------------------|
...
Start with `read_only` if you want to manage secrets yourself. Use `read_and_write` if you want UnifAI to handle it - useful when migrating existing plaintext keys, since UnifAI pushes the value to the vault on the next save.
...
```json
{
  "config_store": {
    ...
    "vault_store": {
      ...
      "aws": {
        "region": "us-east-1",
        "role_arn": "arn:aws:iam::123456789012:role/UnifAISecretsReader"
      }
    }
  }
}
  ...
  "Statement": [
    {
      ...
      "Resource": "arn:aws:secretsmanager:us-east-1:*:secret:unifai/*"
    },
    {
      "Effect": "Allow",
...
UnifAI uses the KV v2 secrets engine. Auth is resolved in order: explicit `token` → AppRole → ambient `VAULT_TOKEN` env var.
...
If `VAULT_TOKEN` is set in the environment and no `token` or AppRole is configured, UnifAI inherits it automatically. Useful with Vault Agent injection.
...
```hcl
path "secret/data/unifai/*" {
  capabilities = ["read"]
}
path "secret/metadata/unifai/*" {
  capabilities = ["list"]
}
```
...
Any secret field in UnifAI that supports `env.<VAR>` also supports `vault.<path>`. This covers provider keys, virtual key values, MCP auth headers, plugin credentials, observability tokens, and more. Type the reference directly in the dashboard or set it in `config.json`.
      ...
      "keys": [
        {
          ...
          "value": "vault.unifai/providers/openai/key"
        }
      ]
    }
  }
...
<Note>
Fragment references are never auto-deleted by UnifAI in `read_and_write` mode, because the secret is externally managed and may be shared with other services.
</Note>
...
Update the value in your secret manager - no UnifAI restart needed. UnifAI checks for updated secrets every hour in the background and picks up the new value automatically.
...
**UnifAI fails to start with "vault: ping failed"**
