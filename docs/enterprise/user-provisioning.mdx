...
UnifAI Enterprise connects your organization's identity provider to UnifAI through OAuth 2.0 / OIDC login, provider-backed directory sync, and inbound SCIM 2.0 provisioning. A single configuration gives you:
...
- **Team synchronization** from IdP groups into UnifAI teams
- **Business unit mapping** from IdP attributes to UnifAI business units
...
- **Inbound SCIM 2.0** — IdPs can push user and group changes to UnifAI in real time via the `/scim/v2` API

Once configured, users sign in to UnifAI with their corporate credentials and inherit the right [role and permissions](./rbac) immediately — no manual account creation.

<Frame>
  <img src="/media/user-provisioning/scim-overview.png" alt="User Provisioning overview in UnifAI dashboard" />
</Frame>
...
Pick your IdP to follow a step-by-step setup guide. All providers share the same UnifAI configuration surface — the only difference is how the OAuth client and role/group claims are created on the provider side.
...
1. **Login** — UnifAI redirects unauthenticated users to the provider's authorization endpoint (Authorization Code flow).
2. **Token exchange** — on callback, UnifAI exchanges the code for an access token and refresh token, stores them in an `HttpOnly` cookie / server session, and validates the JWT against the provider's JWKS.
3. **Identity extraction** — configurable JWT claims (`userIdField`, `rolesField`, `teamIdsField`) are mapped to a UnifAI user, role, and teams. Provider-specific app roles or custom attributes override claim lookup.
4. **Attribute mapping** — optional `attributeRoleMappings`, `attributeTeamMappings`, and `attributeBusinessUnitMappings` translate arbitrary claim values (e.g., a department string or Okta group name) into UnifAI roles, teams, or business units.
5. **Session refresh checks** — every 15 minutes, UnifAI refreshes the OIDC session. If the session cannot be refreshed, UnifAI checks with the OIDC server whether the user is still active.
6. **Background reconciliation** — UnifAI periodically calls the configured provider's directory APIs to reconcile imported users and mapped roles, teams, and business units.
...
8. **Daily sync** — UnifAI reconciles imported users every 24 hours.
9. **SCIM push** — IdPs configured with a SCIM provisioning connector can push user creates, updates, and deletes to UnifAI in real time via `/scim/v2`.
10. **Decommissioning** — if the OIDC server reports that a user is no longer active, the 24-hour reconciliation no longer finds them in the active source set, or a SCIM DELETE arrives, UnifAI decommissions that user locally.
...
| **Team mapping** | Map multiple claim values to UnifAI teams in a single pass (a user can belong to many teams). |
...
| **Bulk import** | Import matched users into UnifAI with role + team + BU assignments applied. |
| **Team sync** | Sync IdP groups as UnifAI teams with a single action. |
| **Business unit sync** | Sync IdP organizational units as UnifAI business units. |
| **Inbound SCIM 2.0** | IdPs push user and group changes to UnifAI via `/scim/v2` in real time. Bearer-token authenticated. |
...
| **Deprovisioning** | UnifAI checks user status during each 15-minute OIDC session refresh and reconciles imported users against the provider directory every 24 hours. SCIM DELETE/deactivation (`active: false`) is also handled immediately. Users that are inactive, disabled, unassigned, or missing from the source set are decommissioned locally. |
| **API key pass-through** | Requests using UnifAI API keys (`bfst-*`) bypass OIDC user-provisioning middleware so inference traffic is not affected. |

---

## Background lifecycle reconciliation

UnifAI's lifecycle model combines source-side reconciliation, OIDC session validation, and real-time SCIM push.

Every **15 minutes**, UnifAI refreshes active OIDC sessions. If a session cannot be refreshed, UnifAI checks with the OIDC server whether the user is still active; if the provider reports the user is inactive, UnifAI decommissions that user locally.

After users are imported, UnifAI also uses the configured provider credentials to sync with the IdP in the background every **24 hours**. That sync updates mapped roles, teams, and business units, and decommissions imported users that are disabled, unassigned, or no longer present in the provider source set.
...
<Note>
  Changing `scim_config` at runtime through the UI is applied after saving. For file-based configuration, restart the UnifAI server to pick up changes.
</Note>
...
Fields marked **env.\* supported** accept `"env.VAR_NAME"` in addition to a literal value — UnifAI resolves the variable from the process environment at startup. Attribute mapping arrays are always plain JSON (they cannot reference env vars).
...
1. Navigate to **Governance → User Provisioning** in the UnifAI dashboard.
...
<Frame>
  <img src="/media/user-provisioning/scim-provider-select.png" alt="Selecting an OIDC provider in the UnifAI dashboard" />
</Frame>

4. Click **Verify** to test credentials end-to-end. UnifAI will reach the provider's JWKS / directory endpoint and report any failures.
...
Attribute mappings let you translate claim values into UnifAI roles, teams, or business units without forcing your IdP admins to restructure claim names.
...
Claim values can be strings, arrays, or nested objects — UnifAI resolves dotted paths (e.g., `realm_access.roles`).
...
Team and business unit mappings also support an optional `attributeValue` field. This is the key UnifAI uses to look up the attribute in a SCIM-provisioned user's stored profile (a flat key-value map).
...
UnifAI exposes a SCIM 2.0 endpoint that identity providers can use to push user and group changes in real time, without waiting for the next 24-hour reconciliation cycle.

### Base URL

```
https://<your-unifai-host>/scim/v2
```
...
The provisioning token is generated per SCIM provider and can be rotated from the UnifAI dashboard under **Governance → User Provisioning → SCIM Settings**.
...
There are two ways to get users into UnifAI, depending on what your IdP supports:
...
4. Click **Import** to create them in UnifAI with role / team / BU assignments applied.
...
If the IdP supports SCIM provisioning (e.g. Okta with a SCIM app), you can configure it to push users to UnifAI automatically — no manual import needed. Users are created, updated, and deactivated in UnifAI as they change in the IdP. See the [Inbound SCIM 2.0 provisioning](#inbound-scim-20-provisioning) section and your IdP's setup guide for configuration steps.
...
| --- | --- |
| Access denied: no application role or group mapping is assigned to this user. | Make sure you have assigned the user to the UnifAI IdP application and they have a valid group/attribute mapping to a role in UnifAI. |
| Redirect loop on login | Make sure you have restarted pods/UnifAI instance after changing OIDC configuration, or check for a redirect URI mismatch. Exact string match required — check trailing slashes and `http` vs `https`. |
