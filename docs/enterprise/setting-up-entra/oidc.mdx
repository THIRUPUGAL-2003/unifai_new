...
description: "Configure Microsoft Entra ID (Azure AD) as your identity provider for UnifAI Enterprise using OpenID Connect."
---
...
| **Redirect URI** | Web: `https://<your-unifai-domain>/login` |
...
<Tip>
  You can add an app icon to make the application easily recognizable. The UnifAI logo is available at `https://www.getmaxim.ai/unifai/unifai-logo-only.png`.
</Tip>
...
| URI | Purpose |
| --- | --- |
| `https://<your-unifai-domain>/login` | OIDC sign-in callback |
| `https://<your-unifai-domain>/workspace/scim/oauth-discover-callback` | Attribute discovery callback |

<Frame caption="Authentication blade — confirm both redirect URIs are listed.">
  <img src="/media/user-provisioning/entra/entra-oidc-redirect-uri.png" alt="Authentication blade showing the two UnifAI redirect URIs configured" />
</Frame>
...
Navigate to **App registrations → UnifAI SSO → Overview**.
...
<Note>
  This step is optional. App roles are one way to assign UnifAI roles to users, but you can also map any other Entra attribute (e.g. groups, department) instead. Role mapping itself is required.
</Note>

In your app registration, go to **App roles** and click **Create app role**.

Create a role for each UnifAI role you want to use. For example, for the Viewer role:
...
| **Value** | `viewer` |
| **Description** | Viewer role on UnifAI |
| **State** | Enabled |
...
<Frame caption="App Roles list — Admin, Developer, and Viewer roles configured for the application.">
  <img src="/media/user-provisioning/entra/entra-oidc-app-roles.png" alt="Entra App Roles list showing Admin, Developer, and Viewer roles configured for the UnifAI Enterprise application" />
</Frame>
...
| Field | Value |
| --- | --- |
| **Description** | `UnifAI Enterprise Secret` |
...
From the Entra ID main menu, go to **Enterprise applications** and find **UnifAI SSO**.

<Frame caption="Enterprise Applications list — select UnifAI SSO to open the enterprise application settings.">
  <img src="/media/user-provisioning/entra/entra-oidc-enterprise-apps-list.png" alt="Enterprise Applications list in Microsoft Entra showing the UnifAI SSO entry" />
</Frame>
...
Go to **UnifAI SSO → Users and groups** and click **Add user/group**.
...
<Frame caption="Select Microsoft Entra from the list of identity providers.">
  <img src="/media/user-provisioning/entra/unifai-provider-selection.png" alt="UnifAI Choose Provider screen with Microsoft Entra highlighted" />
</Frame>
...
UnifAI opens a sign-in popup — no session is created. Once you authenticate, it returns the exact claims your Entra tenant is sending in the JWT.
...
<Frame caption="Discover Claims — shows the exact claims Entra is returning in the JWT, including roles, groups, and any custom attributes.">
  <img src="/media/user-provisioning/entra/unifai-discover-claims.png" alt="UnifAI Discover Claims screen listing all claims returned by Entra including roles, groups, email, and custom attributes" />
</Frame>
...
Use the sections below the claim list to map Entra claim values to UnifAI roles, teams, and business units.
...
<Frame caption="Attribute Mapping — configure role, team, and business unit rules based on the claims Entra sends.">
  <img src="/media/user-provisioning/entra/unifai-attribute-setup.png" alt="UnifAI Attribute Mapping screen showing role, team, and business unit mapping rules" />
</Frame>
...
UnifAI uses the app credentials from this setup to sync users in the background every **24 hours**. During that sync, UnifAI reconciles imported users, role mappings, team mappings, and business-unit mappings from Entra.

Every **15 minutes**, UnifAI also refreshes active OIDC sessions. If a session cannot be refreshed, UnifAI checks with Entra whether the user is still active; if Entra reports the user as inactive, UnifAI decommissions that user locally.

---

## Optional next steps

**Sync users and groups in real time** — OIDC alone provisions users on first login. To keep UnifAI in sync with Entra as users are created, deactivated, or moved between groups, set up [SCIM with Entra](./scim).

---

## Troubleshooting

**User is not redirected to Microsoft login** — verify the provider is enabled in UnifAI and the server was restarted after saving. Check that the Tenant ID and Client ID are correct.

**`AADSTS50011: The reply URL does not match`** — verify both redirect URIs in **Authentication** (Step 1) exactly match your UnifAI URLs. Check for trailing slashes or http vs https mismatches.
