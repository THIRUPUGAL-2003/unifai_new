...
Navigate to **Circuit Breaker** in the UnifAI dashboard to create and manage policies.
...
Signals define what UnifAI watches for in the provider's HTTP response. Each signal inspects a single response header using one of three match modes:
...
When the circuit opens, UnifAI blocks the primary provider for a cooldown duration before probing it again.
...
When `retry-after-ms` is present and parsable, UnifAI uses its value as the cooldown. If the header is absent or cannot be parsed, `default_cooldown` is used as the fallback.
...
3. UnifAI detects the header, opens the circuit for 30 seconds.
...
5. After 30 seconds, UnifAI probes the PTU deployment again. If spillover is no longer signalled, the circuit closes and PTU traffic resumes.
