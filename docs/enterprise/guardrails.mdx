---
title: "Guardrails"
description: "Enterprise-grade content safety and security validation with support for native regex and secrets detection, AWS Bedrock Guardrails, Azure Content Safety, Google Model Armor, CrowdStrike AIDR, GraySwan Cygnal, and Patronus AI."
icon: "road-barrier"
---

## Overview

**Guardrails** in UnifAI provide enterprise-grade content safety, security validation, and policy enforcement for LLM requests and responses. The system validates inputs and outputs in real-time against your specified policies, ensuring responsible AI deployment with protection against harmful content, prompt injection, PII leakage, credential leakage, and policy violations.
...
UnifAI Guardrails are built around two core concepts that work together to provide flexible and powerful content protection:
...
| **Profiles** | Configurations for guardrail providers, including UnifAI-native providers (Custom Regex, Secrets Detection) and external providers (AWS Bedrock, Azure Content Safety, Google Model Armor, CrowdStrike AIDR, GraySwan, Patronus AI). Profiles are reusable and can be shared across multiple rules. |

**How They Work Together:**
- **Profiles** define *how* content is evaluated using native UnifAI checks or external provider capabilities
...
| Feature | Description |
|---------|-------------|
| **Multi-Provider Support** | UnifAI-native Custom Regex and Secrets Detection, plus AWS Bedrock, Azure Content Safety, Google Model Armor, CrowdStrike AIDR, GraySwan, and Patronus AI integrations |
...
Access Guardrails from the UnifAI dashboard:
...
**Flow Description:**
1. **Incoming Request** - LLM request arrives at UnifAI
...
When an output guardrail is used with a streaming response, UnifAI accumulates the response until the model is done generating it, then checks the full response. If the response passes, UnifAI sends it to the client. If a guardrail blocks or changes the response, UnifAI applies that result instead.

This adds time before the client receives output: UnifAI waits for response generation and guardrail evaluation to finish. The added wait depends on the model's generation time and the configured guardrail profiles.

Input guardrails check the request before UnifAI sends it to the LLM provider.
...
<Note>
  If the same rule also uses another output guardrail profile, UnifAI waits for that profile to check the completed response. GraySwan's text-only behavior only skips the GraySwan call; it does not bypass the other profile.
</Note>
...
Profiles are reusable configurations for guardrail providers. External providers include credentials, endpoints, and detection thresholds. UnifAI-native providers such as Custom Regex and Secrets Detection run locally and do not require external service credentials.
...
Profiles are managed per provider type at `/api/guardrails/{provider}`, where `{provider}` is one of `secrets`, `regex`, `bedrock`, `azure`, `model-armor`, `crowdstrike-aidr`, `grayswan`, or `patronus-ai`. The API assigns the configuration ID after creation.
  ...
  guardrail_providers:
    ...
    - id: 3
      ...
      config:
        ...
        # (UnifAI will use IAM credentials from the environment)
    - id: 4
      provider_name: "azure"
      policy_name: "Content Safety Profile"
      enabled: true
...
Third-party guardrail providers offer different capabilities. UnifAI-native providers are documented separately: [Secrets Detection](/enterprise/guardrails/secrets-detection) covers credential leakage, and [Custom Regex](/enterprise/guardrails/custom-regex) covers deterministic pattern checks, including the PII Detection template.
...
<Note>
  CrowdStrike AIDR capabilities depend on the AIDR policy and detectors configured in CrowdStrike. UnifAI sends the request to AIDR, then enforces the returned `blocked` or `transformed` decision.
</Note>
...
curl -X POST http://localhost:8080/v1/chat/completions \
  -H "Content-Type: application/json" \
  -H "x-uf-guardrail-id: bedrock-prod-guardrail" \
  -d '{
    "model": "gpt-4o-mini",
    "messages": [
      {
...
curl -X POST http://localhost:8080/v1/chat/completions \
  -H "Content-Type: application/json" \
  -H "x-uf-guardrail-ids: bedrock-prod-guardrail,azure-content-safety-001" \
  -d '{
    "model": "gpt-4o-mini",
    "messages": [
      {
