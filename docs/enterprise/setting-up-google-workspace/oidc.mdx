...
description: "Configure Google Workspace as your identity provider for UnifAI Enterprise using OAuth 2.0 / OIDC."
---
...
<Frame caption="Create credentials - select OAuth client ID to generate a client for UnifAI.">
...
| **Name** | `UnifAI Enterprise` |
| **Authorized JavaScript origins** | `https://<your-unifai-domain>` |
| **Authorized redirect URIs** | `https://<your-unifai-domain>/login` |
...
Copy the **Client ID** and **Client Secret** - you will need them in [Step 5](#step-5-configure-unifai).
...
<Frame caption="Create a service account - the account is used to call the Admin Directory API on behalf of UnifAI.">
...
Give it a name (e.g. `unifai-provisioning`). Skip the **Grant this service account access to project** step - no GCP IAM roles are required; access is granted via domain-wide delegation in Step 4.
...
<Note>
  Domain-wide delegation requires impersonating an admin user. Choose an admin email that will persist (e.g. a dedicated `sso-admin@company.com`) - UnifAI uses this as the **Admin Email** in Step 5.
</Note>
...
<Frame caption="Provider Configuration - enter your Google OAuth credentials and optional service account details.">
  <img src="/media/user-provisioning/gws-form.png" alt="UnifAI Provider Configuration form for Google Workspace showing Domain, Client ID, Client Secret, and service account fields" />
</Frame>
...
Use the sections below the claim list to map Google claim values to UnifAI roles, teams, and business units.
...
<Frame caption="Attribute Mapping - configure role, team, and business unit rules based on the claims Google sends.">
  <img src="/media/user-provisioning/attribute-to-entity-mapping.png" alt="UnifAI Attribute Mapping screen showing role, team, and business unit mapping rules" />
</Frame>

<Frame caption="Custom attribute mapping - map any custom claim from Google to a UnifAI role, team, or business unit.">
  <img src="/media/user-provisioning/custom-attribute-mapping.png" alt="UnifAI custom attribute mapping configuration" />
</Frame>
...
UnifAI uses the Directory API credentials from this setup to sync users in the background every **24 hours**. During that sync, UnifAI reconciles imported users, role mappings, team mappings, and business-unit mappings from Google Workspace.

Every **15 minutes**, UnifAI also refreshes active OIDC sessions. If a session cannot be refreshed, UnifAI checks with Google whether the user is still active; if Google reports the user as inactive, UnifAI decommissions that user locally.
...
**`admin_policy_enforced` or `access_denied` during OAuth** - the Workspace admin has blocked third-party OAuth apps. In the Admin Console, go to **Security → Access and data control → API controls** and allow the UnifAI OAuth client.
