...
Data Access Control (DAC) decides **which rows** a user is allowed to see and act on inside UnifAI Enterprise. It complements Role-Based Access Control: RBAC controls **what operations** a user can perform; DAC scopes the **result set** of those operations to the rows the user is entitled to view.
...
Every role in UnifAI Enterprise has a data access scope with one of three values:
...
DAC tracks ownership on every row created through the dashboard or API. When a user makes a request, UnifAI determines who is asking, looks up the membership context (team, business unit, customer), and filters every query so the result set matches the role's scope.
...
UnifAI resolves the caller's identity at two entry points:
...
- **Inference path** - The request carries no dashboard session, only a credential. UnifAI accepts three credential types and resolves each to an identity, after which the same scope logic applies:
