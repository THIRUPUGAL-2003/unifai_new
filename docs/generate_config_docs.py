#!/usr/bin/env python3
"""Customer-clear UnifAI Configuration PDF + DOCX (what each setting means)."""
...
MD = ROOT / "docs" / "CONFIGURATION.md"
PDF = ROOT / "docs" / "CONFIGURATION.pdf"
DOCX = ROOT / "docs" / "CONFIGURATION.docx"
...
def parse(text: str):
    ...
    while i < len(lines):
        ...
        if re.match(r"^\d+\.\s+", line):
            blocks.append(("num", re.sub(r"^\d+\.\s+", "", line.strip())))
            i += 1
            continue
        if not line.strip():
            i += 1
            continue
        if line.startswith("That is the configuration"):
...
def styles():
    ...
    return {
        ...
        "code": ParagraphStyle(
            ...
            textColor=RED, spaceBefore=6, spaceAfter=8, leftIndent=8,
        ),
...
def make_table(rows, st):
    ...
    if n == 2:
        ...
        widths = [total * 0.26, total * 0.42, total * 0.32]
    else:
        widths = [total / n] * n
    data = []
...
def build_story(blocks, st):
    ...
    story.append(Paragraph("UnifAI Configuration", st["cover"]))
...
def footer(c, doc):
    ...
    c.drawString(16 * mm, 7 * mm, "UnifAI Configuration")
...
def build_docx(blocks):
    ...
    r = p.add_run("UnifAI Configuration")
    ...
    for kind, data in blocks:
        ...
        if kind == "h2":
            ...
            p = doc.add_paragraph("\n".join(data))
            for run in p.runs:
                set_run(run, size=9, bold=True, color=RGBColor(220, 38, 38), name="Consolas")
        elif kind == "table":
