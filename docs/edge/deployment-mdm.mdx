...
description: "Roll UnifAI Edge out to every machine silently through your device management platform. Edge ships with a managed configuration so Jamf, Intune, and Kandji can install and point it at your UnifAI with no per-user setup."
icon: "cloud-arrow-up"
---

UnifAI Edge is built for fleet-wide deployment. Rather than asking users to download and configure anything, you push Edge to every machine through your existing device management platform with a managed configuration that points it at your organization's UnifAI. The first time it runs, a user signs in once, and from then on it is invisible.
...
Your MDM delivers a small managed configuration that tells Edge which UnifAI to connect to. Because this is delivered centrally, machines arrive pre-pointed at your organization and users never have to enter a server address or paste a key.

<CardGroup cols={2}>
  <Card title="Pre-pointed at your UnifAI" icon="location-dot">
...
<Steps>
  ...
  <Step title="Governance turns on">
    From that point on, all supported AI traffic on the machine is routed through UnifAI, with policies kept in sync automatically.
  </Step>
</Steps>

<Note>
After setup, Edge keeps its policies and configuration in sync with UnifAI on its own. Changes you make centrally - app policy, MCP allow and deny lists, routing - reach the fleet without revisiting individual machines.
</Note>
