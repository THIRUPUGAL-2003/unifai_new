...
description: "UnifAI Edge routes AI traffic from every app through your guardrails, so PII redaction, secrets detection, content safety, and policy enforcement apply everywhere - out of the box, with the profiles you already configured."
icon: "road-barrier"
---

Because UnifAI Edge routes AI traffic through your UnifAI, every [guardrail](/enterprise/guardrails) you have configured applies automatically to the AI people use on their machines. There is nothing extra to set up on the endpoint: the same rules and profiles that protect your gateway traffic now protect prompts and responses from desktop apps, browser AI, and coding agents. Configure once, and it works across every supported app.

```mermaid
flowchart LR
    A[AI app on a machine] --> B[UnifAI Edge]
    B --> C[UnifAI guardrails<br/>PII, secrets, content safety, policy]
...
<Note>
Guardrails are configured in UnifAI using reusable **profiles** and **rules**. Edge does not change any of that - it simply brings more traffic under the same protection. To set up or adjust guardrails, go to [Guardrails configuration](/enterprise/guardrails).
</Note>
...
<Frame>
  <img src="/media/edge/edge-guardrail-chatgpt.png" alt="A guardrail applied to a ChatGPT web prompt through UnifAI Edge" />
</Frame>
...
<Frame>
  <img src="/media/edge/edge-guardrails-cowork.png" alt="A guardrail applied to AI activity in CoWork through UnifAI Edge" />
</Frame>
