...
<Tabs>
  <Tab title="NPX">
    ```bash
    npx -y @unifai/unifai --transport-version v1.6.2
    ```
  </Tab>
  <Tab title="Docker">
    ```bash
    docker pull unifai/unifai:v1.6.2
    docker run -p 8080:8080 unifai/unifai:v1.6.2
    ```
  </Tab>
</Tabs>

<Update label="UnifAI(HTTP)" description="1.6.2">
## ✨ Features
...
- [#4797](https://github.com/unifai/unifai/issues/4797) — [Bug]: configstore migration order — refresh_config_hash_after_mcp_external_server_url_removal (#139) selects dump_errors_in_console_logs before add_dump_errors_in_console_logs_column (#160) adds it
