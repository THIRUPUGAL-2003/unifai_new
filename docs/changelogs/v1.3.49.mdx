...
<Tabs>
  <Tab title="NPX">
    ```bash
    npx -y @unifai/unifai --transport-version v1.3.49
    ```
  </Tab>
  <Tab title="Docker">
    ```bash
    docker pull unifai/unifai:v1.3.49
    docker run -p 8080:8080 unifai/unifai:v1.3.49
    ```
  </Tab>
</Tabs>

<Update label="UnifAI(HTTP)" description="1.3.49">
- feat: add `x-uf-api-key` header to send requests with a key by name
- feat: parse `x-uf-eh-*` request headers as extra headers
