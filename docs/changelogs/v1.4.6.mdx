...
<Tabs>
  <Tab title="NPX">
    ```bash
    npx -y @unifai/unifai --transport-version v1.4.6
    ```
  </Tab>
  <Tab title="Docker">
    ```bash
    docker pull unifai/unifai:v1.4.6
    docker run -p 8080:8080 unifai/unifai:v1.4.6
    ```
  </Tab>
</Tabs>

<Update label="UnifAI(HTTP)" description="1.4.6">
...
- fix: **Bedrock: Stop reason mapping** - Now uses own `bedrockFinishReasonToUnifAI` map with Bedrock-specific stop reasons (`guardrail_intervened` -> `content_filter`, `content_filtered` -> `content_filter`)
