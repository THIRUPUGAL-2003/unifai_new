...
<Tabs>
  <Tab title="NPX">
    ```bash
    npx -y @unifai/unifai --transport-version v1.5.0-prerelease1
    ```
  </Tab>
  <Tab title="Docker">
    ```bash
    docker pull unifai/unifai:v1.5.0-prerelease1
    docker run -p 8080:8080 unifai/unifai:v1.5.0-prerelease1
    ```
  </Tab>
</Tabs>

<Update label="UnifAI(HTTP)" description="1.5.0-prerelease1">
...
- refactor: standardize empty array conventions in unifai. Empty array means no tools/keys are allowed, ["*"] means all tools/keys are allowed.
- feat: add support for request level extra headers in MCP tool execution.
- fix: add support for `x-uf-mcp-include-clients` and `x-uf-mcp-include-tools` request headers to filter MCP tools/list response when using unifai as an MCP gateway.
...
- feat: add UnifAIContextKeyMCPAddedTools to context to track MCP tools added to the request
- refactor: standardize empty array conventions in unifai. Empty array means deny all, ["*"] means allow all for models/tools/keys.
- feat: add support for request-level extra headers in MCP tool execution using UnifAIContextKeyMCPExtraHeaders key in context.
- fix: send back accumulated usage in MCP agent mode.
- feat: add StabilityAI provider support to Bedrock.
- fix: handle text, vtt, srt response formats in OpenAI transcription response.
