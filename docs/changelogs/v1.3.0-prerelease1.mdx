...
<Tabs>
  <Tab title="NPX">
    ```bash
    npx -y @unifai/unifai --transport-version v1.3.0-prerelease1
    ```
  </Tab>
  <Tab title="Docker">
    ```bash
    docker pull unifai/unifai:v1.3.0-prerelease1
    docker run -p 8080:8080 unifai/unifai:v1.3.0-prerelease1
    ```
  </Tab>
</Tabs>

<Update label="UnifAI(HTTP)" description="v1.3.0-prerelease1">
...
- Fix: Fixes UnifAI specific error logging in first party and third party logging plugins
...
- This release contains multiple breaking changes for UnifAI Core. These were necessary to ensure we incorporate responses without compromising on speed or architecture.
...
- Feat: Now UnifAI supports provider level fallbacks
...
- Fix: Captures UnifAI-specific errors in logs (e.g. provider not configured)
...
- Fix: Maxim plugin now captures UnifAI gateway specific errors.
