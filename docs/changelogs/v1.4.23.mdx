...
<Tabs>
  <Tab title="NPX">```bash npx -y @unifai/unifai --transport-version v1.4.23 ```</Tab>
  <Tab title="Docker">
    ```bash docker pull unifai/unifai:v1.4.23 docker run -p 8080:8080 unifai/unifai:v1.4.23 ```
  </Tab>
</Tabs>

<Update label="UnifAI(HTTP)" description="1.4.23">
## ✨ Features
...
- **MCP Tool Annotations** - Preserve MCP tool annotations (`title`, `readOnly`, `destructive`, `idempotent`, `openWorld`) in bidirectional conversion between MCP tools and UnifAI chat tools so agents can reason about tool behavior
