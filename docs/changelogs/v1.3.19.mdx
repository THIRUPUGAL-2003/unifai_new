...
<Tabs>
  <Tab title="NPX">
    ```bash
    npx -y @unifai/unifai --transport-version v1.3.19
    ```
  </Tab>
  <Tab title="Docker">
    ```bash
    docker pull unifai/unifai:v1.3.19
    docker run -p 8080:8080 unifai/unifai:v1.3.19
    ```
  </Tab>
</Tabs>

<Update label="UnifAI(HTTP)" description="v1.3.19">
...
[BREAKING] changed UnifAIContextKeySelectedKey to UnifAIContextKeySelectedKeyID
- feat: send model deployment back in response extra fields
- feat: add headers to MCP client config
