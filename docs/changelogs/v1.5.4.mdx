...
<Tabs>
  <Tab title="NPX">
    ```bash
    npx -y @unifai/unifai --transport-version v1.5.4
    ```
  </Tab>
  <Tab title="Docker">
    ```bash
    docker pull unifai/unifai:v1.5.4
    docker run -p 8080:8080 unifai/unifai:v1.5.4
    ```
  </Tab>
</Tabs>

<Update label="UnifAI(HTTP)" description="1.5.4">
## ✨ Features
...
- **Vertex Service Tier** — Map the Vertex traffic type to the correct UnifAI service tier (#3662)
...
- fix: map Vertex traffic type to UnifAI service tier (#3662)
