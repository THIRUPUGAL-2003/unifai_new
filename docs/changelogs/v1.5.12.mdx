...
<Tabs>
  <Tab title="NPX">
    ```bash
    npx -y @unifai/unifai --transport-version v1.5.12
    ```
  </Tab>
  <Tab title="Docker">
    ```bash
    docker pull unifai/unifai:v1.5.12
    docker run -p 8080:8080 unifai/unifai:v1.5.12
    ```
  </Tab>
</Tabs>

<Update label="UnifAI(HTTP)" description="1.5.12">
<Warning>
...
- feat: tagged MCP health check requests via UnifAIContextKeyMCPHealthCheck
...
- feat: enforce VK tool-grant boundary on caller-provided x-uf-mcp-include-* headers
