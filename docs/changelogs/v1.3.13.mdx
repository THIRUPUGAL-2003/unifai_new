...
<Tabs>
  <Tab title="NPX">
    ```bash
    npx -y @unifai/unifai --transport-version v1.3.13
    ```
  </Tab>
  <Tab title="Docker">
    ```bash
    docker pull unifai/unifai:v1.3.13
    docker run -p 8080:8080 unifai/unifai:v1.3.13
    ```
  </Tab>
</Tabs>

<Update label="UnifAI(HTTP)" description="v1.3.13">
...
- feat: provider config hot reloading added (no need to restart UnifAI after updating provider configs now)
