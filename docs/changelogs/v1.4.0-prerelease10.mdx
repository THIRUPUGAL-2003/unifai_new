...
<Tabs>
  <Tab title="NPX">
    ```bash
    npx -y @unifai/unifai --transport-version v1.4.0-prerelease10
    ```
  </Tab>
  <Tab title="Docker">
    ```bash
    docker pull unifai/unifai:v1.4.0-prerelease10
    docker run -p 8080:8080 unifai/unifai:v1.4.0-prerelease10
    ```
  </Tab>
</Tabs>

<Update label="UnifAI(HTTP)" description="1.4.0-prerelease10">
...
- feat: add raw request data to unifai error responses
