...
<Tabs>
  <Tab title="NPX">
    ```bash
    npx -y @unifai/unifai --transport-version v1.3.37
    ```
  </Tab>
  <Tab title="Docker">
    ```bash
    docker pull unifai/unifai:v1.3.37
    docker run -p 8080:8080 unifai/unifai:v1.3.37
    ```
  </Tab>
</Tabs>

<Update label="UnifAI(HTTP)" description="1.3.37">
- feat: pydantic SDK support
- feat: bedrock SDK support
- feat: adds versioning support for plugins
- **breaking change**: plugins now accept *schemas.UnifAIContext instead of *context.Context
