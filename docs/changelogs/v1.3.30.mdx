...
<Tabs>
  <Tab title="NPX">
    ```bash
    npx -y @unifai/unifai --transport-version v1.3.30
    ```
  </Tab>
  <Tab title="Docker">
    ```bash
    docker pull unifai/unifai:v1.3.30
    docker run -p 8080:8080 unifai/unifai:v1.3.30
    ```
  </Tab>
</Tabs>

<Update label="UnifAI(HTTP)" description="1.3.30">
- feat: adds migration for missing provider column in key table

<Warning>
"keys" in "provider_config" in `config.json` file requires unique name. If there is any collision, UnifAI wont be able to boot.
</Warning>
