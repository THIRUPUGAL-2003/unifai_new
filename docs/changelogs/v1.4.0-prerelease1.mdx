...
<Tabs>
  <Tab title="NPX">
    ```bash
    npx -y @unifai/unifai --transport-version v1.4.0-prerelease1
    ```
  </Tab>
  <Tab title="Docker">
    ```bash
    docker pull unifai/unifai:v1.4.0-prerelease1
    docker run -p 8080:8080 unifai/unifai:v1.4.0-prerelease1
    ```
  </Tab>
</Tabs>

<Update label="UnifAI(HTTP)" description="1.4.0-prerelease1">
...
- **Plugin Interface: TransportInterceptor removed, replaced with HTTPTransportMiddleware**
  ...
  **Migration summary:**
  ```
  // v1.3.x (removed)
  TransportInterceptor(ctx *UnifAIContext, url string, headers map[string]string, body map[string]any) (map[string]string, map[string]any, error)

  // v1.4.x+ (new)
  HTTPTransportMiddleware() UnifAIHTTPMiddleware
  // where UnifAIHTTPMiddleware = func(next fasthttp.RequestHandler) fasthttp.RequestHandler
  ```
...
- **DynamicPlugin: TransportInterceptor replaced with HTTPTransportMiddleware**
  ...
  **Old symbol lookup (removed in framework v1.2.0):**
  ...
  // Expected: func(ctx *UnifAIContext, url string, headers map[string]string, body map[string]any) (map[string]string, map[string]any, error)
  ```

  **New symbol lookup (framework v1.2.0+):**
  ```go
  plugin.Lookup("HTTPTransportMiddleware")
  // Expected: func() UnifAIHTTPMiddleware
  ```
...
- **Plugin Interface: TransportInterceptor replaced with HTTPTransportMiddleware**
  ...
  **What changed:**
  ...
  - New: `HTTPTransportMiddleware() UnifAIHTTPMiddleware`
