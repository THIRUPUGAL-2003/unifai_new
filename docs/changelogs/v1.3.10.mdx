...
<Tabs>
  <Tab title="NPX">
    ```bash
    npx -y @unifai/unifai --transport-version v1.3.10
    ```
  </Tab>
  <Tab title="Docker">
    ```bash
    docker pull unifai/unifai:v1.3.10
    docker run -p 8080:8080 unifai/unifai:v1.3.10
    ```
  </Tab>
</Tabs>

<Update label="UnifAI(HTTP)" description="v1.3.10">
...
- bug: fixed embedding request not being handled in `GetExtraFields()` method of `UnifAIResponse`
- fix: added latency calculation for vertex native requests
