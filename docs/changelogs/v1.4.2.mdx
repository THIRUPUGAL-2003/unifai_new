...
<Tabs>
  <Tab title="NPX">
    ```bash
    npx -y @unifai/unifai --transport-version v1.4.2
    ```
  </Tab>
  <Tab title="Docker">
    ```bash
    docker pull unifai/unifai:v1.4.2
    docker run -p 8080:8080 unifai/unifai:v1.4.2
    ```
  </Tab>
</Tabs>

<Update label="UnifAI(HTTP)" description="1.4.2">
...
- feat: support for listing models from specific provider or all providers via a header flag x-uf-list-models-provider from integrations.
...
- fix: call enrich error in vertex to return raw request and response in unifai errors
