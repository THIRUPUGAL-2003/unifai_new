...
<Update label="UnifAI Enterprise" description="v1.4.0">
...
- **Prompts Plugin Missing Header** - Prompts plugin's deployment resolver no longer returns an error when `x-uf-prompt-id` is absent - missing header is treated as "plugin not needed for this request" instead of a failure.
...
- **SGL Extra Params Passthrough (OSS)** - SGL provider now sets `UnifAIContextKeyPassthroughExtraParams`, so SGLang vLLM-style extra-body params (`chat_template_kwargs`, `guided_json`, `guided_regex`, `separate_reasoning`) are no longer dropped.
...
- **`UnifAIError` String Output (OSS)** - Added `String()` method so logged errors render as JSON instead of decimal byte dumps.
...
require (
	...
	github.com/unifai/unifai/core v1.5.8-0.20260501201305-734f02d4cd7d
	github.com/unifai/unifai/framework v1.3.8-0.20260501201305-734f02d4cd7d
	github.com/unifai/unifai/plugins/governance v1.5.8-0.20260501201305-734f02d4cd7d
	github.com/unifai/unifai/plugins/prompts v1.0.8-0.20260501201305-734f02d4cd7d
	github.com/unifai/unifai/transports v1.5.0-prerelease8.0.20260501201305-734f02d4cd7d
