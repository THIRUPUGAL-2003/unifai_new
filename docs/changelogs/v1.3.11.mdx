...
<Tabs>
  <Tab title="NPX">
    ```bash
    npx -y @unifai/unifai --transport-version v1.3.11
    ```
  </Tab>
  <Tab title="Docker">
    ```bash
    docker pull unifai/unifai:v1.3.11
    docker run -p 8080:8080 unifai/unifai:v1.3.11
    ```
  </Tab>
</Tabs>

<Update label="UnifAI(HTTP)" description="v1.3.11">
...
- feat: enabled provider tracking in UnifAI core for API exposure
