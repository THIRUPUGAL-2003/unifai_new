...
<Tabs>
  <Tab title="NPX">
    ```bash
    npx -y @unifai/unifai --transport-version v1.3.56
    ```
  </Tab>
  <Tab title="Docker">
    ```bash
    docker pull unifai/unifai:v1.3.56
    docker run -p 8080:8080 unifai/unifai:v1.3.56
    ```
  </Tab>
</Tabs>

<Update label="UnifAI(HTTP)" description="1.3.56">
- fix: handles overwriting of key values in case of collision between config.json and db
- fix: fixes support for referring allowed keys in virtual_keys when setting up UnifAI using config.json
