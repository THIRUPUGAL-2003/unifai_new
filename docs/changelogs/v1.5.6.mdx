...
<Tabs>
  <Tab title="NPX">
    ```bash
    npx -y @unifai/unifai --transport-version v1.5.6
    ```
  </Tab>
  <Tab title="Docker">
    ```bash
    docker pull unifai/unifai:v1.5.6
    docker run -p 8080:8080 unifai/unifai:v1.5.6
    ```
  </Tab>
</Tabs>

<Update label="UnifAI(HTTP)" description="1.5.6">
## ✨ Features
...
- **User-Mode Flows** - Gate user-mode flows on caller user_id, skip temp token mint, and UnifAI flow/credential kind filtering for pending flows (#3841, #3859)
...
- feat: add triggered_rotation to KeyAttemptRecord and tighten unifai_key_rotation_events_total semantics (#3430)
...
- feat: tighten unifai_key_rotation_events_total semantics with triggered_rotation (#3430, #3491)
- feat: add provider cache and semantic cache attributes in metrics export (#3816)
- chore: upgraded core to v1.5.14 and framework to v1.3.14
