...
<Tabs>
  <Tab title="NPX">
    ```bash
    npx -y @unifai/unifai --transport-version v1.4.8
    ```
  </Tab>
  <Tab title="Docker">
    ```bash
    docker pull unifai/unifai:v1.4.8
    docker run -p 8080:8080 unifai/unifai:v1.4.8
    ```
  </Tab>
</Tabs>

<Update label="UnifAI(HTTP)" description="1.4.8">
...
- feat: added support for capturing configured logging headers and x-uf-lh-* prefixed headers into log metadata for both LLM and MCP requests
