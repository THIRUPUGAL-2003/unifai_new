...
<Update label="UnifAI Helm" description="v2.1.22">
...
- Added `calendar_aligned` to `unifai.governance.modelConfigs[]`. When `true`, the config's budget reset windows snap to calendar boundaries rather than rolling windows; `virtual_key`-scoped configs inherit the virtual key's setting. Default `false`.
