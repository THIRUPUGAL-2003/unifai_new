...
<Tabs>
  <Tab title="NPX">
    ```bash
    npx -y @unifai/unifai --transport-version v1.5.14
    ```
  </Tab>
  <Tab title="Docker">
    ```bash
    docker pull unifai/unifai:v1.5.14
    docker run -p 8080:8080 unifai/unifai:v1.5.14
    ```
  </Tab>
</Tabs>

<Update label="UnifAI(HTTP)" description="1.5.14">
## ✨ Features
...
- **Routing-Pinned Key ID** — Commit the routing-pinned key ID to the reserved `UnifAIContextKeyAPIKeyID` after `PreRequestHook` unblock.
