...
<Tabs>
  <Tab title="NPX">
    ```bash
    npx -y @unifai/unifai --transport-version v1.5.1
    ```
  </Tab>
  <Tab title="Docker">
    ```bash
    docker pull unifai/unifai:v1.5.1
    docker run -p 8080:8080 unifai/unifai:v1.5.1
    ```
  </Tab>
</Tabs>

<Update label="UnifAI(HTTP)" description="1.5.1">
## ✨ Features
...
- **Configurable Matview Refresh** - Added `matviewRefreshInterval` config option and bumped the unifai chart to 2.1.15 (#3329, #3386)
...
- **x-unifai-* Response Headers** - Surface routed identity (provider, model, key alias, etc.) as `x-unifai-*` response headers (#3280) (thanks [@Edward-Upton](https://github.com/Edward-Upton)!)
