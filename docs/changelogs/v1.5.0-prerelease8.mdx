...
<Tabs>
  <Tab title="NPX">
    ```bash
    npx -y @unifai/unifai --transport-version v1.5.0-prerelease8
    ```
  </Tab>
  <Tab title="Docker">
    ```bash
    docker pull unifai/unifai:v1.5.0-prerelease8
    docker run -p 8080:8080 unifai/unifai:v1.5.0-prerelease8
    ```
  </Tab>
</Tabs>

<Update label="UnifAI(HTTP)" description="1.5.0-prerelease8">
## ✨ Features
...
- **SGL Extra Params Passthrough** - SGL provider now sets `UnifAIContextKeyPassthroughExtraParams`, so SGLang vLLM-style extra-body params (`chat_template_kwargs`, `guided_json`, `guided_regex`, `separate_reasoning`) are no longer silently dropped (thanks [@hensapir](https://github.com/hensapir)!)
...
- **`UnifAIError` String Output** - Added `String()` method so logged errors render as JSON instead of decimal byte dumps
...
- feat: add `extractJSONSchemaObject` helper in Bedrock utils to UnifAI composite and decomposed JSON schema fields for OpenAI-compat structured output
...
- fix: add `String()` method on `UnifAIError` so logged errors no longer dump raw bytes as decimal
