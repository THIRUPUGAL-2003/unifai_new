...
<Tabs>
  <Tab title="NPX">
    ```bash
    npx -y @unifai/unifai --transport-version v1.5.3
    ```
  </Tab>
  <Tab title="Docker">
    ```bash
    docker pull unifai/unifai:v1.5.3
    docker run -p 8080:8080 unifai/unifai:v1.5.3
    ```
  </Tab>
</Tabs>

<Update label="UnifAI(HTTP)" description="1.5.3">
## ✨ Features
...
- **DB Safety** - Replace unsafe inline jsonb cast with a `unifai_safe_jsonb` PL/pgSQL helper (#3407)
...
- fix: preserve OpenAI responses stream metadata (#3528, #3530) (thanks [@etnperlong](https://github.com/etnperlong)!)
- fix: handle context cancel before read errors in streaming (#3522)
- fix: replace unsafe inline jsonb cast with `unifai_safe_jsonb` PL/pgSQL helper (#3407)
- chore: dependency and AWS SDK security updates (#3461)
