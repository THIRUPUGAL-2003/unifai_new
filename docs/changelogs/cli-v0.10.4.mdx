...
<Update label="UnifAI CLI" description="v0.10.4">
- feat: adds Codex preLaunch hook to merge UnifAI virtual key, base URL, and model into `~/.codex/auth.json` and `~/.codex/config.toml` so stale Codex credentials don't shadow UnifAI-injected env vars
