...
<Tabs>
  <Tab title="NPX">
    ```bash
    npx -y @unifai/unifai --transport-version v1.4.10
    ```
  </Tab>
  <Tab title="Docker">
    ```bash
    docker pull unifai/unifai:v1.4.10
    docker run -p 8080:8080 unifai/unifai:v1.4.10
    ```
  </Tab>
</Tabs>

<Update label="UnifAI(HTTP)" description="1.4.10">
## ✨ Features
...
- **Logprobs JSON Tag** - Fixed logprobs JSON tag in UnifAIResponseChoice (thanks [@robechun](https://github.com/robechun)!)
...
- fix: fixed logprobs JSON tag in UnifAIResponseChoice (thanks [@robechun](https://github.com/robechun)!)
