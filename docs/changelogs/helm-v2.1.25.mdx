...
<Update label="UnifAI Helm" description="v2.1.25">

## Changelog

- `evaluation_mode` for guardrail rules (`unifai.guardrails.rules[].evaluation_mode`): `bundled` (default, evaluate all turns together) or `per_turn` (evaluate each turn independently).
- `group_traces_by_session` for the OTEL and Datadog plugin configs. When `true`, requests sharing the same `x-uf-session-id` header are grouped into a single trace; an inbound W3C `traceparent` always takes priority. Defaults to `false`.
