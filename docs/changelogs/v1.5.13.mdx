...
<Tabs>
  <Tab title="NPX">
    ```bash
    npx -y @unifai/unifai --transport-version v1.5.13
    ```
  </Tab>
  <Tab title="Docker">
    ```bash
    docker pull unifai/unifai:v1.5.13
    docker run -p 8080:8080 unifai/unifai:v1.5.13
    ```
  </Tab>
</Tabs>

<Update label="UnifAI(HTTP)" description="1.5.13">
## 🐞 Fixed
