...
<Update label="UnifAI CLI" description="v0.10.0">
- feat: tabbed multiplexer for running multiple coding-agent sessions in a single terminal
- feat: self-update flow with `unifai update` command and background version checks
- feat: `unifai version` subcommand
...
- feat: npx installer rewrite with persistent install to ~/.unifai/bin/ and automatic shell PATH setup
- feat: Claude Code simple terminal mode (CLAUDE_CODE_SIMPLE=1) for tab compatibility
- fix: opencode harness model reference format and provider config (unifai/ prefix, dedicated provider)
