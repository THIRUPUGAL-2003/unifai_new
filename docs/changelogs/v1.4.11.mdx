...
<Tabs>
  <Tab title="NPX">
    ```bash
    npx -y @unifai/unifai --transport-version v1.4.11
    ```
  </Tab>
  <Tab title="Docker">
    ```bash
    docker pull unifai/unifai:v1.4.11
    docker run -p 8080:8080 unifai/unifai:v1.4.11
    ```
  </Tab>
</Tabs>

<Update label="UnifAI(HTTP)" description="1.4.11">
## ✨ Features
...
<Danger>
**Breaking change**: We have enforced all security headers to UnifAI based on recent pentest feedback. If you face any issues please create an issue on Github or reach out to us on Discord.
</Danger>
