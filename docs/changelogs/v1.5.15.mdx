...
<Tabs>
  <Tab title="NPX">
    ```bash
    npx -y @unifai/unifai --transport-version v1.5.15
    ```
  </Tab>
  <Tab title="Docker">
    ```bash
    docker pull unifai/unifai:v1.5.15
    docker run -p 8080:8080 unifai/unifai:v1.5.15
    ```
  </Tab>
</Tabs>

<Update label="UnifAI(HTTP)" description="1.5.15">
## ✨ Features

- **pprof Profiling Server** - Optional runtime profiling server gated by `UNIFAI_PPROF_PORT`, with env-tunable block/mutex sampling rates and graceful shutdown alongside the main server.
