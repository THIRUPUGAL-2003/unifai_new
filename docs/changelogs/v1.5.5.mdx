...
<Tabs>
  <Tab title="NPX">
    ```bash
    npx -y @unifai/unifai --transport-version v1.5.5
    ```
  </Tab>
  <Tab title="Docker">
    ```bash
    docker pull unifai/unifai:v1.5.5
    docker run -p 8080:8080 unifai/unifai:v1.5.5
    ```
  </Tab>
</Tabs>

<Update label="UnifAI(HTTP)" description="1.5.5">
## ✨ Features
...
- **OTel Extra Header Forwarding** — `x-uf-eh-*` extra headers forwarded to upstream providers are now also emitted on the request span under `gen_ai.request.extra_header.*` for end-to-end tracing (#3730)
- **OTel Semantic Conventions** — Aligned OTel attribute keys with the OpenTelemetry GenAI spec (canonical `gen_ai.*` and new `unifai.*` attributes); legacy attributes are retained in parallel to avoid breaking existing dashboards (#3732)
...
- **Stream Cancellation Race** — Set `UnifAIContextKeyConnectionClosed` before closing the stream and short-circuit `idleTimeoutReader.Read` when the connection is already closed to avoid panics and hangs on cancellation (#3733)
...
- feat: forward `x-uf-eh-*` extra headers as `gen_ai.request.extra_header.*` span attributes (#3730)
...
- fix: set `UnifAIContextKeyConnectionClosed` before closing stream and short-circuit `idleTimeoutReader.Read` on closed connections (#3733)
