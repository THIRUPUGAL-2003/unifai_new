...
<Tabs>
  <Tab title="NPX">
    ```bash
    npx -y @unifai/unifai --transport-version v1.3.45
    ```
  </Tab>
  <Tab title="Docker">
    ```bash
    docker pull unifai/unifai:v1.3.45
    docker run -p 8080:8080 unifai/unifai:v1.3.45
    ```
  </Tab>
</Tabs>

<Update label="UnifAI(HTTP)" description="1.3.45">
