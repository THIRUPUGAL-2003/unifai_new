...
description: "Step-by-step guide to benchmark UnifAI in your own environment using the official benchmarking tool."
icon: "stopwatch"
---

## Overview

Want to see UnifAI's performance in your specific environment? The [**UnifAI Benchmarking Repository**](https://github.com/unifai/unifai-benchmarking) provides everything you need to conduct comprehensive performance tests tailored to your infrastructure and workload requirements.
...
- **Different Configurations** - Compare various UnifAI settings
...
- **[mocker](https://github.com/unifai/unifai-benchmarking/tree/main/mocker)** — a mock LLM provider server with configurable latency, failures, and rate limits. Point your gateways at it to measure pure gateway overhead with zero API costs.
- **[hitter](https://github.com/unifai/unifai-benchmarking/tree/main/hitter)** — a load generator for stress-testing a single UnifAI deployment with realistic multi-model/streaming traffic.
...
- **UnifAI instance** running and accessible
- **Target providers** configured in UnifAI (real providers, or the [mocker](https://github.com/unifai/unifai-benchmarking/tree/main/mocker) for cost-free runs)
- **Network access** between benchmark tool and UnifAI
...
```bash
git clone https://github.com/unifai/unifai-benchmarking.git
cd unifai-benchmarking
```
...
To compare against other gateways, add their port variables too — the [repo README](https://github.com/unifai/unifai-benchmarking#readme) lists every supported gateway and its `.env` variable.
...
./benchmark -provider unifai -rate 500

# Custom benchmark: 1000 RPS for 30 seconds
./benchmark -provider unifai -rate 1000 -duration 30 -output my_results.json
```
...
| `-provider <name>` | ❌ | Gateway to benchmark: `unifai`, `openai`, or another supported gateway (full list in the [repo README](https://github.com/unifai/unifai-benchmarking#readme)); empty runs all | None (all) |
...
```bash
./benchmark -provider unifai -rate 1000 -duration 60 -output basic_test.json
```
...
```bash
./benchmark -provider unifai -rate 5000 -duration 120 -output stress_test.json
```
...
```bash
./benchmark -provider unifai -rate 500 -duration 60 -big-payload -output large_payload.json
```
...
```bash
./benchmark -provider unifai -rate 1000 -duration 1800 -timeout 2100 -output endurance_test.json
```
...
```bash
./benchmark -provider unifai -users 500 -duration 600 -ramp-up -ramp-up-duration 120 -output rampup_test.json
```
...
Compare UnifAI against other gateways (each gateway's port comes from `.env`):

```bash
# Test UnifAI
./benchmark -provider unifai -rate 1000 -duration 60 -output unifai_results.json
...
```bash
# Normal load
./benchmark -provider unifai -rate 1000 -duration 300 -output normal_load.json

# Burst load (simulate 5x spike)
./benchmark -provider unifai -rate 5000 -duration 60 -output burst_load.json
```
...
```bash
# Instance 1
UNIFAI_PORT=8080 ./benchmark -provider unifai -rate 2500 -duration 120 -output instance_1.json &

# Instance 2
UNIFAI_PORT=8081 ./benchmark -provider unifai -rate 2500 -duration 120 -output instance_2.json &
...
```bash
./benchmark -provider unifai -request-type embedding -path embeddings \
...
./benchmark -provider unifai -rate 1000 -duration 300 -output "$OUTPUT_DIR/standard.json"
./benchmark -provider unifai -rate 3000 -duration 180 -output "$OUTPUT_DIR/high_load.json"
./benchmark -provider unifai -rate 500 -duration 600 -big-payload -output "$OUTPUT_DIR/large_payload.json"
...
**Connection Refused:**
```bash
# Check if UnifAI is running
curl http://localhost:8080/health

# Verify port configuration
netstat -an | grep 8080
```
- Check the provider's port (e.g. `UNIFAI_PORT`) is defined in the `.env` file at the repo root.
...
- Point the gateway at the repo's [mock provider](https://github.com/unifai/unifai-benchmarking/tree/main/mocker) to eliminate upstream variability
...
**Ready to benchmark? Clone the [repository](https://github.com/unifai/unifai-benchmarking) and start testing!**
