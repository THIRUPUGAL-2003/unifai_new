...
Runware exposes a single synchronous endpoint (`https://api.runware.ai/v1`) that accepts an **array of tasks**. UnifAI wraps each request in this array envelope and maps the unified image and video schemas onto Runware's `imageInference` and `videoInference` task types. Image tasks return synchronously; video tasks are submitted asynchronously and polled to completion.
...
**Response**: [`UnifAIImageGenerationResponse`](https://github.com/unifai/unifai/blob/main/core/schemas/images.go) with `data[].url` or `data[].b64_json`.
...
**Response**: [`UnifAIVideoGenerationResponse`](https://github.com/unifai/unifai/blob/main/core/schemas/videos.go) with `id`, `status`, `videos[]`.

**Generation Modes** (auto-detected): **text-to-video** (`prompt` only) · **image-to-video** (`prompt` + `input_reference`).

**UnifAI statuses** (normalized): `queued` → `in_progress` → `completed` / `failed`. Runware's native statuses are `processing`, `success`, `error`.
...
- [UnifAI Runware Provider Source](https://github.com/unifai/unifai/tree/main/core/providers/runware)
