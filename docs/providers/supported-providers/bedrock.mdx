...
AWS Bedrock supports multiple model families (Claude, Nova, Mistral, Llama, Cohere, Titan) with significant structural differences from OpenAI's format. UnifAI performs extensive conversion including:
...
<Frame>
  <img
    ...
    alt="AWS Bedrock explicit credentials authentication setup in the UnifAI Web UI showing Access Key, Secret Key, Session Token, and Region fields"
  />
</Frame>
...
<Frame>
  <img
    ...
    alt="AWS Bedrock IAM Role authentication setup in the UnifAI Web UI showing optional Assume Role ARN, External ID, and Session Name fields"
  />
</Frame>
...
8. _(Optional)_ Set **Session Name**: identifies the session in CloudTrail (default: `unifai-session`)
9. Save
        ...
        return []schemas.Key{
            {
                ...
                BedrockKeyConfig: &schemas.BedrockKeyConfig{
                    ...
                    RoleSessionName: schemas.NewEnvVar("unifai-session"),          // optional
                },
            },
        }, nil
    }
...
<Frame>
  <img
    ...
    alt="AWS Bedrock API Key authentication setup in the UnifAI Web UI showing the API Key field and Region"
  />
</Frame>
...
| `session_name`  | No       | `unifai-session` | Session name for AssumeRole CloudTrail logs                                                                           |
...
For Claude models on Bedrock, UnifAI validates `anthropic-beta` headers and drops unsupported headers from the request.
...
| `response_format`       | → Structured output tool (see [Structured Output](#structured-output))            | Creates `uf_so_*` tool     |
...
```go
resp, err := client.ChatCompletionRequest(schemas.NewUnifAIContext(ctx, schemas.NoDeadline), &schemas.UnifAIChatRequest{
...
**Documentation**: See [UnifAI Reasoning Reference](/providers/reasoning)
...
- **Document support**: UnifAI's Bedrock conversion path currently supports PDF, CSV, DOC, DOCX, XLS, XLSX, HTML, TXT, MD formats

### Supported Chat Content Blocks

The Chat Completions request format is OpenAI-compatible for standard blocks (`type: "text"`, `type: "image_url"`, `type: "file"`). UnifAI converts these blocks to Bedrock Converse blocks internally. Bedrock-specific extensions (for example, standalone `cachePoint`) are also accepted when using the Bedrock provider.

| Block Type             | Request Shape (UnifAI/OpenAI)                                         | Bedrock Handling                                            | Support                         |
| ---------------------- | ---------------------------------------------------------------------- | ----------------------------------------------------------- | ------------------------------- |
...
- **Request shape (client → UnifAI)**: `type: "image_url"` with `image_url.url` set to a data URI/base64 image
- **Internal Bedrock shape (UnifAI → Bedrock)**: Converted to `image: { format, source: { bytes } }`
...
Formats currently supported by UnifAI's Bedrock document conversion path: `pdf`, `txt`, `md`, `html`, `csv`, `doc`, `docx`, `xls`, `xlsx`.
...
This standalone `cachePoint` block is a UnifAI/Bedrock extension (not OpenAI-standard) and should be used only with the Bedrock provider.
...
- **UnifAI → Bedrock**: Tool call arguments converted from JSON object to `input` field
- **Bedrock → UnifAI**: Tool use results with `toolUseId`, converted back to UnifAI format
...
// Bedrock conversion (internal)
{
  ...
  "toolChoice": {"type": "tool", "name": "uf_so_response"}
}
...
- Tool call with name `uf_so_*` is treated as structured output
...
- Each Bedrock streaming event → Multiple UnifAI chunks as needed
...
```go
resp, err := client.ResponsesRequest(schemas.NewUnifAIContext(ctx, schemas.NoDeadline), &schemas.UnifAIResponsesRequest{
    Provider: schemas.Bedrock,
    Model:    "anthropic.claude-3-5-sonnet-20241022-v2:0",
    Input:    messages,
...
| Parameter(UnifAI) | Transformation (Bedrock)                                                  |
| ------------------ | ------------------------------------------------------------------------- |
...
| Parameter(Bedrock) | Transformation (UnifAI) |
| ------------------ | ------------------------ |
...
```go
resp, err := client.ImageGenerationRequest(schemas.NewUnifAIContext(ctx, schemas.NoDeadline), &schemas.UnifAIImageGenerationRequest{
    Provider: schemas.Bedrock,
...
These models use a flat JSON body (not the nested Bedrock `taskType` structure). UnifAI detects them automatically - any model ID containing `"stability."` is converted via `ToStabilityAIImageGenerationRequest`.
...
- Uses the same response structure as image generation: `BedrockImageGenerationResponse` → `UnifAIImageGenerationResponse`
- Response includes:
...
UnifAI signs every Bedrock request with AWS Signature Version 4 (SigV4). Credentials are resolved in the following priority order, and STS AssumeRole can be layered on top of any of them.
...
Leave `access_key` and `secret_key` empty (or omit them). UnifAI calls AWS `LoadDefaultConfig` which automatically resolves credentials from the environment in this order:
...
| `session_name` | No            | `unifai-session` | Identifies the session in CloudTrail logs        |
...
| Status | UnifAI Error Type        | Notes                           |
| ------ | ------------------------- | ------------------------------- |
...
<Accordion title="Structured Output via Tool">
  **Severity**: Low **Behavior**: `response_format` converted to special
  `uf_so_*` tool **Impact**: Tool call count and structure changes internally
