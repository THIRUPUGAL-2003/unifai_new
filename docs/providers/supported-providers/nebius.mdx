...
Nebius is an **OpenAI-compatible provider** offering comprehensive API support. UnifAI delegates to the OpenAI implementation with standard parameter filtering. Key features:
...
- **Model & Prompt**: `unifaiReq.Model` → `req.Model` (pointer), `unifaiReq.Input.Prompt` → `req.Prompt` (pointer)
