...
The Hugging Face provider in UnifAI (`core/providers/huggingface`) implements a complex integration that supports multiple inference providers (like `hf-inference`, `fal-ai`, `cerebras`, `sambanova`, etc.) through a unified interface.
...
<Info>All Chat-supported models automatically support Responses(`v1/responses`) as well via UnifAI's internal conversion logic.</Info>
...
Requests use the model format `huggingface/<inference_provider>/<model_id>` so UnifAI can route each call to the correct Hugging Face inference backend.

## Model Aliases & Identification

Unlike standard providers where model IDs are direct strings (e.g., `gpt-4`), Hugging Face models in UnifAI are identified by a composite key to route requests to the correct inference backend.
...
This parsing logic is handled in `utils.go` and `models.go`, allowing UnifAI to dynamically route requests based on the model string.
...
- **Request Structure**:
  ...
  ```
- **Response**: Raw image bytes (PNG/JPEG), automatically base64-encoded in UnifAI response
...
- **Response**: Uses Nebius response format, converted to UnifAI format
...
Each provider's response is converted to UnifAI's unified `UnifAIImageGenerationResponse` format:

- **hf-inference**: Raw bytes → base64-encoded in `b64_json`
- **fal-ai**: `images[]` array → `ImageData[]` with `url` and/or `b64_json`
- **nebius**: Uses Nebius converter → UnifAI format
...
- **Error Handling**: Errors in SSE format are parsed and sent as `UnifAIError` chunks
...
```go
streamChan, err := client.ImageGenerationStreamRequest(schemas.NewUnifAIContext(ctx, schemas.NoDeadline), &schemas.UnifAIImageGenerationRequest{
    Provider: schemas.HuggingFace,
    Model:    "huggingface/fal-ai/fal-ai/flux/dev",
    Input: &schemas.ImageGenerationInput{
        Prompt: "A futuristic cityscape at sunset",
...
for stream := range streamChan {
    if stream.UnifAIImageGenerationStreamResponse != nil {
        chunk := stream.UnifAIImageGenerationStreamResponse
        if chunk.URL != "" {
            // Handle image URL
...
- **Image Conversion**: Each image in `unifaiReq.Input.Images` is converted to a base64 data URL:
...
├── chat.go              # Chat completion converters (UnifAI ↔ HF)
