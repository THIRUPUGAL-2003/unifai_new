...
Groq is an **OpenAI-compatible provider** offering the same API interface with identical parameter handling. UnifAI delegates most functionality to the OpenAI provider implementation with minimal modifications. Key features:
...
<Note>
**Text Completions (⚠️)**: Not supported natively by Groq. When enabled via `x-litellm-fallback` context, UnifAI internally converts text completion requests to chat completion requests, processes them through Chat Completions, and converts the response back to text completion format.
...
UnifAI converts from the internal `Reasoning` structure to `reasoning_effort` string.
