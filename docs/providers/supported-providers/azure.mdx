...
Azure is a cloud provider offering access to OpenAI and Anthropic models through the Azure OpenAI Service. UnifAI performs conversions including:
...
Leave `value` and all Entra ID fields empty. UnifAI calls `azidentity.NewDefaultAzureCredential(nil)`, which tries credential sources in this order:
...
<Frame>
  <img
    ...
    alt="Azure Default Credential authentication setup in the UnifAI Web UI showing Endpoint and API Version fields with no credential inputs"
  />
</Frame>
...
func (a *MyAccount) GetKeysForProvider(ctx *context.Context, provider schemas.ModelProvider) ([]schemas.Key, error) {
    ...
    case schemas.Azure:
        return []schemas.Key{
            {
                Value:  schemas.EnvVar{}, // Leave empty - UnifAI uses DefaultAzureCredential
                Models: []string{"*"},
                Weight: 1.0,
...
<Frame>
  <img
    ...
    alt="Azure Entra ID (Service Principal) authentication setup in the UnifAI Web UI showing Client ID, Client Secret, Tenant ID, and Endpoint fields"
  />
</Frame>
...
<Frame>
  <img
    ...
    alt="Azure API Key authentication setup in the UnifAI Web UI showing API Key, Endpoint, and API Version fields"
  />
</Frame>
...
For Anthropic models on Azure, UnifAI validates `anthropic-beta` headers and drops unsupported headers from the request. Azure supports most Anthropic beta features.
...
If no API key and no Entra ID credentials are provided, UnifAI automatically uses `DefaultAzureCredential`, which
...
When using OpenAI models (GPT-4, GPT-4 Turbo, GPT-3.5-Turbo, etc.), UnifAI passes through OpenAI-compatible parameters directly.
...
When using Anthropic models through Azure (Claude 3 family), UnifAI converts requests to Anthropic format.
...
- **Model & Prompt**: `unifaiReq.Model` → `req.Model` (mapped to deployment), `unifaiReq.Prompt` → `req.Prompt`
- **Parameters**: All other fields from `unifaiReq` are embedded directly into the request struct via struct embedding
...
- **Non-streaming**: Azure responses are unmarshaled directly into `UnifAIImageGenerationResponse` since UnifAI's response schema is a superset of OpenAI's format. All fields are passed through as-is.
...
- **Response Conversion**: Same as OpenAI - responses unmarshaled directly into `UnifAIImageGenerationResponse`
...
UnifAI routes the request to `{endpoint}/openai/v1/responses/compact` using the configured Azure deployment. Deployment mapping and authentication (API key, Entra ID, or managed identity) are applied automatically, identical to the Responses API.
...
**Note**: UnifAI uses the Azure OpenAI v1 API. No `api-version` query parameter is needed.
