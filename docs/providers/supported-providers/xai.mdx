...
xAI is an **OpenAI-compatible provider** powering the Grok family of models. UnifAI delegates to the OpenAI implementation with standard parameter filtering. Key features:
...
UnifAI converts from the internal `Reasoning` structure to xAI's `reasoning_effort` string format.
...
- **Parameters**: All fields from `unifaiReq` (`ImageGenerationParameters`) are embedded directly into the request struct via struct embedding
...
Responses are unmarshaled directly into `UnifAIImageGenerationResponse`.
...
xAI supports context compaction via its OpenAI-compatible `/v1/responses/compact` endpoint. UnifAI forwards the request to `https://api.x.ai/v1/responses/compact`.
