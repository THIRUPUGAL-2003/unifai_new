...
description: "Route requests through Databricks AI Gateway using Unified (MLflow) or Native (Anthropic Messages) APIs as custom providers in UnifAI"
icon: "database"
---

## Overview

[Databricks AI Gateway](https://docs.databricks.com/en/ai-gateway/index.html) (Beta) is a governance layer on top of Databricks Model Serving that adds rate limiting, usage tracking, and inference logging to your LLM endpoints. UnifAI connects to AI Gateway endpoints as custom providers.
...
In UnifAI, each API category maps to a different custom provider base format:

| API Category | UnifAI Base Format | Chat | Chat (stream) | Responses | Responses (stream) | Text | Coding Agents | List Models |
|--------------|---------------------|------|---------------|-----------|---------------------|------|---------------|-------------|
...
Before configuring UnifAI, you need:
...
AI Gateway exposes every endpoint at a `/mlflow/v1/chat/completions` path. Because this follows the OpenAI spec, UnifAI treats it as an OpenAI-compatible custom provider. The full endpoint URL looks like:

```
https://<workspace-id>.ai-gateway.cloud.databricks.com/mlflow/v1/chat/completions
```

You register only the base portion (`/mlflow`) as the custom provider's Base URL - UnifAI appends the standard `/v1/chat/completions` path automatically.

## Step 1: Create the Custom Provider

In UnifAI, go to **Models > Model Providers** in the sidebar. Click **Add New Provider** and select **Custom provider...** at the bottom of the dropdown.
...
AI Gateway exposes every endpoint at an `/anthropic/v1/messages` path that follows the Anthropic API spec. UnifAI treats this as an Anthropic-compatible custom provider. The full endpoint URL looks like:

```
https://<workspace-id>.ai-gateway.cloud.databricks.com/anthropic/v1/messages
```

You register only the base portion (`/anthropic`) as the custom provider's Base URL - UnifAI appends the standard Anthropic paths automatically.
...
The Native Anthropic Messages API works with Claude Code and other coding agents that depend on the Responses API. Point your coding agent at your UnifAI instance and use the `databricks-anthropic/<model>` prefix to route through your AI Gateway endpoint.
...
| **UnifAI Base Format** | `openai` | `anthropic` |
...
- [Custom Providers in UnifAI](/providers/custom-providers)
