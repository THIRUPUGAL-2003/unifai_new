...
Replicate is architecturally different from other providers in UnifAI. It uses a **prediction-based API** where every request creates a "prediction" that runs asynchronously. Each model on Replicate defines its own input schema, making it highly flexible but requiring model-specific parameter knowledge.
...
Set `replicate_key_config.use_deployments_endpoint` to `true` when requests should target `/v1/deployments/{owner}/{deployment}/predictions`. Use key `aliases` to map a UnifAI model name to the Replicate deployment path.
...
UnifAI uses sync mode with the `Prefer: wait` header if it is present in the request headers. The request blocks until the prediction completes or times out (default 60 seconds).
...
It is the default mode of Replicate predictions. UnifAI automatically polls the prediction URL every 2 seconds until completion.
...
**Streaming Flow:**
1. UnifAI sets `stream: true` in prediction input
2. Replicate returns `urls.stream` in initial response
3. UnifAI connects to stream URL and processes SSE events
...
ResponsesRequest → ReplicatePredictionRequest → ReplicatePredictionResponse → UnifAIResponsesResponse
```
...
**Important**: Different Replicate models expect input images in different fields. UnifAI automatically maps `input_images` to the correct field based on the model.
...
**Endpoint**: `/v1/images/edits` (UnifAI) → Replicate `/v1/predictions` or deployment predictions.

### Parameter Mapping

| UnifAI / Request | Replicate input |
|-------------------|-----------------|
...
```go
resp, err := client.ImageEditRequest(schemas.NewUnifAIContext(ctx, schemas.NoDeadline), &schemas.UnifAIImageEditRequest{
    Provider: schemas.Replicate,
...
Same as Image Generation: single URL → `data[0].url`, array of URLs → `data[i].url`, or data URIs. Response shape is `UnifAIImageGenerationResponse` with `data[].url` or `data[].b64_json`.
...
**Pagination**: Uses cursor-based pagination with `next` URL in response. UnifAI serializes this into the `after` cursor.
...
The most important feature for Replicate integration is **extra_params**. Parameters not in UnifAI's standard schema are flattened directly into the prediction `input` object.
...
**Impact**: UnifAI automatically maps to correct field based on model
...
**Response**: [`UnifAIVideoGenerationResponse`](https://github.com/unifai/unifai/blob/main/core/schemas/videos.go) - `id`, `status`, `model`, `videos[]`
...
- [UnifAI Replicate Provider Source](https://github.com/unifai/unifai/tree/main/core/providers/replicate)
