...
Google Gemini's API has different structure from OpenAI. UnifAI performs extensive conversion including:
...
- **Thinking/Reasoning** - Thinking configuration mapped to UnifAI reasoning structure
...
Gemini uses the `x-goog-api-key` header for standard endpoints. UnifAI automatically selects query-parameter authentication where Gemini requires it, such as Imagen and custom endpoint flows.
...
UnifAI automatically selects the appropriate authentication method based on the endpoint type.
...
- Computer tools auto-configured (if specified in UnifAI request)
...
```go
resp, err := client.TranscriptionRequest(schemas.NewUnifAIContext(ctx, schemas.NoDeadline), &schemas.UnifAITranscriptionRequest{
    Provider: schemas.Gemini,
    Model:    "gemini-2.0-flash",
...
**Response Mapping**:
- `embeddings[].values` → UnifAI embedding array
...
- **Model mapping**: `unifaiReq.Model` → `req.Model`, with `unifaiReq.Input.Prompt` → `req.Contents[0].Parts[0].Text`
- **Response modality**: Set by unifai internally to `generationConfig.responseModalities = ["IMAGE"]` to indicate image generation
...
- **Prompt**: `unifaiReq.Prompt` → `req.Instances[0].Prompt`
...
- **Model & Prompt**: `unifaiReq.Model` → `req.Model`, `unifaiReq.Input.Prompt` → `req.Contents[0].Parts[0].Text`
- **Images**: Each image in `unifaiReq.Input.Images` is converted to a `Part` with:
...
- **Reference Images**: Each image in `unifaiReq.Input.Images` is converted to `ReferenceImage` with:
...
**Response**: [`UnifAIVideoGenerationResponse`](https://github.com/unifai/unifai/blob/main/core/schemas/videos.go) - `id`, `status`, `videos[]`
...
UnifAI supports the following content modalities through Gemini:
