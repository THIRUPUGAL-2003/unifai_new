...
UnifAI implements both providers through a shared codebase, differing only in default base URL and provider key.
...
Configure either provider (or both) in UnifAI:
...
<Note>
**Provider-specific parameters**: Not all parameters are forwarded to the underlying model. UnifAI applies standard OpenAI normalization through the OpenAI-compatible handler. Model-specific features (e.g., thinking mode, prompt caching) depend on the underlying provider serving the model.
</Note>
...
Cache tracking depends on the underlying model. Some providers report `prompt_cache_hit_tokens` and `prompt_cache_miss_tokens` fields. UnifAI maps these into the standard `cached_tokens` field for pricing calculations.
...
UnifAI's custom error converter parses this format transparently. Error types include:
...
request.ToChatRequest() → ChatCompletion → ToUnifAIResponsesResponse()
```
...
OpenCode's model listing endpoint returns the available models for your gateway tier. Model IDs are prefixed with the provider key in UnifAI (e.g., `opencode-go/deepseek-v4-flash`).
...
OpenCode Zen pricing is publicly available at [opencode.ai/docs/zen](https://opencode.ai/docs/zen). OpenCode Go pricing is subscription-based. UnifAI does not yet include OpenCode entries in the central [pricing datasheet](https://getunifai.ai/datasheet).
...
**Severity**: Low
**Behavior**: OpenCode models do not yet have entries in the central UnifAI pricing datasheet. Cost tracking and context window awareness require local overrides.
...
**Severity**: Low
**Behavior**: OpenCode returns `{"type":"error","error":{"type":"...","message":"..."}}` instead of OpenAI's `{"error":{...}}` envelope. UnifAI's custom error converter handles this transparently.
