...
Mistral is an **OpenAI-compatible provider** with custom compatibility handling for specific features. UnifAI converts requests to Mistral's expected format while supporting their unique API endpoints. Key characteristics:
...
<Note>
  **Unsupported Operations** (❌): Text Completions, Speech (TTS), Files, and
  ...
  currently supported by UnifAI's Mistral integration (Mistral API supports
  image generation, but UnifAI has not yet implemented this feature). These
...
```go
// UnifAI allows specifying a specific tool
{
  "tool_choice": {
    "type": "function",
...
| Parameter                 | UnifAI      | Mistral         | Notes                   |
| ------------------------- | ------------ | --------------- | ----------------------- |
...
| Image Generation | Not yet implemented in UnifAI integration (Mistral API supports this) |
