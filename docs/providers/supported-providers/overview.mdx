---
title: "Overview"
description: "UnifAI supports multiple AI providers with consistent OpenAI-compatible response formats, enabling seamless provider switching without code changes."
icon: "circle-info"
---

## Overview

UnifAI supports a wide range of AI providers, all accessible through a consistent OpenAI-compatible interface. This standardization allows you to switch between providers without modifying your application code, as all responses follow the same structure regardless of the underlying provider.

UnifAI can also act as a provider-compatible gateway (for example, <u>[Anthropic](../../integrations/anthropic-sdk/overview)</u>, <u>[Google Gemini](../../integrations/genai-sdk/overview)</u>, <u>Cohere</u>, <u>[Bedrock](../../integrations/bedrock-sdk/overview)</u>, and others), exposing provider-specific endpoints so you can use existing provider SDKs or integrations with no code changes, see [What is an integration?](../../integrations/what-is-an-integration) for details.

## Provider Support Matrix

The following table summarizes which operations are supported by each provider via UnifAI’s unified interface.
...
- 🟡 Not supported by the downstream provider, but internally implemented by UnifAI as a fallback.
- ❌ Not supported by the downstream provider, hence not supported by UnifAI.
- ✅ Fully supported by the downstream provider, or internally implemented by UnifAI.

<Note>
  Some operations are not supported by the downstream provider, and their
  internal implementation in UnifAI is optional. 🟡 Like Text completions are
  not supported by Groq, but UnifAI can emulate them internally using the Chat
...
- "Responses" refers to the OpenAI-style Responses API (`/v1/responses`). Depending on the provider, UnifAI either uses a native responses endpoint or maps to an equivalent chat API.
...
- "Passthrough" refers to the Passthrough API, which forwards requests directly to the provider without UnifAI's transformation layer.

## Response Format

All providers return responses in the OpenAI-compatible format. UnifAI handles the translation between different provider-specific formats automatically.
...
UnifAI can optionally return the raw request that was sent to the provider and the raw response received back. This is useful for debugging, auditing, and understanding how UnifAI transforms requests between different provider formats.

**What's included:**

- **`raw_request`** - The exact request body (JSON/data structure) that was sent to the provider's API endpoint
- **`raw_response`** - The exact response body received from the provider (before UnifAI's normalization)
- **Provider transformation details** - Shows exactly how UnifAI converted your input to provider-specific format

**Example**: When you send a Chat Completions request with `max_completion_tokens` to Anthropic, UnifAI converts it to `max_tokens` in the raw request. Enabling raw request/response reveals this transformation.
...
- **Cost analysis** - See actual token counts before UnifAI's normalization
