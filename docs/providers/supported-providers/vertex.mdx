...
Vertex AI is Google's unified ML platform providing access to Google's Gemini models, Anthropic Claude models, and other third-party LLMs through a single API. UnifAI performs conversions including:
...
<Frame>
  <img
    ...
    alt="Google Vertex AI Service Account (JSON) authentication setup in the UnifAI Web UI showing Project ID, Region, and Auth Credentials fields"
  />
</Frame>
...
Leave `auth_credentials` empty. UnifAI calls `google.FindDefaultCredentials()` - Google's ADC library - which resolves credentials in this order:
...
<Frame>
  <img
    ...
    alt="Google Vertex AI Application Default Credentials setup in the UnifAI Web UI showing Project ID and Region fields with no credential inputs"
  />
</Frame>
...
<Frame>
  <img
    ...
    alt="Google Vertex AI API Key authentication setup in the UnifAI Web UI showing API Key, Project ID, Region, and Project Number fields"
  />
</Frame>
...
When running UnifAI on GKE, [Workload Identity Federation](https://cloud.google.com/kubernetes-engine/docs/concepts/workload-identity) (WIF) lets pods authenticate to Vertex AI without managing service account keys. The pod inherits an IAM identity through the Kubernetes ServiceAccount, and UnifAI picks it up automatically via [Application Default Credentials](#2-application-default-credentials).

**What you need:**

1. The GCP-side prerequisites below (API enabled, IAM service account, WIF binding)
2. A UnifAI Vertex key using **"Service Account (Attached)"** auth - see [Application Default Credentials](#2-application-default-credentials) for Web UI, API, config.json, and Go SDK setup. For Helm, see [Helm - Google Vertex AI](/deployment-guides/helm/providers#google-vertex-ai).
...
Replace `NAMESPACE` and `KSA_NAME` with your UnifAI pod's namespace and Kubernetes ServiceAccount name.
...
If deploying with the UnifAI Helm chart, set the annotation via `serviceAccount.annotations` in your values file - see [Helm - Google Vertex AI](/deployment-guides/helm/providers#google-vertex-ai) for the full example.

</Accordion>

### Verify

From inside the UnifAI pod, confirm the GKE metadata server returns a token:
...
Replace `NAMESPACE` and `POD_NAME` with your UnifAI namespace and any running UnifAI pod name (e.g., `unifai-0` for a StatefulSet or use `kubectl get pods -n NAMESPACE` to find it).

A JSON response with an `access_token` field confirms WIF is working. Then send a request through UnifAI to a Vertex model (e.g., `vertex/gemini-2.5-flash`) to verify end-to-end.
...
| Wrong project or region errors | UnifAI config mismatch | Check `project_id` and `region` in the Vertex key configuration |

---

## Beta Headers

For Anthropic models on Vertex AI, UnifAI validates `anthropic-beta` headers and drops unsupported headers from the request.
...
When using Google's Gemini models, UnifAI converts requests to Gemini's API format.
...
When using Anthropic models through Vertex AI, UnifAI converts requests to Anthropic's message format.
...
```go
resp, err := client.EmbeddingRequest(schemas.NewUnifAIContext(ctx, schemas.NoDeadline), &schemas.UnifAIEmbeddingRequest{
    Provider: schemas.Vertex,
    Model:    "text-embedding-004",
...
- **Gemini Models**: Responses converted using `GenerateContentResponse.ToUnifAIImageGenerationResponse()` - same as standard Gemini
- **Imagen Models**: Responses converted using `GeminiImagenResponse.ToUnifAIImageGenerationResponse()` - Imagen-specific format
...
To provide a complete model listing experience, UnifAI performs **multi-pass model discovery**:
...
Model listing is paginated automatically. If more than 100 models exist, `next_page_token` will be present. UnifAI handles pagination internally.
...
<Accordion title="Embeddings Precision Preservation">
  **Severity**: Low **Behavior**: Vertex returns float64 embeddings, and UnifAI
...
<Accordion title="List Models API Returns Only Custom Models">
  **Severity**: High **Behavior**: Vertex AI's List Models API only returns
  custom fine-tuned models, NOT foundation models **Impact**: UnifAI performs
