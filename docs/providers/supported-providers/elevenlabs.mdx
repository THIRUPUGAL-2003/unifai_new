...
ElevenLabs is a specialized audio provider for text-to-speech and speech-to-text operations. UnifAI performs conversions including:
...
For text-to-speech calls, the UnifAI `model` is the ElevenLabs voice ID unless you pass a provider-specific voice override in the request.
...
```go
resp, err := client.SpeechRequest(schemas.NewUnifAIContext(ctx, schemas.NoDeadline), &schemas.UnifAISpeechRequest{
