...
UnifAI dispatches each request to the correct surface automatically based on the model family — you address the provider the same way regardless: `bedrock_mantle/<model>`.
...
Leave `access_key` and `secret_key` empty and UnifAI inherits credentials from the AWS SDK default chain — IRSA (IAM Roles for Service Accounts), EC2 instance profile, or `AWS_*` environment variables. Only `region` is required.
...
Both surfaces are addressed identically — `bedrock_mantle/anthropic.claude-opus-4-8` (native Anthropic) and `bedrock_mantle/openai.gpt-oss-120b` (OpenAI-compatible) — UnifAI routes each to the right API.
