...
vLLM is an **OpenAI-compatible provider** for self-hosted inference. UnifAI delegates to the shared OpenAI provider implementation. Key characteristics:
...
2. Verify the server:
   ...
   ```
3. Use UnifAI with model prefix `vllm/<model_id>` (e.g. `vllm/meta-llama/Llama-3.2-1B-Instruct`).
...
UnifAI converts Responses API requests to Chat Completions and back:

```
UnifAIResponsesRequest
  → ToChatRequest()
  → ChatCompletion
...
vLLM supports reranking for pooling/cross-encoder reranker models. UnifAI sends requests to `/v1/rerank` and automatically falls back to `/rerank` when required by your vLLM deployment.
...
**Impact**: UnifAI normalizes these into standard error responses so clients see consistent error handling.  
</Accordion>
