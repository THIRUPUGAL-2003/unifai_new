...
Runway ML provides video generation via an asynchronous task-based API. UnifAI maps the unified video schema to Runway's task API and polls until completion.
...
**UnifAI statuses** (normalized): `queued` → `in_progress` → `completed` / `failed`

These values are the normalized view returned by UnifAI's API. Runway's native statuses are: `PENDING`, `THROTTLED`, `RUNNING`, `SUCCEEDED`, `FAILED`, `CANCELLED`.
