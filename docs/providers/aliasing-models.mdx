...
Model aliasing lets you decouple the model name your application sends from the identifier UnifAI actually uses when calling a provider. You can:

- Send `"best-model"` and have UnifAI resolve it to whatever model you've decided is best - without touching your application code
...
<Info>Static aliasing is available in **UnifAI v1.5.0-prerelease2 and above**.</Info>
...
2. UnifAI selects a key that supports `"best-model"` (alias names are treated as model identifiers for key selection and allowlists)
3. Before calling the provider, UnifAI resolves `"best-model"` → `"gpt-4o-2024-11-20"` using that key's `aliases` map
...
UnifAI rejects an aliases map that violates any of these:
...
Every response includes a `routing_info` block in `extra_fields` describing the routing decisions UnifAI made:
...
Setting `chain_rule: true` on a rule causes UnifAI to re-evaluate the full scope chain with the new provider/model as the new context. This lets you build layered alias resolution where a global rule establishes provider intent and a VK-scoped rule applies the final key selection.
