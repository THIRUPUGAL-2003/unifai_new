...
<Info>
UnifAI normalizes all provider-specific reasoning formats to a consistent OpenAI-compatible structure using `reasoning` in requests and `reasoning_details` in responses.
</Info>
...
OpenAI uses effort-based reasoning only. UnifAI applies priority logic:
...
```go
// UnifAI request with effort (native field)
chatReq := &schemas.UnifAIChatRequest{
	Provider: schemas.OpenAI,
	Model:    "gpt-4o",
	Input:    messages,
...
```json
// UnifAI Request (with max_tokens only)
{
...
// UnifAI estimates effort from max_tokens
...
// UnifAI converts to Anthropic format:
// {
//   "thinking": {
//     "type": "enabled",
//     "budget_tokens": 4096
...
```go
// After calling UnifAI Chat Completions with reasoning
resp, err := client.ChatCompletionRequest(schemas.NewUnifAIContext(ctx, schemas.NoDeadline), chatReq)
if err != nil {
  log.Fatal(err)
}
...
```go
// Using UnifAI Go SDK with Bedrock provider
chatReq := &schemas.UnifAIChatRequest{
...
// UnifAI converts to Bedrock format with reasoning_config
```
...
```go
// Using UnifAI Go SDK with Bedrock Nova
chatReq := &schemas.UnifAIChatRequest{
  Provider: schemas.Bedrock,
...
// UnifAI converts to Bedrock Nova format:
// reasoningConfig: {
//   type: "enabled",
//   maxReasoningEffort: "high"
// }
...
| UnifAI Effort | Nova Effort | Configuration |
|---|---|---|
...
<Warning>
**Important**: Only ONE parameter (`thinkingBudget` or `thinkingLevel`) should be sent to Gemini at a time. When both `reasoning.max_tokens` and `reasoning.effort` are provided in a UnifAI request, `max_tokens` takes priority and is converted to `thinkingBudget`.
</Warning>
...
```json
// UnifAI Request - Both fields provided
{
...
| UnifAI Effort | Non-Pro Models | Pro Models | Notes |
|----------------|----------------|------------|-------|
...
```go
// Using UnifAI Go SDK with Gemini
...
```go
// After calling UnifAI Chat Completions with Gemini
resp, err := client.ChatCompletionRequest(schemas.NewUnifAIContext(ctx, schemas.NoDeadline), chatReq)
if err != nil {
  log.Fatal(err)
}
...
**Gemini → UnifAI (Response)**:

| Gemini Field | UnifAI Field | Conversion |
|--------------|---------------|------------|
...
UnifAI supports two distinct reasoning models across different providers:
...
**Important**: Both effort and max_tokens can be specified in a single request. UnifAI uses a **priority hierarchy** to determine which field is used.

### Priority Logic: Native vs. Estimated

When both `effort` and `max_tokens` are present in a request, UnifAI prioritizes the **native compatible field** for the target provider:
...
UnifAI provides two estimator functions to convert between reasoning methods. These are used when the native field is not available.
...
// UnifAI automatically converts effort to budget tokens:
...
// Using UnifAI Go SDK with max_tokens (not effort)
chatReq := &schemas.UnifAIChatRequest{
  Provider: schemas.Bedrock,
  Model:    "us.amazon.nova-pro-v1:0",
  Input:    messages,
...
// UnifAI automatically estimates effort from max_tokens:
...
// Using UnifAI Go SDK with BOTH effort and max_tokens
chatReq := &schemas.UnifAIChatRequest{
  Provider: schemas.Anthropic,
  Model:    "claude-3-5-sonnet-20241022",
  Input:    messages,
...
**Note**: UnifAI automatically detects version and uses appropriate conversion
</Accordion>
