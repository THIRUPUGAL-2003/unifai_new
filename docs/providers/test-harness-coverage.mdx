...
| Bash tool | ✅ |
| Memory tool | ✅ |
| Vision (image; URLs auto-fetched + inlined by UnifAI) | ✅ |
...
**UnifAI-side normalizations applied automatically** (don't appear as separate rows):
...
These exercise UnifAI's translation layer between provider shapes — every check uses the unified `POST /v1/chat/completions` endpoint with `provider/model` prefix routing.
...
UnifAI exposes umbrella SDK routes that accept the OpenAI / Anthropic / Gemini / Bedrock / Cohere request shape byte-for-byte and dispatch to the right typed provider underneath. Each umbrella has its own smoke folder plus a shared vision matrix.
...
### UnifAI-only behaviors

| Feature | Status |
|---|---|
| Failover via `X-UnifAI-Fallback-Models` | ✅ |
| Virtual key auth via `X-UnifAI-VK` | ✅ |
...
Catch-all forwarding routes that strip incoming auth headers and inject UnifAI's configured provider key.
...
**Bedrock**: passthrough is **not supported by design**. AWS SigV4 signing requires unifai to sign the request with its own credentials, which fundamentally conflicts with byte-for-byte forwarding. Use the typed `/bedrock/model/{modelId}/converse`, `/converse-stream`, or `/invoke` routes instead — those go through unifai's typed Bedrock provider with proper sigv4 handling.
...
| **cross-model** | Unified `/v1/chat/completions` with `provider/model` prefix. Tests UnifAI's translation layer. |
...
| **passthrough** | `/*_passthrough/*` byte-for-byte forwarding with auth-header strip + UnifAI key injection. |
...
- **Vertex-Anthropic URL document sources** — Vertex doesn't accept URL document sources upstream; unifai auto-fetches and inlines via `inlineDocumentURLs` for parity with direct Anthropic
...
The full backlog of candidate test additions lives in [`tests/e2e/api/HARNESS_COVERAGE_BACKLOG.md`](https://github.com/unifai/unifai/blob/main/tests/e2e/api/HARNESS_COVERAGE_BACKLOG.md) — provider-by-provider feature inventory sourced from each provider's official docs. Adding a new test is a one-line entry in the relevant sub-folder of `provider-harness.json`.
