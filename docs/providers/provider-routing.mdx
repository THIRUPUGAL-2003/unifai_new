...
description: "Understand how UnifAI routes requests across AI providers using governance rules and adaptive load balancing."
icon: "route"
---

## Overview

UnifAI offers two powerful methods for routing requests across AI providers, each serving different use cases:
...
The Model Catalog is UnifAI's central registry that tracks which models are available from which providers. It powers both governance-based routing and adaptive load balancing by maintaining an up-to-date mapping of models to providers.
...
1. **Pricing Data** (Primary source)
   - Downloaded from a remote URL (configurable, defaults to `https://getunifai.ai/datasheet`)
...
UnifAI uses a sophisticated multi-step process to determine if a model is available for a provider:
...
<AccordionGroup>
  <Accordion title="Initial Sync (Startup)">
    When UnifAI starts, it performs a complete model catalog initialization:
    ...
    **Result**: UnifAI is ready with a comprehensive model catalog combining both sources.

  </Accordion>

  <Accordion title="Ongoing Sync (Background)">
    While UnifAI is running, the catalog stays up-to-date through background workers:
  ...
  <Accordion title="Fallback Strategy">
    UnifAI's multi-layered approach ensures high availability:
...
- UnifAI calls `GetModelsForProvider("openai")`
...
```bash
# ✅ Allowed (in catalog)
curl -H "x-uf-vk: vk-123" -d '{"model": "gpt-4o"}'

# ✅ Allowed (in catalog)
curl -H "x-uf-vk: vk-123" -d '{"model": "gpt-3.5-turbo"}'

# ❌ Rejected (not in OpenAI catalog)
curl -H "x-uf-vk: vk-123" -d '{"model": "claude-3-5-sonnet"}'
```
...
- UnifAI validates request model against explicit list
...
curl -H "x-uf-vk: vk-123" -d '{"model": "gpt-4-turbo"}'
# Even though gpt-4-turbo is in the OpenAI catalog!

# ✅ Allowed (exact match for Anthropic)
curl -H "x-uf-vk: vk-123" -d '{"model": "claude-3-5-sonnet-20241022"}'

# ❌ Rejected (version mismatch)
curl -H "x-uf-vk: vk-123" -d '{"model": "claude-3-5-sonnet-20240620"}'
```

**Provider-Prefixed Entries**:

You can also use provider-prefixed model names in `allowed_models`. UnifAI will strip the prefix and match against the requested model:
...
3. **UnifAI validates**: `gpt-4o` is in derived allowed models ✅
4. **UnifAI resolves alias**: `gpt-4o` → `my-prod-gpt4o-deployment`
...
3. **UnifAI validates**: `claude-sonnet` in allowed models ✅
...
<Steps>
  <Step title="OpenRouter as Universal Proxy">
    ...
    Request `claude-3-5-sonnet`:
    - UnifAI checks: `GetModelsForProvider("openrouter")`
  ...
  <Step title="Weighted Routing via Proxy Provider">
    ...
    **Why this works**: UnifAI now supports provider-prefixed entries in `allowed_models`, so `"openai/gpt-4o"` matches requests for `"gpt-4o"`.
...
This is how UnifAI achieves **intelligent cross-provider routing** without manual configuration.

</Tab>
</Tabs>

<Note>
  **Model Catalog is essential for cross-provider routing**. Without it, UnifAI
...
3. **Catalog lookup** (if `req.Provider` is still empty): UnifAI calls `GetProvidersForModel("gpt-4o")` to find all providers that support the model
...
When a Virtual Key has `provider_configs`, governance publishes the VK's allowed-provider set to the request context (`UnifAIContextKeyRoutingAllowedProviders`). The constraint is then enforced at **two levels**:
...
Custom routing plugins can set the same context key to constrain downstream routing for any reason — geo restrictions, A/B test cohorts, tier-based gating, etc. The semantics are **fail-closed**: setting `UnifAIContextKeyRoutingAllowedProviders` to an empty slice means "no provider is permitted for this request" → HTTP 400.
...
1. **Request arrives** with a Virtual Key (e.g., `x-uf-vk: vk-prod-main`)
2. **Model validation**: UnifAI checks if the requested model is allowed for any configured provider
...
<Info>
  **Enterprise Feature**: Adaptive Load Balancing is available in UnifAI
  Enterprise. [Contact us](https://www.getmaxim.ai/unifai/enterprise) to enable
  it.
</Info>
...
When both methods are available in your UnifAI deployment, they work together in a complementary way across two levels.
...
- **No Virtual Key** (do not send `x-uf-vk`) → this is the **Load Balancing–only** setup
