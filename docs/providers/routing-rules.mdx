...
**How it works:**
1. When a request arrives with a Virtual Key, UnifAI builds a scope chain
...
UnifAI analyzes each request's prompt content and assigns a tier before routing rules are evaluated. This lets you route cheap/fast requests to small models and deep reasoning tasks to frontier models with no application-side changes.
...
UnifAI resolves which limits to read in this order (most specific first):
...
<Info>Rule chaining is available in **UnifAI v1.5.0-prerelease2 and above**.</Info>
...
- **Memory**: Compiled programs cached in memory until UnifAI restart
...
<AccordionGroup>
  ...
  <Accordion title="Monitoring">
    ✅ **Track rule usage:**
    - Log which rules match (logged in UnifAI logs as `[RoutingEngine]`)
...
**This is normal!** UnifAI treats missing headers as non-matches:
```cel
