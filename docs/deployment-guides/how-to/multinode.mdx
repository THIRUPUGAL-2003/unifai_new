...
description: "Deploy multiple UnifAI nodes with shared configuration for high availability in OSS deployments"
icon: "layer-group"
---

## Overview

Running multiple UnifAI nodes provides high availability, load distribution, and fault tolerance for your AI gateway. This guide covers the recommended approach for deploying multiple UnifAI nodes in OSS deployments.
...
All configuration in UnifAI is loaded into memory at startup. For OSS multinode deployments, the recommended approach is to use `config.json` **without** `config_store` enabled.
...
UnifAI Enterprise includes **P2P clustering** with gossip protocol that automatically syncs configuration changes across all nodes in real-time. See the [Clustering documentation](/enterprise/clustering) for details.
...
http {
    upstream unifai {
        least_conn;
        server unifai-1:8080;
        server unifai-2:8080;
        server unifai-3:8080;
    }
...
for node in node1 node2 node3; do
  rsync -avz /etc/unifai/config.json $node:/etc/unifai/config.json
done

# Restart nodes after config update
for node in node1 node2 node3; do
  ssh $node "systemctl restart unifai"
done
```
...
# Watch the rollout
kubectl rollout status deployment/unifai
```
...
docker-compose restart unifai-1
docker-compose restart unifai-2
docker-compose restart unifai-3
```
...
Always put a load balancer in front of your UnifAI nodes:
