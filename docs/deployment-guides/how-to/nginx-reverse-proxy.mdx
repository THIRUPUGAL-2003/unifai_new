...
description: "Run UnifAI behind NGINX with streaming-safe settings for SSE and WebSocket traffic"
icon: "shuffle"
---

This guide shows how to put NGINX in front of UnifAI for TLS termination, centralized routing, and load balancing.
...
- You want HTTPS termination in front of UnifAI.
- You run multiple UnifAI replicas and want L7 load balancing.
...
Use this when UnifAI and NGINX run as services in the same Compose project.
...
curl -i http://unifai.example.com/health

# Streaming check through NGINX
curl -N http://unifai.example.com/v1/chat/completions \
...
- [docker-compose.yml](https://github.com/unifai/unifai/blob/main/examples/configs/withnginxreverseproxy/docker-compose.yml)
- [helm-values.yaml](https://github.com/unifai/unifai/blob/main/examples/configs/withnginxreverseproxy/helm-values.yaml)
- [k8s-ingress.yaml](https://github.com/unifai/unifai/blob/main/examples/configs/withnginxreverseproxy/k8s-ingress.yaml)
