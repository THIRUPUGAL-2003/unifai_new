---
title: "ECS"
description: "Deploy UnifAI as a service in ECS AWS clusters"
icon: "aws"
---

Deploy UnifAI on AWS ECS using either Makefile automation or direct AWS CLI commands. This guide covers both Fargate and EC2 launch types, with options for managing configuration secrets.
...
The easiest way to deploy UnifAI to ECS is using the provided Makefile.
...
```bash
# First, create your config.json file with your UnifAI configuration
cat > /tmp/unifai-config.json <<EOF
{
  "config_store": {
    "enabled": true,
    "type": "postgres",
...
make deploy-ecs \
  ...
  CONFIG_JSON_FILE='/tmp/unifai-config.json'
...
|-----------|---------|-------------|
| `ECS_CLUSTER_NAME` | `unifai-cluster` | Name of the ECS cluster |
| `ECS_SERVICE_NAME` | `unifai-service` | Name of the ECS service |
| `ECS_TASK_FAMILY` | `unifai-task` | Task definition family name |
...
| `SECRET_NAME` | `unifai/config` | Secret/parameter name |
...
2. Injects the secret as an environment variable `UNIFAI_CONFIG` into the container
3. Uses a custom entrypoint that:
   - Silently writes the secret content to `/app/data/config.json`
   - Exits with error only if `UNIFAI_CONFIG` is not set
   - Then starts UnifAI normally
4. UnifAI reads the configuration from the file at startup

This approach ensures your configuration is securely stored and properly mounted as a file, which is required by UnifAI. The entrypoint does not log any config data to keep logs clean and secure.
</Note>
...
Deploy UnifAI to ECS using direct AWS CLI commands. This section provides step-by-step instructions for both Fargate and EC2 launch types.
...
Choose between AWS Secrets Manager or SSM Parameter Store to store your UnifAI configuration.

<Tabs>
<Tab title="Secrets Manager">

Create a secret containing the UnifAI configuration with Postgres backend:
...
aws secretsmanager create-secret \
  --name unifai/config \
  --secret-string file:///tmp/unifai-config.json \
  --region us-east-1

# Get the secret ARN (save this for later)
aws secretsmanager describe-secret \
  --secret-id unifai/config \
  --region us-east-1 \
  --query 'ARN' \
  --output text
```
...
Create a parameter containing the UnifAI configuration:
...
aws ssm put-parameter \
  --name /unifai/config \
  --value file:///tmp/unifai-config.json \
  --type SecureString \
  --region us-east-1
...
1. Reads the `UNIFAI_CONFIG` environment variable (injected from the secret)
2. Silently writes it to `/app/data/config.json` (where UnifAI expects the configuration file)
3. Exits with error if `UNIFAI_CONFIG` is not set
4. Then starts the UnifAI application

This is necessary because ECS injects secrets as environment variables, but UnifAI reads configuration from a file. The entrypoint does not log any config data to keep logs clean and secure.
</Note>
...
cat > /tmp/unifai-task-definition.json <<EOF
{
  "family": "unifai-task",
  "networkMode": "awsvpc",
  ...
  "containerDefinitions": [
    {
      ...
      "essential": true,
      "entryPoint": ["/bin/sh", "-c"],
      "command": ["if [ -n \"$UNIFAI_CONFIG\" ]; then echo \"$UNIFAI_CONFIG\" > /app/data/config.json; else echo \"ERROR: UNIFAI_CONFIG not set\" >&2 && exit 1; fi && exec /app/docker-entrypoint.sh /app/main"],
      "portMappings": [
        {
          "containerPort": 8080,
          "protocol": "tcp"
      ...
      "secrets": [
        {
          "name": "UNIFAI_CONFIG",
          "valueFrom": "arn:aws:secretsmanager:us-east-1:YOUR_ACCOUNT_ID:secret:unifai/config"
        }
      ],
      "healthCheck": {
...
aws ecs register-task-definition \
  --cli-input-json file:///tmp/unifai-task-definition.json \
  --region us-east-1
```
  ...
  "containerDefinitions": [
    {
      ...
      "secrets": [
        {
          "name": "UNIFAI_CONFIG",
          "valueFrom": "arn:aws:ssm:us-east-1:YOUR_ACCOUNT_ID:parameter/unifai/config"
        }
      ],
      "healthCheck": {
...
aws ecs create-service \
  --cluster unifai-cluster \
  --service-name unifai-service \
  --task-definition unifai-task \
  --desired-count 1 \
  --launch-type FARGATE \
...
aws ecs create-service \
  ...
  --launch-type FARGATE \
  --network-configuration "awsvpcConfiguration={subnets=[subnet-xxx,subnet-yyy],securityGroups=[sg-xxx],assignPublicIp=ENABLED}" \
  --load-balancers "targetGroupArn=arn:aws:elasticloadbalancing:us-east-1:YOUR_ACCOUNT_ID:targetgroup/unifai-tg/xxx,containerName=unifai,containerPort=8080" \
...
Deploy UnifAI to ECS using AWS CloudFormation for infrastructure as code management.
...
1. Injects the secret as an environment variable `UNIFAI_CONFIG` into the container
2. Uses a custom entrypoint that:
   - Silently writes the secret content to `/app/data/config.json`
   - Exits with error if secret is not set
3. This ensures UnifAI can read the configuration from the expected file location
...
Description: 'Deploy UnifAI service on ECS'

Parameters:
  ClusterName:
    Type: String
    Default: unifai-cluster
    Description: Name of the ECS cluster
  
  ServiceName:
    Type: String
    Default: unifai-service
...
Resources:
            ...
            Statement:
              - Effect: Allow
                Action:
                  ...
                  - ssm:GetParameters
                Resource:
                  - !Sub 'arn:aws:secretsmanager:${AWS::Region}:${AWS::AccountId}:secret:unifai/*'
                  - !Sub 'arn:aws:ssm:${AWS::Region}:${AWS::AccountId}:parameter/unifai/*'
              - Effect: Allow
                Action:
                  - kms:Decrypt
                Resource: '*'
  ...
  TaskDefinition:
    ...
    Properties:
      ...
      ContainerDefinitions:
        - Name: unifai
          Image: !Sub 'unifai/unifai:${ImageTag}'
          Essential: true
          EntryPoint: !If
            - HasSecret
            - - /bin/sh
          ...
          Command: !If
            - HasSecret
            - - 'if [ -n "$UNIFAI_CONFIG" ]; then echo "$UNIFAI_CONFIG" > /app/data/config.json; else echo "ERROR: UNIFAI_CONFIG not set" >&2 && exit 1; fi && exec /app/docker-entrypoint.sh /app/main'
            - !Ref AWS::NoValue
          PortMappings:
...
```bash
# Tail logs
aws logs tail /ecs/unifai-task --follow --region us-east-1

# View recent logs
LOG_STREAM=$(aws logs describe-log-streams \
  --log-group-name /ecs/unifai-task \
...
|-----------|---------|----------|-------------|
| `ClusterName` | `unifai-cluster` | No | ECS cluster name (must exist) |
| `ServiceName` | `unifai-service` | No | ECS service name |
| `TaskFamily` | `unifai-task` | No | Task definition family |
...
<Note>
The Makefile automatically creates the CloudWatch log group `/ecs/unifai-task`, so the execution role only needs `CreateLogStream` and `PutLogEvents` permissions, not `CreateLogGroup`.
</Note>
...
```json
{
  ...
  "Statement": [
    ...
    },
    {
      ...
      "Resource": "arn:aws:logs:*:*:log-group:/ecs/unifai-task:*"
    },
    {
      "Effect": "Allow",
      "Action": [
      ...
      "Resource": "arn:aws:secretsmanager:us-east-1:YOUR_ACCOUNT_ID:secret:unifai/config*"
    }
  ]
}
```
...
```json
{
  ...
  "Statement": [
    ...
    },
    {
      ...
      "Resource": "arn:aws:ssm:us-east-1:YOUR_ACCOUNT_ID:parameter/unifai/config"
    },
    {
      "Effect": "Allow",
      "Action": [
...
aws logs get-log-events \
  --log-group-name /ecs/unifai-task \
  --log-stream-name unifai/unifai/TASK_ID \
  --region us-east-1
```
