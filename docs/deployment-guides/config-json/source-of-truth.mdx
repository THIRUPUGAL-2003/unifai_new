...
UnifAI can use `config.json` in two different ways:
...
That distinction matters at startup. UnifAI uses `source_of_truth` and per-section reconciliation metadata, including `config_hash` for many config rows, to decide when file-backed configuration should preserve DB edits, update stored values, or prune DB-only rows.
...
|-------|--------------|--------------------|------------------|
| No `config.json` | Default SQLite `config.db` in app-dir | Enabled | UnifAI starts with defaults and stores runtime changes in SQLite |
...
In split mode, UnifAI treats `config.json` as a bootstrap and drift-detection source:
...
In this mode, explicitly present sections in `config.json` are authoritative at startup. UnifAI applies the file values even if the stored `config_hash` still matches, because UI/API edits do not update the file hash.
