...
`cluster_config` enables multi-node UnifAI enterprise clustering. The `type` field selects how nodes form a cluster:
...
The same UnifAI binary runs as the broker when started with `-mode=broker`
(or `UNIFAI_MODE=broker`). The broker process reads `cluster_config.broker`
...
| `mdns_service` | string | Optional mDNS service type override (e.g. `"_unifai-cluster._tcp"`) |

<Note>
For `discovery.type: "mdns"`, `service_name` is sufficient for most setups. When `mdns_service` is omitted, UnifAI derives the mDNS service type as `"_<service_name>._tcp"`. If you set `mdns_service`, it **overrides** the derived value and is used for both mDNS registration and browsing.
</Note>
