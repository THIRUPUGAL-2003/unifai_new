...
<Note>
Secret Management is an **enterprise-only** feature and requires the enterprise UnifAI image and a PostgreSQL config store.
</Note>

Connect an external secret manager so provider keys and other credentials are never stored in UnifAI's database. Configure `vault_store` under `config_store` in `config.json`.
