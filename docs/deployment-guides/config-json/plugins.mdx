---
title: "Plugins"
description: "Configure UnifAI plugins in config.json - semantic cache, OpenTelemetry, Maxim, Datadog, and custom plugins"
icon: "puzzle-piece"
---
...
| `config.default_cache_key` | No | - | Default cache key when no `x-uf-cache-key` header is sent |
...
| `config.service_name` | No | `"unifai"` | Service name reported to the collector |
...
| `config.service_name` | `"unifai"` | Service name in Datadog |
...
Load a custom Go plugin binary or WASM plugin at startup using the `path` field. Custom plugins must implement one of the UnifAI plugin interfaces.
