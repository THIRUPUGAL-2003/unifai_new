...
description: "Configure the UnifAI client in config.json - connection pool, logging, CORS, header filtering, compat shims, and MCP settings"
icon: "gear"
---

The `client` block controls how UnifAI manages its internal worker pool, request logging, authentication enforcement, header policies, SDK compatibility shims, and MCP agent behaviour.
...
When UnifAI acts as an OAuth client to upstream MCP servers, the `redirect_uri` it registers is built from the incoming `Host` header by default - behind a reverse proxy that is the proxy's internal address, not its public one. One setting lets you override it with the proxy's public URL.

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `mcp_external_client_url` | string or EnvVar | - | Public base URL UnifAI uses as the `redirect_uri` against upstream MCP servers |

This field supports env var syntax (`"env.MY_VAR"`). When unset, it falls back to the incoming `Host` header.

### The client role

When UnifAI connects to an upstream MCP server (Notion, Jira, GitHub, etc.) on behalf of a user, it acts as an OAuth *client*. `mcp_external_client_url` is the callback UnifAI registers as its `redirect_uri` with those upstream providers - they redirect the user's browser to `<client URL>/api/oauth/callback` after login. Set it to the proxy's public URL so the callback resolves to a publicly reachable address.
...
{
  "client": {
    "mcp_external_client_url": "env.UNIFAI_EXTERNAL_URL"
  }
}
```
...
When left empty, the `redirect_uri` falls back to the request's `Host` header - correct for development or when no reverse proxy fronts UnifAI.
...
| `allow_direct_keys` | boolean | `false` | Allow callers to bypass the registered key pool by sending `x-uf-direct-key: true` and a raw provider key in `Authorization` / `x-api-key` / `x-goog-api-key`. See [Direct API Key](../../providers/request-options#direct-api-key) |
...
Controls which `x-uf-eh-*` extra headers are forwarded to upstream LLM providers.
...
When both `allowlist` and `denylist` are empty, all `x-uf-eh-*` headers pass through. Specifying an `allowlist` enables strict whitelist mode - only listed headers are forwarded.
...
`governance.auth_config` protects the UnifAI dashboard and management API with username/password auth.
...
```json
{
  "governance": {
    "auth_config": {
      "is_enabled": true,
      "admin_username": "env.UNIFAI_ADMIN_USERNAME",
...
| Notes |
|-------|
| Accepts any string; UnifAI derives a 32-byte AES-256 key using Argon2id |
| Can also be set via the `UNIFAI_ENCRYPTION_KEY` environment variable |
