...
<Note>
The live schema is published at [`https://www.getunifai.ai/schema`](https://www.getunifai.ai/schema). Add `"$schema": "https://www.getunifai.ai/schema"` to your `config.json` for IDE autocomplete and inline validation.
</Note>
...
|-----|------|-------------|-------|
| `$schema` | string | Schema URL for IDE validation. Set to `"https://www.getunifai.ai/schema"` | - |
...
| `encryption_key` | string | Optional AES-256 key (derived via Argon2id). Accepts `env.VAR` prefix and is also read from `UNIFAI_ENCRYPTION_KEY`. If omitted, data is stored in plaintext. | [Client](/deployment-guides/config-json/client#encryption-key) |
...
| `allow_direct_keys` | boolean | `false` | Let callers bypass the key pool with `x-uf-direct-key: true` + a raw provider key |
...
| `header_filter_config` | object | - | `allowlist` / `denylist` for `x-uf-eh-*` forwarded headers |
