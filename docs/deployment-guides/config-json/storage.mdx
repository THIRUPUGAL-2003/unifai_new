---
title: "Storage"
description: "Configure UnifAI storage backends in config.json - config_store, logs_store, vector_store, and object storage for logs"
icon: "database"
---

UnifAI persists two types of data - **config** (providers, virtual keys, governance rules) and **logs** (request/response records). Each has its own store. A **vector store** is required for semantic caching.
...
<Note>
When `config_store` is omitted, UnifAI creates a default SQLite config store in the app directory. Set `config_store.enabled` to `false` only when you want file-only configuration with no config-backed Web UI/API edits. See [Source of Truth & Reconciliation](/deployment-guides/config-json/source-of-truth).
</Note>
...
Simplest setup - no external database required. UnifAI stores configuration in a local SQLite file.
...
Use this when you want UnifAI to read all configuration from `config.json` only - no configuration database and no config-backed Web UI/API edits.
...
```json
{
  "object_storage": {
    ...
    "region": "us-east-1",
    "compress": true,
    "role_arn": "arn:aws:iam::123456789012:role/UnifAIS3Role"
  }
}
```
...
| `prefix` | Key prefix for stored objects (default: `"unifai"`) |
