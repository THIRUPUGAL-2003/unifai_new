...
Azure requires `azure_key_config` on every key with `endpoint`. UnifAI uses the Azure OpenAI v1 API — no `api_version` needed. List your Azure deployment names in `models` - UnifAI routes requests using the model name as the deployment name. If your deployment names differ from the model names you use in requests, add an `aliases` map on the key.
...
When `value` is empty or omitted, UnifAI uses `DefaultAzureCredential` - which resolves credentials from Workload Identity, VM managed identity, or `az login`.
...
When only `region` is set, UnifAI inherits credentials from the AWS SDK default chain - IRSA, EC2 instance profile, or `AWS_*` env vars.
...
When `auth_credentials` is omitted, UnifAI calls `google.FindDefaultCredentials` - which resolves to GKE Workload Identity, GCE metadata server, or `gcloud auth application-default login`.
...
With three providers and the weights above, traffic is distributed: 50% OpenAI, 25% Anthropic, 25% Groq. If any provider returns an error, UnifAI automatically retries on the next key or provider.
