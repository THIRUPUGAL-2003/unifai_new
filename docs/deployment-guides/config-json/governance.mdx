...
The `governance` block lets you seed all governance resources directly in `config.json`. On startup, UnifAI loads these into the configuration store. This is the recommended approach for GitOps workflows where governance state is managed as code.
