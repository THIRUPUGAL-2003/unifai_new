...
description: "Optimize UnifAI container performance with Go runtime tuning, resource limits, and system configuration"
icon: "docker"
---

This guide covers performance tuning for UnifAI when running in Docker containers. Proper tuning ensures UnifAI can fully utilize container resources and achieve optimal throughput.
...
UnifAI automatically detects container CPU limits using [automaxprocs](https://github.com/uber-go/automaxprocs). This sets `GOMAXPROCS` to match your container's CPU quota from cgroups (v1 and v2).
...
services:
  unifai-1:
    ...
    environment:
      ...
      - GOGC=200
      - GOMEMLIMIT=1800MiB
      - UNIFAI_DB_TYPE=postgres
      - UNIFAI_DB_DSN=postgres://user:pass@postgres:5432/unifai?sslmode=disable
    ulimits:
      nofile:
        soft: 65536
        hard: 65536
...
## UnifAI Application Settings

Align UnifAI's internal settings with your container resources:
...
<Steps>
  ...
  <Step title="Align UnifAI settings">
...
- **[Performance Tuning](/providers/performance)** - UnifAI-specific performance configuration
