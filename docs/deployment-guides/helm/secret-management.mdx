...
description: "Configure AWS Secrets Manager, GCP Secret Manager, or HashiCorp Vault in UnifAI Helm deployments"
icon: "vault"
---

<Note>
Secret Management is an **enterprise-only** feature. It requires the enterprise UnifAI image and a PostgreSQL config store.
</Note>

Connect an external secret manager so provider keys and other credentials are never stored in UnifAI's database. Configure `vaultStore` under `storage.configStore` in your values file.
...
storage:
  configStore:
    vaultStore:
      ...
      aws:
        region: us-east-1
        roleArn: arn:aws:iam::123456789012:role/UnifAISecretsReader
```
...
| `prefix` | No | Path prefix for UnifAI-managed secrets. Defaults to `unifai`. |
...
unifai:
  providers:
    ...
    anthropic:
      keys:
        - value: vault.unifai/providers/anthropic/key
          models:
            - claude-opus-4-8
            - claude-sonnet-4-6
```
...
```bash
helm install unifai unifai/unifai -f vault-values.yaml
```
