...
description: "Configure LLM providers in the UnifAI Helm chart - API keys, cloud-native auth, and self-hosted endpoints"
icon: "plug"
---

All providers are configured under `unifai.providers` in your values file. Each provider entry contains a `keys` list where each key has a `name`, `value`, `weight`, and optional provider-specific config.
...
```bash
helm install unifai unifai/unifai -f openai-values.yaml
```
...
```bash
helm install unifai unifai/unifai -f anthropic-values.yaml
```
...
```bash
helm install unifai unifai/unifai -f azure-apikey-values.yaml
```

</Tab>
<Tab title="Managed Identity / Workload Identity">

When `value` is empty, UnifAI uses `DefaultAzureCredential` - which automatically resolves credentials from:
...
kubectl annotate serviceaccount unifai \
...
```bash
helm install unifai unifai/unifai -f azure-msi-values.yaml
```
...
```bash
helm install unifai unifai/unifai -f bedrock-static-values.yaml
```

</Tab>
<Tab title="IRSA / EKS Pod Identity">

When only `region` is set, UnifAI inherits credentials from the AWS SDK default chain - IRSA (IAM Roles for Service Accounts), EC2 instance profile, or `AWS_*` env vars.

**Step 1 - Annotate the service account with the IAM role**

```bash
kubectl annotate serviceaccount unifai \
  eks.amazonaws.com/role-arn="arn:aws:iam::123456789012:role/UnifAIBedrockRole"
```
...
```bash
helm install unifai unifai/unifai -f bedrock-irsa-values.yaml
```
...
```bash
helm install unifai unifai/unifai -f bedrock-assumerole-values.yaml
```
...
```bash
helm install unifai unifai/unifai -f vertex-sa-values.yaml
```

</Tab>
<Tab title="GKE Workload Identity / ADC">

When `auth_credentials` is omitted, UnifAI calls `google.FindDefaultCredentials` - which resolves to:
...
gcloud iam service-accounts add-iam-policy-binding \
  unifai-sa@my-project.iam.gserviceaccount.com \
  --role roles/iam.workloadIdentityUser \
  --member "serviceAccount:my-project.svc.id.goog[default/unifai]"
```

```yaml
serviceAccount:
  annotations:
    iam.gke.io/gcp-service-account: "unifai-sa@my-project.iam.gserviceaccount.com"
```
...
```bash
helm install unifai unifai/unifai -f vertex-wli-values.yaml
```
...
```bash
helm install unifai unifai/unifai -f ollama-values.yaml
```
...
```bash
helm install unifai unifai/unifai -f vllm-values.yaml
```
...
```bash
helm install unifai unifai/unifai -f sgl-values.yaml
```
...
helm install unifai unifai/unifai -f multi-provider-values.yaml
```
