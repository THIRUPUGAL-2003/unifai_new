...
description: "Run UnifAI in a multi-replica cluster with gossip-based peer discovery, distributed state sync, and high-availability configuration"
icon: "network-wired"
---

Cluster mode enables multiple UnifAI replicas to share state - rate limits, budget counters, and governance data - across pods. When `unifai.cluster.enabled` is `false` (the default), each replica operates independently and state is only shared via the database.
...
<Warning>
`unifai.cluster.*` is an enterprise capability. OSS images accept these values but do not run cluster mode at runtime.
</Warning>
...
helm install unifai unifai/unifai -f cluster-values.yaml
```
...
UnifAI uses a gossip protocol (memberlist) for peer-to-peer state sync. Configure how peers find each other:

<Note>
For `consul`, `etcd`, and `udp` discovery, set `unifai.cluster.discovery.serviceName` so nodes register/discover under a stable service identity.
</Note>
...
UnifAI queries the Kubernetes API to find other UnifAI pods by label selector. No static peer list needed - works with HPA.
...
unifai:
  cluster:
    ...
    discovery:
      enabled: true
      type: kubernetes
      k8sNamespace: "default"           # namespace where UnifAI runs
      k8sLabelSelector: "app.kubernetes.io/name=unifai"
    gossip:
      port: 7946
```
...
```bash
helm install unifai unifai/unifai -f cluster-k8s-discovery-values.yaml
```
...
unifai:
  cluster:
    ...
    discovery:
      ...
      type: dns
      dnsNames:
        - "unifai-headless.default.svc.cluster.local"
    gossip:
      port: 7946
```

The chart automatically creates a headless service (`unifai-headless`) when cluster mode is enabled with a StatefulSet. For Deployments, create it manually:
...
```bash
helm install unifai unifai/unifai -f cluster-dns-discovery-values.yaml
```
...
unifai:
  cluster:
    enabled: true
    peers:
      - "unifai-0.unifai-headless.default.svc.cluster.local:7946"
      - "unifai-1.unifai-headless.default.svc.cluster.local:7946"
      - "unifai-2.unifai-headless.default.svc.cluster.local:7946"
    gossip:
      port: 7946
```
...
```bash
helm install unifai unifai/unifai -f cluster-consul-discovery-values.yaml
```
...
# Install
helm install unifai unifai/unifai -f ha-production-values.yaml

# Verify all peers have found each other (check logs)
kubectl logs -l app.kubernetes.io/name=unifai --tail=50 | grep -i gossip
```
