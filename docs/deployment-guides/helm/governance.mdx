---
title: "Governance"
description: "Configure UnifAI governance in Helm - budgets, rate limits, virtual keys, routing rules, and admin authentication"
icon: "shield"
---

Governance lets you control who can call which providers, how much they can spend, how fast they can go, and how traffic is routed. Everything is declared under `unifai.governance` in your values file and seeded into the database at startup.
...
Protect the UnifAI dashboard and management API with username/password auth.
...
```bash
helm upgrade unifai unifai/unifai --reuse-values -f governance-auth-values.yaml
```
...
curl http://localhost:8080/v1/chat/completions \
  -H "x-uf-vk: vk-openai-prod-secret-token" \
...
helm install unifai unifai/unifai -f governance-full-values.yaml
```
...
You can seed enterprise `access_profiles` directly from Helm values. The chart renders `unifai.accessProfiles` into top-level `access_profiles` in `config.json`.
