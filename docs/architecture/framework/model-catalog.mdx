...
- **`PricingURL`**: Overrides the default URL (`https://getunifai.ai/datasheet`) for downloading the pricing sheet.
...
The Model Catalog is designed to be shared across all UnifAI plugins, providing consistent model information and validation logic for governance, load balancing, and other routing mechanisms.
...
In UnifAI's gateway, the `ModelCatalog` is initialized once at the start and shared across all plugins:
...
Calculate costs from a UnifAI response:
...
cost := modelCatalog.CalculateCost(result, nil) // *schemas.UnifAIResponse, *PricingLookupScopes
...
<Note>
  This cross-provider logic powers UnifAI's intelligent routing capabilities.
...
```go
// response is *schemas.UnifAIListModelsResponse
modelCatalog.AddModelDataToPool(response)
```

This is automatically done in UnifAI gateway initialization for all providers that are supported by UnifAI.
...
The Model Catalog provides a robust, production-ready foundation for implementing billing, budgeting, and cost monitoring features in UnifAI plugins.
