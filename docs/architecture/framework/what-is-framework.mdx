...
description: "Framework is UnifAI's shared storage and utilities SDK package that provides common database interfaces and logic for the plugin ecosystem."
icon: "play"
---
...
```bash
go get github.com/unifai/unifai/framework
```
...
Framework seamlessly integrates with the UnifAI ecosystem, providing the storage foundation that powers core features like provider management, request logging, semantic caching, and governance. When plugins use framework components, they automatically participate in UnifAI's unified data management strategy.
