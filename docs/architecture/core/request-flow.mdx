---
title: "Request Flow"
description: "Deep dive into UnifAI's request processing pipeline - from transport layer ingestion through provider execution to response delivery."
icon: "route"
---
...
sequenceDiagram
    ...
    Validation->>Router: UnifAIRequest
...
4. **Request Transformation** - Convert to internal `UnifAIRequest` schema
...
sequenceDiagram
    ...
    Application->>SDK: unifai.ChatCompletion(req)
...
```mermaid
flowchart TD
    Response[UnifAIResponse] --> Format{Response Format}
