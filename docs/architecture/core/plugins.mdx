---
title: "Plugins"
description: "Deep dive into UnifAI's extensible plugin architecture - how plugins work internally, lifecycle management, execution model, and integration patterns."
icon: "puzzle-piece"
---
...
UnifAI's plugin system is built around five key principles that ensure extensibility without compromising performance or reliability:
...
stateDiagram-v2
    ...
    PluginInit --> Registered: Add to UnifAIConfig
    ...
    Registered --> CleanupCall: UnifAI Shutdown
...
sequenceDiagram
    ...
    Client->>UnifAI: Request
    Note over UnifAI,Plugin2: PreRequestHook phase (once per request, before any fan-out)
    UnifAI->>Plugin1: PreRequestHook(request)
    Plugin1-->>UnifAI: routed request
    UnifAI->>Plugin2: PreRequestHook(request)
    Plugin2-->>UnifAI: routed request
    Note over UnifAI,Plugin2: PreLLMHook phase (per provider attempt)
    UnifAI->>Plugin1: PreLLMHook(request)
    Plugin1-->>UnifAI: modified request
    UnifAI->>Plugin2: PreLLMHook(request)
    Plugin2-->>UnifAI: modified request
    UnifAI->>Provider: API Call
    Provider-->>UnifAI: response
    UnifAI->>Plugin2: PostLLMHook(response)
    Plugin2-->>UnifAI: modified response
    UnifAI->>Plugin1: PostLLMHook(response)
    Plugin1-->>UnifAI: modified response
    UnifAI-->>Client: Final Response
```
...
sequenceDiagram
    ...
    Client->>UnifAI: Request
    Note over UnifAI,Cache: PreRequestHook phase (routing decided)
    UnifAI->>Auth: PreLLMHook(request)
    Auth-->>UnifAI: modified request
    UnifAI->>Cache: PreLLMHook(request)
    Cache-->>UnifAI: LLMPluginShortCircuit{Response}
    Note over Provider: Provider call skipped
    UnifAI->>Cache: PostLLMHook(response)
    Cache-->>UnifAI: modified response
    UnifAI->>Auth: PostLLMHook(response)
    Auth-->>UnifAI: modified response
    UnifAI-->>Client: Cached Response
```
...
sequenceDiagram
    ...
    Client->>UnifAI: Stream Request
    Note over UnifAI,Plugin2: PreRequestHook phase (routing decided)
    UnifAI->>Plugin1: PreLLMHook(request)
    Plugin1-->>UnifAI: modified request
    UnifAI->>Plugin2: PreLLMHook(request)
    Plugin2-->>UnifAI: modified request
    UnifAI->>Provider: Stream API Call

    loop For Each Delta
        Provider-->>UnifAI: stream delta
        UnifAI->>Plugin2: PostLLMHook(delta)
        Plugin2-->>UnifAI: modified delta
        UnifAI->>Plugin1: PostLLMHook(delta)
        Plugin1-->>UnifAI: modified delta
        UnifAI-->>Client: Send Delta
    end

    Provider-->>UnifAI: final chunk (finish reason)
    UnifAI->>Plugin2: PostLLMHook(final)
    Plugin2-->>UnifAI: modified final
    UnifAI->>Plugin1: PostLLMHook(final)
    Plugin1-->>UnifAI: modified final
    UnifAI-->>Client: Final Chunk
```
...
sequenceDiagram
    ...
    Client->>UnifAI: Request (Provider1 + Fallback Provider2)
    UnifAI->>Plugin1: PreLLMHook(request)
    Plugin1-->>UnifAI: LLMPluginShortCircuit{Error, AllowFallbacks=true}
    Note over Provider1: Provider1 call skipped
    UnifAI->>Plugin1: PostLLMHook(error)
    Plugin1-->>UnifAI: error unchanged

    Note over UnifAI: Try fallback provider
    UnifAI->>Plugin1: PreLLMHook(request for Provider2)
    Plugin1-->>UnifAI: modified request
    UnifAI->>Provider2: API Call
    Provider2-->>UnifAI: response
    UnifAI->>Plugin1: PostLLMHook(response)
    Plugin1-->>UnifAI: modified response
    UnifAI-->>Client: Final Response
```
...
sequenceDiagram
    ...
    UnifAI->>Plugin2: PreLLMHook(request)
    Plugin2-->>UnifAI: modified request
    UnifAI->>RecoveryPlugin: PreLLMHook(request)
    RecoveryPlugin-->>UnifAI: modified request
    UnifAI->>Provider: API Call
    Provider-->>UnifAI: error
    UnifAI->>RecoveryPlugin: PostLLMHook(error)
    RecoveryPlugin-->>UnifAI: recovered response
    UnifAI->>Plugin2: PostLLMHook(response)
    Plugin2-->>UnifAI: modified response
    UnifAI->>Plugin1: PostLLMHook(response)
    Plugin1-->>UnifAI: modified response
    UnifAI-->>Client: Recovered Response
```
...
UnifAI provides detailed metrics for plugin performance and health monitoring:
