...
description: "Deep dive into UnifAI's Model Context Protocol (MCP) integration - how external tool discovery, execution, and integration work internally."
icon: "toolbox"
---

## MCP Architecture Overview

### **What is MCP in UnifAI?**

The Model Context Protocol (MCP) system in UnifAI enables AI models to seamlessly discover and execute external tools, transforming static chat models into dynamic, action-capable agents. This architecture bridges the gap between AI reasoning and real-world tool execution.

**Core MCP Principles:**

- **Dynamic Discovery** - Tools are discovered at runtime, not hardcoded
- **Client-Side Execution** - UnifAI controls all tool execution for security
...
UnifAI supports four MCP connection types, each optimized for different tool deployment patterns:
...
sequenceDiagram
    ...
    Note over UnifAI: System Startup
    UnifAI->>MCPManager: Initialize MCP System
    ...
    Note over UnifAI: Runtime Request Processing
...
3. **Tool Validation** - UnifAI validates tool definitions and security
...
UnifAI provides granular control over tool availability through a sophisticated filtering system:
...
curl -X POST http://localhost:8080/v1/chat/completions \
  -H "x-uf-mcp-include-clients: filesystem,websearch" \
  -d '{"model": "gpt-4o-mini", "messages": [...]}'

# Include only specific tools
curl -X POST http://localhost:8080/v1/chat/completions \
  -H "x-uf-mcp-include-tools: filesystem-read_file,websearch-search" \
  -d '{"model": "gpt-4o-mini", "messages": [...]}'
```
...
```mermaid
flowchart TD
    A["👤 User Message<br/>\"List files in current directory\""] --> B["🤖 UnifAI Core"]
    ...
    I -->|Approve| K["⚙️ client.ExecuteMCPTool()<br/>UnifAI executes via MCP"]
...
Agent Mode transforms UnifAI into an autonomous agent runtime by automatically executing pre-approved tools. This section details the internal architecture of the agent execution loop.
...
When the LLM returns tool calls, UnifAI classifies each tool based on the client configuration:
