---
title: "Overview"
description: "Use UnifAI with LibreChat, Claude Code, Codex CLI, Gemini CLI, Qwen Code, and more by pointing each tool at the correct UnifAI endpoint."
icon: "Book"
---

## Overview

UnifAI provides **100% compatible endpoints** for OpenAI, Anthropic, and Gemini APIs, making it seamless to integrate with any agent that uses these providers. Point each agent at the UnifAI URL shape it expects (see [Configuration](#configuration) below and the guide for your tool). That unlocks:

- **Universal Model Access**: Use **any provider/model** configured in UnifAI with any agent (e.g., use GPT-5 with Claude Code, or Claude Sonnet 4.5 with Codex CLI)
- **MCP Tools Integration**: All Model Context Protocol tools configured in UnifAI become available to your agents
- **Built-in Observability**: Monitor all agent interactions in real-time through UnifAI's logging dashboard
...
<Note>
If your Allowed Headers are already set to `*`, you can skip this note. If not and you face issues integrating UnifAI with your CLI agent, try switching to `*` or adding the specific headers required by your client. By default, UnifAI whitelists: `Content-Type`, `Authorization`, `X-Requested-With`, `X-Stainless-Timeout`, and `X-Api-Key`.
</Note>
...
Agent integrations work with your existing UnifAI configuration. Ensure you have:
...
UnifAI exposes all configured MCP tools through a dedicated MCP server endpoint at `/mcp`. This allows external MCP clients (Claude Code, Cursor, and other MCP-compatible applications) to connect directly and access your tools.

### Connect Claude Code to UnifAI MCP

Add UnifAI as an MCP server to Claude Code:
...
If you have virtual key authentication enabled in UnifAI, connect using the JSON configuration format:

```bash
claude mcp add-json unifai '{"type":"http","url":"http://localhost:8080/mcp","headers":{"Authorization":"Bearer sk-uf-virtual-key"}}'
```

Replace `sk-uf-virtual-key` with your actual UnifAI virtual key.

Claude Code will only have access to the specific MCP tools permitted by the virtual key's configuration. To grant access to additional tools, verify or modify the virtual key's MCP tool permissions in the UnifAI dashboard.

### Supported Agents

You can connect any MCP-compatible client to UnifAI's MCP server:
...
- UnifAI automatically enforces these permissions
...
- **[Integrations](/integrations/what-is-an-integration)** - Understand how UnifAI works with existing AI provider SDKs
