---
title: "Gemini CLI"
description: "Use Google's Gemini CLI with UnifAI for advanced reasoning capabilities with any provider."
icon: "diamond"
---
...
<Note>
If your Allowed Headers are already set to `*`, you can skip this note. If not and you face issues integrating UnifAI with Gemini CLI, try switching to `*` or adding the specific headers required by your client. By default, UnifAI whitelists: `Content-Type`, `Authorization`, `X-Requested-With`, `X-Stainless-Timeout`, and `X-Api-Key`.
</Note>
...
## Configuring Gemini CLI to work with UnifAI
...
2. **Run Gemini CLI and sign in**
   ...
   gemini
   ```
   Select **Login with Google** and authenticate via your browser. All traffic automatically routes through UnifAI.
...
1. **Configure environment variables**
   ```bash
   export GEMINI_API_KEY=your-api-key  # Gemini API key or UnifAI virtual key
   export GOOGLE_GEMINI_BASE_URL=http://localhost:8080/genai
   ```
...
```bash
export GOOGLE_API_KEY=your-api-key  # Google API key or UnifAI virtual key
...
Now all Gemini CLI traffic flows through UnifAI, giving you access to any provider/model configured in your UnifAI setup, plus observability and governance.
...
UnifAI automatically translates GenAI API requests to other providers, so you can use Gemini CLI with models from OpenAI, Anthropic, Mistral, and more. Use the `provider/model-name` format to specify any UnifAI-configured model.
