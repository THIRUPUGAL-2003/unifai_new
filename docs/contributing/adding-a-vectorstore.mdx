...
description: "Learn how to contribute a backend for the vector store in UnifAI"
icon: "circle-nodes"
---

The Vector store in UnifAI is designed to be extensible, allowing support for different vector database backends. This guide outlines the philosophy, architecture, and steps to add support for a new vector database.

This guide will help you add a new custom backend for the vector store. Currently, UnifAI supports Weaviate, Redis and Qdrant.
...
The vector store is used for semantic search and similarity matching in UnifAI. This enables features like RAG (Retrieval-Augmented Generation) and intelligent document retrieval. UnifAI exposes a single interface (`VectorStore`) for all vector operations.

Any custom backend for vector store should implement the `VectorStore` interface. The interface is defined in [vectorstore/store.go](https://github.com/unifai/unifai/blob/main/framework/vectorstore/store.go).
