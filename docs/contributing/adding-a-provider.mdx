...
description: "Learn how to contribute a new provider to UnifAI."
icon: "box"
---

This guide will walk you through creating a provider for UnifAI, testing locally, adding it to frontend and CI/CD.
...
1.  **Fork and Clone**:
    *   Fork the repository: https://github.com/unifai/unifai/
    *   Clone your fork: `git clone https://github.com/<your_github_username>/unifai/`
2.  **Initialize**:
    *   Run `make dev` at the root of the project to set up dependencies and tools.

## Provider Structure

UnifAI acts as a gateway:
...
5.  Converts it back to the standard UnifAI response format.

To implement a new provider, first step is to add the provider name in `core/schemas/unifai.go`
...
5. **Handle Errors**: Parse and convert provider errors to `schemas.UnifAIError`
6. **Convert Response**: Call `ToUnifAI[Feature]Response()` from feature file
7. **Return Result**: Return UnifAI response or error
...
func (p *ProviderNameProvider) ChatCompletion(
    ...
    return ToUnifAIChatResponse(&providerResp)
}
```
...
- **To Provider Format**: `To[ProviderName][Feature]Request(unifaiReq *schemas.UnifAI[Feature]Request) *[ProviderName][Feature]Request`
- **To UnifAI Format**: `ToUnifAI[Feature]Response(providerResp *[ProviderName][Feature]Response) (*schemas.UnifAI[Feature]Response, *schemas.UnifAIError)`

**Examples**:
- `ToHuggingFaceChatCompletionRequest`
- `ToUnifAIChatResponse`
- `ToHuggingFaceEmbeddingRequest`
- `ToUnifAIEmbeddingResponse`

**Required Converter Pairs** (if feature is supported):
- Request converter: UnifAI → Provider
- Response converter: Provider → UnifAI
...
// ToHuggingFaceChatCompletionRequest converts a UnifAI chat request to HuggingFace format
func ToHuggingFaceChatCompletionRequest(unifaiReq *schemas.UnifAIChatRequest) *HuggingFaceChatRequest {
    if unifaiReq == nil || unifaiReq.Input == nil {
        return nil
    }

    // Convert messages from UnifAI format to HuggingFace format
    hfMessages := make([]HuggingFaceChatMessage, 0, len(unifaiReq.Input))
...
// ToProviderNameEmbeddingRequest converts a UnifAI embedding request to provider format
func ToProviderNameEmbeddingRequest(unifaiReq *schemas.UnifAIEmbeddingRequest) *ProviderNameEmbeddingRequest {
    if unifaiReq == nil {
        return nil
    }
    ...
    if unifaiReq.Input != nil {
        if unifaiReq.Input.Text != nil {
            providerReq.Input = *unifaiReq.Input.Text
        } else if unifaiReq.Input.Texts != nil {
            providerReq.Input = unifaiReq.Input.Texts
        }
    }

    // Map provider-specific parameters from ExtraParams
    if unifaiReq.Params != nil && unifaiReq.Params.ExtraParams != nil {
        if normalize, ok := unifaiReq.Params.ExtraParams["normalize"].(bool); ok {
...
// ToUnifAIListModelsResponse converts provider models list to UnifAI format
...
// Helper to determine which UnifAI methods a model supports
...
func (provider *CerebrasProvider) ChatCompletionStream(
    ...
    key schemas.Key,
    request *schemas.UnifAIChatRequest,
) (chan *schemas.UnifAIStream, *schemas.UnifAIError) {
...
**Error Handling**:
- OpenAI handlers return `*schemas.UnifAIError` - propagate directly
...
5. **Create `utils.go` - Define Constants and Helper Functions**:
   ...
   **Order of Definitions**:
   ...
   func parseErrorResponse(body []byte) *schemas.UnifAIError { ... }
...
6. **Create Feature Files in Order of Complexity** (simplest first):
   ...
   **a. Create `models.go` (if supported)**:
   ...
   // ToUnifAIListModelsResponse converts provider models to UnifAI format
   func (response *[ProviderName]ListModelsResponse) ToUnifAIListModelsResponse(
       providerKey schemas.ModelProvider,
   ) *schemas.UnifAIListModelsResponse {
       if response == nil {
           return nil
   ...
   **b. Create `embedding.go` (if supported)**:
   ...
   // To[ProviderName]EmbeddingRequest converts UnifAI request to provider format
   func To[ProviderName]EmbeddingRequest(
       ...
       if unifaiReq.Params != nil {
           ...
           if unifaiReq.Params.Dimensions != nil {
               providerReq.Dimensions = unifaiReq.Params.Dimensions
           }

           // Provider-specific parameters from ExtraParams
           if unifaiReq.Params.ExtraParams != nil {
               if val, ok := unifaiReq.Params.ExtraParams["provider_param"].(string); ok {
   ...
   // ToUnifAIEmbeddingResponse converts provider response to UnifAI format
   func ToUnifAIEmbeddingResponse(
       ...
       for i, embedding := range providerResp.Data {
           unifaiResp.Data = append(unifaiResp.Data, schemas.EmbeddingData{
               Index:     i,
       ...
       if providerResp.Usage != nil {
           unifaiResp.Usage = &schemas.Usage{
   ...
   **c. Create `chat.go` (most complex)**:
   ...
   // To[ProviderName]ChatCompletionRequest converts UnifAI chat request to provider format
   func To[ProviderName]ChatCompletionRequest(
       ...
       // Convert messages
       providerMessages := make([][ProviderName]ChatMessage, 0, len(unifaiReq.Input))
       for _, msg := range unifaiReq.Input {
...
**Key conversion patterns to implement**:
   ```go
   // Request Converter - Maps UnifAI standard to provider format
   func To[ProviderName][Feature]Request(unifaiReq) *[ProviderName]Request {
       // 1. Nil check
   ...
   // Response Converter - Maps provider format back to UnifAI
   func ToUnifAI[Feature]Response(providerResp) (*schemas.UnifAIResponse, *schemas.UnifAIError) {
   ...
   **Converter Checklist for Each Feature File**:
   ...
   - ✅ Response converter: `ToUnifAI[Feature]Response`
...
- ✅ Import `github.com/unifai/unifai/core/providers/openai`
...
Once your provider is implemented and tested, you need to integrate it into the UnifAI UI and CI/CD pipelines.
...
### Step 7: Register Provider in Core (`core/unifai.go`)
...
import (
    // ... existing imports
    "github.com/unifai/unifai/core/providers/[providername]"
)
```
...
func (unifai *UnifAI) createBaseProvider(providerKey schemas.ModelProvider, config *schemas.ProviderConfig) (schemas.Provider, error) {
    ...
    case schemas.ProviderName:
        return providername.NewProviderNameProvider(config, unifai.logger), nil
...
```go
case schemas.ProviderName:
    return providername.NewProviderNameProvider(config, unifai.logger)
```
...
- ✅ Provider import added to `core/unifai.go`
- ✅ Provider case added to `createBaseProvider` in `core/unifai.go`
...
- How UnifAI converts requests to/from this provider's format
...
[Provider Name] is a **[type: OpenAI-compatible/custom API/local-first]** provider offering [key features]. UnifAI converts requests to [Provider]'s expected format with [specific features]. Key characteristics:
...
Field mapping from provider format back to UnifAI standard.
```
...
`````go
response, err := client.ChatCompletion(ctx, &schemas.UnifAIChatRequest{
...
- ✅ Error handling properly converts to `schemas.UnifAIError`
...
**Schema & Core**:
- ✅ Provider added to `core/schemas/unifai.go` (ModelProvider type + arrays)
- ✅ Provider registered in `core/unifai.go` (import + case)
