...
description: "Complete guide to setting up the UnifAI repository for local development."
icon: "hammer"
---

This guide walks you through setting up the UnifAI repository for local development, from prerequisites to running your first development server.
...
UnifAI uses a modular architecture with the following structure:
...
make build        # Build UI and unifai-http binary
...
```bash
make test         # Run unifai-http tests
...
## Understanding UnifAI Architecture

Before diving into development, it's helpful to understand how UnifAI works internally. The architecture documentation provides detailed insights into:
...
- **API changes**: Edit files in `transports/unifai-http/`
...
# Check if .air.toml exists in transports/unifai-http/
ls transports/unifai-http/.air.toml
...
3. **Understand the architecture**: Read our [request flow documentation](/architecture/core/request-flow) to understand how UnifAI works internally
4. **Read the documentation**: Check out our [complete documentation](https://docs.unifai.ai)
