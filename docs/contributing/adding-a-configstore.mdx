...
description: "Learn how to contribute a backend for the config store in UnifAI"
icon: "database"
---

The Config store in UnifAI is designed to be extensible, allowing support for different database backends. This guide outlines the philosophy, architecture, and steps to add support for a new database.

This guide will help you add a new custom backend for the config store. Currently, unifai supports PostgreSQL and SQLite.

## Setup

We assume you have some idea about how UnifAI works and you have already [set up unifai for local development](./setting-up-repo).
...
The config store is used to save all your unifai configurations. This can be a simple in-memory store or a postgres database. UnifAI exposes a single interface (ConfigStore) for all configuration CRUD (Create, Read, Update, Delete) operations.

Any custom backend for config store should implement the `ConfigStore` interface. The interface is defined in [configstore/store.go](https://github.com/unifai/unifai/blob/main/framework/configstore/store.go).
...
GORM provides implementations for the functions listed in the `ConfigStore` interface. This significantly simplifies the implementation of the config store (see [postgres.go](https://github.com/unifai/unifai/blob/main/framework/configstore/postgres.go) as an example).
...
If you need help, please reach out to the UnifAI team on [Discord](https://discord.gg/exN5KAydbU).