...
description: "Learn how to contribute a backend for the log store in UnifAI"
icon: "database"
---

The Log store in UnifAI is designed to be extensible, allowing support for different database backends. This guide outlines the philosophy, architecture, and steps to add support for a new database.

This guide will help you add a new custom backend for the log store. Currently, UnifAI supports PostgreSQL and SQLite.
...
The log store is used to persist all request/response logs from your UnifAI proxy. This can be a lightweight SQLite database or a production-grade Postgres database. UnifAI exposes a single interface (`LogStore`) for all logging operations.

Any custom backend for log store should implement the `LogStore` interface. The interface is defined in [logstore/store.go](https://github.com/unifai/unifai/blob/main/framework/logstore/store.go).
...
GORM provides implementations for the functions listed in the `LogStore` interface. This significantly simplifies the implementation of the log store (see [postgres.go](https://github.com/unifai/unifai/blob/main/framework/logstore/postgres.go) as an example).
