/**
 * UnifAI docs - Base URL persistence
 *
...
(function () {
  ...
  if (window.__unifaiBaseUrlSwitcherLoaded) return;
  window.__unifaiBaseUrlSwitcherLoaded = true;

  var DEFAULT_URL = "http://localhost:8080";
  var STORAGE_KEY = "unifai_base_url";
  ...
  function findPlaygroundUrlInputs() {
    ...
    if (!el || el.__unifaiPlaygroundBound) return [];
    return [el];
  }
  ...
  function syncPlaygroundInputs(state) {
    ...
    for (var i = 0; i < inputs.length; i++) {
      var el = inputs[i];
      if (el.__unifaiPlaygroundBound) continue;
      el.__unifaiPlaygroundBound = true;
