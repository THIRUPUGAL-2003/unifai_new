...
<Note>
**Smart Conversion**: The conversion only happens when the model doesn't support text completions natively. If a model has native text completion support (like OpenAI's davinci models), UnifAI uses the text completion endpoint directly without any conversion.
</Note>
...
When LiteLLM compatibility is enabled and you make a text completion request, UnifAI first checks if the model supports text completion:
...
<Note>
**Smart Conversion**: The conversion only happens when the model doesn't support chat completions natively. If a model has native chat completion support (like OpenAI's gpt-4 models), UnifAI uses the chat completion endpoint directly without any conversion.
</Note>
...
When LiteLLM compatibility is enabled and you make a chat completion request, UnifAI first checks if the model supports chat completion:
...
- UnifAI uses the model catalog to check if a model supports text completion
...
To enable compat plugins per request, you can use `x-uf-compat` header:
- When `x-uf-compat: true` or `x-uf-compat: ["*"]`, enables all compat plugins options
- When `x-uf-compat: ["<setting1>","<setting2>"]`, enables the specified settings (available settings: `convert_text_to_chat`, `convert_chat_to_responses`, `should_drop_params`, `should_convert_params`)
...
**Good Use Cases:**
- Migrating from LiteLLM to UnifAI without code changes
...
- [Drop-in Replacement](/features/drop-in-replacement) - Use existing SDKs with UnifAI
- [LiteLLM Integration](/integrations/litellm-sdk) - Using LiteLLM SDK with UnifAI