...
UnifAI provides two complementary layers of resilience:

- **Retries** - When a provider returns a transient server error (network issue, 5xx) or a per-key failure (`429` rate-limit, `401`/`403` auth, `402` billing), UnifAI automatically retries the same request against the same provider. Transient-server retries reuse the same key with exponential backoff; per-key failures rotate to a different API key from your pool. Backoff is skipped only when rotating away from a *permanent* per-key failure (`401`/`402`/`403`) where waiting offers nothing — for `429` rotations a backoff is still applied to let account-level quota windows slide.
- **Fallbacks** - When the primary provider fails after exhausting all retries, UnifAI moves on to the next provider in your fallback chain. Each fallback provider gets its own full retry budget.
...
When a request fails with a retryable error, UnifAI:
...
4. Continues until the request succeeds, `max_retries` is exhausted, or every key is permanently dead (in which case UnifAI returns `502 upstream_credentials_exhausted` rather than the raw `4xx`, to make it clear the caller's UnifAI API key is fine — the configured provider credentials are not).
...
<Frame>
  <img src="/media/ui-retries-network-config.png" alt="Retries configuration in the UnifAI Web UI showing Max Retries, Retry Backoff Initial, and Retry Backoff Max fields under Network Config" />
</Frame>
...
When you configure multiple API keys for a provider, UnifAI automatically rotates to a fresh key when the failure is bound to the credential rather than the request:
...
**Rate-limited keys** are tracked in a per-request `used` set. Once all keys in the pool have been tried, UnifAI resets that set and starts a fresh weighted round — a previously rate-limited key may have free quota by then. With 3 keys and `max_retries: 5`, UnifAI can cycle through all three keys twice before giving up.

**Auth and billing failures** (401/402/403) are different: the failing key is marked **permanently dead** for the remainder of the request and is never reset. A bad credential won't become valid by waiting. If every configured key ends up permanently dead, UnifAI returns `502 upstream_credentials_exhausted` and skips any remaining retries.
...
2. **Fallback decision**: If the primary fails (and the error is retryable at the provider level), UnifAI moves to the first fallback
...
func chatWithFallbacks(client *unifai.UnifAI) {
    ctx := context.Background()

    response, err := client.ChatCompletionRequest(
        schemas.NewUnifAIContext(ctx, schemas.NoDeadline),
            ...
            Input: []schemas.ChatMessage{
                {
                    ...
                    Content: &schemas.ChatMessageContent{
                        ContentStr: unifai.Ptr("Explain quantum computing in simple terms"),
                    },
                },
            },
    ...
    fmt.Printf("Response from %s: %s\n",
        ...
        *response.Choices[0].UnifAINonStreamResponseChoice.Message.Content.ContentStr)
}
```
...
sequenceDiagram
    ...
    App->>UnifAI: Request (primary + fallbacks)

    rect rgb(220, 235, 250)
        note over UnifAI,Primary: Primary provider attempt (with retries)
        UnifAI->>Primary: Attempt 1
        Primary-->>UnifAI: 401 Unauthorized
        note over UnifAI: Key marked dead, rotate (no backoff)
        UnifAI->>Primary: Attempt 2 (different key)
        Primary-->>UnifAI: 429 Rate Limit
        note over UnifAI: Backoff + rotate key
        UnifAI->>Primary: Attempt 3 (different key)
        Primary-->>UnifAI: 503 Unavailable
        note over UnifAI: Backoff (same key)
        UnifAI->>Primary: Attempt 4
        Primary-->>UnifAI: 503 Unavailable
        note over UnifAI: max_retries exhausted
    end

    rect rgb(235, 250, 220)
        note over UnifAI,FB1: Fallback 1 attempt (with its own retries)
        UnifAI->>FB1: Attempt 1
        FB1-->>UnifAI: 500 Server Error
        note over UnifAI: Backoff
        UnifAI->>FB1: Attempt 2
        FB1-->>UnifAI: ✓ Success
    end

    UnifAI-->>App: Response (from Fallback 1)
```
...
OpenAI key 1 hits its rate limit. UnifAI rotates to key 2 on the next retry - no fallback needed, the request succeeds within the same provider.

**Scenario 2: Provider outage**

OpenAI is experiencing downtime (returning `503`). UnifAI retries with the same key (transient server issue), exhausts `max_retries`, then fails over to Anthropic. Anthropic succeeds on the first attempt.

**Scenario 3: Cascading failure**

Both primary and first fallback are down. UnifAI works through each provider's retry budget sequentially until the second fallback succeeds.
...
OpenAI key 1 was rotated out-of-band and now returns `401`. UnifAI marks key 1 permanently dead for this request and immediately rotates to key 2 (no backoff), which succeeds. Future requests will retry key 1 again — the dead-key set is per-request, not persistent. If every configured key was revoked, UnifAI would return `502 upstream_credentials_exhausted` instead of bubbling up the raw `401` (which would falsely suggest the *caller's* UnifAI API key is the problem).
