...
sequenceDiagram
    participant Client
    participant Gateway as UnifAI Gateway
...
curl -X POST http://localhost:8080/v1/async/chat/completions \
  ...
  -H "x-uf-async-job-result-ttl: 3600" \
  -d '{
...
| `failed` | Operation failed and error is stored | Provider call returns a UnifAI error |
...
- Per-request override uses `x-uf-async-job-result-ttl`.
- If the header is invalid or `<= 0`, UnifAI falls back to the default TTL.
...
- Background execution still uses UnifAI request APIs, so LLM plugin hooks (governance, logging, cost tracking, etc.) are executed for the actual inference run.
