---
title: "Kafka"
description: "Stream UnifAI request traces as JSON to a Kafka topic for custom analytics, archival, and downstream processing"
icon: "database"
---

<Note>
The Kafka connector is an **Enterprise** feature. It requires a UnifAI Enterprise license.
</Note>

## Overview

The **Kafka connector** publishes completed UnifAI request traces as JSON messages to a configured Kafka topic. Each message is keyed by the trace ID, so all spans for a trace land on the same partition and arrive in order.
...
**Fix:** Either create the topic manually before starting UnifAI, or set `auto_create_topic: true` to have the connector create it automatically on startup (requires broker admin permissions).
...
**Checks:**
- Verify the broker address is reachable from the UnifAI host: `nc -zv <broker-host> <port>`
...
**Checks:**
- Messages are written asynchronously — check UnifAI logs for `kafka plugin: failed to write trace ...` errors.
...
- **[Built-in observability](./default)** - Query logs directly from the UnifAI dashboard
