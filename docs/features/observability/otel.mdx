...
The **OTel plugin** enables seamless integration with OpenTelemetry Protocol (OTLP) collectors, allowing you to send LLM traces to your existing observability infrastructure. Connect UnifAI to platforms like Grafana Cloud, Datadog, New Relic, Honeycomb, or self-hosted collectors.
...
|-------|------|----------|-------------|
| `service_name` | `string` | ❌ No | Service name to be used for tracing, defaults to `unifai` |
...
| `group_traces_by_session` | `boolean` | ❌ No | Group requests sharing the same `x-uf-session-id` into one trace (default: `false`). See [Grouping Traces by Session](#grouping-traces-by-session) |
...
Whenever a request carries the [`x-uf-session-id`](/providers/request-options) header, UnifAI tags the trace's root span with the OTEL-conventional `session.id` attribute. This happens **regardless** of the `group_traces_by_session` setting, so you can always filter and correlate traces by session in your backend even when each request remains its own trace.

### Grouping Traces by Session

By default, each request UnifAI handles becomes its own OTEL trace (still tagged with `session.id` as described above). Enable `group_traces_by_session` to instead group every request that carries the same `x-uf-session-id` header into a **single trace**, with each request's root span appearing as a top-level sibling under that trace. This is useful for viewing a multi-turn conversation or agent run as one trace in your backend.
...
When enabled, requests sharing a session ID adopt a deterministic trace ID derived from that session ID, so they land in the same trace regardless of which UnifAI node handled them.
...
To send a session ID, pass the [`x-uf-session-id`](/providers/request-options) header on each request you want grouped together.
...
import (
    ...
    "github.com/unifai/unifai/framework/pricing"
    otel "github.com/unifai/unifai/plugins/otel"
)
  ...
  "plugins": [
    {
      ...
      "config": {
        ...
        "headers": {
          ...
          "x-honeycomb-dataset": "unifai-traces"
        }
      }
    }
  ]
...
- **Service Info**: `service.name=unifai`, `service.version`
- **Provider & Model**: `gen_ai.provider.name`, `gen_ai.request.model`
- **Session**: `session.id` on the root span when the request carries an `x-uf-session-id` header (see [Session Tracking](#session-tracking))
...
Headers UnifAI forwards to the upstream provider — both `x-uf-eh-*` prefixed headers and headers matched by the [direct allowlist](/deployment-guides/config-json/client#header-filtering) — are also surfaced on the `llm.call` span as `gen_ai.request.extra_header.<name>` attributes. This makes it easy to filter or correlate traces by caller context (session ID, tenant ID, correlation IDs) without standing up extra plumbing.

For example, sending `x-uf-eh-session-id: sess-abc-123` produces the span attribute `gen_ai.request.extra_header.session-id = "sess-abc-123"`. See [Extra Headers](/providers/request-options#extra-headers-x-uf-eh) for the full request format.
...
The OTel plugin captures all UnifAI request types:
...
<Note>
**Multi-node deployments**: If you are running multiple UnifAI nodes, use push-based metrics for accurate aggregation. Pull-based `/metrics` scraping may miss nodes behind a load balancer.
</Note>
...
|--------|------|-------------|
| `unifai_upstream_requests_total` | Counter | Total requests to upstream providers |
| `unifai_success_requests_total` | Counter | Successful upstream requests |
| `unifai_error_requests_total` | Counter | Error requests with status code labels |
| `unifai_input_tokens_total` | Counter | Total input tokens |
| `unifai_output_tokens_total` | Counter | Total output tokens |
| `unifai_cache_hits_total` | Counter | Cache hits |
| `unifai_cost_total` | Counter | Total cost in USD |
| `unifai_upstream_latency_seconds` | Histogram | Upstream request latency |
| `unifai_stream_first_token_latency_seconds` | Histogram | Time to first token |
| `unifai_stream_inter_token_latency_seconds` | Histogram | Inter-token latency |
...
Check UnifAI logs for emission errors:
