...
UnifAI provides comprehensive LLM observability through the **Maxim plugin**, enabling seamless tracking, evaluation, and analysis of AI interactions. The plugin automatically forwards all LLM requests and responses to Maxim's platform for detailed monitoring and performance insights.

![Maxim Logs](https://github.com/unifai/unifai/blob/main/docs/media/maxim-logs.png?raw=true)
...
import (
    ...
    maxim "github.com/unifai/unifai/plugins/maxim"
)
...
curl -X POST http://localhost:8080/v1/chat/completions \
  -H "x-uf-maxim-log-repo-id: project-specific-repo" \
  -d '{"model": "gpt-4", "messages": [...]}'
```
...
```bash
curl -X POST http://localhost:8080/v1/chat/completions \
  -H "x-uf-maxim-trace-id: custom-trace-123" \
  -H "x-uf-maxim-generation-id: custom-gen-456" \
  -H "x-uf-maxim-session-id: user-session-789" \
  -H "x-uf-maxim-trace-name: checkout-flow" \
  -H "x-uf-maxim-generation-name: rerank-step" \
  -d '{"model": "gpt-4", "messages": [...]}'
```
</Tab>
</Tabs>
...
```bash
curl -X POST http://localhost:8080/v1/chat/completions \
  -H "x-uf-maxim-environment: production" \
  -H "x-uf-maxim-user-id: user-123" \
  -H "x-uf-maxim-feature-flag: new-ui" \
  -d '{"model": "gpt-4", "messages": [...]}'
```

Reserved keys are `session-id`, `trace-id`, `trace-name`, `generation-id`, `generation-name`, `log-repo-id`. All other `x-uf-maxim-*` headers are treated as tags.
...
For tags that should be visible beyond Maxim, use `x-uf-dim-*` instead of `x-uf-maxim-*`.

`x-uf-dim-*` is the canonical per-request dimension prefix in UnifAI. Those dimensions are forwarded to Maxim as tags at lower priority than explicit `x-uf-maxim-*` tags, and they are also propagated to internal logs, OpenTelemetry spans, and Prometheus custom labels.

```bash
curl -X POST http://localhost:8080/v1/chat/completions \
  -H "x-uf-dim-environment: production" \
  -H "x-uf-dim-team: platform" \
  -d '{"model": "gpt-4", "messages": [...]}'
```

If you send both `x-uf-dim-environment` and `x-uf-maxim-environment`, Maxim uses the explicit `x-uf-maxim-*` tag value while the shared dimension still flows to the other observability backends.

## Supported Request Types

The plugin supports the following UnifAI request types:
