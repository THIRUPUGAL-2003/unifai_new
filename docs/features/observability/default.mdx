...
UnifAI traces comprehensive information for every request, without any changes to your application code.
...
When UnifAI retries a request (per-key failure or transient network/5xx error) the following fields are recorded:
...
- **Ad-hoc Headers**: Any `x-uf-lh-*` prefixed header is automatically captured into metadata
...
The logging plugin intercepts all requests flowing through UnifAI using the plugin architecture, ensuring your LLM requests maintain optimal performance:
...
When using UnifAI as a Go SDK, initialize the logging plugin manually:
...
func main() {
    ...
    // Initialize UnifAI with logging plugin
...
The logging plugin captures all UnifAI request types:
...
**2. `x-uf-lh-*` Prefix (Automatic)** - Any request header with the `x-uf-lh-` prefix is automatically captured into metadata with no configuration needed. The prefix is stripped and the remainder becomes the metadata key.

| Request Header | Metadata Key | Metadata Value |
|----------------|-------------|----------------|
| `x-uf-lh-tenant-id: acme` | `tenant-id` | `acme` |
| `x-uf-lh-env: production` | `env` | `production` |
| `x-uf-lh-region: us-east-1` | `region` | `us-east-1` |

Both methods can be used together - configured headers and `x-uf-lh-*` headers are merged into the same metadata map.
...
**Ad-hoc `x-uf-lh-*` headers (no config needed):**

```bash
curl http://localhost:8080/v1/chat/completions \
  -H "Content-Type: application/json" \
  -H "x-uf-lh-env: production" \
  -H "x-uf-lh-version: v2.1.0" \
  -d '{"model": "gpt-4o", "messages": [{"role": "user", "content": "Hello"}]}'
```
...
- **[Gateway Setup](../../quickstart/gateway/setting-up)** - Get UnifAI running with tracing enabled
