---
title: "Prometheus"
description: "Monitor UnifAI metrics with Prometheus scraping or Push Gateway for multi-node deployments"
icon: "chart-line"
---

## Overview

UnifAI exposes Prometheus metrics via two methods:
...
UnifAI automatically exposes a `/metrics` endpoint when the telemetry plugin is enabled (enabled by default). No additional configuration is needed.

<Info>
  When UnifAI's authentication is enabled (`auth_config.is_enabled = true`), the `/metrics` endpoint requires Basic auth credentials. You must include the same `admin_username` and `admin_password` from your `auth_config` in the Prometheus scrape configuration. Without this, Prometheus will receive `401 Unauthorized` responses and scraping will silently fail.
</Info>

### Prometheus Configuration

Add UnifAI to your Prometheus `prometheus.yml`:

```yaml
scrape_configs:
  - job_name: 'unifai'
    static_configs:
      - targets: ['unifai-host:8080']
    scrape_interval: 15s
```

If UnifAI authentication is enabled, add `basic_auth` to your scrape config:
...
| `job_name` | `string` | ❌ No | `unifai` | Job label for pushed metrics |
...
1. Navigate to **Observability** → **Prometheus** in the UnifAI UI
...
| Metric | Type | Description |
|--------|------|-------------|
| `unifai_upstream_requests_total` | Counter | Total requests to LLM providers |
| `unifai_upstream_latency_seconds` | Histogram | Provider request latency |
| `unifai_success_requests_total` | Counter | Successful provider requests |
| `unifai_error_requests_total` | Counter | Failed provider requests |
| `unifai_input_tokens_total` | Counter | Total input tokens processed |
| `unifai_output_tokens_total` | Counter | Total output tokens generated |
| `unifai_cost_total` | Counter | Total cost in USD |
| `unifai_cache_hits_total` | Counter | Cache hits by type |
| `unifai_stream_first_token_latency_seconds` | Histogram | Time to first token (streaming) |
| `unifai_stream_inter_token_latency_seconds` | Histogram | Inter-token latency (streaming) |
| `unifai_active_requests` | Gauge | LLM requests currently in-flight (labeled by `method` only) |
| `unifai_provider_key_up` | Gauge | Per-key health. `1` after a successful attempt, `0` after a failed attempt. Labels: `provider`, `key_id`, `key_name`. |
| `unifai_key_rotation_events_total` | Counter | Key rotations triggered by per-key failures — rate-limit (429), auth (401/403), or billing (402) — see below <sup>v1.5.0-prerelease4+</sup> |
| `unifai_request_retries` | Histogram | Number of retries used per request (observed once per request; buckets `0,1,2,3,5,10`). |

### Default Labels

Most request-level UnifAI LLM metrics include these labels (the `unifai_key_rotation_events_total` counter is an exception — see [Key Rotation Events](#key-rotation-events) below for its narrower label set):
...
- `routing_engine_used` - Comma-separated list of routing engines that contributed to the decision (e.g. `governance`, `routing-rule`, `loadbalancing`, `model-catalog`, `core`). `core` is emitted when the UnifAI orchestrator itself makes a routing decision — fallback transitions or retry transitions.
...
`unifai_key_rotation_events_total` is incremented once per **actual key rotation** — i.e. when a per-key failure causes the next retry to switch to a different key. Rotation-triggering failures are bound to the specific key/account rather than the request:
...
<Note>
  The `honor_labels: true` setting is important - it preserves the `job` and `instance` labels pushed by UnifAI instead of overwriting them with the Push Gateway's labels.
</Note>
...
|----------|-------------------|
| Single UnifAI instance | Pull (scraping) |
...
When multiple UnifAI instances run behind a load balancer:
...
- Verify the Push Gateway URL is correct and reachable from UnifAI
- Check firewall rules between UnifAI and Push Gateway
...
- Check UnifAI logs for push errors
