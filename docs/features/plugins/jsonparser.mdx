---
title: JSON Parser
description: A simple UnifAI plugin that handles partial JSON chunks in streaming responses by making them valid JSON objects.
icon: "code-branch"
---
...
import (
    ...
    "github.com/unifai/unifai/plugins/jsonparser"
)
...
func main() {
    ...
    stream, unifaiErr := client.ChatCompletionStreamRequest(schemas.NewUnifAIContext(ctx, schemas.NoDeadline), request)
    if unifaiErr != nil {
            // handle error
    }
...
func main() {
        ...
        Input: []schemas.ChatMessage{
            {
                ...
                Content: schemas.ChatMessageContent{
                    ContentStr: unifai.Ptr("Return user profile as JSON: {\"name\": \"John Doe\", \"email\": \"john@example.com\"}"),
                },
            },
        },
    }
    ...
    // Stream the response
    stream, unifaiErr := client.ChatCompletionStreamRequest(schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline), request)
    if unifaiErr != nil {
        panic(unifaiErr)
    }
