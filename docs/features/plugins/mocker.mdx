...
func main() {
    ...
    chatResponse, _ := client.ChatCompletionRequest(schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline), &schemas.UnifAIChatRequest{
    ...
    // Responses request
    responsesResponse, _ := client.ResponsesRequest(schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline), &schemas.UnifAIResponsesRequest{
        Provider: schemas.OpenAI,
        Model:    "gpt-4o",
...
Add the plugin to your project:
   ...
   ```bash
   go get github.com/unifai/unifai/plugins/mocker
   ```

Import in your code:

   ```go
   import mocker "github.com/unifai/unifai/plugins/mocker"
   ```
...
The Mocker plugin supports the following UnifAI request types:
...
ctx := context.WithValue(context.Background(), 
    schemas.UnifAIContextKey("skip-mocker"), true)
