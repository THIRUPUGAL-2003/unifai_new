...
UnifAI uses weighted random selection to distribute requests across multiple keys. This allows you to:
...
For cloud providers with deployment-based routing, UnifAI validates deployment availability:
...
UnifAI supports referencing a stored provider key by name or by ID instead of sending the raw secret. This can be useful when you want callers to reference logical key names or stable IDs and let the gateway resolve the actual secret from configured provider keys.

**When both are provided, ID takes priority over name.**

### By ID

- Header: send `x-uf-api-key-id: <key-id>` on the request. The gateway will look up the key with that ID.
- Context (Go SDK):
...
- Header: send `x-uf-api-key: <key-name>` on the request. The gateway will look up the named key and use its secret for the upstream provider call.
- Context (Go SDK):

```go
ctx := context.Background()
ctx = context.WithValue(ctx, schemas.UnifAIContextKeyAPIKeyName, "openai-key-1")
```
...
curl -X POST http://localhost:8080/v1/chat/completions \
  ...
  -H "x-uf-api-key: non_existant_key" \
  -d '{
...
```json
{"is_unifai_error":false,"error":{"error":"no key found with name \"non_existant_key\" for provider: anthropic","message":"no key found with name \"non_existant_key\" for provider: anthropic"},"extra_fields":{"provider":"anthropic","model_requested":"claude-haiku-4-5","request_type":"chat_completion"}}
```
...
curl -X POST http://localhost:8080/v1/chat/completions \
  -H "Content-Type: application/json" \
  -H "x-uf-api-key: key_with_model_disabled" \
  -d '{
...
```json
{"is_unifai_error":false,"error":{"error":"no keys found that support model: claude-sonnet-4-5","message":"no keys found that support model: claude-sonnet-4-5"},"extra_fields":{"provider":"anthropic","model_requested":"claude-sonnet-4-5","request_type":"chat_completion"}}
```

Note: This is not a weighted selection, by providing a specific key name you are explicitly telling UnifAI which stored key to use, so weighted distribution is bypassed. The example above demonstrates the error returned when a referenced key name cannot be resolved.
...
- **HTTP gateway:** the `allow_direct_keys` config flag and the `Authorization` / `x-api-key` / `x-goog-api-key` header pass-through (plus the Bedrock `x-uf-bedrock-*` and Azure `x-uf-azure-endpoint` integration paths) no longer forward keys to upstream providers.
- **Go SDK:** the `schemas.UnifAIContextKeyDirectKey` context value and the `Direct Key (Go SDK Only)` API have been removed.

All requests must use UnifAI-managed provider keys. To pin a specific key per request from the Go SDK, set `schemas.UnifAIContextKeyAPIKeyID` or `schemas.UnifAIContextKeyAPIKeyName` against a key managed by UnifAI (for example, one created via the providers API or returned by your `Account` implementation). See the [v1.5.0 migration guide](/migration-guides/v1.5.0) for the full rationale and migration recipes.
</Warning>
