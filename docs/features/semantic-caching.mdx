...
UnifAI can cache LLM responses and replay them for repeated requests, avoiding a round-trip to the provider. It offers two complementary lookup paths:
...
1. **A cache key is mandatory.** Caching only engages when a request carries a cache key (the `x-uf-cache-key` header, or the `CacheKey` context value in the Go SDK). Without one — and without a configured `default_cache_key` — the request bypasses the cache entirely. This is the single most common reason "nothing is being cached."
2. **Direct runs before semantic.** When both paths are enabled, a direct hash hit is served first; the semantic search only runs on a direct miss. You can narrow a request to one path with the `x-uf-cache-type` header.
3. **Writes are asynchronous.** On a cache miss, UnifAI returns the provider's response immediately and stores it in the background, so the *first* request never blocks on a cache write.
4. **Entries persist across restarts.** Cache entries live in your vector store with a per-entry expiry (`expires_at`). They are **not** purged when UnifAI shuts down — a restart keeps serving warm cache (see [Cache lifecycle](#lifecycle--cleanup)).
...
1. **A vector store** is required as the storage backend for *both* modes — even direct-only mode stores its entries there. UnifAI supports:
...
2. In the UnifAI UI, go to **Settings → Caching**. You'll see the **Local Cache** panel.
...
5. **For semantic mode**, fill in the embedding provider, model, and dimension that appear below the tabs:
   - **Configured Providers** — an embedding-capable provider already set up in UnifAI. Its API keys are inherited automatically.
...
8. Send a request with an `x-uf-cache-key` header to start caching (see [Triggering the cache](#triggering-the-cache)).
...
curl -X POST http://localhost:8080/api/plugins \
  ...
  -d '{
    ...
    "config": {
      ...
      "vector_store_namespace": "UnifAISemanticCachePlugin",
      "default_cache_key": ""
    }
  }'
```
...
cacheConfig := &semanticcache.Config{
    ...
    ExcludeSystemPrompt:          unifai.Ptr(false),

    // Cache key composition
    CacheByModel:    unifai.Ptr(true),
    CacheByProvider: unifai.Ptr(true),

    // Storage & default key (optional)
    VectorStoreNamespace: "UnifAISemanticCachePlugin",
    DefaultCacheKey:      "",
}
...
unifaiConfig := schemas.UnifAIConfig{
...
| `vector_store_namespace` | string | `UnifAISemanticCachePlugin` | Bucket/index where entries live. Changing it points the plugin at a different (possibly empty) bucket; old entries aren't deleted, just no longer queried. |
| `default_cache_key` | string | `""` (empty) | Fallback cache key used when a request doesn't send `x-uf-cache-key`. **Left empty, caching is disabled for any request without the header.** |
...
<Warning>
If you set `dimension: 1` **and** also provide a `provider`, UnifAI treats the config as semantic mode, not direct-only. To use direct-only mode, omit `provider` entirely.
</Warning>
...
In direct-only mode, all requests use hash matching regardless of the `x-uf-cache-type` header — no embeddings are generated and no embedding credentials are needed.
...
Set the cache key in the `x-uf-cache-key` header:

```bash
# This request WILL be cached
curl -H "x-uf-cache-key: session-123" ...
...
response, err := client.ChatCompletionRequest(schemas.NewUnifAIContext(ctx, schemas.NoDeadline), request)

// This request will NOT be cached (no context value)
response, err := client.ChatCompletionRequest(schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline), request)
```
...
|--------|------------------|-------|--------|
| `x-uf-cache-key` | `CacheKey` | string | Scope this request to a cache partition. Required (or `default_cache_key`) for caching to engage. |
| `x-uf-cache-ttl` | `CacheTTLKey` | duration string or seconds | Override TTL for this request. Invalid values are ignored. |
| `x-uf-cache-threshold` | `CacheThresholdKey` | float (0–1) | Override the semantic similarity threshold. Clamped to `[0,1]`. |
| `x-uf-cache-type` | `CacheTypeKey` | `direct` or `semantic` | Limit lookup to a single path. |
| `x-uf-cache-no-store` | `CacheNoStoreKey` | `true` | Skip writing the response (still serves cached hits). |
...
curl -H "x-uf-cache-key: session-123" \
     -H "x-uf-cache-ttl: 30s" \
     -H "x-uf-cache-threshold: 0.9" ...

# Force direct-only matching
curl -H "x-uf-cache-key: session-123" \
     -H "x-uf-cache-type: direct" ...

# Read from cache but don't store the response
curl -H "x-uf-cache-key: session-123" \
     -H "x-uf-cache-no-store: true" ...
```
...
<Note>
In direct-only mode (no embedding provider), `x-uf-cache-type` and `x-uf-cache-threshold` have no effect — every request uses direct matching.
</Note>
...
- **Entries persist across restarts** — cache data lives in your vector store and is **not** purged when UnifAI shuts down. A restart resumes serving the existing (unexpired) cache. To wipe entries, use the [cache-clear APIs](#cache-management) or clear the namespace in your vector store directly.
...
**Most common cause:** no cache key. Caching only engages when a request sends `x-uf-cache-key` (or you've set a `default_cache_key`). Confirm the header is present, then check `cache_debug` on the response.
</Accordion>
...
- **[Plugins overview](/features/plugins)** — how UnifAI's plugin pipeline works.
