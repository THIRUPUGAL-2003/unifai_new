---
title: "OpenTelemetry"
description: "Native OpenTelemetry tracing for every LLM call routed through UnifAI — export to any OTLP collector."
icon: "telescope"
---

## Overview

UnifAI provides built-in OpenTelemetry support that emits a fully-attributed span for every LLM request, including retries and fallbacks. Spans follow the [OpenTelemetry GenAI semantic conventions](https://opentelemetry.io/docs/specs/semconv/gen-ai/), so they correlate cleanly with the rest of your application traces and ship over OTLP to any compatible backend — Grafana, Datadog, New Relic, Honeycomb, Langfuse, Arize Phoenix, or a self-hosted OTel Collector.
...
- **Dynamic Attributes** — runtime span enrichment via `x-uf-eh-*` headers
...
| **UnifAI context** | `gen_ai.virtual_key_id`, `gen_ai.team_id`, `gen_ai.customer_id`, `gen_ai.number_of_retries` |
...
Any header UnifAI forwards to the upstream provider is also surfaced on the `llm.call` span. This includes:

- `x-uf-eh-*` prefixed extra headers (the standard mechanism)
- Headers matched by the [direct allowlist](/deployment-guides/config-json/client#header-filtering) (e.g. `anthropic-beta`)

For `x-uf-eh-*` headers the `x-uf-eh-` prefix is stripped and the remainder is lowercased; for direct-allowlist headers the header name is used as-is (lowercased). The result becomes the `<name>` in `gen_ai.request.extra_header.<name>`. The same security denylist and filter config that gates provider forwarding gates the span attribute — they are always the same set.
...
curl -X POST http://localhost:8080/v1/chat/completions \
  ...
  -H 'x-uf-eh-session-id: sess-abc-123' \
  -H 'x-uf-eh-tenant-id: acme-corp' \
  -d '{
...
<Note>
Want runtime labels on Prometheus metrics instead of OTel spans? Use `x-uf-dim-*` headers — see [Telemetry → Dynamic Label Injection](./telemetry#dynamic-label-injection). The same `x-uf-dim-*` values also flow through to OTel as span attributes.
</Note>
...
OpenTelemetry export is configured through the UnifAI UI, `config.json`, or the Go SDK. Full configuration options, popular platform recipes (Grafana Cloud, Datadog, New Relic, Honeycomb, Langfuse, self-hosted), cluster-mode metrics push, and the local Docker Compose stack are documented on the integrations page:
...
- **[Extra Headers Reference](/providers/request-options#extra-headers-x-uf-eh)** — Full `x-uf-eh-*` request format
