...
```mermaid
flowchart TB
    Client([Client]) --> Gateway[UnifAI HTTP]
    Gateway --> PreHook["HTTP transport pre-hook:<br/>copy x-uf-prompt-id / x-uf-prompt-version to context"]
    PreHook --> PreLLM["PreLLM hook:<br/>resolve version, merge params,<br/>prepend template messages"]
    PreLLM --> Provider[Provider]
```

1. **Transport (HTTP):** Incoming headers `x-uf-prompt-id` and `x-uf-prompt-version` are copied onto the UnifAI context (header name matching is case-insensitive).
2. **Resolve:** The plugin looks up the prompt and the requested version. If **`x-uf-prompt-version` is omitted**, the prompt’s **latest committed version** is used.
...
|--------|----------|-------------|
| `x-uf-prompt-id` | Yes, to enable injection | UUID of the prompt in the repository. |
| `x-uf-prompt-version` | No | **Integer version number** (e.g. `3` for v3). If omitted, the **latest** committed version for that prompt is used. |
...
curl -X POST http://localhost:8080/v1/chat/completions \
  ...
  -H "x-uf-prompt-id: YOUR-PROMPT-UUID" \
  -H "x-uf-vk: sk-uf-your-virtual-key" \
  -d '{
    "model": "openai/gpt-5.4",
    "messages": [
      {
        "role": "user",
        "content": "Tell me about UnifAI Gateway?"
      }
    ]
  }'
```
...
For embedded UnifAI (Go SDK), register the plugin with `prompts.Init` and a **config store** that implements the prompt tables API. The default resolver reads the same logical keys from `UnifAIContext`:

- `prompts.PromptIDKey` (`x-uf-prompt-id`)
- `prompts.PromptVersionKey` (`x-uf-prompt-version`)
...
- `schemas.UnifAIContextKeySelectedPromptID` - UUID of the applied prompt
- `schemas.UnifAIContextKeySelectedPromptName` - Display name of the prompt
- `schemas.UnifAIContextKeySelectedPromptVersion` - Version number as a string (e.g. `"3"`)
...
- Built-in plugin name in code: `prompts` (`github.com/unifai/unifai/plugins/prompts`).
