...
Required headers let you enforce that specific HTTP headers are present on every LLM and MCP request passing through UnifAI. If a request is missing any required header, the governance plugin rejects it with a **400 Bad Request** error before it reaches the provider.
...
1. The HTTP transport middleware stores all request headers in the UnifAI context (lowercased keys)
