...
Virtual Keys are the primary governance entity in UnifAI. Users and applications authenticate using the given headers to access virtual keys and get specific access permissions, budgets, and rate limits.

**Allowed Headers:**
- `x-uf-vk` - Virtual key header, eg. `sk-uf-*`
- `Authorization` - Authorization header, eg. `Bearer sk-uf-*` (OpenAI style)
- `x-api-key` - API key header, eg. `sk-uf-*` (Anthropic style)
- `x-goog-api-key` - API key header, eg. `sk-uf-*` (Google Gemini style)

<Note>Old virtual keys(without `sk-uf-*` prefix) are only supported by `x-uf-vk` header.</Note>
...
When the governance header is enforced, the request will be rejected if the `x-uf-vk` header is not present.
...
| **Virtual Keys** | Request routing and governance | `x-uf-vk`, `Authorization`[^1], `x-api-key`, `x-goog-api-key` |
...
```bash
# Using x-uf-vk header
curl -X POST http://localhost:8080/v1/chat/completions \
  -H "x-uf-vk: <VIRTUAL_KEY>" \
  -H "Content-Type: application/json" \
...
When you call `GET /v1/models` with a virtual key, UnifAI **only lists (and only queries) providers that are allowed by that virtual key**. This avoids unnecessary “provider not allowed” errors in logs and keeps error-rate metrics meaningful.
...
You must provide both authentication credentials AND the virtual key. Use `x-uf-vk` for the virtual key since the `Authorization` header is used for authentication:
