...
**Model Validation:**
When you configure provider restrictions on a Virtual Key, UnifAI validates that the requested model is allowed for the selected provider:
...
- **Model Catalog Sync**: On startup and provider updates, UnifAI calls each provider's list models API. If this fails, you'll see a warning: `{"level":"warn","message":"failed to list models for provider <name>: failed to execute HTTP request to provider API"}`
...
When multiple providers are configured on a Virtual Key, UnifAI automatically creates fallback chains for resilience. This feature provides automatic failover without manual intervention.
...
If you see warnings like this in your UnifAI logs during startup or provider updates:
```json
...
**What this means:**
- UnifAI attempted to call the provider's list models API to populate the Model Catalog
