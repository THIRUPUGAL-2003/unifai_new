...
They are the unified control plane for all model-level governance in UnifAI:
...
<Note>
The `user` scope is available in UnifAI Enterprise. Support for **customer** and **team** scopes is coming soon.
</Note>
...
|-------|-------------------|----------------------|
| `global` | All traffic through UnifAI | No |
...
Navigate to **Budget & Limits → Model Limits** in the UnifAI dashboard.
...
When a request arrives, UnifAI checks every applicable model limit **independently**. All must pass:
