...
2.  **With MCP Configuration on Virtual Key**
    ...
    - UnifAI automatically generates an `x-uf-mcp-include-tools` header based on your VK configuration (unless `disable_auto_tool_inject` is enabled or the caller already sent the header). This acts as a strict allow-list for the request.
    - If the caller already includes an `x-uf-mcp-include-tools` header, auto-injection is skipped - but the VK allow-list is enforced at inference time and still enforced again at MCP tool execution time.
...
<Note>
When a Virtual Key has MCP configurations, UnifAI enforces the allow-list at both inference time and MCP tool execution time. Auto-injection of the `x-uf-mcp-include-tools` header is skipped if the caller already provides it or if `disable_auto_tool_inject` is enabled - but the VK's restrictions are always applied regardless. You can still use the `x-uf-mcp-include-clients` header to filter MCP clients per request.
</Note>