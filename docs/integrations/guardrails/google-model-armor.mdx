...
description: "Integrate Google Cloud Model Armor with UnifAI Enterprise to inspect LLM prompts and responses, block policy violations, and apply Sensitive Data Protection de-identification."
icon: "/media/google-model-armor-nav.svg"
---

## Overview

UnifAI Enterprise supports [**Google Cloud Model Armor**](https://docs.cloud.google.com/model-armor/overview) as a guardrail provider for LLM request and response traffic.

Use it when your safety and data protection policies are managed in Google Cloud and you want UnifAI to enforce those policies inline before prompts reach an LLM and before model responses are returned.

Google owns the Model Armor template. UnifAI owns the gateway enforcement path: it selects when to call Model Armor, sends the relevant text to the template, then blocks or rewrites the UnifAI request/response based on the sanitize result.

<Note>
  **Streaming output:** When this profile is used in an `output` or `both` rule, UnifAI accumulates the stream until the model response is complete, then checks the full response. It does not check individual stream chunks. See [Streaming Output Guardrails](/enterprise/guardrails#streaming-output-guardrails) for details.
</Note>
...
- Keeping policy configuration in Google Cloud while enforcing it at the UnifAI gateway

<Note>
UnifAI follows the Model Armor template result. If Model Armor returns a non-mutable match, UnifAI returns `GUARDRAIL_INTERVENED`. If Model Armor returns SDP de-identified text, UnifAI applies the transformed text and allows the request or response to continue.
</Note>

## Prerequisites

- UnifAI Enterprise with the guardrails plugin enabled
...
- Network egress from UnifAI to the Model Armor regional endpoint over HTTPS
...
3. Create a template.
4. Note the template values UnifAI needs:
   - **Project ID**: for example `my-gcp-project`
   - **Location**: for example `us`, `eu`, or `us-central1`
   - **Template ID**: for example `unifai-prod`
5. Grant the UnifAI runtime identity `roles/modelarmor.user` or higher:
   - Go to **IAM & Admin** > **IAM**.
   - Click **Grant access**.
   - Add the service account or user identity UnifAI will use.
...
UnifAI supports two OAuth-based Google authentication modes.

| Auth mode | Use when | UnifAI config |
|-----------|----------|----------------|
| Google ADC | UnifAI should use credentials from the runtime environment | `auth_type: "default_credential"` |
...
Application Default Credentials (ADC) lets Google client libraries find credentials from the environment. UnifAI uses ADC when `auth_type` is `default_credential` or omitted.
...
<Note>
With ADC, no key JSON is stored in the UnifAI profile. Grant the identity that ADC resolves to the Model Armor User role or higher.
</Note>
...
2. Select or create the service account that UnifAI should use.
3. Open **Keys**.
4. Click **Add key** > **Create new key**.
5. Choose **JSON** and download the file.
6. Grant that service account **Model Armor User** or higher.

In UnifAI, either paste the full JSON into **Service Account Key JSON** or store it in an environment variable:
...
1. Create a UnifAI guardrail provider with `provider_name: "model-armor"`.
2. Attach that provider configuration to one or more guardrail rules.
3. When an input rule matches, UnifAI sends text to `sanitizeUserPrompt`.
4. When an output rule matches, UnifAI sends text to `sanitizeModelResponse`.
5. If Model Armor returns no match, UnifAI allows the content unchanged.
6. If Model Armor returns a blocking match, UnifAI returns `GUARDRAIL_INTERVENED`.
7. If Model Armor returns SDP de-identified text, UnifAI replaces the original text with the transformed text and continues.

### API Calls

UnifAI sends text-only Model Armor requests:
...
For output checks, UnifAI sends:
...
<Note>
UnifAI currently sends text content to Model Armor. It does not send file bytes or per-request multi-language detection metadata.
</Note>
...
<Frame>
  <img src="/media/ui-google-model-armor-config.png" alt="Google Model Armor configuration in UnifAI dashboard" />
</Frame>

4. Enter a descriptive **Name**, such as `model-armor-prod`.
5. Choose an authentication method:
   - **Google ADC** to use credentials available to the UnifAI runtime.
...
| Model Armor result | UnifAI behavior |
|--------------------|------------------|
...
| `invocationResult: "FAILURE"`, missing `sanitizationResult`, timeout, non-2xx response, or malformed response | Treats the provider call as failed. Check UnifAI logs for the exact error. |

UnifAI records Model Armor usage metadata for logs and spans:
...
When Google Model Armor blocks content, UnifAI returns HTTP `400` with `type: "guardrail_intervention"`.
...
|---------|---------------|
| `failed to load Google default credentials` | `auth_type` is `default_credential`, but ADC is not available to the UnifAI process. Run `gcloud auth application-default login` locally, set `GOOGLE_APPLICATION_CREDENTIALS`, or attach a service account/Workload Identity to the runtime. |
| `403 Permission denied` with `modelarmor.templates.useToSanitizeUserPrompt` or `useToSanitizeModelResponse` | Grant the UnifAI principal `roles/modelarmor.user` or higher on the project or template. |
...
| Direct Google curl works, but UnifAI fails | UnifAI may be using a different identity than your shell. Check whether the profile uses ADC or service account JSON, and check the process environment of the running UnifAI server. |
...
| Base URL errors | Leave Base URL blank unless you need a proxy. UnifAI derives `https://modelarmor.LOCATION.rep.googleapis.com` from the configured location. |
| Multiple mutating guardrails match the same request | UnifAI refuses ambiguous transformed output. Use one mutating profile per rule path or make the rules mutually exclusive. |
