---
title: "Patronus AI"
description: "Integrate Patronus AI with UnifAI for LLM security and safety including hallucination detection, PII identification, toxicity screening, and custom evaluators."
icon: "/media/patronus-logo-nav.svg"
---

## Overview

UnifAI Enterprise supports [**Patronus AI**](https://www.patronus.ai/) as a third-party guardrail provider for evaluating LLM request and response text with Patronus evaluators.
...
5. UnifAI calls the Patronus Evaluate API at `/v1/evaluate`.
6. If any evaluator returns `pass: false`, UnifAI returns `GUARDRAIL_INTERVENED`.

The Patronus evaluator flow supported here is text-based: UnifAI sends selected request or response text as the evaluation input.
...
| `base_url` | string | No | `https://api.patronus.ai` | Custom Patronus API base URL. UnifAI appends `/v1/evaluate`. |
...
The UnifAI dashboard exposes common Patronus evaluator presets:
...
<Frame>
  <img src="/media/ui-patronus-config.png" alt="Patronus AI configuration in UnifAI dashboard" />
</Frame>
...
8. Choose a **Capture** mode. UnifAI defaults to **None**.
