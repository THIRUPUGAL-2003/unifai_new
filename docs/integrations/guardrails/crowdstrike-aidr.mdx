...
description: "Integrate CrowdStrike AI Detection and Response with UnifAI Enterprise to inspect LLM inputs and outputs, block policy violations, redact sensitive content, and send AI security telemetry to AIDR."
icon: "/media/crowdstrike-nav.svg"
---

## Overview

UnifAI Enterprise supports **CrowdStrike AI Detection and Response (AIDR)** as a third-party guardrail provider for LLM request and response traffic.

Use it when your organization already manages AI security policies in CrowdStrike and you want UnifAI to enforce those policies inline before prompts reach an LLM and before model responses reach users.

CrowdStrike owns the detection policy. UnifAI owns the gateway enforcement path: it selects when to call AIDR, sends the relevant AI traffic, then blocks or rewrites the UnifAI request/response based on AIDR's verdict.
...
<Note>
UnifAI follows the AIDR policy response. Detector findings alone do not block traffic unless AIDR returns `blocked: true`. If you want UnifAI to stop a request, configure the relevant AIDR policy rule action to block.
</Note>
...
- An AIDR subscription: **AIDR for Workforce** or **AIDR for Agents**. For UnifAI gateway/application traffic, **AIDR for Agents** is the relevant subscription.
- A Falcon user with permission to manage AIDR collectors, typically the AIDR Admin role
- A CrowdStrike AIDR collector assigned to the policy you want UnifAI to enforce
- Network egress from UnifAI to the configured AIDR API URL over HTTPS
...
Before configuring UnifAI, create or open the CrowdStrike AIDR collector that UnifAI will use:

1. In the Falcon console, open the menu and go to **AI Detection and Response** > **Collectors**.
2. Create an **Application** collector for UnifAI, or open an existing collector your security team already created.
3. Configure the collector name, logging mode, and policy.
   - Assign a policy if you want AIDR to block or redact traffic.
   - If no policy is assigned, AIDR can still provide visibility, but UnifAI will not receive policy block/redaction decisions to enforce.
4. Save the collector.
5. Open the collector's **Config** tab.
6. Copy the **API token** into UnifAI as `api_key`.
7. Copy the **Base URL** into UnifAI as `base_url`.
...
1. Create a UnifAI guardrail provider with `provider_name: "crowdstrike-aidr"`.
2. Attach that provider configuration to one or more guardrail rules.
3. When a rule matches, UnifAI extracts text content and tool context from the request or response.
4. UnifAI calls AIDR at `{base_url}/v1/guard_chat_completions` with `event_type: "input"` or `event_type: "output"`.
5. AIDR evaluates the payload with the policy assigned to your collector.
6. If AIDR returns `blocked: true`, UnifAI returns `GUARDRAIL_INTERVENED` and does not continue that request/response path.
7. If AIDR returns `transformed: true` with `guard_output`, UnifAI applies the transformed text to the request or response.
8. If AIDR returns neither `blocked` nor `transformed`, UnifAI allows the original content through.

### Payload Sent To AIDR

UnifAI sends AIDR an OpenAI Chat Completions-shaped `guard_input` payload:
...
| `llm_provider` | Provider selected by UnifAI for the LLM request |
| `model` | Model requested through UnifAI |
...
| `base_url` | string | No | `https://api.crowdstrike.com/aidr/aiguard` | AIDR API base URL. UnifAI appends `/v1/guard_chat_completions`; it also accepts a value that already includes that suffix. |
...
<Frame>
  <img src="/media/ui-crowdstrike-aidr-config.png" alt="CrowdStrike AIDR configuration in UnifAI dashboard" />
</Frame>
...
UnifAI maps the AIDR response into UnifAI guardrail behavior like this:

| AIDR response | UnifAI behavior |
|---------------|------------------|
...
| Missing `result`, malformed response, timeout, non-2xx response, or transformed output count mismatch | Treats the provider call as failed. Check UnifAI logs for the exact guardrail error. |

UnifAI also records AIDR usage metadata for logs and spans:
...
When CrowdStrike AIDR blocks content, UnifAI returns HTTP `400` with `type: "guardrail_intervention"`. The error message uses the AIDR-provided reason when available.
...
<Note>
CrowdStrike AIDR inspects and can block streaming output after UnifAI accumulates the complete response. Streaming output transformation or redaction is not supported: if AIDR returns `transformed: true` for a streaming response, UnifAI ends the stream rather than applying `guard_output`.
</Note>
...
|---------|---------------|
| AIDR does not block a prompt | Confirm the AIDR policy action is set to block. UnifAI does not independently block on detector findings when `blocked` is false. |
| Redaction does not appear | Confirm AIDR returned `transformed: true` with `guard_output.messages`. UnifAI only rewrites content when the transformed output count matches the original text count. |
| AIDR returns `401` or `403` | Check the collector token and make sure UnifAI is using the token for the correct AIDR collector. |
...
| No AIDR findings are easy to correlate | Set `app_id` and `collector_instance_id`, and use UnifAI request logs/spans alongside the AIDR Findings page. |
