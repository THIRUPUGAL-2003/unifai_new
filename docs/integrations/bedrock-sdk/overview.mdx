---
title: "Overview"
description: "Use UnifAI as a Bedrock-compatible gateway for the Converse and Invoke APIs, with UnifAI features on top."
icon: "book"
---

## Overview

UnifAI provides a Bedrock-compatible endpoint for the **Converse** and **Invoke** APIs via protocol adaptation. The integration handles request transformation, response normalization, and error mapping between AWS Bedrock's API specification and UnifAI's internal processing pipeline.

This integration enables you to utilize UnifAI's features like governance, load balancing, semantic caching, multi-provider support, and more, all while preserving your existing Bedrock SDK-based architecture.
...
# Configure boto3 Bedrock client to use UnifAI
# Note: When using UnifAI keys, dummy credentials are required
# because boto3 needs credentials to sign requests, even though
# UnifAI will use its own configured keys.
client = boto3.client(
    service_name="bedrock-runtime",
    endpoint_url="http://localhost:8080/bedrock",
    region_name="us-west-2",
    aws_access_key_id="unifai-dummy-key",  # Required when using UnifAI keys
    aws_secret_access_key="unifai-dummy-secret"  # Required when using UnifAI keys
)
...
Because Bedrock itself is a multi-provider platform, you can use any Bedrock-supported model ID and still route through UnifAI. UnifAI will handle governance, observability, and other cross-cutting concerns.
...
client = boto3.client(
    ...
    endpoint_url="http://localhost:8080/bedrock",
    region_name="us-west-2",
    aws_access_key_id="unifai-dummy-key",
    aws_secret_access_key="unifai-dummy-secret"
)
...
Pass custom headers required by UnifAI plugins (like governance, telemetry, etc.) using boto3's event system:
...
def add_unifai_headers(request, **kwargs):
    """Add custom UnifAI headers to the request before signing."""
    request.headers.add_header("x-uf-vk", "vk_12345")  # Virtual key for governance
    request.headers.add_header("x-uf-env", "production")  # Environment tag
...
- All UnifAI core features that apply to these flows (governance, load balancing, semantic cache, observability, etc.)
