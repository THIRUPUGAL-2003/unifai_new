---
title: "Overview"
description: "Use UnifAI as a drop-in replacement for Google GenAI API with full compatibility and enhanced features."
icon: "book"
---

## Overview

UnifAI provides complete Google GenAI API compatibility through protocol adaptation. The integration handles request transformation, response normalization, and error mapping between Google's GenAI API specification and UnifAI's internal processing pipeline.

This integration enables you to utilize UnifAI's features like governance, load balancing, semantic caching, multi-provider support, and more, all while preserving your existing Google GenAI SDK-based architecture.
...
const genAI = new GoogleGenerativeAI("dummy-key", {
  baseUrl: "http://localhost:8080/genai", // Keys handled by UnifAI
});
...
When `thinkingConfig.thinkingBudget` is set to `-1`, UnifAI handles it differently per provider:
...
The Google GenAI integration supports all features that are available in both the Google GenAI SDK and UnifAI core functionality. If the Google GenAI SDK supports a feature and UnifAI supports it, the integration will work seamlessly.
