# AGENTS.md — UnifAI AI Gateway

> Context for AI agents (Claude Code, Copilot, Cursor, etc.) working on this codebase. Read this fully before making changes.

## What is UnifAI?

UnifAI is a high-performance AI gateway that unifies 20+ LLM providers behind a single OpenAI-compatible API with ~11µs overhead at 5,000 RPS. It also serves as an MCP (Model Context Protocol) gateway, turning static chat models into tool-calling agents.

GitHub: `unifai/unifai`
...
│   ├── unifai.go                  # Main struct, request queuing, provider lifecycle (~3.4K lines)
...
│   │   ├── unifai.go             # UnifAIConfig, ModelProvider enum, RequestType enum, context keys
...
│   │   ├── context.go             # UnifAIContext (custom context.Context with mutable values)
...
│   └── unifai-http/              # HTTP gateway transport
...
UnifAI is a **multi-module Go workspace**. Each module has its own `go.mod`:

```
go.work
├── core/go.mod              # github.com/unifai/unifai/core
├── framework/go.mod         # github.com/unifai/unifai/framework
├── transports/go.mod        # github.com/unifai/unifai/transports
...
make build                               # Build unifai-http binary
...
Client HTTP Request
  → FastHTTP Transport (parsing, validation ~2µs)
    → SDK Integration Layer (OpenAI/Anthropic/Bedrock format → UnifAI format)
...
- **Streaming**: SSE chunks flow through `chan chan *schemas.UnifAIStreamChunk`. Accumulated into full response for post-hooks via `framework/streaming/accumulator.go`.

### UnifAIContext — Custom Context

`UnifAIContext` (`core/schemas/context.go`) is a custom `context.Context` with **thread-safe mutable values**. Unlike standard Go contexts, values can be set after creation:

```go
ctx := schemas.NewUnifAIContext(parent, deadline)
ctx.SetValue(key, value)     // Thread-safe, uses RWMutex
ctx.WithValue(key, value)    // Chainable variant
```

**Reserved context keys** (set by UnifAI internals — DO NOT set manually):
- `UnifAIContextKeySelectedKeyID/Name` — Set by governance plugin
- `UnifAIContextKeyGovernance*` — Set by governance plugin
- `UnifAIContextKeyNumberOfRetries`, `UnifAIContextKeyFallbackIndex` — Set by retry/fallback logic
- `UnifAIContextKeyStreamEndIndicator` — Set by streaming infrastructure
- `UnifAIContextKeyTrace*`, `UnifAIContextKeySpan*` — Set by tracing middleware

**User-settable keys** (plugins and handlers can set these):
- `UnifAIContextKeyVirtualKey` (`x-uf-vk`) — Virtual key for governance
- `UnifAIContextKeyAPIKeyName` (`x-uf-api-key`) — Explicit key selection by name
- `UnifAIContextKeyAPIKeyID` (`x-uf-api-key-id`) — Explicit key selection by ID (takes priority over name)
- `UnifAIContextKeyRequestID` — Request ID
- `UnifAIContextKeyExtraHeaders` — Extra headers to forward to provider
- `UnifAIContextKeyURLPath` — Custom URL path for provider
- `UnifAIContextKeySkipKeySelection` — Skip key selection (pass empty key)
- `UnifAIContextKeyUseRawRequestBody` — Send raw body directly to provider
...
├── errors.go              # Error parsing: provider HTTP error → *schemas.UnifAIError
...
- `To<ProviderName><Feature>Request()` — UnifAI schema → Provider API format
- `ToUnifAI<Feature>Response()` — Provider API format → UnifAI schema
...
**Streaming methods** receive a `PostHookRunner` callback and return `chan *UnifAIStreamChunk`:
```go
ChatCompletionStream(ctx *UnifAIContext, postHookRunner PostHookRunner, key Key, request *UnifAIChatRequest) (chan *UnifAIStreamChunk, *UnifAIError)
```
...
```go
type ErrorConverter func(resp *fasthttp.Response, requestType schemas.RequestType, providerName schemas.ModelProvider, model string) *schemas.UnifAIError
```
...
```go
unifaiErr.ExtraFields.Provider = providerName
unifaiErr.ExtraFields.ModelRequested = model
...
- `UnifAIError.AllowFallbacks` controls whether fallback providers are tried: `nil` or `&true` = allow, `&false` = block
...
**SDK integration layers** (`transports/unifai-http/integrations/`) provide request/response converters between provider-native SDK formats and UnifAI's internal format. This enables drop-in replacement of OpenAI SDK, Anthropic SDK, AWS Bedrock SDK, Google GenAI SDK, LangChain, and LiteLLM.
...
3. Add `RequestType` constant in `core/schemas/unifai.go`
4. Add to `AllowedRequests` struct and `IsOperationAllowed()` switch
5. Add handler endpoint in `transports/unifai-http/handlers/`
6. Wire up in `core/unifai.go` and `core/inference.go`
...
### 11. UnifAIContext Reserved Keys Are Silently Dropped
...
UnifAI uses `github.com/valyala/fasthttp` for provider HTTP calls. The API is different from `net/http`:
...
`UnifAI` uses `atomic.Pointer` for providers and plugins lists. On updates: create new slice → atomically swap pointer. **Never mutate the slice in place** — concurrent readers would see partial state.
...
2. Add `ModelProvider` constant in `core/schemas/unifai.go`
3. Add to `StandardProviders` list in `core/schemas/unifai.go`
4. Register in `core/unifai.go` — add import + case in provider init switch
...
```go
func RunMyScenarioTest(t *testing.T, client *unifai.UnifAI, ctx context.Context, cfg ComprehensiveTestConfig) {
...
Investigate a GitHub issue from `unifai/unifai`. Fetches issue details, classifies by type/area, searches codebase, traces dependencies, analyzes side effects, suggests tests (LLM/MCP/E2E), and presents an implementation plan with per-change approval gates.
...
2. Map in provider response converter (`ToUnifAI*Response`)
...
3. Add to `go.work`
4. Register in transport layer or UnifAI config
...
| HTTP inference handler | `transports/unifai-http/handlers/inference.go` |
| Governance handler | `transports/unifai-http/handlers/governance.go` |
| Config schema (source of truth) | `transports/config.schema.json` |
| Pool debug profiler | `transports/unifai-http/handlers/devpprof.go` |
...
- **Context keys**: Use `UnifAIContextKey` type. Custom plugins should define their own key types to avoid collisions.
