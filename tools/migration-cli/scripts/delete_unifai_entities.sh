#!/usr/bin/env bash
#
# delete_unifai_entities.sh — delete all migration entities from a running UnifAI
...
#   DELETE_MODEL_CONFIGS=0 ./delete_unifai_entities.sh # skip model configs
#   DELETE_TEAMS=0 ./delete_unifai_entities.sh        # skip teams
#   DELETE_CUSTOMERS=0 ./delete_unifai_entities.sh     # skip customers
#   DELETE_USERS=0 ./delete_unifai_entities.sh         # skip users
#   DELETE_KEYS=0 ./delete_unifai_entities.sh          # skip provider keys
#   DELETE_PROVIDERS=0 ./delete_unifai_entities.sh     # skip providers
#
# Endpoints used (UnifAI management API, all require a management bearer token):
...
#
# Note: UnifAI exposes no batch delete for these entities, so each row is
...
if [[ -n "${UNIFAI_API_KEY}" ]]; then
  curl_args+=(--header "Authorization: Bearer ${UNIFAI_API_KEY}")
fi

# get issues an authenticated GET against a UnifAI path and prints the body.
get() {
  curl "${curl_args[@]}" --location "${UNIFAI_URL}$1"
}
...
delete_one() {
  ...
  if curl "${curl_args[@]}" --request DELETE --location "${UNIFAI_URL}${path}" >/dev/null; then
...
# its owned budgets and rate limits in UnifAI.
