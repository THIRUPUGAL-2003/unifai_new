...
func conformanceEnv(t *testing.T) MigrationRunConfig {
	...
	for _, key := range []string{
		...
		if strings.TrimSpace(os.Getenv(key)) == "" {
			t.Skipf("skipping LiteLLM→UnifAI conformance test: %s is not set", key)
		}
	}
...
// customBase is a real, resolvable host (UnifAI validates the base URL by
...
// wantProvider is an expected UnifAI provider: base provider type, base URL
...
// wantModelConfig is an expected global UnifAI model config.
type wantModelConfig struct {
	provider    string  // "" => all providers (nil in UnifAI)
...
func TestConformance_Models(t *testing.T) {
	...
	// envKey backs linked credentials; the UnifAI key is named after the
	// credential, so the literal masked value is all that matters.
	const envKey = "os.environ/OPENAI_API_KEY"
	// inlineKey backs inline deployments. UnifAI resolves env-ref key values at
	// create time (so the var must be in UnifAI's environment) AND names the key
	// after the env var, which must not collide with an existing UnifAI key — so
	// this is a real env var that is not already configured as a UnifAI key.
	...
	cases := []modelCase{
		...
		//     env-ref UnifAI resolves at create time. ------------------------
		{
...
// migrateModels runs the real transform and writes the result to UnifAI,
...
func (e MigrationRunConfig) migrateModels(t *testing.T, suffix string) {
	...
	plans, modelConfigs, _ := LiteLLMModelsToUnifAIProviders(models, credByName, deployments, e)

	for _, p := range plans {
		if err := e.UnifAIClient.EnsureProvider(ctx, p); err != nil {
		...
		for _, k := range p.Keys {
			if err := e.UnifAIClient.CreateProviderKey(ctx, p.Name, k); err != nil {
	...
	for _, mc := range modelConfigs {
		if err := e.UnifAIClient.CreateModelConfig(ctx, mc); err != nil {
...
// (and, for vllm, the served model in model_name). UnifAI masks the URL value
...
func (e MigrationRunConfig) assertModelConfig(t *testing.T, w wantModelConfig) {
	...
	if w.rpm > 0 {
		assertLimit(t, "rpm", w.rpm, rateLimitField(mc.RateLimit, func(r unifaiRateLimit) *int64 { return r.RequestMaxLimit }))
	}
	if w.tpm > 0 {
		assertLimit(t, "tpm", w.tpm, rateLimitField(mc.RateLimit, func(r unifaiRateLimit) *int64 { return r.TokenMaxLimit }))
	}
}
...
// maskedValue is UnifAI's wrapper for a secret-bearing field (key value, per-key
...
type unifaiModelConfig struct {
	...
	RateLimit *unifaiRateLimit `json:"rate_limit"`
}
...
func (e MigrationRunConfig) fetchProvider(t *testing.T, name string) (providerResponse, bool) {
	t.Helper()
	body, status, err := e.UnifAIClient.doRequest(context.Background(), http.MethodGet, "/api/providers/"+name, nil)
...
func (e MigrationRunConfig) fetchProviderKeys(t *testing.T, name string) providerKeysResponse {
	t.Helper()
	body, err := e.UnifAIClient.getJSON(context.Background(), "/api/providers/"+name+"/keys", "get provider keys "+name)
	if err != nil {
		t.Fatalf("%v", err)
	}
...
func (e MigrationRunConfig) fetchModelConfig(t *testing.T, model, provider string) (unifaiModelConfig, bool) {
	t.Helper()
	body, err := e.UnifAIClient.getJSON(context.Background(), "/api/governance/model-configs?search="+url.QueryEscape(model), "get model config "+model)
	if err != nil {
		t.Fatalf("%v", err)
	}
	var resp struct {
		ModelConfigs []unifaiModelConfig `json:"model_configs"`
	}
	...
	return unifaiModelConfig{}, false
}
...
func (e MigrationRunConfig) cleanupProvider(t *testing.T, name string) {
	...
	t.Cleanup(func() {
		_, _, _ = e.UnifAIClient.doRequest(context.Background(), http.MethodDelete, "/api/providers/"+name, nil)
	})
}
...
func (e MigrationRunConfig) cleanupStandardKey(t *testing.T, provider, keyName string) {
	...
	t.Cleanup(func() {
		for _, k := range e.fetchProviderKeys(t, provider).Keys {
			if k.Name == keyName {
				_, _, _ = e.UnifAIClient.doRequest(context.Background(), http.MethodDelete, "/api/providers/"+provider+"/keys/"+k.ID, nil)
			}
		}
	})
}
...
func (e MigrationRunConfig) cleanupModelConfig(t *testing.T, model string, provider *string) {
	...
	t.Cleanup(func() {
		if mc, ok := e.fetchModelConfig(t, model, want); ok {
			_, _, _ = e.UnifAIClient.doRequest(context.Background(), http.MethodDelete, "/api/governance/model-configs/"+mc.ID, nil)
		}
	})
}
...
func rateLimitField(rl *unifaiRateLimit, pick func(unifaiRateLimit) *int64) *int64 {
	if rl == nil {
		return nil
	}
	return pick(*rl)
