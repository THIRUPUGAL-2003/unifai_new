...
// GetDefaultConfigDir returns the OS-specific default configuration directory for UnifAI.
...
func GetDefaultConfigDir(appDir string) string {
	...
	case "windows":
		...
		if appData := os.Getenv("APPDATA"); appData != "" {
			configDir = filepath.Join(appData, "unifai")
		} else {
			// Fallback to user home directory
			if homeDir, err := os.UserHomeDir(); err == nil {
				configDir = filepath.Join(homeDir, "AppData", "Roaming", "unifai")
			}
		}
	default:
		// Linux, macOS and other Unix-like systems: ~/.config/unifai
		if homeDir, err := os.UserHomeDir(); err == nil {
			configDir = filepath.Join(homeDir, ".config", "unifai")
		}
	}

	// If we couldn't determine the config directory, fall back to current directory
	if configDir == "" {
		configDir = "./unifai-data"
	}
...
func (s *UnifAIHTTPServer) registerPluginWithStatus(ctx context.Context, name string, path *string, config any, failOnError bool) error {
...
func (s *UnifAIHTTPServer) CollectObservabilityPlugins() []schemas.ObservabilityPlugin {
...
func (s *UnifAIHTTPServer) updateKeyStatus(
	ctx context.Context,
