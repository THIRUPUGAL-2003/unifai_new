// Package server provides the HTTP server for UnifAI.
package server
...
import (
	...
	dynamicPlugins "github.com/unifai/unifai/framework/plugins"
...
// UnifAIHTTPServer represents a HTTP server instance.
type UnifAIHTTPServer struct {
...
// NewUnifAIHTTPServer creates a new instance of UnifAIHTTPServer.
func NewUnifAIHTTPServer(version string, uiContent embed.FS) *UnifAIHTTPServer {
	return &UnifAIHTTPServer{
		Version:        version,
		UIContent:      uiContent,
		Port:           DefaultPort,
		Host:           DefaultHost,
...
func (s *UnifAIHTTPServer) AddMCPClient(ctx context.Context, clientConfig *schemas.MCPClientConfig) error {
...
func (s *UnifAIHTTPServer) ReconnectMCPClient(ctx context.Context, id string) error {
	// Check if client is registered in UnifAI (can be not registered if client initialization failed)
	...
	// Config exists in store, but not in UnifAI (can happen if client initialization failed)
...
func (s *UnifAIHTTPServer) UpdateMCPClient(ctx context.Context, id string, updatedConfig *schemas.MCPClientConfig) error {
...
// UpdateMCPClientConnection reconnects an existing MCP client using updated headers
func (s *UnifAIHTTPServer) UpdateMCPClientConnection(ctx context.Context, id string, newConfig *schemas.MCPClientConfig) error {
...
func (s *UnifAIHTTPServer) RemoveMCPClient(ctx context.Context, id string) error {
...
func (s *UnifAIHTTPServer) DisableMCPClient(ctx context.Context, id string) error {
...
func (s *UnifAIHTTPServer) EnableMCPClient(ctx context.Context, id string) error {
...
// VerifyHeadersConnection delegates to the UnifAI client to verify an MCP
// server with caller-supplied header values and discover its tools.
func (s *UnifAIHTTPServer) VerifyHeadersConnection(ctx context.Context, config *schemas.MCPClientConfig, userHeaders map[string]string) (map[string]schemas.ChatTool, map[string]string, error) {
	return s.Client.VerifyHeadersConnection(ctx, config, userHeaders)
}

// VerifyPerUserOAuthConnection delegates to the UnifAI client to verify an MCP
// server using a temporary access token and discover available tools.
func (s *UnifAIHTTPServer) VerifyPerUserOAuthConnection(ctx context.Context, config *schemas.MCPClientConfig, accessToken string) (map[string]schemas.ChatTool, map[string]string, error) {
	return s.Client.VerifyPerUserOAuthConnection(ctx, config, accessToken)
}

// SetClientTools delegates to the UnifAI client to update tool map for an existing MCP client,
// then re-syncs the MCP server so the new tools are immediately visible via /mcp.
func (s *UnifAIHTTPServer) SetClientTools(clientID string, tools map[string]schemas.ChatTool, toolNameMapping map[string]string) {
...
func (s *UnifAIHTTPServer) ExecuteChatMCPTool(ctx context.Context, toolCall *schemas.ChatAssistantMessageToolCall) (*schemas.ChatMessage, *schemas.UnifAIError) {
	unifaiCtx := schemas.NewUnifAIContext(ctx, schemas.NoDeadline)
	return s.Client.ExecuteChatMCPTool(unifaiCtx, toolCall)
}

// ExecuteResponsesMCPTool executes an MCP tool call and returns the result as a responses message.
func (s *UnifAIHTTPServer) ExecuteResponsesMCPTool(ctx context.Context, toolCall *schemas.ResponsesToolMessage) (*schemas.ResponsesMessage, *schemas.UnifAIError) {
	unifaiCtx := schemas.NewUnifAIContext(ctx, schemas.NoDeadline)
	return s.Client.ExecuteResponsesMCPTool(unifaiCtx, toolCall)
}

func (s *UnifAIHTTPServer) GetAvailableMCPTools(ctx context.Context) []schemas.ChatTool {
	unifaiCtx := schemas.NewUnifAIContext(ctx, schemas.NoDeadline)
	return s.Client.GetAvailableMCPTools(unifaiCtx)
}

// markPluginDisabled marks a plugin as disabled in the plugin status
func (s *UnifAIHTTPServer) markPluginDisabled(name string) error {
...
func (s *UnifAIHTTPServer) getGovernancePluginName() string {
	if name, ok := s.Ctx.Value(schemas.UnifAIContextKeyGovernancePluginName).(string); ok && name != "" {
		return name
	}
...
func (s *UnifAIHTTPServer) getPromptsPluginName() string {
	if name, ok := s.Ctx.Value(schemas.UnifAIContextKeyPromptsPluginName).(string); ok && name != "" {
		return name
	}
...
func (s *UnifAIHTTPServer) getGovernancePlugin() (governance.BaseGovernancePlugin, error) {
...
func (s *UnifAIHTTPServer) ReloadVirtualKey(ctx context.Context, id string) (*tables.TableVirtualKey, error) {
...
func (s *UnifAIHTTPServer) RemoveVirtualKey(ctx context.Context, id string) error {
	governancePlugin, err := s.getGovernancePlugin()
	if err != nil {
		return err
	}
...
func (s *UnifAIHTTPServer) ReloadTeam(ctx context.Context, id string) (*tables.TableTeam, error) {
	// Load relationships for response
...
func (s *UnifAIHTTPServer) RemoveTeam(ctx context.Context, id string) error {
	governancePlugin, err := s.getGovernancePlugin()
	if err != nil {
		return err
	}
...
func (s *UnifAIHTTPServer) ReloadCustomer(ctx context.Context, id string) (*tables.TableCustomer, error) {
...
func (s *UnifAIHTTPServer) RemoveCustomer(ctx context.Context, id string) error {
	governancePlugin, err := s.getGovernancePlugin()
	if err != nil {
		return err
	}
...
func (s *UnifAIHTTPServer) ReloadModelConfig(ctx context.Context, id string) (*tables.TableModelConfig, error) {
...
func (s *UnifAIHTTPServer) RemoveModelConfig(ctx context.Context, id string) error {
	governancePlugin, err := s.getGovernancePlugin()
	if err != nil {
		return err
	}
...
func (s *UnifAIHTTPServer) ReloadProvider(ctx context.Context, provider schemas.ModelProvider) (*tables.TableProvider, error) {
	...
	if s.Client == nil {
		return nil, fmt.Errorf("unifai client not found")
	}
...
func (s *UnifAIHTTPServer) RemoveProvider(ctx context.Context, provider schemas.ModelProvider) error {
	...
	if err != nil && !errors.Is(err, lib.ErrNotFound) {
		logger.Error("failed to remove provider from config: %v. Client and config may be out of sync, please restart unifai", err)
		return fmt.Errorf("failed to remove provider from config: %w. Client and config may be out of sync, please restart unifai", err)
	}
	governancePlugin, err := s.getGovernancePlugin()
	if err != nil {
		return err
...
func (s *UnifAIHTTPServer) OnKeyAdded(ctx context.Context, provider schemas.ModelProvider, key schemas.Key) error {
	if s.Config == nil || s.Config.ModelCatalog == nil {
...
func (s *UnifAIHTTPServer) OnKeyUpdated(ctx context.Context, provider schemas.ModelProvider, key schemas.Key) error {
	if s.Config == nil || s.Config.ModelCatalog == nil {
		return fmt.Errorf("model catalog not found")
	}
	keys, err := s.Config.GetProviderKeysRaw(provider)
...
// entries stay valid.
func (s *UnifAIHTTPServer) OnKeyDeleted(ctx context.Context, provider schemas.ModelProvider, keyID string) error {
	if s.Config == nil || s.Config.ModelCatalog == nil {
		return fmt.Errorf("model catalog not found")
	}
	keys, err := s.Config.GetProviderKeysRaw(provider)
...
func (s *UnifAIHTTPServer) GetGovernanceData(ctx context.Context) *governance.GovernanceData {
	// Use type-safe finder from Config
...
func (s *UnifAIHTTPServer) ReloadRoutingRule(ctx context.Context, id string) error {
...
func (s *UnifAIHTTPServer) RemoveRoutingRule(ctx context.Context, id string) error {
	governancePluginName := governance.PluginName
	if name, ok := s.Ctx.Value(schemas.UnifAIContextKeyGovernancePluginName).(string); ok && name != "" {
		governancePluginName = name
	}
...
func (s *UnifAIHTTPServer) ReloadClientConfigFromConfigStore(ctx context.Context) error {
	if s.Config == nil || s.Config.ConfigStore == nil {
	...
	*s.Config.ClientConfig = *config
	...
	// Reloading config in unifai client
	if s.Client != nil {
		...
		s.Client.ReloadConfig(schemas.UnifAIConfig{
			Account:            account,
...
func (s *UnifAIHTTPServer) UpdateAuthConfig(ctx context.Context, authConfig *configstore.AuthConfig) error {
	if authConfig == nil {
...
func (s *UnifAIHTTPServer) UpdateDropExcessRequests(ctx context.Context, value bool) {
	if s.Config == nil {
		return
	}
...
func (s *UnifAIHTTPServer) UpdateMCPToolManagerConfig(ctx context.Context, maxAgentDepth int, toolExecutionTimeoutInSeconds int, codeModeBindingLevel string, disableAutoToolInject bool) error {
	if s.Config == nil {
...
func (s *UnifAIHTTPServer) UpdateSyncConfig(ctx context.Context) error {
	if s.Config == nil || s.Config.ModelCatalog == nil {
		return fmt.Errorf("pricing manager not found")
	}
...
func (s *UnifAIHTTPServer) RefreshLiveModelsForProvider(ctx context.Context, provider schemas.ModelProvider, keys []schemas.Key) {
	if len(keys) == 0 {
...
func (s *UnifAIHTTPServer) FetchAndStoreLiveForKey(ctx context.Context, provider schemas.ModelProvider, keyID string) {
	...
	// One UnifAIContext per goroutine. UnifAIContext.SetValue mutates state
	...
	newListModelsCtx := func() *schemas.UnifAIContext {
		c := schemas.NewUnifAIContext(ctx, time.Now().Add(15*time.Second))
		c.SetValue(schemas.UnifAIContextKeySkipPluginPipeline, true)
		c.SetValue(schemas.UnifAIContextKeyValidateKeys, true)
		return c
	}
	...
	go func() {
		...
		defer bfCtx.Cancel()
		resp, bfErr := s.Client.ListModelsRequest(bfCtx, &schemas.UnifAIListModelsRequest{
			Provider: provider,
			KeyID:    keyIDPtr,
		})
		if bfErr != nil {
			logger.Warn("filtered list-models failed for provider %s key %s: %v: falling back onto the static datasheet", provider, keyID, unifai.GetErrorMessage(bfErr))
	...
	go func() {
		...
		if bfErr != nil {
			logger.Warn("unfiltered list-models failed for provider %s key %s: %v: falling back onto the static datasheet", provider, keyID, unifai.GetErrorMessage(bfErr))
			return
		}
		if resp == nil {
			return
...
// pricing-only.
func (s *UnifAIHTTPServer) ForceReloadPricing(ctx context.Context) error {
	if s.Config == nil {
...
// pricing-only.
func (s *UnifAIHTTPServer) ReloadPricingFromDBAndPopulateModelPool(ctx context.Context) error {
	if s.Config == nil {
		return fmt.Errorf("server config not initialized")
	}
	if s.Config.ModelCatalog != nil {
...
func (s *UnifAIHTTPServer) UpsertPricingOverride(ctx context.Context, override *tables.TablePricingOverride) error {
	if s.Config == nil || s.Config.ModelCatalog == nil {
		return fmt.Errorf("pricing manager not found")
	}
...
func (s *UnifAIHTTPServer) DeletePricingOverride(ctx context.Context, id string) error {
	if s.Config == nil || s.Config.ModelCatalog == nil {
		return fmt.Errorf("pricing manager not found")
	}
...
// reload after commit.
func (s *UnifAIHTTPServer) UpsertModelPricingAttributes(ctx context.Context, entries []handlers.ModelPricingAttributesEntry) error {
	if s.Config == nil || s.Config.ModelCatalog == nil {
...
func (s *UnifAIHTTPServer) ReloadProxyConfig(ctx context.Context, config *tables.GlobalProxyConfig) error {
	if s.Config == nil {
		return fmt.Errorf("config not found")
	}
...
func (s *UnifAIHTTPServer) ReloadHeaderFilterConfig(ctx context.Context, config *tables.GlobalHeaderFilterConfig) error {
	if s.Config == nil {
		return fmt.Errorf("config not found")
	}
...
func (s *UnifAIHTTPServer) GetModelsForProvider(provider schemas.ModelProvider) []string {
	if s.Config == nil || s.Config.ModelCatalog == nil {
		return []string{}
	}
...
func (s *UnifAIHTTPServer) GetUnfilteredModelsForProvider(provider schemas.ModelProvider) []string {
	if s.Config == nil || s.Config.ModelCatalog == nil {
		return []string{}
	}
...
func (s *UnifAIHTTPServer) GetPluginStatus(ctx context.Context) map[string]schemas.PluginStatus {
...
func (s *UnifAIHTTPServer) GetLoadedPluginNames() []string {
	if s.Config == nil {
		return []string{}
	}
...
func (s *UnifAIHTTPServer) NormalizePluginConfig(name string, config map[string]any) (map[string]any, error) {
...
// Returns nil, nil when the plugin is not loaded or does not implement ConfigMarshallerPlugin.
func (s *UnifAIHTTPServer) ExpandPluginConfigForAPI(name string, config map[string]any) (map[string]any, error) {
	if m := s.Config.ConfigMarshallers.Load(); m != nil {
		if cm, ok := (*m)[name]; ok {
...
func (s *UnifAIHTTPServer) updatePluginErrorStatus(name, step string, originalErr error) error {
...
// SyncLoadedPlugin syncs a loaded plugin to the UnifAI client and updates the plugin status
func (s *UnifAIHTTPServer) SyncLoadedPlugin(ctx context.Context, name string, plugin schemas.BasePlugin, placement *schemas.PluginPlacement, order *int) error {
	...
	// 3. Update UnifAI client
	if err := s.Client.ReloadPlugin(plugin, InferPluginTypes(plugin)); err != nil {
		return s.updatePluginErrorStatus(plugin.GetName(), "reloading unifai config for", err)
	}
...
// ReloadPlugin reloads a plugin with new instance and updates UnifAI core.
...
func (s *UnifAIHTTPServer) ReloadPlugin(ctx context.Context, name string, path *string, pluginConfig any, placement *schemas.PluginPlacement, order *int) error {
...
func (s *UnifAIHTTPServer) RemovePlugin(ctx context.Context, displayName string) error {
	...
	// 2. Update UnifAI client
	if err := s.Client.RemovePlugin(name, InferPluginTypes(plugin)); err != nil {
		logger.Warn("failed to reload unifai config after plugin removal: %v", err)
	}
...
func (s *UnifAIHTTPServer) RegisterInferenceRoutes(ctx context.Context, middlewares ...schemas.UnifAIHTTPMiddleware) error {
...
// RegisterAPIRoutes initializes the routes for the UnifAI HTTP server.
func (s *UnifAIHTTPServer) RegisterAPIRoutes(ctx context.Context, callbacks ServerCallbacks, middlewares ...schemas.UnifAIHTTPMiddleware) error {
	var err error
	...
	governancePluginName := governance.PluginName
	if name, ok := ctx.Value(schemas.UnifAIContextKeyGovernancePluginName).(string); ok && name != "" {
		governancePluginName = name
	}
	...
	browserAIHandler.RegisterRoutes(s.Router, middlewares...)
...
func (s *UnifAIHTTPServer) RegisterUIRoutes(middlewares ...schemas.UnifAIHTTPMiddleware) {
...
func (s *UnifAIHTTPServer) GetAllRedactedKeys(ctx context.Context, ids []string) []schemas.Key {
	if s.Config == nil || s.Config.ConfigStore == nil {
		return nil
	}
...
func (s *UnifAIHTTPServer) GetAllRedactedVirtualKeys(ctx context.Context, ids []string) []tables.TableVirtualKey {
	if s.Config == nil || s.Config.ConfigStore == nil {
		return nil
	}
...
func (s *UnifAIHTTPServer) GetAllRedactedRoutingRules(ctx context.Context, ids []string) []tables.TableRoutingRule {
	if s.Config == nil || s.Config.ConfigStore == nil {
		return nil
	}
...
// PrepareCommonMiddlewares gets the common middlewares for the UnifAI HTTP server
func (s *UnifAIHTTPServer) PrepareCommonMiddlewares() []schemas.UnifAIHTTPMiddleware {
	commonMiddlewares := []schemas.UnifAIHTTPMiddleware{}
...
// Bootstrap initializes the UnifAI HTTP server with all necessary components.
// It:
// 1. Initializes Prometheus collectors for monitoring
// 2. Reads and parses configuration from the specified config file
// 3. Initializes the UnifAI client with the configuration
...
func (s *UnifAIHTTPServer) Bootstrap(ctx context.Context) error {
	var err error
	s.Ctx, s.cancel = schemas.NewUnifAIContextWithCancel(ctx)
	...
	if tableMCPConfig != nil {
		mcpConfig = s.Config.MCPConfig
		if mcpConfig != nil {
			mcpConfig.FetchNewRequestIDFunc = func(ctx *schemas.UnifAIContext) string {
				return uuid.New().String()
			}
		}
	}
	// Initialize unifai client
	...
	account := lib.NewBaseAccount(s.Config)
	s.Client, err = unifai.Init(ctx, schemas.UnifAIConfig{
		Account:            account,
		InitialPoolSize:    s.Config.ClientConfig.InitialPoolSize,
		DropExcessRequests: s.Config.ClientConfig.DropExcessRequests,
		LLMPlugins:         s.Config.GetLoadedLLMPlugins(),
	...
	if err != nil {
		return fmt.Errorf("failed to initialize unifai: %v", err)
	}
	logger.Info("unifai client initialized")
	...
	s.Config.SetUnifAIClient(s.Client)
	// Initialize routes
	s.Router = router.New()
	...
	if ctx.Value(schemas.UnifAIContextKeyIsEnterprise) == nil && s.AuthMiddleware != nil {
	...
	inferenceMiddlewares = append([]schemas.UnifAIHTTPMiddleware{handlers.TransportInterceptorMiddleware(s.Config)}, inferenceMiddlewares...)
	inferenceMiddlewares = append([]schemas.UnifAIHTTPMiddleware{s.TracingMiddleware.Middleware()}, inferenceMiddlewares...)
	...
	// Construction (unifai.Init) no longer connects MCP, so connecting here ensures
...
func (s *UnifAIHTTPServer) Start() error {
	...
	go func() {
		logger.Info("successfully started unifai, serving UI on http://%s", serverAddr)
	...
	case sig := <-sigChan:
		...
		go func() {
			defer close(done)
			logger.Info("shutting down unifai client...")
			s.Client.Shutdown()
			logger.Info("unifai client shutdown completed")
