...
//
// Integration Support:
// UnifAI supports multiple AI provider integrations through dedicated HTTP endpoints.
// Each integration exposes API-compatible endpoints that accept the provider's native request format,
// automatically convert it to UnifAI's unified format, process it, and return the expected response format.
//
...
// from UnifAI's unified model routing, fallbacks, monitoring capabilities, and high-performance configuration management.
//
...
import (
	...
	"github.com/unifai/unifai/transports/unifai-http/profiling"
	unifaiServer "github.com/unifai/unifai/transports/unifai-http/server"
)
...
var logger = unifai.NewDefaultLogger(schemas.LogLevelInfo)
var server *unifaiServer.UnifAIHTTPServer
...
func init() {
	...
	defaultHost := os.Getenv("UNIFAI_HOST")
	if defaultHost == "" {
		defaultHost = unifaiServer.DefaultHost
	}
	defaultLogLevel := strings.ToLower(os.Getenv("LOG_LEVEL"))
	if defaultLogLevel == "" {
		defaultLogLevel = unifaiServer.DefaultLogLevel
	}
	// Initializing server
	server = unifaiServer.NewUnifAIHTTPServer(Version, uiContent)
	// Updating server properties from flags
	flag.StringVar(&server.Port, "port", unifaiServer.DefaultPort, "Port to run the server on")
	flag.StringVar(&server.Host, "host", defaultHost, "Host to bind the server to (default: localhost, override with UNIFAI_HOST env var)")
	flag.StringVar(&server.AppDir, "app-dir", unifaiServer.DefaultAppDir, "Application data directory (contains config.json and logs)")
	flag.StringVar(&server.LogLevel, "log-level", defaultLogLevel, "Logger level (debug, info, warn, error). Default is info.")
	flag.StringVar(&server.LogOutputStyle, "log-style", unifaiServer.DefaultLogOutputStyle, "Logger output type (json or pretty). Default is JSON.")
}
...
║═══════════════════════════════════════════════════════════║
║             https://github.com/unifai/unifai            ║
╚═══════════════════════════════════════════════════════════╝
...
`, versionLine)
	...
	logger.Info("Time spent in UnifAI server bootstrap %d ms", time.Since(t).Milliseconds())
	err = server.Start()
	if err != nil {
