...
// Start launches the pprof debug server when UNIFAI_PPROF_PORT is set and
...
func Start() *http.Server {
	port := os.Getenv("UNIFAI_PPROF_PORT")
	if port == "" {
	...
	//   UNIFAI_PPROF_BLOCK_RATE    nanoseconds blocked per sample (lower = more overhead); default 10µs
	//   UNIFAI_PPROF_MUTEX_FRACTION 1/N contention events sampled (lower = more overhead); default 1%
	blockRate := 10000
	if v, err := strconv.Atoi(os.Getenv("UNIFAI_PPROF_BLOCK_RATE")); err == nil && v > 0 {
		blockRate = v
	}
	mutexFraction := 100
	if v, err := strconv.Atoi(os.Getenv("UNIFAI_PPROF_MUTEX_FRACTION")); err == nil && v > 0 {
		mutexFraction = v
	}
	...
	// deliberately opts in via UNIFAI_PPROF_HOST (e.g. "0.0.0.0").
	host := os.Getenv("UNIFAI_PPROF_HOST")
	if host == "" {
		host = "127.0.0.1"
	}
	...
	if !isLoopbackHost(host) &&
		(os.Getenv("UNIFAI_PPROF_USERNAME") == "" || os.Getenv("UNIFAI_PPROF_PASSWORD") == "") {
		log.Printf("pprof: refusing to expose profiling on non-loopback host %q without UNIFAI_PPROF_USERNAME and UNIFAI_PPROF_PASSWORD", host)
		return nil
	}
...
// basicAuth wraps h with HTTP Basic Auth when both UNIFAI_PPROF_USERNAME and
// UNIFAI_PPROF_PASSWORD are set. If either is unset/empty, h is returned
...
func basicAuth(h http.Handler) http.Handler {
	wantUser := os.Getenv("UNIFAI_PPROF_USERNAME")
	wantPass := os.Getenv("UNIFAI_PPROF_PASSWORD")
