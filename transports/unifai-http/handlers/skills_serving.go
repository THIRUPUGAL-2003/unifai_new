...
func (h *SkillsServingHandler) RegisterRoutes(r *router.Router, middlewares ...schemas.UnifAIHTTPMiddleware) {
...
func (h *SkillsServingHandler) claudeCodeMarketplace(ctx *fasthttp.RequestCtx) {
	...
	if len(skills) > 0 {
		plugins = append(plugins, map[string]any{
			...
			"description": "All UnifAI skills bundled in a single plugin.",
...
// Plugin names in the URL are prefixed with "unifai-". The special name
// "unifai-all-skills" serves a bundled repo containing every skill.
func (h *SkillsServingHandler) servePluginGit(harness string) fasthttp.RequestHandler {
	return func(ctx *fasthttp.RequestCtx) {
		...
		// Strip the "unifai-" prefix to look up the actual skill name.
...
func exportToTempBareRepo(memStorage *memory.Storage) (string, error) {
	tempDir, err := os.MkdirTemp("", "unifai-skill-*")
	if err != nil {
