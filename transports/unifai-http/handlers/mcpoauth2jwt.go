...
// jwtMCPClaims are the custom claims embedded in UnifAI-issued /mcp JWTs.
...
// string when the header is absent or the token is a VK (starts with "sk-uf-").
func extractBearerJWT(ctx *fasthttp.RequestCtx) string {
	...
	// VKs start with the "sk-uf-" prefix. Anything not starting with "eyJ" is
...
// verifyMCPJWT parses and verifies a UnifAI-issued JWT for the /mcp endpoint.
...
//
// uf_mode=user    → UnifAIContextKeyUserID
// uf_mode=vk      → UnifAIContextKeyVirtualKey (governance derives the VK row ID from it)
// uf_mode=session → UnifAIContextKeyMCPSessionID
func injectJWTContext(unifaiCtx *schemas.UnifAIContext, claims *jwtMCPClaims, vk *configtables.TableVirtualKey) error {
	sub := claims.Subject
	if sub == "" {
	...
	case schemas.MCPAuthModeUser:
		unifaiCtx.SetValue(schemas.UnifAIContextKeyUserID, sub)
	case schemas.MCPAuthModeVK:
		...
		// the VK row ID (UnifAIContextKeyGovernanceVirtualKeyID) on the connect
		...
		unifaiCtx.SetValue(schemas.UnifAIContextKeyVirtualKey, vk.Value.GetValue())
	case schemas.MCPAuthModeSession:
		unifaiCtx.SetValue(schemas.UnifAIContextKeyMCPSessionID, sub)
	default:
