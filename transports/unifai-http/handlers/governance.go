...
type complexityAnalyzerConfigReloader interface {
	// HTTP server bridge signature: UnifAIHTTPServer implements this and adapts
...
func (h *GovernanceHandler) RegisterRoutes(r *router.Router, middlewares ...schemas.UnifAIHTTPMiddleware) {
	...
	r.DELETE("/api/governance/virtual-keys/{vk_id}/users", lib.ChainMiddlewares(h.deleteVirtualKeyUser, middlewares...))
...
func (h *GovernanceHandler) updateComplexityAnalyzerConfig(ctx *fasthttp.RequestCtx) {
	...
	if err := h.reloadComplexityAnalyzerConfig(ctx, normalized); err != nil {
		SendError(ctx, fasthttp.StatusInternalServerError, fmt.Sprintf("failed to reload complexity analyzer config in memory: %v, please restart unifai to sync with the database", err))
		return
	}
...
func (h *GovernanceHandler) createRoutingRule(ctx *fasthttp.RequestCtx) {
	...
	if err := h.governanceManager.ReloadRoutingRule(ctx, rule.ID); err != nil {
		SendError(ctx, 500, fmt.Sprintf("Failed to reload routing rule in memory: %v, please restart unifai to sync with the database", err))
		return
	}
...
func (h *GovernanceHandler) getVirtualKeyQuota(ctx *fasthttp.RequestCtx) {
	...
	var vkValue string
	if v := string(ctx.Request.Header.Peek("x-uf-vk")); v != "" {
		vkValue = v
	} else if v := governance.ParseVirtualKeyFromFastHTTPRequest(ctx); v != nil {
		vkValue = *v
	}
	if vkValue == "" {
		SendError(ctx, 401, "Missing virtual key. Provide it via x-uf-vk header, Authorization Bearer, x-api-key, or x-goog-api-key header.")
		return
	}
