...
func TestConsentFlowSubmit_VK(t *testing.T) {
	...
	inactiveVK := &configtables.TableVirtualKey{ID: "vk-row-2", Value: *schemas.NewSecretVar("sk-uf-inactive"), IsActive: new(false)}

	t.Run("active VK mints a code", func(t *testing.T) {
		store := newConsentStore()
		store.vksByValue["sk-uf-active"] = activeVK
		seedPendingFlow(store, "flow-1", time.Now().Add(time.Minute))
		h := newConsentHandler(store, nil, false)
		ctx := consentCtx("flow-1", `{"mode":"vk","value":"sk-uf-active"}`)
		h.flowSubmit(ctx)
	...
	t.Run("inactive VK is rejected", func(t *testing.T) {
		...
		seedPendingFlow(store, "flow-1", time.Now().Add(time.Minute))
		h := newConsentHandler(store, nil, false)
		ctx := consentCtx("flow-1", `{"mode":"vk","value":"sk-uf-inactive"}`)
		h.flowSubmit(ctx)
		assert.Equal(t, fasthttp.StatusBadRequest, ctx.Response.StatusCode())
	})
	...
	t.Run("unknown VK is rejected", func(t *testing.T) {
		...
		seedPendingFlow(store, "flow-1", time.Now().Add(time.Minute))
		h := newConsentHandler(store, nil, false)
		ctx := consentCtx("flow-1", `{"mode":"vk","value":"sk-uf-missing"}`)
		h.flowSubmit(ctx)
		assert.Equal(t, fasthttp.StatusBadRequest, ctx.Response.StatusCode())
	})
	...
	t.Run("double submit returns 410 on the second attempt", func(t *testing.T) {
		...
		first := consentCtx("flow-1", `{"mode":"vk","value":"sk-uf-active"}`)
		h.flowSubmit(first)
		require.Equal(t, fasthttp.StatusOK, first.Response.StatusCode())

		second := consentCtx("flow-1", `{"mode":"vk","value":"sk-uf-active"}`)
		h.flowSubmit(second)
...
func TestConsentFlowSubmit_VKUserBinding(t *testing.T) {
	boundVK := &configtables.TableVirtualKey{ID: "vk-row-1", Value: *schemas.NewSecretVar("sk-uf-bound"), IsActive: new(true)}
	...
	t.Run("bound VK upgrades to user when logged-in user matches", func(t *testing.T) {
		...
		ctx := consentCtx("flow-1", `{"mode":"vk","value":"sk-uf-bound"}`)
		h.flowSubmit(ctx)
		require.Equal(t, fasthttp.StatusOK, ctx.Response.StatusCode(), string(ctx.Response.Body()))
		assert.Equal(t, "user", store.authReqs["flow-1"].BfMode)
