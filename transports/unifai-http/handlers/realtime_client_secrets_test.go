...
func TestCacheRealtimeEphemeralKeyMappingStoresKeyID(t *testing.T) {
	...
	cacheRealtimeEphemeralKeyMapping(store, body, "key_123", "sk-uf-test")
	...
	if mapping.VirtualKey != "sk-uf-test" {
		t.Fatalf("mapping.VirtualKey = %q, want %q", mapping.VirtualKey, "sk-uf-test")
	}
}
...
func (m *mockRealtimeMintingGovernancePlugin) EvaluateGovernanceRequest(ctx *schemas.UnifAIContext, evaluationRequest *governance.EvaluationRequest, _ schemas.RequestType) (*governance.EvaluationResult, *schemas.UnifAIError) {
	m.evaluateCalls++
	m.seenUserID = ""
	m.seenVirtualKey = ""
	m.seenProvider = ""
	...
	if ctx != nil && m.seenVirtualKey == "" {
		m.seenVirtualKey = unifai.GetStringFromContext(ctx, schemas.UnifAIContextKeyVirtualKey)
	}
	if m.err != nil {
		return nil, m.err
	}
...
func (m *mockRealtimeMintingGovernancePlugin) HTTPTransportPreHook(_ *schemas.UnifAIContext, _ *schemas.HTTPRequest) (*schemas.HTTPResponse, error) {
	return nil, nil
}

func (m *mockRealtimeMintingGovernancePlugin) HTTPTransportPostHook(_ *schemas.UnifAIContext, _ *schemas.HTTPRequest, _ *schemas.HTTPResponse) error {
	return nil
}

func (m *mockRealtimeMintingGovernancePlugin) PreRequestHook(_ *schemas.UnifAIContext, _ *schemas.UnifAIRequest) error {
	return nil
}

func (m *mockRealtimeMintingGovernancePlugin) PreLLMHook(_ *schemas.UnifAIContext, req *schemas.UnifAIRequest) (*schemas.UnifAIRequest, *schemas.LLMPluginShortCircuit, error) {
	return req, nil, nil
}

func (m *mockRealtimeMintingGovernancePlugin) PostLLMHook(_ *schemas.UnifAIContext, result *schemas.UnifAIResponse, unifaiErr *schemas.UnifAIError) (*schemas.UnifAIResponse, *schemas.UnifAIError, error) {
	return result, unifaiErr, nil
}

func (m *mockRealtimeMintingGovernancePlugin) PreMCPHook(_ *schemas.UnifAIContext, req *schemas.UnifAIMCPRequest) (*schemas.UnifAIMCPRequest, *schemas.MCPPluginShortCircuit, error) {
	return req, nil, nil
}

func (m *mockRealtimeMintingGovernancePlugin) PostMCPHook(_ *schemas.UnifAIContext, resp *schemas.UnifAIMCPResponse, unifaiErr *schemas.UnifAIError) (*schemas.UnifAIMCPResponse, *schemas.UnifAIError, error) {
...
func TestRealtimeClientSecretsEvaluateMintingGovernance_RequiresAccess(t *testing.T) {
	...
	plugin := &mockRealtimeMintingGovernancePlugin{
		err: &schemas.UnifAIError{
			...
			Error: &schemas.ErrorField{
				Message: "virtual key is required. Provide a virtual key via the x-uf-vk header.",
			},
		},
	}
	...
	defer unifaiCtx.Done()

	err := handler.evaluateMintingGovernance(unifaiCtx, schemas.OpenAI, "gpt-realtime")
	if err == nil {
...
func TestRealtimeClientSecretsEvaluateMintingGovernance_PassesContext(t *testing.T) {
	...
	unifaiCtx := schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline)
	defer unifaiCtx.Done()
	unifaiCtx.SetValue(schemas.UnifAIContextKeyUserID, "user_123")
	unifaiCtx.SetValue(schemas.UnifAIContextKeyVirtualKey, "sk-uf-123")

	if err := handler.evaluateMintingGovernance(unifaiCtx, schemas.OpenAI, "gpt-realtime"); err != nil {
	...
	if plugin.seenVirtualKey != "sk-uf-123" {
		t.Fatalf("virtual key = %q, want %q", plugin.seenVirtualKey, "sk-uf-123")
	}
