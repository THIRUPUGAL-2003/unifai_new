...
func NewWebRTCRealtimeHandler(client *unifai.UnifAI, config *lib.Config) *WebRTCRealtimeHandler {
...
func (h *WebRTCRealtimeHandler) RegisterRoutes(r *router.Router, middlewares ...schemas.UnifAIHTTPMiddleware) {
	handler := lib.ChainMiddlewares(h.handleRequest, middlewares...)

	// Base unifai route — GA /calls format (multipart sdp + session)
	r.POST("/v1/realtime/calls", handler)

	// Base unifai route — legacy format (raw SDP or multipart on /v1/realtime)
...
func (h *WebRTCRealtimeHandler) handleCallsRequest(ctx *fasthttp.RequestCtx) {
	sdpOffer, providerKey, model, normalizedSession, unifaiErr := parseCallsWebRTCRequest(ctx, h.config)
	if unifaiErr != nil {
		SendUnifAIError(ctx, unifaiErr)
		return
	}

	rtProvider, unifaiErr := h.resolveWebRTCProvider(providerKey)
	if unifaiErr != nil {
		SendUnifAIError(ctx, unifaiErr)
		return
	}

	exchangeSDP := func(rCtx *schemas.UnifAIContext, key schemas.Key, upstreamOffer string) (string, *schemas.UnifAIError) {
...
func parseCallsWebRTCRequest(ctx *fasthttp.RequestCtx, config *lib.Config) (string, schemas.ModelProvider, string, []byte, *schemas.UnifAIError) {
	...
	if strings.HasPrefix(contentType, "multipart/form-data") {
		...
		providerKey, model, normalizedSession, unifaiErr := resolveRealtimeSDPTarget(ctx, config, path, []byte(sessionField))
		if unifaiErr != nil {
...
func (h *WebRTCRealtimeHandler) handleLegacyRequest(ctx *fasthttp.RequestCtx, defaultProvider schemas.ModelProvider) {
	sdpOffer, rawModel, sessionJSON, unifaiErr := parseLegacyWebRTCRequest(ctx, defaultProvider)
	if unifaiErr != nil {
		SendUnifAIError(ctx, unifaiErr)
		return
	}
	...
	if providerKey == "" || model == "" {
		SendUnifAIError(ctx, newRealtimeWebRTCError(fasthttp.StatusBadRequest, "invalid_request_error", "invalid model: "+rawModel, nil))
		return
	}
	...
	if !ok {
		SendUnifAIError(ctx, newRealtimeWebRTCError(fasthttp.StatusBadRequest, "invalid_request_error", "provider does not support legacy realtime WebRTC: "+string(providerKey), nil))
		return
	}
...
func parseLegacyWebRTCRequest(ctx *fasthttp.RequestCtx, defaultProvider schemas.ModelProvider) (sdpOffer, rawModel string, sessionJSON json.RawMessage, err *schemas.UnifAIError) {
...
// runWebRTCRelay is the shared relay setup: creates unifai context, selects key, establishes relay.
func (h *WebRTCRealtimeHandler) runWebRTCRelay(
	...
	model string,
	sdpOffer string,
	exchangeSDP func(ctx *schemas.UnifAIContext, key schemas.Key, upstreamOffer string) (string, *schemas.UnifAIError),
) {
	...
	// ConvertToUnifAIContext creates a fresh context that doesn't carry the user
	// values the middleware stored on the fasthttp RequestCtx via SetUserValue.
	applyRealtimeMiddlewareValues(unifaiCtx, snapshotRealtimeMiddlewareValues(ctx))
	...
	authKey, selectedKey, err := h.resolveRealtimeWebRTCKeys(ctx, unifaiCtx, providerKey, model)
	if err != nil {
		SendUnifAIError(ctx, newRealtimeWebRTCError(fasthttp.StatusBadRequest, "invalid_request_error", err.Error(), nil))
		return
	}
	...
	boundExchange := func(rCtx *schemas.UnifAIContext, upstreamOffer string) (string, *schemas.UnifAIError) {
		return exchangeSDP(rCtx, authKey, upstreamOffer)
	}

	relayCtx, relayCancel := newRealtimeRelayContext(unifaiCtx)
...
func (h *WebRTCRealtimeHandler) resolveRealtimeWebRTCKeys(
	...
	if isRealtimeEphemeralToken(inboundToken) && !mapped {
		unifaiCtx.ClearValue(schemas.UnifAIContextKeyAPIKeyID)
		unifaiCtx.ClearValue(schemas.UnifAIContextKeyAPIKeyName)
		unifaiCtx.ClearValue(schemas.UnifAIContextKeySelectedKeyID)
		unifaiCtx.ClearValue(schemas.UnifAIContextKeySelectedKeyName)
		authKey := schemas.Key{Value: *schemas.NewSecretVar(inboundToken)}
		return authKey, nil, nil
	}

	selectedKey, err := h.client.SelectKeyForProviderRequestType(unifaiCtx, schemas.RealtimeRequest, providerKey, model)
...
func applyRealtimeEphemeralKeyMapping(unifaiCtx *schemas.UnifAIContext, mapping realtimeEphemeralKeyMapping) {
	if unifaiCtx == nil {
		return
	}
	if mapping.VirtualKey != "" {
		unifaiCtx.SetValue(schemas.UnifAIContextKeyVirtualKey, mapping.VirtualKey)
	}
	if mapping.KeyID != "" {
		unifaiCtx.SetValue(schemas.UnifAIContextKeyAPIKeyID, mapping.KeyID)
	}
}
...
func (h *WebRTCRealtimeHandler) resolveWebRTCProvider(providerKey schemas.ModelProvider) (schemas.RealtimeProvider, *schemas.UnifAIError) {
...
func (h *WebRTCRealtimeHandler) establishRelay(
	relayCtx *schemas.UnifAIContext,
	...
	key *schemas.Key,
	browserOffer string,
	exchangeSDP func(ctx *schemas.UnifAIContext, upstreamOffer string) (string, *schemas.UnifAIError),
	...
	providerToBrowserTrack, err := webrtc.NewTrackLocalStaticRTP(defaultAudioCodec, "audio", "unifai-provider-audio")
	if err != nil {
		relay.close()
	...
	browserToProviderTrack, err := webrtc.NewTrackLocalStaticRTP(defaultAudioCodec, "audio", "unifai-browser-audio")
	if err != nil {
		relay.close()
...
func (r *webrtcRealtimeRelay) handleDownstreamMessage(msg webrtc.DataChannelMessage) {
	...
	if startsTurn {
		if r.session.PeekRealtimeTurnHooks() != nil {
			r.sendDownstream(newRealtimeTurnErrorEventPayload(newRealtimeWireUnifAIError(400, "invalid_request_error", "Conversation already has an active response in progress.")), true)
			return
		}
		if unifaiErr := startRealtimeTurnHooks(r.client, r.unifaiCtx, r.session, r.provider, r.providerKey, r.model, r.key, event.Type); unifaiErr != nil {
			r.closeWithErrorEvent(newRealtimeTurnErrorEventPayload(unifaiErr))
			return
		}
	}
	...
	if err != nil {
		if startsTurn {
			...
			r.closeWithErrorEvent(newRealtimeTurnErrorEventPayload(newRealtimeWireUnifAIError(400, "invalid_request_error", err.Error())))
			return
		}
...
func (r *webrtcRealtimeRelay) handleUpstreamMessage(msg webrtc.DataChannelMessage) {
	event, err := r.provider.ToUnifAIRealtimeEvent(msg.Data)
	if err != nil {
		if finalizeErr := finalizeRealtimeTurnHooksOnTransportError(
			r.client,
			r.unifaiCtx,
		...
		r.closeWithErrorEvent(newRealtimeTurnErrorEventPayload(newRealtimeWireUnifAIError(502, "server_error", "failed to translate upstream realtime event")))
		return
	}
	if event != nil {
	...
	if event != nil {
		...
		if event.Type == r.provider.RealtimeTurnFinalEvent() {
			...
			if unifaiErr := finalizeRealtimeTurnHooks(r.client, r.unifaiCtx, r.session, r.provider, r.providerKey, r.model, r.key, msg.Data, contentOverride); unifaiErr != nil {
				r.closeWithErrorEvent(newRealtimeTurnErrorEventPayload(unifaiErr))
				return
			}
			...
			if finalizeErr := finalizeRealtimeTurnHooksWithError(
				...
				event.Type,
				msg.Data,
				newUnifAIErrorFromRealtimeError(r.providerKey, r.model, msg.Data, event.Error),
			); finalizeErr != nil {
				r.closeWithErrorEvent(newRealtimeTurnErrorEventPayload(finalizeErr))
				return
			}
		...
		if err != nil {
			...
			r.closeWithErrorEvent(newRealtimeTurnErrorEventPayload(
				newRealtimeWireUnifAIError(502, "server_error", "failed to encode translated realtime event: "+err.Error()),
			))
			return
		}
	}
...
func newRealtimeRelayContext(requestCtx *schemas.UnifAIContext) (*schemas.UnifAIContext, context.CancelFunc) {
	relayCtx, cancel := schemas.NewUnifAIContextWithCancel(context.Background())
	if requestCtx == nil {
		return relayCtx, cancel
	}

	for _, key := range []any{
		schemas.UnifAIContextKeyRequestID,
		schemas.UnifAIContextKeyHTTPRequestType,
		schemas.UnifAIContextKeyIntegrationType,
		schemas.UnifAIContextKeyParentRequestID,
		schemas.UnifAIContextKeyVirtualKey,
		schemas.UnifAIContextKeyAPIKeyName,
		schemas.UnifAIContextKeyAPIKeyID,
		schemas.UnifAIContextKeyExtraHeaders,
		schemas.UnifAIContextKeyRequestHeaders,
		schemas.UnifAIContextKeyUserAgent,
		...
		schemas.UnifAIContextKeyIsEnterprise,
		...
		schemas.UnifAIContextKeyAllowPerRequestStorageOverride,
		schemas.UnifAIContextKeyAllowPerRequestRawOverride,
		schemas.UnifAIContextKeyStoreRawRequestResponse,
		schemas.UnifAIContextKeyDisableContentLogging,
	...
	relayCtx.SetValue(schemas.UnifAIContextKeyRealtimeTransport, "webrtc")
...
func resolveRealtimeSDPTarget(ctx *fasthttp.RequestCtx, config *lib.Config, path string, sessionJSON []byte) (schemas.ModelProvider, string, []byte, *schemas.UnifAIError) {
...
func newRealtimeWebRTCError(status int, errorType, message string, err error) *schemas.UnifAIError {
