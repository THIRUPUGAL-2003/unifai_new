...
// ConfigHandler manages runtime configuration updates for UnifAI.
...
// It requires the UnifAI client, a logger, and the config store.
...
func (h *ConfigHandler) RegisterRoutes(r *router.Router, middlewares ...schemas.UnifAIHTTPMiddleware) {
...
func (h *ConfigHandler) updateConfig(ctx *fasthttp.RequestCtx) {
	...
	// Toggle allowing direct key bypass via x-uf-direct-key header
	...
	if frameworkConfig == nil {
		frameworkConfig = &configstoreTables.TableFrameworkConfig{
			ID:                     0,
			PricingURL:             unifai.Ptr(modelcatalog.DefaultPricingURL),
			PricingSyncInterval:    unifai.Ptr(int64(modelcatalog.DefaultSyncInterval.Seconds())),
			ModelParametersURL:     unifai.Ptr(modelcatalog.DefaultModelParametersURL),
			MCPLibraryURL:          unifai.Ptr(modelcatalog.DefaultMCPLibraryURL),
			MCPLibrarySyncInterval: unifai.Ptr(int64(modelcatalog.DefaultSyncInterval.Seconds())),
		}
	}
	// Handling individual nil cases
	if frameworkConfig.PricingURL == nil {
		frameworkConfig.PricingURL = unifai.Ptr(modelcatalog.DefaultPricingURL)
	}
	if frameworkConfig.PricingSyncInterval == nil {
		frameworkConfig.PricingSyncInterval = unifai.Ptr(int64(modelcatalog.DefaultSyncInterval.Seconds()))
	}
	if frameworkConfig.ModelParametersURL == nil {
		frameworkConfig.ModelParametersURL = unifai.Ptr(modelcatalog.DefaultModelParametersURL)
	}
	if frameworkConfig.MCPLibraryURL == nil {
		frameworkConfig.MCPLibraryURL = unifai.Ptr(modelcatalog.DefaultMCPLibraryURL)
	}
	if frameworkConfig.MCPLibrarySyncInterval == nil {
		frameworkConfig.MCPLibrarySyncInterval = unifai.Ptr(int64(modelcatalog.DefaultSyncInterval.Seconds()))
	}
	...
	if payload.AuthConfig != nil {
		...
		// Note: AuthMiddleware is updated via ServerCallbacks.UpdateAuthConfig (handled by UnifAIHTTPServer)
	}
