...
func TestChainMiddlewares_SingleMiddleware(t *testing.T) {
	...
	middleware := schemas.UnifAIHTTPMiddleware(func(next fasthttp.RequestHandler) fasthttp.RequestHandler {
...
func TestChainMiddlewares_MultipleMiddlewares(t *testing.T) {
	...
	middleware1 := schemas.UnifAIHTTPMiddleware(func(next fasthttp.RequestHandler) fasthttp.RequestHandler {
		return func(ctx *fasthttp.RequestCtx) {
	...
	middleware2 := schemas.UnifAIHTTPMiddleware(func(next fasthttp.RequestHandler) fasthttp.RequestHandler {
		return func(ctx *fasthttp.RequestCtx) {
	...
	middleware3 := schemas.UnifAIHTTPMiddleware(func(next fasthttp.RequestHandler) fasthttp.RequestHandler {
		return func(ctx *fasthttp.RequestCtx) {
...
func TestAuthMiddleware_InferenceMiddleware_DelegatesAuthToGovernance(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "chat completion with virtual key", uri: "/v1/chat/completions", headerKey: "x-uf-vk", headerVal: "sk-uf-abc123"},
...
func TestAuthMiddleware_APIMiddleware_VirtualKeyDoesNotBypass(t *testing.T) {
	...
	ctx.Request.Header.Set("x-uf-vk", "sk-uf-abc123")
...
func TestFasthttpToHTTPRequest_PathParams(t *testing.T) {
	...
	ctx.SetUserValue("UnifAIContextKeyRequestID", "req-123")
	...
	systemKeys := []string{"UnifAIContextKeyRequestID", "trace_id", "span_id"}
...
func TestRequestDecompressionMiddleware_InvalidCompressedBody(t *testing.T) {
	...
	if err := json.Unmarshal(ctx.Response.Body(), &unifaiErr); err != nil {
		t.Fatalf("failed to decode error response: %v", err)
	}
	if unifaiErr.Error == nil || !strings.Contains(unifaiErr.Error.Message, "invalid compressed request body") {
		t.Fatalf("unexpected error message: %#v", unifaiErr.Error)
	}
}
...
func TestRequestDecompressionMiddleware_UnsupportedEncoding(t *testing.T) {
	...
	if unifaiErr.Error == nil || !strings.Contains(unifaiErr.Error.Message, "unsupported Content-Encoding") {
		t.Fatalf("unexpected error message: %#v", unifaiErr.Error)
	}
}
...
func TestRequestDecompressionMiddleware_DecompressedSizeLimit(t *testing.T) {
	...
	if unifaiErr.Error == nil || !strings.Contains(unifaiErr.Error.Message, "decompressed request body exceeds max allowed size") {
		t.Fatalf("unexpected error message: %#v", unifaiErr.Error)
	}
}
...
func TestTracingMiddleware_StreamingRootSpanEndsAfterLLMSpan(t *testing.T) {
	...
	next := func(ctx *fasthttp.RequestCtx) {
		traceID, _ = ctx.UserValue(schemas.UnifAIContextKeyTraceID).(string)
		rootSpanID, _ = ctx.UserValue(schemas.UnifAIContextKeySpanID).(string)
		completer, _ = ctx.UserValue(schemas.UnifAIContextKeyTraceCompleter).(func([]schemas.PluginLogEntry))
	...
	goCtx := context.WithValue(context.Background(), schemas.UnifAIContextKeyTraceID, traceID)
	goCtx = context.WithValue(goCtx, schemas.UnifAIContextKeySpanID, rootSpanID)
...
func TestCollectDimensionHeaders(t *testing.T) {
	...
	ctx.Request.Header.Set("x-uf-dim-team", "ml")
	ctx.Request.Header.Set("x-uf-dim-", "ignored")  // empty dimension name
