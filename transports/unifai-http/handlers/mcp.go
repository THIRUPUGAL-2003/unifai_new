...
func NewMCPHandler(mcpManager MCPManager, governanceManager GovernanceManager, client *unifai.UnifAI, store *lib.Config, oauthHandler *OAuthHandler) *MCPHandler {
	return &MCPHandler{
		client:            client,
		store:             store,
		mcpManager:        mcpManager,
...
func (h *MCPHandler) RegisterRoutes(r *router.Router, middlewares ...schemas.UnifAIHTTPMiddleware) {
...
func (h *MCPHandler) getMCPClientsPaginated(ctx *fasthttp.RequestCtx, params configstore.MCPClientsQueryParams, states []string) {
	// Get connected clients from UnifAI engine — used both to resolve the
	// runtime state filter and to merge live state/tools onto each row below.
	clientsInUnifAI, err := h.client.GetMCPClients()
	if err != nil {
		SendError(ctx, fasthttp.StatusInternalServerError, fmt.Sprintf("Failed to get MCP clients from UnifAI: %v", err))
		return
	}
	connectedClientsMap := make(map[string]schemas.MCPClient)
	for _, client := range clientsInUnifAI {
	...
	if wantConnected, wantDisconnected := slices.Contains(states, "connected"), slices.Contains(states, "disconnected"); wantConnected != wantDisconnected {
		connectedIDs := make([]string, 0, len(clientsInUnifAI))
		for _, c := range clientsInUnifAI {
...
func (h *MCPHandler) addMCPClient(ctx *fasthttp.RequestCtx) {
	...
	unifaiCtx, cancel := lib.ConvertToUnifAIContext(ctx, h.store)
	defer cancel()
	...
	if req.AuthType == string(schemas.MCPAuthTypePerUserHeaders) {
		...
		if err := h.mcpManager.AddMCPClient(unifaiCtx, schemasConfig); err != nil {
	...
	if req.AuthType == string(schemas.MCPAuthTypeOauth) {
		...
		// Build redirect URI - use UnifAI's own callback endpoint
		redirectURI := lib.BuildBaseURL(ctx, h.store.GetMCPExternalClientURL()) + "/api/oauth/callback"
	...
	// Convert to schemas.MCPClientConfig for runtime unifai client (without tool_pricing)
	schemasConfig := &schemas.MCPClientConfig{
		ID:                    req.ClientID,
		Name:                  req.Name,
		IsCodeModeClient:      req.IsCodeModeClient,
	...
	if err := h.mcpManager.AddMCPClient(unifaiCtx, schemasConfig); err != nil {
		...
		if h.store.ConfigStore != nil {
			if err := h.store.ConfigStore.DeleteMCPClientConfig(ctx, schemasConfig.ID); err != nil {
				logger.Error(fmt.Sprintf("Failed to delete MCP client config from database: %v. please restart unifai to keep core and database in sync", err))
				SendError(ctx, fasthttp.StatusInternalServerError, fmt.Sprintf("Failed to delete MCP client config from database: %v. please restart unifai to keep core and database in sync", err))
				return
			}
		}
...
func (h *MCPHandler) completeMCPClientOAuth(ctx *fasthttp.RequestCtx) {
	...
	if mcpClientConfig.AuthType == schemas.MCPAuthTypePerUserOauth {
		...
		tools, toolNameMapping, err := h.mcpManager.VerifyPerUserOAuthConnection(unifaiCtx, mcpClientConfig, accessToken)
		if err != nil {
		...
		if isUpdateFlow {
			...
			if err := h.updateMCPClientWithRetry(unifaiCtx, mcpClientConfig.ID, mcpClientConfig); err != nil {
			...
			if err := h.mcpManager.AddMCPClient(unifaiCtx, mcpClientConfig); err != nil {
				// Clean up DB entry on failure
				if h.store.ConfigStore != nil {
					if delErr := h.store.ConfigStore.DeleteMCPClientConfig(ctx, mcpClientConfig.ID); delErr != nil {
						logger.Error(fmt.Sprintf("Failed to delete MCP client config from database: %v. please restart unifai to keep core and database in sync", delErr))
						SendError(ctx, fasthttp.StatusInternalServerError, fmt.Sprintf("Failed to delete MCP client config from database: %v. please restart unifai to keep core and database in sync", delErr))
						return
					}
				}
				SendError(ctx, fasthttp.StatusInternalServerError, fmt.Sprintf("Failed to register MCP client: %v", err))
	...
	if isUpdateFlow {
		...
		if err := h.updateMCPClientConnectionWithRetry(unifaiCtx, mcpClientConfig.ID, mcpClientConfig); err != nil {
			if rollbackErr := h.store.ConfigStore.UpdateMCPClientConfig(ctx, mcpClientConfig.ID, &oldDBConfig); rollbackErr != nil {
				logger.Error(fmt.Sprintf("Failed to rollback MCP client DB update: %v. please restart unifai to keep core and database in sync", rollbackErr))
			}
		...
		// Add MCP client to UnifAI and connect
		if err := h.mcpManager.AddMCPClient(unifaiCtx, mcpClientConfig); err != nil {
			if h.store.ConfigStore != nil {
				if delErr := h.store.ConfigStore.DeleteMCPClientConfig(ctx, mcpClientConfig.ID); delErr != nil {
