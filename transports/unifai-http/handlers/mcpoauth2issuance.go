...
func (h *OAuth2IssuanceHandler) RegisterRoutes(r *router.Router, middlewares ...schemas.UnifAIHTTPMiddleware) {
...
func (h *OAuth2IssuanceHandler) handleTokenRefresh(ctx *fasthttp.RequestCtx) {
	...
	// uf_sub liveness check: for VK-mode tokens, verify the VK still exists
	...
	// uf_sub liveness check: for user-mode tokens, verify the user still exists
