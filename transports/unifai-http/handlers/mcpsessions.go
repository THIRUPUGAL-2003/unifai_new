...
func (h *MCPSessionsHandler) RegisterRoutes(r *router.Router, middlewares ...schemas.UnifAIHTTPMiddleware) {
...
type mcpSessionRow struct {
	...
	SessionID       *string            `json:"session_id,omitempty"`        // Session-mode identity: caller-issued x-uf-mcp-session-id value
...
func parseMCPSessionsListQuery(ctx *fasthttp.RequestCtx) (mcpSessionsListQuery, bool) {
	...
	// Identity is an exact-match SQL key (uf_sub), unlike the fuzzy Search term —
...
func callerUserIDFromCtx(ctx *fasthttp.RequestCtx) string {
	v, _ := ctx.UserValue(schemas.UnifAIContextKeyUserID).(string)
	return v
}
...
func (h *MCPSessionsHandler) reauth(ctx *fasthttp.RequestCtx) {
	...
	bfCtx, cancel := lib.ConvertToUnifAIContext(ctx, h.store)
	defer cancel()
	...
	case schemas.MCPAuthModeSession:
		if tok.SessionID != "" {
			bfCtx.SetValue(schemas.UnifAIContextKeyMCPSessionID, tok.SessionID)
		}
	case schemas.MCPAuthModeVK:
		if tok.VirtualKeyID != nil && *tok.VirtualKeyID != "" {
			bfCtx.SetValue(schemas.UnifAIContextKeyGovernanceVirtualKeyID, *tok.VirtualKeyID)
		}
	case schemas.MCPAuthModeUser:
		if tok.UserID != nil && *tok.UserID != "" {
			bfCtx.SetValue(schemas.UnifAIContextKeyUserID, *tok.UserID)
		}
	}
...
func (h *MCPSessionsHandler) reauthHeaderCredential(ctx *fasthttp.RequestCtx, bfCtx *schemas.UnifAIContext, cred *tables.TableMCPPerUserHeaderCredential) {
