...
// and *ufws.Session (post-session, mutex-protected) satisfy this interface.
...
// Each event is routed through the standard UnifAI inference pipeline (PreLLMHook, key selection,
...
func NewWSResponsesHandler(client *unifai.UnifAI, config *lib.Config, pool *ufws.Pool) *WSResponsesHandler {
...
func (h *WSResponsesHandler) RegisterRoutes(r *router.Router, middlewares ...schemas.UnifAIHTTPMiddleware) {
...
func captureAuthHeaders(ctx *fasthttp.RequestCtx) *authHeaders {
	ah := &authHeaders{
		...
		virtualKey:    string(ctx.Request.Header.Peek("x-uf-vk")),
...
func (h *WSResponsesHandler) eventLoop(conn *ws.Conn, session *ufws.Session, auth *authHeaders) {
	for {
...
func (h *WSResponsesHandler) handleResponseCreate(session *ufws.Session, auth *authHeaders, message []byte) {
	...
	unifaiReq, err := h.convertEventToRequest(&event)
	if err != nil {
	...
	if extractErr == nil && len(extraParams) > 0 {
		if unifaiReq.Params == nil {
			unifaiReq.Params = &schemas.ResponsesParameters{}
		}
	...
	if parentRequestID, _ := unifaiCtx.Value(schemas.UnifAIContextKeyParentRequestID).(string); parentRequestID == "" {
		unifaiCtx.SetValue(schemas.UnifAIContextKeyParentRequestID, session.ID())
	}

	// Rewrite the raw event for upstream: strip provider/ prefix from model,
	// apply store override. The original bytes contain the unifai-format model
	// (e.g. "openai/gpt-5.5") which upstream providers don't understand.
	upstreamEvent, rewriteErr := rewriteUpstreamEvent(message, unifaiReq.Model, event.Store)
	if rewriteErr != nil {
	...
	if h.tryNativeWSUpstream(session, unifaiCtx, unifaiReq, upstreamEvent) {
		cancel()
		return
	}

	// Fall back to HTTP bridge
	h.executeHTTPBridge(session, unifaiCtx, cancel, unifaiReq)
}
...
func (h *WSResponsesHandler) tryNativeWSUpstream(
	...
	if hasForwardedHeaders {
		...
		upstream, err = ufws.DialUpstream(wsURL, headers, req.Provider, key.ID)
		if err != nil {
	...
	hooks, preErr := h.client.RunStreamPreHooks(ctx, unifaiReq)
	if preErr != nil {
		writeWSUnifAIError(session, preErr)
		return true
	}
	defer hooks.Cleanup()
	...
	finalizeTerminalPostHooks := func(unifaiErr *schemas.UnifAIError) {
		if unifaiErr == nil {
			return
		}
		ctx.SetValue(schemas.UnifAIContextKeyStreamEndIndicator, true)
		if _, postErr := hooks.PostHookRunner(ctx, nil, unifaiErr); postErr != nil {
	...
	for {
		if err := upstream.SetReadDeadline(time.Now().Add(streamIdleTimeout)); err != nil {
			...
			closeUpstream()
			finalizeTerminalPostHooks(newUnifAIError(502, "upstream_connection_error", "failed to arm upstream read deadline"))
		...
		if readErr != nil {
			if isWSReadTimeout(readErr) {
				...
				closeUpstream()
				finalizeTerminalPostHooks(newUnifAIError(504, "upstream_timeout", "upstream websocket stream timed out"))
			...
			finalizeTerminalPostHooks(newUnifAIError(502, "upstream_connection_error", "upstream websocket stream interrupted"))
		...
		if streamResp != nil {
			resp := &schemas.UnifAIResponse{ResponsesStreamResponse: streamResp}
			...
			if postErr != nil {
				closeUpstream()
				writeWSUnifAIError(session, postErr)
				return true
			}
		}
		...
		if writeErr := session.WriteMessage(msgType, data); writeErr != nil {
			...
			if streamResp == nil || !isTerminal {
				finalizeTerminalPostHooks(newUnifAIError(499, "client_connection_error", "client websocket connection interrupted"))
			}
			return true
		}
		forwardedAny = true
...
func isExpectedResponsesClientClose(err error, session *ufws.Session) bool {
	if err == nil {
		return true
	}
...
func newUnifAIError(statusCode int, errType, message string) *schemas.UnifAIError {
...
func writeWSShortCircuitResponse(session *ufws.Session, resp *schemas.UnifAIResponse) {
...
// parseUpstreamWSEvent attempts to parse a raw upstream WS event into a UnifAIResponsesStreamResponse.
...
func parseUpstreamWSEvent(data []byte, provider schemas.ModelProvider, model string) *schemas.UnifAIResponsesStreamResponse {
	var streamResp schemas.UnifAIResponsesStreamResponse
...
func (h *WSResponsesHandler) trackResponseID(session *ufws.Session, data []byte) {
	var envelope struct {
		Response struct {
			ID string `json:"id"`
		} `json:"response"`
...
// convertEventToRequest converts a WebSocket response.create event to a UnifAIResponsesRequest.
func (h *WSResponsesHandler) convertEventToRequest(event *schemas.WebSocketResponsesEvent) (*schemas.UnifAIResponsesRequest, error) {
	provider, modelName := schemas.ParseModelString(event.Model, schemas.OpenAI)
	if provider == "" || modelName == "" {
...
// createUnifAIContextFromAuth builds a UnifAIContext from the auth headers captured during upgrade.
func createUnifAIContextFromAuth(handlerStore lib.HandlerStore, auth *authHeaders) (*schemas.UnifAIContext, context.CancelFunc) {
	ctx, cancel := schemas.NewUnifAIContextWithCancel(context.Background())
	if auth == nil {
		return ctx, cancel
	}

	if sessionID := lib.ParseSessionIDFromBaggage(auth.baggage); sessionID != "" {
		ctx.SetValue(schemas.UnifAIContextKeyParentRequestID, sessionID)
	}

	if auth.virtualKey != "" {
		ctx.SetValue(schemas.UnifAIContextKeyVirtualKey, auth.virtualKey)
	}
	...
	if auth.authorization != "" {
		if strings.HasPrefix(auth.authorization, "Bearer ") {
			...
			if strings.HasPrefix(token, "sk-uf-") {
				ctx.SetValue(schemas.UnifAIContextKeyVirtualKey, token)
			}
		}
	}
	if auth.apiKey != "" {
		if strings.HasPrefix(auth.apiKey, "sk-uf-") {
			ctx.SetValue(schemas.UnifAIContextKeyVirtualKey, auth.apiKey)
		}
	}
	if auth.googAPIKey != "" {
		if strings.HasPrefix(auth.googAPIKey, "sk-uf-") {
			ctx.SetValue(schemas.UnifAIContextKeyVirtualKey, auth.googAPIKey)
		}
	}

	// Forward x-uf-* headers
	...
	for k, values := range auth.headers {
		for _, v := range values {
			...
			case k == "x-uf-api-key":
				ctx.SetValue(schemas.UnifAIContextKeyAPIKeyName, v)
			...
			case matcher != nil && matcher.HasAllowlist() && matcher.MatchesAllow(k):
				if !strings.HasPrefix(k, "x-uf-") && !isSecurityDeniedExtraHeader(k) && !matcher.MatchesDeny(k) {
	...
	if len(extraHeaders) > 0 {
		ctx.SetValue(schemas.UnifAIContextKeyExtraHeaders, extraHeaders)
	}
...
func isSecurityDeniedExtraHeader(name string) bool {
	...
	case "authorization", "proxy-authorization", "cookie", "host", "content-length", "connection", "transfer-encoding",
		"upgrade", "origin", "x-api-key", "x-goog-api-key", "x-uf-api-key", "x-uf-api-key-id", "x-uf-vk":
		return true
	default:
		return false
	}
...
func mergeWebSocketHeaders(ctx *schemas.UnifAIContext, providerHeaders map[string]string) http.Header {
	merged := http.Header{}
...
func hasWebSocketForwardedHeaders(ctx *schemas.UnifAIContext) bool {
...
func (h *WSResponsesHandler) executeHTTPBridge(
	...
	stream, unifaiErr := h.client.ResponsesStreamRequest(ctx, req)
	if unifaiErr != nil {
		writeWSUnifAIError(session, unifaiErr)
		return
	}
	...
	for chunk := range stream {
		...
		if chunk.UnifAIResponsesStreamResponse != nil &&
			isTerminalStreamType(chunk.UnifAIResponsesStreamResponse.Type) {
			session.MarkResponsesTurnCompleted()
			if chunk.UnifAIResponsesStreamResponse.Response != nil &&
				chunk.UnifAIResponsesStreamResponse.Response.ID != nil &&
				*chunk.UnifAIResponsesStreamResponse.Response.ID != "" {
				session.SetLastResponseID(*chunk.UnifAIResponsesStreamResponse.Response.ID)
			}
		}
	}
}
...
// Accepts either a raw *ws.Conn (pre-session) or a *ufws.Session (mutex-protected).
...
// writeWSUnifAIError converts a UnifAIError to a WS error event.
func writeWSUnifAIError(w wsWriter, unifaiErr *schemas.UnifAIError) {
	status := 500
	if unifaiErr.StatusCode != nil && *unifaiErr.StatusCode > 0 {
		status = *unifaiErr.StatusCode
	}
	code := "server_error"
	msg := "internal server error"
	if unifaiErr.Error != nil {
		if unifaiErr.Error.Code != nil && *unifaiErr.Error.Code != "" {
			code = *unifaiErr.Error.Code
		} else if unifaiErr.Error.Type != nil && *unifaiErr.Error.Type != "" {
			code = *unifaiErr.Error.Type
		}
		if unifaiErr.Error.Message != "" {
			msg = unifaiErr.Error.Message
		}
	}
...
func completeTrace(ctx *schemas.UnifAIContext) {
	if ctx == nil {
		return
	}
