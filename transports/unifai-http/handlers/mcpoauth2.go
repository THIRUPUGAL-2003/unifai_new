...
func NewOAuthHandler(oauthProvider *oauth2.OAuth2Provider, client *unifai.UnifAI, store *lib.Config) *OAuthHandler {
	return &OAuthHandler{
		client:        client,
		store:         store,
...
func (h *OAuthHandler) RegisterRoutes(r *router.Router, middlewares ...schemas.UnifAIHTTPMiddleware) {
...
//   - Per-user runtime flow (UnifAI-as-client to upstream for an MCP server's
