...
func TestNewRealtimeRelayContextCopiesValuesWithoutRequestCancellation(t *testing.T) {
	requestCtx, requestCancel := schemas.NewUnifAIContextWithCancel(context.Background())
	requestCtx.SetValue(schemas.UnifAIContextKeyHTTPRequestType, schemas.RealtimeRequest)
	requestCtx.SetValue(schemas.UnifAIContextKeyIntegrationType, "openai")
	requestCtx.SetValue(schemas.UnifAIContextKeyGovernanceVirtualKeyID, "vk_test")
	...
	if got := relayCtx.Value(schemas.UnifAIContextKeyHTTPRequestType); got != schemas.RealtimeRequest {
		t.Fatalf("request type = %v, want %v", got, schemas.RealtimeRequest)
	}
	if got := relayCtx.Value(schemas.UnifAIContextKeyIntegrationType); got != "openai" {
		t.Fatalf("integration type = %v, want %q", got, "openai")
	}
	if got := relayCtx.Value(schemas.UnifAIContextKeyGovernanceVirtualKeyID); got != "vk_test" {
...
func TestLookupRealtimeEphemeralKeyMappingKeepsEntryUntilTTLExpiry(t *testing.T) {
	...
	payload, err := json.Marshal(realtimeEphemeralKeyMapping{KeyID: "key_123", VirtualKey: "sk-uf-test"})
	if err != nil {
...
func TestResolveRealtimeWebRTCKeys_UnmappedEphemeralTokenStaysAnonymous(t *testing.T) {
	...
	unifaiCtx.SetValue(schemas.UnifAIContextKeySelectedKeyID, "selected")
	unifaiCtx.SetValue(schemas.UnifAIContextKeySelectedKeyName, "selected-name")
	unifaiCtx.SetValue(schemas.UnifAIContextKeyAPIKeyID, "mapped-id")
	unifaiCtx.SetValue(schemas.UnifAIContextKeyAPIKeyName, "mapped-name")

	authKey, selectedKey, err := handler.resolveRealtimeWebRTCKeys(&ctx, unifaiCtx, schemas.OpenAI, "gpt-realtime")
	if err != nil {
	...
	if got := unifaiCtx.Value(schemas.UnifAIContextKeySelectedKeyID); got != nil {
		t.Fatalf("selected key id context = %#v, want nil", got)
	}
	if got := unifaiCtx.Value(schemas.UnifAIContextKeySelectedKeyName); got != nil {
		t.Fatalf("selected key name context = %#v, want nil", got)
	}
	if got := unifaiCtx.Value(schemas.UnifAIContextKeyAPIKeyID); got != nil {
		t.Fatalf("api key id context = %#v, want nil", got)
	}
	if got := unifaiCtx.Value(schemas.UnifAIContextKeyAPIKeyName); got != nil {
...
func TestApplyRealtimeEphemeralKeyMapping_RestoresVirtualKeyAndKeyID(t *testing.T) {
	...
	unifaiCtx := schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline)
	applyRealtimeEphemeralKeyMapping(unifaiCtx, realtimeEphemeralKeyMapping{
		KeyID:      "key_123",
		VirtualKey: "sk-uf-test",
	})

	if got := unifaiCtx.Value(schemas.UnifAIContextKeyVirtualKey); got != "sk-uf-test" {
		t.Fatalf("virtual key context = %#v, want %q", got, "sk-uf-test")
	}
	if got := unifaiCtx.Value(schemas.UnifAIContextKeyAPIKeyID); got != "key_123" {
