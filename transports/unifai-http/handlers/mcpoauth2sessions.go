...
func (h *OAuth2SessionsHandler) RegisterRoutes(r *router.Router, middlewares ...schemas.UnifAIHTTPMiddleware) {
...
// Modes filters on uf_mode (user/vk/session); Search is a case-insensitive
...
func parseOAuth2SessionsListQuery(ctx *fasthttp.RequestCtx) (oauth2SessionsListQuery, bool) {
	...
	q.Modes = parseCommaSeparated(string(args.Peek("uf_mode")))
	// uf_mode only ever holds user/vk/session. Reject anything else with a 400
	...
	for _, mode := range q.Modes {
		...
		default:
			SendError(ctx, fasthttp.StatusBadRequest, "Invalid uf_mode parameter: must be one or more of user, vk, session")
			return q, false
		}
	}
