...
// allows all headers through (same behavior as no config — x-uf-eh-* headers forwarded as-is).
func TestValidateHeaderFilterConfig_EmptyConfigStillForwardsHeaders(t *testing.T) {
	...
	// Matcher with empty lists should allow all headers (x-uf-eh-* forwarded as-is)
