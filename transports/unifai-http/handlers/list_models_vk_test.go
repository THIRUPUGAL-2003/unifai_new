...
func TestApplyListModelsVirtualKeyProviderFilterSetsActiveVKProviders(t *testing.T) {
	h := &CompletionHandler{
		config: &lib.Config{
			ConfigStore: &mockListModelsVKConfigStore{vk: &configstoreTables.TableVirtualKey{
				Value:    *schemas.NewSecretVar("sk-uf-active"),
				IsActive: new(true),
	...
	ctx.Request.Header.Set("Authorization", "Bearer sk-uf-active")
	unifaiCtx := schemas.NewUnifAIContext(context.Background(), time.Time{})

	if ok := h.applyListModelsVirtualKeyProviderFilter(ctx, unifaiCtx); !ok {
		t.Fatalf("expected active VK to apply provider filter")
	}
	got, ok := unifaiCtx.Value(schemas.UnifAIContextKeyAvailableProviders).([]schemas.ModelProvider)
	if !ok {
...
func TestApplyListModelsVirtualKeyProviderFilterSkipsWhenVKNotFound(t *testing.T) {
	...
	ctx := &fasthttp.RequestCtx{}
	ctx.Request.Header.Set("Authorization", "Bearer sk-uf-missing")
	unifaiCtx := schemas.NewUnifAIContext(context.Background(), time.Time{})

	if ok := h.applyListModelsVirtualKeyProviderFilter(ctx, unifaiCtx); !ok {
		t.Fatalf("expected missing VK to be ignored without failing request")
	}
	if got := unifaiCtx.Value(schemas.UnifAIContextKeyAvailableProviders); got != nil {
...
func TestApplyListModelsVirtualKeyProviderFilterSkipsInactiveVK(t *testing.T) {
	h := &CompletionHandler{
		config: &lib.Config{
			ConfigStore: &mockListModelsVKConfigStore{vk: &configstoreTables.TableVirtualKey{
				Value:    *schemas.NewSecretVar("sk-uf-inactive"),
				IsActive: new(false),
				ProviderConfigs: []configstoreTables.TableVirtualKeyProviderConfig{
					{Provider: "openai"},
				},
	...
	ctx := &fasthttp.RequestCtx{}
	ctx.Request.Header.Set("Authorization", "Bearer sk-uf-inactive")
	unifaiCtx := schemas.NewUnifAIContext(context.Background(), time.Time{})
