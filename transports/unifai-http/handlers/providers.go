...
func NewProviderHandler(modelsManager ModelsManager, inMemoryStore *lib.Config, client *unifai.UnifAI) *ProviderHandler {
...
func (h *ProviderHandler) RegisterRoutes(r *router.Router, middlewares ...schemas.UnifAIHTTPMiddleware) {
	...
	r.POST("/api/providers/{provider}/keys/{key_id}/discover", lib.ChainMiddlewares(h.rediscoverProviderKey, middlewares...))
...
func (h *ProviderHandler) addProvider(ctx *fasthttp.RequestCtx) {
	...
	if payload.CustomProviderConfig != nil {
		...
		if unifai.IsStandardProvider(payload.Provider) {
		...
		if !unifai.IsSupportedBaseProvider(payload.CustomProviderConfig.BaseProviderType) {
	...
	if payload.NetworkConfig != nil {
		...
		if payload.NetworkConfig.BaseURL != "" {
			if err := unifai.ValidateExternalURL(payload.NetworkConfig.BaseURL, payload.NetworkConfig.AllowPrivateNetwork); err != nil {
...
func (h *ProviderHandler) updateProvider(ctx *fasthttp.RequestCtx) {
	...
	if nc.BaseURL != "" {
		if err := unifai.ValidateExternalURL(nc.BaseURL, nc.AllowPrivateNetwork); err != nil {
			SendError(ctx, fasthttp.StatusBadRequest, fmt.Sprintf("Invalid base URL: %v", err))
			return
		}
	}
