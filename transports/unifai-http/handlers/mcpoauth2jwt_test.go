...
const testIssuer = "https://unifai.test"
...
func mintTestToken(t *testing.T, priv *rsa.PrivateKey, kid string, mutate func(jwt.MapClaims)) string {
	...
	claims := jwt.MapClaims{
		...
		"sub":     "vk-123",
		"uf_mode": string(schemas.MCPAuthModeVK),
		"scope":   "mcp",
		"iat":     now.Unix(),
		"nbf":     now.Unix(),
...
func TestExtractBearerJWT(t *testing.T) {
	cases := []struct {
		...
		{name: "virtual key is not a jwt", header: "Bearer sk-uf-abc123", want: ""},
...
func TestVerifyMCPJWT_Rejections(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			raw: func() string {
				return mintTestToken(t, priv, key.KID, func(c jwt.MapClaims) {
					c["aud"] = jwt.ClaimStrings{"https://unifai.test/other"}
				})
			},
		},
		{
		...
		},
		{
			...
			raw: func() string {
				tok := jwt.NewWithClaims(jwt.SigningMethodHS256, jwt.MapClaims{
					...
					"sub": "vk-123", "uf_mode": "vk", "iat": time.Now().Unix(),
		...
		},
		{
			...
			raw: func() string {
				...
				payload := base64.RawURLEncoding.EncodeToString([]byte(`{"iss":"https://unifai.test","aud":["https://unifai.test/mcp"],"sub":"vk-123","uf_mode":"vk","iat":1700000000,"nbf":1700000000,"exp":9999999999}`))
...
func TestInjectJWTContext(t *testing.T) {
	activeVK := &configtables.TableVirtualKey{ID: "vk-row-1", Value: *schemas.NewSecretVar("sk-uf-active")}

	t.Run("user mode sets user id", func(t *testing.T) {
		bc := schemas.NewUnifAIContext(context.Background(), time.Time{})
		err := injectJWTContext(bc, &jwtMCPClaims{
			RegisteredClaims: jwt.RegisteredClaims{Subject: "user-1"}, BfMode: "user",
		}, nil)
		require.NoError(t, err)
		assert.Equal(t, "user-1", bc.Value(schemas.UnifAIContextKeyUserID))
	})
	...
	t.Run("vk mode sets the raw vk value and lets governance derive the id", func(t *testing.T) {
		...
		require.NoError(t, err)
		assert.Equal(t, "sk-uf-active", bc.Value(schemas.UnifAIContextKeyVirtualKey))
		// The VK row ID is resolved later by governance's PreMCPConnectionHook from
		// the value, not stamped here — mirrors the x-uf-vk header path.
		assert.Nil(t, bc.Value(schemas.UnifAIContextKeyGovernanceVirtualKeyID))
	})
	...
	t.Run("session mode sets session id", func(t *testing.T) {
		...
		require.NoError(t, err)
		assert.Equal(t, "session-abc", bc.Value(schemas.UnifAIContextKeyMCPSessionID))
	})
	...
	t.Run("unknown uf_mode errors", func(t *testing.T) {
		bc := schemas.NewUnifAIContext(context.Background(), time.Time{})
		err := injectJWTContext(bc, &jwtMCPClaims{
			RegisteredClaims: jwt.RegisteredClaims{Subject: "x"}, BfMode: "bogus",
		}, nil)
		require.Error(t, err)
		assert.True(t, strings.Contains(err.Error(), "uf_mode"))
	})
}
