...
import (
	...
	governanceplugin "github.com/unifai/unifai/plugins/governance"
...
func TestAddProvider_ReturnsErrorWhenRuntimeReloadFails(t *testing.T) {
	...
	if unifaiErr.Error == nil || unifaiErr.Error.Message == "" {
		t.Fatalf("expected error message in response, got %#v", unifaiErr)
	}
	if unifaiErr.Error.Message != "Failed to initialize provider after add: context deadline exceeded" {
		t.Fatalf("unexpected error message: %q", unifaiErr.Error.Message)
	}
	if _, exists := h.inMemoryStore.Providers["mock-openai"]; exists {
...
func TestUpdateProvider_RejectsKeysInBody(t *testing.T) {
	...
	if !strings.Contains(unifaiErr.Error.Message, "/keys") {
		t.Fatalf("expected error message to mention the /keys endpoint, got %q", unifaiErr.Error.Message)
	}
...
// `inMemoryStore.UpdateProviderConfig` requires a real *unifai.UnifAI client
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if unifaiErr.Error == nil {
				t.Fatalf("expected error in response, got %#v", unifaiErr)
			}
			if strings.Contains(unifaiErr.Error.Message, "keys are not accepted on this endpoint") {
				t.Fatalf("keys-guard should NOT fire for empty/absent keys, got: %s", unifaiErr.Error.Message)
			}
			if !strings.Contains(unifaiErr.Error.Message, "Concurrency") {
				t.Fatalf("expected concurrency error (proves we passed the keys-guard), got: %s", unifaiErr.Error.Message)
			}
		})
	}
}
...
func TestEnrichAndFilterListModelsResponse_FiltersDeprecatedPricingRows(t *testing.T) {
	...
	resp := &schemas.UnifAIListModelsResponse{Data: []schemas.Model{
...
func TestParseVKValueFromRequest(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name: "x-uf-vk takes priority over Authorization",
			setup: func(ctx *fasthttp.RequestCtx) {
				ctx.Request.Header.Set("x-uf-vk", vk)
				ctx.Request.Header.Set("Authorization", "Bearer sk-uf-other")
			},
			wantVK: vk,
		},
	}
...
func TestParseModelListQuery_VKWithoutDBStoreReturnsServiceUnavailable(t *testing.T) {
	...
	ctx.Request.Header.Set("x-uf-vk", "sk-uf-test-virtual-key")
