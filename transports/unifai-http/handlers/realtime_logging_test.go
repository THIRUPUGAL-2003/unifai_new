...
func TestFinalizedRealtimeInputSummary(t *testing.T) {
	userCreate := &schemas.UnifAIRealtimeEvent{
		Type: schemas.RTEventConversationItemCreate,
		Item: &schemas.RealtimeItem{
			Role:    "user",
	...
	userRetrieved := &schemas.UnifAIRealtimeEvent{
	...
	userCreated := &schemas.UnifAIRealtimeEvent{
	...
	userAdded := &schemas.UnifAIRealtimeEvent{
	...
	userCreatedWithoutTranscript := &schemas.UnifAIRealtimeEvent{
		Type: schemas.RTEventConversationItemCreated,
		Item: &schemas.RealtimeItem{
			Role:    "user",
			Type:    "message",
	...
	userDoneWithoutTranscript := &schemas.UnifAIRealtimeEvent{
	...
	inputTranscript := &schemas.UnifAIRealtimeEvent{
	...
	emptyInputTranscript := &schemas.UnifAIRealtimeEvent{
		Type: schemas.RTEventInputAudioTransCompleted,
		ExtraParams: map[string]json.RawMessage{
	...
	missingInputTranscript := &schemas.UnifAIRealtimeEvent{
		Type:    schemas.RTEventInputAudioTransCompleted,
	...
	assistantCreate := &schemas.UnifAIRealtimeEvent{
		Type: schemas.RTEventConversationItemCreate,
		Item: &schemas.RealtimeItem{
			Role:    "assistant",
...
func TestFinalizedRealtimeToolOutputSummary(t *testing.T) {
	...
	retrieved := &schemas.UnifAIRealtimeEvent{
		Type: schemas.RTEventConversationItemRetrieved,
		Item: &schemas.RealtimeItem{
			Type:   "function_call_output",
	...
	created := &schemas.UnifAIRealtimeEvent{
		Type: schemas.RTEventConversationItemCreated,
		Item: &schemas.RealtimeItem{
			Type:   "function_call_output",
	...
	added := &schemas.UnifAIRealtimeEvent{
		Type: schemas.RTEventConversationItemAdded,
		Item: &schemas.RealtimeItem{
			Type:   "function_call_output",
...
func TestPendingRealtimeInputUpdate(t *testing.T) {
	...
	transcriptEvent := &schemas.UnifAIRealtimeEvent{
		Type: schemas.RTEventInputAudioTransCompleted,
		ExtraParams: map[string]json.RawMessage{
	...
	retrievedEvent := &schemas.UnifAIRealtimeEvent{
		Type: schemas.RTEventConversationItemRetrieved,
		Item: &schemas.RealtimeItem{
			ID:      "item_123",
			Role:    "user",
...
func TestPendingRealtimeToolOutputUpdate(t *testing.T) {
	...
	toolOutputEvent := &schemas.UnifAIRealtimeEvent{
		Type: schemas.RTEventConversationItemDone,
		Item: &schemas.RealtimeItem{
			ID:     "item_tool_123",
			Type:   "function_call_output",
	...
	retrievedToolOutputEvent := &schemas.UnifAIRealtimeEvent{
		Type: schemas.RTEventConversationItemRetrieved,
		Item: &schemas.RealtimeItem{
			ID:     "item_tool_123",
			Type:   "function_call_output",
...
func TestFinalizeRealtimeTurnHooksWithErrorCompletesActiveHooks(t *testing.T) {
	...
	var (
		capturedResp *schemas.UnifAIResponse
		capturedErr  *schemas.UnifAIError
		cleanedUp    bool
	)
	session.SetRealtimeTurnHooks(&ufws.RealtimeTurnPluginState{
		...
		PreHookValues: map[any]any{
			schemas.UnifAIContextKeyGovernanceVirtualKeyID: "vk_123",
		},
	...
	postErr := finalizeRealtimeTurnHooksWithError(
		...
		rawResponse,
		newRealtimeWireUnifAIError(401, "server_error", "Virtual key is required."),
	)
	if postErr != nil {
...
func TestNewUnifAIErrorFromRealtimeErrorCarriesRealtimeMetadata(t *testing.T) {
	t.Parallel()
	...
	unifaiErr := newUnifAIErrorFromRealtimeError(
		schemas.OpenAI,
		"gpt-realtime",
		rawResponse,
	...
	if unifaiErr == nil {
		t.Fatal("expected unifai error")
	}
	if unifaiErr.StatusCode == nil || *unifaiErr.StatusCode != 400 {
		t.Fatalf("status code = %#v, want 400", unifaiErr.StatusCode)
	}
	if unifaiErr.ExtraFields.RequestType != schemas.RealtimeRequest {
		t.Fatalf("request type = %q, want %q", unifaiErr.ExtraFields.RequestType, schemas.RealtimeRequest)
	}
	if unifaiErr.ExtraFields.Provider != schemas.OpenAI {
		t.Fatalf("provider = %q, want %q", unifaiErr.ExtraFields.Provider, schemas.OpenAI)
	}
	if unifaiErr.ExtraFields.OriginalModelRequested != "gpt-realtime" {
		t.Fatalf("model requested = %q, want %q", unifaiErr.ExtraFields.OriginalModelRequested, "gpt-realtime")
	}
	rawResp, ok := unifaiErr.ExtraFields.RawResponse.(json.RawMessage)
	if !ok || string(rawResp) != string(rawResponse) {
		t.Fatalf("raw response = %#v, want %s", unifaiErr.ExtraFields.RawResponse, string(rawResponse))
	}
	if unifaiErr.Error == nil || unifaiErr.Error.Param != "session.type" {
		t.Fatalf("error param = %#v, want session.type", unifaiErr.Error)
	}
}
