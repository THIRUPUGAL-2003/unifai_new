...
import (
	...
	configtables "github.com/unifai/unifai/framework/configstore/tables"
...
// the inference-path header set (x-uf-vk, Authorization Bearer, x-api-key,
// x-goog-api-key) so MCP and inference stay at parity.
func TestGetVKFromRequest(t *testing.T) {
	const vk = "sk-uf-test-virtual-key"
	...
	cases := []struct {
		...
		{
			name:   "x-uf-vk header",
			setup:  func(ctx *fasthttp.RequestCtx) { ctx.Request.Header.Set("x-uf-vk", vk) },
			wantVK: vk,
		},
		{
		...
		{
			name: "x-uf-vk takes priority over x-goog-api-key",
			setup: func(ctx *fasthttp.RequestCtx) {
				ctx.Request.Header.Set("x-uf-vk", vk)
				ctx.Request.Header.Set("x-goog-api-key", "sk-uf-other")
			},
			wantVK: vk,
		},
	}
...
func TestGetMCPServerForRequest_JWTPath(t *testing.T) {
	...
	t.Run("oauth mode: valid vk JWT with active key is accepted", func(t *testing.T) {
		activeVK := &configtables.TableVirtualKey{ID: "vk-row-1", Value: *schemas.NewSecretVar("sk-uf-active"), IsActive: new(true)}
		...
		raw := mintTestToken(t, priv, key.KID, func(c jwt.MapClaims) {
			c["uf_mode"] = string(schemas.MCPAuthModeVK)
			c["sub"] = "vk-row-1"
		})
		ctx := &fasthttp.RequestCtx{}
	...
	t.Run("vk JWT with inactive key is rejected", func(t *testing.T) {
		inactiveVK := &configtables.TableVirtualKey{ID: "vk-row-1", Value: *schemas.NewSecretVar("sk-uf-x"), IsActive: new(false)}
		store := &mockOAuth2Store{
			signingKey: key,
	...
	t.Run("user JWT without a session is allowed", func(t *testing.T) {
		...
		raw := mintTestToken(t, priv, key.KID, func(c jwt.MapClaims) {
			c["uf_mode"] = string(schemas.MCPAuthModeUser)
			c["sub"] = "user-1"
		})
		ctx := &fasthttp.RequestCtx{}
		ctx.Request.Header.Set("Authorization", "Bearer "+raw)
	...
	t.Run("user JWT is scoped to the user's representative virtual key", func(t *testing.T) {
		activeVK := &configtables.TableVirtualKey{ID: "vk-row-1", Value: *schemas.NewSecretVar("sk-uf-user-rep"), IsActive: new(true)}
		store := &mockOAuth2Store{
			signingKey: key,
			vksByID:    map[string]*configtables.TableVirtualKey{"vk-row-1": activeVK},
		}
	...
	t.Run("user JWT with a matching session is accepted", func(t *testing.T) {
		...
		ctx := &fasthttp.RequestCtx{}
		ctx.Request.Header.Set("Authorization", "Bearer "+raw)
		ctx.SetUserValue(schemas.UnifAIContextKeyUserID, "user-1")
	...
	t.Run("user JWT with a mismatched session is rejected", func(t *testing.T) {
		...
		ctx := &fasthttp.RequestCtx{}
		ctx.Request.Header.Set("Authorization", "Bearer "+raw)
		ctx.SetUserValue(schemas.UnifAIContextKeyUserID, "someone-else")
	...
	t.Run("session JWT is rejected when auth is enforced", func(t *testing.T) {
		...
		raw := mintTestToken(t, priv, key.KID, func(c jwt.MapClaims) {
			c["uf_mode"] = string(schemas.MCPAuthModeSession)
			c["sub"] = "session-abc"
		})
		ctx := &fasthttp.RequestCtx{}
		ctx.Request.Header.Set("Authorization", "Bearer "+raw)
	...
	t.Run("both mode: session token with a header VK is rejected", func(t *testing.T) {
		...
		ctx := &fasthttp.RequestCtx{}
		ctx.Request.Header.Set("Authorization", "Bearer "+raw)
		ctx.Request.Header.Set(string(schemas.UnifAIContextKeyVirtualKey), "sk-uf-header")
...
// the user id onto the request context (UnifAIContextKeyUserID). In headers/both
// modes the request is scoped to the user's representative virtual key, just like
// a user-mode token; oauth-strict ignores it (UnifAI-issued tokens only).
...
func TestGetMCPServerForRequest_HeaderAndAnonPath(t *testing.T) {
	...
	t.Run("headers mode: active header VK connects", func(t *testing.T) {
		...
		store := &mockOAuth2Store{
			signingKey: key,
			vksByValue: map[string]*configtables.TableVirtualKey{"sk-uf-active": activeVK},
		}
		...
		ctx := &fasthttp.RequestCtx{}
		ctx.Request.Header.Set(string(schemas.UnifAIContextKeyVirtualKey), "sk-uf-active")
	...
	t.Run("inactive header VK is rejected at the shared chokepoint", func(t *testing.T) {
		...
		store := &mockOAuth2Store{
			signingKey: key,
			vksByValue: map[string]*configtables.TableVirtualKey{"sk-uf-x": inactiveVK},
		}
		cfg := newTestOAuth2Config(store, configtables.MCPServerAuthModeHeaders, true)
		h := newTestMCPHandler(cfg) // not pre-seeded: must traverse ensureVKMCPServer

		ctx := &fasthttp.RequestCtx{}
		ctx.Request.Header.Set(string(schemas.UnifAIContextKeyVirtualKey), "sk-uf-x")
