...
func SecurityHeadersMiddleware() schemas.UnifAIHTTPMiddleware {
	return func(next fasthttp.RequestHandler) fasthttp.RequestHandler {
		return func(ctx *fasthttp.RequestCtx) {
			...
			ctx.Response.Header.Set("Permissions-Policy", `camera=(), microphone=(self "https://unifaiv2.dev-yp.com"), geolocation=()`)
...
func (m *AuthMiddleware) middleware(shouldSkip func(*configstore.AuthConfig, string) bool) schemas.UnifAIHTTPMiddleware {
	return func(next fasthttp.RequestHandler) fasthttp.RequestHandler {
		return func(ctx *fasthttp.RequestCtx) {
			...
			if shouldSkip(authConfig, url) || isPublicBrowserAIRoute(string(ctx.Method()), url) {
				next(ctx)
				return
			}
			...
			if scheme == "Bearer" {
				// We are checking for API keys first; it it seems like a valid UnifAI API key
