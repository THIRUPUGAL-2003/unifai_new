...
type MCPToolManager interface {
	...
	ExecuteChatMCPTool(ctx context.Context, toolCall *schemas.ChatAssistantMessageToolCall) (*schemas.ChatMessage, *schemas.UnifAIError)
	ExecuteResponsesMCPTool(ctx context.Context, toolCall *schemas.ResponsesToolMessage) (*schemas.ResponsesMessage, *schemas.UnifAIError)
}
...
func (h *MCPServerHandler) RegisterRoutes(r *router.Router, middlewares ...schemas.UnifAIHTTPMiddleware) {
...
func (h *MCPServerHandler) handleMCPServer(ctx *fasthttp.RequestCtx) {
	...
	unifaiCtx.SetValue(schemas.UnifAIContextKeyIsMCPGateway, true)
	defer cancel()

	// Inject JWT identity into UnifAIContext so downstream resolvers
	...
	if authResult.jwtClaims != nil {
		if injErr := injectJWTContext(unifaiCtx, authResult.jwtClaims, authResult.jwtVK); injErr != nil {
	...
	response := mcpServer.HandleMessage(unifaiCtx, ctx.PostBody())
...
func (h *MCPServerHandler) handleMCPServerSSE(ctx *fasthttp.RequestCtx) {
	...
	go func() {
		...
		// false — fasthttp.RequestCtx never cancels unifaiCtx on its own.
...
func (h *MCPServerHandler) syncServer(server *server.MCPServer, availableTools []schemas.ChatTool, toolFilter []string) {
	...
	for _, tool := range availableTools {
		...
		handler := func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			...
			// Convert to UnifAI tool call format
			...
			if err != nil {
				...
				return mcp.NewToolResultError(fmt.Sprintf("Tool execution failed: %v", unifai.GetErrorMessage(err))), nil
			}
		...
		// Map UnifAI annotations back to MCP tool annotations
...
func convertToolFunctionParametersToMCPInputSchema(params *schemas.ToolFunctionParameters) mcp.ToolInputSchema {
	...
	// Sanitize any root level pattern lookahead
	if params.Pattern != nil && strings.Contains(*params.Pattern, "(?") {
		params.Pattern = nil
	}
	if params.Items != nil {
		sanitizeOrderedMap(params.Items)
	}
	for i := range params.AnyOf {
		sanitizeOrderedMap(&params.AnyOf[i])
	}
	for i := range params.OneOf {
		sanitizeOrderedMap(&params.OneOf[i])
	}
	for i := range params.AllOf {
		sanitizeOrderedMap(&params.AllOf[i])
	}
	...
	if params.Properties != nil {
		...
		params.Properties.Range(func(key string, value interface{}) bool {
			props[key] = sanitizeSchemaValue(value)
			return true
		})
	...
	if params.Defs != nil {
		...
		params.Defs.Range(func(key string, value interface{}) bool {
			defs[key] = sanitizeSchemaValue(value)
			return true
		})
...
func sanitizeOrderedMap(om *schemas.OrderedMap) {
	if om == nil {
		return
	}
	if pat, ok := om.Get("pattern"); ok {
	...
	for _, key := range keys {
		if val, ok := om.Get(key); ok {
			om.Set(key, sanitizeSchemaValue(val))
		}
	}
}

func sanitizeSchemaValue(val interface{}) interface{} {
	switch v := val.(type) {
	case map[string]interface{}:
		...
		for k, val := range v {
			v[k] = sanitizeSchemaValue(val)
		}
		return v
	case *schemas.OrderedMap:
		sanitizeOrderedMap(v)
		return v
	case schemas.OrderedMap:
		sanitizeOrderedMap(&v)
		return v
...
// response based on the x-uf-mcp-include-clients and x-uf-mcp-include-tools request headers.
...
type mcpAuthResult struct {
	...
	jwtVK     *tables.TableVirtualKey // non-nil when jwt uf_mode=vk
}
...
// into the UnifAIContext after it is created.
//
...
func (h *MCPServerHandler) getMCPServerForRequest(ctx *fasthttp.RequestCtx) (*mcpAuthResult, error) {
	...
	// virtual key. oauth-strict accepts only UnifAI-issued tokens and is excluded.
	if h.identityResolver != nil &&
		(authMode == tables.MCPServerAuthModeHeaders || authMode == tables.MCPServerAuthModeBoth) {
		if userID, _ := ctx.UserValue(schemas.UnifAIContextKeyUserID).(string); userID != "" {
	...
	if rawJWT := extractBearerJWT(ctx); rawJWT != "" && discoveryEnabled {
		...
		// header-based virtual key (x-uf-vk / x-api-key / x-goog-api-key / Bearer vk) is also
		...
		// (UnifAIContextKeyUserID, set by the auth middleware) it must match
		...
		if schemas.MCPAuthMode(claims.BfMode) == schemas.MCPAuthModeUser {
			sessionUserID, _ := ctx.UserValue(schemas.UnifAIContextKeyUserID).(string)
	...
	if vk == "" {
		...
		return nil, fmt.Errorf("virtual key required to access mcp server; set one of x-uf-vk, Authorization: Bearer <vk>, x-api-key, or x-goog-api-key in your MCP client config")
	}
...
//  1. x-uf-vk        — taken verbatim (no prefix check)
...
// provider credentials pass through untouched, so only UnifAI virtual keys are
...
func getVKFromRequest(ctx *fasthttp.RequestCtx) string {
	if value := strings.TrimSpace(string(ctx.Request.Header.Peek(string(schemas.UnifAIContextKeyVirtualKey)))); value != "" {
		return value
	}
