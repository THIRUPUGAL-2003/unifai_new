...
// extra_fields that UnifAIChatResponse would include.
...
func cursorChunkID(extras *schemas.UnifAIResponseExtraFields) string {
	return "chatcmpl-unifai-" + strconv.Itoa(extras.ChunkIndex)
}

// cursorModel returns the best model name available from extra fields.
func cursorModel(extras *schemas.UnifAIResponseExtraFields) string {
...
func convertResponsesStreamToChatChunk(resp *schemas.UnifAIResponsesStreamResponse) (string, interface{}, error) {
	switch resp.Type {
...
func convertResponsesResponseToChatCompletion(resp *schemas.UnifAIResponsesResponse) *cursorChatCompletion {
...
// normalizeUnifAIInputContentBlocks ensures all input messages in a UnifAIResponsesRequest
// have ContentBlocks instead of ContentStr. This is a defense-in-depth normalization that runs
// AFTER ToUnifAIResponsesRequest, which can re-introduce ContentStr when the input is a string.
func normalizeUnifAIInputContentBlocks(req *schemas.UnifAIResponsesRequest) {
	if req == nil {
		return
	}
		...
		routes = append(routes, RouteConfig{
			...
			RequestConverter: func(ctx *schemas.UnifAIContext, req interface{}) (*schemas.UnifAIRequest, error) {
				if openaiReq, ok := req.(*openai.OpenAIResponsesRequest); ok {
					unifaiReq := openaiReq.ToUnifAIResponsesRequest(ctx)
					if unifaiReq == nil {
						return nil, errors.New("unifai responses request conversion returned nil")
					}
			...
			PreCallback: func(ctx *fasthttp.RequestCtx, unifaiCtx *schemas.UnifAIContext, req interface{}) error {
				...
				unifaiCtx.SetValue(schemas.UnifAIContextKeyUserAgent, schemas.Cursor.String())
				return nil
			},
		})
	}
...
// NewCursorRouter creates a new CursorRouter with the given unifai client.
func NewCursorRouter(client *unifai.UnifAI, handlerStore lib.HandlerStore, logger schemas.Logger) *CursorRouter {
	routes := []RouteConfig{}
