...
func TestRequestWithSettableExtraParams_OpenAIChatRequest(t *testing.T) {
	...
	t.Run("extra params propagate through ToUnifAIChatRequest", func(t *testing.T) {
		req := &openai.OpenAIChatRequest{
		...
		unifaiReq := req.ToUnifAIChatRequest(ctx)

		require.NotNil(t, unifaiReq)
		require.NotNil(t, unifaiReq.Params)
		assert.Contains(t, unifaiReq.Params.ExtraParams, "guardrailConfig")
	})
}
...
func TestExtraParamsRequiresPassthroughHeader(t *testing.T) {
	...
	t.Run("extra_params NOT extracted without passthrough header", func(t *testing.T) {
		...
		unifaiCtx := schemas.NewUnifAIContext(nil, schemas.NoDeadline)
		// Header not set -- simulate router logic
		if unifaiCtx.Value(schemas.UnifAIContextKeyPassthroughExtraParams) == true {
...
func TestExtraParamsPassthrough_EndToEnd(t *testing.T) {
	...
	unifaiReq := req.ToUnifAIChatRequest(unifaiCtx)

	require.NotNil(t, unifaiReq)
	require.NotNil(t, unifaiReq.Params)
	require.Contains(t, unifaiReq.Params.ExtraParams, "guardrailConfig")

	gc, ok := unifaiReq.Params.ExtraParams["guardrailConfig"].(map[string]interface{})
	require.True(t, ok)
	assert.Equal(t, "my-guardrail", gc["guardrailIdentifier"])
	assert.Equal(t, "1", gc["guardrailVersion"])
	assert.Equal(t, "disabled", gc["trace"])

	assert.NotContains(t, unifaiReq.Params.ExtraParams, "model")
	assert.NotContains(t, unifaiReq.Params.ExtraParams, "messages")
	assert.NotContains(t, unifaiReq.Params.ExtraParams, "stream")
	assert.NotContains(t, unifaiReq.Params.ExtraParams, "temperature")
}
...
func TestOpenAIChatStructuredOutputRequestParserAndConverter(t *testing.T) {
	...
	unifaiCtx := schemas.NewUnifAIContext(nil, schemas.NoDeadline)
	unifaiReq, err := chatRoute.RequestConverter(unifaiCtx, req)
	require.NoError(t, err)
	require.NotNil(t, unifaiReq)
	require.NotNil(t, unifaiReq.ChatRequest)
	require.NotNil(t, unifaiReq.ChatRequest.Params)
	require.NotNil(t, unifaiReq.ChatRequest.Params.ResponseFormat)

	assert.Equal(t, schemas.Gemini, unifaiReq.ChatRequest.Provider)
	assert.Equal(t, "gemini-2.5-flash", unifaiReq.ChatRequest.Model)
	assert.False(t, req.(*openai.OpenAIChatRequest).IsStreamingRequested())

	responseFormat, ok := (*unifaiReq.ChatRequest.Params.ResponseFormat).(map[string]interface{})
	require.True(t, ok)
...
func TestCreateHandler_AnthropicRouteClears_UseRawRequestBody_WhenCatalogSelectsBedrock(t *testing.T) {
	...
	route := RouteConfig{
		...
		RequestConverter: func(ctx *schemas.UnifAIContext, req interface{}) (*schemas.UnifAIRequest, error) {
			capturedUseRaw = ctx.Value(schemas.UnifAIContextKeyUseRawRequestBody)
			capturedSendRawResponse = ctx.Value(schemas.UnifAIContextKeySendBackRawResponse)
			capturedPassthroughOverrides = ctx.Value(schemas.UnifAIContextKeyPassthroughOverridesPresent)
			return nil, fmt.Errorf("stop before unifai execution")
		},
...
func TestExtraParamsSetViaInterfaceMutatesOriginalReq(t *testing.T) {
	...
	require.NotNil(t, unifaiReq.ChatRequest)
	require.NotNil(t, unifaiReq.ChatRequest.Params)
	assert.Contains(t, unifaiReq.ChatRequest.Params.ExtraParams, "guardrailConfig",
		"extra params should propagate through RequestConverter to UnifAIChatRequest")
}
