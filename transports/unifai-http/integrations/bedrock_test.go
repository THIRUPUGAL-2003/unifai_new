...
func Test_bedrockStreamErrorConverterEncodesEventStreamException(t *testing.T) {
	...
	converted := bedrockStreamErrorConverter(nil, unifaiErr)
	exception, ok := converted.(*bedrockEventStreamException)
	require.True(t, ok)

	reader := lib.NewSSEStreamReader()
	require.True(t, sendBedrockEventStreamException(reader, eventstream.NewEncoder(), exception, unifai.NewNoOpLogger()))
	reader.Done()
...
func Test_handleStreamingBedrockUnknownErrorResponseFallsBackToEventStreamException(t *testing.T) {
	stream := make(chan *schemas.UnifAIStreamChunk, 1)
	statusCode := fasthttp.StatusInternalServerError
	stream <- &schemas.UnifAIStreamChunk{
	...
	router := NewGenericRouter(nil, &mockHandlerStore{}, nil, nil, unifai.NewNoOpLogger())
...
func Test_createBedrockRerankRouteRequestConverter(t *testing.T) {
	...
	assert.Equal(t, schemas.Bedrock, unifaiReq.RerankRequest.Provider)
	assert.Equal(t, "capital of france", unifaiReq.RerankRequest.Query)
	require.Len(t, unifaiReq.RerankRequest.Documents, 1)
...
func Test_s3PutObjectPostCallback(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			response: &schemas.UnifAIFileUploadResponse{
				ID: "file-123",
			},
			wantStatus: 200,
			wantETag:   "\"file-123\"",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, "unifai", string(ctx.Response.Header.Peek("x-amz-request-id")))
...
func Test_s3GetObjectPostCallback(t *testing.T) {
	tests := []struct {
		...
		{
			name: "valid response",
			response: &schemas.UnifAIFileContentResponse{
...
func Test_s3HeadObjectPostCallback(t *testing.T) {
	tests := []struct {
		...
		{
			name: "valid response",
			response: &schemas.UnifAIFileRetrieveResponse{
				ID:    "file-789",
				Bytes: 1024,
			},
			wantStatus: 200,
...
func Test_extractBedrockBatchListQueryParams(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			unifaiCtx := createTestUnifAIContext()
			err := callback(ctx, unifaiCtx, req)
...
func Test_extractBedrockJobArnFromPath(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			unifaiCtx := createTestUnifAIContextWithProvider(tt.provider)
			err := callback(ctx, unifaiCtx, req)
...
func Test_extractS3ListObjectsV2Params(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, tt.wantBucket, unifaiCtx.Value(s3ContextKeyBucket))
			assert.Equal(t, tt.wantPrefix, unifaiCtx.Value(s3ContextKeyPrefix))
			assert.Equal(t, tt.wantMaxKeys, unifaiCtx.Value(s3ContextKeyMaxKeys))
		})
	}
}
...
func createTestUnifAIContext() *schemas.UnifAIContext {
	unifaiCtx := schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline)
	unifaiCtx.SetValue(unifaiContextKeyProvider, schemas.Bedrock)
	return unifaiCtx
}

func createTestUnifAIContextWithProvider(provider schemas.ModelProvider) *schemas.UnifAIContext {
	unifaiCtx := schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline)
