...
// NewPydanticAIRouter creates a new PydanticAIRouter with the given unifai client.
func NewPydanticAIRouter(client *unifai.UnifAI, handlerStore lib.HandlerStore, logger schemas.Logger) *PydanticAIRouter {
	routes := []RouteConfig{}
...
func withPydanticResponsesNullNormalization(routes []RouteConfig) []RouteConfig {
	for i := range routes {
		...
		if routes[i].ResponsesResponseConverter != nil {
			routes[i].ResponsesResponseConverter = func(ctx *schemas.UnifAIContext, resp *schemas.UnifAIResponsesResponse) (interface{}, error) {
				// For pydantic responses endpoint, prefer normalized unifai output
		...
		if routes[i].StreamConfig != nil && routes[i].StreamConfig.ResponsesStreamResponseConverter != nil {
			...
			routes[i].StreamConfig.ResponsesStreamResponseConverter = func(ctx *schemas.UnifAIContext, resp *schemas.UnifAIResponsesStreamResponse) (string, interface{}, error) {
				if resp == nil {
					return "", nil, nil
				}
...
func ensurePydanticResponsesStreamTextFields(resp *schemas.UnifAIResponsesStreamResponse) {
	if resp == nil {
		return
	}
	...
	case schemas.ResponsesStreamResponseTypeOutputTextDelta:
		if resp.Delta == nil {
			resp.Delta = unifai.Ptr("")
		}
	case schemas.ResponsesStreamResponseTypeOutputTextDone:
		if resp.Text == nil {
			resp.Text = unifai.Ptr("")
		}
	}
}
