// Package lib provides core functionality for the UnifAI HTTP service,
// including context propagation, header management, and integration with monitoring systems.
//
// This package handles the conversion of FastHTTP request contexts to UnifAI contexts,
...
const (
	// FastHTTPUserValueUnifAIContext stores the active *schemas.UnifAIContext on fasthttp.RequestCtx.
	// This allows transport middleware and request handlers to share the same context instance.
	FastHTTPUserValueUnifAIContext = "__unifai_context"
	// FastHTTPUserValueUnifAICancel stores the cancel func for the active shared UnifAI context.
	FastHTTPUserValueUnifAICancel = "__unifai_context_cancel"
	...
	FastHTTPUserValueLargeResponseMode = "__unifai_large_response_mode"
	...
	// provider was auto-resolved. Picked up centrally in ConvertToUnifAIContext to
	// add the routing engine log via EmitModelCatalogRoutingLog.
	FastHTTPUserValueModelCatalogResolution = "__unifai_model_catalog_resolution"
)

// ModelCatalogResolution carries the result of an automatic provider lookup so
// that ConvertToUnifAIContext can emit the routing engine log in one place.
...
// engines-used marker to unifaiCtx for an inline catalog resolution. Used by
// ConvertToUnifAIContext (normal HTTP path) and by realtime handlers that
...
func EmitModelCatalogRoutingLog(unifaiCtx *schemas.UnifAIContext, res *ModelCatalogResolution) {
	if unifaiCtx == nil || res == nil {
		return
	}
	...
	unifaiCtx.AppendRoutingEngineLog(schemas.RoutingEngineModelCatalog, schemas.LogLevelInfo, fmt.Sprintf(
	...
	schemas.AppendToContextList(unifaiCtx, schemas.UnifAIContextKeyRoutingEnginesUsed, schemas.RoutingEngineModelCatalog)
}
...
// ConvertToUnifAIContext converts a FastHTTP RequestCtx to a UnifAI context,
...
//     schemas.UnifAIContextKeyDimensions and are forwarded to all observability integrations
...
//
// 1a. Prometheus Headers (x-uf-prom-*) [DEPRECATED — use x-uf-dim-* instead]:
//   - All headers prefixed with 'x-uf-prom-' are still accepted for backward compatibility.
//
// 2. Maxim Tracing Headers (x-uf-maxim-*):
//   - Specifically handles 'x-uf-maxim-traceID' and 'x-uf-maxim-generationID'
...
//
// 3. MCP Headers (x-uf-mcp-*):
//   - Specifically handles 'x-uf-mcp-include-clients' and 'x-uf-mcp-include-tools' (include-only filtering)
...
//   - x-uf-vk: Virtual key for governance (required for governance to work)
//
// 5. API Key Headers:
...
//   - Keys are extracted and stored in the context using schemas.UnifAIContextKey
...
//   - Any header starting with 'x-uf-eh-' is collected and added to the map stored under schemas.UnifAIContextKeyExtraHeaders
...
//   - x-uf-session-id: Session identifier for key binding (reuse same key across requests)
...
//   - x-uf-send-back-raw-request: include raw provider request in the UnifAIResponse returned to the caller
//   - x-uf-send-back-raw-response: include raw provider response in the UnifAIResponse returned to the caller
...
//
// Returns:
//   - *schemas.UnifAIContext: A new cancellable context containing the propagated values
//   - context.CancelFunc: Function to cancel the context (should be called when request completes)
//
// Example Usage:
//
//	fastCtx := &fasthttp.RequestCtx{...}
//	unifaiCtx, cancel := ConvertToUnifAIContext(fastCtx, handlerStore)
//	defer cancel() // Ensure cleanup
//	// unifaiCtx now contains propagated header values including Prometheus metrics,
//	// Maxim tracing data, MCP filters, governance keys, API keys, cache settings,
//	// session stickiness, and extra headers

func ConvertToUnifAIContext(ctx *fasthttp.RequestCtx, store HandlerStore) (*schemas.UnifAIContext, context.CancelFunc) {
	...
	var unifaiCtx *schemas.UnifAIContext
	var cancel context.CancelFunc
	if existing, ok := ctx.UserValue(FastHTTPUserValueUnifAIContext).(*schemas.UnifAIContext); ok && existing != nil {
		if existingCancel, ok := ctx.UserValue(FastHTTPUserValueUnifAICancel).(context.CancelFunc); ok && existingCancel != nil {
			unifaiCtx = existing
			cancel = existingCancel
		} else {
			// Create one cancellable child context and promote it as the shared context.
			unifaiCtx, cancel = schemas.NewUnifAIContextWithCancel(existing)
			ctx.SetUserValue(FastHTTPUserValueUnifAIContext, unifaiCtx)
			ctx.SetUserValue(FastHTTPUserValueUnifAICancel, cancel)
		}
	}
	if unifaiCtx == nil {
		...
		unifaiCtx, cancel = schemas.NewUnifAIContextWithCancel(parent)
		ctx.SetUserValue(FastHTTPUserValueUnifAIContext, unifaiCtx)
		ctx.SetUserValue(FastHTTPUserValueUnifAICancel, cancel)
	}

	// Preserve existing request-id if already present on the shared context.
	if existingRequestID, ok := unifaiCtx.Value(schemas.UnifAIContextKeyRequestID).(string); !ok || existingRequestID == "" {
		// First, check if x-request-id header exists
		requestID := string(ctx.Request.Header.Peek("x-request-id"))
		if requestID == "" {
			requestID = uuid.New().String()
		}
		unifaiCtx.SetValue(schemas.UnifAIContextKeyRequestID, requestID)
	}
	// Populating all user values from the request context
	ctx.VisitUserValuesAll(func(key, value any) {
		unifaiCtx.SetValue(key, value)
	})
	...
	if res, ok := ctx.UserValue(FastHTTPUserValueModelCatalogResolution).(*ModelCatalogResolution); ok && res != nil {
		EmitModelCatalogRoutingLog(unifaiCtx, res)
	}

	// Initialize tags map for collecting maxim tags
	maximTags := make(map[string]string)
	// Initialize dimensions map for x-uf-dim-* headers
	dimensions := make(map[string]string)
	// Initialize extra headers map for headers prefixed with x-uf-eh-
	...
	securityDenylist := map[string]bool{
		...
		// prevent auth/key overrides via x-uf-eh-*
		"x-api-key":       true,
		"x-goog-api-key":  true,
		"x-uf-api-key":    true,
		"x-uf-api-key-id": true,
		"x-uf-vk":         true,
		"x-uf-direct-key": true,
	}
	...
	ctx.Request.Header.All()(func(key, value []byte) bool {
		...
		if keyStr == "baggage" {
			if sessionID := ParseSessionIDFromBaggage(string(value)); sessionID != "" {
				unifaiCtx.SetValue(schemas.UnifAIContextKeyParentRequestID, sessionID)
			}
			return true
		}
		...
		if labelName, ok := strings.CutPrefix(keyStr, "x-uf-prom-"); ok && labelName != "" {
			// x-uf-prom-* is Prometheus-only and must not flow into unified dimensions
		...
		if labelName, ok := strings.CutPrefix(keyStr, "x-uf-maxim-"); ok {
			...
			case string(maxim.GenerationIDKey):
				unifaiCtx.SetValue(schemas.UnifAIContextKey(labelName), string(value))
		...
		if labelName, ok := strings.CutPrefix(keyStr, "x-uf-mcp-"); ok {
			switch labelName {
			...
			case "include-tools":
				...
				unifaiCtx.SetValue(schemas.UnifAIContextKey("mcp-"+labelName), parsedValues)
				return true
			}
		}
		// Handle MCP session ID header (x-uf-mcp-session-id): a client-issued
		...
		if keyStr == "x-uf-mcp-session-id" {
			if v := strings.TrimSpace(string(value)); v != "" {
				if len(v) > 255 {
					// Don't echo any of the value — x-uf-mcp-session-id is a
					// re-presentable lookup key for session-mode OAuth; the
					// length alone is enough for debugging.
					if logger != nil {
						logger.Warn("x-uf-mcp-session-id exceeds 255 chars, ignoring: length=%d (skipped last %d chars)", len(v), len(v)-255)
					}
					return true
				}
				unifaiCtx.SetValue(schemas.UnifAIContextKeyMCPSessionID, v)
			}
			return true
		}
		// Handle virtual key header (x-uf-vk, authorization, x-api-key, x-goog-api-key headers)
		if keyStr == string(schemas.UnifAIContextKeyVirtualKey) {
			unifaiCtx.SetValue(schemas.UnifAIContextKeyVirtualKey, string(value))
			return true
		}
		if keyStr == "authorization" {
			...
			if strings.HasPrefix(strings.ToLower(valueStr), "bearer ") {
				...
				if authHeaderValue != "" && strings.HasPrefix(strings.ToLower(authHeaderValue), governance.VirtualKeyPrefix) {
					unifaiCtx.SetValue(schemas.UnifAIContextKeyVirtualKey, authHeaderValue)
					return true
				}
			}
		}
		...
		if keyStr == "x-uf-api-key" {
			if keyName := strings.TrimSpace(string(value)); keyName != "" {
				unifaiCtx.SetValue(schemas.UnifAIContextKeyAPIKeyName, keyName)
			}
			return true
		}
		if keyStr == "x-uf-api-key-id" {
			if keyID := strings.TrimSpace(string(value)); keyID != "" {
				unifaiCtx.SetValue(schemas.UnifAIContextKeyAPIKeyID, keyID)
			}
			return true
		}
		// Handle cache key header (x-uf-cache-key)
		if keyStr == "x-uf-cache-key" {
			unifaiCtx.SetValue(semanticcache.CacheKey, string(value))
			return true
		}
		// Handle cache TTL header (x-uf-cache-ttl)
		if keyStr == "x-uf-cache-ttl" {
			valueStr := string(value)
		...
		if keyStr == "x-uf-cache-threshold" {
			...
			if err == nil {
				...
				unifaiCtx.SetValue(semanticcache.CacheThresholdKey, threshold)
			}
		...
		if keyStr == "x-uf-cache-type" {
			unifaiCtx.SetValue(semanticcache.CacheTypeKey, semanticcache.CacheType(string(value)))
			return true
		}
		// Cache no store header
		if keyStr == "x-uf-cache-no-store" {
			if valueStr := string(value); valueStr == "true" {
				unifaiCtx.SetValue(semanticcache.CacheNoStoreKey, true)
			}
			return true
		}
		...
		if keyStr == "x-uf-session-id" {
			if valueStr := strings.TrimSpace(string(value)); valueStr != "" {
				unifaiCtx.SetValue(schemas.UnifAIContextKeySessionID, valueStr)
			}
			return true
		}
		// Session stickiness: per-request TTL override (duration string or seconds integer)
		if keyStr == "x-uf-session-ttl" {
			...
			if err == nil && ttlDuration > 0 {
				unifaiCtx.SetValue(schemas.UnifAIContextKeySessionTTL, ttlDuration)
			}
			return true
		}
		if labelName, ok := strings.CutPrefix(keyStr, "x-uf-eh-"); ok {
		...
		if keyStr == "x-uf-send-back-raw-request" {
			if b, err := strconv.ParseBool(string(value)); err == nil {
				unifaiCtx.SetValue(schemas.UnifAIContextKeySendBackRawRequest, b)
			}
			return true
		}
		if keyStr == "x-uf-send-back-raw-response" {
			if b, err := strconv.ParseBool(string(value)); err == nil {
				unifaiCtx.SetValue(schemas.UnifAIContextKeySendBackRawResponse, b)
			}
			return true
		}
		if keyStr == "x-uf-store-raw-request-response" {
			if b, err := strconv.ParseBool(string(value)); err == nil {
				unifaiCtx.SetValue(schemas.UnifAIContextKeyStoreRawRequestResponse, b)
			}
			return true
		}
		if keyStr == "x-uf-disable-content-logging" {
			if b, err := strconv.ParseBool(string(value)); err == nil {
				unifaiCtx.SetValue(schemas.UnifAIContextKeyDisableContentLogging, b)
			}
			return true
		}
		...
		if keyStr == "x-uf-parent-request-id" {
			if valueStr := strings.TrimSpace(string(value)); valueStr != "" {
				unifaiCtx.SetValue(schemas.UnifAIMCPAgentOriginalRequestID, valueStr)
			}
			return true
		}
		...
		if keyStr == "x-uf-compat" {
			unifaiCtx.ClearValue(schemas.UnifAIContextKeyCompatConvertTextToChat)
			unifaiCtx.ClearValue(schemas.UnifAIContextKeyCompatConvertChatToResponses)
			unifaiCtx.ClearValue(schemas.UnifAIContextKeyCompatShouldDropParams)
			unifaiCtx.ClearValue(schemas.UnifAIContextKeyCompatShouldConvertParams)
			valueStr := strings.TrimSpace(string(value))
			if valueStr == "true" {
				unifaiCtx.SetValue(schemas.UnifAIContextKeyCompatConvertTextToChat, true)
				unifaiCtx.SetValue(schemas.UnifAIContextKeyCompatConvertChatToResponses, true)
				unifaiCtx.SetValue(schemas.UnifAIContextKeyCompatShouldDropParams, true)
				unifaiCtx.SetValue(schemas.UnifAIContextKeyCompatShouldConvertParams, true)
	...
	if len(maximTags) > 0 {
		unifaiCtx.SetValue(schemas.UnifAIContextKey(maxim.TagsKey), maximTags)
	}

	// Store collected dimensions (x-uf-dim-* only) in the context
	if len(dimensions) > 0 {
		unifaiCtx.SetValue(schemas.UnifAIContextKeyDimensions, dimensions)
	}

	// Store collected extra headers in the context if any were found
	if len(extraHeaders) > 0 {
		unifaiCtx.SetValue(schemas.UnifAIContextKeyExtraHeaders, extraHeaders)
	}

	// Store collected MCP extra headers in the context if any were found
	if len(mcpExtraHeaders) > 0 {
		unifaiCtx.SetValue(schemas.UnifAIContextKeyMCPExtraHeaders, mcpExtraHeaders)
	}
	...
	unifaiCtx.SetValue(schemas.UnifAIContextKeyRequestHeaders, allHeaders)
	...
	if queryArgs.Len() > 0 {
		...
		unifaiCtx.SetValue(schemas.UnifAIContextKeyRequestQuery, allQuery)
	}
	...
	// path) resolvers when initiating their respective auth flows. UnifAI is
	...
	if baseURL != "" {
		unifaiCtx.SetValue(schemas.UnifAIContextKeyMCPCallbackBaseURL, baseURL)
	}

	unifaiCtx.SetValue(schemas.UnifAIContextKeyAllowPerRequestStorageOverride, allowPerRequestStorageOverride)
	unifaiCtx.SetValue(schemas.UnifAIContextKeyAllowPerRequestRawOverride, allowPerRequestRawOverride)
	...
	if store != nil && store.ShouldAllowDirectKeys() && string(ctx.Request.Header.Peek("x-uf-direct-key")) == "true" {
		var apiKey string
		...
		if apiKey != "" {
			...
			unifaiCtx.SetValue(schemas.UnifAIContextKeyDirectKey, key)
		}
	}

	return unifaiCtx, cancel
}
...
// internal Host header seen by UnifAI.
func BuildBaseURL(ctx *fasthttp.RequestCtx, externalBaseURL string) string {
	...
	// x-uf-forwarded-proto is honored for setups where a managed LB overwrites the
	// standard X-Forwarded-Proto (e.g. AWS L4 NLB -> ALB hops); the edge injects it.
	xbfProto := strings.ToLower(strings.TrimSpace(string(ctx.Request.Header.Peek("x-uf-forwarded-proto"))))
