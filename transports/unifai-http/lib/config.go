...
import (
	...
	mcputils "github.com/unifai/unifai/core/mcp/utils"
	"github.com/unifai/unifai/core/schemas"
	"github.com/unifai/unifai/framework"
	...
	"github.com/unifai/unifai/framework/envutils"
	"github.com/unifai/unifai/framework/featureflags"
	"github.com/unifai/unifai/framework/kvstore"
	"github.com/unifai/unifai/framework/logstore"
	"github.com/unifai/unifai/framework/mcp_headers"
	"github.com/unifai/unifai/framework/mcpcatalog"
	"github.com/unifai/unifai/framework/modelcatalog"
	"github.com/unifai/unifai/framework/oauth2"
...
type StreamChunkInterceptor interface {
	...
	InterceptChunk(ctx *schemas.UnifAIContext, req *schemas.HTTPRequest, chunk *schemas.UnifAIStreamChunk) (*schemas.UnifAIStreamChunk, error)
}
...
type HandlerStore interface {
	...
	// GetMCPExternalClientURL returns the configured external base URL UnifAI uses as the
...
// ConfigData represents the configuration data for the UnifAI HTTP transport.
...
// (string literal), or {"enabled": "env.UNIFAI_FOO"} (env-var indirection).
...
// Config represents a high-performance in-memory configuration store for UnifAI.
...
type Config struct {
	...
	// verify UnifAI-issued /mcp JWTs. The key is created once via an idempotent
	...
	// Constructed alongside OAuthProvider and passed into the UnifAI core
	...
	// environment (e.g. "staging", "prod"). Set via config.json env_label or UNIFAI_ENV_LABEL env var.
	EnvLabel string
...
func LoadConfig(ctx context.Context, configDirPath string) (*Config, error) {
	...
	if err != nil {
		...
		if schema["$schema"] != "https://www.getunifai.ai/schema" {
			yellowColor := "\033[33m"
			resetColor := "\033[0m"
			message := fmt.Sprintf("config file %s does not include \"$schema\":\"https://www.getunifai.ai/schema\". Use our official schema file to avoid unexpected behavior.", absConfigFilePath)
			boxWidth := 100
			...
			fmt.Printf("%s╚%s╝%s\n", yellowColor, strings.Repeat("═", boxWidth-2), resetColor)
			fmt.Println("")
			logger.Warn("config file %s does not include \"$schema\":\"https://www.getunifai.ai/schema\". Use our official schema file to avoid unexpected behavior.", absConfigFilePath)
		}
		// Parse config data
	...
	// 12. Env label (config.json takes precedence over UNIFAI_ENV_LABEL env var)
	...
	} else if label := strings.TrimSpace(os.Getenv("UNIFAI_ENV_LABEL")); label != "" {
		config.EnvLabel = truncateLabel(label)
	}
	...
	if overlay, err := loadGuardrailsOverlayFromFile(guardrailsOverlayPath(configFilePath)); err != nil {
		logger.Warn("failed to load guardrails overlay file: %v", err)
	} else if overlay != nil {
...
func applyMCPGlobalSettingsToClientConfig(ctx context.Context, config *Config, mcpCfg *schemas.MCPConfig) {
	...
	// Backfill MCPConfig.ToolManagerConfig from ClientConfig so unifai.Init always receives
	...
	// Allocate if absent so unifai.Init never falls back to hardcoded defaults.
...
func resolveGovernanceKeyReferences(ctx context.Context, config *Config, governanceConfig *configstore.GovernanceConfig) error {
	...
	return config.ConfigStore.ExecuteTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i := range governanceConfig.PricingOverrides {
			...
			override.ProviderKeyID = unifai.Ptr(keyID)
			override.ProviderKeyName = nil
		}
	...
	if len(config.PluginConfigs) == 0 {
		...
		for _, plugin := range configData.Plugins {
			if plugin.Version == nil {
				plugin.Version = unifai.Ptr(int16(1))
			}
...
func initEncryption(configData *ConfigData) error {
	if configData.EncryptionKey == nil || configData.EncryptionKey.GetValue() == "" {
		// Checking if UNIFAI_ENCRYPTION_KEY environment variable is set
		if os.Getenv("UNIFAI_ENCRYPTION_KEY") != "" {
			configData.EncryptionKey = schemas.NewSecretVar("env.UNIFAI_ENCRYPTION_KEY")
		}
	}
...
// guardrailsOverlayPath is a writable sidecar file for UI-managed guardrails config.
// Docker often bind-mounts config.json read-only; this file lives in the same data dir volume.
func guardrailsOverlayPath(configPath string) string {
	if configPath == "" {
		return "guardrails_config.json"
	}
	return filepath.Join(filepath.Dir(configPath), "guardrails_config.json")
}

func loadGuardrailsOverlayFromFile(path string) (*GuardrailsConfig, error) {
	...
	if err := json.Unmarshal(data, &cfg); err != nil {
		return nil, fmt.Errorf("parse guardrails overlay: %w", err)
	}
	return &cfg, nil
}

// PersistGuardrailsConfig writes guardrails_config to a writable overlay file and
// best-effort updates config.json when the path is writable.
func (c *Config) PersistGuardrailsConfig(cfg *GuardrailsConfig) error {
	...
	out, err := json.MarshalIndent(json.RawMessage(section), "", "  ")
	if err != nil {
		return fmt.Errorf("marshal guardrails overlay: %w", err)
	}
	out = append(out, '\n')

	overlayPath := guardrailsOverlayPath(c.configPath)
	tmpOverlay := overlayPath + ".tmp"
	if err := os.WriteFile(tmpOverlay, out, 0644); err != nil {
		return fmt.Errorf("write guardrails overlay: %w", err)
	}
	if err := os.Rename(tmpOverlay, overlayPath); err != nil {
		_ = os.Remove(tmpOverlay)
		return fmt.Errorf("replace guardrails overlay: %w", err)
	}

	if c.configPath != "" {
		if err := c.persistGuardrailsInMainConfig(cfg); err != nil {
			logger.Warn("guardrails saved to %s but config.json update skipped: %v", overlayPath, err)
		}
	}
	return nil
}

func (c *Config) persistGuardrailsInMainConfig(cfg *GuardrailsConfig) error {
...
func (c *Config) GetProviderConfigRaw(provider schemas.ModelProvider) (*configstore.ProviderConfig, error) {
	...
	// Return direct reference for maximum performance - this is used by UnifAI core
...
func (i *pluginChunkInterceptor) InterceptChunk(ctx *schemas.UnifAIContext, req *schemas.HTTPRequest, stream *schemas.UnifAIStreamChunk) (*schemas.UnifAIStreamChunk, error) {
	for j := len(i.plugins) - 1; j >= 0; j-- {
		plugin := i.plugins[j]
		pluginName := plugin.GetName()
		var (
			modified *schemas.UnifAIStreamChunk
			err      error
		)
		func() {
...
func initFeatureFlags(ctx context.Context, config *Config, configData *ConfigData) error {
	...
	isEnterprise, _ := ctx.Value(schemas.UnifAIContextKeyIsEnterprise).(bool)
...
func (c *Config) AddProvider(ctx context.Context, provider schemas.ModelProvider, config configstore.ProviderConfig) error {
	...
	skipDBUpdate := false
	if ctx.Value(schemas.UnifAIContextKeySkipDBUpdate) != nil {
		if skip, ok := ctx.Value(schemas.UnifAIContextKeySkipDBUpdate).(bool); ok {
			skipDBUpdate = skip
		}
	}
...
// SetUnifAIClient sets the UnifAI client in the store.
// This is used to allow the store to access the UnifAI client.
// This is useful for the MCP handler to access the UnifAI client.
func (c *Config) SetUnifAIClient(client *unifai.UnifAI) {
	c.muMCP.Lock()
...
func (c *Config) GetMCPClient(id string) (*schemas.MCPClientConfig, error) {
	...
	if c.client == nil {
		return nil, fmt.Errorf("unifai client not set")
	}
...
func (c *Config) AddMCPClient(ctx context.Context, clientConfig *schemas.MCPClientConfig) error {
	if c.client == nil {
		return fmt.Errorf("unifai client not set")
	}
	c.muMCP.Lock()
	defer c.muMCP.Unlock()
	if c.MCPConfig == nil {
...
func (c *Config) UpdateMCPClient(ctx context.Context, id string, updatedConfig *schemas.MCPClientConfig) error {
	...
	// is registered at runtime. We call the core unifai methods directly (not the
...
// It delegates the actual reconnection (with the new credentials) to the UnifAI client.
...
//   - Removes the MCP client from the UnifAI client
...
func ValidateCustomProvider(config configstore.ProviderConfig, provider schemas.ModelProvider) error {
	...
	if unifai.IsStandardProvider(provider) {
	...
	if !unifai.IsSupportedBaseProvider(cpc.BaseProviderType) {
