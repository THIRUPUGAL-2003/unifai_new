...
var localSchemaCandidates = []string{
	...
	"../config.schema.json",         // running from transports/unifai-http/
	"../../config.schema.json",      // running from transports/unifai-http/lib/
...
func ValidateConfigSchema(data []byte, schemaOverride ...[]byte) error {
	...
	if len(schemaOverride) > 0 && len(schemaOverride[0]) > 0 {
		...
		// Pulling config.schema from https://www.getunifai.ai/schema
		configSchemaJSON, err := http.Get("https://www.getunifai.ai/schema")
		if err != nil {
