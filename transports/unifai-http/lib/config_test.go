...
This file contains comprehensive tests for the UnifAI configuration system,
...
import (
	...
	otelPlugin "github.com/unifai/unifai/plugins/otel"
...
func createTempDir(t *testing.T) string {
	t.Helper()
	dir, err := os.MkdirTemp("", "unifai-test-*")
	if err != nil {
...
func TestLoadConfig_Governance_FirstImportUsesMergeIDHandling(t *testing.T) {
	...
	configData.Governance = &configstore.GovernanceConfig{
		...
		VirtualKeys: []tables.TableVirtualKey{
			{ID: "vk-explicit", Name: "Explicit VK", Value: *schemas.NewSecretVar("sk-uf-explicit")},
			{Name: "Generated VK"},
		},
	...
	require.Equal(t, "sk-uf-explicit", explicitVK.Value.GetValue())
	require.NotNil(t, generatedVK)
	_, err = uuid.Parse(generatedVK.ID)
	require.NoError(t, err)
	require.True(t, strings.HasPrefix(generatedVK.Value.GetValue(), "sk-uf-"))
...
func (p *mockLLMPlugin) PreRequestHook(_ *schemas.UnifAIContext, _ *schemas.UnifAIRequest) error {
	return nil
}

func (p *mockLLMPlugin) PreLLMHook(ctx *schemas.UnifAIContext, req *schemas.UnifAIRequest) (*schemas.UnifAIRequest, *schemas.LLMPluginShortCircuit, error) {
	return req, nil, nil
}

func (p *mockLLMPlugin) PostLLMHook(ctx *schemas.UnifAIContext, resp *schemas.UnifAIResponse, unifaiErr *schemas.UnifAIError) (*schemas.UnifAIResponse, *schemas.UnifAIError, error) {
...
func TestResolveFrameworkPricingConfig(t *testing.T) {
	...
	t.Run("unresolved env model_parameters_url does not overwrite db when another field changes", func(t *testing.T) {
		...
		rawModelParams := "env.UNIFAI_TEST_MODEL_PARAMS_URL_NONEXISTENT_XYZ"
		prev, existed := os.LookupEnv("UNIFAI_TEST_MODEL_PARAMS_URL_NONEXISTENT_XYZ")
		os.Unsetenv("UNIFAI_TEST_MODEL_PARAMS_URL_NONEXISTENT_XYZ")
		t.Cleanup(func() {
			if existed {
				os.Setenv("UNIFAI_TEST_MODEL_PARAMS_URL_NONEXISTENT_XYZ", prev)
			}
		})
	...
	t.Run("pricing_url with missing env var falls back to literal string", func(t *testing.T) {
		...
		rawURL := "env.UNIFAI_TEST_PRICING_URL_NONEXISTENT_XYZ"
		prev, existed := os.LookupEnv("UNIFAI_TEST_PRICING_URL_NONEXISTENT_XYZ")
		os.Unsetenv("UNIFAI_TEST_PRICING_URL_NONEXISTENT_XYZ")
		t.Cleanup(func() {
			if existed {
				os.Setenv("UNIFAI_TEST_PRICING_URL_NONEXISTENT_XYZ", prev)
			}
		})
		fileConfig := &framework.FrameworkConfig{
			Pricing: &modelcatalog.Config{
	...
	t.Run("pricing_url with valid env var is resolved", func(t *testing.T) {
		t.Setenv("UNIFAI_TEST_PRICING_URL_VALID", "https://resolved.example.com/pricing.json")
		rawURL := "env.UNIFAI_TEST_PRICING_URL_VALID"
		fileConfig := &framework.FrameworkConfig{
			Pricing: &modelcatalog.Config{
				PricingURL: &rawURL,
			},
	...
	t.Run("partial/embedded env string is treated as literal (no substitution)", func(t *testing.T) {
		...
		t.Setenv("UNIFAI_TEST_PRICING_HOST", "host.example.com")
		embeddedURL := "https://env.UNIFAI_TEST_PRICING_HOST/pricing.json"
		fileConfig := &framework.FrameworkConfig{
			Pricing: &modelcatalog.Config{
	...
	t.Run("db corrupted (zero) with valid file interval uses file value and requests db backfill", func(t *testing.T) {
		// Real-world recovery scenario: a pre-fix UnifAI wrote 0 nanoseconds (interpreted
...
func TestRemoveProvider_SkipDBUpdate(t *testing.T) {
	...
	ctx := context.WithValue(context.Background(), schemas.UnifAIContextKeySkipDBUpdate, true)
