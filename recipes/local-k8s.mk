...
K8S_NAMESPACE ?= unifai
K8S_RELEASE_NAME ?= unifai
K8S_HOST ?= unifai.local
UNIFAI_IMAGE_TAG ?= v1.3.38
...
deploy-local-k8s: ## Deploy UnifAI to local Kubernetes with storage options + HTTPS (Usage: make deploy-local-k8s [OPENAI_API_KEY='...'])
	@echo ""
	@echo "$(BLUE)╔════════════════════════════════════════════════════════════════╗$(NC)"
	@echo "$(BLUE)║  UnifAI Local K8s - Configurable Storage + Secrets + HTTPS    ║$(NC)"
	@echo "$(BLUE)╚════════════════════════════════════════════════════════════════╝$(NC)"
	@echo ""
	...
	case "$$CONFIG_CHOICE" in \
		1) echo "no" > /tmp/unifai-config-store.tmp ;; \
		3) echo "postgres" > /tmp/unifai-config-store.tmp ;; \
		*) echo "sqlite" > /tmp/unifai-config-store.tmp ;; \
	esac
	@echo ""
	...
	read LOG_CHOICE; \
	case "$$LOG_CHOICE" in \
		1) echo "no" > /tmp/unifai-log-store.tmp ;; \
		3) echo "postgres" > /tmp/unifai-log-store.tmp ;; \
		*) echo "sqlite" > /tmp/unifai-log-store.tmp ;; \
	esac
	@echo ""
	...
	@echo "$(CYAN)Step 5/5: Deploying UnifAI$(NC)"
	@CONFIG_STORE=$$(cat /tmp/unifai-config-store.tmp 2>/dev/null || echo "sqlite"); \
	LOG_STORE=$$(cat /tmp/unifai-log-store.tmp 2>/dev/null || echo "sqlite"); \
	$(MAKE) deploy-local-k8s-helm K8S_CONFIG_STORE="$$CONFIG_STORE" K8S_LOG_STORE="$$LOG_STORE" K8S_SKIP_PROMPTS=true
	@rm -f /tmp/unifai-config-store.tmp /tmp/unifai-log-store.tmp
	@echo ""
	@echo "$(GREEN)╔════════════════════════════════════════════════════════════════╗$(NC)"
...
setup-local-k8s-tls: ## Generate self-signed TLS certificate
	@mkdir -p /tmp/unifai-certs
	@openssl genrsa -out /tmp/unifai-certs/tls.key 2048 2>/dev/null
	@openssl req -new -x509 -key /tmp/unifai-certs/tls.key -out /tmp/unifai-certs/tls.crt \
		-days 365 -subj "/CN=$(K8S_HOST)" \
		-addext "subjectAltName=DNS:$(K8S_HOST),DNS:localhost,IP:127.0.0.1" 2>/dev/null
	@kubectl create secret tls unifai-tls-secret \
		--cert=/tmp/unifai-certs/tls.crt --key=/tmp/unifai-certs/tls.key \
		-n $(K8S_NAMESPACE) --dry-run=client -o yaml | kubectl apply -f - >/dev/null 2>&1
	@rm -rf /tmp/unifai-certs
	@echo "  $(GREEN)✓ TLS certificate for $(K8S_HOST)$(NC)"
	@echo ""

setup-local-k8s-secrets: ## Create Kubernetes secrets for UnifAI
	@# Ensure namespace exists first
	@kubectl create namespace $(K8S_NAMESPACE) --dry-run=client -o yaml 2>/dev/null | kubectl apply -f - >/dev/null 2>&1 || true
	$(eval UNIFAI_ENCRYPTION_KEY ?= $(shell openssl rand -base64 32))
	$(eval OPENAI_KEY := $(if $(OPENAI_API_KEY),$(OPENAI_API_KEY),sk-placeholder-replace-me))
	@kubectl create secret generic unifai-provider-keys \
		--from-literal=openai-api-key='$(OPENAI_KEY)' \
		-n $(K8S_NAMESPACE) --dry-run=client -o yaml | kubectl apply -f -
	@echo "  $(GREEN)✓ unifai-provider-keys$(NC)"
	@kubectl create secret generic unifai-encryption-key \
		--from-literal=encryption-key='$(UNIFAI_ENCRYPTION_KEY)' \
		-n $(K8S_NAMESPACE) --dry-run=client -o yaml | kubectl apply -f -
	@echo "  $(GREEN)✓ unifai-encryption-key$(NC)"
...
deploy-local-k8s-helm: ## Deploy UnifAI using Helm (Usage: make deploy-local-k8s-helm [K8S_CONFIG_STORE=sqlite] [K8S_LOG_STORE=postgres] [UNIFAI_IMAGE_TAG=v1.3.38])
	@# Show prompts if not called from deploy-local-k8s (K8S_SKIP_PROMPTS not set)
	@if [ -z "$(K8S_SKIP_PROMPTS)" ]; then \
		echo ""; \
		echo "$(BLUE)╔════════════════════════════════════════════════════════════════╗$(NC)"; \
		echo "$(BLUE)║  UnifAI Helm Deployment                                       ║$(NC)"; \
		echo "$(BLUE)╚════════════════════════════════════════════════════════════════╝$(NC)"; \
		echo ""; \
		echo "$(CYAN)Deployment Configuration$(NC)"; \
		echo ""; \
		echo "$(YELLOW)Image Tag$(NC) [$(UNIFAI_IMAGE_TAG)]:"; \
		printf "  Enter tag: "; \
		read IMAGE_TAG_INPUT; \
		if [ -n "$$IMAGE_TAG_INPUT" ]; then \
			echo "$$IMAGE_TAG_INPUT" > /tmp/unifai-image-tag.tmp; \
		else \
			echo "$(UNIFAI_IMAGE_TAG)" > /tmp/unifai-image-tag.tmp; \
		fi; \
		echo ""; \
		echo "$(YELLOW)Helm Chart Version$(NC) (leave empty for local chart):"; \
		printf "  Enter version: "; \
		read HELM_VERSION_INPUT; \
		echo "$$HELM_VERSION_INPUT" > /tmp/unifai-helm-version.tmp; \
		echo ""; \
		echo "$(YELLOW)Config Store:$(NC)"; \
		echo "  1) no (disabled)"; \
		echo "  2) sqlite"; \
	...
	if [ -z "$$CONFIG_STORE" ] && [ -f /tmp/unifai-config-store.tmp ]; then \
		CONFIG_STORE=$$(cat /tmp/unifai-config-store.tmp); \
	fi; \
	if [ -z "$$CONFIG_STORE" ]; then \
		CONFIG_STORE="sqlite"; \
	fi; \
	LOG_STORE="$(K8S_LOG_STORE)"; \
	if [ -z "$$LOG_STORE" ] && [ -f /tmp/unifai-log-store.tmp ]; then \
		LOG_STORE=$$(cat /tmp/unifai-log-store.tmp); \
	fi; \
	if [ -z "$$LOG_STORE" ]; then \
		LOG_STORE="sqlite"; \
	fi; \
	IMAGE_TAG="$(UNIFAI_IMAGE_TAG)"; \
	if [ -f /tmp/unifai-image-tag.tmp ]; then \
		IMAGE_TAG=$$(cat /tmp/unifai-image-tag.tmp); \
	fi; \
	HELM_VERSION=""; \
	if [ -f /tmp/unifai-helm-version.tmp ]; then \
		HELM_VERSION=$$(cat /tmp/unifai-helm-version.tmp); \
	fi; \
	echo "  $(CYAN)Configuration:$(NC)"; \
	echo "    Image Tag: $$IMAGE_TAG"; \
	if [ -n "$$HELM_VERSION" ]; then \
		echo "    Chart: unifai/unifai v$$HELM_VERSION (remote)"; \
	else \
		echo "    Chart: ./helm-charts/unifai (local)"; \
	fi; \
	echo "    Config Store: $$CONFIG_STORE"; \
	echo "    Log Store: $$LOG_STORE"; \
	echo ""; \
	echo "  $(YELLOW)Deploying UnifAI $$IMAGE_TAG (this may take 1-2 minutes)...$(NC)"; \
	...
	if [ "$$NEEDS_POSTGRES" = "true" ]; then \
		...
		if [ -n "$$EXISTING_PG_PWD" ]; then \
			...
			EXISTING_PG_PWD=$$(kubectl get secret unifai-postgresql-password -n $(K8S_NAMESPACE) -o jsonpath='{.data.password}' 2>/dev/null | base64 -d 2>/dev/null || true); \
			if [ -n "$$EXISTING_PG_PWD" ]; then \
				POSTGRES_PASSWORD="$$EXISTING_PG_PWD"; \
				echo "  $(GREEN)✓ Using existing PostgreSQL password$(NC)"; \
			else \
				POSTGRES_PASSWORD=$$(openssl rand -base64 24 | tr -d '/+=' | head -c 24); \
				kubectl create secret generic unifai-postgresql-password \
	...
	CHART_REF="./helm-charts/unifai"; \
	if [ -n "$$HELM_VERSION" ]; then \
		echo "  $(CYAN)Setting up Helm repository...$(NC)"; \
		helm repo add unifai https://maximhq.github.io/unifai/helm-charts >/dev/null 2>&1 || true; \
		helm repo update >/dev/null 2>&1; \
		CHART_REF="unifai/unifai --version $$HELM_VERSION"; \
	...
	( \
		helm upgrade --install $(K8S_RELEASE_NAME) $$CHART_REF \
			...
			$$HELM_EXTRA_ARGS \
			--wait --timeout 300s > /tmp/unifai-helm-output.log 2>&1 & \
		CMD_PID=$$!; \
		ELAPSED=0; \
		while kill -0 $$CMD_PID 2>/dev/null; do \
			UNIFAI_STATUS=$$(kubectl get pods -n $(K8S_NAMESPACE) -l app.kubernetes.io/name=unifai -o jsonpath='{.items[0].status.phase}' 2>/dev/null || echo "Pending"); \
			if [ "$$NEEDS_POSTGRES" = "true" ]; then \
				...
				STATUS_MSG="postgres: $$PG_STATUS | unifai: $$UNIFAI_STATUS"; \
			else \
				STATUS_MSG="unifai: $$UNIFAI_STATUS"; \
			fi; \
			SPINNER_CHARS="⠋ ⠙ ⠹ ⠸ ⠼ ⠴ ⠦ ⠧ ⠇ ⠏"; \
			CHAR=$$(echo $$SPINNER_CHARS | cut -d' ' -f$$((ELAPSED % 10 + 1))); \
		...
		printf "\r%-80s\r" " "; \
		if [ $$EXIT_CODE -eq 0 ]; then \
			echo "  $(GREEN)✓ UnifAI deployed [$$ELAPSED seconds]$(NC)"; \
		else \
			echo "  $(RED)✗ Deployment failed$(NC)"; \
			echo ""; \
			echo "  $(YELLOW)Helm output:$(NC)"; \
			cat /tmp/unifai-helm-output.log | sed 's/^/    /'; \
			echo ""; \
			echo "  $(YELLOW)Debug commands:$(NC)"; \
			echo "    kubectl get pods -n $(K8S_NAMESPACE)"; \
			echo "    kubectl logs -l app.kubernetes.io/name=unifai -n $(K8S_NAMESPACE)"; \
			rm -f /tmp/unifai-helm-output.log; \
			exit 1; \
		fi; \
		rm -f /tmp/unifai-helm-output.log \
	); \
	rm -f /tmp/unifai-config-store.tmp /tmp/unifai-log-store.tmp /tmp/unifai-image-tag.tmp /tmp/unifai-helm-version.tmp
...
local-k8s-update-key: ## Update OpenAI API key (Usage: make local-k8s-update-key OPENAI_API_KEY='sk-...')
	...
	@kubectl create secret generic unifai-provider-keys \
		...
		-n $(K8S_NAMESPACE) --dry-run=client -o yaml | kubectl apply -f - >/dev/null 2>&1
	@echo "$(GREEN)✓ Updated unifai-provider-keys$(NC)"
	@echo "$(YELLOW)Restarting UnifAI to pick up new key...$(NC)"
	@kubectl rollout restart deployment/$(K8S_RELEASE_NAME) -n $(K8S_NAMESPACE) >/dev/null 2>&1
	@echo "$(GREEN)✓ UnifAI restarted$(NC)"

local-k8s-status: ## Check local K8s deployment status
	@echo "$(BLUE)UnifAI Local K8s Status$(NC)"
	@echo ""
	@echo "$(CYAN)Pods:$(NC)"
...
local-k8s-logs: ## View UnifAI logs
	@kubectl logs -l app.kubernetes.io/name=unifai -n $(K8S_NAMESPACE) -f

local-k8s-port-forward: ## Port-forward UnifAI service to localhost:8080
...
cleanup-local-k8s: ## Remove UnifAI from local K8s
	@echo "$(YELLOW)Cleaning up UnifAI deployment...$(NC)"
