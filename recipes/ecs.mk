...
ECS_CLUSTER_NAME ?= unifai-cluster
ECS_SERVICE_NAME ?= unifai-service
ECS_TASK_FAMILY ?= unifai-task
IMAGE_TAG ?= latest
LAUNCH_TYPE ?= FARGATE
...
deploy-ecs: ## Deploy UnifAI to ECS (Usage: make deploy-ecs SUBNET_IDS='...' SECURITY_GROUP_IDS='...' [CONFIG_JSON_FILE='...'])
...
create-ecs-secret: ## Create configuration secret in AWS (Secrets Manager or SSM)
	@if [ -z "$(CONFIG_JSON_FILE)" ]; then \
		...
		echo "$(YELLOW)Provide CONFIG_JSON_FILE as path to your UnifAI config.json file$(NC)"; \
		exit 1; \
	fi
...
register-ecs-task-definition: ## Register ECS task definition
	...
	{ \
		...
		echo '    "name": "unifai",'; \
		echo '    "image": "unifai/unifai:$(IMAGE_TAG)",'; \
		echo '    "essential": true,'; \
		if [ -n "$(SECRET_VALUE_ARN)" ]; then \
			echo '    "entryPoint": ["/bin/sh", "-c"],'; \
			echo '    "command": ["if [ -n \"$$UNIFAI_CONFIG\" ]; then echo \"$$UNIFAI_CONFIG\" > /app/data/config.json; else echo \"ERROR: UNIFAI_CONFIG not set\" >&2 && exit 1; fi && exec /app/docker-entrypoint.sh /app/main"],'; \
		fi; \
		echo '    "portMappings": [{'; \
		...
		echo '        "awslogs-stream-prefix": "unifai"'; \
		echo '      }'; \
		echo '    }'; \
		echo '  }]'; \
		echo '}'; \
	...
	if aws ecs describe-services --cluster $(ECS_CLUSTER_NAME) --services $(ECS_SERVICE_NAME) --region $(AWS_REGION) 2>/dev/null | grep -q "ACTIVE"; then \
		...
		if [ -z "$(TARGET_GROUP_ARN)" ]; then \
			...
			aws ecs create-service \
				...
				--launch-type $(LAUNCH_TYPE) \
				--network-configuration "awsvpcConfiguration={subnets=[$$SUBNETS],securityGroups=[$(SECURITY_GROUP_IDS)],assignPublicIp=ENABLED}" \
				--load-balancers "targetGroupArn=$(TARGET_GROUP_ARN),containerName=unifai,containerPort=$(CONTAINER_PORT)" \
