...
func newTestAsyncExecutor(t *testing.T) *AsyncJobExecutor {
	...
	govStore := &testGovernanceStore{
		virtualKeys: map[string]*configstoreTables.TableVirtualKey{
			"sk-uf-test": {ID: "vk-123", Value: *schemas.NewSecretVar("sk-uf-test")},
		},
	}
...
func TestSubmitJob_PropagatesContextValues(t *testing.T) {
	...
	capturedCtx := schemas.NewUnifAIContext(context.Background(), <-time.After(1*time.Minute))
	capturedCtx.SetValue(schemas.UnifAIContextKeyVirtualKey, "sk-uf-test")
	capturedCtx.SetValue(schemas.UnifAIContextKey("x-uf-eh-custom"), "custom-value")
	capturedCtx.SetValue(schemas.UnifAIContextKey("x-uf-prom-env"), "production")
	...
	assert.Equal(t, "sk-uf-test", capturedCtx.Value(schemas.UnifAIContextKeyVirtualKey))
	assert.Equal(t, "production", capturedCtx.Value(schemas.UnifAIContextKey("x-uf-prom-env")))
	assert.Equal(t, "custom-value", capturedCtx.Value(schemas.UnifAIContextKey("x-uf-eh-custom")))
	assert.Equal(t, true, capturedCtx.Value(schemas.UnifAIIsAsyncRequest))
}

func TestSubmitJob_NilContextValues(t *testing.T) {
	executor := newTestAsyncExecutor(t)

	var capturedCtx *schemas.UnifAIContext
	var done atomic.Bool
...
func TestSubmitJob_AsyncFlagOverridesContextValues(t *testing.T) {
	...
	inputCtx := schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline)
	inputCtx.SetValue(schemas.UnifAIIsAsyncRequest, false)
	...
	// UnifAIIsAsyncRequest must be true — set AFTER restoring context values
	assert.Equal(t, true, capturedCtx.Value(schemas.UnifAIIsAsyncRequest))
}
...
func TestSubmitJob_OperationFailure_PreservesContext(t *testing.T) {
	...
	inputCtx := schemas.NewUnifAIContext(context.Background(), schemas.NoDeadline)
	inputCtx.SetValue(schemas.UnifAIContextKeyVirtualKey, "sk-uf-test")
