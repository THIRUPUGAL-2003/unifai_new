...
func TestObjectKey(t *testing.T) {
	...
	key := ObjectKey("unifai", ts, "req_abc123")
	assert.Equal(t, "unifai/logs/2026/04/03/14/req_abc123.json.gz", key)
}

func TestMCPToolObjectKey(t *testing.T) {
	ts := time.Date(2026, 4, 3, 14, 0, 0, 0, time.UTC)
	key := MCPToolObjectKey("unifai", ts, "mcp_abc123")
	assert.Equal(t, "unifai/mcp-logs/2026/04/03/14/mcp_abc123.json.gz", key)
}
...
func TestMCPToolLogPayload_RoundTripFullLog(t *testing.T) {
	...
	entry := &MCPToolLog{
		...
		ErrorDetailsParsed: &schemas.UnifAIError{
			IsUnifAIError: true,
