...
// which now routes through the unifai_safe_jsonb PL/pgSQL helper. Skipped
...
// TestUnifAISafeJsonb_DirectInvocation exercises the PL/pgSQL helper in
...
func TestUnifAISafeJsonb_DirectInvocation(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			var got string
			err := db.Raw("SELECT unifai_safe_jsonb(?)", c.in).Scan(&got).Error
			require.NoError(t, err, "unifai_safe_jsonb must not propagate parse errors")
			assert.Equal(t, c.want, got)
		})
	}
}

// TestUnifAISafeJsonb_NullInput verifies the function handles SQL NULL
// (distinct from empty string) by returning NULL without erroring.
func TestUnifAISafeJsonb_NullInput(t *testing.T) {
	_, db := setupPerfTestDB(t)

	var got *string
	err := db.Raw("SELECT unifai_safe_jsonb(NULL::text)").Scan(&got).Error
	require.NoError(t, err, "unifai_safe_jsonb(NULL) must not error")
	assert.Nil(t, got, "unifai_safe_jsonb(NULL) should return NULL")
}
