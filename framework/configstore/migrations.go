...
// directly, so downstream consumers (unifai-enterprise, plugins) can run
...
// a UnifAI-issued bearer token, so hashing buys nothing and the unique index
...
// MCP-gateway-OAuth-server flow (UnifAI acting as an OAuth Authorization
// Server to upstream MCP clients), plus the gateway_session_id column on
// oauth_user_sessions that linked them. UnifAI is now strictly an OAuth
...
//   - oauth_per_user_sessions      (UnifAI-issued bearer tokens for MCP clients)
...
func migrationDropLegacyOAuthServerTables(ctx context.Context, db *gorm.DB, logger schemas.Logger) error {
	...
	return RunSingleMigration(ctx, nil, db, logger, &migrator.Migration{
		...
		Rollback: func(tx *gorm.DB) error {
			...
			// UnifAI-as-OAuth-server flow; their original schemas (dynamic
...
// User-mode and session-mode semantics changed when UnifAI stopped acting as
...
// from config_client. This URL was used to advertise UnifAI as an OAuth
// authorization server (.well-known endpoints, WWW-Authenticate header on
// /mcp). UnifAI no longer acts as an OAuth server (see
...
// as the redirect_uri base when UnifAI acts as an OAuth *client* to upstream
// MCP servers.
...
// migrationRenameOAuth2BfColumnsToUf renames GORM's default bf_mode/bf_sub columns
// (from Go fields BfMode/BfSub) to the uf_mode/uf_sub names used by raw SQL and
// the OAuth Grants API. Without this, ListOAuth2Sessions fails with
// "column rt.uf_sub does not exist".
func migrationRenameOAuth2BfColumnsToUf(ctx context.Context, db *gorm.DB, logger schemas.Logger) error {
	migrationName := "rename_oauth2_bf_columns_to_uf"
	logger.Info("[configstore] starting migration %s", migrationName)
	defer logger.Info("[configstore] finished migration %s", migrationName)
	m := migrator.New(db, migrator.DefaultOptions, []*migrator.Migration{{
		ID: migrationName,
		Migrate: func(tx *gorm.DB) error {
			...
			renames := []struct {
				...
				{"oauth2_refresh_tokens", "bf_mode", "uf_mode"},
				{"oauth2_refresh_tokens", "bf_sub", "uf_sub"},
				{"oauth2_authorize_requests", "bf_mode", "uf_mode"},
				{"oauth2_authorize_requests", "bf_sub", "uf_sub"},
			}
			mg := tx.Migrator()
			for _, r := range renames {
				if !mg.HasTable(r.table) {
					continue
				}
				hasOld := mg.HasColumn(r.table, r.oldName)
				hasNew := mg.HasColumn(r.table, r.newName)
				if hasOld && !hasNew {
					logger.Info("[configstore] %s: renaming %s.%s -> %s", migrationName, r.table, r.oldName, r.newName)
					if err := tx.Exec(fmt.Sprintf("ALTER TABLE %s RENAME COLUMN %s TO %s", r.table, r.oldName, r.newName)).Error; err != nil {
						return fmt.Errorf("rename %s.%s to %s: %w", r.table, r.oldName, r.newName, err)
					}
				} else if !hasOld && !hasNew {
					// Fresh DBs created after column tags were fixed already use uf_*;
					// older partial states may need the column added via AutoMigrate-like create.
					logger.Info("[configstore] %s: %s missing both %s and %s — skipping (CreateTable path owns schema)", migrationName, r.table, r.oldName, r.newName)
				}
			}
			return nil
		},
		Rollback: func(tx *gorm.DB) error {
			...
			renames := []struct {
				...
				{"oauth2_refresh_tokens", "uf_mode", "bf_mode"},
				{"oauth2_refresh_tokens", "uf_sub", "bf_sub"},
				{"oauth2_authorize_requests", "uf_mode", "bf_mode"},
				{"oauth2_authorize_requests", "uf_sub", "bf_sub"},
			}
			mg := tx.Migrator()
			for _, r := range renames {
				if !mg.HasTable(r.table) {
				...
				if mg.HasColumn(r.table, r.oldName) && !mg.HasColumn(r.table, r.newName) {
					if err := tx.Exec(fmt.Sprintf("ALTER TABLE %s RENAME COLUMN %s TO %s", r.table, r.oldName, r.newName)).Error; err != nil {
						return fmt.Errorf("rollback rename %s.%s to %s: %w", r.table, r.oldName, r.newName, err)
					}
				}
			}
			return nil
...
func migrationAddUsersTable(ctx context.Context, db *gorm.DB, logger schemas.Logger) error {
	...
	m := migrator.New(db, migrator.DefaultOptions, []*migrator.Migration{{
		...
		Migrate: func(tx *gorm.DB) error {
			tx = tx.WithContext(ctx)
			mg := tx.Migrator()
			if !mg.HasTable(&tables.TableUser{}) {
				if err := mg.CreateTable(&tables.TableUser{}); err != nil {
					return fmt.Errorf("create governance_users table: %w", err)
				}
			}
			if err := addColumnIfNotExists(tx, logger, &tables.SessionsTable{}, "username"); err != nil {
				return fmt.Errorf("add username to sessions: %w", err)
			}
			if err := addColumnIfNotExists(tx, logger, &tables.SessionsTable{}, "role"); err != nil {
				return fmt.Errorf("add role to sessions: %w", err)
			}
			if err := addColumnIfNotExists(tx, logger, &tables.TableUser{}, "allowed_prompt_repos"); err != nil {
				return fmt.Errorf("add allowed_prompt_repos to users: %w", err)
			}
			return nil
		},
		Rollback: func(tx *gorm.DB) error {
			...
			mg := tx.Migrator()
			if mg.HasTable(&tables.TableUser{}) {
				if err := mg.DropTable(&tables.TableUser{}); err != nil {
					return fmt.Errorf("drop governance_users table: %w", err)
				}
			}
			return nil
		},
...
func migrationAddAllowedPromptReposToUsers(ctx context.Context, db *gorm.DB, logger schemas.Logger) error {
	migrationName := "add_allowed_prompt_repos_to_users"
	logger.Info("[configstore] starting migration %s", migrationName)
	defer logger.Info("[configstore] finished migration %s", migrationName)
	m := migrator.New(db, migrator.DefaultOptions, []*migrator.Migration{{
		ID: migrationName,
...
// migrationAddOAuthResourceIndicator persists the RFC 8707 resource value
// discovered from RFC 9728 protected-resource metadata.
func migrationAddOAuthResourceIndicator(ctx context.Context, db *gorm.DB, logger schemas.Logger) error {
	migrationName := "add_oauth_resource_indicator"
	logger.Info("[configstore] starting migration %s", migrationName)
	defer logger.Info("[configstore] finished migration %s", migrationName)
	m := migrator.New(db, migrator.DefaultOptions, []*migrator.Migration{{
		ID: migrationName,
...
// migrationAddWorkspaceFeatureTables creates typed tables for access profiles,
// RBAC, circuit breaker, audit logs, and related workspace features, then
// copies any rows previously stored in the generic enterprise_records bag.
func migrationAddWorkspaceFeatureTables(ctx context.Context, db *gorm.DB, logger schemas.Logger) error {
	migrationName := "add_workspace_feature_tables"
	logger.Info("[configstore] starting migration %s", migrationName)
	defer logger.Info("[configstore] finished migration %s", migrationName)
	m := migrator.New(db, migrator.DefaultOptions, []*migrator.Migration{{
		ID: migrationName,
		Migrate: func(tx *gorm.DB) error {
			tx = tx.WithContext(ctx)
			migrator := tx.Migrator()
			for _, model := range tables.WorkspaceModels() {
				if migrator.HasTable(model) {
					continue
				}
				logger.Info("[configstore] %s: creating table %T", migrationName, model)
				if err := migrator.CreateTable(model); err != nil {
					return err
				}
			}
			return backfillWorkspaceFeatureTables(tx, logger)
		},
	}})
	if err := m.Migrate(); err != nil {
		return fmt.Errorf("error running %s migration: %w", migrationName, err)
...
func backfillWorkspaceFeatureTables(tx *gorm.DB, logger schemas.Logger) error {
	if !tx.Migrator().HasTable(&tables.TableEnterpriseRecord{}) {
		return nil
	}
	var rows []tables.TableEnterpriseRecord
	if err := tx.Find(&rows).Error; err != nil {
		return err
	}
	for _, row := range rows {
		if err := copyEnterpriseRecord(tx, row); err != nil {
			logger.Warn("[configstore] skip enterprise_records id=%d kind=%s: %v", row.ID, row.Kind, err)
		}
	}
	if !tx.Migrator().HasTable(&tables.TableEnterpriseAuditLog{}) {
		return nil
	}
	var logs []tables.TableEnterpriseAuditLog
	if err := tx.Find(&logs).Error; err != nil {
		return err
	}
	for _, logRow := range logs {
		copied := tables.TableAuditLog{
			Action:     logRow.Action,
			Outcome:    logRow.Outcome,
			Initiator:  logRow.Initiator,
			Target:     logRow.Target,
			Method:     logRow.Method,
			Path:       logRow.Path,
			IP:         logRow.IP,
			DurationMs: logRow.DurationMs,
			Detail:     logRow.Detail,
			CreatedAt:  logRow.CreatedAt,
		}
		if err := tx.Create(&copied).Error; err != nil {
			logger.Warn("[configstore] skip enterprise_audit_logs id=%d: %v", logRow.ID, err)
		}
	}
	return nil
}

func copyEnterpriseRecord(tx *gorm.DB, row tables.TableEnterpriseRecord) error {
	switch row.Kind {
	case "access_profile":
		var payload struct {
			...
			MCPToolGroups    []map[string]any `json:"mcp_tool_groups"`
			MCPServers       []map[string]any `json:"mcp_servers"`
			MCPToolOverrides []map[string]any `json:"mcp_tool_overrides"`
		...
		spec := map[string]any{}
		if payload.ProviderConfigs != nil {
			spec["provider_configs"] = payload.ProviderConfigs
		}
		if payload.Budgets != nil {
			spec["budgets"] = payload.Budgets
		}
		if payload.RateLimit != nil {
			spec["rate_limit"] = payload.RateLimit
		}
		if payload.MCPToolGroups != nil {
			spec["mcp_tool_groups"] = payload.MCPToolGroups
		}
		if payload.MCPServers != nil {
			spec["mcp_servers"] = payload.MCPServers
		}
		if payload.MCPToolOverrides != nil {
			spec["mcp_tool_overrides"] = payload.MCPToolOverrides
		}
		return tx.Create(&tables.TableAccessProfile{
			Name: payload.Name, Description: payload.Description, IsActive: payload.IsActive,
			Version: payload.Version, CalendarAligned: payload.CalendarAligned,
			ParsedTags: payload.Tags, ParsedSpec: spec,
	...
	case "rbac_role":
		...
		return tx.Create(&tables.TableRBACRole{
			Name: payload.Name, Description: payload.Description, IsSystemRole: payload.IsSystemRole,
			DAC: payload.DAC, ParsedPermissionIDs: payload.PermissionIDs,
			CreatedAt: payload.CreatedAt, UpdatedAt: payload.UpdatedAt,
		}).Error
	case "business_unit":
		var payload struct {
		...
		return tx.Create(&tables.TableBusinessUnit{
			ID: payload.ID, Name: payload.Name, ParsedTeamIDs: payload.TeamIDs,
			ParsedBudget: payload.Budget, ParsedRateLimit: payload.RateLimit,
			CreatedAt: payload.CreatedAt, UpdatedAt: payload.UpdatedAt,
		}).Error
	case "alert_channel":
		var payload struct {
		...
		return tx.Create(&tables.TableAlertChannel{
			Name: payload.Name, Type: payload.Type, Enabled: payload.Enabled,
			ParsedConfig: payload.Config, CreatedAt: payload.CreatedAt, UpdatedAt: payload.UpdatedAt,
		}).Error
	case "circuit_breaker_policy":
		...
		primaryKeys, _ := payload["primary_key_ids"].([]any)
		keyIDs := make([]string, 0, len(primaryKeys))
		...
		return tx.Create(&tables.TableCircuitBreakerPolicy{
			Name: name, Enabled: enabled,
			PrimaryProvider:  stringFromAny(payload["primary_provider"]),
			PrimaryModel:     stringFromAny(payload["primary_model"]),
			FallbackProvider: stringFromAny(payload["fallback_provider"]),
			FallbackModel:    stringFromAny(payload["fallback_model"]),
			DefaultCooldown:  firstNonEmptyString(stringFromAny(payload["default_cooldown"]), "30s"),
			CooldownHeader:   stringFromAny(payload["cooldown_header"]),
			ParsedCondition:  condition, ParsedPrimaryKeyIDs: keyIDs,
	...
	case "mcp_tool_group":
		...
		return tx.Create(&tables.TableMCPToolGroup{
			Name: name, Description: description, Enabled: enabled, ParsedSpec: payload,
			CreatedAt: row.CreatedAt, UpdatedAt: row.UpdatedAt,
		}).Error
	case "prompt_deployment":
		var payload struct {
		...
		return tx.Create(&tables.TablePromptDeployment{
			...
			Enabled: payload.Enabled, CreatedAt: payload.CreatedAt, UpdatedAt: payload.UpdatedAt,
		}).Error
	case "cluster_config":
		return tx.Clauses(clause.OnConflict{DoNothing: true}).Create(&tables.TableWorkspaceSetting{
			Key: WorkspaceSettingCluster, Data: row.Data, UpdatedAt: row.UpdatedAt,
		}).Error
	case "load_balancer_config":
		return tx.Clauses(clause.OnConflict{DoNothing: true}).Create(&tables.TableWorkspaceSetting{
			Key: WorkspaceSettingLoadBalancer, Data: row.Data, UpdatedAt: row.UpdatedAt,
		}).Error
	case "scim_config":
		return tx.Clauses(clause.OnConflict{DoNothing: true}).Create(&tables.TableWorkspaceSetting{
			Key: WorkspaceSettingSCIM, Data: row.Data, UpdatedAt: row.UpdatedAt,
		}).Error
	case "audit_settings":
		return tx.Clauses(clause.OnConflict{DoNothing: true}).Create(&tables.TableWorkspaceSetting{
			Key: WorkspaceSettingAudit, Data: row.Data, UpdatedAt: row.UpdatedAt,
		}).Error
	case "connector":
		return tx.Clauses(clause.OnConflict{DoNothing: true}).Create(&tables.TableWorkspaceSetting{
			Key: WorkspaceSettingConnector(row.Key), Data: row.Data, UpdatedAt: row.UpdatedAt,
		}).Error
	default:
		return nil
	}
...
func migrationAddSMTPAuthSecurityTables(ctx context.Context, db *gorm.DB, logger schemas.Logger) error {
	migrationName := "add_smtp_auth_security_tables"
	logger.Info("[configstore] starting migration %s", migrationName)
	defer logger.Info("[configstore] finished migration %s", migrationName)
	m := migrator.New(db, migrator.DefaultOptions, []*migrator.Migration{{
		ID: migrationName,
		Migrate: func(tx *gorm.DB) error {
			tx = tx.WithContext(ctx)
			mg := tx.Migrator()
			if !mg.HasTable(&tables.TableSMTPConfig{}) {
				if err := mg.CreateTable(&tables.TableSMTPConfig{}); err != nil {
					return fmt.Errorf("create config_smtp: %w", err)
				}
			}
			if !mg.HasTable(&tables.TableLoginLockout{}) {
				if err := mg.CreateTable(&tables.TableLoginLockout{}); err != nil {
					return fmt.Errorf("create auth_login_lockouts: %w", err)
				}
			}
...
func migrationAddUserIDToPromptSessions(ctx context.Context, db *gorm.DB, logger schemas.Logger) error {
	migrationName := "add_user_id_to_prompt_sessions"
	logger.Info("[configstore] starting migration %s", migrationName)
	defer logger.Info("[configstore] finished migration %s", migrationName)
	m := migrator.New(db, migrator.DefaultOptions, []*migrator.Migration{{
		ID: migrationName,
		Migrate: func(tx *gorm.DB) error {
			tx = tx.WithContext(ctx)
			if err := addColumnIfNotExists(tx, logger, &tables.TablePromptSession{}, "UserID"); err != nil {
				return fmt.Errorf("add user_id to prompt_sessions: %w", err)
			}
			return nil
		},
		Rollback: func(tx *gorm.DB) error {
			tx = tx.WithContext(ctx)
			return dropColumnIfExists(tx, logger, &tables.TablePromptSession{}, "user_id")
		},
	}})
	if err := m.Migrate(); err != nil {
		return fmt.Errorf("error running %s migration: %w", migrationName, err)
