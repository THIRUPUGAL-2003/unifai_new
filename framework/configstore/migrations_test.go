...
// functions directly.
var testMigrationLogger = unifai.NewDefaultLogger(schemas.LogLevelInfo)

// postgresDSN matches the postgres service in tests/docker-compose.yml and
// framework/docker-compose.yml.
const postgresDSN = "host=localhost user=unifai password=unifai_password dbname=unifai port=5432 sslmode=disable"
...
func setupFullMigrationDB(t *testing.T) (*RDBConfigStore, *gorm.DB) {
	...
	store := &RDBConfigStore{logger: unifai.NewDefaultLogger(schemas.LogLevelInfo)}
	store.db.Store(db)
...
func TestFullMigration_EndToEnd(t *testing.T) {
	...
	for _, vk := range []struct {
		...
		err := store.CreateVirtualKey(ctx, &tables.TableVirtualKey{
			...
			IsActive: unifai.Ptr(true), CreatedAt: now, UpdatedAt: now,
		})
