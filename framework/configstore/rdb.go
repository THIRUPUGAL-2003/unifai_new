...
// UpdateUser updates a specific user in the database.
...
func (s *RDBConfigStore) DeleteVirtualKey(ctx context.Context, id string, tx ...*gorm.DB) error {
	...
	if err := txDB.WithContext(ctx).Transaction(func(txDB *gorm.DB) error {
		...
		// keyed by vk_id in uf_sub). They are revoked rather than deleted so they
		...
		if err := txDB.WithContext(ctx).Model(&tables.TableOAuth2RefreshToken{}).
			Where("uf_mode = ? AND uf_sub = ? AND revoked_at IS NULL", string(schemas.MCPAuthModeVK), id).
...
// uf_mode — a bulk-revoke utility for invalidating every grant of one identity
// type (vk, user, or session).
func (s *RDBConfigStore) RevokeOAuth2RefreshTokensByMode(ctx context.Context, bfMode string) error {
	now := time.Now()
	return s.DB().WithContext(ctx).
		Model(&tables.TableOAuth2RefreshToken{}).
		Where("uf_mode = ? AND revoked_at IS NULL", bfMode).
		Update("revoked_at", &now).Error
}
...
type OAuth2SessionRow struct {
	...
	BfMode       string     `json:"uf_mode"`
	BfSub        string     `json:"uf_sub"`
	BfSubDisplay string     `json:"uf_sub_display,omitempty"` // human-readable: VK name for vk mode
...
func (s *RDBConfigStore) ListOAuth2Sessions(ctx context.Context, params OAuth2SessionsQueryParams) ([]OAuth2SessionRow, int64, error) {
	...
	base := s.ScopedDB(ctx).
		...
		Joins("LEFT JOIN governance_virtual_keys vk ON vk.id = rt.uf_sub AND rt.uf_mode = 'vk'").
		Where("rt.revoked_at IS NULL")

	if search := strings.TrimSpace(params.Search); search != "" {
		like := "%" + strings.ToLower(search) + "%"
		// Match the columns the UI renders: client name/id, the bound identity
		// (uf_sub), and the joined VK name shown as the display name for vk mode.
		base = base.Where(
			"LOWER(COALESCE(c.client_name, '')) LIKE ? OR LOWER(rt.client_id) LIKE ? OR LOWER(rt.uf_sub) LIKE ? OR LOWER(COALESCE(vk.name, '')) LIKE ?",
			like, like, like, like,
		)
	}
	if len(params.Modes) > 0 {
		base = base.Where("rt.uf_mode IN ?", params.Modes)
	}
