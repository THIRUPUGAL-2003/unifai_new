...
func TestVaultCallbacks_AutoStoreAndRemove(t *testing.T) {
	...
	headerPath := "unifai/config_mcp_clients/client-1/headers/Authorization"
...
func TestVaultCallbacks_SelfManagedStoresPlaintext(t *testing.T) {
	...
	for i, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			encrypt.Init(tc.encryptionKey, unifai.NewDefaultLogger(schemas.LogLevelInfo))
			t.Cleanup(func() { encrypt.Init("", unifai.NewDefaultLogger(schemas.LogLevelInfo)) })
			...
			secretPath := fmt.Sprintf("unifai/config_keys/%s/bedrock_secret_key", keyID)
...
func TestVaultCallbacks_SelfManagedRemovesVaultSecrets(t *testing.T) {
	...
	valuePath := fmt.Sprintf("unifai/config_keys/%s/value", keyID)
