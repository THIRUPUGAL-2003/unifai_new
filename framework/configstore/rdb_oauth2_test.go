...
func seedAuthorizeRequest(t *testing.T, s *RDBConfigStore, id string, status tables.OAuth2AuthorizeRequestStatus, codeHash *string, expires time.Time) {
	...
	req := &tables.TableOAuth2AuthorizeRequest{
		...
		State:               "state",
		Scope:               "mcp",
		Resource:            "https://unifai.test/mcp",
...
func TestListOAuth2Sessions_JoinsAndExcludesRevoked(t *testing.T) {
	...
	require.NoError(t, s.DB().Create(&tables.TableVirtualKey{ID: "vk-1", Name: "Alpha VK", Value: *schemas.NewSecretVar("sk-uf-alpha")}).Error)
...
// (independent of the page slice), the uf_mode filter, and case-insensitive
// search across the joined client name, the joined VK display name, and the
// bound identity (uf_sub).
func TestListOAuth2Sessions_FilterAndPaginate(t *testing.T) {
	...
	base := time.Now()
	rtA := makeRefreshToken("rt-a", "fa", "client-1", "h-a") // vk mode, uf_sub vk-1 → display "Alpha VK"
	...
	// Search matches the bound identity (uf_sub).
