...
// matched against the client name/id, the bound identity (uf_sub), and the
// joined virtual key name. Modes filters on uf_mode (user/vk/session); an
...
type ConfigStore interface {
	...
	GetMCPClientByConnectionURL(ctx context.Context, rawURL string) (*tables.TableMCPClient, error)
	...
	// Users CRUD
	GetUsers(ctx context.Context) ([]*tables.TableUser, error)
	GetUserByID(ctx context.Context, id string) (*tables.TableUser, error)
	GetUserByUsername(ctx context.Context, username string) (*tables.TableUser, error)
...
func NewConfigStore(ctx context.Context, config *Config, logger schemas.Logger) (ConfigStore, error) {
	...
	case ConfigStoreTypeMySQL, "mariadb":
		mysqlConfig, err := mysqlConfigFromAny(config.Config)
		if err != nil {
			return nil, err
		}
