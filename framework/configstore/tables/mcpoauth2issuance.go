...
// Registration (RFC 7591). UnifAI only supports public clients
...
type TableOAuth2AuthorizeRequest struct {
	...
	BfMode   string `gorm:"column:uf_mode;type:varchar(20)" json:"uf_mode,omitempty"` // user|vk|session
	BfSub    string `gorm:"column:uf_sub;type:varchar(255)" json:"uf_sub,omitempty"` // resolved identity
...
//   - uf_sub liveness: VK deleted or user deactivated → invalid_grant on next refresh
...
type TableOAuth2RefreshToken struct {
	...
	ClientID   string     `gorm:"type:varchar(255);not null;index" json:"client_id"`
	BfMode     string     `gorm:"column:uf_mode;type:varchar(20);not null" json:"uf_mode"` // user|vk|session
	BfSub      string     `gorm:"column:uf_sub;type:varchar(255);not null" json:"uf_sub"` // resolved identity
	Scope      string     `gorm:"type:varchar(255)" json:"scope"`
