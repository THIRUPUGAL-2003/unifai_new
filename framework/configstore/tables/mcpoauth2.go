...
type TableOauthConfig struct {
	...
	Resource            string             `gorm:"type:text" json:"resource,omitempty"`             // RFC 8707 resource indicator (optional, can be discovered via RFC 9728)
...
type TableOauthUserSession struct {
	...
	SessionID        string    `gorm:"type:varchar(255);index" json:"session_id,omitempty"`     // Session-mode identity: client-asserted x-uf-mcp-session-id. Empty for vk/user mode rows. Stored plaintext (not a bearer credential; same trust model as a VK value).
...
type TableOauthUserToken struct {
	...
	SessionID        string     `gorm:"type:varchar(255);index" json:"session_id,omitempty"`      // Session-mode identity: client-asserted x-uf-mcp-session-id. Empty for vk/user mode rows.
