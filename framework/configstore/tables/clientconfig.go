...
type TableClientConfig struct {
	...
	MCPExternalClientURL                  string `gorm:"type:varchar(512)" json:"mcp_external_client_url,omitempty"`      // Public base URL used as redirect_uri when UnifAI acts as an OAuth client to upstream MCP servers
	...
	AllowDirectKeys                       bool `gorm:"default:false" json:"allow_direct_keys"`                          // Allow callers to bypass the registered key pool via x-uf-direct-key header
