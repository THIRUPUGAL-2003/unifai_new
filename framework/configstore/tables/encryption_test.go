...
func init() {
	encrypt.Init(testEncryptionKey, unifai.NewDefaultLogger(schemas.LogLevelInfo))
}
...
func TestTableOauthToken_EncryptDecrypt(t *testing.T) {
	...
	token := &TableOauthToken{
		...
		TokenType:    "Bearer",
		ExpiresAt:    unifai.Ptr(time.Now().Add(time.Hour)),
	}
...
func disableEncryption(t *testing.T) {
	t.Helper()
	encrypt.Init("", unifai.NewDefaultLogger(schemas.LogLevelInfo))
	t.Cleanup(func() {
		encrypt.Init(testEncryptionKey, unifai.NewDefaultLogger(schemas.LogLevelInfo))
	})
}
