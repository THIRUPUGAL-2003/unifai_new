...
func TestDynamicPluginLifecycle(t *testing.T) {
	...
	t.Run("HTTPTransportPreHook", func(t *testing.T) {
		...
		pluginCtx, cancel := schemas.NewUnifAIContextWithTimeout(ctx, 10*time.Second)
		defer cancel()
	...
	t.Run("PostLLMHook", func(t *testing.T) {
		...
		unifaiErr := (*schemas.UnifAIError)(nil)
		pluginCtx, cancel := schemas.NewUnifAIContextWithTimeout(ctx, 10*time.Second)
		defer cancel()
		modifiedResp, modifiedErr, err := plugin.PostLLMHook(pluginCtx, resp, unifaiErr)
		require.NoError(t, err, "PostLLMHook should not return error")
		assert.Equal(t, resp, modifiedResp, "Response should be unchanged")
		assert.Equal(t, unifaiErr, modifiedErr, "Error should be unchanged")
	})
	...
	t.Run("PostHook_WithError", func(t *testing.T) {
		...
		pluginCtx, cancel := schemas.NewUnifAIContextWithTimeout(ctx, 10*time.Second)
		defer cancel()
		modifiedResp, modifiedErr, err := plugin.PostLLMHook(pluginCtx, nil, unifaiErr)
		require.NoError(t, err, "PostLLMHook should not return error")
...
func TestLoadPluginWithOptionalHooks(t *testing.T) {
	...
	t.Run("UnimplementedMCPHooks", func(t *testing.T) {
		...
		mcpReq := &schemas.UnifAIMCPRequest{}
		...
		mcpResp := &schemas.UnifAIMCPResponse{}
		unifaiErr := &schemas.UnifAIError{}
		returnedResp, returnedErr, hookErr := dynamicPlugin.PostMCPHook(pluginCtx, mcpResp, unifaiErr)
		...
		assert.Equal(t, unifaiErr, returnedErr, "PostMCPHook should return original error")
	})
