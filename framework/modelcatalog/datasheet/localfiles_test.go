...
func TestFilePathFromURL(t *testing.T) {
	cases := []struct {
		...
		{"file:///opt/unifai/pricing.json", "/opt/unifai/pricing.json"},
		{"file://localhost/opt/unifai/pricing.json", "/opt/unifai/pricing.json"},
...
// resolve getunifai.ai.
//
// The fixtures in testdata/ are real entries extracted from the public
// datasheets at https://getunifai.ai/datasheet and
// https://getunifai.ai/datasheet/model-parameters.
func TestLoadFromLocalFiles(t *testing.T) {
	logger := unifai.NewDefaultLogger(schemas.LogLevelWarn)
...
// even when the default getunifai.ai URLs would be unreachable, a file:// URL
