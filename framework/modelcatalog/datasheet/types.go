...
// LookupScopesFromContext builds a LookupScopes from a UnifAIContext. Reads
...
func LookupScopesFromContext(ctx *schemas.UnifAIContext, provider string) *LookupScopes {
	if ctx == nil {
		return nil
	}
	virtualKeyID, _ := ctx.Value(schemas.UnifAIContextKeyGovernanceVirtualKeyID).(string)
	selectedKeyID, _ := ctx.Value(schemas.UnifAIContextKeySelectedKeyID).(string)
	return &LookupScopes{
...
// costInput holds the extracted usage data from a UnifAIResponse,
...
// (vertex_ai, google-vertex, etc.) onto unifai's canonical provider names.
