...
func (p *testRealtimeObservabilityPlugin) PreRequestHook(_ *schemas.UnifAIContext, _ *schemas.UnifAIRequest) error {
	return nil
}
func (p *testRealtimeObservabilityPlugin) PreLLMHook(_ *schemas.UnifAIContext, req *schemas.UnifAIRequest) (*schemas.UnifAIRequest, *schemas.LLMPluginShortCircuit, error) {
	return req, nil, nil
}
func (p *testRealtimeObservabilityPlugin) PostLLMHook(_ *schemas.UnifAIContext, resp *schemas.UnifAIResponse, unifaiErr *schemas.UnifAIError) (*schemas.UnifAIResponse, *schemas.UnifAIError, error) {
...
func TestTracer_StartSpan_RootSpanWithW3CParent(t *testing.T) {
	...
	// the root span in UnifAI correctly links to the upstream service's span.
	store := NewTraceStore(5*time.Minute, nil)
	defer store.Stop()
	...
	ctx := context.WithValue(context.Background(), schemas.UnifAIContextKeyTraceID, traceID)
	ctx = context.WithValue(ctx, schemas.UnifAIContextKeyParentSpanID, externalParentSpanID)

	// Create root span - this should link to the external parent
	newCtx, handle := tracer.StartSpan(ctx, "unifai-http-request", schemas.SpanKindHTTPRequest)
	if handle == nil {
	...
	spanID, ok := newCtx.Value(schemas.UnifAIContextKeySpanID).(string)
	if !ok || spanID == "" {
...
func TestIntegration_FullDistributedTraceFlow(t *testing.T) {
	...
	// Step 4: UnifAI creates LLM call span
