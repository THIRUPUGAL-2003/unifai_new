...
// 1. UnifAIContextKeySpanID - existing span in this service (for child spans)
// 2. UnifAIContextKeyParentSpanID - incoming parent from W3C traceparent (for root spans)
...
func (t *Tracer) StartSpan(ctx context.Context, name string, kind schemas.SpanKind) (context.Context, schemas.SpanHandle) {
	...
	parentSpanID, _ := ctx.Value(schemas.UnifAIContextKeySpanID).(string)
	...
	if parentSpanID == "" {
		parentSpanID, _ = ctx.Value(schemas.UnifAIContextKeyParentSpanID).(string)
	}
	...
	newCtx := context.WithValue(ctx, schemas.UnifAIContextKeySpanID, span.SpanID)
...
func (t *Tracer) PopulateLLMRequestAttributes(handle schemas.SpanHandle, req *schemas.UnifAIRequest) {
	h, ok := handle.(*spanHandle)
	if !ok || h == nil || req == nil {
		return
	}
...
func (t *Tracer) PopulateLLMResponseAttributes(ctx *schemas.UnifAIContext, handle schemas.SpanHandle, resp *schemas.UnifAIResponse, err *schemas.UnifAIError) {
	h, ok := handle.(*spanHandle)
	if !ok || h == nil {
		return
	}
...
func (t *Tracer) AddStreamingChunk(traceID string, response *schemas.UnifAIResponse) {
...
func (t *Tracer) GetAccumulatedChunks(traceID string) (*schemas.UnifAIResponse, int64, int) {
...
func (t *Tracer) EndStream(traceID string, err *schemas.UnifAIError) {
	if traceID == "" || t.accumulator == nil {
		return
	}
...
// GetAccumulatedResponse returns a snapshot UnifAIResponse built on demand
// from the accumulator's current chunks. See schemas.Tracer.GetAccumulatedResponse
// for full semantics.
func (t *Tracer) GetAccumulatedResponse(traceID string) *schemas.UnifAIResponse {
	if traceID == "" || t.accumulator == nil {
		return nil
	}
...
func (t *Tracer) GateSend(traceID string, chunk *schemas.UnifAIStreamChunk, isFinal, isHardErr bool, ch chan *schemas.UnifAIStreamChunk, ctx *schemas.UnifAIContext) (ok bool) {
...
func (t *Tracer) ProcessStreamingChunk(ctx *schemas.UnifAIContext, traceID string, isFinalChunk bool, result *schemas.UnifAIResponse, err *schemas.UnifAIError) *schemas.StreamAccumulatorResult {
	if traceID == "" || t.accumulator == nil {
		return nil
	}

	// Create a new context for accumulator that sets the traceID as the accumulator lookup ID.
	accumCtx := schemas.NewUnifAIContext(context.Background(), time.Time{})
	accumCtx.SetValue(schemas.UnifAIContextKeyAccumulatorID, traceID)
	accumCtx.SetValue(schemas.UnifAIContextKeyStreamEndIndicator, isFinalChunk)

	// Forward relevant context values to the new context
	if ctx != nil {
		accumCtx.SetValue(schemas.UnifAIContextKeySelectedKeyID, ctx.Value(schemas.UnifAIContextKeySelectedKeyID))
		accumCtx.SetValue(schemas.UnifAIContextKeyGovernanceVirtualKeyID, ctx.Value(schemas.UnifAIContextKeyGovernanceVirtualKeyID))
	}
	...
	// On final chunk, store the accumulated UnifAIResponse on the deferred span
	// so that completeDeferredSpan can populate span attributes (e.g., gen_ai.output.messages)
	if isFinalChunk {
		if unifaiResp := processedResp.ToUnifAIResponse(); unifaiResp != nil &&
			(unifaiResp.ChatResponse != nil ||
				unifaiResp.TextCompletionResponse != nil ||
				unifaiResp.SpeechResponse != nil ||
				unifaiResp.TranscriptionResponse != nil ||
				unifaiResp.ImageGenerationResponse != nil ||
				unifaiResp.ResponsesResponse != nil) {
			t.store.SetAccumulatedResponse(traceID, unifaiResp)
		}
	}
