...
func (a *Accumulator) buildCompleteMessageFromChatStreamChunks(chunks []*ChatStreamChunk) *schemas.ChatMessage {
	...
	type rdAccum struct {
		...
		typ                          schemas.UnifAIReasoningDetailsType
		id, signature                *string
	}
...
func (a *Accumulator) processAccumulatedChatStreamingChunks(requestID string, respErr *schemas.UnifAIError, isFinalChunk bool) (*AccumulatedData, error) {
...
func (a *Accumulator) processChatStreamingResponse(ctx *schemas.UnifAIContext, result *schemas.UnifAIResponse, unifaiErr *schemas.UnifAIError) (*ProcessedStreamResponse, error) {
	...
	requestType, provider, model, resolvedModel := unifai.GetResponseFields(result, unifaiErr)
	...
	if unifaiErr != nil {
		...
		if result.TextCompletionResponse.ExtraFields.RawResponse != nil {
			chunk.RawResponse = unifai.Ptr(fmt.Sprintf("%v", result.TextCompletionResponse.ExtraFields.RawResponse))
		}
		if isFinalChunk {
		...
		if result.ChatResponse.ExtraFields.RawResponse != nil {
			chunk.RawResponse = unifai.Ptr(fmt.Sprintf("%v", result.ChatResponse.ExtraFields.RawResponse))
		}
		if isFinalChunk {
			if a.pricingManager != nil {
				cost := a.pricingManager.CalculateCost(result, modelcatalog.PricingLookupScopesFromContext(ctx, string(result.GetExtraFields().Provider)))
	...
	if isFinalChunk {
		...
		data, processErr := a.processAccumulatedChatStreamingChunks(requestID, unifaiErr, isFinalChunk)
		if processErr != nil {
