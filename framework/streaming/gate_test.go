...
// makeChunks builds n distinct *UnifAIStreamChunk pointers tracked by reference equality.
func makeChunks(n int) []*schemas.UnifAIStreamChunk {
	out := make([]*schemas.UnifAIStreamChunk, n)
	for i := 0; i < n; i++ {
		out[i] = &schemas.UnifAIStreamChunk{
			UnifAIChatResponse: &schemas.UnifAIChatResponse{ID: fmt.Sprintf("chunk-%d", i)},
		}
	}
	return out
}
...
type recorder struct {
	mu      sync.Mutex
	chunks  []*schemas.UnifAIStreamChunk
	stamps  []time.Time
	ch      chan *schemas.UnifAIStreamChunk
	stopped chan struct{}
}

func newRecorder(bufSize int) *recorder {
	r := &recorder{
		ch:      make(chan *schemas.UnifAIStreamChunk, bufSize),
...
func (r *recorder) snapshot() ([]*schemas.UnifAIStreamChunk, []time.Time) {
	r.mu.Lock()
	defer r.mu.Unlock()
	cs := make([]*schemas.UnifAIStreamChunk, len(r.chunks))
...
func TestGate_HardErrorWhilePaused(t *testing.T) {
	...
	hardErr := &schemas.UnifAIStreamChunk{UnifAIError: &schemas.UnifAIError{IsUnifAIError: true, Error: &schemas.ErrorField{Message: "boom"}}}
...
func TestGate_EndStreamErrorWhilePaused(t *testing.T) {
	...
	terminal := &schemas.UnifAIError{IsUnifAIError: true, Error: &schemas.ErrorField{Message: "policy"}}
	...
	cs, _ := r.snapshot()
	last := cs[3]
	if last == nil || last.UnifAIError != terminal {
...
func TestGate_CtxCancelWhilePaused(t *testing.T) {
	...
	ch := make(chan *schemas.UnifAIStreamChunk, 1)
	chunks := makeChunks(3)
...
func TestGate_FallbackIsolation(t *testing.T) {
	...
	traceA := "trace-A"
	chA := make(chan *schemas.UnifAIStreamChunk, 4)
...
func TestGate_HardErrorWhileActive(t *testing.T) {
	...
	a.GateSend(traceID, chunks[0], false, false, r.ch, ctx)
	hardErr := &schemas.UnifAIStreamChunk{UnifAIError: &schemas.UnifAIError{IsUnifAIError: true, Error: &schemas.ErrorField{Message: "rate limited"}}}
	if !a.GateSend(traceID, hardErr, false, true, r.ch, ctx) {
...
// TestGate_SoftErrorPassesThrough: a chunk with a non-IsUnifAIError UnifAIError
...
func TestGate_SoftErrorPassesThrough(t *testing.T) {
	...
	soft := &schemas.UnifAIStreamChunk{UnifAIError: &schemas.UnifAIError{IsUnifAIError: false, Error: &schemas.ErrorField{Message: "transient"}}}
...
func TestGate_GateSendAfterEnd(t *testing.T) {
	...
	if a.GateSend(traceID, &schemas.UnifAIStreamChunk{}, true, false, r.ch, ctx) {
...
func TestGate_EndStreamErrorWhileActive(t *testing.T) {
	...
	terminal := &schemas.UnifAIError{IsUnifAIError: true, Error: &schemas.ErrorField{Message: "policy-violation"}}
	a.EndStream(traceID, terminal)
	...
	if cs[1] == nil || cs[1].UnifAIError != terminal {
...
func TestGate_EndStreamErrorBeforeAnyChunk(t *testing.T) {
	...
	terminal := &schemas.UnifAIError{IsUnifAIError: true, Error: &schemas.ErrorField{Message: "early"}}
...
func TestGate_MultipleEndStreamCalls(t *testing.T) {
	...
	first := &schemas.UnifAIError{IsUnifAIError: true, Error: &schemas.ErrorField{Message: "first"}}
	second := &schemas.UnifAIError{IsUnifAIError: true, Error: &schemas.ErrorField{Message: "second"}}
...
func TestGate_CtxDeadlineExpiresDuringActive(t *testing.T) {
	...
	ctx := schemas.NewUnifAIContext(context.Background(), time.Now().Add(40*time.Millisecond))
	defer ctx.Cancel()
...
func TestGate_CtxDeadlineExpiresDuringPause(t *testing.T) {
	...
	ctx := schemas.NewUnifAIContext(context.Background(), time.Now().Add(60*time.Millisecond))
	defer ctx.Cancel()
	ch := make(chan *schemas.UnifAIStreamChunk) // unbuffered: drain blocks
	chunks := makeChunks(3)
...
func TestGate_ProducerContinuesAfterDisconnect(t *testing.T) {
	...
	if a.GateSend(traceID, &schemas.UnifAIStreamChunk{}, true, false, ch, ctx) {
		t.Fatalf("final chunk after disconnect should drop")
	}
	hardErr := &schemas.UnifAIStreamChunk{UnifAIError: &schemas.UnifAIError{IsUnifAIError: true, Error: &schemas.ErrorField{Message: "x"}}}
...
// UnifAIContextKeyStreamGated flag — that would force every subsequent
...
func TestGate_QueriesDontEngageGate(t *testing.T) {
	...
	a := newTestAccumulator(t)
	ctx.SetValue(schemas.UnifAIContextKeyTracer, &gateQueryTracer{a: a})
	ctx.SetValue(schemas.UnifAIContextKeyTraceID, "trace-no-engage")
...
func (t *gateQueryTracer) GetAccumulatedResponse(traceID string) *schemas.UnifAIResponse {
...
// yet — should return nil rather than an empty UnifAIResponse.
...
func TestGate_GetAccumulatedResponse_TextSnapshot(t *testing.T) {
	...
	ctx := schemas.NewUnifAIContext(context.Background(), time.Time{})
	ctx.SetValue(schemas.UnifAIContextKeyAccumulatorID, traceID)
...
// not flip UnifAIContextKeyStreamGated. Same contract as IsStreamEnded.
func TestGate_GetAccumulatedResponse_DoesNotEngageGate(t *testing.T) {
	ctx := schemas.NewUnifAIContext(context.Background(), time.Time{})
	a := newTestAccumulator(t)
	ctx.SetValue(schemas.UnifAIContextKeyTracer, &gateQueryTracer{a: a})
	ctx.SetValue(schemas.UnifAIContextKeyTraceID, "trace-snapshot-no-engage")
