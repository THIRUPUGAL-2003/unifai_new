...
func testResponsesAccumulator(tb testing.TB) *Accumulator {
	tb.Helper()
	acc := NewAccumulator(nil, unifai.NewDefaultLogger(schemas.LogLevelError))
...
func TestForceCleanupStreamAccumulatorReapsRegardlessOfRefcount(t *testing.T) {
	...
	chunk.ChunkIndex = 0
	chunk.StreamResponse = &schemas.UnifAIResponsesStreamResponse{
		Type:         schemas.ResponsesStreamResponseTypeOutputTextDelta,
...
func TestDeepCopyResponsesStreamResponsePreservesAllFields(t *testing.T) {
	original := &schemas.UnifAIResponsesStreamResponse{
		Type:           schemas.ResponsesStreamResponseTypeReasoningSummaryTextDelta,
		SequenceNumber: 4,
