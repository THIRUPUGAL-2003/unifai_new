...
# Test an Auto-OAuth server from the scan: e.g. Atlassian or Canva or Asana
for target_name in ["Canva", "Asana", "Airtable"]:
    payload = {
        "name": f"test_{target_name.lower()}",
        "connection_type": "http",
        "connection_string": {"value": f"https://mcp.{target_name.lower()}.com/mcp", "ref": ""},
        "is_code_mode_client": False,
        "is_ping_available": True,
        "auth_type": "oauth",
        "oauth_config": {
            "client_id": {"value": "", "ref": ""},
            "server_url": f"https://mcp.{target_name.lower()}.com/mcp"
        },
        "tools_to_execute": ["*"],
        "allow_on_all_virtual_keys": True
    }
    code, res = api_req("/api/mcp/client", "POST", payload)
    print(f"Install test for {target_name}: HTTP {code}")
    if isinstance(res, dict):
        status = res.get("status")
        auth_url = res.get("authorize_url")
        print(f"   -> Status: {status} | Authorize URL: {auth_url[:60] if auth_url else 'None'}")
        if status != "pending_oauth":
            err = res.get("error", {}).get("message") or res.get("message")
            print(f"   -> Detail: {err}")
