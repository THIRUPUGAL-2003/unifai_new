# =============================================================================
# AKS Service Module - Azure Kubernetes Service for UnifAI
# =============================================================================
...
resource "azurerm_managed_disk" "unifai_disk" {
...
resource "kubernetes_persistent_volume" "unifai_volume" {
  metadata {
  ...
  spec {
    ...
    persistent_volume_source {
      azure_disk {
        disk_name     = azurerm_managed_disk.unifai_disk.name
        data_disk_uri = azurerm_managed_disk.unifai_disk.id
        kind          = "Managed"
        caching_mode  = "None"
      }
    }
  ...
  depends_on = [azurerm_managed_disk.unifai_disk]
...
resource "kubernetes_persistent_volume_claim" "unifai_volume_claim" {
  metadata {
  ...
  spec {
    ...
    storage_class_name = "managed-premium"
    volume_name        = kubernetes_persistent_volume.unifai_volume.metadata[0].name
  }
}
...
resource "kubernetes_deployment" "unifai" {
      ...
      spec {
        ...
        volume {
          ...
          persistent_volume_claim {
            claim_name = kubernetes_persistent_volume_claim.unifai_volume_claim.metadata[0].name
          }
        }
  ...
  depends_on = [
    ...
    kubernetes_persistent_volume_claim.unifai_volume_claim,
  ]
}
