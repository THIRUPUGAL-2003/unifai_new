# UnifAI Terraform Module

Single entry point for deploying [UnifAI](https://github.com/unifai/unifai) on AWS, GCP, Azure, or any Kubernetes cluster. This module handles configuration merging, image resolution, and routes to the appropriate cloud-provider sub-module based on your `cloud_provider` and `service` selections.

## Usage

```hcl
module "unifai" {
  source = "github.com/unifai/unifai//terraform/modules/unifai"
...
provider "kubernetes" {
  host                   = module.unifai.cluster_endpoint
  cluster_ca_certificate = base64decode(module.unifai.cluster_ca)
...
The module supports three ways to provide UnifAI configuration, which are merged in order of precedence:
...
All 18 top-level properties from the [UnifAI config schema](../../../transports/config.schema.json) are exposed as Terraform variables:
...
| Output             | Description                                     |
|--------------------|--------------------------------------------------|
| `service_url`      | URL to access the deployed UnifAI service       |
