...
resource "aws_ecs_task_definition" "unifai" {
...
resource "aws_ecs_service" "unifai" {
  name            = "${var.name_prefix}-service"
  cluster         = local.cluster_arn
  task_definition = aws_ecs_task_definition.unifai.arn
  ...
  dynamic "load_balancer" {
    ...
    content {
      target_group_arn = aws_lb_target_group.unifai[0].arn
  ...
  depends_on = [
    aws_ecs_task_definition.unifai,
  ]
...
resource "aws_lb_target_group" "unifai" {
  count = var.create_load_balancer ? 1 : 0
...
resource "aws_lb_listener" "http" {
  ...
  load_balancer_arn = aws_lb.unifai[0].arn
  port              = 80
  protocol          = "HTTP"
...
resource "aws_appautoscaling_target" "unifai" {
  count = var.enable_autoscaling ? 1 : 0

  max_capacity       = var.max_capacity
  min_capacity       = var.min_capacity
  resource_id        = "service/${local.cluster_name}/${aws_ecs_service.unifai.name}"
...
resource "aws_appautoscaling_policy" "cpu" {
  ...
  resource_id        = aws_appautoscaling_target.unifai[0].resource_id
  scalable_dimension = aws_appautoscaling_target.unifai[0].scalable_dimension
  service_namespace  = aws_appautoscaling_target.unifai[0].service_namespace
