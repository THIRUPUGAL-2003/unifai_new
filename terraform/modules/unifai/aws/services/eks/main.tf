...
# =============================================================================
#
# Deploys UnifAI on Amazon EKS with:
...
resource "kubernetes_secret" "unifai_config" {
  ...
  depends_on = [kubernetes_namespace.unifai]
}
...
resource "aws_ebs_volume" "unifai_data" {
...
resource "kubernetes_persistent_volume" "unifai_data" {
  metadata {
  ...
  spec {
    ...
    persistent_volume_source {
      aws_elastic_block_store {
        volume_id = aws_ebs_volume.unifai_data.id
        fs_type   = "ext4"
      }
    }
  ...
  depends_on = [aws_ebs_volume.unifai_data]
...
resource "kubernetes_persistent_volume_claim" "unifai_data" {
  metadata {
  ...
  spec {
    ...
    storage_class_name = "gp3"
    volume_name        = kubernetes_persistent_volume.unifai_data.metadata[0].name
  }

  depends_on = [kubernetes_persistent_volume.unifai_data]
}
...
resource "kubernetes_deployment" "unifai" {
      ...
      spec {
        ...
        volume {
          ...
          persistent_volume_claim {
            claim_name = kubernetes_persistent_volume_claim.unifai_data.metadata[0].name
          }
        }
  ...
  depends_on = [
    ...
    kubernetes_persistent_volume_claim.unifai_data,
  ]
}
...
resource "kubernetes_service" "unifai" {
  ...
  depends_on = [kubernetes_deployment.unifai]
}
...
resource "kubernetes_ingress_v1" "unifai" {
  ...
  depends_on = [kubernetes_service.unifai]
}
