...
resource "aws_security_group" "this" {
  ...
  description = "Security group for UnifAI ${var.service} service"
  vpc_id      = local.vpc_id
...
# =============================================================================
# Secrets Manager — stores UnifAI config.json (ECS only)
# EKS uses Kubernetes secrets directly and does not need Secrets Manager.
# =============================================================================

resource "aws_secretsmanager_secret" "unifai_config" {
  count = local.is_ecs ? 1 : 0

  name                    = "${var.name_prefix}/config"
  description             = "UnifAI configuration (config.json)"
...
resource "aws_secretsmanager_secret_version" "unifai_config" {
  count = local.is_ecs ? 1 : 0

  secret_id     = aws_secretsmanager_secret.unifai_config[0].id
...
resource "aws_iam_role_policy" "unifai_secrets" {
  count = local.is_ecs ? 1 : 0
  ...
  policy = jsonencode({
    ...
    Statement = [
      {
        Sid    = "AllowGetUnifAISecret"
        Effect = "Allow"
        Action = ["secretsmanager:GetSecretValue"]
        Resource = [aws_secretsmanager_secret.unifai_config[0].arn]
      },
      {
        Sid    = "AllowCloudWatchLogs"
        Effect = "Allow"
        Action = ["logs:CreateLogStream", "logs:PutLogEvents"]
        Resource = ["${aws_cloudwatch_log_group.unifai[0].arn}:*"]
      }
    ]
  })
}
...
resource "aws_cloudwatch_log_group" "unifai" {
  count = local.is_ecs ? 1 : 0
...
module "ecs" {
  ...
  secret_arn         = aws_secretsmanager_secret.unifai_config[0].arn
  log_group_name     = aws_cloudwatch_log_group.unifai[0].name
