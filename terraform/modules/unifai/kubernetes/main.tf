# ──────────────────────────────────────────────────────────────────────────────
# Generic Kubernetes module – UnifAI on any K8s cluster
# ──────────────────────────────────────────────────────────────────────────────
...
resource "kubernetes_namespace_v1" "unifai" {
  metadata {
...
resource "kubernetes_secret_v1" "unifai_config" {
  metadata {
    name      = "${var.name_prefix}-config"
    namespace = kubernetes_namespace_v1.unifai.metadata[0].name
  }
...
resource "kubernetes_persistent_volume_claim_v1" "unifai" {
  metadata {
...
resource "kubernetes_deployment_v1" "unifai" {
  metadata {
      ...
      spec {
        ...
        volume {
          ...
          persistent_volume_claim {
            claim_name = kubernetes_persistent_volume_claim_v1.unifai.metadata[0].name
          }
        }
        ...
        volume {
          name = "config-volume"
          secret {
            secret_name = kubernetes_secret_v1.unifai_config.metadata[0].name
          }
        }
      }
    }
  ...
  depends_on = [
    kubernetes_secret_v1.unifai_config,
    kubernetes_persistent_volume_claim_v1.unifai,
  ]
}
...
resource "kubernetes_service_v1" "unifai" {
  metadata {
    name      = var.service_name
    namespace = kubernetes_namespace_v1.unifai.metadata[0].name
...
resource "kubernetes_horizontal_pod_autoscaler_v2" "unifai" {
  ...
  spec {
    scale_target_ref {
      api_version = "apps/v1"
      kind        = "Deployment"
      name        = kubernetes_deployment_v1.unifai.metadata[0].name
    }
    ...
    rule {
      ...
      http {
        path {
          ...
          backend {
            service {
              name = kubernetes_service_v1.unifai.metadata[0].name
              port {
                number = 80
              }
            }
