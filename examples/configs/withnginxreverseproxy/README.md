# UnifAI behind NGINX (Docker Compose)

This example runs 3 UnifAI containers behind an NGINX reverse proxy.

## Files

- `docker-compose.yml` - Starts NGINX and 3 UnifAI nodes
- `nginx.conf` - Reverse proxy and load balancing config
- `config.json` - Shared UnifAI config for all nodes
...
NGINX exposes UnifAI on `http://localhost:8080`.
