...
This config makes UnifAI load its pricing datasheet and model parameters
datasheet from local files instead of fetching them from `getunifai.ai`. Use
it when the host has no outbound internet access, or when an HTTP proxy blocks
DNS resolution of `getunifai.ai` (see issue #4305).

Both `pricing_url` and `model_parameters_url` use the `file://` scheme. When the
scheme is `file`, UnifAI reads the path directly off disk and never performs
...
- `config.json` - the UnifAI config, pointing both URLs at the sibling `*.json` files.
...
```bash
curl -fsSL https://getunifai.ai/datasheet -o pricing.json
curl -fsSL https://getunifai.ai/datasheet/model-parameters -o model-parameters.json
```
