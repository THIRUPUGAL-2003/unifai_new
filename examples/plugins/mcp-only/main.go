...
func PreMCPHook(ctx *schemas.UnifAIContext, req *schemas.UnifAIMCPRequest) (*schemas.UnifAIMCPRequest, *schemas.MCPPluginShortCircuit, error) {
	...
	if pluginConfig.EnableAudit {
		auditMsg := fmt.Sprintf("MCP request processed at %v", ctx.Value(schemas.UnifAIContextKey("request_id")))
		ctx.SetValue(schemas.UnifAIContextKey("mcp-audit-trail"), auditMsg)
		if pluginConfig.EnableLogging {
...
func PostMCPHook(ctx *schemas.UnifAIContext, resp *schemas.UnifAIMCPResponse, unifaiErr *schemas.UnifAIError) (*schemas.UnifAIMCPResponse, *schemas.UnifAIError, error) {
	if pluginConfig.EnableLogging {
	...
	if pluginConfig.EnableAudit {
		auditTrail := ctx.Value(schemas.UnifAIContextKey("mcp-audit-trail"))
		if pluginConfig.EnableLogging {
			fmt.Printf("[MCP-Only Plugin] Audit trail: %v\n", auditTrail)
		}
	}
	...
	if unifaiErr != nil && unifaiErr.Error != nil {
		fmt.Printf("[MCP-Only Plugin] Error occurred: %v\n", unifaiErr.Error.Message)
	}

	// Example: Transform error responses (configurable)
	if pluginConfig.TransformErrors && unifaiErr != nil && unifaiErr.StatusCode != nil && *unifaiErr.StatusCode == 404 {
		// Convert 404 to a more user-friendly error
		if unifaiErr.Error != nil {
			unifaiErr.Error.Message = "The requested MCP resource was not found. Please check your request."
			if pluginConfig.EnableLogging {
...
//
// Mutable fields (changes are honored by UnifAI):
...
func PreMCPConnectionHook(ctx *schemas.UnifAIContext, req *schemas.UnifAIMCPConnectRequest) (*schemas.UnifAIMCPConnectRequest, *schemas.MCPConnectionShortCircuit, error) {
	if pluginConfig.EnableLogging {
		...
		allHeaders := ctx.Value(schemas.UnifAIContextKeyRequestHeaders)
	...
	if pluginConfig.AuditHeader != "" &&
		...
		req.Headers[pluginConfig.AuditHeader] = fmt.Sprintf("unifai-mcp-connect:%s", req.ClientName)
	}
...
//
// On a failed handshake, resp is nil and unifaiErr is populated. Plugins
// can return a transformed error (mirroring PostMCPHook's pattern below).
func PostMCPConnectionHook(ctx *schemas.UnifAIContext, resp *schemas.UnifAIMCPConnectResponse, unifaiErr *schemas.UnifAIError) (*schemas.UnifAIMCPConnectResponse, *schemas.UnifAIError, error) {
	if pluginConfig.EnableLogging {
		fmt.Println("[MCP-Only Plugin] PostMCPConnectionHook called")
	}

	if unifaiErr != nil {
		if pluginConfig.EnableLogging && unifaiErr.Error != nil {
