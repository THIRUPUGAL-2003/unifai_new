...
// HTTPTransportPreHook is called at the HTTP layer before requests enter UnifAI core
...
func HTTPTransportPreHook(ctx *schemas.UnifAIContext, req *schemas.HTTPRequest) (*schemas.HTTPResponse, error) {
	...
	ctx.SetValue(schemas.UnifAIContextKey("http-plugin-start-time"), time.Now())

	// Return nil to continue processing
	return nil, nil
}

// HTTPTransportPostHook is called at the HTTP layer after UnifAI core processes the request
...
func HTTPTransportPostHook(ctx *schemas.UnifAIContext, req *schemas.HTTPRequest, resp *schemas.HTTPResponse) error {
	...
	startTime := ctx.Value(schemas.UnifAIContextKey("http-plugin-start-time"))
	...
	if resp.StatusCode >= 400 {
		...
		if err := json.Unmarshal(resp.Body, &errorBody); err == nil {
			...
			errorBody["request_id"] = ctx.Value(schemas.UnifAIContextKey("request_id"))
