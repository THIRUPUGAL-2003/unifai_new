...
func HTTPTransportPreHook(ctx *schemas.UnifAIContext, req *schemas.HTTPRequest) (*schemas.HTTPResponse, error) {
	...
	ctx.SetValue(schemas.UnifAIContextKey("multi-http-request-time"), time.Now())
	ctx.SetValue(schemas.UnifAIContextKey("multi-http-path"), req.Path)
...
func HTTPTransportPostHook(ctx *schemas.UnifAIContext, req *schemas.HTTPRequest, resp *schemas.HTTPResponse) error {
	...
	if startTime, ok := ctx.Value(schemas.UnifAIContextKey("multi-http-request-time")).(time.Time); ok {
...
func PreLLMHook(ctx *schemas.UnifAIContext, req *schemas.UnifAIRequest) (*schemas.UnifAIRequest, *schemas.LLMPluginShortCircuit, error) {
	...
	if pluginConfig.EnableLogging {
		...
		httpPath := ctx.Value(schemas.UnifAIContextKey("multi-http-path"))
		fmt.Printf("[Multi-Interface Plugin] Processing LLM request from path: %v\n", httpPath)
	}

	// Store LLM metadata
	ctx.SetValue(schemas.UnifAIContextKey("multi-llm-start-time"), time.Now())
...
func PostLLMHook(ctx *schemas.UnifAIContext, resp *schemas.UnifAIResponse, unifaiErr *schemas.UnifAIError) (*schemas.UnifAIResponse, *schemas.UnifAIError, error) {
	...
	if startTime, ok := ctx.Value(schemas.UnifAIContextKey("multi-llm-start-time")).(time.Time); ok {
		duration := time.Since(startTime)
		if pluginConfig.EnableLogging {
		...
		ctx.SetValue(schemas.UnifAIContextKey("multi-llm-duration"), duration)
	}
...
func PreMCPHook(ctx *schemas.UnifAIContext, req *schemas.UnifAIMCPRequest) (*schemas.UnifAIMCPRequest, *schemas.MCPPluginShortCircuit, error) {
	...
	// Store MCP metadata
	ctx.SetValue(schemas.UnifAIContextKey("multi-mcp-start-time"), time.Now())
	ctx.SetValue(schemas.UnifAIContextKey("multi-mcp-type"), req.RequestType)
...
func PostMCPHook(ctx *schemas.UnifAIContext, resp *schemas.UnifAIMCPResponse, unifaiErr *schemas.UnifAIError) (*schemas.UnifAIMCPResponse, *schemas.UnifAIError, error) {
	...
	if startTime, ok := ctx.Value(schemas.UnifAIContextKey("multi-mcp-start-time")).(time.Time); ok {
		duration := time.Since(startTime)
		if pluginConfig.EnableLogging {
		...
		// Store for observability
		ctx.SetValue(schemas.UnifAIContextKey("multi-mcp-duration"), duration)
	}
