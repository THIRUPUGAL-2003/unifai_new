# UnifAI WASM Plugin (TypeScript/AssemblyScript)

A comprehensive example of a UnifAI plugin written in TypeScript and compiled to WebAssembly using AssemblyScript. This plugin demonstrates proper structure definitions, JSON parsing, context handling, and request/response modification patterns.
...
class LLMPluginShortCircuit {
  response: UnifAIResponse | null = null  // Success short-circuit
  error: UnifAIError | null = null        // Error short-circuit
}
```
...
export function pre_hook(inputPtr: u32, inputLen: u32): u64 {
  ...
  if (model === 'mock-model') {
    ...
    // Build mock response
    const mockResponse = new UnifAIResponse()
    mockResponse.chat_response = new UnifAIChatResponse()
