...
```bash
NAMESPACE="unifai-examples"
RELEASE_NAME="unifai-statefulset-upgrade"
...
# SQLite base stack
helm upgrade --install "${RELEASE_NAME}" ./helm-charts/unifai \
  --namespace "${NAMESPACE}" \
...
- `env.UNIFAI_ADMIN_USERNAME`
- `env.UNIFAI_ADMIN_PASSWORD`

Create a secret and inject it via `envFrom`:

```bash
kubectl -n "${NAMESPACE}" create secret generic unifai-admin-auth \
  --from-literal=UNIFAI_ADMIN_USERNAME=admin \
  --from-literal=UNIFAI_ADMIN_PASSWORD='change-me' \
...
If you built a local image (for example `unifai-local:v1.5.0-prerelease21`) and want
...
helm upgrade --install "${RELEASE_NAME}" ./helm-charts/unifai \
  ...
  -f examples/k8s/examples/values-client-configs.yaml \
  -f examples/k8s/examples/values-semantic-search-redis.yaml \
  --set image.repository=unifai-local \
...
1. Run:
   `helm upgrade --install "${RELEASE_NAME}" ./helm-charts/unifai --namespace "${NAMESPACE}" -f examples/k8s/examples/values.yaml -f examples/k8s/examples/values-storage-sqlite.yaml -f examples/k8s/examples/values-providers.yaml -f examples/k8s/examples/values-with-pod-label.yaml --wait --timeout 5m`
