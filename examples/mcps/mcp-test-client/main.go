...
// running UnifAI /mcp endpoint under any inbound-auth configuration.
//
// It speaks streamable HTTP and supports the two credential styles UnifAI's
// MCP server accepts:
//
//   - header credentials: a virtual key (x-uf-vk / Authorization: Bearer /
//     x-api-key) or a session id (x-uf-mcp-session-id), used for the
...
func main() {
	extra := headerList{}
	url := flag.String("url", "http://localhost:8080/mcp", "UnifAI /mcp endpoint")
	auth := flag.String("auth", "headers", "credential style: headers | oauth")
	vk := flag.String("vk", "", "virtual key, sent as x-uf-vk (auth=headers)")
	...
	session := flag.String("session", "", "session id, sent as x-uf-mcp-session-id (auth=headers)")
	...
	if *session != "" {
		headers["x-uf-mcp-session-id"] = *session
	}
	for k, v := range extra {
		headers[k] = v
	}
...
func initRequest() mcp.InitializeRequest {
	...
	req.Params.ClientInfo = mcp.Implementation{Name: "unifai-mcp-test-client", Version: "0.1.0"}
...
func authorize(cfg *config, h *transport.OAuthHandler) error {
	...
	if h.GetClientID() == "" {
		...
		err := h.RegisterClient(regCtx, "unifai-mcp-test-client")
		cancel()
		if err != nil {
