...
A tiny interactive MCP client for exercising a running UnifAI `/mcp` endpoint
...
credential styles the UnifAI MCP server accepts:

- **header credentials** — a virtual key (`x-uf-vk`, `Authorization: Bearer`, or
  `x-api-key`) or a session id (`x-uf-mcp-session-id`). Use with server auth
...
Virtual key over `x-uf-vk` (headers / both mode):

```bash
GOWORK=off go run . -url http://localhost:8080/mcp -auth headers -vk sk-uf-xxxxx
```

Same VK as a bearer, or as `x-api-key`:

```bash
GOWORK=off go run . -auth headers -bearer sk-uf-xxxxx
GOWORK=off go run . -auth headers -api-key sk-uf-xxxxx
```
...
GOWORK=off go run . -auth headers -vk sk-uf-xxx -once list
GOWORK=off go run . -auth headers -vk sk-uf-xxx -once 'call echo {"text":"hi"}'
```
