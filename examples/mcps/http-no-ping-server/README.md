...
This is a sample MCP server implementation that runs over HTTP but **does not support the optional `ping` method**. This demonstrates how to configure UnifAI to use the `listTools` health check method instead of ping.
...
2. **How to configure UnifAI** to work with such servers using `is_ping_available: false`
3. **Why this matters**: When `is_ping_available` is `false`, UnifAI will use `listTools` for health checks instead of the lightweight `ping` method
...
Note: This server does NOT support ping. Use is_ping_available=false in UnifAI config.
```

## Connecting via UnifAI
...
When you add this server to UnifAI with `is_ping_available: false`:

1. UnifAI will **NOT** send `ping` requests (since the server doesn't support them)
2. Instead, UnifAI will use `listTools` every 10 seconds to check server health
...
- ✅ Demonstrate the `is_ping_available=false` behavior in UnifAI
...
- [UnifAI MCP Documentation](../../docs/mcp/connecting-to-servers.mdx)
