#!/bin/bash
# UnifAI Helm Chart Installation Script
# This script helps you install UnifAI with different configurations
...
print_banner() {
    ...
    echo -e "${BLUE}╔═══════════════════════════════════════════╗${NC}"
    echo -e "${BLUE}║                                           ║${NC}"
    echo -e "${BLUE}║      UnifAI Helm Chart Installer        ║${NC}"
    echo -e "${BLUE}║                                           ║${NC}"
    echo -e "${BLUE}╚═══════════════════════════════════════════╝${NC}"
    echo ""
}
...
# Get release name
get_release_name() {
    read -p "Enter release name (default: unifai): " RELEASE_NAME
    RELEASE_NAME=${RELEASE_NAME:-unifai}
}
...
install_chart() {
    ...
    if helm install "$RELEASE_NAME" "$CHART_DIR" \
        ...
        print_success "UnifAI installed successfully!"
        echo ""
        print_info "To check the status:"
        echo "  helm status $RELEASE_NAME -n $NAMESPACE"
        echo ""
        print_info "To get the application URL:"
        echo "  kubectl --namespace $NAMESPACE port-forward svc/$RELEASE_NAME 8081:8081"
        echo "  Then visit: http://localhost:8081"
        echo ""
        print_info "To view logs:"
        echo "  kubectl logs -l app.kubernetes.io/name=unifai -n $NAMESPACE -f"
        echo ""
    else
