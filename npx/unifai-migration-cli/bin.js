...
async function getPlatformArchAndBinary() {
    ...
    if (platform === "darwin") {
        ...
        binaryName = "unifai-migration-cli";
        ...
        binaryName = "unifai-migration-cli.exe";
    } else {
...
async function checkVersionExists(version, platformDir, archDir, binaryName) {
    const url = `${BASE_URL}/unifai-migration-cli/${version}/${platformDir}/${archDir}/${binaryName}`;
...
async function main() {
    ...
    if (VERSION === "latest") {
        ...
        if (latestExists) {
            ...
            console.error(`❌ Could not find latest unifai-migration-cli version.`);
        ...
        if (!versionExists) {
            console.error(`❌ unifai-migration-cli version '${VERSION}' not found.`);
            console.error(
                `See https://docs.unifai.ai/changelogs for release versions you can pass to --cli-version.`,
            );
            process.exit(1);
        }
    ...
    const downloadUrl = `${BASE_URL}/unifai-migration-cli/${namedVersion}/${platformDir}/${archDir}/${binaryName}`;

    // Pinned versions are immutable, so they're safe to cache under
    // ~/.unifai/versions/<tag>/. "latest" is a moving target — never cache
    // it, always install fresh to ~/.unifai/bin/ so it can't get stuck stale.
    const isPinned = namedVersion !== "latest";
    const installDir = isPinned
        ? join(homedir(), ".unifai", "versions", namedVersion)
        : join(homedir(), ".unifai", "bin");
    ...
    try {
        ...
        console.error(
            `❌ Failed to run unifai-migration-cli. Error:`,
            execError.message,
        );
        if (execError.code) {
...
main().catch((error) => {
    if (error.name === "AbortError" || error.name === "TimeoutError") {
        ...
        console.error(`❌ Failed to run unifai-migration-cli installer: ${error.message}`);
    }
    process.exit(1);
});