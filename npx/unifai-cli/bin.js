...
async function checkVersionExists(version, platformDir, archDir, binaryName) {
	const url = `${BASE_URL}/unifai-cli/${version}/${platformDir}/${archDir}/${binaryName}`;
...
function getShellConfig() {
	...
	if (shell.endsWith("/fish") || shell.endsWith("/fish.exe")) {
		return {
			...
			exportLine: "fish_add_path $HOME/.unifai/bin",
			shellName: "fish",
		};
	}
	...
	if (shell.endsWith("/zsh")) {
		return {
			...
			exportLine: 'export PATH="$HOME/.unifai/bin:$PATH"',
			shellName: "zsh",
		};
	}
...
function hasPathLine(filePath) {
	...
	try {
		...
		return content.includes(".unifai/bin");
	} catch {
		return false;
	}
}
...
function printStartMessage(message) {
	console.log(`\n${message}`);
	console.log(`Enter 'unifai' when you're ready to start the CLI.`);
}
...
async function main() {
	...
	if (VERSION === "latest") {
		...
		if (!versionExists) {
			...
			console.error(`Please verify the version exists at: ${BASE_URL}/unifai-cli/`);
			process.exit(1);
		}
		namedVersion = VERSION;
	}

	const downloadUrl = `${BASE_URL}/unifai-cli/${namedVersion}/${platformDir}/${archDir}/${binaryName}`;

	// Install to ~/.unifai/bin/
	const installDir = join(homedir(), ".unifai", "bin");
	...
	try {
		...
		const checksumUrl = `${BASE_URL}/unifai-cli/${namedVersion}/${platformDir}/${archDir}/${binaryName}.sha256`;
	...
	console.log(`✅ Installed unifai to ${binaryPath}`);
	...
	if (!shellConfig) {
		...
		printStartMessage("The installer won't start UnifAI automatically.");
		return;
	}
	...
	if (missingRcFiles.length === 0) {
		console.log(`\n✅ PATH already configured for unifai.`);
		printStartMessage("The installer won't start UnifAI automatically.");
		return;
	}
...
main().catch((error) => {
	console.error(`❌ Failed to install UnifAI CLI: ${error.message}`);
	process.exit(1);
});
