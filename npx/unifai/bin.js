...
// Keep in sync with transports/unifai-http/main.go (flag.StringVar names).
const UNIFAI_HTTP_KNOWN_FLAGS = new Set(["app-dir", "host", "log-level", "log-style", "port"]);
...
function unifaiHttpFlagToken(arg) {
...
function validateRemainingArgsForUnifAIHttp(args) {
	...
	for (const arg of args) {
		...
		const token = unifaiHttpFlagToken(arg);
		if (!token) {
			continue;
		}
		if (UNIFAI_HTTP_KNOWN_FLAGS.has(token)) {
			continue;
		}
		...
		// Hyphenated names are treated as likely real unifai-http flags even before this wrapper is updated.
		if (token.includes("-")) {
			continue;
		}
		...
		const knownList = [...UNIFAI_HTTP_KNOWN_FLAGS].sort().map((f) => `-${f}`).join(" ");
		console.error(`Known unifai-http flags: ${knownList}`);
		console.error(`Use --help or -h for gateway usage.`);
		console.error(
			`Pin a release from this wrapper with: --transport-version <tag> (see https://docs.unifai.ai/changelogs).`,
		);
		process.exit(1);
	}
}
...
function parseTransportVersion() {
	...
	// Some runners invoke the bin as `node bin.js unifai ...`; strip one prefix so flags parse reliably.
	if (args.length > 0 && args[0] === "unifai") {
		args.shift();
	}

	let transportVersion = "latest"; // Default to latest
	const envOverride = process.env.UNIFAI_TRANSPORT_VERSION?.trim();

	// Only wrapper-owned flags before "--"; everything after "--" is forwarded to unifai-http verbatim.
...
validateRemainingArgsForUnifAIHttp(remainingArgs);
...
async function getLatestVersion() {
	const releaseUrl = "https://getunifai.ai/latest-release";
...
async function checkVersionExists(version, platformDir, archDir, binaryName) {
	const url = `${BASE_URL}/unifai/${version}/${platformDir}/${archDir}/${binaryName}`;
...
(async () => {
	...
	if (VERSION === "latest") {
		...
		if (!versionExists) {
			...
			console.error(`See https://docs.unifai.ai/changelogs for release versions you can pass to --transport-version.`);
			process.exit(1);
		}
	...
	downloadUrls.push(`${BASE_URL}/unifai/${urlVersion}/${platformDir}/${archDir}/${binaryName}`);

	let lastError = null;
	let binaryWorking = false;

	const unifaiBinDir = namedVersionFound
		? join(cacheDir(), "unifai", namedVersion, "bin")
		: mkdtempSync(join(tmpdir(), "unifai-npx-"));

	if (!namedVersionFound) {
		process.once("exit", () => {
			try {
				if (existsSync(unifaiBinDir)) {
					rmSync(unifaiBinDir, { recursive: true, force: true });
				}
			} catch {
	...
	try {
		if (namedVersionFound && !existsSync(unifaiBinDir)) {
			mkdirSync(unifaiBinDir, { recursive: true });
		}
	} catch (mkdirError) {
		console.error(`❌ Failed to create directory ${unifaiBinDir}:`, mkdirError.message);
		process.exit(1);
	}

	for (let i = 0; i < downloadUrls.length; i++) {
		const binaryPath = join(unifaiBinDir, `${binaryName}-${i}`);
	...
	if (!binaryWorking) {
		console.error(`❌ Failed to start UnifAI. Error:`, lastError.message);
