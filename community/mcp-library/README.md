...
This directory contains the community-curated catalog of [MCP (Model Context Protocol)](https://modelcontextprotocol.io) servers available in the UnifAI MCP Library.
...
```bash
git clone https://github.com/<your-username>/unifai.git
cd unifai
```
...
UnifAI generates the internal server slug from `name`. Do not add a `slug` field to catalog entries.
...
| ----- | ---- | -------- | ----------- |
| `name` | `string` | yes | Human-readable display name. UnifAI derives the internal slug from this value. |
...
- Use a clear, stable `name`; changing it later changes UnifAI's generated internal slug.
...
3. The UnifAI platform periodically fetches `servers.json` from the `dev` branch.
4. UnifAI derives an internal slug from `name`, upserts entries by that slug, and serves the catalog to users.
...
- Open an [issue](https://github.com/unifai/unifai/issues/new) for questions or problems.
- See the [UnifAI docs](https://docs.unifai.ai) for general platform documentation.
